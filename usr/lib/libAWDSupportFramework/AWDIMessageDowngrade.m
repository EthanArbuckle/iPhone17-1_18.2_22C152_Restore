...
- (id)description
{
  v3.receiver = self;
  v3.super_class = (Class)AWDIMessageDowngrade;
  return (id)[NSString stringWithFormat:@"%@ %@", -[AWDIMessageDowngrade description](&v3, sel_description), -[AWDIMessageDowngrade dictionaryRepresentation](self, "dictionaryRepresentation")];
}
...
LABEL_9:
    [v4 setObject:[NSNumber numberWithUnsignedInt:self->_durationBeforeDowngrade] forKey:@"durationBeforeDowngrade"];
...
LABEL_6:
    ...
    [v4 setObject:[NSNumber numberWithUnsignedInt:self->_isManualDowngrade] forKey:@"isManualDowngrade"];
  return v4;
}
...
LABEL_18:
      ...
      if ((*(unsigned char *)&self->_has & 2) != 0)
      {
        if ((*((unsigned char *)a3 + 36) & 2) == 0 || self->_durationBeforeDowngrade != *((_DWORD *)a3 + 4)) {
          goto LABEL_18;
        }
      }
      ...
      if ((*(unsigned char *)&self->_has & 4) != 0)
      {
        if ((*((unsigned char *)a3 + 36) & 4) == 0 || self->_isManualDowngrade != *((_DWORD *)a3 + 8)) {
          goto LABEL_18;
        }
        LOBYTE(v5) = 1;
      }
...
LABEL_3:
  uint64_t v5 = 2654435761 * self->_durationBeforeDowngrade;
  if ((*(unsigned char *)&self->_has & 4) == 0) {
    goto LABEL_7;
  }
LABEL_4:
  uint64_t v6 = 2654435761 * self->_isManualDowngrade;
  return v4 ^ v3 ^ v5 ^ v6;
}
