@interface AWDEventKitSyncSyncedAttendeeResponseFromWatch
...
@implementation AWDEventKitSyncSyncedAttendeeResponseFromWatch
...
{
  v3.receiver = self;
  v3.super_class = (Class)AWDEventKitSyncSyncedAttendeeResponseFromWatch;
  return (id)[NSString stringWithFormat:@"%@ %@", -[AWDEventKitSyncSyncedAttendeeResponseFromWatch description](&v3, sel_description), -[AWDEventKitSyncSyncedAttendeeResponseFromWatch dictionaryRepresentation](self, "dictionaryRepresentation")];
}
...
- (BOOL)readFrom:(id)a3
{
  return AWDEventKitSyncSyncedAttendeeResponseFromWatchReadFrom((uint64_t)self, (uint64_t)a3);
}
...
LABEL_5:
    *((void *)a3 + 1) = self->_deltaTransitTime;
...
- (id)copyWithZone:(_NSZone *)a3
{
  ...
  if (has)
  {
    *((void *)result + 1) = self->_deltaTransitTime;
...
LABEL_11:
    ...
    if (*(unsigned char *)&self->_has)
    {
      if ((*((unsigned char *)a3 + 24) & 1) == 0 || self->_deltaTransitTime != *((void *)a3 + 1)) {
        goto LABEL_11;
      }
      LOBYTE(v5) = 1;
    }
...
LABEL_5:
    self->_deltaTransitTime = *((void *)a3 + 1);
    *(unsigned char *)&self->_has |= 1u;
    return;
  }
