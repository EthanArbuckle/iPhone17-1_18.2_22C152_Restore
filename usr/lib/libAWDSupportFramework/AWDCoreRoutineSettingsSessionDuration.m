...
- (id)description
{
  v3.receiver = self;
  v3.super_class = (Class)AWDCoreRoutineSettingsSessionDuration;
  return (id)[NSString stringWithFormat:@"%@ %@", -[AWDCoreRoutineSettingsSessionDuration description](&v3, sel_description), -[AWDCoreRoutineSettingsSessionDuration dictionaryRepresentation](self, "dictionaryRepresentation")];
}
...
- (id)dictionaryRepresentation
{
  ...
  if ((has & 4) != 0) {
    [v3 setObject:[NSNumber numberWithInt:self->_endedByScreenLock] forKey:@"endedByScreenLock"];
  }
  return v3;
}
...
LABEL_19:
    ...
    if ((has & 2) != 0)
    {
      if ((*((unsigned char *)a3 + 32) & 2) == 0 || self->_duration != *((_DWORD *)a3 + 4)) {
        goto LABEL_19;
      }
    }
    ...
    if ((has & 4) != 0)
    {
      if ((*((unsigned char *)a3 + 32) & 4) == 0 || self->_endedByScreenLock != *((_DWORD *)a3 + 5)) {
        goto LABEL_19;
      }
      LOBYTE(v5) = 1;
    }
...
LABEL_6:
  uint64_t v6 = 2654435761 * self->_endedByScreenLock;
  return v4 ^ v3 ^ v5 ^ v6;
}
