...
- (id)description
{
  v3.receiver = self;
  v3.super_class = (Class)AWDIMessageDeduplicated;
  return (id)[NSString stringWithFormat:@"%@ %@", -[AWDIMessageDeduplicated description](&v3, sel_description), -[AWDIMessageDeduplicated dictionaryRepresentation](self, "dictionaryRepresentation")];
}
...
- (id)dictionaryRepresentation
{
  ...
  if ((has & 2) != 0) {
    [v4 setObject:[NSNumber numberWithUnsignedInt:self->_deduplicationInterval] forKey:@"deduplicationInterval"];
  }
  return v4;
}
...
LABEL_13:
      ...
      {
        if ((*((unsigned char *)a3 + 32) & 2) == 0 || self->_deduplicationInterval != *((_DWORD *)a3 + 4)) {
          goto LABEL_13;
        }
        LOBYTE(v5) = 1;
      }
...
LABEL_3:
  uint64_t v5 = 2654435761 * self->_deduplicationInterval;
  return v4 ^ v3 ^ v5;
}
