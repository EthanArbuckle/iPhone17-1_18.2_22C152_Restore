...
- (id)description
{
  ...
  return (id)[NSString stringWithFormat:@"%@ %@", -[AWDWRMLinkPrefInit description](&v3, sel_description), -[AWDWRMLinkPrefInit dictionaryRepresentation](self, "dictionaryRepresentation")];
}
...
LABEL_7:
    [v3 setObject:[NSNumber numberWithUnsignedInt:self->_countWifi] forKey:@"countWifi"];
...
LABEL_4:
    ...
    [v3 setObject:[NSNumber numberWithUnsignedInt:self->_countCellular] forKey:@"countCellular"];
  return v3;
}
...
LABEL_16:
    ...
    if ((*(unsigned char *)&self->_has & 4) != 0)
    {
      if ((*((unsigned char *)a3 + 24) & 4) == 0 || self->_countWifi != *((_DWORD *)a3 + 5)) {
        goto LABEL_16;
      }
    }
    ...
    if ((*(unsigned char *)&self->_has & 2) != 0)
    {
      if ((*((unsigned char *)a3 + 24) & 2) == 0 || self->_countCellular != *((_DWORD *)a3 + 4)) {
        goto LABEL_16;
      }
      LOBYTE(v5) = 1;
    }
...
LABEL_3:
  uint64_t v3 = 2654435761 * self->_countWifi;
  if ((*(unsigned char *)&self->_has & 2) == 0) {
    goto LABEL_7;
  }
LABEL_4:
  uint64_t v4 = 2654435761 * self->_countCellular;
  return v3 ^ v2 ^ v4;
}
