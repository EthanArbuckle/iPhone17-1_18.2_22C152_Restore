...
- (void)setIsDSUser:(unsigned int)a3;
- (void)setIsPhoneUser:(unsigned int)a3;
...
- (void)dealloc
{
  ...
  v3.receiver = self;
  v3.super_class = (Class)AWDIDSRegistrationAuthenticate;
...
- (id)description
{
  v3.receiver = self;
  v3.super_class = (Class)AWDIDSRegistrationAuthenticate;
  return (id)[NSString stringWithFormat:@"%@ %@", -[AWDIDSRegistrationAuthenticate description](&v3, sel_description), -[AWDIDSRegistrationAuthenticate dictionaryRepresentation](self, "dictionaryRepresentation")];
}
...
LABEL_23:
    [v3 setObject:[NSNumber numberWithUnsignedInt:self->_isPhoneUser] forKey:@"isPhoneUser"];
...
LABEL_14:
    ...
    [v3 setObject:[NSNumber numberWithUnsignedInt:self->_isDSUser] forKey:@"isDSUser"];
  return v3;
}

- (BOOL)readFrom:(id)a3
{
  return AWDIDSRegistrationAuthenticateReadFrom((uint64_t)self, (uint64_t)a3);
}
...
LABEL_23:
  *((_DWORD *)a3 + 9) = self->_isPhoneUser;
  *((_WORD *)a3 + 30) |= 0x10u;
  if ((*(_WORD *)&self->_has & 8) == 0) {
    return;
  }
LABEL_14:
  *((_DWORD *)a3 + 8) = self->_isDSUser;
  *((_WORD *)a3 + 30) |= 8u;
}
...
LABEL_21:
    *(_DWORD *)(v6 + 36) = self->_isPhoneUser;
    *(_WORD *)(v6 + 60) |= 0x10u;
    if ((*(_WORD *)&self->_has & 8) == 0) {
      return (id)v6;
    }
...
LABEL_54:
    ...
    if ((has & 0x10) != 0)
    {
      if ((v9 & 0x10) == 0 || self->_isPhoneUser != *((_DWORD *)a3 + 9)) {
        goto LABEL_54;
      }
    }
    else if ((v9 & 0x10) != 0)
    ...
    if ((has & 8) != 0)
    {
      if ((v9 & 8) == 0 || self->_isDSUser != *((_DWORD *)a3 + 8)) {
        goto LABEL_54;
      }
      LOBYTE(v5) = 1;
    }
...
LABEL_12:
  uint64_t v13 = 2654435761 * self->_isPhoneUser;
  if ((has & 8) == 0) {
    goto LABEL_22;
  }
LABEL_13:
  uint64_t v14 = 2654435761 * self->_isDSUser;
  return v4 ^ v3 ^ v6 ^ v7 ^ v8 ^ v9 ^ v10 ^ v11 ^ v12 ^ v13 ^ v14;
}
...
- (void)mergeFrom:(id)a3
{
  ...
  if (*((void *)a3 + 3)) {
    -[AWDIDSRegistrationAuthenticate setGuid:](self, "setGuid:");
  }
...
LABEL_23:
  self->_isPhoneUser = *((_DWORD *)a3 + 9);
  *(_WORD *)&self->_has |= 0x10u;
  if ((*((_WORD *)a3 + 30) & 8) == 0) {
    return;
  }
LABEL_14:
  self->_isDSUser = *((_DWORD *)a3 + 8);
  *(_WORD *)&self->_has |= 8u;
}
