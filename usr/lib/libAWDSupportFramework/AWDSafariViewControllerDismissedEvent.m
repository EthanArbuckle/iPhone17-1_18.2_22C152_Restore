...
- (id)methodAsString:(int)a3
{
  if (a3 >= 4) {
    ...
    return off_2641BA060[a3];
  }
}

- (int)StringAsMethod:(id)a3
{
  if ([a3 isEqualToString:@"SFVC_OTHER_DISMISSAL"]) {
    return 0;
  }
  if ([a3 isEqualToString:@"SFVC_EDGE_SWIPE"]) {
    return 1;
  }
  if ([a3 isEqualToString:@"SFVC_DONE_BUTTON_IN_TOOLBAR"]) {
    return 2;
  }
  if ([a3 isEqualToString:@"SFVC_KEYBOARD_DISMISSAL"]) {
    return 3;
  }
  return 0;
}
...
{
  v3.receiver = self;
  v3.super_class = (Class)AWDSafariViewControllerDismissedEvent;
  return (id)[NSString stringWithFormat:@"%@ %@", -[AWDSafariViewControllerDismissedEvent description](&v3, sel_description), -[AWDSafariViewControllerDismissedEvent dictionaryRepresentation](self, "dictionaryRepresentation")];
}
...
- (BOOL)readFrom:(id)a3
{
  return AWDSafariViewControllerDismissedEventReadFrom((uint64_t)self, (uint64_t)a3);
}
