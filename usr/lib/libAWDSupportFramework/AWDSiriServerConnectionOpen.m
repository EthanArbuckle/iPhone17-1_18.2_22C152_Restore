...
{
  v3.receiver = self;
  v3.super_class = (Class)AWDSiriServerConnectionOpen;
  return (id)[NSString stringWithFormat:@"%@ %@", -[AWDSiriServerConnectionOpen description](&v3, sel_description), -[AWDSiriServerConnectionOpen dictionaryRepresentation](self, "dictionaryRepresentation")];
}
...
- (BOOL)readFrom:(id)a3
{
  return AWDSiriServerConnectionOpenReadFrom((uint64_t)self, (uint64_t)a3);
}
...
LABEL_16:
    ...
    if ((*(unsigned char *)&self->_has & 4) != 0)
    {
      if ((*((unsigned char *)a3 + 28) & 4) == 0 || self->_connectionType != *((_DWORD *)a3 + 6)) {
        goto LABEL_16;
      }
      LOBYTE(v5) = 1;
    }
