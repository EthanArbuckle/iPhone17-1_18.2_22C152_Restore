...
- (void)dealloc
{
  ...
  v3.receiver = self;
  v3.super_class = (Class)AWDIDSSessionInvitationSent;
...
- (id)description
{
  v3.receiver = self;
  v3.super_class = (Class)AWDIDSSessionInvitationSent;
  return (id)[NSString stringWithFormat:@"%@ %@", -[AWDIDSSessionInvitationSent description](&v3, sel_description), -[AWDIDSSessionInvitationSent dictionaryRepresentation](self, "dictionaryRepresentation")];
}
...
- (BOOL)readFrom:(id)a3
{
  return AWDIDSSessionInvitationSentReadFrom((uint64_t)self, (uint64_t)a3);
}
...
LABEL_14:
    ...
    if ((has & 2) != 0)
    {
      if ((*((unsigned char *)a3 + 28) & 2) == 0 || self->_numberOfRecipients != *((_DWORD *)a3 + 6)) {
        goto LABEL_14;
      }
      LOBYTE(v5) = 1;
    }
...
- (void)mergeFrom:(id)a3
{
  ...
  if (*((void *)a3 + 2)) {
    -[AWDIDSSessionInvitationSent setGuid:](self, "setGuid:");
  }
