...
- (id)description
{
  ...
  return (id)[NSString stringWithFormat:@"%@ %@", -[AWDSafariDidTerminateWebProcessBeforeNavigation description](&v3, sel_description), -[AWDSafariDidTerminateWebProcessBeforeNavigation dictionaryRepresentation](self, "dictionaryRepresentation")];
}
...
- (id)dictionaryRepresentation
{
  ...
  if ((has & 2) != 0)
  {
    int terminationReason = self->_terminationReason;
    if (terminationReason)
    {
      if (terminationReason == 1) {
        ...
        v6 = (__CFString *)[NSString stringWithFormat:@"(unknown: %i)", self->_terminationReason];
      }
    }
    else
    {
...
LABEL_11:
    ...
    if ((*(unsigned char *)&self->_has & 2) != 0)
    {
      if ((*((unsigned char *)a3 + 20) & 2) == 0 || self->_terminationReason != *((_DWORD *)a3 + 4)) {
        goto LABEL_11;
      }
      LOBYTE(v5) = 1;
    }
...
LABEL_3:
  uint64_t v3 = 2654435761 * self->_terminationReason;
  return v3 ^ v2;
}
...
LABEL_5:
    self->_int terminationReason = *((_DWORD *)a3 + 4);
    *(unsigned char *)&self->_has |= 2u;
    return;
  }
