...
{
  v3.receiver = self;
  v3.super_class = (Class)AWDIDSWiProxDidSendData;
  return (id)[NSString stringWithFormat:@"%@ %@", -[AWDIDSWiProxDidSendData description](&v3, sel_description), -[AWDIDSWiProxDidSendData dictionaryRepresentation](self, "dictionaryRepresentation")];
}
...
- (BOOL)readFrom:(id)a3
{
  return AWDIDSWiProxDidSendDataReadFrom((uint64_t)self, (uint64_t)a3);
}
...
- (id)copyWithZone:(_NSZone *)a3
{
  ...
  if (has)
  {
    *((void *)result + 1) = self->_resultCode;
...
LABEL_11:
    ...
    if (*(unsigned char *)&self->_has)
    {
      if ((*((unsigned char *)a3 + 24) & 1) == 0 || self->_resultCode != *((void *)a3 + 1)) {
        goto LABEL_11;
      }
      LOBYTE(v5) = 1;
    }
...
LABEL_3:
  unint64_t v3 = 2654435761u * self->_resultCode;
  return v3 ^ v2;
}
