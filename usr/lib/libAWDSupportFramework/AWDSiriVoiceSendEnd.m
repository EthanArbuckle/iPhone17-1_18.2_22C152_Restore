...
@implementation AWDSiriVoiceSendEnd
...
{
  v3.receiver = self;
  v3.super_class = (Class)AWDSiriVoiceSendEnd;
  return (id)[NSString stringWithFormat:@"%@ %@", -[AWDSiriVoiceSendEnd description](&v3, sel_description), -[AWDSiriVoiceSendEnd dictionaryRepresentation](self, "dictionaryRepresentation")];
}
...
- (BOOL)readFrom:(id)a3
{
  return AWDSiriVoiceSendEndReadFrom((uint64_t)self, (uint64_t)a3);
}
...
LABEL_11:
    ...
    if (*(unsigned char *)&self->_has)
    {
      if ((*((unsigned char *)a3 + 24) & 1) == 0 || self->_duration != *((void *)a3 + 1)) {
        goto LABEL_11;
      }
      LOBYTE(v5) = 1;
    }
