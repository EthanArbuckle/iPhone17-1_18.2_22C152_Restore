...
@implementation AWDWiFiMetricsManagerEvent
...
{
  v3.receiver = self;
  v3.super_class = (Class)AWDWiFiMetricsManagerEvent;
  return (id)[NSString stringWithFormat:@"%@ %@", -[AWDWiFiMetricsManagerEvent description](&v3, sel_description), -[AWDWiFiMetricsManagerEvent dictionaryRepresentation](self, "dictionaryRepresentation")];
}
...
- (BOOL)readFrom:(id)a3
{
  return AWDWiFiMetricsManagerEventReadFrom((uint64_t)self, (uint64_t)a3);
}
...
LABEL_16:
    ...
    if ((*(unsigned char *)&self->_has & 4) != 0)
    {
      if ((*((unsigned char *)a3 + 24) & 4) == 0 || self->_status != *((_DWORD *)a3 + 5)) {
        goto LABEL_16;
      }
    }
    ...
    if ((*(unsigned char *)&self->_has & 2) != 0)
    {
      if ((*((unsigned char *)a3 + 24) & 2) == 0 || self->_reason != *((_DWORD *)a3 + 4)) {
        goto LABEL_16;
      }
      LOBYTE(v5) = 1;
    }
