...
- (void)resolveWithFuture:(id)a3
{
  ...
  if (*((unsigned char *)a3 + 24))
  {
    ...
    [*((id *)a3 + 2) unlock];
    [(GTFuture_tools_diagnostics *)self _setResult:v5 error:v6 notify_NOLOCK:1];
  }
  else
  {
    id v7 = a3;
...
- (void)cancel
{
  [(NSCondition *)self->_condition lock];
  self->_cancelled = 1;
  v3 = (NSError *)+[GTError_tools_diagnostics errorWithDomain:@"DYErrorDomain" code:1027 userInfo:0];

  self->_error = v3;
  [(NSCondition *)self->_condition unlock];
  error = self->_error;

  [(GTFuture_tools_diagnostics *)self _setResult:0 error:error notify_NOLOCK:1];
}

- (id)error
{
  [(GTFuture_tools_diagnostics *)self waitUntilResolved];
  v3 = self->_error;
...
- (void)_setResult:(id)a3 error:(id)a4 notify_NOLOCK:(BOOL)a5
{
  [(GTFuture_tools_diagnostics *)self willChangeValueForKey:@"isFinished"];
  [(GTFuture_tools_diagnostics *)self willChangeValueForKey:@"isExecuting"];
  [(NSCondition *)self->_condition lock];
  if (self->_resolved) {
    goto LABEL_16;
  }
...
LABEL_16:
  ...
  [(GTFuture_tools_diagnostics *)self didChangeValueForKey:@"isFinished"];
  [(GTFuture_tools_diagnostics *)self didChangeValueForKey:@"isExecuting"];
}

- (int)intResult
{
  id v2 = [(GTFuture_tools_diagnostics *)self result];
...
- (void)requestResult:(id)a3
{
  [(GTFuture_tools_diagnostics *)self addResultHandler:a3];

  [(GTFuture_tools_diagnostics *)self _start_NOLOCK];
}
...
- (void)waitUntilResolved
{
  ...
  if (+[GTFuture_tools_diagnostics logPerformance])
  {
    id v3 = +[NSDate date];
    BOOL resolved = self->_resolved;
  }
  ...
  [(GTFuture_tools_diagnostics *)self _start_NOLOCK];
  [(GTFuture_tools_diagnostics *)self waitUntilFinished];
  if (+[GTFuture_tools_diagnostics logPerformance])
  {
    ...
    {
      uint64_t v12 = 0;
      [@"Timestamp, Thread, Symbol, Interval, Resolved, Main Thread\n" stringByAppendingString:v9 writeToFile:v7 atomically:1 encoding:4 error:&v12];
    }
...
{
  [(NSCondition *)self->_condition lock];
  [(GTFuture_tools_diagnostics *)self _addDependency_REQUIRESLOCK:a3];
  condition = self->_condition;
...
- (void)dealloc
{
  id v3 = +[GTError_tools_diagnostics errorWithDomain:@"DYErrorDomain" code:1028 userInfo:0];
  [(NSCondition *)self->_condition lock];
  if (!self->_resolved)
  {
    [(NSCondition *)self->_condition unlock];
    [(GTFuture_tools_diagnostics *)self _setResult:0 error:v3 notify_NOLOCK:1];
    [(NSCondition *)self->_condition lock];
  }
  ...
  v5.receiver = self;
  v5.super_class = (Class)GTFuture_tools_diagnostics;
  [(GTFuture_tools_diagnostics *)&v5 dealloc];
}
...
- (GTFuture_tools_diagnostics)init
{
  v5.receiver = self;
  v5.super_class = (Class)GTFuture_tools_diagnostics;
  id v2 = [(GTFuture_tools_diagnostics *)&v5 init];
  if (v2)
  {
...
+ (BOOL)logPerformance
{
  if (+[GTFuture logPerformance]::onceToken != -1) {
    dispatch_once(&+[GTFuture logPerformance]::onceToken, &__block_literal_global_51);
  }
...
+ (BOOL)automaticallyNotifiesObserversForKey:(id)a3
{
  ...
  return [super automaticallyNotifiesObserversForKey:a3];
}
