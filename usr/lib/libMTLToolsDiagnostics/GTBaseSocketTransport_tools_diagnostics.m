...
- (unsigned)_nextMessageSerial
{
  ...
  if (smTransport)
  {
    unsigned int result = [(GTSharedMemoryTransport_tools_diagnostics *)smTransport _nextMessageSerial];
  ...
  {
    v8.receiver = self;
    v8.super_class = (Class)GTBaseSocketTransport_tools_diagnostics;
    return [(GTTransport_tools_diagnostics *)&v8 _nextMessageSerial];
  }
  return result;
}
...
- (BOOL)relayMessageOverSharedMemoryTransport:(id)a3 error:(id *)a4
{
  return [(GTTransport_tools_diagnostics *)self->_smTransport relayMessage:a3 error:a4];
}
...
- (id)createNewSharedMemoryTransport
{
  return [(GTBaseSocketTransport_tools_diagnostics *)self createNewSharedMemoryTransportWithURL:0];
}
...
- (void)_invalidate
{
  ...
  [(GTBaseSocketTransport_tools_diagnostics *)self _destroySharedMemoryTransport];
  v4.receiver = self;
  v4.super_class = (Class)GTBaseSocketTransport_tools_diagnostics;
  [(GTBaseStreamTransport_tools_diagnostics *)&v4 _invalidate];
}
...
- (void)setPrioritizeOutgoingMessages:(BOOL)a3
{
  ...
  dispatch_sync(queue, block);
  v6.receiver = self;
  v6.super_class = (Class)GTBaseSocketTransport_tools_diagnostics;
  [(GTTransport_tools_diagnostics *)&v6 setPrioritizeOutgoingMessages:v3];
}
...
LABEL_15:
  ...
  if (smTransport)
  {
    id v12 = [(GTSharedMemoryTransport_tools_diagnostics *)smTransport connect];
...
{
  self->_scheduledReadOnWritableSocket = 0;
  while ([(GTBaseStreamTransport_tools_diagnostics *)self _readAndAccumulate] > 0)
    ;
}
...
{
  v3.receiver = self;
  v3.super_class = (Class)GTBaseSocketTransport_tools_diagnostics;
  return [(GTBaseStreamTransport_tools_diagnostics *)&v3 init];
}
