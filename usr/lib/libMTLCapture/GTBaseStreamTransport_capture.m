...
- (int64_t)_relayBufferInner:(const void *)a3 size:(unint64_t)a4
{
  ...
  int64_t v14 = -[GTBaseStreamTransport_capture _syncTmuToHeader:ioBlock:](self, "_syncTmuToHeader:ioBlock:");
  if (v14 == -1) {
    return -1;
  }
...
LABEL_5:
  int64_t v8 = [(GTBaseStreamTransport_capture *)self _writeBuffers:0];
  int64_t v9 = v8;
  if (v8 == -1)
  {
LABEL_8:
...
LABEL_9:
  ...
  if (!v11)
  {
    *(_OWORD *)&v10->var3 = xmmword_1DA250;
    v10->var5 = 0;
    goto LABEL_8;
  }
  return v9;
...
LABEL_10:
  int64_t v15 = [(GTBaseStreamTransport_capture *)self _writeBuffers:a4];
  dispatch_semaphore_signal((dispatch_semaphore_t)self->_sendSema);
  return v15;
}
...
- (BOOL)_packMessage:(id)a3 error:(id *)a4
{
  ...
  if (a4)
  {
    id v12 = +[GTError_capture errorWithDomain:@"DYErrorDomain" code:261 userInfo:0];
    BOOL result = 0;
    *a4 = v12;
    return result;
  }
  ...
  if (v6)
  {
    ...
    while (1)
    {
      do
      {
        while (1)
        {
          unint64_t v15 = [(GTBaseStreamTransport_capture *)self _write:iov->iov_base size:iov->iov_len];
          int64_t v16 = __error();
          if (v15 == -1) {
            break;
          }
...
LABEL_20:
    ...
    [(GTBaseStreamTransport_capture *)self _scheduleInvalidation:v22];
    if (a3) {
      *a3 = v22;
    }
    *__error() = v17;
...
- (int64_t)_readAndAccumulate
{
  ...
  {
    int64_t v4 = -[GTBaseStreamTransport_capture _syncTmuToHeader:ioBlock:](self, "_syncTmuToHeader:ioBlock:");
    if (v4 == -1) {
      return v4;
    }
    id v8 = self->_tmu;
...
LABEL_16:
  ...
  int64_t v11 = [(GTBaseStreamTransport_capture *)self _performRead:self->_messageBufferWritePtr size:var3];
  if (v11 == -1) {
    return -1;
  }
  v4 += v11;
...
- (int64_t)_performRead:(void *)a3 size:(unint64_t)a4
{
  ...
  do
  {
    int64_t result = [(GTBaseStreamTransport_capture *)self _read:a3 size:self->_bytesToRead];
    if (result != -1)
    {
      if (result)
      {
...
LABEL_5:
    ...
    if ((var5 & 0x80000000) != 0)
    {
      ...
      id v12 = [[-[GTVMBuffer subdataWithRange:](self->_messageBuffer, "subdataWithRange:", ((v7 + v4 + tmu->var2 - 1) & -(uint64_t)tmu->var2)- (void)-[GTVMBuffer mutableBytes](self->_messageBuffer, "mutableBytes"), var5 & 0x7FFFFFFF) GTGPUToolsData];
      uint64_t v9 = (v13 + v9);
      tmu = self->_tmu;
    }
    else
    ...
    int v14 = [[GTTransportMessage_capture alloc] initWithKind:tmu->var0.var1 attributes:v8 payload:v12];
    ...
    *(_OWORD *)&v16->unint64_t var3 = xmmword_1DA250;
...
- (int64_t)_syncTmuToHeader:(dy_transport_message_unpack_s *)a3 ioBlock:(id)a4
{
  ...
  if (result != -1 && !a3->var3)
  {
    ...
    *(int8x16_t *)&a3->var0.var0 = vrev32q_s8(*(int8x16_t *)&a3->var0.var0);
    ...
    [(GTBaseStreamTransport_capture *)self _scheduleInvalidation:+[GTError_capture errorWithDomain:@"DYErrorDomain" code:v11 userInfo:0]];
    return -1;
  }
  return result;
}
...
{
  v4.receiver = self;
  v4.super_class = (Class)GTBaseStreamTransport_capture;
...
- (void)_invalidate
{
  ...
  self->_messageBuffer = 0;
  v3.receiver = self;
  v3.super_class = (Class)GTBaseStreamTransport_capture;
...
- (void)dealloc
{
  ...
  v5.receiver = self;
  v5.super_class = (Class)GTBaseStreamTransport_capture;
...
{
  v9.receiver = self;
  v9.super_class = (Class)GTBaseStreamTransport_capture;
  uint64_t v2 = [(GTTransport_capture *)&v9 init];
  objc_super v3 = v2;
  if (v2)
  {
