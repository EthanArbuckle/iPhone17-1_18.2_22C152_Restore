...
- (GTTransportMessage_capture)initWithKind:(int)a3 attributes:(id)a4 BOOLPayload:(BOOL)a5;
- (GTTransportMessage_capture)initWithKind:(int)a3 attributes:(id)a4 objectPayload:(id)a5;
- (GTTransportMessage_capture)initWithKind:(int)a3 attributes:(id)a4 payload:(id)a5;
- (GTTransportMessage_capture)initWithKind:(int)a3 attributes:(id)a4 plistPayload:(id)a5;
- (GTTransportMessage_capture)initWithKind:(int)a3 attributes:(id)a4 stringPayload:(id)a5;
- (GTTransport_capture)transport;
...
- (id)plistPayload
{
  ...
  if (!result)
  {
    ...
    if (result)
    {
      ...
      if (v5)
      {
        unsigned int v6 = [[objc_msgSend(result, "objectForKey:", @"$archiver") isEqual:@"NSKeyedArchiver"];
        id result = self->_decodedPayload;
        if (v6)
        {
          ...
          if (v7)
          {
            id v8 = [result objectForKey:@"$top"][@"root"];
            id result = self->_decodedPayload;
            if (v8)
            {
...
LABEL_7:
    ...
    if ((char *)[(NSData *)payload length] == (char *)&def_CFBEC + 1)
    {
      payload = self->_payload;
      goto LABEL_7;
    }
    ...
    if (!v5)
    {
      id decodedPayload = [(GTTransportMessage_capture *)self objectPayload];
      if (decodedPayload) {
...
- (BOOL)BOOLForKey:(id)a3
{
  id v3 = [(NSDictionary *)[(GTTransportMessage_capture *)self attributes] objectForKey:a3];
...
- (void)_setSerial:(unsigned int)a3 replySerial:(unsigned int)a4 transport:(id)a5
{
  ...
  self->_transport = (GTTransport_capture *)a5;
}

- (BOOL)hasBeenSent
{
  return [(GTTransportMessage_capture *)self serial] != -1;
}

- (id)description
{
  v3.receiver = self;
  v3.super_class = (Class)GTTransportMessage_capture;
...
- (GTTransportMessage_capture)initWithKind:(int)a3 attributes:(id)a4 objectPayload:(id)a5
{
  ...
  v11 = [(GTTransportMessage_capture *)self initWithKind:v7 attributes:a4 payload:v5];
...
- (GTTransportMessage_capture)initWithKind:(int)a3 attributes:(id)a4 stringPayload:(id)a5
{
  ...
  ExternalRepresentation = (GTTransportMessage_capture *)CFStringCreateExternalRepresentation(kCFAllocatorDefault, (CFStringRef)a5, 0x8000100u, 0);
  if (ExternalRepresentation)
  {
LABEL_5:
    id v9 = [(GTTransportMessage_capture *)self initWithKind:v6 attributes:a4 payload:ExternalRepresentation];
...
- (GTTransportMessage_capture)initWithKind:(int)a3 attributes:(id)a4 plistPayload:(id)a5
{
  ...
  id v10 = [(GTTransportMessage_capture *)self initWithKind:v6 attributes:a4 payload:v9];
...
- (GTTransportMessage_capture)initWithKind:(int)a3 attributes:(id)a4 BOOLPayload:(BOOL)a5
{
  ...
  {
    CFDataRef v9 = v8;
    id v10 = [(GTTransportMessage_capture *)self initWithKind:v6 attributes:a4 payload:v8];
  }
  else
  {
...
{
  v11.receiver = self;
  v11.super_class = (Class)GTTransportMessage_capture;
  id v8 = [(GTTransportMessage_capture *)&v11 init];
  CFDataRef v9 = v8;
  if (v8)
  {
