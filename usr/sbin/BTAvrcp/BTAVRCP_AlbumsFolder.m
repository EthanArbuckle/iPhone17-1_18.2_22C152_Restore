...
- (BTAVRCP_AlbumsFolder)initWithName:(id)a3 uid:(unint64_t)a4
{
  return [(BTAVRCP_AlbumsFolder *)self initWithName:a3 uid:a4 isRecentlyAdded:0];
}
...
- (id)albumName:(id)a3
{
  ...
  if (![v4 length])
  {
    ...
    uint64_t v6 = [v5 localizedStringForKey:@"UNKNOWN_ALBUM" value:@"Unknown Album" table:0];
...
- (id)baseQuery
{
  BOOL v3 = +[MPMediaQuery albumsQuery];
  if ([(BTAVRCP_AlbumsFolder *)self isRecentlyAdded]) {
...
- (unint64_t)childrenCount
{
  ...
  return (unint64_t)v5 + [(BTAVRCP_AlbumsFolder *)self showRecentlyAddedFolder];
}

- (unsigned)createFolderWithUid:(unint64_t)a3 folder:(id *)a4
{
  if ([(BTAVRCP_VFSFolder *)self uid] == a3
    && [(BTAVRCP_AlbumsFolder *)self showRecentlyAddedFolder])
  {
    objc_super v7 = [BTAVRCP_AlbumsFolder alloc];
...
{
  unint64_t v4 = a3;
  if ([(BTAVRCP_AlbumsFolder *)self showRecentlyAddedFolder])
  {
    if (!v4)
    {
...
- (id)replyAttributesForUid:(unint64_t)a3 attributeIDs:(id)a4
{
  ...
  uint64_t v6 = [v5 collectionWithUid:a3 property:MPMediaItemPropertyAlbumPersistentID];
...
- (unsigned)playItemWithUid:(unint64_t)a3
{
  ...
  id v5 = [v4 collectionWithUid:a3 property:MPMediaItemPropertyAlbumPersistentID];
