...
- (id)description
{
  ...
  [v5 appendFormat:@"finalData2Bitmask:0x%02x>", self->_finalData2Bitmask];
...
- (AcwgM1MsgStruct)toStruct
{
  sub_1001D9B8C(self->_supportedUwbConfigIds, (uint64_t)v7);
  sub_1001D9DB8(self->_supportedPulseShapeCombos, (unsigned __int8 **)__p);
  sub_1001DB218((uint64_t)retstr, (uint64_t)v7, (uint64_t)__p, self->_channelBitmask, self->_uwbSessionId, self->_finalData2Bitmask);
  if (__p[0])
  {
    __p[1] = __p[0];
...
- (id)copyWithZone:(_NSZone *)a3
{
  ...
  return _[v4 initWithSupportedUwbConfigIds:supportedUwbConfigIds supportedPulseShapeCombos:supportedPulseShapeCombos channelBitmask:channelBitmask uwbSessionId:uwbSessionId finalData2Bitmask:finalData2Bitmask];
}
