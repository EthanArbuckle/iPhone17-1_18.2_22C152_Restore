...
{
  v3.receiver = self;
  v3.super_class = (Class)TransparencyGPBUInt32Array;
  return [(TransparencyGPBUInt32Array *)&v3 init];
}

- (TransparencyGPBUInt32Array)initWithValueArray:(id)a3
{
  return [(TransparencyGPBUInt32Array *)self initWithValues:*((void *)a3 + 2) count:*((void *)a3 + 3)];
}

- (TransparencyGPBUInt32Array)initWithValues:(const unsigned int *)a3 count:(unint64_t)a4
{
  v6 = [(TransparencyGPBUInt32Array *)self init];
  v7 = v6;
  if (v6 && a3 && a4)
  {
...
- (TransparencyGPBUInt32Array)initWithCapacity:(unint64_t)a3
{
  unsigned int v4 = [(TransparencyGPBUInt32Array *)self initWithValues:0 count:0];
  v5 = v4;
  if (a3 && v4) {
    [(TransparencyGPBUInt32Array *)v4 internalResizeToCapacity:a3];
  }
  return v5;
}
...
- (id)copyWithZone:(_NSZone *)a3
{
  unsigned int v4 = +[TransparencyGPBUInt32Array allocWithZone:a3];
  values = self->_values;
  unint64_t count = self->_count;

  return [(TransparencyGPBUInt32Array *)v4 initWithValues:values count:count];
}
...
- (void)dealloc
{
  ...
  v3.receiver = self;
  v3.super_class = (Class)TransparencyGPBUInt32Array;
  [(TransparencyGPBUInt32Array *)&v3 dealloc];
}
...
{
  unsigned int v3 = a3;
  [(TransparencyGPBUInt32Array *)self addValues:&v3 count:1];
}
...
- (void)addValues:(const unsigned int *)a3 count:(unint64_t)a4
{
  if (a3 && a4)
  {
    ...
    if (count + a4 > self->_capacity) {
      [(TransparencyGPBUInt32Array *)self internalResizeToCapacity:(v8 & 0xFFFFFFFFFFFFFFF0) + 16];
    }
...
- (void)removeValueAtIndex:(unint64_t)a3
{
  ...
  {
    [(TransparencyGPBUInt32Array *)self internalResizeToCapacity:(v6 & 0xFFFFFFFFFFFFFFF0) + 16];
  }
}
...
- (void)removeAll
{
  ...
  if (self->_capacity >= 0x21) {
    [(TransparencyGPBUInt32Array *)self internalResizeToCapacity:16];
  }
}
