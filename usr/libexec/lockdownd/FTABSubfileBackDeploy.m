...
- (FTABSubfileBackDeploy)initWithTag:(id)a3 data:(id)a4
{
  ...
  id v8 = [(FTABSubfileBackDeploy *)&v14 init];
  if (v8)
  {
    v9 = (NSString *)[v6 copy];
    tag = v8->_tag;
...
- (NSData)data
{
  return +[NSData dataWithBytes:self->_dataPointer length:self->_dataLength];
}
...
- (id)description
{
  return +[NSString stringWithFormat:@"Subfile: type=%@, length=%u", self->_tag, self->_dataLength];
}
