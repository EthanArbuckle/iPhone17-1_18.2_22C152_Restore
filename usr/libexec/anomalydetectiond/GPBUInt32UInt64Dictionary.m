...
- (BOOL)isEqual:(id)a3;
- (GPBUInt32UInt64Dictionary)init;
- (GPBUInt32UInt64Dictionary)initWithCapacity:(unint64_t)a3;
- (GPBUInt32UInt64Dictionary)initWithDictionary:(id)a3;
- (GPBUInt32UInt64Dictionary)initWithUInt64s:(const unint64_t *)a3 forKeys:(const unsigned int *)a4 count:(unint64_t)a5;
...
- (GPBUInt32UInt64Dictionary)init
{
  return [(GPBUInt32UInt64Dictionary *)self initWithUInt64s:0 forKeys:0 count:0];
}

- (GPBUInt32UInt64Dictionary)initWithUInt64s:(const unint64_t *)a3 forKeys:(const unsigned int *)a4 count:(unint64_t)a5
{
  v13.receiver = self;
  v13.super_class = (Class)GPBUInt32UInt64Dictionary;
  v8 = [(GPBUInt32UInt64Dictionary *)&v13 init];
  if (v8)
  {
...
- (GPBUInt32UInt64Dictionary)initWithDictionary:(id)a3
{
  v4 = [(GPBUInt32UInt64Dictionary *)self initWithUInt64s:0 forKeys:0 count:0];
  v5 = v4;
  if (a3 && v4) {
...
- (void)dealloc
{
  if (self->_autocreator) {
    [+[NSAssertionHandler currentHandler] handleFailureInMethod:a2, self, @"GPBDictionary.m", 1844, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator object file lineNumber description];
  }

  v3.receiver = self;
  v3.super_class = (Class)GPBUInt32UInt64Dictionary;
  [(GPBUInt32UInt64Dictionary *)&v3 dealloc];
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[GPBUInt32UInt64Dictionary allocWithZone:a3];

  return [(GPBUInt32UInt64Dictionary *)v4 initWithDictionary:self];
}
...
- (void)enumerateForTextFormat:(id)a3
{
  ...
  v3[3] = &unk_1003BA5E8;
  v3[4] = a3;
  [(GPBUInt32UInt64Dictionary *)self enumerateKeysAndUInt64sUsingBlock:v3];
}
...
- (void)setUInt64:(unint64_t)a3 forKey:(unsigned int)a4
{
  [(NSMutableDictionary *)self->_dictionary setObject:+[NSNumber numberWithUnsignedLongLong:](NSNumber, "numberWithUnsignedLongLong:", a3) forKey:+[NSNumber numberWithUnsignedInt:*(void *)&a4]];
  autocreator = self->_autocreator;
  if (autocreator)
  {
    GPBAutocreatedDictionaryModified(autocreator, (uint64_t)self);
