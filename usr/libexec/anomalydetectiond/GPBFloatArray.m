...
- (GPBFloatArray)initWithValueArray:(id)a3;
...
{
  v3.receiver = self;
  v3.super_class = (Class)GPBFloatArray;
  return [(GPBFloatArray *)&v3 init];
}

- (GPBFloatArray)initWithValueArray:(id)a3
{
  return [(GPBFloatArray *)self initWithValues:*((void *)a3 + 2) count:*((void *)a3 + 3)];
}
...
- (GPBFloatArray)initWithCapacity:(unint64_t)a3
{
  float v4 = [(GPBFloatArray *)self initWithValues:0 count:0];
  v5 = v4;
  if (a3 && v4) {
...
- (id)copyWithZone:(_NSZone *)a3
{
  float v4 = +[GPBFloatArray allocWithZone:a3];
  values = self->_values;
...
- (void)dealloc
{
  if (self->_autocreator) {
    [+[NSAssertionHandler currentHandler] handleFailureInMethod:a2, self, @"GPBArray.m", 1374, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator object file lineNumber description];
  }
  free(self->_values);
  v3.receiver = self;
  v3.super_class = (Class)GPBFloatArray;
...
{
  unint64_t v5 = 4 * a3;
  CFStringRef v6 = (float *)reallocf(self->_values, 4 * a3);
  self->_values = v6;
  if (!v6)
  {
...
{
  float v3 = a3;
  [(GPBFloatArray *)self addValues:&v3 count:1];
}
