...
- (BOOL)isEqual:(id)a3;
- (GPBStringDoubleDictionary)init;
- (GPBStringDoubleDictionary)initWithCapacity:(unint64_t)a3;
- (GPBStringDoubleDictionary)initWithDictionary:(id)a3;
- (GPBStringDoubleDictionary)initWithDoubles:(const double *)a3 forKeys:(id *)a4 count:(unint64_t)a5;
...
- (GPBStringDoubleDictionary)init
{
  return [(GPBStringDoubleDictionary *)self initWithDoubles:0 forKeys:0 count:0];
}

- (GPBStringDoubleDictionary)initWithDoubles:(const double *)a3 forKeys:(id *)a4 count:(unint64_t)a5
{
  v14.receiver = self;
  v14.super_class = (Class)GPBStringDoubleDictionary;
  v8 = [(GPBStringDoubleDictionary *)&v14 init];
  if (v8)
  {
...
- (GPBStringDoubleDictionary)initWithDictionary:(id)a3
{
  v4 = [(GPBStringDoubleDictionary *)self initWithDoubles:0 forKeys:0 count:0];
  v5 = v4;
  if (a3 && v4) {
...
- (void)dealloc
{
  if (self->_autocreator) {
    [+[NSAssertionHandler currentHandler] handleFailureInMethod:a2, self, @"GPBDictionary.m", 9628, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator object file lineNumber description];
  }

  v3.receiver = self;
  v3.super_class = (Class)GPBStringDoubleDictionary;
  [(GPBStringDoubleDictionary *)&v3 dealloc];
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[GPBStringDoubleDictionary allocWithZone:a3];

  return [(GPBStringDoubleDictionary *)v4 initWithDictionary:self];
}
...
- (void)enumerateForTextFormat:(id)a3
{
  ...
  v3[3] = &unk_1003BAB88;
  v3[4] = a3;
  [(GPBStringDoubleDictionary *)self enumerateKeysAndDoublesUsingBlock:v3];
}
