...
- (BOOL)isEqual:(id)a3;
- (GPBUInt32UInt32Dictionary)init;
- (GPBUInt32UInt32Dictionary)initWithCapacity:(unint64_t)a3;
- (GPBUInt32UInt32Dictionary)initWithDictionary:(id)a3;
- (GPBUInt32UInt32Dictionary)initWithUInt32s:(const unsigned int *)a3 forKeys:(const unsigned int *)a4 count:(unint64_t)a5;
...
- (GPBUInt32UInt32Dictionary)init
{
  return [(GPBUInt32UInt32Dictionary *)self initWithUInt32s:0 forKeys:0 count:0];
}

- (GPBUInt32UInt32Dictionary)initWithUInt32s:(const unsigned int *)a3 forKeys:(const unsigned int *)a4 count:(unint64_t)a5
{
  v14.receiver = self;
  v14.super_class = (Class)GPBUInt32UInt32Dictionary;
...
- (void)dealloc
{
  if (self->_autocreator) {
    [+[NSAssertionHandler currentHandler] handleFailureInMethod:a2, self, @"GPBDictionary.m", 1478, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator object file lineNumber description];
  }

  v3.receiver = self;
  v3.super_class = (Class)GPBUInt32UInt32Dictionary;
  [(GPBUInt32UInt32Dictionary *)&v3 dealloc];
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[GPBUInt32UInt32Dictionary allocWithZone:a3];
...
- (void)enumerateKeysAndUInt32sUsingBlock:(id)a3
{
  ...
  do
  {
    ...
    (*((void (**)(id, id, id, char *))a3 + 2))(a3, [v6 unsignedIntValue], [objc_msgSend(-[NSMutableDictionary objectForKeyedSubscript:](dictionary, "objectForKeyedSubscript:", v6) unsignedIntValue], &v7);
  }
  while (!v7);
}
...
- (void)setUInt32:(unsigned int)a3 forKey:(unsigned int)a4
{
  [(NSMutableDictionary *)self->_dictionary setObject:+[NSNumber numberWithUnsignedInt:](NSNumber, "numberWithUnsignedInt:", *(void *)&a3) forKey:+[NSNumber numberWithUnsignedInt:*(void *)&a4]];
  autocreator = self->_autocreator;
  if (autocreator)
  {
    GPBAutocreatedDictionaryModified(autocreator, (uint64_t)self);
