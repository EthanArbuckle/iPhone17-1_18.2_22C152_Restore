...
- (BOOL)isEqual:(id)a3;
- (GPBInt64UInt32Dictionary)init;
- (GPBInt64UInt32Dictionary)initWithCapacity:(unint64_t)a3;
...
- (GPBInt64UInt32Dictionary)init
{
  return [(GPBInt64UInt32Dictionary *)self initWithUInt32s:0 forKeys:0 count:0];
}

- (GPBInt64UInt32Dictionary)initWithUInt32s:(const unsigned int *)a3 forKeys:(const int64_t *)a4 count:(unint64_t)a5
{
  v14.receiver = self;
  v14.super_class = (Class)GPBInt64UInt32Dictionary;
...
- (void)dealloc
{
  if (self->_autocreator) {
    [+[NSAssertionHandler currentHandler] handleFailureInMethod:a2, self, @"GPBDictionary.m", 6728, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator object file lineNumber description];
  }

  v3.receiver = self;
  v3.super_class = (Class)GPBInt64UInt32Dictionary;
...
- (void)enumerateKeysAndUInt32sUsingBlock:(id)a3
{
  ...
  do
  {
    ...
    [dictionary objectForKeyedSubscript:v6][v6 unsignedIntValue];
  }
  while (!v7);
}
