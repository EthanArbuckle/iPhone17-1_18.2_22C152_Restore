...
- (BOOL)isEqual:(id)a3;
- (GPBInt32BoolDictionary)init;
- (GPBInt32BoolDictionary)initWithBools:(const BOOL *)a3 forKeys:(const int *)a4 count:(unint64_t)a5;
- (GPBInt32BoolDictionary)initWithCapacity:(unint64_t)a3;
- (GPBInt32BoolDictionary)initWithDictionary:(id)a3;
...
- (GPBInt32BoolDictionary)init
{
  return [(GPBInt32BoolDictionary *)self initWithBools:0 forKeys:0 count:0];
}

- (GPBInt32BoolDictionary)initWithBools:(const BOOL *)a3 forKeys:(const int *)a4 count:(unint64_t)a5
{
  v14.receiver = self;
  v14.super_class = (Class)GPBInt32BoolDictionary;
...
- (void)dealloc
{
  if (self->_autocreator) {
    [+[NSAssertionHandler currentHandler] handleFailureInMethod:a2, self, @"GPBDictionary.m", 3960, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator object file lineNumber description];
  }

  v3.receiver = self;
  v3.super_class = (Class)GPBInt32BoolDictionary;
  [(GPBInt32BoolDictionary *)&v3 dealloc];
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[GPBInt32BoolDictionary allocWithZone:a3];
