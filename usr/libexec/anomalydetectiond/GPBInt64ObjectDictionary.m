...
- (BOOL)isEqual:(id)a3;
- (BOOL)isInitialized;
- (GPBInt64ObjectDictionary)init;
- (GPBInt64ObjectDictionary)initWithCapacity:(unint64_t)a3;
- (GPBInt64ObjectDictionary)initWithDictionary:(id)a3;
- (GPBInt64ObjectDictionary)initWithObjects:(const void *)a3 forKeys:(const int64_t *)a4 count:(unint64_t)a5;
...
- (GPBInt64ObjectDictionary)init
{
  return [(GPBInt64ObjectDictionary *)self initWithObjects:0 forKeys:0 count:0];
}

- (GPBInt64ObjectDictionary)initWithObjects:(const void *)a3 forKeys:(const int64_t *)a4 count:(unint64_t)a5
{
  v12.receiver = self;
  v12.super_class = (Class)GPBInt64ObjectDictionary;
...
- (void)dealloc
{
  if (self->_autocreator) {
    [+[NSAssertionHandler currentHandler] handleFailureInMethod:a2, self, @"GPBDictionary.m", 8265, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator object file lineNumber description];
  }

  v3.receiver = self;
  v3.super_class = (Class)GPBInt64ObjectDictionary;
  [(GPBInt64ObjectDictionary *)&v3 dealloc];
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[GPBInt64ObjectDictionary allocWithZone:a3];
...
- (void)enumerateKeysAndObjectsUsingBlock:(id)a3
{
  ...
  do
  {
    ...
    (*((void (**)(id, id, id, char *))a3 + 2))(a3, [v6 longLongValue], -[NSMutableDictionary objectForKeyedSubscript:](dictionary, "objectForKeyedSubscript:", v6), &v7);
  }
  while (!v7);
}
...
- (id)deepCopyWithZone:(_NSZone *)a3
{
  uint64_t v5 = objc_alloc_init(GPBInt64ObjectDictionary);
...
- (void)writeToCodedOutputStream:(id)a3 asField:(id)a4
{
  ...
  if (v12)
  {
    ...
    do
    {
      ...
      int v16 = sub_1002EDB6C((unint64_t)v15, 1, v8);
...
- (void)setGPBGenericValue:(id *)a3 forGPBGenericValueKey:(id *)a4
{
  ...
  unint64_t v6 = +[NSNumber numberWithLongLong:a4->var2];
