...
- (GPBInt32UInt64Dictionary)init
{
  return [(GPBInt32UInt64Dictionary *)self initWithUInt64s:0 forKeys:0 count:0];
}
...
- (GPBInt32UInt64Dictionary)initWithUInt64s:(const unint64_t *)a3 forKeys:(const int *)a4 count:(unint64_t)a5
{
  ...
  v8 = [(GPBInt32UInt64Dictionary *)&v13 init];
  if (v8)
  {
...
- (GPBInt32UInt64Dictionary)initWithDictionary:(id)a3
{
  v4 = [(GPBInt32UInt64Dictionary *)self initWithUInt64s:0 forKeys:0 count:0];
  v5 = v4;
  if (a3 && v4) {
...
- (void)dealloc
{
  if (self->_autocreator) {
    [+[NSAssertionHandler currentHandler] handleFailureInMethod:a2, self, @"GPBDictionary.m", 3594, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator object file lineNumber description];
  }

  v3.receiver = self;
  v3.super_class = (Class)GPBInt32UInt64Dictionary;
  [(GPBInt32UInt64Dictionary *)&v3 dealloc];
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[GPBInt32UInt64Dictionary allocWithZone:a3];

  return [(GPBInt32UInt64Dictionary *)v4 initWithDictionary:self];
}
...
- (void)enumerateKeysAndUInt64sUsingBlock:(id)a3
{
  ...
  do
  {
    ...
    (*((void (**)(id, id, id, char *))a3 + 2))(a3, [v6 intValue], [objc_msgSend(-[NSMutableDictionary objectForKeyedSubscript:](dictionary, "objectForKeyedSubscript:", v6) unsignedLongLongValue], &v7);
  }
  while (!v7);
}
...
- (void)setUInt64:(unint64_t)a3 forKey:(int)a4
{
  [(NSMutableDictionary *)self->_dictionary setObject:+[NSNumber numberWithUnsignedLongLong:](NSNumber, "numberWithUnsignedLongLong:", a3) forKey:+[NSNumber numberWithInt:*(void *)&a4]];
  autocreator = self->_autocreator;
  if (autocreator)
  {
    GPBAutocreatedDictionaryModified(autocreator, (uint64_t)self);
