...
- (BOOL)isEqual:(id)a3;
- (GPBInt32DoubleDictionary)init;
- (GPBInt32DoubleDictionary)initWithCapacity:(unint64_t)a3;
...
- (GPBInt32DoubleDictionary)init
{
  return [(GPBInt32DoubleDictionary *)self initWithDoubles:0 forKeys:0 count:0];
}

- (GPBInt32DoubleDictionary)initWithDoubles:(const double *)a3 forKeys:(const int *)a4 count:(unint64_t)a5
{
  v13.receiver = self;
  v13.super_class = (Class)GPBInt32DoubleDictionary;
...
- (void)dealloc
{
  if (self->_autocreator) {
    [+[NSAssertionHandler currentHandler] handleFailureInMethod:a2, self, @"GPBDictionary.m", 4326, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator object file lineNumber description];
  }

  v3.receiver = self;
  v3.super_class = (Class)GPBInt32DoubleDictionary;
