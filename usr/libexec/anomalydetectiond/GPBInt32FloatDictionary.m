...
- (GPBInt32FloatDictionary)init
{
  return [(GPBInt32FloatDictionary *)self initWithFloats:0 forKeys:0 count:0];
}
...
- (void)dealloc
{
  if (self->_autocreator) {
    [+[NSAssertionHandler currentHandler] handleFailureInMethod:a2, self, @"GPBDictionary.m", 4143, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator object file lineNumber description];
  }
...
- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[GPBInt32FloatDictionary allocWithZone:a3];
...
- (void)writeToCodedOutputStream:(id)a3 asField:(id)a4
{
  ...
  if (v12)
  {
    ...
    do
    {
      ...
      [v14 floatValue];
      int v17 = v16;
      LODWORD(v14) = sub_1002EC618(v15, 1, v8);
...
- (void)setGPBGenericValue:(id *)a3 forGPBGenericValueKey:(id *)a4
{
  ...
  unsigned int v8 = +[NSNumber numberWithInt:a4->var3];
...
- (void)setFloat:(float)a3 forKey:(int)a4
{
  [(NSMutableDictionary *)self->_dictionary setObject:+[NSNumber numberWithFloat:](NSNumber, "numberWithFloat:") forKey:+[NSNumber numberWithInt:*(void *)&a4]];
  autocreator = self->_autocreator;
  if (autocreator)
  {
    GPBAutocreatedDictionaryModified(autocreator, (uint64_t)self);
...
{
  dictionary = self->_dictionary;
  double v4 = +[NSNumber numberWithInt:*(void *)&a3];
