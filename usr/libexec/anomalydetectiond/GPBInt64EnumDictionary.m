...
- (GPBInt64EnumDictionary)init
{
  return [(GPBInt64EnumDictionary *)self initWithValidationFunction:0 rawValues:0 forKeys:0 count:0];
}

- (GPBInt64EnumDictionary)initWithValidationFunction:(void *)a3
{
  return [(GPBInt64EnumDictionary *)self initWithValidationFunction:a3 rawValues:0 forKeys:0 count:0];
}
...
- (GPBInt64EnumDictionary)initWithDictionary:(id)a3
{
  v4 = -[GPBInt64EnumDictionary initWithValidationFunction:rawValues:forKeys:count:](self, "initWithValidationFunction:rawValues:forKeys:count:", [a3 validationFunc], 0, 0, 0);
  v5 = v4;
  if (a3 && v4) {
...
- (void)dealloc
{
  if (self->_autocreator) {
    [+[NSAssertionHandler currentHandler] handleFailureInMethod:a2, self, @"GPBDictionary.m", 8022, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator object file lineNumber description];
  }
...
- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[GPBInt64EnumDictionary allocWithZone:a3];
...
- (void)enumerateKeysAndRawValuesUsingBlock:(id)a3
{
  ...
  do
  {
    ...
    [dictionary objectForKeyedSubscript:v6] = [v6 intValue];
  }
  while (!v7);
}
...
- (void)setEnum:(int)a3 forKey:(int64_t)a4
{
  ...
  if ((((uint64_t (*)(void, SEL))self->_validationFunc)(*(void *)&a3, a2) & 1) == 0) {
    +[NSException raise:NSInvalidArgumentException, @"GPBInt64EnumDictionary: Attempt to set an unknown enum value (%d)", v5 format];
  }
