...
+ (id)processDataWithFunction:(int64_t)a3 algorithm:(int64_t)a4 content:(id)a5 progressBlock:(id)a6
{
  id v9 = a5;
  v10 = (void (**)(id, void))a6;
  v11 = [[_CSCompressionEngine alloc] initWithFunction:a3 algorithm:a4 destination:0];
  v12 = (char *)[v9 length];
  id v13 = v9;
  v14 = (char *)[v13 bytes];
  if (v12)
  {
    ...
    while (1)
    {
      uint64_t v17 = (unint64_t)&v12[-v16] >= 0x10000 ? 0x10000 : (uint64_t)&v12[-v16];
      if (![(_CSCompressionEngine *)v11 _processIncomingData:&v15[v16] length:v17 flags:0])break; {
      v16 += v17;
      }
      if (v10) {
        v10[2](v10, v16);
...
LABEL_10:
    ...
    v18 = [(_CSCompressionEngine *)v11 sinkContent];
  }
...
- (_CSCompressionEngine)initWithFunction:(int64_t)a3 algorithm:(int64_t)a4 destination:(id)a5
{
  ...
  id v9 = [(_CSCompressionEngine *)&v15 init];
  v10 = v9;
  if (v9)
  {
    ...
    [(_CSCompressionEngine *)v10 _decodeEngineFunction:a3 algorithm:a4];
    objc_storeWeak((id *)&v10->_destination, v8);
    v12 = (NSMutableData *)[objc_alloc((Class)NSMutableData) initWithLength:0x10000];
...
- (void)writeSourceContent:(id)a3
{
  ...
  [(_CSCompressionEngine *)self _processIncomingData:v5 length:v6 flags:0];
}
...
LABEL_6:
  if ((unint64_t)a4 <= 3) {
    self->_algorithm = dword_100337AD0[a4];
  }
}
...
- (void)_gatherReadSinkContent:(id)a3
{
  ...
  if (!sinkContent)
  {
    id v6 = (NSMutableData *)[objc_alloc((Class)NSMutableData) initWithCapacity:0x10000];
...
- (BOOL)_initializeStream
{
  if (self->_stream)
  {
    ...
    [v8 handleFailureInMethod:a2, self, @"_CSCompressionEngine.m", 161, @"Invalid parameter not satisfying: %@", @"_stream == nil" object file lineNumber description];
  }
...
- (BOOL)_processIncomingData:(const void *)a3 length:(unint64_t)a4 flags:(int)a5
{
  ...
  while (1)
  {
    while (1)
    {
      ...
      unint64_t v13 = self->_stream->var3 | [(_CSCompressionEngine *)self _deliverDestinationContent];
...
- (_CSStreamingDestination)destination
{
  ...
  return (_CSStreamingDestination *)WeakRetained;
}
