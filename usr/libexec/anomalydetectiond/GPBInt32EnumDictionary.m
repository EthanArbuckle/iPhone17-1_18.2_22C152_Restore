...
- (BOOL)isEqual:(id)a3;
- (GPBInt32EnumDictionary)init;
- (GPBInt32EnumDictionary)initWithDictionary:(id)a3;
- (GPBInt32EnumDictionary)initWithValidationFunction:(void *)a3;
- (GPBInt32EnumDictionary)initWithValidationFunction:(void *)a3 capacity:(unint64_t)a4;
...
- (GPBInt32EnumDictionary)init
{
  return [(GPBInt32EnumDictionary *)self initWithValidationFunction:0 rawValues:0 forKeys:0 count:0];
}

- (GPBInt32EnumDictionary)initWithValidationFunction:(void *)a3
{
  return [(GPBInt32EnumDictionary *)self initWithValidationFunction:a3 rawValues:0 forKeys:0 count:0];
}

- (GPBInt32EnumDictionary)initWithValidationFunction:(void *)a3 rawValues:(const int *)a4 forKeys:(const int *)a5 count:(unint64_t)a6
{
  v18.receiver = self;
  v18.super_class = (Class)GPBInt32EnumDictionary;
...
- (void)dealloc
{
  if (self->_autocreator) {
    [+[NSAssertionHandler currentHandler] handleFailureInMethod:a2, self, @"GPBDictionary.m", 4522, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator object file lineNumber description];
  }

  v3.receiver = self;
  v3.super_class = (Class)GPBInt32EnumDictionary;
...
- (BOOL)getEnum:(int *)a3 forKey:(int)a4
{
  id v6 = [(NSMutableDictionary *)self->_dictionary objectForKey:+[NSNumber numberWithInt:*(void *)&a4]];
  id v7 = v6;
  if (a3 && v6)
  {
...
- (void)setEnum:(int)a3 forKey:(int)a4
{
  ...
  if ((((uint64_t (*)(void, SEL))self->_validationFunc)(*(void *)&a3, a2) & 1) == 0) {
    +[NSException raise:NSInvalidArgumentException, @"GPBInt32EnumDictionary: Attempt to set an unknown enum value (%d)", v5 format];
  }
  [(NSMutableDictionary *)self->_dictionary setObject:+[NSNumber numberWithInt:](NSNumber, "numberWithInt:", v5) forKey:+[NSNumber numberWithInt:v4]];
  autocreator = self->_autocreator;
  if (autocreator)
  {
    GPBAutocreatedDictionaryModified(autocreator, (uint64_t)self);
