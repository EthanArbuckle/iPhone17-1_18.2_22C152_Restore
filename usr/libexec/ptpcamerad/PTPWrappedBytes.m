...
- (id)mutableData
{
  return +[NSMutableData dataWithBytes:self->_bytes length:self->_offset];
}

- (id)data
{
  return +[NSData dataWithBytes:self->_bytes length:self->_offset];
}
...
LABEL_8:
  ...
  unint64_t offset = self->_offset;
  if (self->_capacity - offset >= (unint64_t)v5) {
    goto LABEL_8;
  }
  unint64_t capacity = (unint64_t)v5 + offset;
  v9 = (char *)malloc_type_realloc(self->_bytes, (size_t)v5 + offset, 0x5B2E4939uLL);
  if (v9)
  {
    self->_bytes = v9;
...
LABEL_9:
    ...
    if (bytes) {
      ...
      ssize_t v6 = write(self->_fd, [v4 bytes], v6);
    }
    self->_offset += v6;
  }
LABEL_13:
  self->_excessDataLength += (unint64_t)[v4 length] - v6;
