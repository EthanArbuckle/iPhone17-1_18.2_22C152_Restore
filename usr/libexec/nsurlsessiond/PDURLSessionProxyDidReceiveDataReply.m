@interface PDURLSessionProxyDidReceiveDataReply
- (BOOL)hasTask;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
...
@implementation PDURLSessionProxyDidReceiveDataReply
...
- (void)mergeFrom:(id)a3
{
  ...
  else
  {
    ...
    v7 = v4;
    -[PDURLSessionProxyDidReceiveDataReply setTask:](self, "setTask:");
  }
  v4 = v7;
LABEL_7:
}
...
- (unint64_t)hash
{
  return [(PDURLSessionProxyTaskMessage *)self->_task hash];
}
...
- (BOOL)isEqual:(id)a3
{
  ...
  if ([v4 isMemberOfClass:objc_opt_class()])
  {
    ...
    if ((unint64_t)task | v4[1]) {
      unsigned __int8 v6 = -[PDURLSessionProxyTaskMessage isEqual:](task, "isEqual:");
    }
    else {
...
{
  v7.receiver = self;
  v7.super_class = (Class)PDURLSessionProxyDidReceiveDataReply;
  v3 = [(PDURLSessionProxyDidReceiveDataReply *)&v7 description];
  v4 = [(PDURLSessionProxyDidReceiveDataReply *)self dictionaryRepresentation];
