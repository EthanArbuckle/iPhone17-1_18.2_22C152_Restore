@interface PowerMgr
+ (id)sharedInstance;
- (FMConcurrentMutableDictionary)assertionsDict;
- (void)_disableAssertion:(unsigned int)a3 withName:(id)a4;
- (void)powerAssertionDisableWithReason:(id)a3;
- (void)powerAssertionEnableWithReason:(id)a3;
- (void)powerAssertionEnableWithReason:(id)a3 timeout:(int64_t)a4;
- (void)powerAssertionEnableWithReason:(id)a3 type:(id)a4;
- (void)powerAssertionEnableWithReason:(id)a3 type:(id)a4 timeout:(int64_t)a5;
- (void)setAssertionsDict:(id)a3;
@end

@implementation PowerMgr

+ (id)sharedInstance
{
  if (qword_10006D428 != -1) {
    dispatch_once(&qword_10006D428, &stru_1000597B8);
  }
  v2 = (void *)qword_10006D420;

  return v2;
}

- (void)powerAssertionEnableWithReason:(id)a3
{
}

- (void)powerAssertionEnableWithReason:(id)a3 timeout:(int64_t)a4
{
}

- (void)powerAssertionEnableWithReason:(id)a3 type:(id)a4
{
}

- (void)powerAssertionEnableWithReason:(id)a3 type:(id)a4 timeout:(int64_t)a5
{
  id v8 = a3;
  v9 = (char *)a4;
  v10 = +[NSBundle mainBundle];
  v11 = [v10 bundleIdentifier];

  v12 = +[NSString stringWithFormat:@"%@-%@", v11, v8];
  v13 = [(PowerMgr *)self assertionsDict];
  if (![v13 count])
  {

    v15 = 0;
    goto LABEL_7;
  }
  v14 = [(PowerMgr *)self assertionsDict];
  v15 = [v14 objectForKeyedSubscript:v12];

  if (!v15)
  {
LABEL_7:
    int v18 = 0;
    uint64_t v16 = 0xFFFFFFFFLL;
    goto LABEL_8;
  }
  uint64_t v16 = (uint64_t)[v15 intValue];
  v17 = sub_10001B6D4();
  if (os_log_type_enabled(v17, OS_LOG_TYPE_DEBUG)) {
    sub_10003746C();
  }

  int v18 = 1;
LABEL_8:
  v19 = sub_10001B6D4();
  if (os_log_type_enabled(v19, OS_LOG_TYPE_DEBUG))
  {
    *(_DWORD *)buf = 138412802;
    v32 = v12;
    __int16 v33 = 2112;
    v34 = v9;
    __int16 v35 = 2048;
    int64_t v36 = a5;
    _os_log_debug_impl((void *)&_mh_execute_header, v19, OS_LOG_TYPE_DEBUG, "Enable power assertion %@ with type %@ & timeout %ld", buf, 0x20u);
  }

  v20 = +[NSMutableDictionary dictionary];
  if ([v9 isEqualToString:@"ApplePushServiceTask"])
  {
    [v20 setObject:v12 forKeyedSubscript:@"AssertName"];
    v21 = +[NSNumber numberWithInteger:a5];
    [v20 setObject:v21 forKeyedSubscript:@"TimeoutSeconds"];

    [v20 setObject:@"TimeoutActionTurnOff" forKeyedSubscript:@"TimeoutAction"];
    if (v9) {
      [v20 setObject:v9 forKeyedSubscript:@"AssertType"];
    }
    [v20 setObject:&off_10005EED0 forKeyedSubscript:@"AssertLevel"];
  }
  else
  {
    [v20 setObject:v12 forKeyedSubscript:@"AssertName"];
    v22 = +[NSNumber numberWithInteger:a5];
    [v20 setObject:v22 forKeyedSubscript:@"TimeoutSeconds"];

    [v20 setObject:@"TimeoutActionTurnOff" forKeyedSubscript:@"TimeoutAction"];
    if (v9) {
      [v20 setObject:v9 forKeyedSubscript:@"AssertType"];
    }
    [v20 setObject:&off_10005EED0 forKeyedSubscript:@"AssertLevel"];
    [v20 setObject:kCFBooleanTrue forKeyedSubscript:@"AppliesToLimitedPower"];
  }
  IOPMAssertionID AssertionID = 0;
  IOReturn v23 = IOPMAssertionCreateWithProperties((CFDictionaryRef)v20, &AssertionID);
  if (v23)
  {
    v24 = sub_10001B6D4();
    if (os_log_type_enabled(v24, OS_LOG_TYPE_DEFAULT))
    {
      v25 = SCErrorString(v23);
      *(_DWORD *)buf = 138412802;
      v32 = v20;
      __int16 v33 = 2080;
      v34 = v25;
      __int16 v35 = 1024;
      LODWORD(v36) = v23;
      _os_log_impl((void *)&_mh_execute_header, v24, OS_LOG_TYPE_DEFAULT, "IOPMAssertionCreateWithProperties %@ failed with error: %s (%d)", buf, 0x1Cu);
    }
  }
  else
  {
    v26 = sub_10001B6D4();
    if (os_log_type_enabled(v26, OS_LOG_TYPE_DEBUG)) {
      sub_1000373F0((uint64_t)v12, &AssertionID, v26);
    }

    if (v18)
    {
      v27 = sub_10001B6D4();
      if (os_log_type_enabled(v27, OS_LOG_TYPE_DEBUG)) {
        sub_100037388();
      }

      [(PowerMgr *)self _disableAssertion:v16 withName:v12];
    }
    v24 = +[NSNumber numberWithUnsignedInt:AssertionID];
    v28 = [(PowerMgr *)self assertionsDict];
    [v28 setObject:v24 forKeyedSubscript:v12];
  }
}

- (void)powerAssertionDisableWithReason:(id)a3
{
  id v4 = a3;
  v5 = +[NSBundle mainBundle];
  v6 = [v5 bundleIdentifier];

  v7 = +[NSString stringWithFormat:@"%@-%@", v6, v4];
  id v8 = [(PowerMgr *)self assertionsDict];
  if ([v8 count])
  {
    v9 = [(PowerMgr *)self assertionsDict];
    v10 = [v9 objectForKeyedSubscript:v7];

    if (v10)
    {
      -[PowerMgr _disableAssertion:withName:](self, "_disableAssertion:withName:", [v10 intValue], v7);
      goto LABEL_8;
    }
  }
  else
  {
  }
  v11 = sub_10001B6D4();
  if (os_log_type_enabled(v11, OS_LOG_TYPE_DEFAULT))
  {
    *(_DWORD *)buf = 138412290;
    v13 = v7;
    _os_log_impl((void *)&_mh_execute_header, v11, OS_LOG_TYPE_DEFAULT, "Did not find any existing assertion with name %@ when trying to disable it", buf, 0xCu);
  }

  v10 = 0;
LABEL_8:
}

- (void)_disableAssertion:(unsigned int)a3 withName:(id)a4
{
  id v6 = a4;
  v7 = [(PowerMgr *)self assertionsDict];
  [v7 removeObjectForKey:v6];

  id v8 = sub_10001B6D4();
  v9 = v8;
  if (a3)
  {
    if (os_log_type_enabled(v8, OS_LOG_TYPE_DEBUG)) {
      sub_10003753C();
    }

    IOReturn v10 = IOPMAssertionRelease(a3);
    v11 = sub_10001B6D4();
    v9 = v11;
    if (v10)
    {
      if (os_log_type_enabled(v11, OS_LOG_TYPE_DEFAULT))
      {
        int v15 = 138413058;
        id v16 = v6;
        __int16 v17 = 1024;
        unsigned int v18 = a3;
        __int16 v19 = 2080;
        v20 = SCErrorString(v10);
        __int16 v21 = 1024;
        IOReturn v22 = v10;
        v12 = "IOPMAssertionRelease failed for assertion %@ with ID 0x%x: %s (%d)";
        v13 = v9;
        uint32_t v14 = 34;
LABEL_9:
        _os_log_impl((void *)&_mh_execute_header, v13, OS_LOG_TYPE_DEFAULT, v12, (uint8_t *)&v15, v14);
      }
    }
    else if (os_log_type_enabled(v11, OS_LOG_TYPE_DEBUG))
    {
      sub_1000374D4();
    }
  }
  else if (os_log_type_enabled(v8, OS_LOG_TYPE_DEFAULT))
  {
    int v15 = 138412290;
    id v16 = v6;
    v12 = "Found Null assertionID for assertion with name %@ when trying to disable it";
    v13 = v9;
    uint32_t v14 = 12;
    goto LABEL_9;
  }
}

- (FMConcurrentMutableDictionary)assertionsDict
{
  return self->_assertionsDict;
}

- (void)setAssertionsDict:(id)a3
{
}

- (void).cxx_destruct
{
}

@end