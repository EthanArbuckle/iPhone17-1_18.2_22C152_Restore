...
+ (void)initialize
{
  if ((id)objc_opt_class() == a1) {
    qword_10006EC40 = (uint64_t)os_log_create("com.apple.SensorKit", "SRLogDataStream");
  }
}

+ (id)sensorDescriptionForSensor:(id)a3
{
  uint64_t v4 = sub_10001A21C();

  return sub_10001A0F8(v4, a3);
}
...
- (SRSensorDescription)initWithDictionary:(id)a3
{
  ...
  if (v4)
  {
    ...
    {
      v6 = qword_10006EC40;
      if (os_log_type_enabled((os_log_t)qword_10006EC40, OS_LOG_TYPE_FAULT))
      {
...
+ (id)sensorDescriptionForDeletionRecordFromDescription:(id)a3
{
  ...
  v8[0] = @"Name";
  v9[0] = [objc_msgSend(a3, "name") sr_sensorForDeletionRecordsFromSensor];
  v8[1] = @"AuthService";
  ...
  id v5 = [[v4 initWithDictionary:+[NSDictionary dictionaryWithObjects:forKeys:count:](NSDictionary, "dictionaryWithObjects:forKeys:count:", v9, v8, 5)];
  if ([a3 legacyName]) {
    [v5 setObject:objc_msgSend(objc_msgSend(a3, "legacyName"), "sr_sensorForDeletionRecordsFromSensor") forKeyedSubscript:@"LegacyName"];
  }
...
- (NSString)localizedAdditionalSampleDataNote
{
  ...
  if (!result)
  {
    ...
    if (v4)
    {
      result = [(NSBundle *)v4 localizedStringForKey:self->_additionalSampleDataNoteKey value:&stru_10005E2D0 table:0];
...
- (BOOL)supportsPlatform:(id)a3
{
  ...
  id v4 = [[-[NSDictionary objectForKeyedSubscript:](supportedPlatforms, "objectForKeyedSubscript:") objectForKeyedSubscript:@"Enabled"];
