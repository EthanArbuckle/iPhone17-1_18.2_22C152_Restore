...
- (GKDelayedDeletePhotoCacheWriter)initWithDatabaseConnection:(id)a3;
...
- (void)bindParametersForPhotoStatement:(sqlite3_stmt *)a3 requestsRowID:(int64_t)a4;
...
- (GKDelayedDeletePhotoCacheWriter)initWithDatabaseConnection:(id)a3
{
  id v4 = a3;
  v8.receiver = self;
  v8.super_class = (Class)GKDelayedDeletePhotoCacheWriter;
  id v5 = [(GKDelayedDeletePhotoCacheWriter *)&v8 init];
  v6 = v5;
  if (v5) {
    [(GKDelayedDeletePhotoCacheWriter *)v5 setConnection:v4];
  }
...
- (id)photosInsertSQL
{
  return +[NSString stringWithFormat:](NSString, "stringWithFormat:", @"INSERT INTO delete_photos(           request_id)             VALUES (%s);",
           ":request_id");
}
...
- (id)getOldPhotosSQL
{
  return +[NSString stringWithFormat:](NSString, "stringWithFormat:", @"            SELECT request_id             FROM requests             INNER JOIN delete_photos USING (request_id)             WHERE player_gcid = %s;",
           ":player_gcid");
}
