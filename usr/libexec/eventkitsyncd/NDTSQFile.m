...
- (NDTSQFile)initWithPath:(id)a3
{
  return [(NDTSQFile *)self initWithPath:a3 queue:0];
}

- (NDTSQFile)initWithPath:(id)a3 queue:(id)a4
{
  return [(NDTSQFile *)self initWithPath:a3 queue:a4 readOnly:0];
}
...
- (NDTSQFile)initWithPath:(id)a3 queue:(id)a4 readOnly:(BOOL)a5
{
  ...
  v19.receiver = self;
  v19.super_class = (Class)NDTSQFile;
  v11 = [(NDTSQFile *)&v19 init];
  v12 = v11;
  if (v11)
  {
    objc_storeStrong((id *)&v11->_pathTemplate, a3);
    v13 = (NSHashTable *)[objc_alloc((Class)NSHashTable) initWithOptions:5 capacity:4];
    ...
    if (!v10)
    {
      id v15 = +[NSString stringWithFormat:@"NDTSQFile.%@.%p", v9, v12];
      v16 = (const char *)[v15 UTF8String];
      v17 = dispatch_queue_attr_make_with_autorelease_frequency(0, DISPATCH_AUTORELEASE_FREQUENCY_WORK_ITEM);
      dispatch_queue_t v10 = dispatch_queue_create(v16, v17);
    }
    objc_storeStrong((id *)&v12->_q, v10);
...
- (void)_newPath
{
  ...
  v5 = [NSString stringWithFormat:@"%04d%02d%02d-%02d%02d%02d", [v4 year], [v4 month], [v4 day], [v4 hour], [v4 minute], [v4 second]];
...
- (void)_openDatabase
{
  ...
  id v4 = [(NDTSQFile *)self path];
  int v5 = sqlite3_open_v2((const char *)[v4 UTF8String], &self->_db, v3, 0);

  if (v5)
  {
    [(NDTSQFile *)self _makeSqlErrorReasonString:@"_openDatabase(1)"];
    id v6 = +[NDTSQException exceptionWithName:@"NDTSQError" reason:objc_claimAutoreleasedReturnValue() userInfo:0];
...
- (void)_prepDatabase
{
  ...
  if (![(NDTSQFile *)self readOnly])
  {
    int v3 = [[NDTSQKeyValueStore alloc] initWithName:@"_SQ_Versions"];
    instanceVersions = self->_instanceVersions;
    self->_instanceVersions = v3;

    int v5 = self->_instanceVersions;
...
- (void)_watchWalFile
{
  ...
  int v3 = [(NDTSQFile *)self path];
  id v4 = +[NSString stringWithFormat:@"%@-wal", v3];
  int v5 = open((const char *)[v4 UTF8String], 0x8000);

  if (v5 != -1)
  {
    id v6 = (OS_dispatch_source *)dispatch_source_create((dispatch_source_type_t)&_dispatch_source_type_vnode, v5, 2uLL, (dispatch_queue_t)self->_q);
