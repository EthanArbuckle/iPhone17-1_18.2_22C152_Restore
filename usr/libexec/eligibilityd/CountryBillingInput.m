@interface CountryBillingInput
+ (BOOL)supportsSecureCoding;
- (BOOL)isEqual:(id)a3;
- (CountryBillingInput)initWithBillingCountry:(id)a3 status:(unint64_t)a4 process:(id)a5;
- (CountryBillingInput)initWithCoder:(id)a3;
- (NSString)countryCode;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
- (unint64_t)hash;
- (void)encodeWithCoder:(id)a3;
- (void)setCountryCode:(id)a3;
@end

@implementation CountryBillingInput

- (void).cxx_destruct
{
}

- (void)setCountryCode:(id)a3
{
}

- (NSString)countryCode
{
  return self->_countryCode;
}

- (id)description
{
  v3 = [(CountryBillingInput *)self countryCode];
  v7.receiver = self;
  v7.super_class = (Class)CountryBillingInput;
  v4 = [(EligibilityInput *)&v7 description];
  v5 = +[NSString stringWithFormat:@"[CountryBillingInput countryCode:\"%@\" %@]", v3, v4];

  return v5;
}

- (BOOL)isEqual:(id)a3
{
  v4 = (CountryBillingInput *)a3;
  v11.receiver = self;
  v11.super_class = (Class)CountryBillingInput;
  if (![(EligibilityInput *)&v11 isEqual:v4]) {
    goto LABEL_9;
  }
  if (v4 == self)
  {
    char v8 = 1;
    goto LABEL_11;
  }
  objc_opt_class();
  if (objc_opt_isKindOfClass())
  {
    v5 = v4;
    v6 = [(CountryBillingInput *)self countryCode];
    objc_super v7 = [(CountryBillingInput *)v5 countryCode];
    char v8 = sub_100019BF8(v6, v7);

    if ((v8 & 1) == 0)
    {
      v9 = sub_10000D0EC();
      if (os_log_type_enabled(v9, OS_LOG_TYPE_DEFAULT))
      {
        *(_DWORD *)buf = 136315394;
        v13 = "-[CountryBillingInput isEqual:]";
        __int16 v14 = 2080;
        v15 = "countryCode";
        _os_log_impl((void *)&_mh_execute_header, v9, OS_LOG_TYPE_DEFAULT, "%s: Property %s did not match", buf, 0x16u);
      }
    }
  }
  else
  {
LABEL_9:
    char v8 = 0;
  }
LABEL_11:

  return v8;
}

- (unint64_t)hash
{
  v7.receiver = self;
  v7.super_class = (Class)CountryBillingInput;
  unint64_t v3 = [(EligibilityInput *)&v7 hash];
  v4 = [(CountryBillingInput *)self countryCode];
  unint64_t v5 = (unint64_t)[v4 hash] ^ v3;

  return v5;
}

- (id)copyWithZone:(_NSZone *)a3
{
  v9.receiver = self;
  v9.super_class = (Class)CountryBillingInput;
  id v5 = -[EligibilityInput copyWithZone:](&v9, "copyWithZone:");
  v6 = [(CountryBillingInput *)self countryCode];
  id v7 = [v6 copyWithZone:a3];
  [v5 setCountryCode:v7];

  return v5;
}

- (CountryBillingInput)initWithCoder:(id)a3
{
  id v4 = a3;
  v9.receiver = self;
  v9.super_class = (Class)CountryBillingInput;
  id v5 = [(EligibilityInput *)&v9 initWithCoder:v4];
  if (v5)
  {
    uint64_t v6 = [v4 decodeObjectOfClass:objc_opt_class() forKey:@"countryCode"];
    countryCode = v5->_countryCode;
    v5->_countryCode = (NSString *)v6;
  }
  return v5;
}

- (void)encodeWithCoder:(id)a3
{
  v6.receiver = self;
  v6.super_class = (Class)CountryBillingInput;
  id v4 = a3;
  [(EligibilityInput *)&v6 encodeWithCoder:v4];
  id v5 = [(CountryBillingInput *)self countryCode];
  [v4 encodeObject:v5 forKey:@"countryCode"];
}

- (CountryBillingInput)initWithBillingCountry:(id)a3 status:(unint64_t)a4 process:(id)a5
{
  id v8 = a3;
  id v9 = a5;
  if (v8)
  {
    xpc_type_t type = xpc_get_type(v8);
    if (type != (xpc_type_t)&_xpc_type_string)
    {
      objc_super v11 = type;
      v12 = sub_10000D0EC();
      if (os_log_type_enabled(v12, OS_LOG_TYPE_ERROR))
      {
        *(_DWORD *)buf = 136315394;
        v19 = "-[CountryBillingInput initWithBillingCountry:status:process:]";
        __int16 v20 = 2080;
        name = xpc_type_get_name(v11);
        _os_log_error_impl((void *)&_mh_execute_header, v12, OS_LOG_TYPE_ERROR, "%s: Billing country code input is wrong data type: %s", buf, 0x16u);
      }
      v13 = 0;
      goto LABEL_11;
    }
    v12 = _CFXPCCreateCFObjectFromXPCObject();
  }
  else
  {
    v12 = 0;
  }
  v17.receiver = self;
  v17.super_class = (Class)CountryBillingInput;
  __int16 v14 = [(EligibilityInput *)&v17 initWithInputType:2 status:a4 process:v9];
  v15 = v14;
  if (v14) {
    objc_storeStrong((id *)&v14->_countryCode, v12);
  }
  self = v15;
  v13 = self;
LABEL_11:

  return v13;
}

+ (BOOL)supportsSecureCoding
{
  return 1;
}

@end