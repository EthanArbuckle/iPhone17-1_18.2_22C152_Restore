...
- (void)loadContentView
{
  ...
  double v11 = [(UIImageView *)[objc_alloc((Class)UIImageView) initWithFrame:CGRectZero.origin.x, y, width, height]];
...
- (void)loadExpandedViewsIfNecessary
{
  ...
  objc_super v3 = &selRef_transcriptBottomConstraint;
  if (self->_voicemailTipView)
  {
    ...
    objc_super v8 = [v4 constraintEqualToAnchor:v7];
...
LABEL_8:
  ...
  if (!self->_playerControlsView)
  {
    ...
    [*(id *)&self->TPTableViewCell_opaque[*((int *)v3 + 962)] addSubview:self->_playerControlsView];
  }
  if (!self->_toolbar)
  {
    ...
    [*(id *)&self->TPTableViewCell_opaque[*((int *)v3 + 962)] addSubview:self->_toolbar];
  }
  if (!self->_transcriptView)
  {
    ...
    [*(id *)&self->TPTableViewCell_opaque[*((int *)v3 + 962)] addSubview:self->_transcriptView];
  }
  if (!self->_restrictedView)
  {
    ...
    [*(id *)&self->TPTableViewCell_opaque[*((int *)v3 + 962)] addSubview:self->_restrictedView];
  }
}
...
- (void)setRead:(BOOL)a3
{
  BOOL v3 = a3;
  self->_read = a3;
  BOOL v4 = [(PHVoicemailMessageTableViewCell *)self unreadIndicatorImageView];
  id v6 = v4;
  double v5 = 1.0;
  if (v3) {
    double v5 = 0.0;
