@interface CellularUmtsRrcStateHist
+ (Class)stateHistType;
- (BOOL)hasNumSubs;
- (BOOL)hasPlmn;
- (BOOL)hasPsPref;
- (BOOL)hasSubsId;
- (BOOL)hasTimestamp;
- (BOOL)hasTotalDurationMs;
- (BOOL)hasWcdma;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (BOOL)wcdma;
- (NSData)plmn;
- (NSMutableArray)stateHists;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
- (id)dictionaryRepresentation;
- (id)stateHistAtIndex:(unint64_t)a3;
- (unint64_t)hash;
- (unint64_t)stateHistsCount;
- (unint64_t)timestamp;
- (unsigned)numSubs;
- (unsigned)psPref;
- (unsigned)subsId;
- (unsigned)totalDurationMs;
- (void)addStateHist:(id)a3;
- (void)clearStateHists;
- (void)copyTo:(id)a3;
- (void)mergeFrom:(id)a3;
- (void)setHasNumSubs:(BOOL)a3;
- (void)setHasPsPref:(BOOL)a3;
- (void)setHasSubsId:(BOOL)a3;
- (void)setHasTimestamp:(BOOL)a3;
- (void)setHasTotalDurationMs:(BOOL)a3;
- (void)setHasWcdma:(BOOL)a3;
- (void)setNumSubs:(unsigned int)a3;
- (void)setPlmn:(id)a3;
- (void)setPsPref:(unsigned int)a3;
- (void)setStateHists:(id)a3;
- (void)setSubsId:(unsigned int)a3;
- (void)setTimestamp:(unint64_t)a3;
- (void)setTotalDurationMs:(unsigned int)a3;
- (void)setWcdma:(BOOL)a3;
- (void)writeTo:(id)a3;
@end

@implementation CellularUmtsRrcStateHist

- (void)setTimestamp:(unint64_t)a3
{
  *(unsigned char *)&self->_has |= 1u;
  self->_timestamp = a3;
}

- (void)setHasTimestamp:(BOOL)a3
{
  *(unsigned char *)&self->_has = *(unsigned char *)&self->_has & 0xFE | a3;
}

- (BOOL)hasTimestamp
{
  return *(unsigned char *)&self->_has & 1;
}

- (void)setTotalDurationMs:(unsigned int)a3
{
  *(unsigned char *)&self->_has |= 0x10u;
  self->_totalDurationMs = a3;
}

- (void)setHasTotalDurationMs:(BOOL)a3
{
  if (a3) {
    char v3 = 16;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_has = *(unsigned char *)&self->_has & 0xEF | v3;
}

- (BOOL)hasTotalDurationMs
{
  return (*(unsigned char *)&self->_has >> 4) & 1;
}

- (void)setWcdma:(BOOL)a3
{
  *(unsigned char *)&self->_has |= 0x20u;
  self->_wcdma = a3;
}

- (void)setHasWcdma:(BOOL)a3
{
  if (a3) {
    char v3 = 32;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_has = *(unsigned char *)&self->_has & 0xDF | v3;
}

- (BOOL)hasWcdma
{
  return (*(unsigned char *)&self->_has >> 5) & 1;
}

- (void)clearStateHists
{
}

- (void)addStateHist:(id)a3
{
  id v4 = a3;
  stateHists = self->_stateHists;
  id v8 = v4;
  if (!stateHists)
  {
    v6 = (NSMutableArray *)objc_alloc_init((Class)NSMutableArray);
    v7 = self->_stateHists;
    self->_stateHists = v6;

    id v4 = v8;
    stateHists = self->_stateHists;
  }
  [(NSMutableArray *)stateHists addObject:v4];
}

- (unint64_t)stateHistsCount
{
  return (unint64_t)[(NSMutableArray *)self->_stateHists count];
}

- (id)stateHistAtIndex:(unint64_t)a3
{
  return [(NSMutableArray *)self->_stateHists objectAtIndex:a3];
}

+ (Class)stateHistType
{
  return (Class)objc_opt_class();
}

- (void)setNumSubs:(unsigned int)a3
{
  *(unsigned char *)&self->_has |= 2u;
  self->_numSubs = a3;
}

- (void)setHasNumSubs:(BOOL)a3
{
  if (a3) {
    char v3 = 2;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_has = *(unsigned char *)&self->_has & 0xFD | v3;
}

- (BOOL)hasNumSubs
{
  return (*(unsigned char *)&self->_has >> 1) & 1;
}

- (BOOL)hasPlmn
{
  return self->_plmn != 0;
}

- (void)setSubsId:(unsigned int)a3
{
  *(unsigned char *)&self->_has |= 8u;
  self->_subsId = a3;
}

- (void)setHasSubsId:(BOOL)a3
{
  if (a3) {
    char v3 = 8;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_has = *(unsigned char *)&self->_has & 0xF7 | v3;
}

- (BOOL)hasSubsId
{
  return (*(unsigned char *)&self->_has >> 3) & 1;
}

- (void)setPsPref:(unsigned int)a3
{
  *(unsigned char *)&self->_has |= 4u;
  self->_psPref = a3;
}

- (void)setHasPsPref:(BOOL)a3
{
  if (a3) {
    char v3 = 4;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_has = *(unsigned char *)&self->_has & 0xFB | v3;
}

- (BOOL)hasPsPref
{
  return (*(unsigned char *)&self->_has >> 2) & 1;
}

- (id)description
{
  v7.receiver = self;
  v7.super_class = (Class)CellularUmtsRrcStateHist;
  char v3 = [(CellularUmtsRrcStateHist *)&v7 description];
  id v4 = [(CellularUmtsRrcStateHist *)self dictionaryRepresentation];
  v5 = +[NSString stringWithFormat:@"%@ %@", v3, v4];

  return v5;
}

- (id)dictionaryRepresentation
{
  char v3 = +[NSMutableDictionary dictionary];
  char has = (char)self->_has;
  if (has)
  {
    v19 = +[NSNumber numberWithUnsignedLongLong:self->_timestamp];
    [v3 setObject:v19 forKey:@"timestamp"];

    char has = (char)self->_has;
    if ((has & 0x10) == 0)
    {
LABEL_3:
      if ((has & 0x20) == 0) {
        goto LABEL_5;
      }
      goto LABEL_4;
    }
  }
  else if ((*(unsigned char *)&self->_has & 0x10) == 0)
  {
    goto LABEL_3;
  }
  v20 = +[NSNumber numberWithUnsignedInt:self->_totalDurationMs];
  [v3 setObject:v20 forKey:@"total_duration_ms"];

  if ((*(unsigned char *)&self->_has & 0x20) != 0)
  {
LABEL_4:
    v5 = +[NSNumber numberWithBool:self->_wcdma];
    [v3 setObject:v5 forKey:@"wcdma"];
  }
LABEL_5:
  if ([(NSMutableArray *)self->_stateHists count])
  {
    id v6 = [objc_alloc((Class)NSMutableArray) initWithCapacity:-[NSMutableArray count](self->_stateHists, "count")];
    long long v21 = 0u;
    long long v22 = 0u;
    long long v23 = 0u;
    long long v24 = 0u;
    objc_super v7 = self->_stateHists;
    id v8 = [(NSMutableArray *)v7 countByEnumeratingWithState:&v21 objects:v25 count:16];
    if (v8)
    {
      id v9 = v8;
      uint64_t v10 = *(void *)v22;
      do
      {
        for (i = 0; i != v9; i = (char *)i + 1)
        {
          if (*(void *)v22 != v10) {
            objc_enumerationMutation(v7);
          }
          v12 = [*(id *)(*((void *)&v21 + 1) + 8 * i) dictionaryRepresentation];
          [v6 addObject:v12];
        }
        id v9 = [(NSMutableArray *)v7 countByEnumeratingWithState:&v21 objects:v25 count:16];
      }
      while (v9);
    }

    [v3 setObject:v6 forKey:@"state_hist"];
  }
  if ((*(unsigned char *)&self->_has & 2) != 0)
  {
    v13 = +[NSNumber numberWithUnsignedInt:self->_numSubs];
    [v3 setObject:v13 forKey:@"num_subs"];
  }
  plmn = self->_plmn;
  if (plmn) {
    [v3 setObject:plmn forKey:@"plmn"];
  }
  char v15 = (char)self->_has;
  if ((v15 & 8) != 0)
  {
    v16 = +[NSNumber numberWithUnsignedInt:self->_subsId];
    [v3 setObject:v16 forKey:@"subs_id"];

    char v15 = (char)self->_has;
  }
  if ((v15 & 4) != 0)
  {
    v17 = +[NSNumber numberWithUnsignedInt:self->_psPref];
    [v3 setObject:v17 forKey:@"ps_pref"];
  }

  return v3;
}

- (BOOL)readFrom:(id)a3
{
  return sub_10006FEEC((uint64_t)self, (uint64_t)a3);
}

- (void)writeTo:(id)a3
{
  id v4 = a3;
  char has = (char)self->_has;
  if (has)
  {
    PBDataWriterWriteUint64Field();
    char has = (char)self->_has;
    if ((has & 0x10) == 0)
    {
LABEL_3:
      if ((has & 0x20) == 0) {
        goto LABEL_5;
      }
      goto LABEL_4;
    }
  }
  else if ((*(unsigned char *)&self->_has & 0x10) == 0)
  {
    goto LABEL_3;
  }
  PBDataWriterWriteUint32Field();
  if ((*(unsigned char *)&self->_has & 0x20) != 0) {
LABEL_4:
  }
    PBDataWriterWriteBOOLField();
LABEL_5:
  if (self->_plmn) {
    PBDataWriterWriteDataField();
  }
  long long v14 = 0u;
  long long v15 = 0u;
  long long v12 = 0u;
  long long v13 = 0u;
  id v6 = self->_stateHists;
  objc_super v7 = (char *)[(NSMutableArray *)v6 countByEnumeratingWithState:&v12 objects:v16 count:16];
  if (v7)
  {
    id v8 = v7;
    uint64_t v9 = *(void *)v13;
    do
    {
      for (i = 0; i != v8; ++i)
      {
        if (*(void *)v13 != v9) {
          objc_enumerationMutation(v6);
        }
        PBDataWriterWriteSubmessage();
      }
      id v8 = (char *)[(NSMutableArray *)v6 countByEnumeratingWithState:&v12 objects:v16 count:16];
    }
    while (v8);
  }

  char v11 = (char)self->_has;
  if ((v11 & 2) == 0)
  {
    if ((*(unsigned char *)&self->_has & 8) == 0) {
      goto LABEL_16;
    }
LABEL_23:
    PBDataWriterWriteUint32Field();
    if ((*(unsigned char *)&self->_has & 4) == 0) {
      goto LABEL_18;
    }
    goto LABEL_17;
  }
  PBDataWriterWriteUint32Field();
  char v11 = (char)self->_has;
  if ((v11 & 8) != 0) {
    goto LABEL_23;
  }
LABEL_16:
  if ((v11 & 4) != 0) {
LABEL_17:
  }
    PBDataWriterWriteUint32Field();
LABEL_18:
}

- (void)copyTo:(id)a3
{
  id v4 = a3;
  char has = (char)self->_has;
  if (has)
  {
    v4[1] = self->_timestamp;
    *((unsigned char *)v4 + 60) |= 1u;
    char has = (char)self->_has;
    if ((has & 0x10) == 0)
    {
LABEL_3:
      if ((has & 0x20) == 0) {
        goto LABEL_5;
      }
      goto LABEL_4;
    }
  }
  else if ((*(unsigned char *)&self->_has & 0x10) == 0)
  {
    goto LABEL_3;
  }
  *((_DWORD *)v4 + 13) = self->_totalDurationMs;
  *((unsigned char *)v4 + 60) |= 0x10u;
  if ((*(unsigned char *)&self->_has & 0x20) != 0)
  {
LABEL_4:
    *((unsigned char *)v4 + 56) = self->_wcdma;
    *((unsigned char *)v4 + 60) |= 0x20u;
  }
LABEL_5:
  id v11 = v4;
  if (self->_plmn) {
    [v4 setPlmn:];
  }
  if ([(CellularUmtsRrcStateHist *)self stateHistsCount])
  {
    [v11 clearStateHists];
    unint64_t v6 = [(CellularUmtsRrcStateHist *)self stateHistsCount];
    if (v6)
    {
      unint64_t v7 = v6;
      for (uint64_t i = 0; i != v7; ++i)
      {
        uint64_t v9 = [(CellularUmtsRrcStateHist *)self stateHistAtIndex:i];
        [v11 addStateHist:v9];
      }
    }
  }
  char v10 = (char)self->_has;
  if ((v10 & 2) != 0)
  {
    *((_DWORD *)v11 + 4) = self->_numSubs;
    *((unsigned char *)v11 + 60) |= 2u;
    char v10 = (char)self->_has;
    if ((v10 & 8) == 0)
    {
LABEL_13:
      if ((v10 & 4) == 0) {
        goto LABEL_15;
      }
      goto LABEL_14;
    }
  }
  else if ((*(unsigned char *)&self->_has & 8) == 0)
  {
    goto LABEL_13;
  }
  *((_DWORD *)v11 + 12) = self->_subsId;
  *((unsigned char *)v11 + 60) |= 8u;
  if ((*(unsigned char *)&self->_has & 4) != 0)
  {
LABEL_14:
    *((_DWORD *)v11 + 8) = self->_psPref;
    *((unsigned char *)v11 + 60) |= 4u;
  }
LABEL_15:
}

- (id)copyWithZone:(_NSZone *)a3
{
  v5 = objc_msgSend(objc_msgSend((id)objc_opt_class(), "allocWithZone:", a3), "init");
  unint64_t v6 = v5;
  char has = (char)self->_has;
  if (has)
  {
    v5[1] = self->_timestamp;
    *((unsigned char *)v5 + 60) |= 1u;
    char has = (char)self->_has;
    if ((has & 0x10) == 0)
    {
LABEL_3:
      if ((has & 0x20) == 0) {
        goto LABEL_5;
      }
      goto LABEL_4;
    }
  }
  else if ((*(unsigned char *)&self->_has & 0x10) == 0)
  {
    goto LABEL_3;
  }
  *((_DWORD *)v5 + 13) = self->_totalDurationMs;
  *((unsigned char *)v5 + 60) |= 0x10u;
  if ((*(unsigned char *)&self->_has & 0x20) != 0)
  {
LABEL_4:
    *((unsigned char *)v5 + 56) = self->_wcdma;
    *((unsigned char *)v5 + 60) |= 0x20u;
  }
LABEL_5:
  id v8 = [(NSData *)self->_plmn copyWithZone:a3];
  uint64_t v9 = (void *)v6[3];
  v6[3] = v8;

  long long v20 = 0u;
  long long v21 = 0u;
  long long v18 = 0u;
  long long v19 = 0u;
  char v10 = self->_stateHists;
  id v11 = [(NSMutableArray *)v10 countByEnumeratingWithState:&v18 objects:v22 count:16];
  if (v11)
  {
    id v12 = v11;
    uint64_t v13 = *(void *)v19;
    do
    {
      for (uint64_t i = 0; i != v12; uint64_t i = (char *)i + 1)
      {
        if (*(void *)v19 != v13) {
          objc_enumerationMutation(v10);
        }
        id v15 = [*(id *)(*((void *)&v18 + 1) + 8 * i) copyWithZone:a3];
        [v6 addStateHist:v15];
      }
      id v12 = [(NSMutableArray *)v10 countByEnumeratingWithState:&v18 objects:v22 count:16];
    }
    while (v12);
  }

  char v16 = (char)self->_has;
  if ((v16 & 2) == 0)
  {
    if ((*(unsigned char *)&self->_has & 8) == 0) {
      goto LABEL_14;
    }
LABEL_21:
    *((_DWORD *)v6 + 12) = self->_subsId;
    *((unsigned char *)v6 + 60) |= 8u;
    if ((*(unsigned char *)&self->_has & 4) == 0) {
      return v6;
    }
    goto LABEL_15;
  }
  *((_DWORD *)v6 + 4) = self->_numSubs;
  *((unsigned char *)v6 + 60) |= 2u;
  char v16 = (char)self->_has;
  if ((v16 & 8) != 0) {
    goto LABEL_21;
  }
LABEL_14:
  if ((v16 & 4) != 0)
  {
LABEL_15:
    *((_DWORD *)v6 + 8) = self->_psPref;
    *((unsigned char *)v6 + 60) |= 4u;
  }
  return v6;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_38;
  }
  if (*(unsigned char *)&self->_has)
  {
    if ((*((unsigned char *)v4 + 60) & 1) == 0 || self->_timestamp != *((void *)v4 + 1)) {
      goto LABEL_38;
    }
  }
  else if (*((unsigned char *)v4 + 60))
  {
    goto LABEL_38;
  }
  if ((*(unsigned char *)&self->_has & 0x10) != 0)
  {
    if ((*((unsigned char *)v4 + 60) & 0x10) == 0 || self->_totalDurationMs != *((_DWORD *)v4 + 13)) {
      goto LABEL_38;
    }
  }
  else if ((*((unsigned char *)v4 + 60) & 0x10) != 0)
  {
    goto LABEL_38;
  }
  if ((*(unsigned char *)&self->_has & 0x20) != 0)
  {
    if ((*((unsigned char *)v4 + 60) & 0x20) != 0)
    {
      if (self->_wcdma)
      {
        if (!*((unsigned char *)v4 + 56)) {
          goto LABEL_38;
        }
        goto LABEL_20;
      }
      if (!*((unsigned char *)v4 + 56)) {
        goto LABEL_20;
      }
    }
LABEL_38:
    BOOL v7 = 0;
    goto LABEL_39;
  }
  if ((*((unsigned char *)v4 + 60) & 0x20) != 0) {
    goto LABEL_38;
  }
LABEL_20:
  plmn = self->_plmn;
  if ((unint64_t)plmn | *((void *)v4 + 3) && !-[NSData isEqual:](plmn, "isEqual:")) {
    goto LABEL_38;
  }
  stateHists = self->_stateHists;
  if ((unint64_t)stateHists | *((void *)v4 + 5))
  {
    if (!-[NSMutableArray isEqual:](stateHists, "isEqual:")) {
      goto LABEL_38;
    }
  }
  if ((*(unsigned char *)&self->_has & 2) != 0)
  {
    if ((*((unsigned char *)v4 + 60) & 2) == 0 || self->_numSubs != *((_DWORD *)v4 + 4)) {
      goto LABEL_38;
    }
  }
  else if ((*((unsigned char *)v4 + 60) & 2) != 0)
  {
    goto LABEL_38;
  }
  if ((*(unsigned char *)&self->_has & 8) != 0)
  {
    if ((*((unsigned char *)v4 + 60) & 8) == 0 || self->_subsId != *((_DWORD *)v4 + 12)) {
      goto LABEL_38;
    }
  }
  else if ((*((unsigned char *)v4 + 60) & 8) != 0)
  {
    goto LABEL_38;
  }
  BOOL v7 = (*((unsigned char *)v4 + 60) & 4) == 0;
  if ((*(unsigned char *)&self->_has & 4) != 0)
  {
    if ((*((unsigned char *)v4 + 60) & 4) == 0 || self->_psPref != *((_DWORD *)v4 + 8)) {
      goto LABEL_38;
    }
    BOOL v7 = 1;
  }
LABEL_39:

  return v7;
}

- (unint64_t)hash
{
  if ((*(unsigned char *)&self->_has & 1) == 0)
  {
    unint64_t v3 = 0;
    if ((*(unsigned char *)&self->_has & 0x10) != 0) {
      goto LABEL_3;
    }
LABEL_6:
    uint64_t v4 = 0;
    if ((*(unsigned char *)&self->_has & 0x20) != 0) {
      goto LABEL_4;
    }
    goto LABEL_7;
  }
  unint64_t v3 = 2654435761u * self->_timestamp;
  if ((*(unsigned char *)&self->_has & 0x10) == 0) {
    goto LABEL_6;
  }
LABEL_3:
  uint64_t v4 = 2654435761 * self->_totalDurationMs;
  if ((*(unsigned char *)&self->_has & 0x20) != 0)
  {
LABEL_4:
    uint64_t v5 = 2654435761 * self->_wcdma;
    goto LABEL_8;
  }
LABEL_7:
  uint64_t v5 = 0;
LABEL_8:
  unint64_t v6 = (unint64_t)[(NSData *)self->_plmn hash];
  unint64_t v7 = (unint64_t)[(NSMutableArray *)self->_stateHists hash];
  if ((*(unsigned char *)&self->_has & 2) == 0)
  {
    uint64_t v8 = 0;
    if ((*(unsigned char *)&self->_has & 8) != 0) {
      goto LABEL_10;
    }
LABEL_13:
    uint64_t v9 = 0;
    if ((*(unsigned char *)&self->_has & 4) != 0) {
      goto LABEL_11;
    }
LABEL_14:
    uint64_t v10 = 0;
    return v4 ^ v3 ^ v5 ^ v6 ^ v7 ^ v8 ^ v9 ^ v10;
  }
  uint64_t v8 = 2654435761 * self->_numSubs;
  if ((*(unsigned char *)&self->_has & 8) == 0) {
    goto LABEL_13;
  }
LABEL_10:
  uint64_t v9 = 2654435761 * self->_subsId;
  if ((*(unsigned char *)&self->_has & 4) == 0) {
    goto LABEL_14;
  }
LABEL_11:
  uint64_t v10 = 2654435761 * self->_psPref;
  return v4 ^ v3 ^ v5 ^ v6 ^ v7 ^ v8 ^ v9 ^ v10;
}

- (void)mergeFrom:(id)a3
{
  id v4 = a3;
  id v5 = v4;
  char v6 = *((unsigned char *)v4 + 60);
  if (v6)
  {
    self->_timestamp = *((void *)v4 + 1);
    *(unsigned char *)&self->_has |= 1u;
    char v6 = *((unsigned char *)v4 + 60);
    if ((v6 & 0x10) == 0)
    {
LABEL_3:
      if ((v6 & 0x20) == 0) {
        goto LABEL_5;
      }
      goto LABEL_4;
    }
  }
  else if ((*((unsigned char *)v4 + 60) & 0x10) == 0)
  {
    goto LABEL_3;
  }
  self->_totalDurationMs = *((_DWORD *)v4 + 13);
  *(unsigned char *)&self->_has |= 0x10u;
  if ((*((unsigned char *)v4 + 60) & 0x20) != 0)
  {
LABEL_4:
    self->_wcdma = *((unsigned char *)v4 + 56);
    *(unsigned char *)&self->_has |= 0x20u;
  }
LABEL_5:
  if (*((void *)v4 + 3)) {
    -[CellularUmtsRrcStateHist setPlmn:](self, "setPlmn:");
  }
  long long v15 = 0u;
  long long v16 = 0u;
  long long v13 = 0u;
  long long v14 = 0u;
  id v7 = *((id *)v5 + 5);
  id v8 = [v7 countByEnumeratingWithState:&v13 objects:v17 count:16];
  if (v8)
  {
    id v9 = v8;
    uint64_t v10 = *(void *)v14;
    do
    {
      for (uint64_t i = 0; i != v9; uint64_t i = (char *)i + 1)
      {
        if (*(void *)v14 != v10) {
          objc_enumerationMutation(v7);
        }
        -[CellularUmtsRrcStateHist addStateHist:](self, "addStateHist:", *(void *)(*((void *)&v13 + 1) + 8 * i), (void)v13);
      }
      id v9 = [v7 countByEnumeratingWithState:&v13 objects:v17 count:16];
    }
    while (v9);
  }

  char v12 = *((unsigned char *)v5 + 60);
  if ((v12 & 2) == 0)
  {
    if ((*((unsigned char *)v5 + 60) & 8) == 0) {
      goto LABEL_16;
    }
LABEL_23:
    self->_subsId = *((_DWORD *)v5 + 12);
    *(unsigned char *)&self->_has |= 8u;
    if ((*((unsigned char *)v5 + 60) & 4) == 0) {
      goto LABEL_18;
    }
    goto LABEL_17;
  }
  self->_numSubs = *((_DWORD *)v5 + 4);
  *(unsigned char *)&self->_has |= 2u;
  char v12 = *((unsigned char *)v5 + 60);
  if ((v12 & 8) != 0) {
    goto LABEL_23;
  }
LABEL_16:
  if ((v12 & 4) != 0)
  {
LABEL_17:
    self->_psPref = *((_DWORD *)v5 + 8);
    *(unsigned char *)&self->_has |= 4u;
  }
LABEL_18:
}

- (unint64_t)timestamp
{
  return self->_timestamp;
}

- (unsigned)totalDurationMs
{
  return self->_totalDurationMs;
}

- (BOOL)wcdma
{
  return self->_wcdma;
}

- (NSMutableArray)stateHists
{
  return self->_stateHists;
}

- (void)setStateHists:(id)a3
{
}

- (unsigned)numSubs
{
  return self->_numSubs;
}

- (NSData)plmn
{
  return self->_plmn;
}

- (void)setPlmn:(id)a3
{
}

- (unsigned)subsId
{
  return self->_subsId;
}

- (unsigned)psPref
{
  return self->_psPref;
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_stateHists, 0);

  objc_storeStrong((id *)&self->_plmn, 0);
}

@end