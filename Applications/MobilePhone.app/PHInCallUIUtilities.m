...
+ (id)sharedInstance
{
  ...
  v2 = (void *)sharedInstance_sInCallUIUtilities;
...
uint64_t __37__PHInCallUIUtilities_sharedInstance__block_invoke(uint64_t a1)
{
  ...
  uint64_t v2 = sharedInstance_sInCallUIUtilities;
  sharedInstance_sInCallUIUtilities = (uint64_t)v1;
...
- (PHInCallUIUtilities)init
{
  ...
  uint64_t v2 = [(PHInCallUIUtilities *)&v9 init];
  if (v2)
  {
    ...
    else
    {
      ...
      v7 = (unsigned int (*)(void))getSBSGetScreenLockStatusSymbolLoc_ptr;
      v14 = getSBSGetScreenLockStatusSymbolLoc_ptr;
      if (!getSBSGetScreenLockStatusSymbolLoc_ptr)
      {
        v10[0] = _NSConcreteStackBlock;
        v10[1] = 3221225472;
        v10[2] = __getSBSGetScreenLockStatusSymbolLoc_block_invoke;
        v10[3] = &unk_10027C860;
        v10[4] = &v11;
        __getSBSGetScreenLockStatusSymbolLoc_block_invoke((uint64_t)v10);
...
+ (id)telephonyClient
{
  if (telephonyClient_onceToken != -1) {
    dispatch_once(&telephonyClient_onceToken, &__block_literal_global_87);
  }
  uint64_t v2 = (void *)telephonyClient_telephonyClient;
...
void __38__PHInCallUIUtilities_telephonyClient__block_invoke(id a1)
{
  ...
  uint64_t v2 = telephonyClient_telephonyClient;
  telephonyClient_telephonyClient = (uint64_t)v1;
...
+ (BOOL)hasMultipleSenderIdentities
{
  if (hasMultipleSenderIdentities_onceToken != -1) {
    dispatch_once(&hasMultipleSenderIdentities_onceToken, &__block_literal_global_100);
  }
  uint64_t v2 = [(id)hasMultipleSenderIdentities_telephonyProvider prioritizedSenderIdentities];
...
void __50__PHInCallUIUtilities_hasMultipleSenderIdentities__block_invoke(id a1)
{
  ...
  id v4 = (void *)hasMultipleSenderIdentities_telephonyProvider;
...
+ (id)fbsOrientationObserver
{
  if (fbsOrientationObserver_onceToken != -1) {
    dispatch_once(&fbsOrientationObserver_onceToken, &__block_literal_global_103);
  }
  uint64_t v2 = (void *)fbsOrientationObserver_sFBSOrientationObserver;
...
void __45__PHInCallUIUtilities_fbsOrientationObserver__block_invoke(id a1)
{
  ...
  uint64_t v2 = fbsOrientationObserver_sFBSOrientationObserver;
  fbsOrientationObserver_sFBSOrientationObserver = (uint64_t)v1;
    ...
    for (i = 0; i != v15; i = (char *)i + 1)
    {
      ...
      if (([v19 isSimHidden] & 1) == 0)
      {
        ...
        if (v22)
        {
          ...
          id v33 = v16;
          id v25 = [v24 copyCarrierBundleValue:v19 keyHierarchy:&off_10028CC98 bundleType:v23 error:&v33];
          id v7 = v33;
...
+ (int64_t)frontboardOrientation
{
  uint64_t v2 = [a1 fbsOrientationObserver];
  uint64_t v3 = (char *)[v2 activeInterfaceOrientation];
  if ((unint64_t)(v3 - 1) > 3) {
    int64_t v4 = 0;
  }
  else {
    int64_t v4 = qword_100203B58[(void)(v3 - 1)];
  }
...
- (void)init
{
  ...
  id v1 = +[NSString stringWithUTF8String:"Boolean PHSBSGetScreenLockStatus(Boolean *)"];
...
{
  OUTLINED_FUNCTION_1();
  OUTLINED_FUNCTION_0((void *)&_mh_execute_header, v0, v1, "Received subscriptionInfo: %@ with no subscriptions", v2, v3, v4, v5, v6);
}

+ (void)shouldRequestPasscodeUnlockForMMICode:.cold.2()
{
  OUTLINED_FUNCTION_1();
  OUTLINED_FUNCTION_0((void *)&_mh_execute_header, v0, v1, "Received error: %@ while querying -getSubscriptionInfoWithError", v2, v3, v4, v5, v6);
}

+ (void)isExplicitTransferSupportedForSubscriptionLabelIdentifier:.cold.1()
{
  OUTLINED_FUNCTION_1();
  OUTLINED_FUNCTION_0((void *)&_mh_execute_header, v0, v1, "ShowExplicitCallTransferButton: received subscriptionInfo: %@ with no subscriptions", v2, v3, v4, v5, v6);
}

+ (void)isExplicitTransferSupportedForSubscriptionLabelIdentifier:.cold.2()
{
  OUTLINED_FUNCTION_1();
  OUTLINED_FUNCTION_0((void *)&_mh_execute_header, v0, v1, "ShowExplicitCallTransferButton: received error: %@ while querying -getSubscriptionInfoWithError", v2, v3, v4, v5, v6);
}
