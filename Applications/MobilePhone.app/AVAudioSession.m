...
- (BOOL)activateVoicemailAudioSession
{
  ...
  if (![(AVAudioSession *)self isCategoryVoicemail])
  {
    ...
    if ((v6 & 1) == 0)
    {
      ...
      if (os_log_type_enabled(v8, OS_LOG_TYPE_ERROR)) {
        [(AVAudioSession(VoicemailUI) *)v5 activateVoicemailAudioSession];
      }
    }
  }
  if (![(AVAudioSession *)self isCategoryVoicemail]) {
  ...
  if (!v9)
  {
    ...
    if (os_log_type_enabled(v11, OS_LOG_TYPE_ERROR)) {
      [(AVAudioSession(VoicemailUI) *)(uint64_t)v10 activateVoicemailAudioSession];
    }
  }
  return v9;
}
...
- (BOOL)deactivateVoicemailAudioSession
{
  ...
  if ((v6 & 1) == 0)
  {
    ...
    {
      [(AVAudioSession(VoicemailUI) *)(uint64_t)v8 deactivateVoicemailAudioSession];
    }
  }
  return v5;
}
