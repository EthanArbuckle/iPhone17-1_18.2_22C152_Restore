...
+ (id)backgroundImageForSize:(CGSize)a3 state:(unint64_t)a4
{
  ...
  v7 = [a1 backgroundImageNameForSize:a4];
  v8 = [a1 imageCache];
  ...
  if (!v9)
  {
    ...
    [v10 setScale:0.0];
    id v11 = [objc_alloc((Class)UIGraphicsImageRenderer) initWithSize:v10 format:width, height];
...
void __48__VMAccountButton_backgroundImageForSize_state___block_invoke(uint64_t a1)
{
  ...
  +[UIBezierPath bezierPathWithRoundedRect:cornerRadius:](UIBezierPath, "bezierPathWithRoundedRect:cornerRadius:", v6.origin.x, v6.origin.y, v6.size.width, v6.size.height, 7.5);
...
+ (NSCache)imageCache
{
  if (imageCache_onceToken != -1) {
    dispatch_once(&imageCache_onceToken, &__block_literal_global_18);
  }
...
void __29__VMAccountButton_imageCache__block_invoke(id a1)
{
  ...
  uint64_t v2 = imageCache_imageCache;
  imageCache_imageCache = (uint64_t)v1;
...
- (VMAccountButton)initWithFrame:(CGRect)a3
{
  ...
  id v3 = -[VMAccountButton initWithFrame:](&v6, "initWithFrame:", a3.origin.x, a3.origin.y, a3.size.width, a3.size.height);
  v4 = v3;
  if (v3)
  {
...
- (void)commonInit
{
  ...
  id v3 = +[UIFont telephonyUIBodyShortFont];
  [v5 setFont:v3];
...
- (void)layoutSubviews
{
  ...
  [(VMAccountButton *)self frame];
  double v6 = [v3 backgroundImageForSize:0 state:v4, v5];
  if (v6) {
...
- (double)titleLabelFirstBaselineAnchorLayoutConstraintConstant
{
  uint64_t v2 = [(VMAccountButton *)self titleLabel];
  double v3 = [v2 font];
