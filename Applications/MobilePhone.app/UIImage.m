...
+ (id)voicemailPauseButtonImage
{
  if (voicemailPauseButtonImage_onceToken != -1) {
    dispatch_once(&voicemailPauseButtonImage_onceToken, &__block_literal_global_11);
  }
  v2 = (void *)voicemailPauseButtonImage___sPauseButtonImage;

  return v2;
}

void __49__UIImage_VoicemailUI__voicemailPauseButtonImage__block_invoke(id a1)
{
  uint64_t v1 = +[UIImage tpImageForSymbolType:26 textStyle:UIFontTextStyleBody scale:3 isStaticSize:0];
  uint64_t v2 = voicemailPauseButtonImage___sPauseButtonImage;
  voicemailPauseButtonImage___sPauseButtonImage = v1;
...
+ (id)voicemailPlayButtonImage
{
  if (voicemailPlayButtonImage_onceToken != -1) {
    dispatch_once(&voicemailPlayButtonImage_onceToken, &__block_literal_global_42_0);
  }
  uint64_t v2 = (void *)voicemailPlayButtonImage___sPlayButtonImage;

  return v2;
}

void __48__UIImage_VoicemailUI__voicemailPlayButtonImage__block_invoke(id a1)
{
  uint64_t v1 = +[UIImage tpImageForSymbolType:25 textStyle:UIFontTextStyleBody scale:3 isStaticSize:0];
  uint64_t v2 = voicemailPlayButtonImage___sPlayButtonImage;
  voicemailPlayButtonImage___sPlayButtonImage = v1;
