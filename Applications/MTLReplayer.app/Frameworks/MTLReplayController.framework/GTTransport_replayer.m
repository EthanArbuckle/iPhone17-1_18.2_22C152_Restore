...
- (BOOL)relayMessage:(id)a3 error:(id *)a4
{
  if ([a3 transport] == self)
  {
    ...
    uint64_t v11 = 40;
LABEL_15:
    id v12 = +[GTError_replayer errorWithDomain:@"DYErrorDomain" code:v11 userInfo:0];
    LOBYTE(v7) = 0;
    *a4 = v12;
    return v7;
  }
...
LABEL_16:
  ...
  BOOL v7 = [(GTTransport_replayer *)self _packMessage:a3 error:a4];
  if (v7)
  {
...
- (BOOL)sendNewMessage:(int)a3 error:(id *)a4
{
  return [(GTTransport_replayer *)self sendNewMessage:*(void *)&a3 error:a4 replyQueue:0 timeout:0 handler:0];
}

- (BOOL)sendNewMessage:(int)a3 error:(id *)a4 replyQueue:(id)a5 timeout:(unint64_t)a6 handler:(id)a7
{
  id v12 = [[GTTransportMessage_replayer alloc] initWithKind:*(void *)&a3 attributes:0 payload:0];
  LOBYTE(a7) = [(GTTransport_replayer *)self send:v12 inReplyTo:0 error:a4 replyQueue:a5 timeout:a6 handler:a7];
...
- (BOOL)send:(id)a3 error:(id *)a4
{
  return [(GTTransport_replayer *)self send:a3 inReplyTo:0 error:a4 replyQueue:0 timeout:0 handler:0];
}

- (BOOL)send:(id)a3 error:(id *)a4 replyQueue:(id)a5 timeout:(unint64_t)a6 handler:(id)a7
{
  return [(GTTransport_replayer *)self send:a3 inReplyTo:0 error:a4 replyQueue:a5 timeout:a6 handler:a7];
}

- (BOOL)send:(id)a3 inReplyTo:(id)a4 error:(id *)a5
{
  return [(GTTransport_replayer *)self send:a3 inReplyTo:a4 error:a5 replyQueue:0 timeout:0 handler:0];
}
...
- (BOOL)send:(id)a3 inReplyTo:(id)a4 error:(id *)a5 replyQueue:(id)a6 timeout:(unint64_t)a7 handler:(id)a8
{
  if ([a3 hasBeenSent])
  {
    ...
    uint64_t v26 = 34;
LABEL_31:
    id v27 = +[GTError_replayer errorWithDomain:@"DYErrorDomain" code:v26 userInfo:0];
    LOBYTE(v15) = 0;
    *a5 = v27;
    return v15;
  }
...
LABEL_32:
  ...
  BOOL v15 = [(GTTransport_replayer *)self _packMessage:a3 error:a5];
  if (v15)
  {
    uint64_t v16 = [(GTTransport_replayer *)self _nextMessageSerial];
    if (a4) {
...
- (void)invalidate
{
  id v3 = +[GTError_replayer errorWithDomain:@"DYErrorDomain" code:6 userInfo:0];
...
- (void)_dispatchMessage:(id)a3
{
  if (!LOBYTE(self->_interposerVersion))
  {
    ...
    id v6 = [(GTIntKeyedDictionary_replayer *)replyHandlersMap countByEnumeratingWithState:&v18 objects:v22 count:16];
    if (v6)
    {
      id v7 = v6;
      ...
      do
      {
        ...
        id v7 = [(GTIntKeyedDictionary_replayer *)replyHandlersMap countByEnumeratingWithState:&v18 objects:v22 count:16];
      }
      while (v7);
    }
...
- (void)_handleReplyTimeout:(unsigned int)a3 count:(unsigned int)a4
{
  ...
  if (v7 && v7[10] <= a4)
  {
    ...
    [v9 dispatchError:[GTError_replayer errorWithDomain:@"DYErrorDomain" code:5 userInfo:0]];
  }
}

- (void)_invalidate
{
  [(GTIntKeyedDictionary_replayer *)self->_replyHandlersMap makeObjectsPerformSelector:"cancel"];
  error = self->_error;
  v4[0] = _NSConcreteStackBlock;
  v4[1] = 3221225472;
...
- (id)newSourceWithQueue:(id)a3
{
  ...
  char v10 = __Block_byref_object_copy__1487;
  unsigned int v11 = __Block_byref_object_dispose__1488;
  uint64_t v12 = 0;
  queue = self->_queue;
  block[0] = _NSConcreteStackBlock;
  block[1] = 3221225472;
...
- (NSURL)url
{
  ...
  uint64_t v9 = __Block_byref_object_copy__1487;
  char v10 = __Block_byref_object_dispose__1488;
  uint64_t v11 = 0;
  queue = self->_queue;
...
- (id)debugDescription
{
  id v3 = [(GTTransport_replayer *)self description];
  char v4 = [(GTTransport_replayer *)self url];
  unsigned int v5 = [(GTTransport_replayer *)self connected];
  unsigned int v6 = [(GTTransport_replayer *)self invalid];
  uint64_t v7 = [(GTTransport_replayer *)self error];
...
{
  v8.receiver = self;
  v8.super_class = (Class)GTTransport_replayer;
  id v2 = [(GTTransport_replayer *)&v8 init];
  if (v2)
  {
