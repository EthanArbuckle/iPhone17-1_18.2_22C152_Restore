...
- (void)resolveWithFuture:(id)a3
{
  ...
  else
  {
    ...
    v9[4] = a3;
    v9[5] = self;
    [v8 addObject:[NSBlockOperation blockOperationWithBlock:v9]];
    [*((id *)a3 + 2) unlock];
  }
}
...
- (void)cancel
{
  [(NSCondition *)self->_condition lock];
  self->_cancelled = 1;
  v3 = (NSError *)+[GTError_replayer errorWithDomain:@"DYErrorDomain" code:1027 userInfo:0];
...
- (int)intResult
{
  id v2 = [(GTFuture_replayer *)self result];
...
- (void)dealloc
{
  id v3 = +[GTError_replayer errorWithDomain:@"DYErrorDomain" code:1028 userInfo:0];
  [(NSCondition *)self->_condition lock];
  if (!self->_resolved)
  {
    [(NSCondition *)self->_condition unlock];
...
- (GTFuture_replayer)init
{
  v5.receiver = self;
  v5.super_class = (Class)GTFuture_replayer;
  id v2 = [(GTFuture_replayer *)&v5 init];
  if (v2)
  {
...
+ (BOOL)logPerformance
{
  if (+[GTFuture logPerformance]::onceToken != -1) {
    dispatch_once(&+[GTFuture logPerformance]::onceToken, &__block_literal_global_1389);
  }
