...
- (void)_dequeueIncomingMessages
{
  ...
  while (1)
  {
    while (1)
    {
      ...
      ((void (*)(GTSharedMemoryTransport_replayer *, SEL))self->_consumeBytesIMP)(self, consumeBytesSEL);
      if (self->_currentPacketBytesLeft) {
        goto LABEL_24;
      }
...
- (void)_dequeuePacket
{
  unint64_t v3 = [(GTSharedMemoryTransport_replayer *)self _computeBytesAvailableToRead];
  if (v3 <= 3)
  {
...
- (void)_accumulateMessageBytes
{
  while (self->_currentPacketBytesLeft
       && [(GTBaseStreamTransport_replayer *)self _readAndAccumulate] > 0)
    ;
}
...
- (int64_t)_write:(const void *)a3 size:(unint64_t)a4
{
  ...
  if (a4)
  {
    ...
    unint64_t v8 = [(GTSharedMemoryTransport_replayer *)self _computeBytesAvailableToWrite];
    if (v8 > 4)
    {
      if (v8 - 4 < v4) {
        unint64_t v4 = v8 - 4;
...
- (BOOL)_clientConnect:(id *)a3
{
  ...
  BOOL result = [(GTSharedMemoryTransport_replayer *)self _createAndRunSources:a3];
...
- (BOOL)_createAndRunSources:(id *)a3
{
  ...
  BOOL v13 = __Block_byref_object_copy__1487;
  v14 = __Block_byref_object_dispose__1488;
  uint64_t v15 = 0;
  unsigned int v4 = [GTContinuation_replayer alloc];
  queue = self->super.super._queue;
  ...
  v9[4] = self;
  v9[5] = &v10;
  unsigned int v6 = [(GTContinuation_replayer *)v4 initWithQueue:queue block:v9];
  v11[5] = (uint64_t)v6;
  pthread_t v8 = 0;
...
- (BOOL)_openSMRegion:(sm_region_header *)a3 size:(unint64_t)a4 name:(const char *)a5 error:(id *)a6
{
  ...
  pthread_attr_t v16 = [(GTSharedMemoryTransport_replayer *)self _mapSharedMemoryFile:v12 size:v15 error:a6];
  if (v16)
  {
    v17 = v16;
    close(v14);
...
- (BOOL)_initializeSMRegion:(sm_region_header *)a3 size:(unint64_t)a4 name:(const char *)a5 error:(id *)a6
{
  ...
  {
    uint64_t v12 = [(GTSharedMemoryTransport_replayer *)self _mapSharedMemoryFile:v11 size:v9 error:a6];
    if (v12)
    {
      BOOL v13 = v12;
      close(v11);
...
- (void)_mapSharedMemoryFile:(int)a3 size:(unint64_t)a4 error:(id *)a5
{
  size_t v8 = [(GTSharedMemoryTransport_replayer *)self _calculateVMRegionMapSize:a4];
...
- (BOOL)send:(id)a3 inReplyTo:(id)a4 error:(id *)a5 replyQueue:(id)a6 timeout:(unint64_t)a7 handler:(id)a8
{
  if (self->_deferred)
  {
    BOOL v13 = objc_alloc_init(GTBufferedSendOperation_replayer);
...
{
  v16.receiver = self;
  v16.super_class = (Class)GTSharedMemoryTransport_replayer;
  unsigned int v6 = [(GTBaseStreamTransport_replayer *)&v16 init];
  unsigned int v7 = v6;
  if (!v6) {
    return v7;
  }
  ...
  id v13 = [(GTSharedMemoryTransport_replayer *)v7 methodForSelector:v12];
...
- (GTSharedMemoryTransport_replayer)initWithMode:(int)a3
{
  return [(GTSharedMemoryTransport_replayer *)self initWithMode:*(void *)&a3 initialMessageSerial:0];
}
