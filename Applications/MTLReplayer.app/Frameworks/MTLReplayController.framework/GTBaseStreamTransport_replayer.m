...
LABEL_5:
  int64_t v8 = [(GTBaseStreamTransport_replayer *)self _writeBuffers:0];
  int64_t v9 = v8;
  if (v8 == -1)
  {
LABEL_8:
...
LABEL_9:
  ...
  if (!v11)
  {
    *(_OWORD *)&v10->var3 = xmmword_4C8020;
    v10->var5 = 0;
    goto LABEL_8;
  }
  return v9;
...
LABEL_10:
  int64_t v15 = [(GTBaseStreamTransport_replayer *)self _writeBuffers:a4];
  dispatch_semaphore_signal((dispatch_semaphore_t)self->_sendSema);
  return v15;
}
...
- (BOOL)_packMessage:(id)a3 error:(id *)a4
{
  if ([a3 attributes])
  {
    ...
    if (!v7)
    {
      if (a4)
      {
        ...
        *a4 = +[GTError_replayer errorWithDomain:@"DYErrorDomain" code:36 userInfo:v13];
      }
      return 0;
    }
  ...
  if (a4)
  {
    id v12 = +[GTError_replayer errorWithDomain:@"DYErrorDomain" code:261 userInfo:0];
    BOOL result = 0;
    *a4 = v12;
    return result;
  }
  ...
  if (v6)
  {
    ...
    while (1)
    {
      do
      {
        while (1)
        {
          unint64_t v15 = [(GTBaseStreamTransport_replayer *)self _write:iov->iov_base size:iov->iov_len];
          int64_t v16 = __error();
          if (v15 == -1) {
            break;
          }
...
LABEL_20:
      ...
      {
        if (a3) {
          *a3 = [(GTTransport_replayer *)self error];
        }
        goto LABEL_24;
      }
    ...
    [(GTBaseStreamTransport_replayer *)self _scheduleInvalidation:v22];
    if (a3) {
      *a3 = v22;
    }
    *__error() = v17;
...
LABEL_16:
  ...
  int64_t v11 = [(GTBaseStreamTransport_replayer *)self _performRead:self->_messageBufferWritePtr size:var3];
  if (v11 == -1) {
    return -1;
  }
  v4 += v11;
...
- (int64_t)_performRead:(void *)a3 size:(unint64_t)a4
{
  ...
  do
  {
    int64_t result = [(GTBaseStreamTransport_replayer *)self _read:a3 size:self->_bytesToRead];
    if (result != -1)
    {
      if (result)
      {
...
LABEL_5:
    ...
    if ((var5 & 0x80000000) != 0)
    {
      ...
      id v12 = [[self->_messageBuffer subdataWithRange:(((v7 + v4 + tmu->var2 - 1) & -(uint64_t)tmu->var2)- (void)-[GTVMBuffer mutableBytes](self->_messageBuffer, "mutableBytes"), var5 & 0x7FFFFFFF)] GTGPUToolsData];
      uint64_t v9 = (v13 + v9);
      tmu = self->_tmu;
    }
    else
    ...
    int v14 = [[GTTransportMessage_replayer alloc] initWithKind:tmu->var0.var1 attributes:v8 payload:v12];
    [(GTTransportMessage_replayer *)v14 _setSerial:self->_tmu->var0.var2 replySerial:self->_tmu->var0.var3 transport:self];
    [(GTTransportMessage_replayer *)v14 _setTransportSize:v9];
    objc_opt_class();
    ...
    [(GTTransport_replayer *)self _dispatchMessage:v14];

    int64_t v16 = self->_tmu;
    *(_OWORD *)&v16->unint64_t var3 = xmmword_4C8020;
...
- (int64_t)_syncTmuToHeader:(dy_transport_message_unpack_s *)a3 ioBlock:(id)a4
{
  ...
  if (result != -1 && !a3->var3)
  {
    ...
    *(int8x16_t *)&a3->var0.var0 = vrev32q_s8(*(int8x16_t *)&a3->var0.var0);
    ...
    [(GTBaseStreamTransport_replayer *)self _scheduleInvalidation:+[GTError_replayer errorWithDomain:@"DYErrorDomain" code:v11 userInfo:0]];
    return -1;
  }
  return result;
}
...
- (void)_scheduleInvalidation:(id)a3
{
  ...
  [(GTTransport_replayer *)&v4 _scheduleInvalidation:a3];
  dispatch_semaphore_signal((dispatch_semaphore_t)self->_sendSema);
}
...
- (void)_invalidate
{
  ...
  [(GTTransport_replayer *)&v3 _invalidate];
}
...
- (GTBaseStreamTransport_replayer)init
{
  ...
  if (v2)
  {
    ...
    *((void *)v2 + 13) = v3;
    if (v3)
    {
      *(void *)(v3 + 24) = 0x1000000001;
      *(_OWORD *)(v3 + 32) = xmmword_4C8020;
      *(unsigned char *)(v3 + 48) = 0;
      vm_address_t v4 = GTCoreAlloc(0x31uLL);
      *((void *)v2 + 28) = v4;
      if (v4)
      {
        *(void *)(v4 + 24) = 0x100000001;
        *(_OWORD *)(v4 + 32) = xmmword_4C8020;
        *(unsigned char *)(v4 + 48) = 0;
        objc_super v5 = [+[NSString stringWithFormat:](NSString, "stringWithFormat:", @"gputools.%@.%p.%@", +[NSString stringWithUTF8String:object_getClassName(v2)], v2, @"relay") UTF8String];
