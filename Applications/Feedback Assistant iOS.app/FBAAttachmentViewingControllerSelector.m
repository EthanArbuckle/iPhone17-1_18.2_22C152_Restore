...
+ (id)controllerForUrl:(id)a3
{
  return [a1 controllerForUrl:a3 useFallback:1];
}
...
+ (id)controllerForUrl:(id)a3 useFallback:(BOOL)a4
{
  ...
  {
    if (!+[FBABugFormAttachmentPreview canPreviewItem:v6])goto LABEL_15; {
    if (+[FBABugFormAttachmentPreview urlPointsToTextFile:v6])
    }
    {
      uint64_t v12 = +[FBABugFormAttachmentPreview textFileForURL:v6];
...
LABEL_15:
  id v19 = 0;
  unsigned int v13 = [v6 getResourceValue:&v19 forKey:NSURLIsDirectoryKey error:0];
  id v14 = v19;
  v15 = v14;
...
{
  id v3 = a3;
  BOOL v4 = +[FBADraftManager sharedInstance];
  v5 = [v4 appStoryboard];

  id v6 = [v5 instantiateViewControllerWithIdentifier:@"FBAOpaqueFileViewController"];
  [v6 setItem:v3];
