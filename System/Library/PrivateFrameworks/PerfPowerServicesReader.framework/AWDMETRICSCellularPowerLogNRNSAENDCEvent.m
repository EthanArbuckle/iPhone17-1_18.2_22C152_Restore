@interface AWDMETRICSCellularPowerLogNRNSAENDCEvent
- (BOOL)hasEvent;
- (BOOL)hasFr;
- (BOOL)hasSubsId;
- (BOOL)hasTimestamp;
...
@implementation AWDMETRICSCellularPowerLogNRNSAENDCEvent
...
{
  id v3 = a3;
  if ([v3 isEqualToString:@"ENDC_OFF"]) {
    int v4 = 0;
  }
  else {
    int v4 = [v3 isEqualToString:@"ENDC_ON"];
  }
...
- (id)description
{
  id v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)AWDMETRICSCellularPowerLogNRNSAENDCEvent;
  int v4 = [(AWDMETRICSCellularPowerLogNRNSAENDCEvent *)&v8 description];
  v5 = [(AWDMETRICSCellularPowerLogNRNSAENDCEvent *)self dictionaryRepresentation];
...
- (BOOL)readFrom:(id)a3
{
  return AWDMETRICSCellularPowerLogNRNSAENDCEventReadFrom((uint64_t)self, (uint64_t)a3);
}
...
LABEL_21:
  ...
  if ((*(unsigned char *)&self->_has & 2) != 0)
  {
    if ((*((unsigned char *)v4 + 28) & 2) == 0 || self->_event != *((_DWORD *)v4 + 4)) {
      goto LABEL_21;
    }
  }
  ...
  if ((*(unsigned char *)&self->_has & 4) != 0)
  {
    if ((*((unsigned char *)v4 + 28) & 4) == 0 || self->_fr != *((_DWORD *)v4 + 5)) {
      goto LABEL_21;
    }
    BOOL v5 = 1;
  }
