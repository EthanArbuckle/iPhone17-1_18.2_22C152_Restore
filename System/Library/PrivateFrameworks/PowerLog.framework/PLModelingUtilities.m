...
{
  if (isiPod_once != -1) {
    dispatch_once(&isiPod_once, &__block_literal_global_30_0);
  }
...
{
  if (isAppleTV_once != -1) {
    dispatch_once(&isAppleTV_once, &__block_literal_global_40);
  }
...
+ (BOOL)internalBuild
{
  if (internalBuild_onceToken != -1) {
    dispatch_once(&internalBuild_onceToken, &__block_literal_global_156);
  }
...
+ (BOOL)hasAOD
{
  if (hasAOD_onceToken != -1) {
    dispatch_once(&hasAOD_onceToken, &__block_literal_global_218);
  }
