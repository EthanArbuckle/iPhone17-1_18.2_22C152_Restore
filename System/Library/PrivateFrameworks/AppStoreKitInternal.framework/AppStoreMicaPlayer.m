...
LABEL_6:
  ...
  v10 = v9;
  objc_msgSend(v9, "setGeometryFlipped:", +[AppStoreMicaPlayer BOOLFromDictionary:key:defaultValue:](AppStoreMicaPlayer, "BOOLFromDictionary:key:defaultValue:", v8, @"geometryFlipped", 0) ^ 1);
...
- (id)copyWithZone:(_NSZone *)a3
{
  ...
  uint64_t v9 = [(AppStoreMicaPlayer *)self publishedObjects];
  uint64_t v10 = [(AppStoreMicaPlayer *)self rootLayer];
  uint64_t v11 = +[AppStoreMicaPlayer updatePublishedObjects:v9 toReferenceLayersInTree:v8 ratherThanLayersInTree:v10];
...
- (void)setPlaybackTime:(double)a3
{
  BOOL v5 = [(AppStoreMicaPlayer *)self isPlaying];
  double v6 = 0.0;
  if (v5)
  {
...
- (BOOL)isTimerNeeded
{
  if ([(AppStoreMicaPlayer *)self isPlaying])
  {
    ...
    if (v3 == INFINITY)
    {
      double v5 = [(AppStoreMicaPlayer *)self delegate];
      if (v5)
      {
        id v6 = [(AppStoreMicaPlayer *)self delegate];
...
{
  id WeakRetained = objc_loadWeakRetained((id *)&self->_delegate);
  return (AppStoreMicaPlayerDelegate *)WeakRetained;
}
