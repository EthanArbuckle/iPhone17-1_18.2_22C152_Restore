@interface AppleDeviceQueryBasebandArmory
+ (BOOL)hasBaseband;
...
LABEL_10:
  ...
  ZhuGeLog(262400, "/Library/Caches/com.apple.xbs/Sources/ZhuGe_Service/ZhuGeCommon/ZhuGeBaseband.m", "+[AppleDeviceQueryBasebandArmory getBasebandClass]", 198, @"baseband class: %@", v3, v4, v5, v2);
  objc_opt_class();
...
+ (BOOL)isBasebandReady
{
  ...
  uint64_t v10 = 0;
  uint64_t v4 = +[AppleDeviceQueryBasebandPostponementStatusArmory query:v3 isCachable:0 withError:&v10];
  ZhuGeLog(262400, "/Library/Caches/com.apple.xbs/Sources/ZhuGe_Service/ZhuGeCommon/ZhuGeBaseband.m", "+[AppleDeviceQueryBasebandArmory isBasebandReady]", 234, @"postponement status %@", v5, v6, v7, (uint64_t)v4);
...
+ (BOOL)waitUntilBasebandReady:(int)a3 retryInterval:(int)a4
{
  ...
  if (v6)
  {
    LOBYTE(v6) = +[AppleDeviceQueryBasebandArmory isBasebandReady];
    if ((v6 & 1) == 0 && a3 >= 1)
    {
      double v7 = (double)a4;
      int v8 = 1;
...
+ (id)getProperty:(char)a3[512] inPlane:(char)a4[128] withName:(__CFString *)a5 withOption:(unsigned int)a6
{
  ...
  if (v10)
  {
    ...
    IOObjectRelease(v14);
    if (!v15) {
      ZhuGeLog(256, "/Library/Caches/com.apple.xbs/Sources/ZhuGe_Service/ZhuGeCommon/ZhuGeBaseband.m", "+[AppleDeviceQueryBasebandArmory getProperty:inPlane:withName:withOption:]", 276, @"can not find property %@ under %s", v16, v17, v18, (uint64_t)a5);
    }
  }
  else
  {
    ZhuGeLog(256, "/Library/Caches/com.apple.xbs/Sources/ZhuGe_Service/ZhuGeCommon/ZhuGeBaseband.m", "+[AppleDeviceQueryBasebandArmory getProperty:inPlane:withName:withOption:]", 269, @"can not find entry at %s", v11, v12, v13, (uint64_t)a3);
    v15 = 0;
  }
