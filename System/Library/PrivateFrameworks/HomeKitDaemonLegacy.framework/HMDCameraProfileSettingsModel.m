@interface HMDCameraProfileSettingsModel
+ (id)hmbProperties;
- (BOOL)accessModeChangeNotificationEnabled;
- (BOOL)accessModeIndicatorEnabled;
- (BOOL)activityZonesIncludedForSignificantEventDetection;
- (BOOL)cameraManuallyDisabled;
- (BOOL)hasAnyNotificationFieldsSet;
- (BOOL)nightVisionModeEnabled;
- (BOOL)periodicSnapshotsAllowed;
- (BOOL)reachabilityEventNotificationEnabled;
- (BOOL)recordingAudioEnabled;
- (BOOL)snapshotsAllowed;
- (HMDBulletinBoardNotification)smartBulletinBoardNotification;
- (NSSet)activityZones;
- (int64_t)version;
- (unint64_t)accessModeAtHome;
- (unint64_t)accessModeNotAtHome;
- (unint64_t)currentAccessMode;
- (unint64_t)mostPrivateAccessMode;
- (unint64_t)recordingEventTriggers;
- (void)setAccessModeAtHome:(unint64_t)a3;
- (void)setAccessModeIndicatorEnabled:(BOOL)a3;
- (void)setAccessModeNotAtHome:(unint64_t)a3;
- (void)setActivityZones:(id)a3;
- (void)setActivityZonesIncludedForSignificantEventDetection:(BOOL)a3;
- (void)setCameraManuallyDisabled:(BOOL)a3;
- (void)setCurrentAccessMode:(unint64_t)a3;
- (void)setNightVisionModeEnabled:(BOOL)a3;
- (void)setPeriodicSnapshotsAllowed:(BOOL)a3;
- (void)setRecordingAudioEnabled:(BOOL)a3;
- (void)setRecordingEventTriggers:(unint64_t)a3;
- (void)setSnapshotsAllowed:(BOOL)a3;
- (void)setVersion:(int64_t)a3;
@end

@implementation HMDCameraProfileSettingsModel

+ (id)hmbProperties
{
  if (hmbProperties_onceToken_72677 != -1) {
    dispatch_once(&hmbProperties_onceToken_72677, &__block_literal_global_72678);
  }
  v2 = (void *)hmbProperties__properties_72679;
  return v2;
}

- (void)setVersion:(int64_t)a3
{
  id v4 = [NSNumber numberWithInteger:a3];
  [(HMDCameraProfileSettingsModel *)self setVersionField:v4];
}

- (void)setRecordingEventTriggers:(unint64_t)a3
{
  id v4 = [NSNumber numberWithUnsignedInteger:a3];
  [(HMDCameraProfileSettingsModel *)self setRecordingEventTriggersField:v4];
}

- (void)setAccessModeNotAtHome:(unint64_t)a3
{
  id v4 = [NSNumber numberWithUnsignedInteger:a3];
  [(HMDCameraProfileSettingsModel *)self setAccessModeNotAtHomeField:v4];
}

- (void)setAccessModeAtHome:(unint64_t)a3
{
  id v4 = [NSNumber numberWithUnsignedInteger:a3];
  [(HMDCameraProfileSettingsModel *)self setAccessModeAtHomeField:v4];
}

- (unint64_t)recordingEventTriggers
{
  v2 = [(HMDCameraProfileSettingsModel *)self recordingEventTriggersField];
  unint64_t v3 = [v2 unsignedIntegerValue];

  return v3;
}

- (unint64_t)currentAccessMode
{
  return self->_currentAccessMode;
}

- (BOOL)cameraManuallyDisabled
{
  return self->_cameraManuallyDisabled;
}

- (NSSet)activityZones
{
  v23[2] = *MEMORY[0x1E4F143B8];
  unint64_t v3 = [(HMDCameraProfileSettingsModel *)self activityZonesField];

  if (v3)
  {
    id v4 = (void *)MEMORY[0x1E4F28DC0];
    v5 = (void *)MEMORY[0x1E4F1CAD0];
    v23[0] = objc_opt_class();
    v23[1] = objc_opt_class();
    v6 = [MEMORY[0x1E4F1C978] arrayWithObjects:v23 count:2];
    v7 = [v5 setWithArray:v6];
    v8 = [(HMDCameraProfileSettingsModel *)self activityZonesField];
    id v18 = 0;
    v9 = [v4 unarchivedObjectOfClasses:v7 fromData:v8 error:&v18];
    id v10 = v18;

    if (v9)
    {
      id v11 = v9;
    }
    else
    {
      v13 = (void *)MEMORY[0x1D9452090]();
      v14 = self;
      v15 = HMFGetOSLogHandle();
      if (os_log_type_enabled(v15, OS_LOG_TYPE_ERROR))
      {
        v16 = HMFGetLogIdentifier();
        *(_DWORD *)buf = 138543618;
        v20 = v16;
        __int16 v21 = 2112;
        id v22 = v10;
        _os_log_impl(&dword_1D49D5000, v15, OS_LOG_TYPE_ERROR, "%{public}@Failed to decode activity zones: %@", buf, 0x16u);
      }
      id v11 = [MEMORY[0x1E4F1CAD0] set];
    }
    v12 = v11;
  }
  else
  {
    v12 = [MEMORY[0x1E4F1CAD0] set];
  }
  return (NSSet *)v12;
}

- (BOOL)activityZonesIncludedForSignificantEventDetection
{
  v2 = [(HMDCameraProfileSettingsModel *)self activityZonesIncludedForSignificantEventDetectionField];
  char v3 = [v2 BOOLValue];

  return v3;
}

- (unint64_t)accessModeNotAtHome
{
  v2 = [(HMDCameraProfileSettingsModel *)self accessModeNotAtHomeField];
  unint64_t v3 = [v2 unsignedIntegerValue];

  return v3;
}

- (unint64_t)accessModeAtHome
{
  v2 = [(HMDCameraProfileSettingsModel *)self accessModeAtHomeField];
  unint64_t v3 = [v2 unsignedIntegerValue];

  return v3;
}

- (void)setCurrentAccessMode:(unint64_t)a3
{
  self->_currentAccessMode = a3;
}

- (void)setCameraManuallyDisabled:(BOOL)a3
{
  self->_cameraManuallyDisabled = a3;
}

void __46__HMDCameraProfileSettingsModel_hmbProperties__block_invoke()
{
  v33[15] = *MEMORY[0x1E4F143B8];
  v32[0] = @"versionField";
  v28 = [MEMORY[0x1E4F69E98] fieldWithClass:objc_opt_class()];
  v33[0] = v28;
  v32[1] = @"accessModeAtHomeField";
  v27 = [MEMORY[0x1E4F69E98] fieldWithClass:objc_opt_class()];
  v33[1] = v27;
  v32[2] = @"accessModeNotAtHomeField";
  v26 = [MEMORY[0x1E4F69E98] fieldWithClass:objc_opt_class()];
  v33[2] = v26;
  v32[3] = @"recordingStorageDurationField";
  v25 = [MEMORY[0x1E4F69E98] fieldWithClass:objc_opt_class()];
  v33[3] = v25;
  v32[4] = @"recordingEventTriggersField";
  v24 = [MEMORY[0x1E4F69E98] fieldWithClass:objc_opt_class()];
  v33[4] = v24;
  v32[5] = @"accessModeIndicatorEnabledField";
  v23 = [MEMORY[0x1E4F69E98] fieldWithClass:objc_opt_class()];
  v33[5] = v23;
  v32[6] = @"nightVisionModeEnabledField";
  id v22 = [MEMORY[0x1E4F69E98] fieldWithClass:objc_opt_class()];
  v33[6] = v22;
  v32[7] = @"snapshotsAllowedField";
  __int16 v21 = [MEMORY[0x1E4F69E98] fieldWithClass:objc_opt_class()];
  v33[7] = v21;
  v32[8] = @"periodicSnapshotsAllowedField";
  v20 = [MEMORY[0x1E4F69E98] fieldWithClass:objc_opt_class()];
  v33[8] = v20;
  v32[9] = @"recordingAudioEnabledField";
  v19 = [MEMORY[0x1E4F69E98] fieldWithClass:objc_opt_class()];
  v33[9] = v19;
  v32[10] = @"smartBulletinBoardNotificationDataField";
  v0 = (void *)MEMORY[0x1E4F69E98];
  uint64_t v1 = objc_opt_class();
  id v18 = [MEMORY[0x1E4F69EA0] excludeFromCloudStorage];
  v31 = v18;
  v17 = [MEMORY[0x1E4F1C978] arrayWithObjects:&v31 count:1];
  v16 = [v0 fieldWithClass:v1 options:v17];
  v33[10] = v16;
  v32[11] = @"accessModeChangeNotificationEnabledField";
  v2 = (void *)MEMORY[0x1E4F69E98];
  uint64_t v3 = objc_opt_class();
  id v4 = [MEMORY[0x1E4F69EA0] excludeFromCloudStorage];
  v30 = v4;
  v5 = [MEMORY[0x1E4F1C978] arrayWithObjects:&v30 count:1];
  v6 = [v2 fieldWithClass:v3 options:v5];
  v33[11] = v6;
  v32[12] = @"reachabilityEventNotificationEnabledField";
  v7 = (void *)MEMORY[0x1E4F69E98];
  uint64_t v8 = objc_opt_class();
  v9 = [MEMORY[0x1E4F69EA0] excludeFromCloudStorage];
  v29 = v9;
  id v10 = [MEMORY[0x1E4F1C978] arrayWithObjects:&v29 count:1];
  id v11 = [v7 fieldWithClass:v8 options:v10];
  v33[12] = v11;
  v32[13] = @"activityZonesField";
  v12 = [MEMORY[0x1E4F69E98] fieldWithClass:objc_opt_class()];
  v33[13] = v12;
  v32[14] = @"activityZonesIncludedForSignificantEventDetectionField";
  v13 = [MEMORY[0x1E4F69E98] fieldWithClass:objc_opt_class()];
  v33[14] = v13;
  uint64_t v14 = [MEMORY[0x1E4F1C9E8] dictionaryWithObjects:v33 forKeys:v32 count:15];
  v15 = (void *)hmbProperties__properties_72679;
  hmbProperties__properties_72679 = v14;
}

- (void)setActivityZonesIncludedForSignificantEventDetection:(BOOL)a3
{
  id v4 = [NSNumber numberWithBool:a3];
  [(HMDCameraProfileSettingsModel *)self setActivityZonesIncludedForSignificantEventDetectionField:v4];
}

- (void)setActivityZones:(id)a3
{
  uint64_t v16 = *MEMORY[0x1E4F143B8];
  id v4 = a3;
  id v11 = 0;
  v5 = [MEMORY[0x1E4F28DB0] archivedDataWithRootObject:v4 requiringSecureCoding:1 error:&v11];
  id v6 = v11;
  if (v5)
  {
    [(HMDCameraProfileSettingsModel *)self setActivityZonesField:v5];
  }
  else
  {
    v7 = (void *)MEMORY[0x1D9452090]();
    uint64_t v8 = self;
    v9 = HMFGetOSLogHandle();
    if (os_log_type_enabled(v9, OS_LOG_TYPE_INFO))
    {
      id v10 = HMFGetLogIdentifier();
      *(_DWORD *)buf = 138543618;
      v13 = v10;
      __int16 v14 = 2112;
      id v15 = v6;
      _os_log_impl(&dword_1D49D5000, v9, OS_LOG_TYPE_INFO, "%{public}@Failed to encode activity zones: %@", buf, 0x16u);
    }
  }
}

- (HMDBulletinBoardNotification)smartBulletinBoardNotification
{
  uint64_t v19 = *MEMORY[0x1E4F143B8];
  uint64_t v3 = [(HMDCameraProfileSettingsModel *)self smartBulletinBoardNotificationDataField];

  if (v3)
  {
    id v4 = (void *)MEMORY[0x1E4F28DC0];
    uint64_t v5 = objc_opt_class();
    id v6 = [(HMDCameraProfileSettingsModel *)self smartBulletinBoardNotificationDataField];
    id v14 = 0;
    uint64_t v3 = [v4 unarchivedObjectOfClass:v5 fromData:v6 error:&v14];
    id v7 = v14;

    if (v3)
    {
      id v8 = v3;
    }
    else
    {
      v9 = (void *)MEMORY[0x1D9452090]();
      id v10 = self;
      id v11 = HMFGetOSLogHandle();
      if (os_log_type_enabled(v11, OS_LOG_TYPE_ERROR))
      {
        v12 = HMFGetLogIdentifier();
        *(_DWORD *)buf = 138543618;
        uint64_t v16 = v12;
        __int16 v17 = 2112;
        id v18 = v7;
        _os_log_impl(&dword_1D49D5000, v11, OS_LOG_TYPE_ERROR, "%{public}@Failed to decode HMDBulletinBoardNotification: %@", buf, 0x16u);
      }
    }
  }
  return (HMDBulletinBoardNotification *)v3;
}

- (BOOL)reachabilityEventNotificationEnabled
{
  v2 = [(HMDCameraProfileSettingsModel *)self reachabilityEventNotificationEnabledField];
  char v3 = [v2 BOOLValue];

  return v3;
}

- (BOOL)accessModeChangeNotificationEnabled
{
  v2 = [(HMDCameraProfileSettingsModel *)self accessModeChangeNotificationEnabledField];
  char v3 = [v2 BOOLValue];

  return v3;
}

- (void)setRecordingAudioEnabled:(BOOL)a3
{
  id v4 = [NSNumber numberWithBool:a3];
  [(HMDCameraProfileSettingsModel *)self setRecordingAudioEnabledField:v4];
}

- (BOOL)recordingAudioEnabled
{
  v2 = [(HMDCameraProfileSettingsModel *)self recordingAudioEnabledField];
  char v3 = [v2 BOOLValue];

  return v3;
}

- (void)setPeriodicSnapshotsAllowed:(BOOL)a3
{
  id v4 = [NSNumber numberWithBool:a3];
  [(HMDCameraProfileSettingsModel *)self setPeriodicSnapshotsAllowedField:v4];
}

- (BOOL)periodicSnapshotsAllowed
{
  v2 = [(HMDCameraProfileSettingsModel *)self periodicSnapshotsAllowedField];
  char v3 = [v2 BOOLValue];

  return v3;
}

- (void)setSnapshotsAllowed:(BOOL)a3
{
  id v4 = [NSNumber numberWithBool:a3];
  [(HMDCameraProfileSettingsModel *)self setSnapshotsAllowedField:v4];
}

- (BOOL)snapshotsAllowed
{
  v2 = [(HMDCameraProfileSettingsModel *)self snapshotsAllowedField];
  char v3 = [v2 BOOLValue];

  return v3;
}

- (void)setNightVisionModeEnabled:(BOOL)a3
{
  id v4 = [NSNumber numberWithBool:a3];
  [(HMDCameraProfileSettingsModel *)self setNightVisionModeEnabledField:v4];
}

- (BOOL)nightVisionModeEnabled
{
  v2 = [(HMDCameraProfileSettingsModel *)self nightVisionModeEnabledField];
  char v3 = [v2 BOOLValue];

  return v3;
}

- (void)setAccessModeIndicatorEnabled:(BOOL)a3
{
  id v4 = [NSNumber numberWithBool:a3];
  [(HMDCameraProfileSettingsModel *)self setAccessModeIndicatorEnabledField:v4];
}

- (BOOL)accessModeIndicatorEnabled
{
  v2 = [(HMDCameraProfileSettingsModel *)self accessModeIndicatorEnabledField];
  char v3 = [v2 BOOLValue];

  return v3;
}

- (int64_t)version
{
  v2 = [(HMDCameraProfileSettingsModel *)self versionField];
  int64_t v3 = [v2 integerValue];

  return v3;
}

- (unint64_t)mostPrivateAccessMode
{
  int64_t v3 = objc_msgSend(NSNumber, "numberWithUnsignedInteger:", -[HMDCameraProfileSettingsModel accessModeAtHome](self, "accessModeAtHome"));
  unint64_t v4 = [&unk_1F2DC6C88 indexOfObject:v3];

  uint64_t v5 = objc_msgSend(NSNumber, "numberWithUnsignedInteger:", -[HMDCameraProfileSettingsModel accessModeNotAtHome](self, "accessModeNotAtHome"));
  unint64_t v6 = [&unk_1F2DC6C88 indexOfObject:v5];

  if (v4 >= v6) {
    unint64_t v7 = v6;
  }
  else {
    unint64_t v7 = v4;
  }
  id v8 = [&unk_1F2DC6C88 objectAtIndex:v7];
  unint64_t v9 = [v8 unsignedIntegerValue];

  return v9;
}

- (BOOL)hasAnyNotificationFieldsSet
{
  int64_t v3 = [(HMDCameraProfileSettingsModel *)self reachabilityEventNotificationEnabledField];
  if (v3)
  {
    BOOL v4 = 1;
  }
  else
  {
    uint64_t v5 = [(HMDCameraProfileSettingsModel *)self accessModeChangeNotificationEnabledField];
    if (v5)
    {
      BOOL v4 = 1;
    }
    else
    {
      unint64_t v6 = [(HMDCameraProfileSettingsModel *)self smartBulletinBoardNotificationDataField];
      BOOL v4 = v6 != 0;
    }
  }

  return v4;
}

@end