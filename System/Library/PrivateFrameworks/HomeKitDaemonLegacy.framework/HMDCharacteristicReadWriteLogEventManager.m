...
+ (HMDCharacteristicReadWriteLogEventManager)sharedInstance
{
  if (sharedInstance_onceToken_139461 != -1) {
    dispatch_once(&sharedInstance_onceToken_139461, &__block_literal_global_139462);
  }
  v2 = (void *)sharedInstance_readWriteLogEventManager;
  return (HMDCharacteristicReadWriteLogEventManager *)v2;
}

uint64_t __59__HMDCharacteristicReadWriteLogEventManager_sharedInstance__block_invoke()
{
  v0 = objc_alloc_init(HMDCharacteristicReadWriteLogEventManager);
  uint64_t v1 = sharedInstance_readWriteLogEventManager;
