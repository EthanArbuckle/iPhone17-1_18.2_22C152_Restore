...
- (void)completeCurrentReport
{
  ...
  if (os_log_type_enabled(v6, OS_LOG_TYPE_INFO))
  {
    ...
    _os_log_impl(&dword_1D49D5000, v6, OS_LOG_TYPE_INFO, "%{public}@WatchConnectivityLogEventManagerTimer fired: Watch added: %lu Watch removed: %lu reportComplete: %d", (uint8_t *)&v11, 0x26u);
  }
  os_unfair_lock_unlock(p_lock);
}
...
- (HMDWatchConnectivityLogEventManager)init
{
  ...
  if (v2)
  {
    uint64_t v3 = [objc_alloc(MEMORY[0x1E4F65580]) initWithTimeInterval:13 options:86400.0];
...
+ (HMDWatchConnectivityLogEventManager)sharedInstance
{
  if (sharedInstance_onceToken_53966 != -1) {
    dispatch_once(&sharedInstance_onceToken_53966, &__block_literal_global_53967);
  }
...
uint64_t __53__HMDWatchConnectivityLogEventManager_sharedInstance__block_invoke()
{
  ...
  uint64_t v1 = sharedInstance_watchConnectivityLogEventManager;
