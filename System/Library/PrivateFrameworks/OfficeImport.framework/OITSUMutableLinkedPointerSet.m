...
{
  v5.receiver = self;
  v5.super_class = (Class)OITSUMutableLinkedPointerSet;
  v2 = [(OITSUMutableLinkedPointerSet *)&v5 init];
  if (v2)
  {
    CFMutableDictionaryRef Mutable = CFDictionaryCreateMutable(0, 0, 0, MEMORY[0x263EFFF90]);
    v2->mHead = 0;
    v2->mTail = 0;
...
- (void)dealloc
{
  ...
  v4.receiver = self;
  v4.super_class = (Class)OITSUMutableLinkedPointerSet;
...
- (id)objectEnumeratorAfterObject:(id)a3
{
  ...
  if (result)
  {
    objc_super v4 = [[OITSULinkedPointerSetEnumerator alloc] initWithFirstEntry:*((void *)result + 3)];
    return v4;
  }
  return result;
}
...
- (unint64_t)countByEnumeratingWithState:(id *)a3 objects:(id *)a4 count:(unint64_t)a5
{
  ...
  p_mHead = &self->mHead;
  if (a3->var0) {
    p_mHead = (OITSULinkedPointerSetEntry **)(a3->var0 + 24);
  }
  v7 = *p_mHead;
  unint64_t result = 0;
  if (v7) {
...
LABEL_10:
      ...
      while (v7)
      {
        ...
        if (mObject != *(id *)(v7 + 8))
        {
          int v6 = objc_msgSend(mObject, "isEqual:");
          if (!v6) {
            return v6;
          }
        }
...
- (void)addObject:(id)a3
{
  ...
  {
    uint64_t v5 = [[OITSULinkedPointerSetEntry alloc] initWithObject:a3 previousEntry:self->mTail];
    v8 = v5;
    mTail = self->mTail;
    if (mTail) {
      p_mNext = &mTail->mNext;
...
- (void)insertFirstObject:(id)a3
{
  -[OITSUMutableLinkedPointerSet removeObject:](self, "removeObject:");
  uint64_t v5 = [[OITSULinkedPointerSetEntry alloc] initWithObject:a3 previousEntry:0];
  mHead = self->mHead;
  uint64_t v7 = self;
  v8 = v5;
  if (mHead)
  {
    mHead->mPrevious = v5;
    v8 = self->mHead;
    uint64_t v7 = (OITSUMutableLinkedPointerSet *)v5;
  }
  v7->mTail = v8;
  self->mHead = v5;
  BOOL v9 = v5;
...
- (void)removeObject:(id)a3
{
  Value = (OITSULinkedPointerSetEntry *)CFDictionaryGetValue(self->mDictionary, a3);
  if (Value)
  {
    if (Value == self->mHead) {
...
- (id)array
{
  ...
  if (result)
  {
    ...
    int v6 = (void *)[MEMORY[0x263EFF8C0] arrayWithObjects:v5 count:Count];
    free(v5);
    return v6;
  }
  return result;
