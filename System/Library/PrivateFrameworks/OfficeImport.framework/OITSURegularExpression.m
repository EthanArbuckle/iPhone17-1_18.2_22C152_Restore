...
+ (void)initialize;
- (BOOL)isEqual:(id)a3;
- (OITSURegularExpression)init;
- (OITSURegularExpression)initWithCoder:(id)a3;
- (OITSURegularExpression)initWithExpressionString:(id)a3;
...
- (void)dealloc
{
  ...
  {
    MEMORY[0x23EC9A770]();
    NSZoneFree((NSZone *)[(OITSURegularExpression *)self zone], self->_reserved);
  }
  v3.receiver = self;
  v3.super_class = (Class)OITSURegularExpression;
  [(OITSURegularExpression *)&v3 dealloc];
}

- (OITSURegularExpression)initWithExpressionString:(id)a3
{
  return [(OITSURegularExpression *)self initWithExpressionString:a3 options:0];
}
...
- (OITSURegularExpression)initWithExpressionString:(id)a3 options:(unsigned int)a4
{
  ...
  v14.receiver = self;
  v14.super_class = (Class)OITSURegularExpression;
  id v6 = [(OITSURegularExpression *)&v14 init];
  if (v6)
  {
    size_t v7 = [a3 length];
    CFIndex v8 = v7;
    v9 = buffer;
    if (v7 >= 0x40) {
      v9 = (UInt8 *)malloc_type_malloc(v7, 0xCC2EE461uLL);
    }
    v17.location = 0;
    v17.length = v8;
    ...
    *((void *)v6 + 2) = v10;
    ...
    if (v12)
    {
      NSLog((NSString *)@"*** %s: error %d", "-[OITSURegularExpression initWithExpressionString:options:]", v12);
  ...
  return (OITSURegularExpression *)v6;
}
...
- (OITSURegularExpression)init
{
  return [(OITSURegularExpression *)self initWithExpressionString:&stru_26EBF24D8];
}

- (id)copyWithZone:(_NSZone *)a3
{
  if ((_NSZone *)[(OITSURegularExpression *)self zone] == a3)
  {
    return self;
  }
  else
...
- (_NSRange)matchedRangeForCString:(const char *)a3 range:(_NSRange)a4 subexpressionRanges:(_NSRange *)a5 count:(unsigned int)a6
{
  ...
  unint64_t v12 = [(OITSURegularExpression *)self numberOfSubexpressions];
  unint64_t v13 = a6;
  if (v12 >= a6) {
    unint64_t v14 = a6;
  }
...
- (_NSRange)matchedRangeForString:(id)a3 range:(_NSRange)a4 subexpressionRanges:(_NSRange *)a5 count:(unsigned int)a6
{
  ...
  uint64_t v12 = -[OITSURegularExpression matchedRangeForCString:range:subexpressionRanges:count:](self, "matchedRangeForCString:range:subexpressionRanges:count:", [(__CFData *)ASCIIData bytes], location, length, a5, v6);
  NSUInteger v14 = v13;
