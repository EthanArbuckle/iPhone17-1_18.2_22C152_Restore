...
- (CARMicaPlayer)initWithFileName:(id)a3 retinaScale:(double)a4
{
  ...
  {
    v9 = [v8 path];
    v10 = [(CARMicaPlayer *)self initWithPath:v9 retinaScale:a4];
    self = v9;
  }
  else
  {
...
{
  id v6 = a3;
  v7 = +[CARMicaPlayer rootDictForPath:v6];
  v8 = v7;
  if (!v7
...
LABEL_6:
  ...
  v10 = v9;
  objc_msgSend(v9, "setGeometryFlipped:", +[CARMicaPlayer BOOLFromDictionary:key:defaultValue:](CARMicaPlayer, "BOOLFromDictionary:key:defaultValue:", v8, @"geometryFlipped", 0) ^ 1);
  [v10 setMasksToBounds:1];
  [v10 setRepeatCount:0.0];
  v11 = [v8 objectForKeyedSubscript:@"publishedObjects"];
  v12 = [(CARMicaPlayer *)self initWithPath:v6 retinaScale:v10 rootLayer:v11 publishedObjects:a4];
...
- (CARMicaPlayer)initWithPath:(id)a3 retinaScale:(double)a4 rootLayer:(id)a5 publishedObjects:(id)a6
{
  ...
  if (v13)
  {
    [(CARMicaPlayer *)v13 setPath:v10];
    [(CARMicaPlayer *)v14 setRootLayer:v11];
    [(CARMicaPlayer *)v14 setPublishedObjects:v12];
    LODWORD(v15) = 1.0;
    [(CARMicaPlayer *)v14 setPreferredPlaybackSpeed:v15];
    [(CARMicaPlayer *)v14 setRetinaScale:a4];
    [v11 duration];
...
- (id)copyWithZone:(_NSZone *)a3
{
  v5 = [(CARMicaPlayer *)self path];
  id v6 = (void *)[v5 copy];

  v7 = [(CARMicaPlayer *)self rootLayer];
  v8 = objc_msgSend(v7, "car_deepCopyLayer");

  uint64_t v9 = [(CARMicaPlayer *)self publishedObjects];
  uint64_t v10 = [(CARMicaPlayer *)self rootLayer];
  uint64_t v11 = +[CARMicaPlayer updatePublishedObjects:v9 toReferenceLayersInTree:v8 ratherThanLayersInTree:v10];
...
+ (id)updatePublishedObjects:(id)a3 toReferenceLayersInTree:(id)a4 ratherThanLayersInTree:(id)a5
{
  ...
  if (v7 && [v7 count])
  {
    uint64_t v10 = objc_msgSend(v9, "car_allLayersInTree");
    id v24 = v9;
    v26 = objc_msgSend(v9, "car_allAnimationsInTree");
    uint64_t v11 = objc_msgSend(v8, "car_allLayersInTree");
    v25 = objc_msgSend(v8, "car_allAnimationsInTree");
    long long v29 = 0u;
    long long v30 = 0u;
    long long v31 = 0u;
    long long v32 = 0u;
...
- (void)addToLayer:(id)a3 onTop:(BOOL)a4 gravity:(id)a5
{
  ...
  if (!v9)
  {
    ...
    [(CARMicaPlayer *)self moveAndResizeWithinParentLayer:v11 usingGravity:v8 animate:0];
...
- (id)publishedLayerWithKey:(id)a3 required:(BOOL)a4
{
  v4 = [(CARMicaPlayer *)self publishedObjectWithKey:a3 required:a4];
  if (v4)
  {
    objc_opt_class();
...
- (void)setPlaybackTime:(double)a3
{
  BOOL v5 = [(CARMicaPlayer *)self isPlaying];
  double v6 = 0.0;
  if (v5)
  {
...
- (BOOL)isTimerNeeded
{
  if ([(CARMicaPlayer *)self isPlaying])
  {
    ...
    if (v3 == INFINITY)
    {
      id v5 = [(CARMicaPlayer *)self delegate];
      if (v5)
      {
        char v6 = [(CARMicaPlayer *)self delegate];
...
{
  id WeakRetained = objc_loadWeakRetained((id *)&self->_delegate);
  return (CARMicaPlayerDelegate *)WeakRetained;
}
