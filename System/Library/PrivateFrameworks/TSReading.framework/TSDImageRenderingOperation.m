...
- (TSDImageRenderingOperation)initWithPreset:(id)a3 imageSize:(CGSize)a4 imageScale:(double)a5 swatchFrame:(CGRect)a6 imageInfo:(id)a7 documentRoot:(id)a8;
...
{
  v13.receiver = self;
  v13.super_class = (Class)TSDImageRenderingOperation;
  ...
  if (v11)
  {
    v11->mImageInfo = (TSDImageInfo *)a7;
...
- (TSDImageRenderingOperation)initWithPreset:(id)a3 imageSize:(CGSize)a4 imageScale:(double)a5 swatchFrame:(CGRect)a6 imageInfo:(id)a7 documentRoot:(id)a8
{
  return -[TSDImageRenderingOperation initWithPreset:imageSize:imageScale:swatchFrame:imageInfo:documentRoot:shouldClipVertically:](self, "initWithPreset:imageSize:imageScale:swatchFrame:imageInfo:documentRoot:shouldClipVertically:", a3, a7, a8, 0, a4.width, a4.height, a5, a6.origin.x, a6.origin.y, a6.size.width, a6.size.height);
}

- (void)dealloc
{
  v3.receiver = self;
  v3.super_class = (Class)TSDImageRenderingOperation;
...
- (void)doWorkWithReadLock
{
  ...
  if (v3) {
    v8 = objc_msgSend(v4, "imageForImagePreset:imageSize:imageScale:swatchFrame:imageInfo:shouldClipVertically:documentRoot:", mPreset, self->mImageInfo, self->mShouldClipVertically, self->super.mDocumentRoot, p_mImageSize->width, self->super.mImageSize.height, mImageScale, self->super.mSwatchFrame.origin.x, self->super.mSwatchFrame.origin.y, self->super.mSwatchFrame.size.width, self->super.mSwatchFrame.size.height);
  }
  else {
    v8 = objc_msgSend(v4, "imageForImagePreset:imageSize:imageScale:imageInfo:shouldClipVertically:documentRoot:", mPreset, self->mImageInfo, self->mShouldClipVertically, self->super.mDocumentRoot, p_mImageSize->width, self->super.mImageSize.height, mImageScale);
  }
...
- (TSDImageInfo)imageInfo
{
  return (TSDImageInfo *)objc_getProperty(self, a2, 360, 1);
}
