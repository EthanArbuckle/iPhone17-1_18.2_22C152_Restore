@interface SKSetupAppleIDSignInServer
...
- (void)_oobEnsureStarted
{
  if (!self->super._skCnx)
  {
    ...
    [(SKSetupBase *)self _connectionStartWithSKConnection:v3 clientMode:0 completeOnFailure:0 completion:&__block_literal_global_22_150];
  }
}
...
LABEL_4:
  ...
  else
  {
    ...
    [(SKSetupBase *)self _connectionStartWithSKConnection:v5 clientMode:0 completeOnFailure:0 completion:&__block_literal_global_153];
  }
LABEL_8:
}
...
- (void)_bleServerEnsureStarted
{
  ...
  if (!self->_bleServer)
  {
    ...
    v8[1] = 3221225472;
    v8[2] = __53__SKSetupAppleIDSignInServer__bleServerEnsureStarted__block_invoke;
    v8[3] = &unk_264548FF8;
    v8[4] = v4;
    v8[5] = self;
    ...
    v7[1] = 3221225472;
    v7[2] = __53__SKSetupAppleIDSignInServer__bleServerEnsureStarted__block_invoke_2;
    v7[3] = &unk_2645496B0;
    v7[4] = v4;
    v7[5] = self;
...
uint64_t __53__SKSetupAppleIDSignInServer__bleServerEnsureStarted__block_invoke_2(uint64_t a1, void *a2)
{
  id v3 = a2;
...
uint64_t __57__SKSetupAppleIDSignInServer__bleAdvertiserEnsureStarted__block_invoke(uint64_t a1, void *a2)
{
  id v3 = a2;
  uint64_t v4 = *(void *)(a1 + 40);
...
- (void)_run
{
  if ([(SKSetupAppleIDSignInServer *)self _bleAdvertiserShouldRun])
  {
    [(SKSetupAppleIDSignInServer *)self _bleAdvertiserEnsureStarted];
  }
  else
  {
    [(SKSetupAppleIDSignInServer *)self _bleAdvertiserEnsureStopped];
  }
}
...
{
  [(SKSetupAppleIDSignInServer *)self _bleAdvertiserEnsureStopped];
  [(SKSetupAppleIDSignInServer *)self _bleServerEnsureStopped];
  v3.receiver = self;
...
- (void)_activate
{
  if (self->super._sendDataHandler) {
    [(SKSetupAppleIDSignInServer *)self _oobEnsureStarted];
  }
  else {
    [(SKSetupAppleIDSignInServer *)self _bleServerEnsureStarted];
  }
  [(SKSetupAppleIDSignInServer *)self _prepareSteps];
  v3.receiver = self;
  v3.super_class = (Class)SKSetupAppleIDSignInServer;
...
- (SKSetupAppleIDSignInServer)init
{
  ...
  int v2 = [(SKSetupBase *)&v6 initWithLogCategory:&gLogCategory_SKSetupAppleIDSignInServer];
  objc_super v3 = v2;
  if (v2) {
    uint64_t v4 = v2;
  }
