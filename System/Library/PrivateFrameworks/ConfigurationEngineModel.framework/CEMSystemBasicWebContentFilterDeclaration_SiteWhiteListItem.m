@interface CEMSystemBasicWebContentFilterDeclaration_SiteWhiteListItem
+ (NSSet)allowedPayloadKeys;
+ (id)buildRequiredOnlyWithAddress:(id)a3;
+ (id)buildWithAddress:(id)a3 withPageTitle:(id)a4;
- (BOOL)loadPayload:(id)a3 error:(id *)a4;
- (NSString)payloadAddress;
- (NSString)payloadBookmarkPath;
- (NSString)payloadPageTitle;
- (id)copyWithZone:(_NSZone *)a3;
- (id)serializePayloadWithAssetProviders:(id)a3;
- (void)setPayloadAddress:(id)a3;
- (void)setPayloadBookmarkPath:(id)a3;
- (void)setPayloadPageTitle:(id)a3;
@end

@implementation CEMSystemBasicWebContentFilterDeclaration_SiteWhiteListItem

+ (NSSet)allowedPayloadKeys
{
  v6[3] = *MEMORY[0x263EF8340];
  v2 = (void *)MEMORY[0x263EFFA08];
  v6[0] = @"address";
  v6[1] = @"bookmarkPath";
  v6[2] = @"pageTitle";
  v3 = [MEMORY[0x263EFF8C0] arrayWithObjects:v6 count:3];
  v4 = [v2 setWithArray:v3];

  return (NSSet *)v4;
}

+ (id)buildWithAddress:(id)a3 withPageTitle:(id)a4
{
  id v5 = a4;
  id v6 = a3;
  v7 = objc_opt_new();
  [v7 setPayloadAddress:v6];

  [v7 setPayloadPageTitle:v5];

  return v7;
}

+ (id)buildRequiredOnlyWithAddress:(id)a3
{
  id v3 = a3;
  v4 = objc_opt_new();
  [v4 setPayloadAddress:v3];

  return v4;
}

- (BOOL)loadPayload:(id)a3 error:(id *)a4
{
  id v6 = a3;
  v7 = (void *)MEMORY[0x263EFF9C0];
  v8 = [v6 allKeys];
  v9 = [v7 setWithArray:v8];

  v10 = +[CEMSystemBasicWebContentFilterDeclaration_SiteWhiteListItem allowedPayloadKeys];
  [v9 minusSet:v10];

  v11 = (NSSet *)[v9 copy];
  unknownPayloadKeys = self->super._unknownPayloadKeys;
  self->super._unknownPayloadKeys = v11;

  id v23 = 0;
  v13 = [(CEMPayloadBase *)self loadStringFromDictionary:v6 withKey:@"address" isRequired:1 defaultValue:0 error:&v23];
  id v14 = v23;
  payloadAddress = self->_payloadAddress;
  self->_payloadAddress = v13;

  if (!v14)
  {
    id v22 = 0;
    v16 = [(CEMPayloadBase *)self loadStringFromDictionary:v6 withKey:@"bookmarkPath" isRequired:0 defaultValue:0 error:&v22];
    id v14 = v22;
    payloadBookmarkPath = self->_payloadBookmarkPath;
    self->_payloadBookmarkPath = v16;

    if (!v14)
    {
      id v21 = 0;
      v18 = [(CEMPayloadBase *)self loadStringFromDictionary:v6 withKey:@"pageTitle" isRequired:0 defaultValue:0 error:&v21];
      id v14 = v21;
      payloadPageTitle = self->_payloadPageTitle;
      self->_payloadPageTitle = v18;
    }
  }
  if (a4 && v14) {
    *a4 = v14;
  }

  return v14 == 0;
}

- (id)serializePayloadWithAssetProviders:(id)a3
{
  v4 = objc_msgSend(MEMORY[0x263EFF9A0], "dictionary", a3);
  [(CEMPayloadBase *)self serializeStringIntoDictionary:v4 withKey:@"address" withValue:self->_payloadAddress isRequired:1 defaultValue:0];
  [(CEMPayloadBase *)self serializeStringIntoDictionary:v4 withKey:@"bookmarkPath" withValue:self->_payloadBookmarkPath isRequired:0 defaultValue:0];
  [(CEMPayloadBase *)self serializeStringIntoDictionary:v4 withKey:@"pageTitle" withValue:self->_payloadPageTitle isRequired:0 defaultValue:0];
  id v5 = (void *)[v4 copy];

  return v5;
}

- (id)copyWithZone:(_NSZone *)a3
{
  v12.receiver = self;
  v12.super_class = (Class)CEMSystemBasicWebContentFilterDeclaration_SiteWhiteListItem;
  v4 = [(CEMPayloadBase *)&v12 copyWithZone:a3];
  uint64_t v5 = [(NSString *)self->_payloadAddress copy];
  id v6 = (void *)v4[2];
  v4[2] = v5;

  uint64_t v7 = [(NSString *)self->_payloadBookmarkPath copy];
  v8 = (void *)v4[3];
  v4[3] = v7;

  uint64_t v9 = [(NSString *)self->_payloadPageTitle copy];
  v10 = (void *)v4[4];
  v4[4] = v9;

  return v4;
}

- (NSString)payloadAddress
{
  return self->_payloadAddress;
}

- (void)setPayloadAddress:(id)a3
{
}

- (NSString)payloadBookmarkPath
{
  return self->_payloadBookmarkPath;
}

- (void)setPayloadBookmarkPath:(id)a3
{
}

- (NSString)payloadPageTitle
{
  return self->_payloadPageTitle;
}

- (void)setPayloadPageTitle:(id)a3
{
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_payloadPageTitle, 0);
  objc_storeStrong((id *)&self->_payloadBookmarkPath, 0);

  objc_storeStrong((id *)&self->_payloadAddress, 0);
}

@end