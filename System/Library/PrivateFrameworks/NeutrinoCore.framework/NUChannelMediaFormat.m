...
- (id)debugDescription
{
  ...
  int64_t v5 = [(NUChannelMediaFormat *)self mediaType];
  if ((unint64_t)(v5 - 1) > 3) {
    v6 = @"???";
  }
  else {
    v6 = off_1E5D94268[v5 - 1];
  }
...
{
  v2 = NSString;
  int64_t v3 = [(NUChannelMediaFormat *)self mediaType];
  if ((unint64_t)(v3 - 1) > 3) {
    uint64_t v4 = @"???";
  }
  else {
    uint64_t v4 = off_1E5D94268[v3 - 1];
  }
  return (id)[v2 stringWithFormat:@"media:%@", v4];
}
...
- (BOOL)isCompatibleWithChannelFormat:(id)a3
{
  ...
  if (objc_opt_isKindOfClass())
  {
    ...
    BOOL v6 = v5 == [(NUChannelMediaFormat *)self mediaType];
  }
  else
  {
    BOOL v6 = 0;
...
- (unint64_t)hash
{
  ...
  return (0x73F763B1ABLL * self->_mediaType) ^ [(NUChannelFormat *)&v3 hash];
}
...
LABEL_15:
  ...
  _NUAssertFailHandler((uint64_t)"-[NUChannelMediaFormat init]", (uint64_t)"/Library/Caches/com.apple.xbs/Sources/Photos/workspaces/neutrino/Core/Pipeline/API/NUChannel.m", 258, @"Initializer not available: -[%@ %@], use designated initializer instead.", v24, v25, v26, v27, v23);
}
