@interface ICASSnapshotHealthPrivateEventData
+ (BOOL)requiresDiagnosticsConsent;
+ (NSString)dataName;
- (ICASSnapshotHealthPrivateEventData)initWithSnapshotHealthPrivateSessionID:(id)a3;
- (NSString)snapshotHealthPrivateSessionID;
- (id)toDict;
@end

@implementation ICASSnapshotHealthPrivateEventData

- (ICASSnapshotHealthPrivateEventData)initWithSnapshotHealthPrivateSessionID:(id)a3
{
  id v5 = a3;
  v9.receiver = self;
  v9.super_class = (Class)ICASSnapshotHealthPrivateEventData;
  v6 = [(ICASSnapshotHealthPrivateEventData *)&v9 init];
  v7 = v6;
  if (v6) {
    objc_storeStrong((id *)&v6->_snapshotHealthPrivateSessionID, a3);
  }

  return v7;
}

+ (NSString)dataName
{
  return (NSString *)@"SnapshotHealthPrivateEventData";
}

+ (BOOL)requiresDiagnosticsConsent
{
  return 0;
}

- (id)toDict
{
  v9[1] = *MEMORY[0x1E4F143B8];
  v8 = @"snapshotHealthPrivateSessionID";
  v3 = [(ICASSnapshotHealthPrivateEventData *)self snapshotHealthPrivateSessionID];
  if (v3)
  {
    uint64_t v4 = [(ICASSnapshotHealthPrivateEventData *)self snapshotHealthPrivateSessionID];
  }
  else
  {
    uint64_t v4 = objc_opt_new();
  }
  id v5 = (void *)v4;
  v9[0] = v4;
  v6 = [MEMORY[0x1E4F1C9E8] dictionaryWithObjects:v9 forKeys:&v8 count:1];

  return v6;
}

- (NSString)snapshotHealthPrivateSessionID
{
  return self->_snapshotHealthPrivateSessionID;
}

- (void).cxx_destruct
{
}

@end