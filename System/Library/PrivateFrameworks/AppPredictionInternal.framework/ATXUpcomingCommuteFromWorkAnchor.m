...
+ (id)longDescription
{
  return @"The user is preparing to return home.";
}
...
+ (id)fetchAnchorOccurrencesBetweenStartDate:(id)a3 endDate:(id)a4
{
  ...
  v6 = objc_msgSend(v5, "_pas_filteredArrayWithTest:", &__block_literal_global_128);
...
{
  id v2 = a2;
  BOOL v3 = [v2 origin] == 2 && objc_msgSend(v2, "destination") == 1;
...
+ (id)predicateForContextStoreRegistration
{
  ...
  double v7 = [v3 predicateForKeyPath:v2, @"SUBQUERY(SELF.%@.value, $prediction, $prediction.%K = %@ AND $prediction.%K <= %@ AND $prediction.%K > %@).@count > 0", v2, v4, &unk_1F27F17E8, v5, &unk_1F27F1800, v6, &unk_1F27F1818 withFormat];
...
+ (id)sampleEvent
{
  ...
  id v5 = [(ATXTripDuetEvent *)v2 initWithOrigin:2 destination:1 durationAtOrigin:v3 startDate:v4 endDate:0.0];
