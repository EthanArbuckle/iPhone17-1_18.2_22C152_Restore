...
- (id)description
{
  ...
  int v4 = [(ATXMPBAppClipSuppressionTracker *)&v8 description];
...
- (id)dictionaryRepresentation
{
  ...
  if (*(unsigned char *)&self->_has)
  {
    uint64_t suppressionType = self->_suppressionType;
    if (suppressionType >= 3)
    {
      objc_msgSend(NSString, "stringWithFormat:", @"(unknown: %i)", self->_suppressionType);
      v5 = (__CFString *)objc_claimAutoreleasedReturnValue();
    }
    else
    {
      v5 = off_1E68B4460[suppressionType];
    }
...
- (BOOL)isEqual:(id)a3
{
  ...
  if (*(unsigned char *)&self->_has)
  {
    if ((*((unsigned char *)v4 + 32) & 1) == 0 || self->_suppressionType != *((_DWORD *)v4 + 4)) {
      goto LABEL_11;
    }
  }
...
{
  if (*(unsigned char *)&self->_has) {
    uint64_t v3 = 2654435761 * self->_suppressionType;
  }
  else {
    uint64_t v3 = 0;
  }
...
- (void)mergeFrom:(id)a3
{
  ...
  if (v4[8])
  {
    self->_uint64_t suppressionType = v4[4];
    *(unsigned char *)&self->_has |= 1u;
  }
  uint64_t v5 = v4;
  if (*((void *)v4 + 1))
