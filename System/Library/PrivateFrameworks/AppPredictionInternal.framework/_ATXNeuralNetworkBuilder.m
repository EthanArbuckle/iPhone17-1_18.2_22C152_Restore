...
{
  v11.receiver = self;
  v11.super_class = (Class)_ATXNeuralNetworkBuilder;
...
- (void)addHiddenLayer:(unint64_t)a3 weights:(const float *)a4 bias:(const float *)a5 activation:(unsigned __int8)a6 skip:(BOOL)a7
{
  ...
  if (self->_done)
  {
    ...
    [v21 handleFailureInMethod:a2, self, @"_ATXNeuralNetwork.m", 194, @"Invalid parameter not satisfying: %@", @"!_done" object file lineNumber description];
  ...
  [v22 handleFailureInMethod:a2, self, @"_ATXNeuralNetwork.m", 195, @"Invalid parameter not satisfying: %@", @"outputSize > 0" object file lineNumber description];
...
- (void)addOutputLayer:(unint64_t)a3 weights:(const float *)a4 bias:(const float *)a5 activation:(unsigned __int8)a6 softmax:(BOOL)a7
{
  ...
  if (self->_haveOutputLayer)
  {
    ...
    [v20 handleFailureInMethod:a2, self, @"_ATXNeuralNetwork.m", 213, @"Invalid parameter not satisfying: %@", @"!_haveOutputLayer" object file lineNumber description];
  }
  if (self->_done)
  {
    v21 = [MEMORY[0x1E4F28B00] currentHandler];
    [v21 handleFailureInMethod:a2, self, @"_ATXNeuralNetwork.m", 214, @"Invalid parameter not satisfying: %@", @"!_done" object file lineNumber description];
  }
...
- (id)serialize
{
  if (!self->_haveOutputLayer)
  {
    ...
    [v14 handleFailureInMethod:a2, self, @"_ATXNeuralNetwork.m", 243, @"Invalid parameter not satisfying: %@", @"_haveOutputLayer" object file lineNumber description];
  }
  if (self->_done)
  {
    id v15 = [MEMORY[0x1E4F28B00] currentHandler];
    [v15 handleFailureInMethod:a2, self, @"_ATXNeuralNetwork.m", 244, @"Invalid parameter not satisfying: %@", @"!_done" object file lineNumber description];
  }
  v4 = objc_opt_new();
  ...
  if (self->_nlayers)
  {
    ...
    do
    {
      uint64_t v8 = [(NSMutableArray *)self->_weightsAndBiases objectAtIndexedSubscript:v7];
