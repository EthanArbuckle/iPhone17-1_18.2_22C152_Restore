...
- (void)deleteCache
{
  ...
  int v2 = 138412290;
  uint64_t v3 = a1;
  _os_log_debug_impl(&dword_218640000, a2, OS_LOG_TYPE_DEBUG, "Cleared RK cache with error: %@", (uint8_t *)&v2, 0xCu);
}
...
- (void)cacheRecoveryKey:(os_log_t)log forAltDSID:error:.cold.1(os_log_t log)
{
  ...
  int v1 = 138412290;
  uint64_t v2 = 0;
  _os_log_debug_impl(&dword_218640000, log, OS_LOG_TYPE_DEBUG, "Cached RK with error: %@", (uint8_t *)&v1, 0xCu);
}
...
- (void)fetchRecoveryKeyFromCacheForAltDSID:(uint64_t)a1 .cold.1(uint64_t a1, NSObject *a2)
{
  ...
  int v2 = 138412290;
  uint64_t v3 = a1;
  _os_log_error_impl(&dword_218640000, a2, OS_LOG_TYPE_ERROR, "Failed to fetch RK from cache: %@", (uint8_t *)&v2, 0xCu);
}
