...
- (id)matchingJobsWhereSQLClause
{
  return (id)[MEMORY[0x263F8C710] formatInjection:@"throttle_id = %lld AND zone_rowid = %@", self->_intID, self->_zoneRowID];
}

- (id)columns
{
  return (id)objc_msgSend(MEMORY[0x263F8C720], "rawInjection:length:", "throttle_id, zone_rowid", 23);
}

- (id)columnsValues
{
  return (id)[MEMORY[0x263F8C710] formatInjection:@"%lld, %@", self->_intID, self->_zoneRowID];
}
...
- (void)initWithIntID:(uint64_t)a1 zoneRowID:(NSObject *)a2 .cold.1(uint64_t a1, NSObject *a2)
{
  ...
  int v2 = 138412290;
  uint64_t v3 = a1;
  _os_log_fault_impl(&dword_23FA42000, a2, OS_LOG_TYPE_FAULT, "[CRIT] Assertion failed: zoneRowID%@", (uint8_t *)&v2, 0xCu);
}
