...
- (id)fallbackParentAppLibraryOnFS
{
  ...
  v10 = [(BRCAccountSessionFPFS *)session fallbackAppLibraryForClientZone:v5 extension:v9];
...
- (id)fallbackParentItemOnFS
{
  ...
  else
  {
    ...
    if (os_log_type_enabled(v10, OS_LOG_TYPE_FAULT))
    {
      ...
      __int16 v21 = 2112;
      v22 = v9;
      _os_log_fault_impl(&dword_23FA42000, v10, OS_LOG_TYPE_FAULT, "[CRIT] UNREACHABLE: can't find %@ in %@%@", (uint8_t *)&v17, 0x20u);
    }
    v7 = [(BRCAccountSessionFPFS *)self->super._session appLibraryByID:*MEMORY[0x263F324E8]];
...
LABEL_5:
  ...
  v6 = [(BRCAccountSessionFPFS *)self->super._session serverAliasItemForSharedItem:self];
  char v3 = v6;
  if (v6)
  {
    v4 = [v6 clientZone];
...
- (id)parentItemIDOnFSInDB:(id)a3
{
  ...
  if (os_log_type_enabled(v6, OS_LOG_TYPE_DEBUG))
  {
    ...
    *(_DWORD *)buf = 134218498;
    ...
    __int16 v22 = 2112;
    uint64_t v23 = v5;
    _os_log_debug_impl(&dword_23FA42000, v6, OS_LOG_TYPE_DEBUG, "[DEBUG] ┏%llx computing the parent item id of %@%@", buf, 0x20u);
  }
  if ([(BRCSharedServerItem *)self isSharedToMeChildItem])
  {
  ...
  {
    v8 = [(BRCAccountSessionFPFS *)self->super._session serverAliasItemForSharedItem:self db:v4];
    if (v8)
    {
      v9 = brc_bread_crumbs();
      v10 = brc_default_log();
...
- (id)aliasDerivedStructure
{
  ...
  {
    id v4 = [(BRCAccountSessionFPFS *)self->super._session serverAliasItemForSharedItem:self db:self->super._db];
    id v5 = (id)[v3 copy];
    v6 = [v4 st];
    v7 = [v6 ckInfo];
    [v5 setCkInfo:v7];
...
- (id)parentZoneOnFS
{
  ...
  {
    id v4 = [(BRCAccountSessionFPFS *)self->super._session serverAliasItemForSharedItem:self];
    id v5 = v4;
    if (v4)
    {
...
- (void)fallbackParentItemOnFS
{
  ...
  int v2 = 138412290;
  uint64_t v3 = a1;
  _os_log_fault_impl(&dword_23FA42000, a2, OS_LOG_TYPE_FAULT, "[CRIT] Assertion failed: fallbackItem%@", (uint8_t *)&v2, 0xCu);
}
...
- (void)parentItemIDOnFSInDB:(uint8_t *)buf .cold.1(void *a1, uint64_t a2, uint8_t *buf, os_log_t log)
{
  ...
  *((_WORD *)buf + 6) = 2112;
  *(void *)(buf + 14) = a2;
  _os_log_debug_impl(&dword_23FA42000, log, OS_LOG_TYPE_DEBUG, "[DEBUG] falling back to an app library documents root: %@%@", buf, 0x16u);
}
...
- (void)parentItemIDOnFSInDB:(os_log_t)log .cold.2(uint64_t a1, uint64_t a2, os_log_t log)
{
  ...
  __int16 v5 = 2112;
  uint64_t v6 = a2;
  _os_log_debug_impl(&dword_23FA42000, log, OS_LOG_TYPE_DEBUG, "[DEBUG] found the alias representing this item: %@%@", (uint8_t *)&v3, 0x16u);
}
