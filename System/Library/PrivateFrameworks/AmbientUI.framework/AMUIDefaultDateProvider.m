...
- (AMUIDefaultDateProvider)init
{
  ...
  if (v2)
  {
    uint64_t v3 = [MEMORY[0x263EFF8F0] autoupdatingCurrentCalendar];
...
- (void)_scheduleNextMinuteTimer
{
  uint64_t v3 = [(AMUIDefaultDateProvider *)self date];
  uint64_t v11 = 0;
  uint64_t v12 = 0;
  uint64_t v9 = 0;
  uint64_t v10 = 0;
  [(NSCalendar *)self->_calendar getHour:&v12 minute:&v11 second:&v10 nanosecond:&v9 fromDate:v3];
  double v4 = 60.0 - ((double)v9 / 1000000000.0 + (double)v10);
  [(NSTimer *)self->_minuteTimer invalidate];
  v5 = [v3 dateByAddingTimeInterval:v4];
  objc_super v6 = (NSTimer *)[objc_alloc(MEMORY[0x263EFFA20]) initWithFireDate:v5 interval:self target:sel__minuteTimerFired selector:0 userInfo:0 repeats:0.0];
  minuteTimer = self->_minuteTimer;
  self->_minuteTimer = v6;

  v8 = [MEMORY[0x263EFF9F0] mainRunLoop];
  [v8 addTimer:self->_minuteTimer forMode:*MEMORY[0x263EFF588]];
}
