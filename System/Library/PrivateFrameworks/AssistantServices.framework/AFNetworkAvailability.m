...
+ (id)sharedAvailability
{
  if (sharedAvailability_onceToken != -1) {
    dispatch_once(&sharedAvailability_onceToken, &__block_literal_global_19082);
  }
...
unsigned char *__36__AFNetworkAvailability_isAvailable__block_invoke(uint64_t a1)
{
...
_DWORD *__37__AFNetworkAvailability_addObserver___block_invoke(uint64_t a1)
{
  ...
  if (!v2)
  {
    uint64_t v3 = [objc_alloc(MEMORY[0x1E4F28D30]) initWithOptions:5 capacity:0];
...
LABEL_9:
  ...
  char v9 = v6 & 0xFD;
LABEL_10:
  *(unsigned char *)p_char state = v9;
LABEL_12:
  MEMORY[0x1F40CD0D0](v4);
}
...
- (AFNetworkAvailability)init
{
  ...
  if (v2)
  {
    ...
    dispatch_queue_t v5 = dispatch_queue_create("AFNetworkAvailability", v4);
    queue = v2->_queue;
