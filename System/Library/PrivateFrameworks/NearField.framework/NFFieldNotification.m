@interface NFFieldNotification
+ (BOOL)supportsSecureCoding;
+ (id)fieldNotificationFromXPCObject:(id)a3;
+ (id)notificationWithDictionary:(id)a3;
- (BOOL)cachedBeforeRFReset;
- (BOOL)isCHAutoNegotiationField;
- (BOOL)isCHInitiatorDetected;
- (BOOL)isCHReceiverDetected;
- (BOOL)isCHTerminal;
- (BOOL)isEqual:(id)a3;
- (BOOL)isEqualTerminalInfo:(id)a3;
- (BOOL)isEqualWithoutRFTech:(id)a3;
- (NFFieldNotification)initWithCoder:(id)a3;
- (NFFieldNotification)initWithDictionary:(id)a3;
- (NFFieldNotification)initWithNotificationType:(unint64_t)a3 rfTechnology:(unsigned int)a4 typeFSystemCode:(unsigned __int16)a5 creationDate:(id)a6 cachedBeforeRFReset:(BOOL)a7;
- (NSDate)creationDate;
- (id)_creationDateString;
- (id)asXPCObject;
- (id)chRandomData;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
- (int64_t)category;
- (unint64_t)chFieldType;
- (unint64_t)notificationType;
- (unsigned)rfTechnology;
- (unsigned)typeFSystemCode;
- (void)encodeWithCoder:(id)a3;
- (void)setCachedBeforeRFReset:(BOOL)a3;
- (void)setCategory:(int64_t)a3;
- (void)setCreationDate:(id)a3;
- (void)setNotificationType:(unint64_t)a3;
@end

@implementation NFFieldNotification

- (unsigned)rfTechnology
{
  return self->_rfTechnology;
}

- (unint64_t)notificationType
{
  return self->_notificationType;
}

- (void).cxx_destruct
{
}

- (NFFieldNotification)initWithCoder:(id)a3
{
  id v4 = a3;
  v9.receiver = self;
  v9.super_class = (Class)NFFieldNotification;
  v5 = [(NFFieldNotification *)&v9 init];
  if (v5)
  {
    v5->_rfTechnology = [v4 decodeIntegerForKey:@"rfTechnology"];
    v5->_typeFSystemCode = [v4 decodeIntegerForKey:@"typeFSystemCode"];
    v5->_notificationType = [v4 decodeIntegerForKey:@"notificationType"];
    uint64_t v6 = [v4 decodeObjectOfClass:objc_opt_class() forKey:@"creationDate"];
    creationDate = v5->_creationDate;
    v5->_creationDate = (NSDate *)v6;

    v5->_cachedBeforeRFReset = [v4 decodeBoolForKey:@"cachedBeforeRFReset"];
    v5->_category = [v4 decodeIntegerForKey:@"category"];
  }

  return v5;
}

+ (id)fieldNotificationFromXPCObject:(id)a3
{
  uint64_t v61 = *MEMORY[0x1E4F143B8];
  id v5 = a3;
  if (object_getClass(v5) != (Class)MEMORY[0x1E4F14580])
  {
    uint64_t v6 = (const void **)MEMORY[0x1E4FBA898];
    dispatch_get_specific((const void *)*MEMORY[0x1E4FBA898]);
    uint64_t Logger = NFLogGetLogger();
    if (Logger)
    {
      v8 = (void (*)(uint64_t, const char *, ...))Logger;
      Class = object_getClass(a1);
      BOOL isMetaClass = class_isMetaClass(Class);
      ClassName = object_getClassName(a1);
      Name = sel_getName(a2);
      uint64_t v11 = 45;
      if (isMetaClass) {
        uint64_t v11 = 43;
      }
      v8(3, "%c[%{public}s %{public}s]:%i Unexpected parameter: %@", v11, ClassName, Name, 98, v5);
    }
    dispatch_get_specific(*v6);
    v12 = NFSharedLogGetLogger();
    if (!os_log_type_enabled(v12, OS_LOG_TYPE_ERROR)) {
      goto LABEL_12;
    }
    v13 = object_getClass(a1);
    if (class_isMetaClass(v13)) {
      int v14 = 43;
    }
    else {
      int v14 = 45;
    }
    *(_DWORD *)buf = 67110146;
    int v52 = v14;
    __int16 v53 = 2082;
    v54 = object_getClassName(a1);
    __int16 v55 = 2082;
    v56 = sel_getName(a2);
    __int16 v57 = 1024;
    int v58 = 98;
    __int16 v59 = 2112;
    id v60 = v5;
LABEL_11:
    _os_log_impl(&dword_19D636000, v12, OS_LOG_TYPE_ERROR, "%c[%{public}s %{public}s]:%i Unexpected parameter: %@", buf, 0x2Cu);
LABEL_12:
    id v15 = 0;
    goto LABEL_36;
  }
  uint64_t v16 = _CFXPCCreateCFObjectFromXPCObject();
  if (!v16)
  {
    v36 = (const void **)MEMORY[0x1E4FBA898];
    dispatch_get_specific((const void *)*MEMORY[0x1E4FBA898]);
    uint64_t v37 = NFLogGetLogger();
    if (v37)
    {
      v38 = (void (*)(uint64_t, const char *, ...))v37;
      v39 = object_getClass(a1);
      BOOL v40 = class_isMetaClass(v39);
      v46 = object_getClassName(a1);
      v49 = sel_getName(a2);
      uint64_t v41 = 45;
      if (v40) {
        uint64_t v41 = 43;
      }
      v38(3, "%c[%{public}s %{public}s]:%i Unexpected parameter: %@", v41, v46, v49, 104, v5);
    }
    dispatch_get_specific(*v36);
    v12 = NFSharedLogGetLogger();
    if (!os_log_type_enabled(v12, OS_LOG_TYPE_ERROR)) {
      goto LABEL_12;
    }
    v42 = object_getClass(a1);
    if (class_isMetaClass(v42)) {
      int v43 = 43;
    }
    else {
      int v43 = 45;
    }
    *(_DWORD *)buf = 67110146;
    int v52 = v43;
    __int16 v53 = 2082;
    v54 = object_getClassName(a1);
    __int16 v55 = 2082;
    v56 = sel_getName(a2);
    __int16 v57 = 1024;
    int v58 = 104;
    __int16 v59 = 2112;
    id v60 = v5;
    goto LABEL_11;
  }
  v12 = v16;
  v17 = (void *)MEMORY[0x1E4F1CAD0];
  uint64_t v18 = objc_opt_class();
  uint64_t v19 = objc_opt_class();
  v20 = objc_msgSend(v17, "setWithObjects:", v18, v19, objc_opt_class(), 0);
  id v50 = 0;
  v21 = [MEMORY[0x1E4F28DC0] unarchivedArrayOfObjectsOfClasses:v20 fromData:v12 error:&v50];
  id v22 = v50;
  if (v22)
  {
    v23 = (const void **)MEMORY[0x1E4FBA898];
    dispatch_get_specific((const void *)*MEMORY[0x1E4FBA898]);
    uint64_t v24 = NFLogGetLogger();
    if (v24)
    {
      v25 = (void (*)(uint64_t, const char *, ...))v24;
      v26 = object_getClass(a1);
      BOOL v27 = class_isMetaClass(v26);
      v28 = object_getClassName(a1);
      v48 = sel_getName(a2);
      BOOL v29 = !v27;
      v23 = (const void **)MEMORY[0x1E4FBA898];
      uint64_t v30 = 45;
      if (!v29) {
        uint64_t v30 = 43;
      }
      v25(3, "%c[%{public}s %{public}s]:%i Fail to decode: %@", v30, v28, v48, 119, v22);
    }
    dispatch_get_specific(*v23);
    v31 = NFSharedLogGetLogger();
    if (os_log_type_enabled(v31, OS_LOG_TYPE_ERROR))
    {
      v32 = object_getClass(a1);
      if (class_isMetaClass(v32)) {
        int v33 = 43;
      }
      else {
        int v33 = 45;
      }
      v34 = object_getClassName(a1);
      v35 = sel_getName(a2);
      *(_DWORD *)buf = 67110146;
      int v52 = v33;
      __int16 v53 = 2082;
      v54 = v34;
      __int16 v55 = 2082;
      v56 = v35;
      __int16 v57 = 1024;
      int v58 = 119;
      __int16 v59 = 2112;
      id v60 = v22;
      _os_log_impl(&dword_19D636000, v31, OS_LOG_TYPE_ERROR, "%c[%{public}s %{public}s]:%i Fail to decode: %@", buf, 0x2Cu);
    }

    id v15 = 0;
  }
  else
  {
    id v15 = v21;
  }

LABEL_36:
  return v15;
}

+ (BOOL)supportsSecureCoding
{
  return 1;
}

- (void)encodeWithCoder:(id)a3
{
  uint64_t rfTechnology = self->_rfTechnology;
  id v5 = a3;
  [v5 encodeInteger:rfTechnology forKey:@"rfTechnology"];
  [v5 encodeInteger:self->_typeFSystemCode forKey:@"typeFSystemCode"];
  [v5 encodeInteger:self->_notificationType forKey:@"notificationType"];
  [v5 encodeObject:self->_creationDate forKey:@"creationDate"];
  [v5 encodeBool:self->_cachedBeforeRFReset forKey:@"cachedBeforeRFReset"];
  [v5 encodeInteger:self->_category forKey:@"category"];
}

+ (id)notificationWithDictionary:(id)a3
{
  uint64_t v46 = *MEMORY[0x1E4F143B8];
  id v5 = a3;
  if (![v5 count])
  {
    v12 = [MEMORY[0x1E4F28B00] currentHandler];
    [v12 handleFailureInMethod:a2 object:a1 file:@"NFFieldNotification.m" lineNumber:63 description:@"Empty dictionary"];

    v13 = 0;
    goto LABEL_34;
  }
  uint64_t v6 = [v5 objectForKey:@"ECPData"];
  if ((unint64_t)[v6 length] < 3) {
    goto LABEL_31;
  }
  id v7 = v6;
  v8 = (unsigned char *)[v7 bytes];
  if (*v8 != 106)
  {
    int v14 = (const void **)MEMORY[0x1E4FBA898];
    dispatch_get_specific((const void *)*MEMORY[0x1E4FBA898]);
    uint64_t Logger = NFLogGetLogger();
    if (Logger)
    {
      uint64_t v16 = (void (*)(uint64_t, const char *, ...))Logger;
      Class = object_getClass(a1);
      BOOL isMetaClass = class_isMetaClass(Class);
      ClassName = object_getClassName(a1);
      Name = sel_getName(a2);
      uint64_t v20 = 45;
      if (isMetaClass) {
        uint64_t v20 = 43;
      }
      v16(4, "%c[%{public}s %{public}s]:%i Unexpected ECP command; initialized to non ECP",
        v20,
        ClassName,
        Name,
        88);
    }
    dispatch_get_specific(*v14);
    v21 = NFSharedLogGetLogger();
    if (!os_log_type_enabled(v21, OS_LOG_TYPE_ERROR)) {
      goto LABEL_30;
    }
    id v22 = object_getClass(a1);
    if (class_isMetaClass(v22)) {
      int v23 = 43;
    }
    else {
      int v23 = 45;
    }
    *(_DWORD *)buf = 67109890;
    int v39 = v23;
    __int16 v40 = 2082;
    uint64_t v41 = object_getClassName(a1);
    __int16 v42 = 2082;
    int v43 = sel_getName(a2);
    __int16 v44 = 1024;
    int v45 = 88;
    uint64_t v24 = "%c[%{public}s %{public}s]:%i Unexpected ECP command; initialized to non ECP";
LABEL_29:
    _os_log_impl(&dword_19D636000, v21, OS_LOG_TYPE_ERROR, v24, buf, 0x22u);
LABEL_30:

LABEL_31:
    uint64_t v11 = NFFieldNotification;
    goto LABEL_32;
  }
  objc_super v9 = v8;
  int v10 = v8[1];
  if (v10 != 2)
  {
    if (v10 == 1)
    {
      uint64_t v11 = NFFieldNotificationECP1_0;
      goto LABEL_32;
    }
    v25 = (const void **)MEMORY[0x1E4FBA898];
    dispatch_get_specific((const void *)*MEMORY[0x1E4FBA898]);
    uint64_t v26 = NFLogGetLogger();
    if (v26)
    {
      BOOL v27 = (void (*)(uint64_t, const char *, ...))v26;
      v28 = object_getClass(a1);
      BOOL v29 = class_isMetaClass(v28);
      uint64_t v30 = object_getClassName(a1);
      uint64_t v37 = sel_getName(a2);
      uint64_t v31 = 45;
      if (v29) {
        uint64_t v31 = 43;
      }
      v27(4, "%c[%{public}s %{public}s]:%i Unexpected ECP version; initialized to non ECP", v31, v30, v37, 85);
    }
    dispatch_get_specific(*v25);
    v21 = NFSharedLogGetLogger();
    if (!os_log_type_enabled(v21, OS_LOG_TYPE_ERROR)) {
      goto LABEL_30;
    }
    v32 = object_getClass(a1);
    if (class_isMetaClass(v32)) {
      int v33 = 43;
    }
    else {
      int v33 = 45;
    }
    *(_DWORD *)buf = 67109890;
    int v39 = v33;
    __int16 v40 = 2082;
    uint64_t v41 = object_getClassName(a1);
    __int16 v42 = 2082;
    int v43 = sel_getName(a2);
    __int16 v44 = 1024;
    int v45 = 85;
    uint64_t v24 = "%c[%{public}s %{public}s]:%i Unexpected ECP version; initialized to non ECP";
    goto LABEL_29;
  }
  if ([v7 length] == 8 && *v9 == 0x820202C3026ALL)
  {
    v34 = [[NFFieldNotificationECP1_0 alloc] initWithDictionaryForDavenport:v5];
    goto LABEL_33;
  }
  uint64_t v11 = NFFieldNotificationECP2_0;
LABEL_32:
  v34 = (NFFieldNotificationECP1_0 *)[[v11 alloc] initWithDictionary:v5];
LABEL_33:
  v13 = v34;

LABEL_34:
  return v13;
}

- (NFFieldNotification)initWithNotificationType:(unint64_t)a3 rfTechnology:(unsigned int)a4 typeFSystemCode:(unsigned __int16)a5 creationDate:(id)a6 cachedBeforeRFReset:(BOOL)a7
{
  id v13 = a6;
  v18.receiver = self;
  v18.super_class = (Class)NFFieldNotification;
  int v14 = [(NFFieldNotification *)&v18 init];
  id v15 = v14;
  if (v14)
  {
    v14->_notificationType = a3;
    v14->_uint64_t rfTechnology = a4;
    v14->_typeFSystemCode = a5;
    objc_storeStrong((id *)&v14->_creationDate, a6);
    v15->_cachedBeforeRFReset = a7;
    uint64_t v16 = v15;
  }

  return v15;
}

- (NFFieldNotification)initWithDictionary:(id)a3
{
  id v4 = a3;
  v11.receiver = self;
  v11.super_class = (Class)NFFieldNotification;
  id v5 = [(NFFieldNotification *)&v11 init];
  if (v5)
  {
    uint64_t v6 = [v4 objectForKey:@"FieldTechnology"];
    v5->_uint64_t rfTechnology = [v6 integerValue];

    id v7 = [v4 objectForKey:@"SystemCode"];
    v5->_typeFSystemCode = [v7 integerValue];

    v5->_notificationType = 1;
    uint64_t v8 = [MEMORY[0x1E4F1C9C8] now];
    creationDate = v5->_creationDate;
    v5->_creationDate = (NSDate *)v8;
  }
  return v5;
}

- (void)setNotificationType:(unint64_t)a3
{
  self->_notificationType = a3;
}

- (id)description
{
  v3 = objc_opt_new();
  id v4 = v3;
  unsigned int rfTechnology = self->_rfTechnology;
  if (rfTechnology)
  {
    [v3 appendString:@"A,"];
    unsigned int rfTechnology = self->_rfTechnology;
    if ((rfTechnology & 2) == 0)
    {
LABEL_3:
      if ((rfTechnology & 4) == 0) {
        goto LABEL_4;
      }
      goto LABEL_14;
    }
  }
  else if ((rfTechnology & 2) == 0)
  {
    goto LABEL_3;
  }
  [v4 appendString:@"B,"];
  unsigned int rfTechnology = self->_rfTechnology;
  if ((rfTechnology & 4) == 0)
  {
LABEL_4:
    if ((rfTechnology & 0x10) == 0) {
      goto LABEL_5;
    }
    goto LABEL_15;
  }
LABEL_14:
  [v4 appendString:@"F,"];
  unsigned int rfTechnology = self->_rfTechnology;
  if ((rfTechnology & 0x10) == 0)
  {
LABEL_5:
    if ((rfTechnology & 8) == 0) {
      goto LABEL_7;
    }
    goto LABEL_6;
  }
LABEL_15:
  [v4 appendString:@"V,"];
  if ((self->_rfTechnology & 8) != 0) {
LABEL_6:
  }
    [v4 appendString:@"ECP,"];
LABEL_7:
  if (![v4 length]) {
    [v4 appendString:@"None"];
  }
  id v6 = [NSString alloc];
  ClassName = object_getClassName(self);
  uint64_t v8 = bswap32(self->_typeFSystemCode) >> 16;
  objc_super v9 = [(NFFieldNotification *)self _creationDateString];
  int v10 = (void *)[v6 initWithFormat:@"<%s: tech=%@ sc=0x%X date=%@ cached=%d>", ClassName, v4, v8, v9, self->_cachedBeforeRFReset];

  return v10;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = (NFFieldNotification *)a3;
  if (v4 == self)
  {
    BOOL v9 = 1;
  }
  else
  {
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      id v5 = v4;
      unsigned int v6 = [(NFFieldNotification *)v5 rfTechnology];
      if (v6 == [(NFFieldNotification *)self rfTechnology]
        && (int v7 = [(NFFieldNotification *)v5 typeFSystemCode],
            v7 == [(NFFieldNotification *)self typeFSystemCode]))
      {
        uint64_t v8 = [(NFFieldNotification *)v5 notificationType];
        BOOL v9 = v8 == [(NFFieldNotification *)self notificationType];
      }
      else
      {
        BOOL v9 = 0;
      }
    }
    else
    {
      BOOL v9 = 0;
    }
  }

  return v9;
}

- (BOOL)isEqualWithoutRFTech:(id)a3
{
  id v4 = (NFFieldNotification *)a3;
  if (v4 == self)
  {
    BOOL v6 = 1;
  }
  else
  {
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      uint64_t v5 = [(NFFieldNotification *)v4 notificationType];
      BOOL v6 = v5 == [(NFFieldNotification *)self notificationType];
    }
    else
    {
      BOOL v6 = 0;
    }
  }

  return v6;
}

- (BOOL)isEqualTerminalInfo:(id)a3
{
  id v4 = a3;
  unint64_t v5 = [(NFFieldNotification *)self notificationType];
  if (v5 != [v4 notificationType]) {
    goto LABEL_8;
  }
  if ([(NFFieldNotification *)self notificationType] != 2)
  {
    if ([(NFFieldNotification *)self notificationType] == 3)
    {
      uint64_t v8 = [(NFFieldNotification *)self terminalType];
      if (v8 == [v4 terminalType])
      {
        int v6 = [(NFFieldNotification *)self terminalSubType];
        int v7 = [v4 terminalSubType];
        goto LABEL_7;
      }
    }
LABEL_8:
    BOOL v9 = 0;
    goto LABEL_9;
  }
  int v6 = [(NFFieldNotification *)self terminalType];
  int v7 = [v4 terminalType];
LABEL_7:
  BOOL v9 = v6 == v7;
LABEL_9:

  return v9;
}

- (id)asXPCObject
{
  uint64_t v32 = *MEMORY[0x1E4F143B8];
  id v21 = 0;
  id v4 = [MEMORY[0x1E4F28DB0] archivedDataWithRootObject:self requiringSecureCoding:1 error:&v21];
  id v5 = v21;
  if (v5)
  {
    int v6 = (const void **)MEMORY[0x1E4FBA898];
    dispatch_get_specific((const void *)*MEMORY[0x1E4FBA898]);
    uint64_t Logger = NFLogGetLogger();
    if (Logger)
    {
      uint64_t v8 = (void (*)(uint64_t, const char *, ...))Logger;
      Class = object_getClass(self);
      BOOL isMetaClass = class_isMetaClass(Class);
      ClassName = object_getClassName(self);
      Name = sel_getName(a2);
      uint64_t v11 = 45;
      if (isMetaClass) {
        uint64_t v11 = 43;
      }
      v8(3, "%c[%{public}s %{public}s]:%i Failed to encode: %@", v11, ClassName, Name, 233, v5);
    }
    dispatch_get_specific(*v6);
    v12 = NFSharedLogGetLogger();
    if (os_log_type_enabled(v12, OS_LOG_TYPE_ERROR))
    {
      id v13 = object_getClass(self);
      if (class_isMetaClass(v13)) {
        int v14 = 43;
      }
      else {
        int v14 = 45;
      }
      id v15 = object_getClassName(self);
      uint64_t v16 = sel_getName(a2);
      *(_DWORD *)buf = 67110146;
      int v23 = v14;
      __int16 v24 = 2082;
      v25 = v15;
      __int16 v26 = 2082;
      BOOL v27 = v16;
      __int16 v28 = 1024;
      int v29 = 233;
      __int16 v30 = 2112;
      id v31 = v5;
      _os_log_impl(&dword_19D636000, v12, OS_LOG_TYPE_ERROR, "%c[%{public}s %{public}s]:%i Failed to encode: %@", buf, 0x2Cu);
    }

    id v17 = 0;
  }
  else
  {
    id v17 = v4;
  }

  return v17;
}

- (id)copyWithZone:(_NSZone *)a3
{
  id v4 = [NFFieldNotification alloc];
  uint64_t rfTechnology = self->_rfTechnology;
  uint64_t typeFSystemCode = self->_typeFSystemCode;
  unint64_t notificationType = self->_notificationType;
  creationDate = self->_creationDate;
  BOOL cachedBeforeRFReset = self->_cachedBeforeRFReset;
  return [(NFFieldNotification *)v4 initWithNotificationType:notificationType rfTechnology:rfTechnology typeFSystemCode:typeFSystemCode creationDate:creationDate cachedBeforeRFReset:cachedBeforeRFReset];
}

- (id)_creationDateString
{
  v3 = objc_opt_new();
  [v3 setDateFormat:@"yyyy-MM-dd HH:mm:ss.SSSS"];
  id v4 = [v3 stringFromDate:self->_creationDate];

  return v4;
}

- (unsigned)typeFSystemCode
{
  return self->_typeFSystemCode;
}

- (NSDate)creationDate
{
  return self->_creationDate;
}

- (void)setCreationDate:(id)a3
{
}

- (BOOL)cachedBeforeRFReset
{
  return self->_cachedBeforeRFReset;
}

- (void)setCachedBeforeRFReset:(BOOL)a3
{
  self->_BOOL cachedBeforeRFReset = a3;
}

- (int64_t)category
{
  return self->_category;
}

- (void)setCategory:(int64_t)a3
{
  self->_category = a3;
}

- (BOOL)isCHTerminal
{
  if ([(NFFieldNotification *)self notificationType] != 3) {
    return 0;
  }
  v3 = self;
  BOOL v4 = [(NFFieldNotification *)v3 terminalType] == 5
    && [(NFFieldNotification *)v3 terminalSubType] == 0;

  return v4;
}

- (BOOL)isCHInitiatorDetected
{
  return [(NFFieldNotification *)self chFieldType] == 2;
}

- (BOOL)isCHReceiverDetected
{
  return [(NFFieldNotification *)self chFieldType] == 3;
}

- (BOOL)isCHAutoNegotiationField
{
  if ([(NFFieldNotification *)self isCHTerminal])
  {
    v3 = self;
    BOOL v4 = [(NFFieldNotification *)v3 userInfo];

    if (v4)
    {
      id v5 = [(NFFieldNotification *)v3 userInfo];
      int v6 = [v5 objectForKeyedSubscript:@"CHCfgBytes"];

      BOOL v4 = (void *)([v6 unsignedShortValue] & 1);
    }
  }
  else
  {
    LOBYTE(v4) = 0;
  }
  return (char)v4;
}

- (unint64_t)chFieldType
{
  uint64_t v51 = *MEMORY[0x1E4F143B8];
  if ([(NFFieldNotification *)self isCHTerminal])
  {
    BOOL v4 = self;
    id v5 = [(NFFieldNotification *)v4 userInfo];

    if (!v5)
    {
      unint64_t v23 = 4;
LABEL_33:

      return v23;
    }
    int v6 = [(NFFieldNotification *)v4 userInfo];
    int v7 = [v6 objectForKeyedSubscript:@"CHCfgBytes"];

    if ([v7 unsignedShortValue])
    {
      if ([v7 unsignedShortValue])
      {
        __int16 v24 = [(NFFieldNotification *)v4 userInfo];
        v25 = [v24 objectForKey:@"CHRemoteRandom"];

        if (!v25)
        {
          __int16 v28 = (const void **)MEMORY[0x1E4FBA898];
          dispatch_get_specific((const void *)*MEMORY[0x1E4FBA898]);
          uint64_t Logger = NFLogGetLogger();
          if (Logger)
          {
            __int16 v30 = (void (*)(uint64_t, const char *, ...))Logger;
            Class = object_getClass(v4);
            BOOL isMetaClass = class_isMetaClass(Class);
            ClassName = object_getClassName(v4);
            Name = sel_getName(a2);
            uint64_t v34 = 45;
            if (isMetaClass) {
              uint64_t v34 = 43;
            }
            v30(4, "%c[%{public}s %{public}s]:%i Unexpected state; missing randome value",
              v34,
              ClassName,
              Name,
              1015);
          }
          dispatch_get_specific(*v28);
          id v17 = NFSharedLogGetLogger();
          if (os_log_type_enabled(v17, OS_LOG_TYPE_ERROR))
          {
            v35 = object_getClass(v4);
            if (class_isMetaClass(v35)) {
              int v36 = 43;
            }
            else {
              int v36 = 45;
            }
            *(_DWORD *)buf = 67109890;
            int v42 = v36;
            __int16 v43 = 2082;
            __int16 v44 = object_getClassName(v4);
            __int16 v45 = 2082;
            uint64_t v46 = sel_getName(a2);
            __int16 v47 = 1024;
            int v48 = 1015;
            _os_log_impl(&dword_19D636000, v17, OS_LOG_TYPE_ERROR, "%c[%{public}s %{public}s]:%i Unexpected state; missing randome value",
              buf,
              0x22u);
          }
          goto LABEL_31;
        }
        __int16 v26 = [(NFFieldNotification *)v4 userInfo];
        BOOL v27 = [v26 objectForKey:@"CHInitiatorDetected"];

        if (v27)
        {
          unint64_t v23 = 2;
          goto LABEL_32;
        }
        v38 = [(NFFieldNotification *)v4 userInfo];
        int v39 = [v38 objectForKey:@"CHReceiverDetected"];

        if (v39)
        {
          unint64_t v23 = 3;
          goto LABEL_32;
        }
      }
      if (([v7 unsignedShortValue] & 2) == 0)
      {
        uint64_t v8 = (const void **)MEMORY[0x1E4FBA898];
        dispatch_get_specific((const void *)*MEMORY[0x1E4FBA898]);
        uint64_t v9 = NFLogGetLogger();
        if (v9)
        {
          int v10 = (void (*)(uint64_t, const char *, ...))v9;
          uint64_t v11 = object_getClass(v4);
          BOOL v12 = class_isMetaClass(v11);
          id v13 = object_getClassName(v4);
          int v14 = sel_getName(a2);
          id v15 = [(NFFieldNotification *)v4 debugDescription];
          uint64_t v16 = 45;
          if (v12) {
            uint64_t v16 = 43;
          }
          v10(4, "%c[%{public}s %{public}s]:%i Unexpected state; %{public}@", v16, v13, v14, 1032, v15);
        }
        dispatch_get_specific(*v8);
        id v17 = NFSharedLogGetLogger();
        if (os_log_type_enabled(v17, OS_LOG_TYPE_ERROR))
        {
          objc_super v18 = object_getClass(v4);
          if (class_isMetaClass(v18)) {
            int v19 = 43;
          }
          else {
            int v19 = 45;
          }
          uint64_t v20 = object_getClassName(v4);
          id v21 = sel_getName(a2);
          id v22 = [(NFFieldNotification *)v4 debugDescription];
          *(_DWORD *)buf = 67110146;
          int v42 = v19;
          __int16 v43 = 2082;
          __int16 v44 = v20;
          __int16 v45 = 2082;
          uint64_t v46 = v21;
          __int16 v47 = 1024;
          int v48 = 1032;
          __int16 v49 = 2114;
          id v50 = v22;
          _os_log_impl(&dword_19D636000, v17, OS_LOG_TYPE_ERROR, "%c[%{public}s %{public}s]:%i Unexpected state; %{public}@",
            buf,
            0x2Cu);
        }
LABEL_31:

        unint64_t v23 = 1;
        goto LABEL_32;
      }
      unint64_t v23 = 5;
    }
    else
    {
      unint64_t v23 = 4;
    }
LABEL_32:

    goto LABEL_33;
  }
  return 1;
}

- (id)chRandomData
{
  if ([(NFFieldNotification *)self isCHTerminal])
  {
    v3 = self;
    BOOL v4 = [(NFFieldNotification *)v3 userInfo];

    if (v4)
    {
      id v5 = [(NFFieldNotification *)v3 userInfo];
      BOOL v4 = [v5 objectForKeyedSubscript:@"CHRemoteRandom"];
    }
  }
  else
  {
    BOOL v4 = 0;
  }
  return v4;
}

@end