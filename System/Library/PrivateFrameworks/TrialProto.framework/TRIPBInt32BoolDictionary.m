...
- (TRIPBInt32BoolDictionary)init
{
  return [(TRIPBInt32BoolDictionary *)self initWithBools:0 forKeys:0 count:0];
}
...
- (TRIPBInt32BoolDictionary)initWithBools:(const BOOL *)a3 forKeys:(const int *)a4 count:(unint64_t)a5
{
  ...
  v8 = [(TRIPBInt32BoolDictionary *)&v14 init];
  if (v8)
  {
...
- (TRIPBInt32BoolDictionary)initWithDictionary:(id)a3
{
  v4 = [(TRIPBInt32BoolDictionary *)self initWithBools:0 forKeys:0 count:0];
  v5 = v4;
  if (a3 && v4) {
...
- (void)dealloc
{
  if (self->_autocreator) {
    objc_msgSend((id)objc_msgSend(MEMORY[0x1E4F28B00], "currentHandler"), "handleFailureInMethod:object:file:lineNumber:description:", a2, self, @"TRIPBDictionary.m", 3955, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator);
  }

  v3.receiver = self;
  v3.super_class = (Class)TRIPBInt32BoolDictionary;
  [(TRIPBInt32BoolDictionary *)&v3 dealloc];
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[TRIPBInt32BoolDictionary allocWithZone:a3];
  return [(TRIPBInt32BoolDictionary *)v4 initWithDictionary:self];
}
...
- (void)enumerateKeysAndBoolsUsingBlock:(id)a3
{
  ...
  do
  {
    ...
    (*((void (**)(id, uint64_t, uint64_t, char *))a3 + 2))(a3, [v6 intValue], objc_msgSend((id)-[NSMutableDictionary objectForKeyedSubscript:](dictionary, "objectForKeyedSubscript:", v6), "BOOLValue"), &v7);
  }
  while (!v7);
}
...
- (void)enumerateForTextFormat:(id)a3
{
  ...
  v3[3] = &unk_1E576C210;
  v3[4] = a3;
  [(TRIPBInt32BoolDictionary *)self enumerateKeysAndBoolsUsingBlock:v3];
}
