...
- (BOOL)isEqual:(id)a3;
- (TRIPBUInt64BoolDictionary)init;
- (TRIPBUInt64BoolDictionary)initWithBools:(const BOOL *)a3 forKeys:(const unint64_t *)a4 count:(unint64_t)a5;
- (TRIPBUInt64BoolDictionary)initWithCapacity:(unint64_t)a3;
- (TRIPBUInt64BoolDictionary)initWithDictionary:(id)a3;
...
@implementation TRIPBUInt64BoolDictionary

- (TRIPBUInt64BoolDictionary)init
{
  return [(TRIPBUInt64BoolDictionary *)self initWithBools:0 forKeys:0 count:0];
}

- (TRIPBUInt64BoolDictionary)initWithBools:(const BOOL *)a3 forKeys:(const unint64_t *)a4 count:(unint64_t)a5
{
  v14.receiver = self;
  v14.super_class = (Class)TRIPBUInt64BoolDictionary;
  v8 = [(TRIPBUInt64BoolDictionary *)&v14 init];
  if (v8)
  {
...
- (TRIPBUInt64BoolDictionary)initWithDictionary:(id)a3
{
  v4 = [(TRIPBUInt64BoolDictionary *)self initWithBools:0 forKeys:0 count:0];
  v5 = v4;
  if (a3 && v4) {
...
- (void)dealloc
{
  if (self->_autocreator) {
    objc_msgSend((id)objc_msgSend(MEMORY[0x1E4F28B00], "currentHandler"), "handleFailureInMethod:object:file:lineNumber:description:", a2, self, @"TRIPBDictionary.m", 5703, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator);
  }

  v3.receiver = self;
  v3.super_class = (Class)TRIPBUInt64BoolDictionary;
  [(TRIPBUInt64BoolDictionary *)&v3 dealloc];
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[TRIPBUInt64BoolDictionary allocWithZone:a3];
  return [(TRIPBUInt64BoolDictionary *)v4 initWithDictionary:self];
}
...
- (void)enumerateKeysAndBoolsUsingBlock:(id)a3
{
  ...
  do
  {
    ...
    (*((void (**)(id, uint64_t, uint64_t, char *))a3 + 2))(a3, [v6 unsignedLongLongValue], objc_msgSend((id)-[NSMutableDictionary objectForKeyedSubscript:](dictionary, "objectForKeyedSubscript:", v6), "BOOLValue"), &v7);
  }
  while (!v7);
}
...
- (void)enumerateForTextFormat:(id)a3
{
  ...
  v3[1] = 3221225472;
  v3[2] = __52__TRIPBUInt64BoolDictionary_enumerateForTextFormat___block_invoke;
  v3[3] = &unk_1E576C350;
  v3[4] = a3;
  [(TRIPBUInt64BoolDictionary *)self enumerateKeysAndBoolsUsingBlock:v3];
}

uint64_t __52__TRIPBUInt64BoolDictionary_enumerateForTextFormat___block_invoke(uint64_t a1, uint64_t a2, int a3)
{
