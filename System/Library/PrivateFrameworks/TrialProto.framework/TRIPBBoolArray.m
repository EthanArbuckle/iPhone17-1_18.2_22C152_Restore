...
- (TRIPBBoolArray)init;
- (TRIPBBoolArray)initWithCapacity:(unint64_t)a3;
- (TRIPBBoolArray)initWithValueArray:(id)a3;
- (TRIPBBoolArray)initWithValues:(const BOOL *)a3 count:(unint64_t)a4;
...
{
  v3.receiver = self;
  v3.super_class = (Class)TRIPBBoolArray;
  return [(TRIPBBoolArray *)&v3 init];
}

- (TRIPBBoolArray)initWithValueArray:(id)a3
{
  return [(TRIPBBoolArray *)self initWithValues:*((void *)a3 + 2) count:*((void *)a3 + 3)];
}

- (TRIPBBoolArray)initWithValues:(const BOOL *)a3 count:(unint64_t)a4
{
  v6 = [(TRIPBBoolArray *)self init];
  v7 = v6;
  if (v6 && a3 && a4)
  {
    ...
    else
    {
      ...
      objc_msgSend(MEMORY[0x1E4F1CA00], "raise:format:", *MEMORY[0x1E4F1C4A0], @"Failed to allocate %lu bytes", a4);
    }
  }
  return v7;
}
...
- (TRIPBBoolArray)initWithCapacity:(unint64_t)a3
{
  BOOL v4 = [(TRIPBBoolArray *)self initWithValues:0 count:0];
  v5 = v4;
  if (a3 && v4) {
    [(TRIPBBoolArray *)v4 internalResizeToCapacity:a3];
  }
  return v5;
}
...
- (id)copyWithZone:(_NSZone *)a3
{
  BOOL v4 = +[TRIPBBoolArray allocWithZone:a3];
  values = self->_values;
  unint64_t count = self->_count;
  return [(TRIPBBoolArray *)v4 initWithValues:values count:count];
}

- (void)dealloc
{
  if (self->_autocreator) {
    objc_msgSend((id)objc_msgSend(MEMORY[0x1E4F28B00], "currentHandler"), "handleFailureInMethod:object:file:lineNumber:description:", a2, self, @"TRIPBArray.m", 1861, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator);
  }
  free(self->_values);
  v3.receiver = self;
  v3.super_class = (Class)TRIPBBoolArray;
  [(TRIPBBoolArray *)&v3 dealloc];
}
...
- (void)internalResizeToCapacity:(unint64_t)a3
{
  ...
  if (!v5)
  {
    ...
    self->_capacity = 0;
    objc_msgSend(MEMORY[0x1E4F1CA00], "raise:format:", *MEMORY[0x1E4F1C4A0], @"Failed to allocate %lu bytes", a3);
  }
  self->_capacity = a3;
}
...
{
  BOOL v3 = a3;
  [(TRIPBBoolArray *)self addValues:&v3 count:1];
}
...
- (void)addValues:(const BOOL *)a3 count:(unint64_t)a4
{
  if (a3 && a4)
  {
    ...
    if (count + a4 > self->_capacity) {
      [(TRIPBBoolArray *)self internalResizeToCapacity:(v8 & 0xFFFFFFFFFFFFFFF0) + 16];
    }
...
- (void)removeValueAtIndex:(unint64_t)a3
{
  ...
  {
    [(TRIPBBoolArray *)self internalResizeToCapacity:(v6 & 0xFFFFFFFFFFFFFFF0) + 16];
  }
}
...
- (void)removeAll
{
  ...
  if (self->_capacity >= 0x21) {
    [(TRIPBBoolArray *)self internalResizeToCapacity:16];
  }
}
