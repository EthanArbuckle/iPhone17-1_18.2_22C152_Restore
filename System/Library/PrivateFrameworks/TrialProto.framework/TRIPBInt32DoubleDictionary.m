...
- (BOOL)isEqual:(id)a3;
- (TRIPBInt32DoubleDictionary)init;
- (TRIPBInt32DoubleDictionary)initWithCapacity:(unint64_t)a3;
- (TRIPBInt32DoubleDictionary)initWithDictionary:(id)a3;
- (TRIPBInt32DoubleDictionary)initWithDoubles:(const double *)a3 forKeys:(const int *)a4 count:(unint64_t)a5;
...
@implementation TRIPBInt32DoubleDictionary

- (TRIPBInt32DoubleDictionary)init
{
  return [(TRIPBInt32DoubleDictionary *)self initWithDoubles:0 forKeys:0 count:0];
}

- (TRIPBInt32DoubleDictionary)initWithDoubles:(const double *)a3 forKeys:(const int *)a4 count:(unint64_t)a5
{
  v13.receiver = self;
  v13.super_class = (Class)TRIPBInt32DoubleDictionary;
  v8 = [(TRIPBInt32DoubleDictionary *)&v13 init];
  if (v8)
  {
    ...
    if (a4)
    {
      if (a3 && a5)
      {
        do
        {
          ...
          LODWORD(v10) = *a4++;
          -[NSMutableDictionary setObject:forKey:](dictionary, "setObject:forKey:", v11, [NSNumber numberWithInt:LODWORD(v10)]);
          --a5;
        }
        while (a5);
      }
...
- (TRIPBInt32DoubleDictionary)initWithDictionary:(id)a3
{
  v4 = [(TRIPBInt32DoubleDictionary *)self initWithDoubles:0 forKeys:0 count:0];
  v5 = v4;
  if (a3 && v4) {
...
- (void)dealloc
{
  if (self->_autocreator) {
    objc_msgSend((id)objc_msgSend(MEMORY[0x1E4F28B00], "currentHandler"), "handleFailureInMethod:object:file:lineNumber:description:", a2, self, @"TRIPBDictionary.m", 4321, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator);
  }

  v3.receiver = self;
  v3.super_class = (Class)TRIPBInt32DoubleDictionary;
  [(TRIPBInt32DoubleDictionary *)&v3 dealloc];
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[TRIPBInt32DoubleDictionary allocWithZone:a3];
  return [(TRIPBInt32DoubleDictionary *)v4 initWithDictionary:self];
}
...
- (void)enumerateForTextFormat:(id)a3
{
  ...
  v3[1] = 3221225472;
  v3[2] = __53__TRIPBInt32DoubleDictionary_enumerateForTextFormat___block_invoke;
  v3[3] = &unk_1E576C260;
  v3[4] = a3;
  [(TRIPBInt32DoubleDictionary *)self enumerateKeysAndDoublesUsingBlock:v3];
}

uint64_t __53__TRIPBInt32DoubleDictionary_enumerateForTextFormat___block_invoke(uint64_t a1, uint64_t a2, double a3)
{
