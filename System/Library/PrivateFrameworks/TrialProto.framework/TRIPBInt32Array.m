...
- (BOOL)isEqual:(id)a3;
- (TRIPBInt32Array)init;
- (TRIPBInt32Array)initWithCapacity:(unint64_t)a3;
- (TRIPBInt32Array)initWithValueArray:(id)a3;
- (TRIPBInt32Array)initWithValues:(const int *)a3 count:(unint64_t)a4;
...
@implementation TRIPBInt32Array
...
{
  v3.receiver = self;
  v3.super_class = (Class)TRIPBInt32Array;
  return [(TRIPBInt32Array *)&v3 init];
}

- (TRIPBInt32Array)initWithValueArray:(id)a3
{
  return [(TRIPBInt32Array *)self initWithValues:*((void *)a3 + 2) count:*((void *)a3 + 3)];
}

- (TRIPBInt32Array)initWithValues:(const int *)a3 count:(unint64_t)a4
{
  v6 = [(TRIPBInt32Array *)self init];
  v7 = v6;
  if (v6 && a3 && a4)
  {
...
- (TRIPBInt32Array)initWithCapacity:(unint64_t)a3
{
  int v4 = [(TRIPBInt32Array *)self initWithValues:0 count:0];
  v5 = v4;
  if (a3 && v4) {
    [(TRIPBInt32Array *)v4 internalResizeToCapacity:a3];
  }
  return v5;
}
...
- (id)copyWithZone:(_NSZone *)a3
{
  int v4 = +[TRIPBInt32Array allocWithZone:a3];
  values = self->_values;
  unint64_t count = self->_count;
  return [(TRIPBInt32Array *)v4 initWithValues:values count:count];
}

- (void)dealloc
{
  if (self->_autocreator) {
    objc_msgSend((id)objc_msgSend(MEMORY[0x1E4F28B00], "currentHandler"), "handleFailureInMethod:object:file:lineNumber:description:", a2, self, @"TRIPBArray.m", 373, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator);
  }
  free(self->_values);
  v3.receiver = self;
  v3.super_class = (Class)TRIPBInt32Array;
  [(TRIPBInt32Array *)&v3 dealloc];
}
...
{
  int v3 = a3;
  [(TRIPBInt32Array *)self addValues:&v3 count:1];
}
...
- (void)addValues:(const int *)a3 count:(unint64_t)a4
{
  if (a3 && a4)
  {
    ...
    if (count + a4 > self->_capacity) {
      [(TRIPBInt32Array *)self internalResizeToCapacity:(v8 & 0xFFFFFFFFFFFFFFF0) + 16];
    }
...
- (void)removeValueAtIndex:(unint64_t)a3
{
  ...
  {
    [(TRIPBInt32Array *)self internalResizeToCapacity:(v6 & 0xFFFFFFFFFFFFFFF0) + 16];
  }
}
...
- (void)removeAll
{
  ...
  if (self->_capacity >= 0x21) {
    [(TRIPBInt32Array *)self internalResizeToCapacity:16];
  }
}
