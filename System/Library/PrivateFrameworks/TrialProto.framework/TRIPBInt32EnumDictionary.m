...
- (BOOL)isEqual:(id)a3;
- (TRIPBInt32EnumDictionary)init;
- (TRIPBInt32EnumDictionary)initWithDictionary:(id)a3;
- (TRIPBInt32EnumDictionary)initWithValidationFunction:(void *)a3;
- (TRIPBInt32EnumDictionary)initWithValidationFunction:(void *)a3 capacity:(unint64_t)a4;
- (TRIPBInt32EnumDictionary)initWithValidationFunction:(void *)a3 rawValues:(const int *)a4 forKeys:(const int *)a5 count:(unint64_t)a6;
...
@implementation TRIPBInt32EnumDictionary

- (TRIPBInt32EnumDictionary)init
{
  return [(TRIPBInt32EnumDictionary *)self initWithValidationFunction:0 rawValues:0 forKeys:0 count:0];
}

- (TRIPBInt32EnumDictionary)initWithValidationFunction:(void *)a3
{
  return [(TRIPBInt32EnumDictionary *)self initWithValidationFunction:a3 rawValues:0 forKeys:0 count:0];
}

- (TRIPBInt32EnumDictionary)initWithValidationFunction:(void *)a3 rawValues:(const int *)a4 forKeys:(const int *)a5 count:(unint64_t)a6
{
  v18.receiver = self;
  v18.super_class = (Class)TRIPBInt32EnumDictionary;
  v10 = [(TRIPBInt32EnumDictionary *)&v18 init];
  if (v10)
  {
    ...
    if (a5 && a4 && a6)
    {
      do
      {
        ...
        unsigned int v16 = *a5++;
        -[NSMutableDictionary setObject:forKey:](dictionary, "setObject:forKey:", v15, [NSNumber numberWithInt:v16]);
        --a6;
      }
      while (a6);
    }
...
- (TRIPBInt32EnumDictionary)initWithDictionary:(id)a3
{
  v4 = -[TRIPBInt32EnumDictionary initWithValidationFunction:rawValues:forKeys:count:](self, "initWithValidationFunction:rawValues:forKeys:count:", [a3 validationFunc], 0, 0, 0);
  v5 = v4;
  if (a3 && v4) {
...
- (void)dealloc
{
  if (self->_autocreator) {
    objc_msgSend((id)objc_msgSend(MEMORY[0x1E4F28B00], "currentHandler"), "handleFailureInMethod:object:file:lineNumber:description:", a2, self, @"TRIPBDictionary.m", 4517, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator);
  }

  v3.receiver = self;
  v3.super_class = (Class)TRIPBInt32EnumDictionary;
  [(TRIPBInt32EnumDictionary *)&v3 dealloc];
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[TRIPBInt32EnumDictionary allocWithZone:a3];
  return [(TRIPBInt32EnumDictionary *)v4 initWithDictionary:self];
}
...
- (void)enumerateKeysAndRawValuesUsingBlock:(id)a3
{
  ...
  do
  {
    ...
    (*((void (**)(id, uint64_t, uint64_t, char *))a3 + 2))(a3, [v6 intValue], objc_msgSend((id)-[NSMutableDictionary objectForKeyedSubscript:](dictionary, "objectForKeyedSubscript:", v6), "intValue"), &v7);
  }
  while (!v7);
}
...
- (id)serializedDataForUnknownValue:(int)a3 forKey:(id *)a4 keyDataType:(unsigned __int8)a5
{
  unsigned int v5 = a5;
  uint64_t v7 = *(void *)&a3;
  uint64_t v8 = ComputeDictInt32FieldSize(a4->var1, 1, a5);
  uint64_t v9 = 2;
  uint64_t v10 = 3;
  uint64_t v11 = 4;
  uint64_t v12 = 5;
  ...
  WriteDictInt32Field(v14, a4->var3, 1, v5);
...
- (void)enumerateForTextFormat:(id)a3
{
  ...
  v3[1] = 3221225472;
  v3[2] = __51__TRIPBInt32EnumDictionary_enumerateForTextFormat___block_invoke;
  v3[3] = &unk_1E576C198;
  v3[4] = a3;
  [(TRIPBInt32EnumDictionary *)self enumerateKeysAndRawValuesUsingBlock:v3];
}

uint64_t __51__TRIPBInt32EnumDictionary_enumerateForTextFormat___block_invoke(uint64_t a1, uint64_t a2, uint64_t a3)
{
...
- (BOOL)getEnum:(int *)a3 forKey:(int)a4
{
  uint64_t v6 = (void *)-[NSMutableDictionary objectForKey:](self->_dictionary, "objectForKey:", [NSNumber numberWithInt:*(void *)&a4]);
  uint64_t v7 = v6;
  if (a3 && v6)
  {
    int v8 = [v6 intValue];
...
- (void)enumerateKeysAndEnumsUsingBlock:(id)a3
{
  ...
  do
  {
    ...
    (*((void (**)(id, uint64_t, uint64_t, char *))a3 + 2))(a3, [v8 intValue], v9, &v10);
  }
  while (!v10);
}
...
- (void)setEnum:(int)a3 forKey:(int)a4
{
  ...
  if ((((uint64_t (*)(void, SEL))self->_validationFunc)(*(void *)&a3, a2) & 1) == 0) {
    objc_msgSend(MEMORY[0x1E4F1CA00], "raise:format:", *MEMORY[0x1E4F1C3C8], @"TRIPBInt32EnumDictionary: Attempt to set an unknown enum value (%d)", v5);
  }
  dictionary = self->_dictionary;
  uint64_t v8 = [NSNumber numberWithInt:v5];
  -[NSMutableDictionary setObject:forKey:](dictionary, "setObject:forKey:", v8, [NSNumber numberWithInt:v4]);
  autocreator = self->_autocreator;
  if (autocreator)
  {
    TRIPBAutocreatedDictionaryModified(autocreator, (uint64_t)self);
