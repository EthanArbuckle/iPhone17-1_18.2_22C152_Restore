...
- (BOOL)isEqual:(id)a3;
- (BOOL)isInitialized;
- (TRIPBInt32ObjectDictionary)init;
- (TRIPBInt32ObjectDictionary)initWithCapacity:(unint64_t)a3;
- (TRIPBInt32ObjectDictionary)initWithDictionary:(id)a3;
- (TRIPBInt32ObjectDictionary)initWithObjects:(const void *)a3 forKeys:(const int *)a4 count:(unint64_t)a5;
...
@implementation TRIPBInt32ObjectDictionary

- (TRIPBInt32ObjectDictionary)init
{
  return [(TRIPBInt32ObjectDictionary *)self initWithObjects:0 forKeys:0 count:0];
}

- (TRIPBInt32ObjectDictionary)initWithObjects:(const void *)a3 forKeys:(const int *)a4 count:(unint64_t)a5
{
  v13.receiver = self;
  v13.super_class = (Class)TRIPBInt32ObjectDictionary;
  v8 = [(TRIPBInt32ObjectDictionary *)&v13 init];
  if (v8)
  {
    ...
    if (a4)
    {
      if (a3 && a5)
      {
        ...
        do
        {
          ...
          unsigned int v11 = *a4++;
          -[NSMutableDictionary setObject:forKey:](v8->_dictionary, "setObject:forKey:", v10, [NSNumber numberWithInt:v11]);
          ++a3;
          --a5;
        }
        while (a5);
...
- (TRIPBInt32ObjectDictionary)initWithDictionary:(id)a3
{
  v4 = [(TRIPBInt32ObjectDictionary *)self initWithObjects:0 forKeys:0 count:0];
  v5 = v4;
  if (a3 && v4) {
...
- (void)dealloc
{
  if (self->_autocreator) {
    objc_msgSend((id)objc_msgSend(MEMORY[0x1E4F28B00], "currentHandler"), "handleFailureInMethod:object:file:lineNumber:description:", a2, self, @"TRIPBDictionary.m", 4760, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator);
  }

  v3.receiver = self;
  v3.super_class = (Class)TRIPBInt32ObjectDictionary;
  [(TRIPBInt32ObjectDictionary *)&v3 dealloc];
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[TRIPBInt32ObjectDictionary allocWithZone:a3];
  return [(TRIPBInt32ObjectDictionary *)v4 initWithDictionary:self];
}
...
- (void)enumerateKeysAndObjectsUsingBlock:(id)a3
{
  ...
  do
  {
    ...
    (*((void (**)(id, uint64_t, uint64_t, char *))a3 + 2))(a3, [v6 intValue], -[NSMutableDictionary objectForKeyedSubscript:](dictionary, "objectForKeyedSubscript:", v6), &v7);
  }
  while (!v7);
}
...
- (id)deepCopyWithZone:(_NSZone *)a3
{
  uint64_t v5 = objc_alloc_init(TRIPBInt32ObjectDictionary);
...
- (void)writeToCodedOutputStream:(id)a3 asField:(id)a4
{
  ...
  if (v12)
  {
    ...
    do
    {
      ...
      int v17 = ComputeDictInt32FieldSize(v16, 1, v8);
...
- (void)setTRIPBGenericValue:(id *)a3 forTRIPBGenericValueKey:(id *)a4
{
  ...
  uint64_t v6 = [NSNumber numberWithInt:a4->var3];
...
- (void)enumerateForTextFormat:(id)a3
{
  ...
  v3[1] = 3221225472;
  v3[2] = __53__TRIPBInt32ObjectDictionary_enumerateForTextFormat___block_invoke;
  v3[3] = &unk_1E576C288;
  v3[4] = a3;
  [(TRIPBInt32ObjectDictionary *)self enumerateKeysAndObjectsUsingBlock:v3];
}

uint64_t __53__TRIPBInt32ObjectDictionary_enumerateForTextFormat___block_invoke(uint64_t a1, uint64_t a2, uint64_t a3)
{
...
- (void)setObject:(id)a3 forKey:(int)a4
{
  ...
  -[NSMutableDictionary setObject:forKey:](self->_dictionary, "setObject:forKey:", a3, [NSNumber numberWithInt:v4]);
  autocreator = self->_autocreator;
  if (autocreator)
  {
    TRIPBAutocreatedDictionaryModified(autocreator, (uint64_t)self);
