...
- (BOOL)isEqual:(id)a3;
- (TRIPBStringUInt32Dictionary)init;
- (TRIPBStringUInt32Dictionary)initWithCapacity:(unint64_t)a3;
- (TRIPBStringUInt32Dictionary)initWithDictionary:(id)a3;
- (TRIPBStringUInt32Dictionary)initWithUInt32s:(const unsigned int *)a3 forKeys:(id *)a4 count:(unint64_t)a5;
...
@implementation TRIPBStringUInt32Dictionary

- (TRIPBStringUInt32Dictionary)init
{
  return [(TRIPBStringUInt32Dictionary *)self initWithUInt32s:0 forKeys:0 count:0];
}

- (TRIPBStringUInt32Dictionary)initWithUInt32s:(const unsigned int *)a3 forKeys:(id *)a4 count:(unint64_t)a5
{
  v15.receiver = self;
  v15.super_class = (Class)TRIPBStringUInt32Dictionary;
  v8 = [(TRIPBStringUInt32Dictionary *)&v15 init];
  if (v8)
  {
...
- (TRIPBStringUInt32Dictionary)initWithDictionary:(id)a3
{
  v4 = [(TRIPBStringUInt32Dictionary *)self initWithUInt32s:0 forKeys:0 count:0];
  v5 = v4;
  if (a3 && v4) {
...
- (void)dealloc
{
  if (self->_autocreator) {
    objc_msgSend((id)objc_msgSend(MEMORY[0x1E4F28B00], "currentHandler"), "handleFailureInMethod:object:file:lineNumber:description:", a2, self, @"TRIPBDictionary.m", 8471, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator);
  }

  v3.receiver = self;
  v3.super_class = (Class)TRIPBStringUInt32Dictionary;
  [(TRIPBStringUInt32Dictionary *)&v3 dealloc];
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[TRIPBStringUInt32Dictionary allocWithZone:a3];
  return [(TRIPBStringUInt32Dictionary *)v4 initWithDictionary:self];
}
...
- (void)enumerateKeysAndUInt32sUsingBlock:(id)a3
{
  ...
  do
  {
    ...
    (*((void (**)(id, uint64_t, uint64_t, char *))a3 + 2))(a3, v6, objc_msgSend((id)-[NSMutableDictionary objectForKeyedSubscript:](dictionary, "objectForKeyedSubscript:", v6), "unsignedIntValue"), &v7);
  }
  while (!v7);
}
...
- (void)writeToCodedOutputStream:(id)a3 asField:(id)a4
{
  ...
  if (v12)
  {
    ...
    do
    {
      ...
      objc_msgSend(a3, "writeInt32NoTag:", ComputeDictUInt32FieldSize(v16, 2, v7) + v17);
      WriteDictStringField(a3, (uint64_t)v13, v8);
      WriteDictUInt32Field(a3, v16, 2, v7);
...
- (void)enumerateForTextFormat:(id)a3
{
  ...
  v3[1] = 3221225472;
  v3[2] = __54__TRIPBStringUInt32Dictionary_enumerateForTextFormat___block_invoke;
  v3[3] = &unk_1E576C530;
  v3[4] = a3;
  [(TRIPBStringUInt32Dictionary *)self enumerateKeysAndUInt32sUsingBlock:v3];
}

uint64_t __54__TRIPBStringUInt32Dictionary_enumerateForTextFormat___block_invoke(uint64_t a1, uint64_t a2, uint64_t a3)
{
  uint64_t v4 = *(void *)(a1 + 32);
  uint64_t v5 = objc_msgSend(NSString, "stringWithFormat:", @"%u", a3);
...
- (void)setUInt32:(unsigned int)a3 forKey:(id)a4
{
  ...
  -[NSMutableDictionary setObject:forKey:](self->_dictionary, "setObject:forKey:", [NSNumber numberWithUnsignedInt:v5], a4);
  autocreator = self->_autocreator;
  if (autocreator)
  {
    TRIPBAutocreatedDictionaryModified(autocreator, (uint64_t)self);
