...
- (BOOL)isEqual:(id)a3;
- (TRIPBUInt32UInt64Dictionary)init;
- (TRIPBUInt32UInt64Dictionary)initWithCapacity:(unint64_t)a3;
- (TRIPBUInt32UInt64Dictionary)initWithDictionary:(id)a3;
- (TRIPBUInt32UInt64Dictionary)initWithUInt64s:(const unint64_t *)a3 forKeys:(const unsigned int *)a4 count:(unint64_t)a5;
...
@implementation TRIPBUInt32UInt64Dictionary

- (TRIPBUInt32UInt64Dictionary)init
{
  return [(TRIPBUInt32UInt64Dictionary *)self initWithUInt64s:0 forKeys:0 count:0];
}

- (TRIPBUInt32UInt64Dictionary)initWithUInt64s:(const unint64_t *)a3 forKeys:(const unsigned int *)a4 count:(unint64_t)a5
{
  v13.receiver = self;
  v13.super_class = (Class)TRIPBUInt32UInt64Dictionary;
  v8 = [(TRIPBUInt32UInt64Dictionary *)&v13 init];
  if (v8)
  {
    ...
    if (a4)
    {
      if (a3 && a5)
      {
        do
        {
          ...
          LODWORD(v10) = *a4++;
          -[NSMutableDictionary setObject:forKey:](dictionary, "setObject:forKey:", v11, [NSNumber numberWithUnsignedInt:v10]);
          --a5;
        }
        while (a5);
      }
...
- (TRIPBUInt32UInt64Dictionary)initWithDictionary:(id)a3
{
  v4 = [(TRIPBUInt32UInt64Dictionary *)self initWithUInt64s:0 forKeys:0 count:0];
  v5 = v4;
  if (a3 && v4) {
...
- (void)dealloc
{
  if (self->_autocreator) {
    objc_msgSend((id)objc_msgSend(MEMORY[0x1E4F28B00], "currentHandler"), "handleFailureInMethod:object:file:lineNumber:description:", a2, self, @"TRIPBDictionary.m", 1841, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator);
  }

  v3.receiver = self;
  v3.super_class = (Class)TRIPBUInt32UInt64Dictionary;
  [(TRIPBUInt32UInt64Dictionary *)&v3 dealloc];
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[TRIPBUInt32UInt64Dictionary allocWithZone:a3];
  return [(TRIPBUInt32UInt64Dictionary *)v4 initWithDictionary:self];
}
...
- (void)enumerateKeysAndUInt64sUsingBlock:(id)a3
{
  ...
  do
  {
    ...
    (*((void (**)(id, uint64_t, uint64_t, char *))a3 + 2))(a3, [v6 unsignedIntValue], objc_msgSend((id)-[NSMutableDictionary objectForKeyedSubscript:](dictionary, "objectForKeyedSubscript:", v6), "unsignedLongLongValue"), &v7);
  }
  while (!v7);
}
...
- (void)enumerateForTextFormat:(id)a3
{
  ...
  v3[1] = 3221225472;
  v3[2] = __54__TRIPBUInt32UInt64Dictionary_enumerateForTextFormat___block_invoke;
  v3[3] = &unk_1E576C080;
  v3[4] = a3;
  [(TRIPBUInt32UInt64Dictionary *)self enumerateKeysAndUInt64sUsingBlock:v3];
}

uint64_t __54__TRIPBUInt32UInt64Dictionary_enumerateForTextFormat___block_invoke(uint64_t a1, uint64_t a2, uint64_t a3)
{
