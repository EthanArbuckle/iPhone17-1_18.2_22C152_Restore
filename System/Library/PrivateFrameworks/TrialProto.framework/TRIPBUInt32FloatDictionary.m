...
- (BOOL)isEqual:(id)a3;
- (TRIPBUInt32FloatDictionary)init;
- (TRIPBUInt32FloatDictionary)initWithCapacity:(unint64_t)a3;
- (TRIPBUInt32FloatDictionary)initWithDictionary:(id)a3;
- (TRIPBUInt32FloatDictionary)initWithFloats:(const float *)a3 forKeys:(const unsigned int *)a4 count:(unint64_t)a5;
...
@implementation TRIPBUInt32FloatDictionary

- (TRIPBUInt32FloatDictionary)init
{
  return [(TRIPBUInt32FloatDictionary *)self initWithFloats:0 forKeys:0 count:0];
}

- (TRIPBUInt32FloatDictionary)initWithFloats:(const float *)a3 forKeys:(const unsigned int *)a4 count:(unint64_t)a5
{
  v15.receiver = self;
  v15.super_class = (Class)TRIPBUInt32FloatDictionary;
  v8 = [(TRIPBUInt32FloatDictionary *)&v15 init];
  if (v8)
  {
    ...
    if (a4)
    {
      if (a3 && a5)
      {
        do
        {
          ...
          unsigned int v13 = *a4++;
          -[NSMutableDictionary setObject:forKey:](dictionary, "setObject:forKey:", v12, [NSNumber numberWithUnsignedInt:v13]);
          --a5;
        }
        while (a5);
      }
...
- (TRIPBUInt32FloatDictionary)initWithDictionary:(id)a3
{
  v4 = [(TRIPBUInt32FloatDictionary *)self initWithFloats:0 forKeys:0 count:0];
  v5 = v4;
  if (a3 && v4) {
...
- (void)dealloc
{
  if (self->_autocreator) {
    objc_msgSend((id)objc_msgSend(MEMORY[0x1E4F28B00], "currentHandler"), "handleFailureInMethod:object:file:lineNumber:description:", a2, self, @"TRIPBDictionary.m", 2390, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator);
  }

  v3.receiver = self;
  v3.super_class = (Class)TRIPBUInt32FloatDictionary;
  [(TRIPBUInt32FloatDictionary *)&v3 dealloc];
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[TRIPBUInt32FloatDictionary allocWithZone:a3];
  return [(TRIPBUInt32FloatDictionary *)v4 initWithDictionary:self];
}
...
- (void)setTRIPBGenericValue:(id *)a3 forTRIPBGenericValueKey:(id *)a4
{
  ...
  uint64_t v8 = [NSNumber numberWithUnsignedInt:a4->var3];
...
- (void)enumerateForTextFormat:(id)a3
{
  ...
  v3[1] = 3221225472;
  v3[2] = __53__TRIPBUInt32FloatDictionary_enumerateForTextFormat___block_invoke;
  v3[3] = &unk_1E576C0F8;
  v3[4] = a3;
  [(TRIPBUInt32FloatDictionary *)self enumerateKeysAndFloatsUsingBlock:v3];
}

uint64_t __53__TRIPBUInt32FloatDictionary_enumerateForTextFormat___block_invoke(uint64_t a1, uint64_t a2, float a3)
{
