...
- (BOOL)isEqual:(id)a3;
- (BOOL)isInitialized;
- (TRIPBInt64ObjectDictionary)init;
- (TRIPBInt64ObjectDictionary)initWithCapacity:(unint64_t)a3;
- (TRIPBInt64ObjectDictionary)initWithDictionary:(id)a3;
- (TRIPBInt64ObjectDictionary)initWithObjects:(const void *)a3 forKeys:(const int64_t *)a4 count:(unint64_t)a5;
...
@implementation TRIPBInt64ObjectDictionary

- (TRIPBInt64ObjectDictionary)init
{
  return [(TRIPBInt64ObjectDictionary *)self initWithObjects:0 forKeys:0 count:0];
}

- (TRIPBInt64ObjectDictionary)initWithObjects:(const void *)a3 forKeys:(const int64_t *)a4 count:(unint64_t)a5
{
  v13.receiver = self;
  v13.super_class = (Class)TRIPBInt64ObjectDictionary;
  v8 = [(TRIPBInt64ObjectDictionary *)&v13 init];
  if (v8)
  {
    ...
    if (a4)
    {
      if (a3 && a5)
      {
        ...
        do
        {
          ...
          uint64_t v11 = *a4++;
          -[NSMutableDictionary setObject:forKey:](v8->_dictionary, "setObject:forKey:", v10, [NSNumber numberWithLongLong:v11]);
          ++a3;
          --a5;
        }
        while (a5);
...
- (TRIPBInt64ObjectDictionary)initWithDictionary:(id)a3
{
  v4 = [(TRIPBInt64ObjectDictionary *)self initWithObjects:0 forKeys:0 count:0];
  v5 = v4;
  if (a3 && v4) {
...
- (void)dealloc
{
  if (self->_autocreator) {
    objc_msgSend((id)objc_msgSend(MEMORY[0x1E4F28B00], "currentHandler"), "handleFailureInMethod:object:file:lineNumber:description:", a2, self, @"TRIPBDictionary.m", 8256, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator);
  }

  v3.receiver = self;
  v3.super_class = (Class)TRIPBInt64ObjectDictionary;
  [(TRIPBInt64ObjectDictionary *)&v3 dealloc];
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[TRIPBInt64ObjectDictionary allocWithZone:a3];
  return [(TRIPBInt64ObjectDictionary *)v4 initWithDictionary:self];
}
...
- (void)enumerateKeysAndObjectsUsingBlock:(id)a3
{
  ...
  do
  {
    ...
    (*((void (**)(id, uint64_t, uint64_t, char *))a3 + 2))(a3, [v6 longLongValue], -[NSMutableDictionary objectForKeyedSubscript:](dictionary, "objectForKeyedSubscript:", v6), &v7);
  }
  while (!v7);
}
...
- (id)deepCopyWithZone:(_NSZone *)a3
{
  uint64_t v5 = objc_alloc_init(TRIPBInt64ObjectDictionary);
...
- (void)writeToCodedOutputStream:(id)a3 asField:(id)a4
{
  ...
  if (v12)
  {
    ...
    do
    {
      ...
      int v17 = ComputeDictInt64FieldSize(v16, 1, v8);
...
- (void)setTRIPBGenericValue:(id *)a3 forTRIPBGenericValueKey:(id *)a4
{
  ...
  uint64_t v6 = [NSNumber numberWithLongLong:a4->var2];
...
- (void)enumerateForTextFormat:(id)a3
{
  ...
  v3[1] = 3221225472;
  v3[2] = __53__TRIPBInt64ObjectDictionary_enumerateForTextFormat___block_invoke;
  v3[3] = &unk_1E576C508;
  v3[4] = a3;
  [(TRIPBInt64ObjectDictionary *)self enumerateKeysAndObjectsUsingBlock:v3];
}

uint64_t __53__TRIPBInt64ObjectDictionary_enumerateForTextFormat___block_invoke(uint64_t a1, uint64_t a2, uint64_t a3)
{
...
- (void)setObject:(id)a3 forKey:(int64_t)a4
{
  ...
  -[NSMutableDictionary setObject:forKey:](self->_dictionary, "setObject:forKey:", a3, [NSNumber numberWithLongLong:a4]);
  autocreator = self->_autocreator;
  if (autocreator)
  {
    TRIPBAutocreatedDictionaryModified(autocreator, (uint64_t)self);
