...
- (BOOL)isEqual:(id)a3;
- (TRIPBStringBoolDictionary)init;
- (TRIPBStringBoolDictionary)initWithBools:(const BOOL *)a3 forKeys:(id *)a4 count:(unint64_t)a5;
- (TRIPBStringBoolDictionary)initWithCapacity:(unint64_t)a3;
- (TRIPBStringBoolDictionary)initWithDictionary:(id)a3;
...
@implementation TRIPBStringBoolDictionary

- (TRIPBStringBoolDictionary)init
{
  return [(TRIPBStringBoolDictionary *)self initWithBools:0 forKeys:0 count:0];
}

- (TRIPBStringBoolDictionary)initWithBools:(const BOOL *)a3 forKeys:(id *)a4 count:(unint64_t)a5
{
  v15.receiver = self;
  v15.super_class = (Class)TRIPBStringBoolDictionary;
  v8 = [(TRIPBStringBoolDictionary *)&v15 init];
  if (v8)
  {
...
- (TRIPBStringBoolDictionary)initWithDictionary:(id)a3
{
  v4 = [(TRIPBStringBoolDictionary *)self initWithBools:0 forKeys:0 count:0];
  v5 = v4;
  if (a3 && v4) {
...
- (void)dealloc
{
  if (self->_autocreator) {
    objc_msgSend((id)objc_msgSend(MEMORY[0x1E4F28B00], "currentHandler"), "handleFailureInMethod:object:file:lineNumber:description:", a2, self, @"TRIPBDictionary.m", 9235, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator);
  }

  v3.receiver = self;
  v3.super_class = (Class)TRIPBStringBoolDictionary;
  [(TRIPBStringBoolDictionary *)&v3 dealloc];
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[TRIPBStringBoolDictionary allocWithZone:a3];
  return [(TRIPBStringBoolDictionary *)v4 initWithDictionary:self];
}
...
- (void)enumerateKeysAndBoolsUsingBlock:(id)a3
{
  ...
  do
  {
    ...
    (*((void (**)(id, uint64_t, uint64_t, char *))a3 + 2))(a3, v6, objc_msgSend((id)-[NSMutableDictionary objectForKeyedSubscript:](dictionary, "objectForKeyedSubscript:", v6), "BOOLValue"), &v7);
  }
  while (!v7);
}
...
- (void)writeToCodedOutputStream:(id)a3 asField:(id)a4
{
  ...
  if (v12)
  {
    ...
    do
    {
      ...
      objc_msgSend(a3, "writeInt32NoTag:", ComputeDictBoolFieldSize(2, v7) + v17);
      WriteDictStringField(a3, (uint64_t)v13, v8);
      WriteDictBoolField(a3, v16, 2, v7);
...
- (void)enumerateForTextFormat:(id)a3
{
  ...
  v3[1] = 3221225472;
  v3[2] = __52__TRIPBStringBoolDictionary_enumerateForTextFormat___block_invoke;
  v3[3] = &unk_1E576C5D0;
  v3[4] = a3;
  [(TRIPBStringBoolDictionary *)self enumerateKeysAndBoolsUsingBlock:v3];
}

uint64_t __52__TRIPBStringBoolDictionary_enumerateForTextFormat___block_invoke(uint64_t a1, uint64_t a2, int a3)
{
  ...
  return (*(uint64_t (**)(uint64_t, uint64_t, __CFString *))(v3 + 16))(v3, a2, v4);
}

- (BOOL)getBool:(BOOL *)a3 forKey:(id)a4
{
  v5 = (void *)[(NSMutableDictionary *)self->_dictionary objectForKey:a4];
  uint64_t v6 = v5;
  if (a3 && v5) {
    *a3 = [v5 BOOLValue];
  }
...
- (void)setBool:(BOOL)a3 forKey:(id)a4
{
  ...
  -[NSMutableDictionary setObject:forKey:](self->_dictionary, "setObject:forKey:", [NSNumber numberWithBool:v5], a4);
  autocreator = self->_autocreator;
  if (autocreator)
  {
    TRIPBAutocreatedDictionaryModified(autocreator, (uint64_t)self);
