...
- (BOOL)isEqual:(id)a3;
- (TRIPBStringInt64Dictionary)init;
- (TRIPBStringInt64Dictionary)initWithCapacity:(unint64_t)a3;
- (TRIPBStringInt64Dictionary)initWithDictionary:(id)a3;
- (TRIPBStringInt64Dictionary)initWithInt64s:(const int64_t *)a3 forKeys:(id *)a4 count:(unint64_t)a5;
...
@implementation TRIPBStringInt64Dictionary

- (TRIPBStringInt64Dictionary)init
{
  return [(TRIPBStringInt64Dictionary *)self initWithInt64s:0 forKeys:0 count:0];
}

- (TRIPBStringInt64Dictionary)initWithInt64s:(const int64_t *)a3 forKeys:(id *)a4 count:(unint64_t)a5
{
  v15.receiver = self;
  v15.super_class = (Class)TRIPBStringInt64Dictionary;
  v8 = [(TRIPBStringInt64Dictionary *)&v15 init];
  if (v8)
  {
...
- (TRIPBStringInt64Dictionary)initWithDictionary:(id)a3
{
  v4 = [(TRIPBStringInt64Dictionary *)self initWithInt64s:0 forKeys:0 count:0];
  v5 = v4;
  if (a3 && v4) {
...
- (void)dealloc
{
  if (self->_autocreator) {
    objc_msgSend((id)objc_msgSend(MEMORY[0x1E4F28B00], "currentHandler"), "handleFailureInMethod:object:file:lineNumber:description:", a2, self, @"TRIPBDictionary.m", 9044, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator);
  }

  v3.receiver = self;
  v3.super_class = (Class)TRIPBStringInt64Dictionary;
  [(TRIPBStringInt64Dictionary *)&v3 dealloc];
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[TRIPBStringInt64Dictionary allocWithZone:a3];
  return [(TRIPBStringInt64Dictionary *)v4 initWithDictionary:self];
}
...
- (void)enumerateKeysAndInt64sUsingBlock:(id)a3
{
  ...
  do
  {
    ...
    (*((void (**)(id, uint64_t, uint64_t, char *))a3 + 2))(a3, v6, objc_msgSend((id)-[NSMutableDictionary objectForKeyedSubscript:](dictionary, "objectForKeyedSubscript:", v6), "longLongValue"), &v7);
  }
  while (!v7);
}
...
- (void)writeToCodedOutputStream:(id)a3 asField:(id)a4
{
  ...
  if (v12)
  {
    ...
    do
    {
      ...
      objc_msgSend(a3, "writeInt32NoTag:", ComputeDictInt64FieldSize(v16, 2, v7) + v17);
      WriteDictStringField(a3, (uint64_t)v13, v8);
      WriteDictInt64Field(a3, v16, 2, v7);
...
- (void)enumerateForTextFormat:(id)a3
{
  ...
  v3[1] = 3221225472;
  v3[2] = __53__TRIPBStringInt64Dictionary_enumerateForTextFormat___block_invoke;
  v3[3] = &unk_1E576C5A8;
  v3[4] = a3;
  [(TRIPBStringInt64Dictionary *)self enumerateKeysAndInt64sUsingBlock:v3];
}

uint64_t __53__TRIPBStringInt64Dictionary_enumerateForTextFormat___block_invoke(uint64_t a1, uint64_t a2, uint64_t a3)
{
  uint64_t v4 = *(void *)(a1 + 32);
  uint64_t v5 = objc_msgSend(NSString, "stringWithFormat:", @"%lld", a3);
...
- (void)setInt64:(int64_t)a3 forKey:(id)a4
{
  ...
  -[NSMutableDictionary setObject:forKey:](self->_dictionary, "setObject:forKey:", [NSNumber numberWithLongLong:a3], a4);
  autocreator = self->_autocreator;
  if (autocreator)
  {
    TRIPBAutocreatedDictionaryModified(autocreator, (uint64_t)self);
