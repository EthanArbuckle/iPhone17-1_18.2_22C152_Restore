...
- (BOOL)isEqual:(id)a3;
- (TRIPBStringDoubleDictionary)init;
- (TRIPBStringDoubleDictionary)initWithCapacity:(unint64_t)a3;
- (TRIPBStringDoubleDictionary)initWithDictionary:(id)a3;
- (TRIPBStringDoubleDictionary)initWithDoubles:(const double *)a3 forKeys:(id *)a4 count:(unint64_t)a5;
...
@implementation TRIPBStringDoubleDictionary

- (TRIPBStringDoubleDictionary)init
{
  return [(TRIPBStringDoubleDictionary *)self initWithDoubles:0 forKeys:0 count:0];
}

- (TRIPBStringDoubleDictionary)initWithDoubles:(const double *)a3 forKeys:(id *)a4 count:(unint64_t)a5
{
  v15.receiver = self;
  v15.super_class = (Class)TRIPBStringDoubleDictionary;
  v8 = [(TRIPBStringDoubleDictionary *)&v15 init];
  if (v8)
  {
...
- (TRIPBStringDoubleDictionary)initWithDictionary:(id)a3
{
  v4 = [(TRIPBStringDoubleDictionary *)self initWithDoubles:0 forKeys:0 count:0];
  v5 = v4;
  if (a3 && v4) {
...
- (void)dealloc
{
  if (self->_autocreator) {
    objc_msgSend((id)objc_msgSend(MEMORY[0x1E4F28B00], "currentHandler"), "handleFailureInMethod:object:file:lineNumber:description:", a2, self, @"TRIPBDictionary.m", 9617, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator);
  }

  v3.receiver = self;
  v3.super_class = (Class)TRIPBStringDoubleDictionary;
  [(TRIPBStringDoubleDictionary *)&v3 dealloc];
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[TRIPBStringDoubleDictionary allocWithZone:a3];
  return [(TRIPBStringDoubleDictionary *)v4 initWithDictionary:self];
}
...
- (void)enumerateKeysAndDoublesUsingBlock:(id)a3
{
  ...
  do
  {
    ...
    uint64_t v7 = v6;
    objc_msgSend((id)-[NSMutableDictionary objectForKeyedSubscript:](dictionary, "objectForKeyedSubscript:", v6), "doubleValue");
...
- (void)writeToCodedOutputStream:(id)a3 asField:(id)a4
{
  ...
  if (v12)
  {
    ...
    do
    {
      ...
      WriteDictDoubleField(a3, v7, v17);
...
- (void)enumerateForTextFormat:(id)a3
{
  ...
  v3[1] = 3221225472;
  v3[2] = __54__TRIPBStringDoubleDictionary_enumerateForTextFormat___block_invoke;
  v3[3] = &unk_1E576C620;
  v3[4] = a3;
  [(TRIPBStringDoubleDictionary *)self enumerateKeysAndDoublesUsingBlock:v3];
}

uint64_t __54__TRIPBStringDoubleDictionary_enumerateForTextFormat___block_invoke(uint64_t a1, uint64_t a2, double a3)
{
  uint64_t v4 = *(void *)(a1 + 32);
  uint64_t v5 = objc_msgSend(NSString, "stringWithFormat:", @"%.*lg", 15, *(void *)&a3);
...
- (void)setDouble:(double)a3 forKey:(id)a4
{
  ...
  -[NSMutableDictionary setObject:forKey:](self->_dictionary, "setObject:forKey:", [NSNumber numberWithDouble:a3], a4);
  autocreator = self->_autocreator;
  if (autocreator)
  {
    TRIPBAutocreatedDictionaryModified(autocreator, (uint64_t)self);
