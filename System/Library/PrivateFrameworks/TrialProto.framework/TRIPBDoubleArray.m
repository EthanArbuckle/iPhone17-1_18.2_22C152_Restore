...
- (BOOL)isEqual:(id)a3;
- (TRIPBDoubleArray)init;
- (TRIPBDoubleArray)initWithCapacity:(unint64_t)a3;
- (TRIPBDoubleArray)initWithValueArray:(id)a3;
- (TRIPBDoubleArray)initWithValues:(const double *)a3 count:(unint64_t)a4;
...
@implementation TRIPBDoubleArray
...
{
  v3.receiver = self;
  v3.super_class = (Class)TRIPBDoubleArray;
  return [(TRIPBDoubleArray *)&v3 init];
}

- (TRIPBDoubleArray)initWithValueArray:(id)a3
{
  return [(TRIPBDoubleArray *)self initWithValues:*((void *)a3 + 2) count:*((void *)a3 + 3)];
}

- (TRIPBDoubleArray)initWithValues:(const double *)a3 count:(unint64_t)a4
{
  v6 = [(TRIPBDoubleArray *)self init];
  v7 = v6;
  if (v6 && a3 && a4)
  {
...
- (TRIPBDoubleArray)initWithCapacity:(unint64_t)a3
{
  double v4 = [(TRIPBDoubleArray *)self initWithValues:0 count:0];
  v5 = v4;
  if (a3 && v4) {
    [(TRIPBDoubleArray *)v4 internalResizeToCapacity:a3];
  }
  return v5;
}
...
- (id)copyWithZone:(_NSZone *)a3
{
  double v4 = +[TRIPBDoubleArray allocWithZone:a3];
  values = self->_values;
  unint64_t count = self->_count;
  return [(TRIPBDoubleArray *)v4 initWithValues:values count:count];
}

- (void)dealloc
{
  if (self->_autocreator) {
    objc_msgSend((id)objc_msgSend(MEMORY[0x1E4F28B00], "currentHandler"), "handleFailureInMethod:object:file:lineNumber:description:", a2, self, @"TRIPBArray.m", 1613, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator);
  }
  free(self->_values);
  v3.receiver = self;
  v3.super_class = (Class)TRIPBDoubleArray;
  [(TRIPBDoubleArray *)&v3 dealloc];
}
...
- (id)description
{
  ...
  if (count)
  {
    for (uint64_t i = 0; i != count; ++i)
    {
      ...
      objc_msgSend(v3, "appendFormat:", v6, *(void *)&self->_values[i]);
    }
  }
  [v3 appendFormat:@" }"];
  return v3;
...
{
  double v3 = a3;
  [(TRIPBDoubleArray *)self addValues:&v3 count:1];
}
...
- (void)addValues:(const double *)a3 count:(unint64_t)a4
{
  if (a3 && a4)
  {
    ...
    if (count + a4 > self->_capacity) {
      [(TRIPBDoubleArray *)self internalResizeToCapacity:(v8 & 0xFFFFFFFFFFFFFFF0) + 16];
    }
...
- (void)removeValueAtIndex:(unint64_t)a3
{
  ...
  {
    [(TRIPBDoubleArray *)self internalResizeToCapacity:(v6 & 0xFFFFFFFFFFFFFFF0) + 16];
  }
}
...
- (void)removeAll
{
  ...
  if (self->_capacity >= 0x21) {
    [(TRIPBDoubleArray *)self internalResizeToCapacity:16];
  }
}
