...
- (BOOL)isEqual:(id)a3;
- (TRIPBStringEnumDictionary)init;
- (TRIPBStringEnumDictionary)initWithDictionary:(id)a3;
- (TRIPBStringEnumDictionary)initWithValidationFunction:(void *)a3;
- (TRIPBStringEnumDictionary)initWithValidationFunction:(void *)a3 capacity:(unint64_t)a4;
- (TRIPBStringEnumDictionary)initWithValidationFunction:(void *)a3 rawValues:(const int *)a4 forKeys:(id *)a5 count:(unint64_t)a6;
...
@implementation TRIPBStringEnumDictionary

- (TRIPBStringEnumDictionary)init
{
  return [(TRIPBStringEnumDictionary *)self initWithValidationFunction:0 rawValues:0 forKeys:0 count:0];
}

- (TRIPBStringEnumDictionary)initWithValidationFunction:(void *)a3
{
  return [(TRIPBStringEnumDictionary *)self initWithValidationFunction:a3 rawValues:0 forKeys:0 count:0];
}

- (TRIPBStringEnumDictionary)initWithValidationFunction:(void *)a3 rawValues:(const int *)a4 forKeys:(id *)a5 count:(unint64_t)a6
{
  v19.receiver = self;
  v19.super_class = (Class)TRIPBStringEnumDictionary;
  v10 = [(TRIPBStringEnumDictionary *)&v19 init];
  if (v10)
  {
    v11 = (NSMutableDictionary *)objc_alloc_init(MEMORY[0x1E4F1CA60]);
    v12 = a3 ? a3 : DictDefault_IsValidValue;
    v10->_dictionary = v11;
    ...
    if (a5)
    {
      if (a4 && a6)
      {
        ...
        {
          if (!*a5) {
            [MEMORY[0x1E4F1CA00] raise:v13 format:@"Attempting to add nil key to a Dictionary"];
          }
          dictionary = v10->_dictionary;
          unsigned int v15 = *a4++;
          uint64_t v16 = [NSNumber numberWithInt:v15];
          uint64_t v17 = (uint64_t)*a5++;
          [(NSMutableDictionary *)dictionary setObject:v16 forKey:v17];
          --a6;
        }
        while (a6);
      }
...
- (TRIPBStringEnumDictionary)initWithDictionary:(id)a3
{
  v4 = -[TRIPBStringEnumDictionary initWithValidationFunction:rawValues:forKeys:count:](self, "initWithValidationFunction:rawValues:forKeys:count:", [a3 validationFunc], 0, 0, 0);
  v5 = v4;
  if (a3 && v4) {
...
- (void)dealloc
{
  if (self->_autocreator) {
    objc_msgSend((id)objc_msgSend(MEMORY[0x1E4F28B00], "currentHandler"), "handleFailureInMethod:object:file:lineNumber:description:", a2, self, @"TRIPBDictionary.m", 9821, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator);
  }

  v3.receiver = self;
  v3.super_class = (Class)TRIPBStringEnumDictionary;
  [(TRIPBStringEnumDictionary *)&v3 dealloc];
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[TRIPBStringEnumDictionary allocWithZone:a3];
  return [(TRIPBStringEnumDictionary *)v4 initWithDictionary:self];
}
...
- (void)writeToCodedOutputStream:(id)a3 asField:(id)a4
{
  ...
  if (v12)
  {
    ...
    do
    {
      ...
      objc_msgSend(a3, "writeInt32NoTag:", ComputeDictEnumFieldSize(v16, v7) + v17);
      WriteDictStringField(a3, (uint64_t)v13, v8);
      WriteDictEnumField(a3, v16, v7);
...
- (id)serializedDataForUnknownValue:(int)a3 forKey:(id *)a4 keyDataType:(unsigned __int8)a5
{
  unsigned int v5 = a5;
  uint64_t v7 = *(void *)&a3;
  uint64_t v8 = ComputeDictStringFieldSize(a4->var7, a5);
  uint64_t v9 = 2;
  uint64_t v10 = 3;
  uint64_t v11 = 4;
  uint64_t v12 = 5;
  ...
  WriteDictStringField(v14, a4->var2, v5);
...
- (void)enumerateForTextFormat:(id)a3
{
  ...
  v3[1] = 3221225472;
  v3[2] = __52__TRIPBStringEnumDictionary_enumerateForTextFormat___block_invoke;
  v3[3] = &unk_1E576C558;
  v3[4] = a3;
  [(TRIPBStringEnumDictionary *)self enumerateKeysAndRawValuesUsingBlock:v3];
}

uint64_t __52__TRIPBStringEnumDictionary_enumerateForTextFormat___block_invoke(uint64_t a1, uint64_t a2)
{
  uint64_t v3 = *(void *)(a1 + 32);
  uint64_t v4 = objc_msgSend(NSNumber, "numberWithInt:");
  unsigned int v5 = *(uint64_t (**)(uint64_t, uint64_t, uint64_t))(v3 + 16);
  return v5(v3, a2, v4);
}

- (BOOL)getEnum:(int *)a3 forKey:(id)a4
{
  uint64_t v6 = (void *)[(NSMutableDictionary *)self->_dictionary objectForKey:a4];
  uint64_t v7 = v6;
  if (a3 && v6)
  {
    int v8 = [v6 intValue];
...
- (BOOL)getRawValue:(int *)a3 forKey:(id)a4
{
  unsigned int v5 = (void *)[(NSMutableDictionary *)self->_dictionary objectForKey:a4];
  uint64_t v6 = v5;
  if (a3 && v5) {
    *a3 = [v5 intValue];
  }
...
- (void)enumerateKeysAndEnumsUsingBlock:(id)a3
{
  ...
  do
  {
    ...
    uint64_t v8 = v7;
    unsigned int v9 = objc_msgSend((id)-[NSMutableDictionary objectForKeyedSubscript:](self->_dictionary, "objectForKeyedSubscript:", v7), "intValue");
    uint64_t v10 = validationFunc() ? v9 : 4222467823;
    (*((void (**)(id, uint64_t, uint64_t, char *))a3 + 2))(a3, v8, v10, &v11);
  }
  while (!v11);
}
...
- (void)setEnum:(int)a3 forKey:(id)a4
{
  ...
  if ((((uint64_t (*)(uint64_t, SEL))self->_validationFunc)(v5, a2) & 1) == 0) {
    objc_msgSend(MEMORY[0x1E4F1CA00], "raise:format:", *MEMORY[0x1E4F1C3C8], @"TRIPBStringEnumDictionary: Attempt to set an unknown enum value (%d)", v5);
  }
  -[NSMutableDictionary setObject:forKey:](self->_dictionary, "setObject:forKey:", [NSNumber numberWithInt:v5], a4);
  autocreator = self->_autocreator;
  if (autocreator)
