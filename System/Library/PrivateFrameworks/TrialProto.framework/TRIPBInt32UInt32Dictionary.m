...
- (BOOL)isEqual:(id)a3;
- (TRIPBInt32UInt32Dictionary)init;
- (TRIPBInt32UInt32Dictionary)initWithCapacity:(unint64_t)a3;
- (TRIPBInt32UInt32Dictionary)initWithDictionary:(id)a3;
- (TRIPBInt32UInt32Dictionary)initWithUInt32s:(const unsigned int *)a3 forKeys:(const int *)a4 count:(unint64_t)a5;
...
@implementation TRIPBInt32UInt32Dictionary

- (TRIPBInt32UInt32Dictionary)init
{
  return [(TRIPBInt32UInt32Dictionary *)self initWithUInt32s:0 forKeys:0 count:0];
}

- (TRIPBInt32UInt32Dictionary)initWithUInt32s:(const unsigned int *)a3 forKeys:(const int *)a4 count:(unint64_t)a5
{
  v14.receiver = self;
  v14.super_class = (Class)TRIPBInt32UInt32Dictionary;
  v8 = [(TRIPBInt32UInt32Dictionary *)&v14 init];
  if (v8)
  {
    ...
    if (a4)
    {
      if (a3 && a5)
      {
        do
        {
          ...
          unsigned int v12 = *a4++;
          -[NSMutableDictionary setObject:forKey:](dictionary, "setObject:forKey:", v11, [NSNumber numberWithInt:v12]);
          --a5;
        }
        while (a5);
      }
...
- (TRIPBInt32UInt32Dictionary)initWithDictionary:(id)a3
{
  v4 = [(TRIPBInt32UInt32Dictionary *)self initWithUInt32s:0 forKeys:0 count:0];
  v5 = v4;
  if (a3 && v4) {
...
- (void)dealloc
{
  if (self->_autocreator) {
    objc_msgSend((id)objc_msgSend(MEMORY[0x1E4F28B00], "currentHandler"), "handleFailureInMethod:object:file:lineNumber:description:", a2, self, @"TRIPBDictionary.m", 3223, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator);
  }

  v3.receiver = self;
  v3.super_class = (Class)TRIPBInt32UInt32Dictionary;
  [(TRIPBInt32UInt32Dictionary *)&v3 dealloc];
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[TRIPBInt32UInt32Dictionary allocWithZone:a3];
  return [(TRIPBInt32UInt32Dictionary *)v4 initWithDictionary:self];
}
...
- (void)enumerateKeysAndUInt32sUsingBlock:(id)a3
{
  ...
  do
  {
    ...
    (*((void (**)(id, uint64_t, uint64_t, char *))a3 + 2))(a3, [v6 intValue], objc_msgSend((id)-[NSMutableDictionary objectForKeyedSubscript:](dictionary, "objectForKeyedSubscript:", v6), "unsignedIntValue"), &v7);
  }
  while (!v7);
}
...
- (void)enumerateForTextFormat:(id)a3
{
  ...
  v3[1] = 3221225472;
  v3[2] = __53__TRIPBInt32UInt32Dictionary_enumerateForTextFormat___block_invoke;
  v3[3] = &unk_1E576C170;
  v3[4] = a3;
  [(TRIPBInt32UInt32Dictionary *)self enumerateKeysAndUInt32sUsingBlock:v3];
}

uint64_t __53__TRIPBInt32UInt32Dictionary_enumerateForTextFormat___block_invoke(uint64_t a1, uint64_t a2, uint64_t a3)
{
