...
- (BOOL)isEqual:(id)a3;
- (TRIPBInt64UInt32Dictionary)init;
- (TRIPBInt64UInt32Dictionary)initWithCapacity:(unint64_t)a3;
- (TRIPBInt64UInt32Dictionary)initWithDictionary:(id)a3;
- (TRIPBInt64UInt32Dictionary)initWithUInt32s:(const unsigned int *)a3 forKeys:(const int64_t *)a4 count:(unint64_t)a5;
...
- (void)setUInt32:(unsigned int)a3 forKey:(int64_t)a4;
- (void)writeToCodedOutputStream:(id)a3 asField:(id)a4;
@end

@implementation TRIPBInt64UInt32Dictionary

- (TRIPBInt64UInt32Dictionary)init
{
  return [(TRIPBInt64UInt32Dictionary *)self initWithUInt32s:0 forKeys:0 count:0];
}

- (TRIPBInt64UInt32Dictionary)initWithUInt32s:(const unsigned int *)a3 forKeys:(const int64_t *)a4 count:(unint64_t)a5
{
  v14.receiver = self;
  v14.super_class = (Class)TRIPBInt64UInt32Dictionary;
  v8 = [(TRIPBInt64UInt32Dictionary *)&v14 init];
  if (v8)
  {
...
- (TRIPBInt64UInt32Dictionary)initWithDictionary:(id)a3
{
  v4 = [(TRIPBInt64UInt32Dictionary *)self initWithUInt32s:0 forKeys:0 count:0];
  v5 = v4;
  if (a3 && v4) {
...
- (void)dealloc
{
  if (self->_autocreator) {
    objc_msgSend((id)objc_msgSend(MEMORY[0x1E4F28B00], "currentHandler"), "handleFailureInMethod:object:file:lineNumber:description:", a2, self, @"TRIPBDictionary.m", 6719, @"%@: Autocreator must be cleared before release, autocreator: %@", objc_opt_class(), self->_autocreator);
  }

  v3.receiver = self;
  v3.super_class = (Class)TRIPBInt64UInt32Dictionary;
  [(TRIPBInt64UInt32Dictionary *)&v3 dealloc];
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = +[TRIPBInt64UInt32Dictionary allocWithZone:a3];
  return [(TRIPBInt64UInt32Dictionary *)v4 initWithDictionary:self];
}
...
- (void)enumerateKeysAndUInt32sUsingBlock:(id)a3
{
  ...
  do
  {
    ...
    (*((void (**)(id, uint64_t, uint64_t, char *))a3 + 2))(a3, [v6 longLongValue], objc_msgSend((id)-[NSMutableDictionary objectForKeyedSubscript:](dictionary, "objectForKeyedSubscript:", v6), "unsignedIntValue"), &v7);
  }
  while (!v7);
}
...
- (void)enumerateForTextFormat:(id)a3
{
  ...
  v3[1] = 3221225472;
  v3[2] = __53__TRIPBInt64UInt32Dictionary_enumerateForTextFormat___block_invoke;
  v3[3] = &unk_1E576C3F0;
  v3[4] = a3;
  [(TRIPBInt64UInt32Dictionary *)self enumerateKeysAndUInt32sUsingBlock:v3];
}

uint64_t __53__TRIPBInt64UInt32Dictionary_enumerateForTextFormat___block_invoke(uint64_t a1, uint64_t a2, uint64_t a3)
{
