...
+ (id)longExposureFusionKernels
{
  ...
  v2 = (void *)longExposureFusionKernels_kernels;
  return v2;
}

uint64_t __46__PIAutoLoopKernels_longExposureFusionKernels__block_invoke()
{
  longExposureFusionKernels_kernels = [MEMORY[0x1E4F1E058] kernelsDictionaryWithString:&cfstr_KernelVec4Join];
...
+ (id)alphaCompositingKernel
{
  if (alphaCompositingKernel_onceToken != -1) {
    dispatch_once(&alphaCompositingKernel_onceToken, &__block_literal_global_12300);
  }
  v2 = (void *)alphaCompositingKernel_kernel;
  return v2;
}

uint64_t __43__PIAutoLoopKernels_alphaCompositingKernel__block_invoke()
{
  alphaCompositingKernel_kernel = [MEMORY[0x1E4F1E010] kernelWithString:@"kernel vec4 alphaCompositing(__sample src, __sample dst, float a)\n{\nreturn mix(dst, src, a);\n}\n\n"];
  return MEMORY[0x1F41817F8]();
}
