...
- (id)asError
{
  if ([(NSString *)self->_fenum isEqualToString:&stru_26EFA73B8]) {
    fenum = 0;
  }
  else {
    fenum = self->_fenum;
  ...
  if ([(NSString *)self->_category isEqualToString:&stru_26EFA73B8]) {
    category = 0;
  }
  else {
  ...
  if ([(NSString *)self->_customMessage isEqualToString:&stru_26EFA73B8]) {
    customMessage = 0;
  }
  else {
  ...
  if ([(NSString *)self->_customRecoverySuggestion isEqualToString:&stru_26EFA73B8]) {
  ...
  uint64_t v12 = v11;
  id v13 = (uint64_t *)MEMORY[0x263F08338];
  if (v6)
  {
    id v14 = [NSString stringWithFormat:@"Capturing %@ is not supported.", v6];
    [v12 setObject:v14 forKey:*MEMORY[0x263F08320]];

    if (!v4) {
      goto LABEL_19;
    }
    objc_super v15 = [NSString stringWithFormat:@"Unsupported method: %@", v4];
    uint64_t v16 = *v13;
  }
  else
  {
    if (!v4)
    {
      [v11 setObject:@"Capturing disabled. Unsupported API usage." forKey:*MEMORY[0x263F08320]];
      if (!v8) {
        goto LABEL_24;
      }
      goto LABEL_20;
    }
    objc_super v15 = [NSString stringWithFormat:@"Capturing %@ is not supported.", v4];
...
LABEL_20:
  uint64_t v17 = *v13;
  objc_super v18 = [v12 objectForKey:v17];
  v19 = v18;
  if (v18)
  {
...
LABEL_24:
  ...
  [v12 setObject:v10 forKey:*MEMORY[0x263F08348]];
  v22 = [MEMORY[0x263F087E8] errorWithDomain:@"com.apple.gputools.capture" code:1000 userInfo:v12];
