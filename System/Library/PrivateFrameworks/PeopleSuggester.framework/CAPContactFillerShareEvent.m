@interface CAPContactFillerShareEvent
+ (Class)contactEventType;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (CAPContactFillerUserEvent)userEvent;
- (NSMutableArray)contactEvents;
- (id)contactEventAtIndex:(unint64_t)a3;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
- (id)dictionaryRepresentation;
- (unint64_t)contactEventsCount;
- (unint64_t)hash;
- (void)addContactEvent:(id)a3;
- (void)clearContactEvents;
- (void)copyTo:(id)a3;
- (void)mergeFrom:(id)a3;
- (void)setContactEvents:(id)a3;
- (void)setUserEvent:(id)a3;
- (void)writeTo:(id)a3;
@end

@implementation CAPContactFillerShareEvent

- (void)clearContactEvents
{
}

- (void)addContactEvent:(id)a3
{
  id v4 = a3;
  contactEvents = self->_contactEvents;
  id v8 = v4;
  if (!contactEvents)
  {
    v6 = (NSMutableArray *)objc_alloc_init(MEMORY[0x1E4F1CA48]);
    v7 = self->_contactEvents;
    self->_contactEvents = v6;

    id v4 = v8;
    contactEvents = self->_contactEvents;
  }
  [(NSMutableArray *)contactEvents addObject:v4];
}

- (unint64_t)contactEventsCount
{
  return [(NSMutableArray *)self->_contactEvents count];
}

- (id)contactEventAtIndex:(unint64_t)a3
{
  return (id)[(NSMutableArray *)self->_contactEvents objectAtIndex:a3];
}

+ (Class)contactEventType
{
  return (Class)objc_opt_class();
}

- (id)description
{
  v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)CAPContactFillerShareEvent;
  id v4 = [(CAPContactFillerShareEvent *)&v8 description];
  v5 = [(CAPContactFillerShareEvent *)self dictionaryRepresentation];
  v6 = [v3 stringWithFormat:@"%@ %@", v4, v5];

  return v6;
}

- (id)dictionaryRepresentation
{
  uint64_t v19 = *MEMORY[0x1E4F143B8];
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  if ([(NSMutableArray *)self->_contactEvents count])
  {
    id v4 = objc_msgSend(objc_alloc(MEMORY[0x1E4F1CA48]), "initWithCapacity:", -[NSMutableArray count](self->_contactEvents, "count"));
    long long v14 = 0u;
    long long v15 = 0u;
    long long v16 = 0u;
    long long v17 = 0u;
    v5 = self->_contactEvents;
    uint64_t v6 = [(NSMutableArray *)v5 countByEnumeratingWithState:&v14 objects:v18 count:16];
    if (v6)
    {
      uint64_t v7 = v6;
      uint64_t v8 = *(void *)v15;
      do
      {
        for (uint64_t i = 0; i != v7; ++i)
        {
          if (*(void *)v15 != v8) {
            objc_enumerationMutation(v5);
          }
          v10 = objc_msgSend(*(id *)(*((void *)&v14 + 1) + 8 * i), "dictionaryRepresentation", (void)v14);
          [v4 addObject:v10];
        }
        uint64_t v7 = [(NSMutableArray *)v5 countByEnumeratingWithState:&v14 objects:v18 count:16];
      }
      while (v7);
    }

    [v3 setObject:v4 forKey:@"contactEvent"];
  }
  userEvent = self->_userEvent;
  if (userEvent)
  {
    v12 = [(CAPContactFillerUserEvent *)userEvent dictionaryRepresentation];
    [v3 setObject:v12 forKey:@"userEvent"];
  }

  return v3;
}

- (BOOL)readFrom:(id)a3
{
  return CAPContactFillerShareEventReadFrom((id *)&self->super.super.isa, (uint64_t)a3);
}

- (void)writeTo:(id)a3
{
  uint64_t v15 = *MEMORY[0x1E4F143B8];
  id v4 = a3;
  long long v10 = 0u;
  long long v11 = 0u;
  long long v12 = 0u;
  long long v13 = 0u;
  v5 = self->_contactEvents;
  uint64_t v6 = [(NSMutableArray *)v5 countByEnumeratingWithState:&v10 objects:v14 count:16];
  if (v6)
  {
    uint64_t v7 = v6;
    uint64_t v8 = *(void *)v11;
    do
    {
      uint64_t v9 = 0;
      do
      {
        if (*(void *)v11 != v8) {
          objc_enumerationMutation(v5);
        }
        PBDataWriterWriteSubmessage();
        ++v9;
      }
      while (v7 != v9);
      uint64_t v7 = [(NSMutableArray *)v5 countByEnumeratingWithState:&v10 objects:v14 count:16];
    }
    while (v7);
  }

  if (!self->_userEvent) {
    -[CAPContactFillerShareEvent writeTo:]();
  }
  PBDataWriterWriteSubmessage();
}

- (void)copyTo:(id)a3
{
  id v8 = a3;
  if ([(CAPContactFillerShareEvent *)self contactEventsCount])
  {
    [v8 clearContactEvents];
    unint64_t v4 = [(CAPContactFillerShareEvent *)self contactEventsCount];
    if (v4)
    {
      unint64_t v5 = v4;
      for (uint64_t i = 0; i != v5; ++i)
      {
        uint64_t v7 = [(CAPContactFillerShareEvent *)self contactEventAtIndex:i];
        [v8 addContactEvent:v7];
      }
    }
  }
  [v8 setUserEvent:self->_userEvent];
}

- (id)copyWithZone:(_NSZone *)a3
{
  uint64_t v20 = *MEMORY[0x1E4F143B8];
  unint64_t v5 = objc_msgSend((id)objc_msgSend((id)objc_opt_class(), "allocWithZone:", a3), "init");
  long long v15 = 0u;
  long long v16 = 0u;
  long long v17 = 0u;
  long long v18 = 0u;
  uint64_t v6 = self->_contactEvents;
  uint64_t v7 = [(NSMutableArray *)v6 countByEnumeratingWithState:&v15 objects:v19 count:16];
  if (v7)
  {
    uint64_t v8 = v7;
    uint64_t v9 = *(void *)v16;
    do
    {
      uint64_t v10 = 0;
      do
      {
        if (*(void *)v16 != v9) {
          objc_enumerationMutation(v6);
        }
        long long v11 = objc_msgSend(*(id *)(*((void *)&v15 + 1) + 8 * v10), "copyWithZone:", a3, (void)v15);
        [v5 addContactEvent:v11];

        ++v10;
      }
      while (v8 != v10);
      uint64_t v8 = [(NSMutableArray *)v6 countByEnumeratingWithState:&v15 objects:v19 count:16];
    }
    while (v8);
  }

  id v12 = [(CAPContactFillerUserEvent *)self->_userEvent copyWithZone:a3];
  long long v13 = (void *)v5[2];
  v5[2] = v12;

  return v5;
}

- (BOOL)isEqual:(id)a3
{
  unint64_t v4 = a3;
  if ([v4 isMemberOfClass:objc_opt_class()]
    && ((contactEvents = self->_contactEvents, !((unint64_t)contactEvents | v4[1]))
     || -[NSMutableArray isEqual:](contactEvents, "isEqual:")))
  {
    userEvent = self->_userEvent;
    if ((unint64_t)userEvent | v4[2]) {
      char v7 = -[CAPContactFillerUserEvent isEqual:](userEvent, "isEqual:");
    }
    else {
      char v7 = 1;
    }
  }
  else
  {
    char v7 = 0;
  }

  return v7;
}

- (unint64_t)hash
{
  uint64_t v3 = [(NSMutableArray *)self->_contactEvents hash];
  return [(CAPContactFillerUserEvent *)self->_userEvent hash] ^ v3;
}

- (void)mergeFrom:(id)a3
{
  uint64_t v17 = *MEMORY[0x1E4F143B8];
  unint64_t v4 = (id *)a3;
  long long v12 = 0u;
  long long v13 = 0u;
  long long v14 = 0u;
  long long v15 = 0u;
  id v5 = v4[1];
  uint64_t v6 = [v5 countByEnumeratingWithState:&v12 objects:v16 count:16];
  if (v6)
  {
    uint64_t v7 = v6;
    uint64_t v8 = *(void *)v13;
    do
    {
      for (uint64_t i = 0; i != v7; ++i)
      {
        if (*(void *)v13 != v8) {
          objc_enumerationMutation(v5);
        }
        -[CAPContactFillerShareEvent addContactEvent:](self, "addContactEvent:", *(void *)(*((void *)&v12 + 1) + 8 * i), (void)v12);
      }
      uint64_t v7 = [v5 countByEnumeratingWithState:&v12 objects:v16 count:16];
    }
    while (v7);
  }

  userEvent = self->_userEvent;
  id v11 = v4[2];
  if (userEvent)
  {
    if (v11) {
      -[CAPContactFillerUserEvent mergeFrom:](userEvent, "mergeFrom:");
    }
  }
  else if (v11)
  {
    -[CAPContactFillerShareEvent setUserEvent:](self, "setUserEvent:");
  }
}

- (NSMutableArray)contactEvents
{
  return self->_contactEvents;
}

- (void)setContactEvents:(id)a3
{
}

- (CAPContactFillerUserEvent)userEvent
{
  return self->_userEvent;
}

- (void)setUserEvent:(id)a3
{
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_userEvent, 0);

  objc_storeStrong((id *)&self->_contactEvents, 0);
}

- (void)writeTo:.cold.1()
{
  __assert_rtn("-[CAPContactFillerShareEvent writeTo:]", "CAPContactFillerShareEvent.m", 146, "self->_userEvent != nil");
}

@end