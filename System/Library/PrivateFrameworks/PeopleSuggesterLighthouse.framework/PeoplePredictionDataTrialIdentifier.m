@interface PeoplePredictionDataTrialIdentifier
- (BOOL)hasDeployment;
- (BOOL)hasTask;
- (BOOL)hasTreatment;
- (BOOL)isEqual:(id)a3;
...
@implementation PeoplePredictionDataTrialIdentifier
...
- (id)description
{
  v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)PeoplePredictionDataTrialIdentifier;
  v4 = [(PeoplePredictionDataTrialIdentifier *)&v8 description];
  v5 = [(PeoplePredictionDataTrialIdentifier *)self dictionaryRepresentation];
...
- (BOOL)readFrom:(id)a3
{
  return PeoplePredictionDataTrialIdentifierReadFrom((uint64_t)self, (uint64_t)a3);
}
...
- (void)mergeFrom:(id)a3
{
  NSUInteger v4 = a3;
  if (v4[2]) {
    -[PeoplePredictionDataTrialIdentifier setTask:](self, "setTask:");
  }
  if (v4[3]) {
    -[PeoplePredictionDataTrialIdentifier setTreatment:](self, "setTreatment:");
  }
  if (v4[1]) {
    -[PeoplePredictionDataTrialIdentifier setDeployment:](self, "setDeployment:");
  }
}
