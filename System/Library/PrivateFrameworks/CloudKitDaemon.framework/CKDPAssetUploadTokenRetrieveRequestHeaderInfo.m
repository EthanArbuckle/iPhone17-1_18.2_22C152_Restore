...
@implementation CKDPAssetUploadTokenRetrieveRequestHeaderInfo

- (id)description
{
  v3 = NSString;
  v11.receiver = self;
  v11.super_class = (Class)CKDPAssetUploadTokenRetrieveRequestHeaderInfo;
  v4 = [(CKDPAssetUploadTokenRetrieveRequestHeaderInfo *)&v11 description];
...
- (BOOL)readFrom:(id)a3
{
  return sub_1C4E12A38((uint64_t)self, (uint64_t)a3);
}
...
- (void)writeTo:(id)a3
{
  id v4 = a3;
  if (!self->_headerKey) {
    __assert_rtn("-[CKDPAssetUploadTokenRetrieveRequestHeaderInfo writeTo:]", "CKDPAssetUploadTokenRetrieveRequestHeaderInfo.m", 83, "nil != self->_headerKey");
  }
  id v5 = v4;
  PBDataWriterWriteStringField();
  if (!self->_headerValue) {
    __assert_rtn("-[CKDPAssetUploadTokenRetrieveRequestHeaderInfo writeTo:]", "CKDPAssetUploadTokenRetrieveRequestHeaderInfo.m", 88, "nil != self->_headerValue");
  }
  PBDataWriterWriteStringField();
}
