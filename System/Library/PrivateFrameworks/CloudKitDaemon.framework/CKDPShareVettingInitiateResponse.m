@interface CKDPShareVettingInitiateResponse
- (BOOL)hasVettingError;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
- (id)dictionaryRepresentation;
- (id)vettingErrorAsString:(int)a3;
- (int)StringAsVettingError:(id)a3;
- (int)vettingError;
- (unint64_t)hash;
- (void)copyTo:(id)a3;
- (void)mergeFrom:(id)a3;
- (void)setHasVettingError:(BOOL)a3;
- (void)setVettingError:(int)a3;
- (void)writeTo:(id)a3;
@end

@implementation CKDPShareVettingInitiateResponse

- (int)vettingError
{
  if (*(unsigned char *)&self->_has) {
    return self->_vettingError;
  }
  else {
    return 1;
  }
}

- (void)setVettingError:(int)a3
{
  *(unsigned char *)&self->_has |= 1u;
  self->_vettingError = a3;
}

- (void)setHasVettingError:(BOOL)a3
{
  *(unsigned char *)&self->_has = *(unsigned char *)&self->_has & 0xFE | a3;
}

- (BOOL)hasVettingError
{
  return *(unsigned char *)&self->_has & 1;
}

- (id)vettingErrorAsString:(int)a3
{
  if ((a3 - 1) >= 5)
  {
    objc_msgSend_stringWithFormat_(NSString, a2, @"(unknown: %i)", a3);
    v3 = (__CFString *)objc_claimAutoreleasedReturnValue();
  }
  else
  {
    v3 = off_1E64F7070[a3 - 1];
  }
  return v3;
}

- (int)StringAsVettingError:(id)a3
{
  id v3 = a3;
  if (objc_msgSend_isEqualToString_(v3, v4, @"isVettedToCaller"))
  {
    int v6 = 1;
  }
  else if (objc_msgSend_isEqualToString_(v3, v5, @"isVettedToOther"))
  {
    int v6 = 2;
  }
  else if (objc_msgSend_isEqualToString_(v3, v7, @"isLimitExceeded"))
  {
    int v6 = 3;
  }
  else if (objc_msgSend_isEqualToString_(v3, v8, @"serverError"))
  {
    int v6 = 4;
  }
  else if (objc_msgSend_isEqualToString_(v3, v9, @"loginRequired"))
  {
    int v6 = 5;
  }
  else
  {
    int v6 = 1;
  }

  return v6;
}

- (id)description
{
  id v3 = NSString;
  v11.receiver = self;
  v11.super_class = (Class)CKDPShareVettingInitiateResponse;
  v4 = [(CKDPShareVettingInitiateResponse *)&v11 description];
  v7 = objc_msgSend_dictionaryRepresentation(self, v5, v6);
  v9 = objc_msgSend_stringWithFormat_(v3, v8, @"%@ %@", v4, v7);

  return v9;
}

- (id)dictionaryRepresentation
{
  v5 = objc_msgSend_dictionary(MEMORY[0x1E4F1CA60], a2, v2);
  if (*(unsigned char *)&self->_has)
  {
    unsigned int v6 = self->_vettingError - 1;
    if (v6 >= 5)
    {
      objc_msgSend_stringWithFormat_(NSString, v4, @"(unknown: %i)", self->_vettingError);
      v7 = (__CFString *)objc_claimAutoreleasedReturnValue();
    }
    else
    {
      v7 = off_1E64F7070[v6];
    }
    objc_msgSend_setObject_forKey_(v5, v4, (uint64_t)v7, @"vettingError");
  }
  return v5;
}

- (BOOL)readFrom:(id)a3
{
  return sub_1C4F6CBD4((uint64_t)self, (uint64_t)a3);
}

- (void)writeTo:(id)a3
{
  if (*(unsigned char *)&self->_has) {
    PBDataWriterWriteInt32Field();
  }
}

- (void)copyTo:(id)a3
{
  if (*(unsigned char *)&self->_has)
  {
    *((_DWORD *)a3 + 2) = self->_vettingError;
    *((unsigned char *)a3 + 12) |= 1u;
  }
}

- (id)copyWithZone:(_NSZone *)a3
{
  v5 = objc_opt_class();
  v7 = objc_msgSend_allocWithZone_(v5, v6, (uint64_t)a3);
  id result = (id)objc_msgSend_init(v7, v8, v9);
  if (*(unsigned char *)&self->_has)
  {
    *((_DWORD *)result + 2) = self->_vettingError;
    *((unsigned char *)result + 12) |= 1u;
  }
  return result;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = a3;
  uint64_t v5 = objc_opt_class();
  if (!objc_msgSend_isMemberOfClass_(v4, v6, v5)) {
    goto LABEL_6;
  }
  BOOL v7 = (*((unsigned char *)v4 + 12) & 1) == 0;
  if (*(unsigned char *)&self->_has)
  {
    if ((*((unsigned char *)v4 + 12) & 1) != 0 && self->_vettingError == *((_DWORD *)v4 + 2))
    {
      BOOL v7 = 1;
      goto LABEL_7;
    }
LABEL_6:
    BOOL v7 = 0;
  }
LABEL_7:

  return v7;
}

- (unint64_t)hash
{
  if (*(unsigned char *)&self->_has) {
    return 2654435761 * self->_vettingError;
  }
  else {
    return 0;
  }
}

- (void)mergeFrom:(id)a3
{
  if (*((unsigned char *)a3 + 12))
  {
    self->_vettingError = *((_DWORD *)a3 + 2);
    *(unsigned char *)&self->_has |= 1u;
  }
}

@end