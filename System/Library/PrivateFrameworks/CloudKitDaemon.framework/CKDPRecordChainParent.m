@interface CKDPRecordChainParent
- (BOOL)hasPublicKeyID;
- (BOOL)hasReference;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (CKDPRecordReference)reference;
- (NSData)publicKeyID;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
- (id)dictionaryRepresentation;
- (unint64_t)hash;
- (void)copyTo:(id)a3;
- (void)mergeFrom:(id)a3;
- (void)setPublicKeyID:(id)a3;
- (void)setReference:(id)a3;
- (void)writeTo:(id)a3;
@end

@implementation CKDPRecordChainParent

- (BOOL)hasPublicKeyID
{
  return self->_publicKeyID != 0;
}

- (BOOL)hasReference
{
  return self->_reference != 0;
}

- (id)description
{
  v3 = NSString;
  v11.receiver = self;
  v11.super_class = (Class)CKDPRecordChainParent;
  v4 = [(CKDPRecordChainParent *)&v11 description];
  v7 = objc_msgSend_dictionaryRepresentation(self, v5, v6);
  v9 = objc_msgSend_stringWithFormat_(v3, v8, @"%@ %@", v4, v7);

  return v9;
}

- (id)dictionaryRepresentation
{
  v4 = objc_msgSend_dictionary(MEMORY[0x1E4F1CA60], a2, v2);
  uint64_t v6 = v4;
  publicKeyID = self->_publicKeyID;
  if (publicKeyID) {
    objc_msgSend_setObject_forKey_(v4, v5, (uint64_t)publicKeyID, @"publicKeyID");
  }
  reference = self->_reference;
  if (reference)
  {
    v9 = objc_msgSend_dictionaryRepresentation(reference, v5, (uint64_t)publicKeyID);
    objc_msgSend_setObject_forKey_(v6, v10, (uint64_t)v9, @"reference");
  }
  return v6;
}

- (BOOL)readFrom:(id)a3
{
  return CKDPRecordChainParentReadFrom((uint64_t)self, (uint64_t)a3);
}

- (void)writeTo:(id)a3
{
  id v4 = a3;
  id v5 = v4;
  if (self->_publicKeyID)
  {
    PBDataWriterWriteDataField();
    id v4 = v5;
  }
  if (self->_reference)
  {
    PBDataWriterWriteSubmessage();
    id v4 = v5;
  }
}

- (void)copyTo:(id)a3
{
  id v4 = a3;
  publicKeyID = self->_publicKeyID;
  id v8 = v4;
  if (publicKeyID)
  {
    objc_msgSend_setPublicKeyID_(v4, v5, (uint64_t)publicKeyID);
    id v4 = v8;
  }
  reference = self->_reference;
  if (reference)
  {
    objc_msgSend_setReference_(v8, v5, (uint64_t)reference);
    id v4 = v8;
  }
}

- (id)copyWithZone:(_NSZone *)a3
{
  id v5 = objc_opt_class();
  v7 = objc_msgSend_allocWithZone_(v5, v6, (uint64_t)a3);
  v10 = objc_msgSend_init(v7, v8, v9);
  uint64_t v12 = objc_msgSend_copyWithZone_(self->_publicKeyID, v11, (uint64_t)a3);
  v13 = (void *)v10[1];
  v10[1] = v12;

  uint64_t v15 = objc_msgSend_copyWithZone_(self->_reference, v14, (uint64_t)a3);
  v16 = (void *)v10[2];
  v10[2] = v15;

  return v10;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = a3;
  uint64_t v5 = objc_opt_class();
  if (objc_msgSend_isMemberOfClass_(v4, v6, v5)
    && ((publicKeyID = self->_publicKeyID, uint64_t v9 = v4[1], !((unint64_t)publicKeyID | v9))
     || objc_msgSend_isEqual_(publicKeyID, v7, v9)))
  {
    reference = self->_reference;
    uint64_t v11 = v4[2];
    if ((unint64_t)reference | v11) {
      char isEqual = objc_msgSend_isEqual_(reference, v7, v11);
    }
    else {
      char isEqual = 1;
    }
  }
  else
  {
    char isEqual = 0;
  }

  return isEqual;
}

- (unint64_t)hash
{
  uint64_t v4 = objc_msgSend_hash(self->_publicKeyID, a2, v2);
  return objc_msgSend_hash(self->_reference, v5, v6) ^ v4;
}

- (void)mergeFrom:(id)a3
{
  uint64_t v4 = a3;
  uint64_t v5 = v4[1];
  id v8 = v4;
  if (v5)
  {
    objc_msgSend_setPublicKeyID_(self, (const char *)v4, v5);
    uint64_t v4 = v8;
  }
  reference = self->_reference;
  uint64_t v7 = v4[2];
  if (reference)
  {
    if (v7) {
      objc_msgSend_mergeFrom_(reference, (const char *)v4, v7);
    }
  }
  else if (v7)
  {
    objc_msgSend_setReference_(self, (const char *)v4, v7);
  }
  MEMORY[0x1F41817F8]();
}

- (NSData)publicKeyID
{
  return self->_publicKeyID;
}

- (void)setPublicKeyID:(id)a3
{
}

- (CKDPRecordReference)reference
{
  return self->_reference;
}

- (void)setReference:(id)a3
{
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_reference, 0);
  objc_storeStrong((id *)&self->_publicKeyID, 0);
}

@end