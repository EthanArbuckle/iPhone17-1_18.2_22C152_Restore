...
- (BOOL)go;
- (Class)generator;
- (GQPSLProcessor)initWithURL:(__CFURL *)a3 indexFileName:(id)a4 outputType:(int)a5 outputPath:(__CFString *)a6 previewRequest:(__QLPreviewRequest *)a7 progressiveHelper:(id)a8 generator:(Class)a9 zipArchive:(id)a10 cryptoKey:(id)a11;
- (GQPSLProcessor)initWithURL:(__CFURL *)a3 zipArchive:(id)a4 indexFileName:(id)a5 outputType:(int)a6 outputPath:(__CFString *)a7 previewRequest:(__QLPreviewRequest *)a8 progressiveHelper:(id)a9 generator:(Class)a10 cryptoKey:(id)a11;
...
- (GQPSLProcessor)initWithURL:(__CFURL *)a3 indexFileName:(id)a4 outputType:(int)a5 outputPath:(__CFString *)a6 previewRequest:(__QLPreviewRequest *)a7 progressiveHelper:(id)a8 generator:(Class)a9 zipArchive:(id)a10 cryptoKey:(id)a11
{
  ...
  if (v15)
  {
    ...
    [(GQSDocument *)v19 setCurrentWrapPointGenerator:v19];
    [(GQSDocument *)v16->super.mDocumentState pushObject:v17];
  }
  return v16;
}
...
{
  v8.receiver = self;
  v8.super_class = (Class)GQPSLProcessor;
  ...
  if (self->mGenerator)
  {
    ...
    if ((v4 & v3) == 1)
    {
      unsigned int v5 = [(objc_class *)self->mGenerator endPublication:[(GQSDocument *)self->super.mDocumentState root] state:self->super.mDocumentState];
      if (!v5) {
        return v5;
      }
      goto LABEL_7;
