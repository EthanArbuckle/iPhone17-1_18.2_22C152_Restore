@interface HKCodableSleepSummaryEntry
+ (Class)periodType;
- (BOOL)hasDateInterval;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (HKCodableDateInterval)dateInterval;
- (NSMutableArray)periods;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
- (id)dictionaryRepresentation;
- (id)periodAtIndex:(unint64_t)a3;
- (unint64_t)hash;
- (unint64_t)periodsCount;
- (void)addPeriod:(id)a3;
- (void)clearPeriods;
- (void)copyTo:(id)a3;
- (void)mergeFrom:(id)a3;
- (void)setDateInterval:(id)a3;
- (void)setPeriods:(id)a3;
- (void)writeTo:(id)a3;
@end

@implementation HKCodableSleepSummaryEntry

- (BOOL)hasDateInterval
{
  return self->_dateInterval != 0;
}

- (void)clearPeriods
{
}

- (void)addPeriod:(id)a3
{
  id v4 = a3;
  periods = self->_periods;
  id v8 = v4;
  if (!periods)
  {
    v6 = (NSMutableArray *)objc_alloc_init(MEMORY[0x1E4F1CA48]);
    v7 = self->_periods;
    self->_periods = v6;

    id v4 = v8;
    periods = self->_periods;
  }
  [(NSMutableArray *)periods addObject:v4];
}

- (unint64_t)periodsCount
{
  return [(NSMutableArray *)self->_periods count];
}

- (id)periodAtIndex:(unint64_t)a3
{
  return (id)[(NSMutableArray *)self->_periods objectAtIndex:a3];
}

+ (Class)periodType
{
  return (Class)objc_opt_class();
}

- (id)description
{
  v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)HKCodableSleepSummaryEntry;
  id v4 = [(HKCodableSleepSummaryEntry *)&v8 description];
  v5 = [(HKCodableSleepSummaryEntry *)self dictionaryRepresentation];
  v6 = [v3 stringWithFormat:@"%@ %@", v4, v5];

  return v6;
}

- (id)dictionaryRepresentation
{
  uint64_t v19 = *MEMORY[0x1E4F143B8];
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  dateInterval = self->_dateInterval;
  if (dateInterval)
  {
    v5 = [(HKCodableDateInterval *)dateInterval dictionaryRepresentation];
    [v3 setObject:v5 forKey:@"dateInterval"];
  }
  if ([(NSMutableArray *)self->_periods count])
  {
    v6 = objc_msgSend(objc_alloc(MEMORY[0x1E4F1CA48]), "initWithCapacity:", -[NSMutableArray count](self->_periods, "count"));
    long long v14 = 0u;
    long long v15 = 0u;
    long long v16 = 0u;
    long long v17 = 0u;
    v7 = self->_periods;
    uint64_t v8 = [(NSMutableArray *)v7 countByEnumeratingWithState:&v14 objects:v18 count:16];
    if (v8)
    {
      uint64_t v9 = v8;
      uint64_t v10 = *(void *)v15;
      do
      {
        for (uint64_t i = 0; i != v9; ++i)
        {
          if (*(void *)v15 != v10) {
            objc_enumerationMutation(v7);
          }
          v12 = objc_msgSend(*(id *)(*((void *)&v14 + 1) + 8 * i), "dictionaryRepresentation", (void)v14);
          [v6 addObject:v12];
        }
        uint64_t v9 = [(NSMutableArray *)v7 countByEnumeratingWithState:&v14 objects:v18 count:16];
      }
      while (v9);
    }

    [v3 setObject:v6 forKey:@"period"];
  }
  return v3;
}

- (BOOL)readFrom:(id)a3
{
  return HKCodableSleepSummaryEntryReadFrom((id *)&self->super.super.isa, (uint64_t)a3);
}

- (void)writeTo:(id)a3
{
  uint64_t v15 = *MEMORY[0x1E4F143B8];
  id v4 = a3;
  if (self->_dateInterval) {
    PBDataWriterWriteSubmessage();
  }
  long long v12 = 0u;
  long long v13 = 0u;
  long long v10 = 0u;
  long long v11 = 0u;
  v5 = self->_periods;
  uint64_t v6 = [(NSMutableArray *)v5 countByEnumeratingWithState:&v10 objects:v14 count:16];
  if (v6)
  {
    uint64_t v7 = v6;
    uint64_t v8 = *(void *)v11;
    do
    {
      uint64_t v9 = 0;
      do
      {
        if (*(void *)v11 != v8) {
          objc_enumerationMutation(v5);
        }
        PBDataWriterWriteSubmessage();
        ++v9;
      }
      while (v7 != v9);
      uint64_t v7 = [(NSMutableArray *)v5 countByEnumeratingWithState:&v10 objects:v14 count:16];
    }
    while (v7);
  }
}

- (void)copyTo:(id)a3
{
  id v8 = a3;
  if (self->_dateInterval) {
    objc_msgSend(v8, "setDateInterval:");
  }
  if ([(HKCodableSleepSummaryEntry *)self periodsCount])
  {
    [v8 clearPeriods];
    unint64_t v4 = [(HKCodableSleepSummaryEntry *)self periodsCount];
    if (v4)
    {
      unint64_t v5 = v4;
      for (uint64_t i = 0; i != v5; ++i)
      {
        uint64_t v7 = [(HKCodableSleepSummaryEntry *)self periodAtIndex:i];
        [v8 addPeriod:v7];
      }
    }
  }
}

- (id)copyWithZone:(_NSZone *)a3
{
  uint64_t v20 = *MEMORY[0x1E4F143B8];
  unint64_t v5 = objc_msgSend((id)objc_msgSend((id)objc_opt_class(), "allocWithZone:", a3), "init");
  id v6 = [(HKCodableDateInterval *)self->_dateInterval copyWithZone:a3];
  uint64_t v7 = (void *)v5[1];
  v5[1] = v6;

  long long v17 = 0u;
  long long v18 = 0u;
  long long v15 = 0u;
  long long v16 = 0u;
  id v8 = self->_periods;
  uint64_t v9 = [(NSMutableArray *)v8 countByEnumeratingWithState:&v15 objects:v19 count:16];
  if (v9)
  {
    uint64_t v10 = v9;
    uint64_t v11 = *(void *)v16;
    do
    {
      uint64_t v12 = 0;
      do
      {
        if (*(void *)v16 != v11) {
          objc_enumerationMutation(v8);
        }
        long long v13 = objc_msgSend(*(id *)(*((void *)&v15 + 1) + 8 * v12), "copyWithZone:", a3, (void)v15);
        [v5 addPeriod:v13];

        ++v12;
      }
      while (v10 != v12);
      uint64_t v10 = [(NSMutableArray *)v8 countByEnumeratingWithState:&v15 objects:v19 count:16];
    }
    while (v10);
  }

  return v5;
}

- (BOOL)isEqual:(id)a3
{
  unint64_t v4 = a3;
  if ([v4 isMemberOfClass:objc_opt_class()]
    && ((dateInterval = self->_dateInterval, !((unint64_t)dateInterval | v4[1]))
     || -[HKCodableDateInterval isEqual:](dateInterval, "isEqual:")))
  {
    periods = self->_periods;
    if ((unint64_t)periods | v4[2]) {
      char v7 = -[NSMutableArray isEqual:](periods, "isEqual:");
    }
    else {
      char v7 = 1;
    }
  }
  else
  {
    char v7 = 0;
  }

  return v7;
}

- (unint64_t)hash
{
  unint64_t v3 = [(HKCodableDateInterval *)self->_dateInterval hash];
  return [(NSMutableArray *)self->_periods hash] ^ v3;
}

- (void)mergeFrom:(id)a3
{
  uint64_t v17 = *MEMORY[0x1E4F143B8];
  id v4 = a3;
  dateInterval = self->_dateInterval;
  uint64_t v6 = *((void *)v4 + 1);
  if (dateInterval)
  {
    if (v6) {
      -[HKCodableDateInterval mergeFrom:](dateInterval, "mergeFrom:");
    }
  }
  else if (v6)
  {
    -[HKCodableSleepSummaryEntry setDateInterval:](self, "setDateInterval:");
  }
  long long v14 = 0u;
  long long v15 = 0u;
  long long v12 = 0u;
  long long v13 = 0u;
  id v7 = *((id *)v4 + 2);
  uint64_t v8 = [v7 countByEnumeratingWithState:&v12 objects:v16 count:16];
  if (v8)
  {
    uint64_t v9 = v8;
    uint64_t v10 = *(void *)v13;
    do
    {
      for (uint64_t i = 0; i != v9; ++i)
      {
        if (*(void *)v13 != v10) {
          objc_enumerationMutation(v7);
        }
        -[HKCodableSleepSummaryEntry addPeriod:](self, "addPeriod:", *(void *)(*((void *)&v12 + 1) + 8 * i), (void)v12);
      }
      uint64_t v9 = [v7 countByEnumeratingWithState:&v12 objects:v16 count:16];
    }
    while (v9);
  }
}

- (HKCodableDateInterval)dateInterval
{
  return self->_dateInterval;
}

- (void)setDateInterval:(id)a3
{
}

- (NSMutableArray)periods
{
  return self->_periods;
}

- (void)setPeriods:(id)a3
{
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_periods, 0);
  objc_storeStrong((id *)&self->_dateInterval, 0);
}

@end