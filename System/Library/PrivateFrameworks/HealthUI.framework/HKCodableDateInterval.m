@interface HKCodableDateInterval
- (BOOL)hasEndDate;
- (BOOL)hasStartDate;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
...
@implementation HKCodableDateInterval
...
- (id)description
{
  char v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)HKCodableDateInterval;
  v4 = [(HKCodableDateInterval *)&v8 description];
  v5 = [(HKCodableDateInterval *)self dictionaryRepresentation];
...
- (BOOL)readFrom:(id)a3
{
  return HKCodableDateIntervalReadFrom((uint64_t)self, (uint64_t)a3);
}
...
- (BOOL)isEqual:(id)a3
{
  ...
  {
    if ((*((unsigned char *)v4 + 24) & 2) == 0 || self->_startDate != *((double *)v4 + 2)) {
      goto LABEL_11;
    }
  }
...
LABEL_11:
  ...
  {
    if ((*((unsigned char *)v4 + 24) & 1) == 0 || self->_endDate != *((double *)v4 + 1)) {
      goto LABEL_11;
    }
    BOOL v5 = 1;
  }
