...
- (id)nextObject
{
  ...
  {
    id v3 = [(BWGraph *)self->super._graph _sourceNodes];
    unint64_t currentSourceIndex = self->_currentSourceIndex;
    if (currentSourceIndex >= [v3 count]) {
      return 0;
    }
    -[NSMutableArray bw_push:](self->_stack, "bw_push:", [v3 objectAtIndexedSubscript:self->_currentSourceIndex]);
    -[BWNodeEnumerator _updateVisitedCount:]((uint64_t)self, [v3 objectAtIndexedSubscript:self->_currentSourceIndex]);
    ++self->_currentSourceIndex;
  }
  stack = self->_stack;
  if (self->_ordering)
  {
    uint64_t v6 = -[BWDepthFirstEnumerator _nextUnvisitedChild:]((uint64_t)self, [(NSMutableArray *)stack bw_peek]);
    if (v6)
    {
      v7 = (void *)v6;
      do
  ...
  else
  {
    ...
    long long v19 = 0u;
    long long v20 = 0u;
    v9 = objc_msgSend((id)objc_msgSend(v8, "outputs", 0), "reverseObjectEnumerator");
    ...
    if (v10)
    {
      ...
      do
      {
        ...
        do
        {
          ...
          uint64_t v14 = objc_msgSend((id)objc_msgSend((id)objc_msgSend(*(id *)(*((void *)&v17 + 1) + 8 * v13), "connection"), "input"), "node");
          if (v14)
          {
            uint64_t v15 = v14;
...
- (uint64_t)_nextUnvisitedChild:(uint64_t)a1
{
  ...
  long long v11 = 0u;
  long long v12 = 0u;
  id v3 = objc_msgSend((id)objc_msgSend(a2, "outputs", 0), "reverseObjectEnumerator");
...
LABEL_4:
  ...
  while (1)
  {
    ...
    uint64_t v8 = objc_msgSend((id)objc_msgSend((id)objc_msgSend(*(id *)(*((void *)&v11 + 1) + 8 * v7), "connection"), "input"), "node");
    if (v8)
    {
      uint64_t v9 = v8;
