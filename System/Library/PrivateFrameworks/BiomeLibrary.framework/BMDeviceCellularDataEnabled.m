@interface BMDeviceCellularDataEnabled
+ (id)columns;
+ (id)eventWithData:(id)a3 dataVersion:(unsigned int)a4;
+ (id)protoFields;
+ (id)validKeyPaths;
+ (unsigned)latestDataVersion;
- (BMDeviceCellularDataEnabled)initWithJSONDictionary:(id)a3 error:(id *)a4;
- (BMDeviceCellularDataEnabled)initWithStarting:(id)a3;
- (BOOL)hasStarting;
- (BOOL)isEqual:(id)a3;
- (BOOL)starting;
...
@implementation BMDeviceCellularDataEnabled
...
- (BOOL)isEqual:(id)a3
{
  ...
  {
    id v5 = v4;
    if ([(BMDeviceCellularDataEnabled *)self hasStarting]
      || [v5 hasStarting])
    {
      if ([(BMDeviceCellularDataEnabled *)self hasStarting]
        && [v5 hasStarting])
      {
        BOOL v6 = [(BMDeviceCellularDataEnabled *)self starting];
...
- (id)jsonDictionary
{
  ...
  if ([(BMDeviceCellularDataEnabled *)self hasStarting])
  {
    uint64_t v3 = objc_msgSend(NSNumber, "numberWithBool:", -[BMDeviceCellularDataEnabled starting](self, "starting"));
    v9 = @"starting";
    if (v3)
    {
      id v4 = (void *)v3;
...
- (BMDeviceCellularDataEnabled)initWithJSONDictionary:(id)a3 error:(id *)a4
{
  ...
  if (!v6 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) != 0))
  {
    a4 = 0;
LABEL_4:
    self = [(BMDeviceCellularDataEnabled *)self initWithStarting:a4];
    int v7 = self;
    goto LABEL_5;
  }
  objc_opt_class();
...
- (id)serialize
{
  ...
  [(BMDeviceCellularDataEnabled *)self writeTo:v3];
...
- (id)initByReadFrom:(id)a3
{
  id v4 = a3;
  v27.receiver = self;
  v27.super_class = (Class)BMDeviceCellularDataEnabled;
...
- (NSString)description
{
  ...
  id v4 = objc_msgSend(NSNumber, "numberWithBool:", -[BMDeviceCellularDataEnabled starting](self, "starting"));
  int v5 = (void *)[v3 initWithFormat:@"BMDeviceCellularDataEnabled with starting: %@", v4];
...
- (BMDeviceCellularDataEnabled)initWithStarting:(id)a3
{
  id v4 = a3;
  v7.receiver = self;
  v7.super_class = (Class)BMDeviceCellularDataEnabled;
...
+ (id)eventWithData:(id)a3 dataVersion:(unsigned int)a4
{
  ...
  else
  {
    ...
    v8 = [[BMDeviceCellularDataEnabled alloc] initByReadFrom:v7];
    id v4 = v8;
    if (v8) {
      v8[5] = 0;
    }
