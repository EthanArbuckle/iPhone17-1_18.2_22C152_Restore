@interface BMDeviceDefaultPairedNearby
+ (id)columns;
+ (id)eventWithData:(id)a3 dataVersion:(unsigned int)a4;
+ (id)protoFields;
+ (id)validKeyPaths;
+ (unsigned)latestDataVersion;
- (BMDeviceDefaultPairedNearby)initWithJSONDictionary:(id)a3 error:(id *)a4;
- (BMDeviceDefaultPairedNearby)initWithStarting:(id)a3;
- (BOOL)hasStarting;
- (BOOL)isEqual:(id)a3;
- (BOOL)starting;
...
@implementation BMDeviceDefaultPairedNearby
...
- (BOOL)isEqual:(id)a3
{
  ...
  {
    id v5 = v4;
    if ([(BMDeviceDefaultPairedNearby *)self hasStarting]
      || [v5 hasStarting])
    {
      if ([(BMDeviceDefaultPairedNearby *)self hasStarting]
        && [v5 hasStarting])
      {
        BOOL v6 = [(BMDeviceDefaultPairedNearby *)self starting];
...
- (id)jsonDictionary
{
  ...
  if ([(BMDeviceDefaultPairedNearby *)self hasStarting])
  {
    uint64_t v3 = objc_msgSend(NSNumber, "numberWithBool:", -[BMDeviceDefaultPairedNearby starting](self, "starting"));
    v9 = @"starting";
    if (v3)
    {
      id v4 = (void *)v3;
...
- (BMDeviceDefaultPairedNearby)initWithJSONDictionary:(id)a3 error:(id *)a4
{
  ...
  if (!v6 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) != 0))
  {
    a4 = 0;
LABEL_4:
    self = [(BMDeviceDefaultPairedNearby *)self initWithStarting:a4];
    int v7 = self;
    goto LABEL_5;
  }
  objc_opt_class();
...
- (id)serialize
{
  ...
  [(BMDeviceDefaultPairedNearby *)self writeTo:v3];
...
- (id)initByReadFrom:(id)a3
{
  id v4 = a3;
  v27.receiver = self;
  v27.super_class = (Class)BMDeviceDefaultPairedNearby;
...
- (NSString)description
{
  ...
  id v4 = objc_msgSend(NSNumber, "numberWithBool:", -[BMDeviceDefaultPairedNearby starting](self, "starting"));
  int v5 = (void *)[v3 initWithFormat:@"BMDeviceDefaultPairedNearby with starting: %@", v4];
...
- (BMDeviceDefaultPairedNearby)initWithStarting:(id)a3
{
  id v4 = a3;
  v7.receiver = self;
  v7.super_class = (Class)BMDeviceDefaultPairedNearby;
...
+ (id)eventWithData:(id)a3 dataVersion:(unsigned int)a4
{
  ...
  else
  {
    ...
    v8 = [[BMDeviceDefaultPairedNearby alloc] initByReadFrom:v7];
    id v4 = v8;
    if (v8) {
      v8[5] = 0;
    }
