@interface MUAttributionArgument
- (BOOL)hasVendorId;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (NSString)vendorId;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
- (id)dictionaryRepresentation;
- (unint64_t)hash;
- (void)copyTo:(id)a3;
- (void)mergeFrom:(id)a3;
- (void)setVendorId:(id)a3;
- (void)writeTo:(id)a3;
@end

@implementation MUAttributionArgument

- (BOOL)hasVendorId
{
  return self->_vendorId != 0;
}

- (id)description
{
  v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)MUAttributionArgument;
  v4 = [(MUAttributionArgument *)&v8 description];
  v5 = [(MUAttributionArgument *)self dictionaryRepresentation];
  v6 = [v3 stringWithFormat:@"%@ %@", v4, v5];

  return v6;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  v4 = v3;
  vendorId = self->_vendorId;
  if (vendorId) {
    [v3 setObject:vendorId forKey:@"vendorId"];
  }
  return v4;
}

- (BOOL)readFrom:(id)a3
{
  return MUAttributionArgumentReadFrom((uint64_t)self, (uint64_t)a3);
}

- (void)writeTo:(id)a3
{
  if (self->_vendorId) {
    PBDataWriterWriteStringField();
  }
}

- (void)copyTo:(id)a3
{
  vendorId = self->_vendorId;
  if (vendorId) {
    [a3 setVendorId:vendorId];
  }
}

- (id)copyWithZone:(_NSZone *)a3
{
  v5 = objc_msgSend((id)objc_msgSend((id)objc_opt_class(), "allocWithZone:", a3), "init");
  uint64_t v6 = [(NSString *)self->_vendorId copyWithZone:a3];
  v7 = (void *)v5[1];
  v5[1] = v6;

  return v5;
}

- (BOOL)isEqual:(id)a3
{
  v4 = a3;
  if ([v4 isMemberOfClass:objc_opt_class()])
  {
    vendorId = self->_vendorId;
    if ((unint64_t)vendorId | v4[1]) {
      char v6 = -[NSString isEqual:](vendorId, "isEqual:");
    }
    else {
      char v6 = 1;
    }
  }
  else
  {
    char v6 = 0;
  }

  return v6;
}

- (unint64_t)hash
{
  return [(NSString *)self->_vendorId hash];
}

- (void)mergeFrom:(id)a3
{
  if (*((void *)a3 + 1)) {
    -[MUAttributionArgument setVendorId:](self, "setVendorId:");
  }
}

- (NSString)vendorId
{
  return self->_vendorId;
}

- (void)setVendorId:(id)a3
{
}

- (void).cxx_destruct
{
}

@end