@interface VISOperatingPointsCollection
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (NSArray)points;
- (NSData)jsonData;
- (VISOperatingPointsCollection)initWithDictionary:(id)a3;
- (VISOperatingPointsCollection)initWithJSON:(id)a3;
- (id)dictionaryRepresentation;
- (id)pointsAtIndex:(unint64_t)a3;
- (unint64_t)hash;
- (unint64_t)pointsCount;
- (void)addPoints:(id)a3;
- (void)clearPoints;
- (void)setPoints:(id)a3;
- (void)writeTo:(id)a3;
@end

@implementation VISOperatingPointsCollection

- (void)setPoints:(id)a3
{
  self->_points = (NSArray *)[a3 copy];
  MEMORY[0x1F41817F8]();
}

- (void)clearPoints
{
}

- (void)addPoints:(id)a3
{
  id v4 = a3;
  points = self->_points;
  id v8 = v4;
  if (!points)
  {
    v6 = [MEMORY[0x1E4F1CA48] array];
    v7 = self->_points;
    self->_points = v6;

    id v4 = v8;
    points = self->_points;
  }
  [(NSArray *)points addObject:v4];
}

- (unint64_t)pointsCount
{
  return [(NSArray *)self->_points count];
}

- (id)pointsAtIndex:(unint64_t)a3
{
  return [(NSArray *)self->_points objectAtIndexedSubscript:a3];
}

- (BOOL)readFrom:(id)a3
{
  return VISOperatingPointsCollectionReadFrom(self, (uint64_t)a3);
}

- (void)writeTo:(id)a3
{
  uint64_t v15 = *MEMORY[0x1E4F143B8];
  id v4 = a3;
  v5 = [(VISOperatingPointsCollection *)self points];
  long long v10 = 0u;
  long long v11 = 0u;
  long long v12 = 0u;
  long long v13 = 0u;
  uint64_t v6 = [v5 countByEnumeratingWithState:&v10 objects:v14 count:16];
  if (v6)
  {
    uint64_t v7 = v6;
    uint64_t v8 = *(void *)v11;
    do
    {
      uint64_t v9 = 0;
      do
      {
        if (*(void *)v11 != v8) {
          objc_enumerationMutation(v5);
        }
        PBDataWriterWriteSubmessage();
        ++v9;
      }
      while (v7 != v9);
      uint64_t v7 = [v5 countByEnumeratingWithState:&v10 objects:v14 count:16];
    }
    while (v7);
  }
}

- (BOOL)isEqual:(id)a3
{
  id v4 = a3;
  if ([v4 isMemberOfClass:objc_opt_class()])
  {
    v5 = [(VISOperatingPointsCollection *)self points];
    uint64_t v6 = [v4 points];
    uint64_t v7 = v6;
    if ((v5 == 0) != (v6 != 0))
    {
      uint64_t v8 = [(VISOperatingPointsCollection *)self points];
      if (!v8)
      {

LABEL_10:
        BOOL v13 = 1;
        goto LABEL_8;
      }
      uint64_t v9 = (void *)v8;
      long long v10 = [(VISOperatingPointsCollection *)self points];
      long long v11 = [v4 points];
      char v12 = [v10 isEqual:v11];

      if (v12) {
        goto LABEL_10;
      }
    }
    else
    {
    }
  }
  BOOL v13 = 0;
LABEL_8:

  return v13;
}

- (unint64_t)hash
{
  return [(NSArray *)self->_points hash];
}

- (id)dictionaryRepresentation
{
  uint64_t v18 = *MEMORY[0x1E4F143B8];
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  if ([(NSArray *)self->_points count])
  {
    id v4 = [MEMORY[0x1E4F1CA48] array];
    long long v13 = 0u;
    long long v14 = 0u;
    long long v15 = 0u;
    long long v16 = 0u;
    v5 = self->_points;
    uint64_t v6 = [(NSArray *)v5 countByEnumeratingWithState:&v13 objects:v17 count:16];
    if (v6)
    {
      uint64_t v7 = v6;
      uint64_t v8 = *(void *)v14;
      do
      {
        for (uint64_t i = 0; i != v7; ++i)
        {
          if (*(void *)v14 != v8) {
            objc_enumerationMutation(v5);
          }
          long long v10 = objc_msgSend(*(id *)(*((void *)&v13 + 1) + 8 * i), "dictionaryRepresentation", (void)v13);
          if (v10)
          {
            [v4 addObject:v10];
          }
          else
          {
            long long v11 = [MEMORY[0x1E4F1CA98] null];
            [v4 addObject:v11];
          }
        }
        uint64_t v7 = [(NSArray *)v5 countByEnumeratingWithState:&v13 objects:v17 count:16];
      }
      while (v7);
    }

    [v3 setObject:v4 forKeyedSubscript:@"points"];
  }
  return v3;
}

- (NSData)jsonData
{
  v2 = [(VISOperatingPointsCollection *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (VISOperatingPointsCollection)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(VISOperatingPointsCollection *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (VISOperatingPointsCollection)initWithDictionary:(id)a3
{
  uint64_t v23 = *MEMORY[0x1E4F143B8];
  id v4 = a3;
  v21.receiver = self;
  v21.super_class = (Class)VISOperatingPointsCollection;
  v5 = [(VISOperatingPointsCollection *)&v21 init];
  if (v5)
  {
    uint64_t v6 = [v4 objectForKeyedSubscript:@"points"];
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      long long v19 = 0u;
      long long v20 = 0u;
      long long v17 = 0u;
      long long v18 = 0u;
      id v7 = v6;
      uint64_t v8 = [v7 countByEnumeratingWithState:&v17 objects:v22 count:16];
      if (v8)
      {
        uint64_t v9 = v8;
        uint64_t v10 = *(void *)v18;
        do
        {
          uint64_t v11 = 0;
          do
          {
            if (*(void *)v18 != v10) {
              objc_enumerationMutation(v7);
            }
            uint64_t v12 = *(void *)(*((void *)&v17 + 1) + 8 * v11);
            objc_opt_class();
            if (objc_opt_isKindOfClass())
            {
              long long v13 = [VISSelectedOperatingPoint alloc];
              long long v14 = -[VISSelectedOperatingPoint initWithDictionary:](v13, "initWithDictionary:", v12, (void)v17);
              [(VISOperatingPointsCollection *)v5 addPoints:v14];
            }
            ++v11;
          }
          while (v9 != v11);
          uint64_t v9 = [v7 countByEnumeratingWithState:&v17 objects:v22 count:16];
        }
        while (v9);
      }
    }
    long long v15 = v5;
  }
  return v5;
}

- (NSArray)points
{
  return self->_points;
}

- (void).cxx_destruct
{
}

@end