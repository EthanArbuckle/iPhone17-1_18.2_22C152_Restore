...
+ (BOOL)setDAManagedDefaults:(id)a3;
...
+ (void)_startListeningForNotifications
{
  ...
  block[3] = &unk_1E6574690;
  id v3 = &__block_literal_global_3;
...
uint64_t __52__DABehaviorOptions__startListeningForNotifications__block_invoke()
{
  ...
  {
    *(_WORD *)v2 = 0;
    _os_log_impl(&dword_1C8854000, v0, OS_LOG_TYPE_DEBUG, "Bumping behavior token number", v2, 2u);
  }
...
+ (id)allowlistedEASProtocols
{
  if (allowlistedEASProtocols___haveCheckedAllowlistedEASProtocols != 1
    || allowlistedEASProtocols___lastToken != sDABehaviorToken)
  {
    ...
    if (v3)
    {
      uint64_t v5 = [v3 objectForKeyedSubscript:@"AllowlistedEASProtocols"];
      uint64_t v6 = (void *)allowlistedEASProtocols___allowlistedEASProtocols;
      allowlistedEASProtocols___allowlistedEASProtocols = v5;
    }
  }
  v7 = (void *)allowlistedEASProtocols___allowlistedEASProtocols;
...
+ (BOOL)cookiesEnabled
{
  ...
  if (cookiesEnabled___flagChecked != 1 || cookiesEnabled___lastToken != sDABehaviorToken)
  {
    ...
    if ((MGGetBoolAnswer() & 1) == 0)
    {
      ...
      {
        *(_WORD *)buf = 0;
        _os_log_impl(&dword_1C8854000, v3, OS_LOG_TYPE_DEFAULT, "Turning off cookie support because we are running on older hardware", buf, 2u);
      }
  ...
  if (v9)
  {
    ...
    if (os_log_type_enabled(v6, OS_LOG_TYPE_DEFAULT))
    {
      ...
      *(_DWORD *)buf = 138412290;
      v11 = v7;
      _os_log_impl(&dword_1C8854000, v6, OS_LOG_TYPE_DEFAULT, "Due to a profile setting, we are forcing cookies to be %@abled", buf, 0xCu);
    }
  }
  else
  {
...
+ (BOOL)useContactsFramerwork
{
  if (useContactsFramerwork___haveCheckedUseContactsFramework != 1
...
+ (BOOL)orphanCheckEnabled
{
  if (orphanCheckEnabled___haveCheckedOrphanCheckEnabled != 1 || orphanCheckEnabled___lastToken != sDABehaviorToken)
  {
    ...
    if (v3)
    {
      ...
      if (v5)
      {
        uint64_t v6 = [v4 objectForKeyedSubscript:@"OrphanCheckEnabled"];
...
+ (id)_daManagedDefaultsPath
{
  if (_daManagedDefaultsPath_onceToken != -1) {
    dispatch_once(&_daManagedDefaultsPath_onceToken, &__block_literal_global_125);
  }
  v2 = (void *)_daManagedDefaultsPath_defaultsPath;
...
{
  v0 = (void *)_daManagedDefaultsPath_defaultsPath;
...
+ (id)DAManagedDefaults
{
  ...
  id v3 = [(id)objc_opt_class() _daManagedDefaultsPath];
  BOOL v4 = [v2 dictionaryWithContentsOfFile:v3];
...
+ (BOOL)setDAManagedDefaults:(id)a3
{
  return _CFPreferencesWriteManagedDomainForUser() != 0;
}

+ (id)DAManagedDefaultForKey:(id)a3
{
  id v4 = a3;
  BOOL v5 = [a1 DAManagedDefaults];
...
{
  id v3 = a3;
  id v4 = [(id)objc_opt_class() DAManagedDefaults];
  ...
  char v7 = [(id)objc_opt_class() setDAManagedDefaults:v3];
...
+ (void)removeDAManagedDefaults:(id)a3
{
  ...
  if (v5)
  {
    ...
    [(id)objc_opt_class() setDAManagedDefaults:v5];
  }
}

+ (int64_t)rem_changeTrackingBehaviors
{
  if (rem_changeTrackingBehaviors___haveCheckedDADREMChangeTrackingBehaviors != 1
    || rem_changeTrackingBehaviors___lastToken != sDABehaviorToken)
  {
    ...
    if (v3)
    {
      BOOL v5 = [v3 objectForKeyedSubscript:@"DADREMChangeTrackingBehaviors"];

      if (v5)
      {
        id v6 = [v4 objectForKeyedSubscript:@"DADREMChangeTrackingBehaviors"];
        rem_changeTrackingBehaviors___remChangeTrackingBehaviors = (int)[v6 intValue];
      }
    }
  }
