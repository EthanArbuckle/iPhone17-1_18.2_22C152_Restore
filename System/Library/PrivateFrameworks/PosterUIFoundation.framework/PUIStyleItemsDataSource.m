...
- (PUIStyleItemsDataSource)initWithConfiguration:(id)a3 includesSuggestedStyle:(BOOL)a4 delegate:(id)a5
{
  ...
  id v10 = a5;
  v14.receiver = self;
  v14.super_class = (Class)PUIStyleItemsDataSource;
...
- (void)buildCoordinators
{
  ...
  if (!self->_coordinators)
  {
    ...
    uint64_t v4 = [(PUIStyleConfiguration *)self->_configuration suggestedStyle];
    v5 = [(PUIStyleConfiguration *)self->_configuration stylePalette];
    v24 = (void *)v4;
    if (self->_includeSuggestedStyle
      && [(PUIStyleConfiguration *)self->_configuration showsSuggestedContentStyleItem]
      && v4)
    {
      ...
      if ([v5 isDefaultPalette] && objc_msgSend(v5, "context") == 1)
      {
        ...
        id v10 = [PUIStylePalette alloc];
        v11 = [v5 localizedName];
        uint64_t v12 = [(PUIStylePalette *)v10 initWithStyles:v9 localizedName:v11];
...
uint64_t __47__PUIStyleItemsDataSource_indexForCoordinator___block_invoke(uint64_t a1, void *a2)
{
...
{
  id WeakRetained = objc_loadWeakRetained((id *)&self->_delegate);
  return (PUIStyleItemsDataSourceDelegate *)WeakRetained;
}
