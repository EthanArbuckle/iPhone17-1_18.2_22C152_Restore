...
- (GTSharedMemoryTransport_capture)smTransport;
...
- (unsigned)_nextMessageSerial
{
  ...
  if (smTransport)
  {
    unsigned int result = [(GTSharedMemoryTransport_capture *)smTransport _nextMessageSerial];
  ...
  {
    v8.receiver = self;
    v8.super_class = (Class)GTBaseSocketTransport_capture;
...
- (BOOL)relayMessageOverSharedMemoryTransport:(id)a3 error:(id *)a4
{
  return [(GTTransport_capture *)self->_smTransport relayMessage:a3 error:a4];
}
...
- (id)createNewSharedMemoryTransport
{
  return [(GTBaseSocketTransport_capture *)self createNewSharedMemoryTransportWithURL:0];
}
...
- (void)_invalidate
{
  ...
  v4.receiver = self;
  v4.super_class = (Class)GTBaseSocketTransport_capture;
...
- (void)setPrioritizeOutgoingMessages:(BOOL)a3
{
  ...
  dispatch_sync(queue, block);
  v6.receiver = self;
  v6.super_class = (Class)GTBaseSocketTransport_capture;
  [(GTTransport_capture *)&v6 setPrioritizeOutgoingMessages:v3];
}
...
LABEL_15:
  ...
  if (smTransport)
  {
    id v12 = [(GTSharedMemoryTransport_capture *)smTransport connect];
...
{
  self->_scheduledReadOnWritableSocket = 0;
  while ([(GTBaseStreamTransport_capture *)self _readAndAccumulate] > 0)
    ;
}
...
{
  v3.receiver = self;
  v3.super_class = (Class)GTBaseSocketTransport_capture;
