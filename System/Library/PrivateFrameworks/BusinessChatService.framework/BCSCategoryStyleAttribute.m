@interface BCSCategoryStyleAttribute
- (BOOL)hasKey;
- (BOOL)hasValue;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
...
@implementation BCSCategoryStyleAttribute
...
- (id)description
{
  char v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)BCSCategoryStyleAttribute;
  v4 = [(BCSCategoryStyleAttribute *)&v8 description];
  v5 = [(BCSCategoryStyleAttribute *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if (has)
  {
    v5 = [NSNumber numberWithInt:self->_key];
...
- (BOOL)readFrom:(id)a3
{
  return BCSCategoryStyleAttributeReadFrom((uint64_t)self, (uint64_t)a3);
}
...
- (BOOL)isEqual:(id)a3
{
  ...
  {
    if ((*((unsigned char *)v4 + 16) & 1) == 0 || self->_key != *((_DWORD *)v4 + 2)) {
      goto LABEL_11;
    }
  }
...
- (unint64_t)hash
{
  if (*(unsigned char *)&self->_has)
  {
    uint64_t v2 = 2654435761 * self->_key;
    if ((*(unsigned char *)&self->_has & 2) != 0) {
      goto LABEL_3;
    }
LABEL_5:
