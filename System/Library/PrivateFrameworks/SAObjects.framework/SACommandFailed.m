@interface SACommandFailed
+ (SACommandFailed)commandFailedWithErrorCode:(int64_t)a3;
+ (SACommandFailed)commandFailedWithReason:(id)a3;
+ (id)commandFailed;
- (BOOL)requiresResponse;
- (NSString)reason;
- (NSString)turnId;
- (SACommandFailed)initWithErrorCode:(int64_t)a3;
- (SACommandFailed)initWithReason:(id)a3;
- (id)encodedClassName;
- (id)groupIdentifier;
- (int64_t)errorCode;
- (void)setErrorCode:(int64_t)a3;
- (void)setReason:(id)a3;
- (void)setTurnId:(id)a3;
@end

@implementation SACommandFailed

- (id)groupIdentifier
{
  return @"com.apple.ace.system";
}

- (id)encodedClassName
{
  return @"CommandFailed";
}

+ (id)commandFailed
{
  id v2 = objc_alloc_init((Class)a1);
  return v2;
}

+ (SACommandFailed)commandFailedWithErrorCode:(int64_t)a3
{
  v3 = [[SACommandFailed alloc] initWithErrorCode:a3];
  return v3;
}

+ (SACommandFailed)commandFailedWithReason:(id)a3
{
  id v3 = a3;
  v4 = [[SACommandFailed alloc] initWithReason:v3];

  return v4;
}

- (SACommandFailed)initWithErrorCode:(int64_t)a3
{
  v4 = [(AceObject *)self init];
  v5 = v4;
  if (v4) {
    [(SACommandFailed *)v4 setErrorCode:a3];
  }
  return v5;
}

- (SACommandFailed)initWithReason:(id)a3
{
  id v4 = a3;
  v5 = [(AceObject *)self init];
  v6 = v5;
  if (v5) {
    [(SACommandFailed *)v5 setReason:v4];
  }

  return v6;
}

- (int64_t)errorCode
{
  return AceObjectIntegerForProperty(self, @"errorCode");
}

- (void)setErrorCode:(int64_t)a3
{
}

- (NSString)reason
{
  return (NSString *)[(AceObject *)self propertyForKey:@"reason"];
}

- (void)setReason:(id)a3
{
}

- (NSString)turnId
{
  return (NSString *)[(AceObject *)self propertyForKey:@"turnId"];
}

- (void)setTurnId:(id)a3
{
}

- (BOOL)requiresResponse
{
  return 0;
}

@end