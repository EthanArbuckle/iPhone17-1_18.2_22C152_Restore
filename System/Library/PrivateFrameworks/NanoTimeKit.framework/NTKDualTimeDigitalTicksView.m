...
- (void)startAnimation
{
  if (!self->_clockTimerToken)
  {
    ...
    double v4 = [(CLKClockTimer *)timer startUpdatesWithUpdateFrequency:2 withHandler:&v6 identificationLog:&__block_literal_global_73];
...
__CFString *__45__NTKDualTimeDigitalTicksView_startAnimation__block_invoke_2()
{
  return @"NTKDualTimeView";
}
...
- (void)_configureTicks
{
  ...
  do
  {
    ...
    if (_disabledLayerActions_onceToken_1 != -1) {
      dispatch_once(&_disabledLayerActions_onceToken_1, &__block_literal_global_85);
    }
...
- (void)_refreshDigitalTicksWithNow:(id)a3 secondFraction:(double)a4
{
  ...
  {
    double v6 = [(NTKDualTimeDigitalTicksView *)self delegate];

    if (v6)
    {
      CGFloat v7 = [(NTKDualTimeDigitalTicksView *)self delegate];
      id v8 = [v7 activeColorForDualTimeDigitalTicksView:self];

      int v9 = [(NTKDualTimeDigitalTicksView *)self delegate];
...
- (NTKDualTimeDigitalTicksViewPaletteDelegate)delegate
{
  ...
  return (NTKDualTimeDigitalTicksViewPaletteDelegate *)WeakRetained;
}
