@interface DNDSUserAvailabilityPublishResult
- (AKAvailability)availability;
- (BOOL)isEqual:(id)a3;
- (DNDSUserAvailabilityPublishResult)initWithIdentifier:(id)a3 availability:(id)a4;
- (NSString)identifier;
- (unint64_t)hash;
@end

@implementation DNDSUserAvailabilityPublishResult

- (DNDSUserAvailabilityPublishResult)initWithIdentifier:(id)a3 availability:(id)a4
{
  id v7 = a3;
  id v8 = a4;
  v12.receiver = self;
  v12.super_class = (Class)DNDSUserAvailabilityPublishResult;
  v9 = [(DNDSUserAvailabilityPublishResult *)&v12 init];
  v10 = v9;
  if (v9)
  {
    objc_storeStrong((id *)&v9->_identifier, a3);
    objc_storeStrong((id *)&v10->_availability, a4);
  }

  return v10;
}

- (unint64_t)hash
{
  v3 = [(DNDSUserAvailabilityPublishResult *)self identifier];
  uint64_t v4 = [v3 hash];
  v5 = [(DNDSUserAvailabilityPublishResult *)self availability];
  unint64_t v6 = [v5 hash] ^ v4;

  return v6;
}

- (BOOL)isEqual:(id)a3
{
  uint64_t v4 = (DNDSUserAvailabilityPublishResult *)a3;
  if (self == v4)
  {
    char v13 = 1;
  }
  else
  {
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      v5 = v4;
      unint64_t v6 = [(DNDSUserAvailabilityPublishResult *)self identifier];
      id v7 = [(DNDSUserAvailabilityPublishResult *)v5 identifier];
      if (v6 != v7)
      {
        uint64_t v8 = [(DNDSUserAvailabilityPublishResult *)self identifier];
        if (!v8)
        {
          char v13 = 0;
LABEL_24:

          goto LABEL_25;
        }
        v9 = (void *)v8;
        v10 = [(DNDSUserAvailabilityPublishResult *)v5 identifier];
        if (!v10)
        {
          char v13 = 0;
LABEL_23:

          goto LABEL_24;
        }
        v11 = [(DNDSUserAvailabilityPublishResult *)self identifier];
        objc_super v12 = [(DNDSUserAvailabilityPublishResult *)v5 identifier];
        if (![v11 isEqual:v12])
        {
          char v13 = 0;
LABEL_22:

          goto LABEL_23;
        }
        v23 = v12;
        v24 = v11;
        v25 = v10;
        v26 = v9;
      }
      v14 = [(DNDSUserAvailabilityPublishResult *)self availability];
      uint64_t v15 = [(DNDSUserAvailabilityPublishResult *)v5 availability];
      if (v14 == (void *)v15)
      {

        char v13 = 1;
      }
      else
      {
        v16 = (void *)v15;
        uint64_t v17 = [(DNDSUserAvailabilityPublishResult *)self availability];
        if (v17)
        {
          v18 = (void *)v17;
          v19 = [(DNDSUserAvailabilityPublishResult *)v5 availability];
          if (v19)
          {
            v22 = [(DNDSUserAvailabilityPublishResult *)self availability];
            v20 = [(DNDSUserAvailabilityPublishResult *)v5 availability];
            char v13 = [v22 isEqual:v20];
          }
          else
          {
            char v13 = 0;
          }
        }
        else
        {

          char v13 = 0;
        }
      }
      v10 = v25;
      v9 = v26;
      objc_super v12 = v23;
      v11 = v24;
      if (v6 == v7) {
        goto LABEL_24;
      }
      goto LABEL_22;
    }
    char v13 = 0;
  }
LABEL_25:

  return v13;
}

- (NSString)identifier
{
  return (NSString *)objc_getProperty(self, a2, 8, 1);
}

- (AKAvailability)availability
{
  return (AKAvailability *)objc_getProperty(self, a2, 16, 1);
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_availability, 0);
  objc_storeStrong((id *)&self->_identifier, 0);
}

@end