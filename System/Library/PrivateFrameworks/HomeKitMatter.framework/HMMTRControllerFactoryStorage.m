@interface HMMTRControllerFactoryStorage
- (BOOL)removeStorageDataForKey:(id)a3;
- (BOOL)setStorageData:(id)a3 forKey:(id)a4;
- (HMMTRControllerFactoryStorage)init;
- (NSMutableDictionary)mtrControllerFactoryKeyValueStore;
- (OS_dispatch_queue)workQueue;
- (id)dictionaryCopy;
- (id)storageDataForKey:(id)a3;
- (void)clear;
@end

@implementation HMMTRControllerFactoryStorage

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_workQueue, 0);
  objc_storeStrong((id *)&self->_mtrControllerFactoryKeyValueStore, 0);
}

- (OS_dispatch_queue)workQueue
{
  return self->_workQueue;
}

- (NSMutableDictionary)mtrControllerFactoryKeyValueStore
{
  return self->_mtrControllerFactoryKeyValueStore;
}

- (void)clear
{
  v3 = [(HMMTRControllerFactoryStorage *)self workQueue];
  block[0] = MEMORY[0x263EF8330];
  block[1] = 3221225472;
  block[2] = __38__HMMTRControllerFactoryStorage_clear__block_invoke;
  block[3] = &unk_265378638;
  block[4] = self;
  dispatch_sync(v3, block);
}

void __38__HMMTRControllerFactoryStorage_clear__block_invoke(uint64_t a1)
{
  id v1 = [*(id *)(a1 + 32) mtrControllerFactoryKeyValueStore];
  [v1 removeAllObjects];
}

- (HMMTRControllerFactoryStorage)init
{
  v8.receiver = self;
  v8.super_class = (Class)HMMTRControllerFactoryStorage;
  v2 = [(HMMTRControllerFactoryStorage *)&v8 init];
  if (v2)
  {
    dispatch_queue_t v3 = dispatch_queue_create("HMMTRControllerFactoryStorage", 0);
    workQueue = v2->_workQueue;
    v2->_workQueue = (OS_dispatch_queue *)v3;

    uint64_t v5 = [MEMORY[0x263EFF9A0] dictionary];
    mtrControllerFactoryKeyValueStore = v2->_mtrControllerFactoryKeyValueStore;
    v2->_mtrControllerFactoryKeyValueStore = (NSMutableDictionary *)v5;
  }
  return v2;
}

- (id)dictionaryCopy
{
  uint64_t v7 = 0;
  objc_super v8 = &v7;
  uint64_t v9 = 0x3032000000;
  v10 = __Block_byref_object_copy__2964;
  v11 = __Block_byref_object_dispose__2965;
  id v12 = 0;
  dispatch_queue_t v3 = [(HMMTRControllerFactoryStorage *)self workQueue];
  v6[0] = MEMORY[0x263EF8330];
  v6[1] = 3221225472;
  v6[2] = __47__HMMTRControllerFactoryStorage_dictionaryCopy__block_invoke;
  v6[3] = &unk_265377F28;
  v6[4] = self;
  v6[5] = &v7;
  dispatch_sync(v3, v6);

  id v4 = (id)v8[5];
  _Block_object_dispose(&v7, 8);

  return v4;
}

void __47__HMMTRControllerFactoryStorage_dictionaryCopy__block_invoke(uint64_t a1)
{
  id v5 = [*(id *)(a1 + 32) mtrControllerFactoryKeyValueStore];
  uint64_t v2 = [v5 copy];
  uint64_t v3 = *(void *)(*(void *)(a1 + 40) + 8);
  id v4 = *(void **)(v3 + 40);
  *(void *)(v3 + 40) = v2;
}

- (BOOL)removeStorageDataForKey:(id)a3
{
  id v4 = a3;
  id v5 = [(HMMTRControllerFactoryStorage *)self workQueue];
  v8[0] = MEMORY[0x263EF8330];
  v8[1] = 3221225472;
  v8[2] = __57__HMMTRControllerFactoryStorage_removeStorageDataForKey___block_invoke;
  v8[3] = &unk_265376E98;
  v8[4] = self;
  id v9 = v4;
  id v6 = v4;
  dispatch_sync(v5, v8);

  return 1;
}

void __57__HMMTRControllerFactoryStorage_removeStorageDataForKey___block_invoke(uint64_t a1)
{
  id v2 = [*(id *)(a1 + 32) mtrControllerFactoryKeyValueStore];
  [v2 setObject:0 forKeyedSubscript:*(void *)(a1 + 40)];
}

- (BOOL)setStorageData:(id)a3 forKey:(id)a4
{
  id v6 = a3;
  id v7 = a4;
  objc_super v8 = [(HMMTRControllerFactoryStorage *)self workQueue];
  block[0] = MEMORY[0x263EF8330];
  block[1] = 3221225472;
  block[2] = __55__HMMTRControllerFactoryStorage_setStorageData_forKey___block_invoke;
  block[3] = &unk_265376F38;
  block[4] = self;
  id v13 = v7;
  id v14 = v6;
  id v9 = v6;
  id v10 = v7;
  dispatch_sync(v8, block);

  return 1;
}

void __55__HMMTRControllerFactoryStorage_setStorageData_forKey___block_invoke(uint64_t a1)
{
  uint64_t v2 = *(void *)(a1 + 48);
  id v3 = [*(id *)(a1 + 32) mtrControllerFactoryKeyValueStore];
  [v3 setObject:v2 forKeyedSubscript:*(void *)(a1 + 40)];
}

- (id)storageDataForKey:(id)a3
{
  id v4 = a3;
  uint64_t v12 = 0;
  id v13 = &v12;
  uint64_t v14 = 0x3032000000;
  v15 = __Block_byref_object_copy__2964;
  v16 = __Block_byref_object_dispose__2965;
  id v17 = 0;
  id v5 = [(HMMTRControllerFactoryStorage *)self workQueue];
  block[0] = MEMORY[0x263EF8330];
  block[1] = 3221225472;
  block[2] = __51__HMMTRControllerFactoryStorage_storageDataForKey___block_invoke;
  block[3] = &unk_265377F50;
  id v10 = v4;
  v11 = &v12;
  block[4] = self;
  id v6 = v4;
  dispatch_sync(v5, block);

  id v7 = (id)v13[5];
  _Block_object_dispose(&v12, 8);

  return v7;
}

void __51__HMMTRControllerFactoryStorage_storageDataForKey___block_invoke(uint64_t a1)
{
  id v5 = [*(id *)(a1 + 32) mtrControllerFactoryKeyValueStore];
  uint64_t v2 = [v5 objectForKeyedSubscript:*(void *)(a1 + 40)];
  uint64_t v3 = *(void *)(*(void *)(a1 + 48) + 8);
  id v4 = *(void **)(v3 + 40);
  *(void *)(v3 + 40) = v2;
}

@end