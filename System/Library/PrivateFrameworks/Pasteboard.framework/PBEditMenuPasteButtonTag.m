...
- (PBEditMenuPasteButtonTag)initWithSecureName:(unsigned int)a3 size:(CGSize)a4 contentOrigin:(CGPoint)a5 displayMode:(unint64_t)a6
{
  ...
  if (result)
  {
    ...
    result->_contentOrigin.CGFloat x = x;
    result->_contentOrigin.CGFloat y = y;
...
- (PBEditMenuPasteButtonTag)initWithCoder:(id)a3
{
  ...
  if (v5)
  {
    ...
    v5->_contentOrigin.CGFloat x = v8;
    v5->_contentOrigin.CGFloat y = v9;
...
- (void)encodeWithCoder:(id)a3
{
  ...
  objc_msgSend(v4, "encodePoint:forKey:", @"titleOrigin", self->_contentOrigin.x, self->_contentOrigin.y);
  [v4 encodeInteger:self->_displayMode forKey:@"displayMode"];
}

- (unint64_t)hash
{
  uint64_t v3 = +[PBEditMenuPasteButtonTag hash];
  int8x16_t v4 = veorq_s8((int8x16_t)vcvtq_u64_f64((float64x2_t)self->_size), (int8x16_t)vcvtq_u64_f64((float64x2_t)self->_contentOrigin));
  return *(void *)&veor_s8(*(int8x8_t *)v4.i8, (int8x8_t)*(_OWORD *)&vextq_s8(v4, v4, 8uLL)) ^ self->_displayMode ^ v3 ^ self->_secureName;
}
...
- (BOOL)isEqual:(id)a3
{
  ...
  BOOL v7 = (objc_opt_isKindOfClass() & 1) != 0
    ...
    && (self->_size.width == v4[3] ? (BOOL v5 = self->_size.height == v4[4]) : (BOOL v5 = 0),
        v5 && (self->_contentOrigin.x == v4[5] ? (BOOL v6 = self->_contentOrigin.y == v4[6]) : (BOOL v6 = 0), v6))
    && self->_displayMode == *((void *)v4 + 2);
...
- (id)_acceptCalloutBarPasteButtonTagVisit:(id)a3 systemInputAssistantPasteButtonTagVisit:(id)a4 undoInteractionHUDIconPasteButtonTagVisit:(id)a5 undoInteractionHUDTextPasteButtonTagVisit:(id)a6 contextMenuPasteButtonTagVisit:(id)a7 editMenuPasteButtonTagVisit:(id)a8
{
  if (a8)
  {
    id v9 = (*((void (**)(id, PBEditMenuPasteButtonTag *, id, id, id, id, id))a8 + 2))(a8, self, a3, a4, a5, a6, a7);
  }
  else
  {
    id v9 = 0;
