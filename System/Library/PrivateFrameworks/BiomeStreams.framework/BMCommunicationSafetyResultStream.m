@interface BMCommunicationSafetyResultStream
- (BMCommunicationSafetyResultStream)init;
- (NSString)identifier;
- (id)publisher;
...
@implementation BMCommunicationSafetyResultStream

- (NSString)identifier
{
  return (NSString *)[MEMORY[0x1E4F4FBA8] libraryPathForStreamIdentifier:@"CommunicationSafetyResult"];
}

- (BMCommunicationSafetyResultStream)init
{
  v8.receiver = self;
  v8.super_class = (Class)BMCommunicationSafetyResultStream;
  v2 = [(BMCommunicationSafetyResultStream *)&v8 init];
  if (v2)
  {
    v3 = [BMRestrictedStream alloc];
    v4 = [(BMCommunicationSafetyResultStream *)v2 identifier];
...
- (id)publisher
{
  ...
  v4 = [(BMCommunicationSafetyResultStream *)self identifier];
