...
- (NSString)identifier
{
  return (NSString *)[MEMORY[0x1E4F4FBA8] libraryPathForStreamIdentifier:@"CommunicationSafetyResultWithoutImage"];
}

- (BMCommunicationSafetyResultWithoutImageStream)init
{
  v8.receiver = self;
  v8.super_class = (Class)BMCommunicationSafetyResultWithoutImageStream;
  v2 = [(BMCommunicationSafetyResultWithoutImageStream *)&v8 init];
  if (v2)
  {
    v3 = [BMRestrictedStream alloc];
    v4 = [(BMCommunicationSafetyResultWithoutImageStream *)v2 identifier];
    uint64_t v5 = [(BMRestrictedStream *)v3 initWithIdentifier:v4 eventDataClass:NSClassFromString(&cfstr_Bmcommunicatio.isa)];
    stream = v2->_stream;
...
- (id)publisher
{
  ...
  v4 = [(BMCommunicationSafetyResultWithoutImageStream *)self identifier];
