...
- (NSString)identifier
{
  return (NSString *)[MEMORY[0x1E4F4FB88] streamIdentifierForStream:28];
}

- (BMSoundDetectionStream)init
{
  v10.receiver = self;
  v10.super_class = (Class)BMSoundDetectionStream;
  v2 = [(BMSoundDetectionStream *)&v10 init];
  if (v2)
  {
    v3 = BMRootLibraryBridge();
    v4 = [v3 Accessibility];
    v5 = [v4 SoundDetection];
    ...
    if (!v2->_storeStream)
    {
      ...
      if (os_log_type_enabled(v8, OS_LOG_TYPE_FAULT)) {
        -[BMSoundDetectionStream init](v8);
      }
    }
  }
  return v2;
...
- (id)publisher
{
  ...
  v4 = [(BMSoundDetectionStream *)self identifier];
...
{
  *(_WORD *)v1 = 0;
  _os_log_fault_impl(&dword_18E67D000, log, OS_LOG_TYPE_FAULT, "Unable to initialize BMSoundDetectionStream, please consult error logs for cause", v1, 2u);
}
