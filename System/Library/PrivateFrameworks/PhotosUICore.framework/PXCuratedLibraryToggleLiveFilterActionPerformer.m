...
- (PXCuratedLibraryToggleLiveFilterActionPerformer)initWithViewModel:(id)a3;
  ...
  if (a3 == 1)
  {
    ...
    if (v6)
    {
      ...
      if ((objc_opt_isKindOfClass() & 1) == 0)
      {
        ...
        [v10 handleFailureInMethod:a2, self, @"PXCuratedLibraryToggleLiveFilterActionPerformer.m", 42, @"%@ should be nil or an instance inheriting from %@, but it is %@", @"self.viewModel.allPhotosContentFilterState", v12, v13 object file lineNumber description];
      }
    }
    if ([v6 live]) {
...
- (int64_t)menuElementState
{
  ...
  int64_t v3 = [v2 isContentFilterActive:11];
...
- (id)activityType
{
  v2 = @"PXActivityTypeToggleLiveFilter";
  return @"PXActivityTypeToggleLiveFilter";
}

- (PXCuratedLibraryToggleLiveFilterActionPerformer)initWithViewModel:(id)a3
{
  v4.receiver = self;
  v4.super_class = (Class)PXCuratedLibraryToggleLiveFilterActionPerformer;
  return [(PXCuratedLibraryActionPerformer *)&v4 initWithActionType:@"PXCuratedLibraryActionToggleLiveFilter" viewModel:a3];
}
