...
+ (BOOL)canPerformActionType:(id)a3 withViewModel:(id)a4
{
  ...
  if (objc_msgSendSuper2(&v9, sel_canPerformActionType_withViewModel_, a3, v6)) {
    BOOL v7 = !+[PXPhotosGridToggleFilterActionPerformer isContentFilterHidden:11 viewModel:v6];
  }
  else {
    LOBYTE(v7) = 0;
  }
  ...
  if (a3 == 1)
  {
    ...
    if (v6)
    {
      ...
      if ((objc_opt_isKindOfClass() & 1) == 0)
      {
        ...
        [v10 handleFailureInMethod:a2, self, @"PXPhotosGridToggleLiveFilterActionPerformer.m", 38, @"%@ should be nil or an instance inheriting from %@, but it is %@", @"self.viewModel.contentFilterState", v12, v13 object file lineNumber description];
      }
    }
    if ([v6 live]) {
      BOOL v7 = @"DISABLE_LIVE_FILTER_SHORTCUT";
    }
    else {
      BOOL v7 = @"ENABLE_LIVE_FILTER_SHORTCUT";
    }
