...
- (void)performUserInteractionTask
{
  v3 = [(PXCuratedLibraryTapToRadarActionPerformer *)self layoutForDiagnosticDescription];
  if (v3)
  {
...
- (BOOL)canPerformWithActivityItems:(id)a3 forActivity:(id)a4
{
  ...
  id v7 = [(PXCuratedLibraryTapToRadarActionPerformer *)self activityType];
  if (v6 == v7)
  {
  ...
  if ((v9 & 1) == 0)
  {
    ...
    [v6 handleFailureInMethod:a2, self, @"PXCuratedLibraryAssetCollectionActionPerformer.m", 491, @"Invalid parameter not satisfying: %@", @"PXStringEqualToString(activity.activityType, self.activityType)" object file lineNumber description];
LABEL_5:
  }
  return 1;
}
...
- (id)activityType
{
  v2 = @"PXActivityTypeFileRadar";
  return @"PXActivityTypeFileRadar";
}
...
- (PXCuratedLibraryTapToRadarActionPerformer)initWithViewModel:(id)a3 assetCollectionReference:(id)a4 diagnosticLayout:(id)a5
{
  ...
  char v9 = [(PXCuratedLibraryAssetCollectionActionPerformer *)&v12 initWithActionType:@"PXCuratedLibraryActionTapToRadar" viewModel:a3 assetCollectionReference:a4];
  v10 = v9;
  if (v9) {
