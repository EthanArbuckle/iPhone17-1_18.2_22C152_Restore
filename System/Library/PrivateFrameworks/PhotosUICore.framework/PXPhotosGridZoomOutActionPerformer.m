...
- (void)performUserInteractionTask
{
  ...
  if (v5 < 0 || [v3 numberOfZoomSteps] <= v5)
  {
    ...
    {
      *(_WORD *)v8 = 0;
      _os_log_impl(&dword_1A9AE7000, v7, OS_LOG_TYPE_DEFAULT, "Action tried to zoom out, but that's no longer possible.", v8, 2u);
    }
...
- (id)localizedTitleForUseCase:(unint64_t)a3
{
  return PXLocalizedStringFromTable(@"PXLibraryAllPhotosZoomOut", @"PhotosUICore");
}
...
- (id)menuElement
{
  ...
  if (v4)
  {
    ...
    if ((objc_opt_isKindOfClass() & 1) == 0)
    {
      ...
      [v6 handleFailureInMethod:a2, self, @"PXPhotosGridZoomOutActionPerformer.m", 35, @"%@ should be nil or an instance inheriting from %@, but it is %@", @"[super menuElement]", v8, v9 object file lineNumber description];
    }
  }
...
- (id)activityType
{
  v2 = @"PXActivityTypeZoomOut";
  return @"PXActivityTypeZoomOut";
}
...
+ (BOOL)canPerformActionType:(id)a3 withViewModel:(id)a4
{
  ...
  {
    [v4 zoomStep];
    BOOL v8 = v7 + 1.0 < (double)[v4 numberOfZoomSteps];
  }
  else
  {
    BOOL v8 = 0;
