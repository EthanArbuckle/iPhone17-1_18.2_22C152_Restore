...
- (FTWordTimingInfo)initWithFlatbuffData:(id)a3;
- (FTWordTimingInfo)initWithFlatbuffData:(id)a3 root:(const WordTimingInfo *)a4;
- (FTWordTimingInfo)initWithFlatbuffData:(id)a3 root:(const WordTimingInfo *)a4 verify:(BOOL)a5;
- (NSString)word;
...
- (FTWordTimingInfo)initWithFlatbuffData:(id)a3
{
  return [(FTWordTimingInfo *)self initWithFlatbuffData:a3 root:0 verify:0];
}

- (id)initAndVerifyWithFlatbuffData:(id)a3
{
  return [(FTWordTimingInfo *)self initWithFlatbuffData:a3 root:0 verify:1];
}

- (FTWordTimingInfo)initWithFlatbuffData:(id)a3 root:(const WordTimingInfo *)a4
{
  return [(FTWordTimingInfo *)self initWithFlatbuffData:a3 root:a4 verify:0];
}
...
- (FTWordTimingInfo)initWithFlatbuffData:(id)a3 root:(const WordTimingInfo *)a4 verify:(BOOL)a5
{
  ...
  id v9 = a3;
  v26.receiver = self;
  v26.super_class = (Class)FTWordTimingInfo;
  v10 = [(FTWordTimingInfo *)&v26 init];
  if (!v10) {
    goto LABEL_13;
  }
  if (!v9 || ![v9 length]) {
  ...
  if (!siri::speech::schema_fb::WordTimingInfo::Verify(v19, (apple::aiml::flatbuffers2::Verifier *)v22)) {
LABEL_14:
  }
    v20 = 0;
  else {
...
- (Offset<siri::speech::schema_fb::WordTimingInfo>)addObjectToBuffer:(void *)a3
{
  uint64_t v5 = [(FTWordTimingInfo *)self word];
  v6 = v5;
  if (!v5) {
  ...
  int v9 = [(FTWordTimingInfo *)self sample_idx];
  int v10 = [(FTWordTimingInfo *)self offset];
  int v11 = [(FTWordTimingInfo *)self length];
  ...
  *((unsigned char *)a3 + 70) = 1;
  ...
  apple::aiml::flatbuffers2::FlatBufferBuilder::AddElement<float>(a3, 12, v13, 0.0);
...
- (id)flatbuffData
{
  ...
  __int16 v10 = 256;
  uint64_t v11 = 0;
  apple::aiml::flatbuffers2::FlatBufferBuilder::Finish((apple::aiml::flatbuffers2::FlatBufferBuilder *)&v3, [(FTWordTimingInfo *)self addObjectToBuffer:&v3], 0, 0);
  operator new();
}
