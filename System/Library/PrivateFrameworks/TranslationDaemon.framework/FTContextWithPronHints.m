...
- (FTContextWithPronHints)initWithFlatbuffData:(id)a3;
- (FTContextWithPronHints)initWithFlatbuffData:(id)a3 root:(const ContextWithPronHints *)a4;
...
- (FTContextWithPronHints)initWithFlatbuffData:(id)a3
{
  return [(FTContextWithPronHints *)self initWithFlatbuffData:a3 root:0 verify:0];
}

- (id)initAndVerifyWithFlatbuffData:(id)a3
{
  return [(FTContextWithPronHints *)self initWithFlatbuffData:a3 root:0 verify:1];
}

- (FTContextWithPronHints)initWithFlatbuffData:(id)a3 root:(const ContextWithPronHints *)a4
{
  return [(FTContextWithPronHints *)self initWithFlatbuffData:a3 root:a4 verify:0];
}
...
- (FTContextWithPronHints)initWithFlatbuffData:(id)a3 root:(const ContextWithPronHints *)a4 verify:(BOOL)a5
{
  ...
  id v9 = a3;
  v26.receiver = self;
  v26.super_class = (Class)FTContextWithPronHints;
  v10 = [(FTContextWithPronHints *)&v26 init];
  if (!v10) {
    goto LABEL_13;
  }
  if (!v9 || ![v9 length]) {
  ...
  if (!a4)
  {
    ...
    a4 = (const ContextWithPronHints *)v12 + *v12;
  }
  v10->_root = a4;
  if (!v5) {
    goto LABEL_13;
...
- (NSArray)pron_hints
{
  id v3 = [(NSMutableDictionary *)self->_storage objectForKeyedSubscript:@"pron_hints"];
  if (!v3)
  {
...
uint64_t __36__FTContextWithPronHints_pron_hints__block_invoke(uint64_t a1, uint64_t a2)
{
...
- (id)pron_hints_objectAtIndex:(unint64_t)a3
{
  uint64_t v5 = [(NSMutableDictionary *)self->_storage objectForKeyedSubscript:@"pron_hints"];
  v6 = v5;
  if (v5)
  {
...
- (void)pron_hints_enumerateObjectsUsingBlock:(id)a3
{
  ...
  unint64_t v5 = [(NSMutableDictionary *)self->_storage objectForKeyedSubscript:@"pron_hints"];
  v6 = v5;
  if (v5)
  {
...
- (Offset<siri::speech::schema_fb::ContextWithPronHints>)addObjectToBuffer:(void *)a3
{
  ...
  unint64_t v5 = [(FTContextWithPronHints *)self contextual_text];
  v6 = v5;
  if (!v5) {
  ...
  uint64_t v9 = [(FTContextWithPronHints *)self pron_hints];
...
- (id)flatbuffData
{
  ...
  __int16 v10 = 256;
  uint64_t v11 = 0;
  apple::aiml::flatbuffers2::FlatBufferBuilder::Finish((apple::aiml::flatbuffers2::FlatBufferBuilder *)&v3, [(FTContextWithPronHints *)self addObjectToBuffer:&v3], 0, 0);
  operator new();
}
