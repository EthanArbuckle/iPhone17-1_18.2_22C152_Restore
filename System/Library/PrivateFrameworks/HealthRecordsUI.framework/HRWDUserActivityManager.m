...
- (id)_castleRockSpotlightBreadcrumbs:(id)a3;
- (id)_handoffActivityBreadcrumbsForOnboarding:(id)a3;
- (id)_handoffActivityBreadcrumbsForSampleType:(id)a3;
...
{
  v12.receiver = self;
  v12.super_class = (Class)HRWDUserActivityManager;
...
- (void)setRootViewController:(id)a3
{
  ...
  {
    currentNode = [MEMORY[0x1E4F1CA00] exceptionWithName:@"_RootControllerNotResponderName" reason:@"The root view controller must implement the HRWDUserActivityResponder protocol" userInfo:0];
    [currentNode raise];
  }
}
...
- (void)changeActivityForResponder:(id)a3 activityDictionary:(id)a4 title:(id)a5 keywords:(id)a6
{
  ...
  {
    v13 = [(_HRWDActivityNode *)self->_topNode changeActivityForResponder:v16 activityDictionary:v10];
    currentNode = self->_currentNode;
    ...
    if (v11)
    {
      if (objc_opt_respondsToSelector())
      {
        ...
        [(HRWDUserActivityManager *)self _userActivityWithTitle:v11 keywords:v12 activityType:v15];
      }
      else
      {
        [(HRWDUserActivityManager *)self _userActivityWithTitle:v11 keywords:v12 activityType:*MEMORY[0x1E4F29E18]];
      }
    }
  }
}
...
- (void)transitionActivityForResponder:(id)a3 newResponder:(id)a4 transitionDictionary:(id)a5
{
  if (self->_recordActivities)
  {
    v6 = [(_HRWDActivityNode *)self->_topNode transitionActivityForResponder:a3 newResponder:a4 transitionDictionary:a5];
    currentNode = self->_currentNode;
...
{
  id v14 = a3;
  BOOL v4 = [(HRWDUserActivityManager *)self _restoreActivityFromPolicies:v14];
  v5 = v14;
  if (!v4)
  {
    v6 = [v14 userInfo];
    ...
    if (v7)
    {
      uint64_t v8 = [(HRWDUserActivityManager *)self _castleRockSpotlightBreadcrumbs:v6];
    }
    else
    {
      id v9 = [v6 objectForKey:*MEMORY[0x1E4F676C0]];

      if (v9)
      {
        uint64_t v8 = [(HRWDUserActivityManager *)self _handoffActivityBreadcrumbsForOnboarding:v6];
      }
      else
      {
        uint64_t v11 = *MEMORY[0x1E4F2A9C0];
        id v12 = [v6 objectForKey:*MEMORY[0x1E4F2A9C0]];

        if (v12)
        {
          v13 = [v6 objectForKey:v11];
          id v10 = [(HRWDUserActivityManager *)self _handoffActivityBreadcrumbsForSampleType:v13];
        ...
        uint64_t v8 = [v6 objectForKey:@"_ActivityBreadcrumbKey"];
      }
    }
    id v10 = (void *)v8;
    if (!v8)
...
- (id)_userActivityWithType:(id)a3
{
  ...
  BOOL v4 = (objc_class *)MEMORY[0x1E4F22488];
  id v5 = a3;
  v6 = (void *)[[v4 alloc] initWithActivityType:v5];
  ...
  id v10 = [MEMORY[0x1E4F1CA48] arrayWithCapacity:10];
...
- (void)_userActivityWithTitle:(id)a3 keywords:(id)a4 activityType:(id)a5
{
  ...
  if (!v10)
  {
    uint64_t v11 = [(HRWDUserActivityManager *)self _userActivityWithType:v9];
    if (v11)
    {
      id v12 = v11;
...
- (void)_restoreFromActivityChain:(id)a3
{
  ...
  id v5 = [(_HRWDActivityNode *)self->_topNode responder];
  long long v17 = 0u;
  long long v18 = 0u;
  long long v19 = 0u;
  long long v20 = 0u;
      ...
      do
      {
        ...
        else
        {
          ...
          if (!v14)
          {
            ...
            id v16 = *MEMORY[0x1E4F29FB0];
            if (os_log_type_enabled((os_log_t)*MEMORY[0x1E4F29FB0], OS_LOG_TYPE_ERROR)) {
              -[HRWDUserActivityManager _restoreFromActivityChain:]((uint64_t)v15, v16);
            }
...
- (void)_setCurrentNodeToResponder:(id)a3
{
  ...
  id v10 = v4;
  if (!currentNode
    || ([(_HRWDActivityNode *)currentNode responder],
        id v6 = (id)objc_claimAutoreleasedReturnValue(),
        v6,
        uint64_t v7 = v10,
        v6 != v10))
  {
    uint64_t v8 = [(_HRWDActivityNode *)self->_topNode changeActivityForResponder:v10 activityDictionary:0];
    uint64_t v9 = self->_currentNode;
...
- (void)decodeRestorableStateWithCoder:(id)a3
{
  ...
  else
  {
    uint64_t v7 = v5;
    _HKInitializeLogging();
    uint64_t v8 = *MEMORY[0x1E4F29FB0];
    if (os_log_type_enabled((os_log_t)*MEMORY[0x1E4F29FB0], OS_LOG_TYPE_DEFAULT))
    {
      ...
      __int16 v11 = 2048;
      uint64_t v12 = 801;
      _os_log_impl(&dword_1C21A1000, v8, OS_LOG_TYPE_DEFAULT, "User Activity Manager version mismatch: %ld != %ld", (uint8_t *)&v9, 0x16u);
    }
  }
}
...
{
  id v5 = a3;
  id v4 = [(HRWDUserActivityManager *)self policies];
...
- (BOOL)_restoreActivityFromPolicies:(id)a3
{
  ...
  long long v17 = 0u;
  long long v18 = 0u;
  id v5 = [(HRWDUserActivityManager *)self policies];
...
- (void)_restoreFromActivityChain:(uint64_t)a1 .cold.1(uint64_t a1, NSObject *a2)
{
  ...
  int v2 = 138543362;
  uint64_t v3 = a1;
  _os_log_error_impl(&dword_1C21A1000, a2, OS_LOG_TYPE_ERROR, "Restoration transition problem: %{public}@", (uint8_t *)&v2, 0xCu);
}
