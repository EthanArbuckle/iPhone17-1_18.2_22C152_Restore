...
- (id)description
{
  v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)NNMKProtoAccountAuthenticationStatusRequest;
...
- (id)dictionaryRepresentation
{
  ...
  {
    v4 = [NSNumber numberWithDouble:self->_requestTime];
...
- (BOOL)readFrom:(id)a3
{
  return NNMKProtoAccountAuthenticationStatusRequestReadFrom((uint64_t)self, (uint64_t)a3);
}
...
- (void)copyTo:(id)a3
{
  if (*(unsigned char *)&self->_has)
  {
    *((void *)a3 + 1) = *(void *)&self->_requestTime;
...
- (id)copyWithZone:(_NSZone *)a3
{
  ...
  if (*(unsigned char *)&self->_has)
  {
    *((void *)result + 1) = *(void *)&self->_requestTime;
...
- (BOOL)isEqual:(id)a3
{
  ...
  if (*(unsigned char *)&self->_has)
  {
    if ((*((unsigned char *)v4 + 16) & 1) != 0 && self->_requestTime == *((double *)v4 + 1))
    {
      BOOL v5 = 1;
      goto LABEL_7;
    }
...
- (unint64_t)hash
{
  ...
  double v4 = -requestTime;
  if (requestTime >= 0.0) {
    double v4 = self->_requestTime;
  }
...
- (void)mergeFrom:(id)a3
{
  ...
  {
    self->_double requestTime = *((double *)a3 + 1);
    *(unsigned char *)&self->_has |= 1u;
  }
}
