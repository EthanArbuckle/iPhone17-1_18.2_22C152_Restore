@interface GEOOfflineServiceRequestCancel
+ (Class)replyClass;
- (BOOL)isValid;
...
- (GEOOfflineServiceRequestCancel)initWithTraits:(id)a3 auditToken:(id)a4 throttleToken:(id)a5
{
  return [(GEOXPCRequest *)self initWithMessage:@"offlineserver.cancel" traits:a3 auditToken:a4 throttleToken:a5];
}
...
- (void)encodeToXPCDictionary:(id)a3
{
  ...
  if (requestMetadata)
  {
    id v6 = [MEMORY[0x1E4F28DB0] archivedDataWithRootObject:requestMetadata requiringSecureCoding:1 error:0];
    xpc_dictionary_set_data(v4, "requestMetadata", (const void *)[v6 bytes], objc_msgSend(v6, "length"));
  }
}
