...
- (id)description
{
  ...
  v4 = [(GEOPDTextItemDisplayConfig *)&v8 description];
  v5 = [(GEOPDTextItemDisplayConfig *)self dictionaryRepresentation];
...
- (BOOL)isEqual:(id)a3
{
  ...
  {
    if ((*((unsigned char *)v4 + 24) & 1) == 0 || self->_maxItemsPerRow != *((_DWORD *)v4 + 4)) {
      goto LABEL_11;
    }
  }
...
LABEL_11:
  ...
  {
    if ((*((unsigned char *)v4 + 24) & 2) == 0 || self->_maxRowCount != *((_DWORD *)v4 + 5)) {
      goto LABEL_11;
    }
    BOOL v5 = 1;
  }
...
- (unint64_t)hash
{
  if (*(unsigned char *)&self->_flags)
  {
    uint64_t v2 = 2654435761 * self->_maxItemsPerRow;
    if ((*(unsigned char *)&self->_flags & 2) != 0) {
      goto LABEL_3;
    }
LABEL_5:
...
LABEL_3:
  uint64_t v3 = 2654435761 * self->_maxRowCount;
  return v3 ^ v2;
}
