@interface GEOTraversalTimes
+ (BOOL)isValid:(id)a3;
- (BOOL)hasAggressiveEstimatedSeconds;
- (BOOL)hasConservativeEstimatedSeconds;
- (BOOL)hasEstimatedSeconds;
- (BOOL)hasFreeflowEstimatedSeconds;
- (BOOL)hasHistoricalEstimatedSeconds;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (GEOTraversalTimes)initWithDictionary:(id)a3;
- (GEOTraversalTimes)initWithJSON:(id)a3;
- (PBUnknownFields)unknownFields;
- (id)_dictionaryRepresentation:(uint64_t)a1;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
- (id)dictionaryRepresentation;
- (id)jsonRepresentation;
- (unint64_t)hash;
- (unsigned)aggressiveEstimatedSeconds;
- (unsigned)conservativeEstimatedSeconds;
- (unsigned)estimatedSeconds;
- (unsigned)freeflowEstimatedSeconds;
- (unsigned)historicalEstimatedSeconds;
- (void)_initWithDictionary:(int)a3 isJSON:;
- (void)clearUnknownFields:(BOOL)a3;
- (void)copyTo:(id)a3;
- (void)mergeFrom:(id)a3;
- (void)setAggressiveEstimatedSeconds:(unsigned int)a3;
- (void)setConservativeEstimatedSeconds:(unsigned int)a3;
- (void)setEstimatedSeconds:(unsigned int)a3;
- (void)setFreeflowEstimatedSeconds:(unsigned int)a3;
- (void)setHasAggressiveEstimatedSeconds:(BOOL)a3;
- (void)setHasConservativeEstimatedSeconds:(BOOL)a3;
- (void)setHasEstimatedSeconds:(BOOL)a3;
- (void)setHasFreeflowEstimatedSeconds:(BOOL)a3;
- (void)setHasHistoricalEstimatedSeconds:(BOOL)a3;
- (void)setHistoricalEstimatedSeconds:(unsigned int)a3;
- (void)writeTo:(id)a3;
@end

@implementation GEOTraversalTimes

- (unsigned)estimatedSeconds
{
  return self->_estimatedSeconds;
}

- (void)setEstimatedSeconds:(unsigned int)a3
{
  *(unsigned char *)&self->_flags |= 4u;
  self->_estimatedSeconds = a3;
}

- (void)setHasEstimatedSeconds:(BOOL)a3
{
  if (a3) {
    char v3 = 4;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_flags = *(unsigned char *)&self->_flags & 0xFB | v3;
}

- (BOOL)hasEstimatedSeconds
{
  return (*(unsigned char *)&self->_flags >> 2) & 1;
}

- (unsigned)historicalEstimatedSeconds
{
  return self->_historicalEstimatedSeconds;
}

- (void)setHistoricalEstimatedSeconds:(unsigned int)a3
{
  *(unsigned char *)&self->_flags |= 0x10u;
  self->_historicalEstimatedSeconds = a3;
}

- (void)setHasHistoricalEstimatedSeconds:(BOOL)a3
{
  if (a3) {
    char v3 = 16;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_flags = *(unsigned char *)&self->_flags & 0xEF | v3;
}

- (BOOL)hasHistoricalEstimatedSeconds
{
  return (*(unsigned char *)&self->_flags >> 4) & 1;
}

- (unsigned)freeflowEstimatedSeconds
{
  return self->_freeflowEstimatedSeconds;
}

- (void)setFreeflowEstimatedSeconds:(unsigned int)a3
{
  *(unsigned char *)&self->_flags |= 8u;
  self->_freeflowEstimatedSeconds = a3;
}

- (void)setHasFreeflowEstimatedSeconds:(BOOL)a3
{
  if (a3) {
    char v3 = 8;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_flags = *(unsigned char *)&self->_flags & 0xF7 | v3;
}

- (BOOL)hasFreeflowEstimatedSeconds
{
  return (*(unsigned char *)&self->_flags >> 3) & 1;
}

- (unsigned)aggressiveEstimatedSeconds
{
  return self->_aggressiveEstimatedSeconds;
}

- (void)setAggressiveEstimatedSeconds:(unsigned int)a3
{
  *(unsigned char *)&self->_flags |= 1u;
  self->_aggressiveEstimatedSeconds = a3;
}

- (void)setHasAggressiveEstimatedSeconds:(BOOL)a3
{
  *(unsigned char *)&self->_flags = *(unsigned char *)&self->_flags & 0xFE | a3;
}

- (BOOL)hasAggressiveEstimatedSeconds
{
  return *(unsigned char *)&self->_flags & 1;
}

- (unsigned)conservativeEstimatedSeconds
{
  return self->_conservativeEstimatedSeconds;
}

- (void)setConservativeEstimatedSeconds:(unsigned int)a3
{
  *(unsigned char *)&self->_flags |= 2u;
  self->_conservativeEstimatedSeconds = a3;
}

- (void)setHasConservativeEstimatedSeconds:(BOOL)a3
{
  if (a3) {
    char v3 = 2;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_flags = *(unsigned char *)&self->_flags & 0xFD | v3;
}

- (BOOL)hasConservativeEstimatedSeconds
{
  return (*(unsigned char *)&self->_flags >> 1) & 1;
}

- (id)description
{
  char v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)GEOTraversalTimes;
  v4 = [(GEOTraversalTimes *)&v8 description];
  v5 = [(GEOTraversalTimes *)self dictionaryRepresentation];
  v6 = [v3 stringWithFormat:@"%@ %@", v4, v5];

  return v6;
}

- (id)dictionaryRepresentation
{
  return -[GEOTraversalTimes _dictionaryRepresentation:]((uint64_t)self, 0);
}

- (id)_dictionaryRepresentation:(uint64_t)a1
{
  if (!a1)
  {
    v4 = 0;
    goto LABEL_15;
  }
  [(id)a1 readAll:1];
  v4 = [MEMORY[0x1E4F1CA60] dictionary];
  char v5 = *(unsigned char *)(a1 + 36);
  if ((v5 & 4) != 0)
  {
    v15 = [NSNumber numberWithUnsignedInt:*(unsigned int *)(a1 + 24)];
    if (a2) {
      v16 = @"estimatedSeconds";
    }
    else {
      v16 = @"estimated_seconds";
    }
    [v4 setObject:v15 forKey:v16];

    char v5 = *(unsigned char *)(a1 + 36);
    if ((v5 & 0x10) == 0)
    {
LABEL_4:
      if ((v5 & 8) == 0) {
        goto LABEL_5;
      }
      goto LABEL_26;
    }
  }
  else if ((*(unsigned char *)(a1 + 36) & 0x10) == 0)
  {
    goto LABEL_4;
  }
  v17 = [NSNumber numberWithUnsignedInt:*(unsigned int *)(a1 + 32)];
  if (a2) {
    v18 = @"historicalEstimatedSeconds";
  }
  else {
    v18 = @"historical_estimated_seconds";
  }
  [v4 setObject:v17 forKey:v18];

  char v5 = *(unsigned char *)(a1 + 36);
  if ((v5 & 8) == 0)
  {
LABEL_5:
    if ((v5 & 1) == 0) {
      goto LABEL_6;
    }
    goto LABEL_30;
  }
LABEL_26:
  v19 = [NSNumber numberWithUnsignedInt:*(unsigned int *)(a1 + 28)];
  if (a2) {
    v20 = @"freeflowEstimatedSeconds";
  }
  else {
    v20 = @"freeflow_estimated_seconds";
  }
  [v4 setObject:v19 forKey:v20];

  char v5 = *(unsigned char *)(a1 + 36);
  if ((v5 & 1) == 0)
  {
LABEL_6:
    if ((v5 & 2) == 0) {
      goto LABEL_11;
    }
    goto LABEL_7;
  }
LABEL_30:
  v21 = [NSNumber numberWithUnsignedInt:*(unsigned int *)(a1 + 16)];
  if (a2) {
    v22 = @"aggressiveEstimatedSeconds";
  }
  else {
    v22 = @"aggressive_estimated_seconds";
  }
  [v4 setObject:v21 forKey:v22];

  if ((*(unsigned char *)(a1 + 36) & 2) != 0)
  {
LABEL_7:
    v6 = [NSNumber numberWithUnsignedInt:*(unsigned int *)(a1 + 20)];
    if (a2) {
      v7 = @"conservativeEstimatedSeconds";
    }
    else {
      v7 = @"conservative_estimated_seconds";
    }
    [v4 setObject:v6 forKey:v7];
  }
LABEL_11:
  objc_super v8 = *(void **)(a1 + 8);
  if (v8)
  {
    v9 = [v8 dictionaryRepresentation];
    v10 = v9;
    if (a2)
    {
      v11 = objc_msgSend(MEMORY[0x1E4F1CA60], "dictionaryWithCapacity:", objc_msgSend(v9, "count"));
      v23[0] = MEMORY[0x1E4F143A8];
      v23[1] = 3221225472;
      v23[2] = __47__GEOTraversalTimes__dictionaryRepresentation___block_invoke;
      v23[3] = &unk_1E53D8860;
      id v12 = v11;
      id v24 = v12;
      [v10 enumerateKeysAndObjectsUsingBlock:v23];
      id v13 = v12;

      v10 = v13;
    }
    [v4 setObject:v10 forKey:@"Unknown Fields"];
  }
LABEL_15:

  return v4;
}

- (id)jsonRepresentation
{
  return -[GEOTraversalTimes _dictionaryRepresentation:]((uint64_t)self, 1);
}

void __47__GEOTraversalTimes__dictionaryRepresentation___block_invoke(uint64_t a1, void *a2, void *a3)
{
  id v10 = a2;
  id v5 = a3;
  objc_opt_class();
  char isKindOfClass = objc_opt_isKindOfClass();
  v7 = *(void **)(a1 + 32);
  if (isKindOfClass)
  {
    objc_super v8 = [v5 base64EncodedStringWithOptions:0];
    v9 = [v10 description];
    [v7 setObject:v8 forKey:v9];
  }
  else
  {
    objc_super v8 = [v10 description];
    [v7 setObject:v5 forKey:v8];
  }
}

- (GEOTraversalTimes)initWithDictionary:(id)a3
{
  return (GEOTraversalTimes *)-[GEOTraversalTimes _initWithDictionary:isJSON:](self, a3, 0);
}

- (void)_initWithDictionary:(int)a3 isJSON:
{
  id v5 = a2;
  if (a1)
  {
    a1 = (void *)[a1 init];
    if (a1)
    {
      if (a3) {
        v6 = @"estimatedSeconds";
      }
      else {
        v6 = @"estimated_seconds";
      }
      v7 = [v5 objectForKeyedSubscript:v6];
      objc_opt_class();
      if (objc_opt_isKindOfClass()) {
        objc_msgSend(a1, "setEstimatedSeconds:", objc_msgSend(v7, "unsignedIntValue"));
      }

      if (a3) {
        objc_super v8 = @"historicalEstimatedSeconds";
      }
      else {
        objc_super v8 = @"historical_estimated_seconds";
      }
      v9 = [v5 objectForKeyedSubscript:v8];
      objc_opt_class();
      if (objc_opt_isKindOfClass()) {
        objc_msgSend(a1, "setHistoricalEstimatedSeconds:", objc_msgSend(v9, "unsignedIntValue"));
      }

      if (a3) {
        id v10 = @"freeflowEstimatedSeconds";
      }
      else {
        id v10 = @"freeflow_estimated_seconds";
      }
      v11 = [v5 objectForKeyedSubscript:v10];
      objc_opt_class();
      if (objc_opt_isKindOfClass()) {
        objc_msgSend(a1, "setFreeflowEstimatedSeconds:", objc_msgSend(v11, "unsignedIntValue"));
      }

      if (a3) {
        id v12 = @"aggressiveEstimatedSeconds";
      }
      else {
        id v12 = @"aggressive_estimated_seconds";
      }
      id v13 = [v5 objectForKeyedSubscript:v12];
      objc_opt_class();
      if (objc_opt_isKindOfClass()) {
        objc_msgSend(a1, "setAggressiveEstimatedSeconds:", objc_msgSend(v13, "unsignedIntValue"));
      }

      if (a3) {
        v14 = @"conservativeEstimatedSeconds";
      }
      else {
        v14 = @"conservative_estimated_seconds";
      }
      v15 = [v5 objectForKeyedSubscript:v14];
      objc_opt_class();
      if (objc_opt_isKindOfClass()) {
        objc_msgSend(a1, "setConservativeEstimatedSeconds:", objc_msgSend(v15, "unsignedIntValue"));
      }
    }
  }

  return a1;
}

- (GEOTraversalTimes)initWithJSON:(id)a3
{
  return (GEOTraversalTimes *)-[GEOTraversalTimes _initWithDictionary:isJSON:](self, a3, 1);
}

+ (BOOL)isValid:(id)a3
{
  return GEOTraversalTimesIsValid((char *)a3);
}

- (BOOL)readFrom:(id)a3
{
  return GEOTraversalTimesReadAllFrom((uint64_t)self, a3);
}

- (void)writeTo:(id)a3
{
  id v5 = a3;
  char flags = (char)self->_flags;
  if ((flags & 4) != 0)
  {
    PBDataWriterWriteUint32Field();
    char flags = (char)self->_flags;
    if ((flags & 0x10) == 0)
    {
LABEL_3:
      if ((flags & 8) == 0) {
        goto LABEL_4;
      }
      goto LABEL_12;
    }
  }
  else if ((*(unsigned char *)&self->_flags & 0x10) == 0)
  {
    goto LABEL_3;
  }
  PBDataWriterWriteUint32Field();
  char flags = (char)self->_flags;
  if ((flags & 8) == 0)
  {
LABEL_4:
    if ((flags & 1) == 0) {
      goto LABEL_5;
    }
    goto LABEL_13;
  }
LABEL_12:
  PBDataWriterWriteUint32Field();
  char flags = (char)self->_flags;
  if ((flags & 1) == 0)
  {
LABEL_5:
    if ((flags & 2) == 0) {
      goto LABEL_7;
    }
    goto LABEL_6;
  }
LABEL_13:
  PBDataWriterWriteUint32Field();
  if ((*(unsigned char *)&self->_flags & 2) != 0) {
LABEL_6:
  }
    PBDataWriterWriteUint32Field();
LABEL_7:
  [(PBUnknownFields *)self->_unknownFields writeTo:v5];
}

- (void)copyTo:(id)a3
{
  id v5 = a3;
  [(GEOTraversalTimes *)self readAll:0];
  char flags = (char)self->_flags;
  if ((flags & 4) != 0)
  {
    v5[6] = self->_estimatedSeconds;
    *((unsigned char *)v5 + 36) |= 4u;
    char flags = (char)self->_flags;
    if ((flags & 0x10) == 0)
    {
LABEL_3:
      if ((flags & 8) == 0) {
        goto LABEL_4;
      }
      goto LABEL_12;
    }
  }
  else if ((*(unsigned char *)&self->_flags & 0x10) == 0)
  {
    goto LABEL_3;
  }
  v5[8] = self->_historicalEstimatedSeconds;
  *((unsigned char *)v5 + 36) |= 0x10u;
  char flags = (char)self->_flags;
  if ((flags & 8) == 0)
  {
LABEL_4:
    if ((flags & 1) == 0) {
      goto LABEL_5;
    }
    goto LABEL_13;
  }
LABEL_12:
  v5[7] = self->_freeflowEstimatedSeconds;
  *((unsigned char *)v5 + 36) |= 8u;
  char flags = (char)self->_flags;
  if ((flags & 1) == 0)
  {
LABEL_5:
    if ((flags & 2) == 0) {
      goto LABEL_7;
    }
    goto LABEL_6;
  }
LABEL_13:
  v5[4] = self->_aggressiveEstimatedSeconds;
  *((unsigned char *)v5 + 36) |= 1u;
  if ((*(unsigned char *)&self->_flags & 2) != 0)
  {
LABEL_6:
    v5[5] = self->_conservativeEstimatedSeconds;
    *((unsigned char *)v5 + 36) |= 2u;
  }
LABEL_7:
}

- (id)copyWithZone:(_NSZone *)a3
{
  uint64_t v4 = objc_msgSend((id)objc_msgSend((id)objc_opt_class(), "allocWithZone:", a3), "init");
  id v5 = (void *)v4;
  char flags = (char)self->_flags;
  if ((flags & 4) != 0)
  {
    *(_DWORD *)(v4 + 24) = self->_estimatedSeconds;
    *(unsigned char *)(v4 + 36) |= 4u;
    char flags = (char)self->_flags;
    if ((flags & 0x10) == 0)
    {
LABEL_3:
      if ((flags & 8) == 0) {
        goto LABEL_4;
      }
      goto LABEL_10;
    }
  }
  else if ((*(unsigned char *)&self->_flags & 0x10) == 0)
  {
    goto LABEL_3;
  }
  *(_DWORD *)(v4 + 32) = self->_historicalEstimatedSeconds;
  *(unsigned char *)(v4 + 36) |= 0x10u;
  char flags = (char)self->_flags;
  if ((flags & 8) == 0)
  {
LABEL_4:
    if ((flags & 1) == 0) {
      goto LABEL_5;
    }
LABEL_11:
    *(_DWORD *)(v4 + 16) = self->_aggressiveEstimatedSeconds;
    *(unsigned char *)(v4 + 36) |= 1u;
    if ((*(unsigned char *)&self->_flags & 2) == 0) {
      goto LABEL_7;
    }
    goto LABEL_6;
  }
LABEL_10:
  *(_DWORD *)(v4 + 28) = self->_freeflowEstimatedSeconds;
  *(unsigned char *)(v4 + 36) |= 8u;
  char flags = (char)self->_flags;
  if (flags) {
    goto LABEL_11;
  }
LABEL_5:
  if ((flags & 2) != 0)
  {
LABEL_6:
    *(_DWORD *)(v4 + 20) = self->_conservativeEstimatedSeconds;
    *(unsigned char *)(v4 + 36) |= 2u;
  }
LABEL_7:
  objc_storeStrong((id *)(v4 + 8), self->_unknownFields);
  return v5;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_26;
  }
  [(GEOTraversalTimes *)self readAll:1];
  [v4 readAll:1];
  if ((*(unsigned char *)&self->_flags & 4) != 0)
  {
    if ((*((unsigned char *)v4 + 36) & 4) == 0 || self->_estimatedSeconds != *((_DWORD *)v4 + 6)) {
      goto LABEL_26;
    }
  }
  else if ((*((unsigned char *)v4 + 36) & 4) != 0)
  {
LABEL_26:
    BOOL v5 = 0;
    goto LABEL_27;
  }
  if ((*(unsigned char *)&self->_flags & 0x10) != 0)
  {
    if ((*((unsigned char *)v4 + 36) & 0x10) == 0 || self->_historicalEstimatedSeconds != *((_DWORD *)v4 + 8)) {
      goto LABEL_26;
    }
  }
  else if ((*((unsigned char *)v4 + 36) & 0x10) != 0)
  {
    goto LABEL_26;
  }
  if ((*(unsigned char *)&self->_flags & 8) != 0)
  {
    if ((*((unsigned char *)v4 + 36) & 8) == 0 || self->_freeflowEstimatedSeconds != *((_DWORD *)v4 + 7)) {
      goto LABEL_26;
    }
  }
  else if ((*((unsigned char *)v4 + 36) & 8) != 0)
  {
    goto LABEL_26;
  }
  if (*(unsigned char *)&self->_flags)
  {
    if ((*((unsigned char *)v4 + 36) & 1) == 0 || self->_aggressiveEstimatedSeconds != *((_DWORD *)v4 + 4)) {
      goto LABEL_26;
    }
  }
  else if (*((unsigned char *)v4 + 36))
  {
    goto LABEL_26;
  }
  BOOL v5 = (*((unsigned char *)v4 + 36) & 2) == 0;
  if ((*(unsigned char *)&self->_flags & 2) != 0)
  {
    if ((*((unsigned char *)v4 + 36) & 2) == 0 || self->_conservativeEstimatedSeconds != *((_DWORD *)v4 + 5)) {
      goto LABEL_26;
    }
    BOOL v5 = 1;
  }
LABEL_27:

  return v5;
}

- (unint64_t)hash
{
  [(GEOTraversalTimes *)self readAll:1];
  if ((*(unsigned char *)&self->_flags & 4) != 0)
  {
    uint64_t v3 = 2654435761 * self->_estimatedSeconds;
    if ((*(unsigned char *)&self->_flags & 0x10) != 0)
    {
LABEL_3:
      uint64_t v4 = 2654435761 * self->_historicalEstimatedSeconds;
      if ((*(unsigned char *)&self->_flags & 8) != 0) {
        goto LABEL_4;
      }
      goto LABEL_9;
    }
  }
  else
  {
    uint64_t v3 = 0;
    if ((*(unsigned char *)&self->_flags & 0x10) != 0) {
      goto LABEL_3;
    }
  }
  uint64_t v4 = 0;
  if ((*(unsigned char *)&self->_flags & 8) != 0)
  {
LABEL_4:
    uint64_t v5 = 2654435761 * self->_freeflowEstimatedSeconds;
    if (*(unsigned char *)&self->_flags) {
      goto LABEL_5;
    }
LABEL_10:
    uint64_t v6 = 0;
    if ((*(unsigned char *)&self->_flags & 2) != 0) {
      goto LABEL_6;
    }
LABEL_11:
    uint64_t v7 = 0;
    return v4 ^ v3 ^ v5 ^ v6 ^ v7;
  }
LABEL_9:
  uint64_t v5 = 0;
  if ((*(unsigned char *)&self->_flags & 1) == 0) {
    goto LABEL_10;
  }
LABEL_5:
  uint64_t v6 = 2654435761 * self->_aggressiveEstimatedSeconds;
  if ((*(unsigned char *)&self->_flags & 2) == 0) {
    goto LABEL_11;
  }
LABEL_6:
  uint64_t v7 = 2654435761 * self->_conservativeEstimatedSeconds;
  return v4 ^ v3 ^ v5 ^ v6 ^ v7;
}

- (void)mergeFrom:(id)a3
{
  uint64_t v5 = (unsigned int *)a3;
  [v5 readAll:0];
  char v4 = *((unsigned char *)v5 + 36);
  if ((v4 & 4) != 0)
  {
    self->_estimatedSeconds = v5[6];
    *(unsigned char *)&self->_flags |= 4u;
    char v4 = *((unsigned char *)v5 + 36);
    if ((v4 & 0x10) == 0)
    {
LABEL_3:
      if ((v4 & 8) == 0) {
        goto LABEL_4;
      }
      goto LABEL_12;
    }
  }
  else if ((v5[9] & 0x10) == 0)
  {
    goto LABEL_3;
  }
  self->_historicalEstimatedSeconds = v5[8];
  *(unsigned char *)&self->_flags |= 0x10u;
  char v4 = *((unsigned char *)v5 + 36);
  if ((v4 & 8) == 0)
  {
LABEL_4:
    if ((v4 & 1) == 0) {
      goto LABEL_5;
    }
    goto LABEL_13;
  }
LABEL_12:
  self->_freeflowEstimatedSeconds = v5[7];
  *(unsigned char *)&self->_flags |= 8u;
  char v4 = *((unsigned char *)v5 + 36);
  if ((v4 & 1) == 0)
  {
LABEL_5:
    if ((v4 & 2) == 0) {
      goto LABEL_7;
    }
    goto LABEL_6;
  }
LABEL_13:
  self->_aggressiveEstimatedSeconds = v5[4];
  *(unsigned char *)&self->_flags |= 1u;
  if ((v5[9] & 2) != 0)
  {
LABEL_6:
    self->_conservativeEstimatedSeconds = v5[5];
    *(unsigned char *)&self->_flags |= 2u;
  }
LABEL_7:
}

- (PBUnknownFields)unknownFields
{
  return self->_unknownFields;
}

- (void)clearUnknownFields:(BOOL)a3
{
  BOOL v3 = a3;
  unknownFields = self->_unknownFields;
  self->_unknownFields = 0;

  if (v3)
  {
    [(GEOTraversalTimes *)self readAll:0];
  }
}

- (void).cxx_destruct
{
}

@end