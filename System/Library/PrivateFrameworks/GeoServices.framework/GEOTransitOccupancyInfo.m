...
{
  id v3 = a3;
  if ([v3 isEqualToString:@"OCCUPANCY_STATUS_UNKNOWN"])
  {
    int v4 = 0;
  }
  else if ([v3 isEqualToString:@"OCCUPANCY_STATUS_NOT_CROWDED"])
  {
    int v4 = 1;
  }
  else if ([v3 isEqualToString:@"OCCUPANCY_STATUS_CROWDED"])
  {
    int v4 = 2;
  }
  else if ([v3 isEqualToString:@"OCCUPANCY_STATUS_VERY_CROWDED"])
  {
    int v4 = 3;
  }
  else if ([v3 isEqualToString:@"OCCUPANCY_STATUS_NOT_ACCEPTING_PASSENGERS"])
  {
    int v4 = 4;
  }
  else
...
- (id)description
{
  ...
  int v4 = [(GEOTransitOccupancyInfo *)&v8 description];
  v5 = [(GEOTransitOccupancyInfo *)self dictionaryRepresentation];
...
- (void)_initWithDictionary:(int)a3 isJSON:
{
  ...
  if (a1)
  {
    ...
    if (a1)
    {
      ...
      {
        id v8 = v7;
        if ([v8 isEqualToString:@"OCCUPANCY_STATUS_UNKNOWN"])
        {
          uint64_t v9 = 0;
        }
        else if ([v8 isEqualToString:@"OCCUPANCY_STATUS_NOT_CROWDED"])
        {
          uint64_t v9 = 1;
        }
        else if ([v8 isEqualToString:@"OCCUPANCY_STATUS_CROWDED"])
        {
          uint64_t v9 = 2;
        }
        else if ([v8 isEqualToString:@"OCCUPANCY_STATUS_VERY_CROWDED"])
        {
          uint64_t v9 = 3;
        }
        else if ([v8 isEqualToString:@"OCCUPANCY_STATUS_NOT_ACCEPTING_PASSENGERS"])
        {
          uint64_t v9 = 4;
        }
        else
...
{
  id v4 = a3;
  [(GEOTransitOccupancyInfo *)self readAll:0];
  if (*(unsigned char *)&self->_flags)
  {
...
- (BOOL)isEqual:(id)a3
{
  ...
  [(GEOTransitOccupancyInfo *)self readAll:1];
  [v4 readAll:1];
  BOOL v5 = (*((unsigned char *)v4 + 20) & 1) == 0;
  if (*(unsigned char *)&self->_flags)
  {
    if ((*((unsigned char *)v4 + 20) & 1) != 0 && self->_occupancyStatus == *((_DWORD *)v4 + 4))
    {
      BOOL v5 = 1;
      goto LABEL_7;
    }
