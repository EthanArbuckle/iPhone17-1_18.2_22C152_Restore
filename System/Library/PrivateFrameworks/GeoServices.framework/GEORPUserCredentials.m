@interface GEORPUserCredentials
+ (BOOL)isValid:(id)a3;
+ (id)_credentialsForPrimaryICloudAccount;
- (BOOL)hasIcloudUserMapsAuthToken;
- (BOOL)hasIcloudUserPersonId;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (GEORPUserCredentials)init;
- (GEORPUserCredentials)initWithData:(id)a3;
- (GEORPUserCredentials)initWithDictionary:(id)a3;
- (GEORPUserCredentials)initWithJSON:(id)a3;
- (NSString)icloudUserMapsAuthToken;
- (NSString)icloudUserPersonId;
- (id)_dictionaryRepresentation:(void *)a1;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
- (id)dictionaryRepresentation;
- (id)jsonRepresentation;
- (unint64_t)hash;
- (void)_initWithDictionary:(int)a3 isJSON:;
- (void)_readIcloudUserMapsAuthToken;
- (void)_readIcloudUserPersonId;
- (void)copyTo:(id)a3;
- (void)mergeFrom:(id)a3;
- (void)readAll:(BOOL)a3;
- (void)setIcloudUserMapsAuthToken:(id)a3;
- (void)setIcloudUserPersonId:(id)a3;
- (void)writeTo:(id)a3;
@end

@implementation GEORPUserCredentials

+ (id)_credentialsForPrimaryICloudAccount
{
  if (GEOConfigGetBOOL(GeoServicesConfig_IncludeSensitiveDataInRAP, (uint64_t)off_1E9113E38)
    && +[GEOUserAccountInfo isAccountInfoAvailable])
  {
    v2 = objc_alloc_init(GEORPUserCredentials);
    v3 = +[GEOUserAccountInfo primaryICloudAccount];
    v4 = objc_msgSend(v3, "aa_personID");
    v5 = [v3 credential];
    v6 = [v5 mapsToken];

    [(GEORPUserCredentials *)v2 setIcloudUserPersonId:v4];
    [(GEORPUserCredentials *)v2 setIcloudUserMapsAuthToken:v6];
  }
  else
  {
    v2 = 0;
  }

  return v2;
}

- (GEORPUserCredentials)init
{
  v6.receiver = self;
  v6.super_class = (Class)GEORPUserCredentials;
  v2 = [(GEORPUserCredentials *)&v6 init];
  v3 = v2;
  if (v2)
  {
    v2->_readerLock._os_unfair_lock_opaque = 0;
    v4 = v2;
  }

  return v3;
}

- (GEORPUserCredentials)initWithData:(id)a3
{
  v7.receiver = self;
  v7.super_class = (Class)GEORPUserCredentials;
  v3 = [(GEORPUserCredentials *)&v7 initWithData:a3];
  v4 = v3;
  if (v3)
  {
    v3->_readerLock._os_unfair_lock_opaque = 0;
    v5 = v3;
  }

  return v4;
}

- (void)_readIcloudUserPersonId
{
  if (a1)
  {
    os_unfair_lock_lock((os_unfair_lock_t)(a1 + 40));
    if ((*(unsigned char *)(a1 + 44) & 2) == 0)
    {
      v2 = *(void **)(a1 + 8);
      if (v2)
      {
        id v3 = v2;
        objc_sync_enter(v3);
        GEORPUserCredentialsReadSpecified(a1, *(void *)(a1 + 8), (int *)&_readIcloudUserPersonId_tags);
        objc_sync_exit(v3);
      }
    }
    os_unfair_lock_unlock((os_unfair_lock_t)(a1 + 40));
  }
}

- (BOOL)hasIcloudUserPersonId
{
  return self->_icloudUserPersonId != 0;
}

- (NSString)icloudUserPersonId
{
  -[GEORPUserCredentials _readIcloudUserPersonId]((uint64_t)self);
  icloudUserPersonId = self->_icloudUserPersonId;

  return icloudUserPersonId;
}

- (void)setIcloudUserPersonId:(id)a3
{
  *(unsigned char *)&self->_flags |= 6u;
  objc_storeStrong((id *)&self->_icloudUserPersonId, a3);
}

- (void)_readIcloudUserMapsAuthToken
{
  if (a1)
  {
    os_unfair_lock_lock((os_unfair_lock_t)(a1 + 40));
    if ((*(unsigned char *)(a1 + 44) & 1) == 0)
    {
      v2 = *(void **)(a1 + 8);
      if (v2)
      {
        id v3 = v2;
        objc_sync_enter(v3);
        GEORPUserCredentialsReadSpecified(a1, *(void *)(a1 + 8), (int *)&_readIcloudUserMapsAuthToken_tags);
        objc_sync_exit(v3);
      }
    }
    os_unfair_lock_unlock((os_unfair_lock_t)(a1 + 40));
  }
}

- (BOOL)hasIcloudUserMapsAuthToken
{
  return self->_icloudUserMapsAuthToken != 0;
}

- (NSString)icloudUserMapsAuthToken
{
  -[GEORPUserCredentials _readIcloudUserMapsAuthToken]((uint64_t)self);
  icloudUserMapsAuthToken = self->_icloudUserMapsAuthToken;

  return icloudUserMapsAuthToken;
}

- (void)setIcloudUserMapsAuthToken:(id)a3
{
  *(unsigned char *)&self->_flags |= 5u;
  objc_storeStrong((id *)&self->_icloudUserMapsAuthToken, a3);
}

- (id)description
{
  id v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)GEORPUserCredentials;
  v4 = [(GEORPUserCredentials *)&v8 description];
  v5 = [(GEORPUserCredentials *)self dictionaryRepresentation];
  objc_super v6 = [v3 stringWithFormat:@"%@ %@", v4, v5];

  return v6;
}

- (id)dictionaryRepresentation
{
  return -[GEORPUserCredentials _dictionaryRepresentation:](self, 0);
}

- (id)_dictionaryRepresentation:(void *)a1
{
  if (a1)
  {
    [a1 readAll:1];
    v4 = [MEMORY[0x1E4F1CA60] dictionary];
    v5 = [a1 icloudUserPersonId];
    if (v5)
    {
      if (a2) {
        objc_super v6 = @"icloudUserPersonId";
      }
      else {
        objc_super v6 = @"icloud_user_person_id";
      }
      [v4 setObject:v5 forKey:v6];
    }

    objc_super v7 = [a1 icloudUserMapsAuthToken];
    if (v7)
    {
      if (a2) {
        objc_super v8 = @"icloudUserMapsAuthToken";
      }
      else {
        objc_super v8 = @"icloud_user_maps_auth_token";
      }
      [v4 setObject:v7 forKey:v8];
    }
  }
  else
  {
    v4 = 0;
  }

  return v4;
}

- (id)jsonRepresentation
{
  return -[GEORPUserCredentials _dictionaryRepresentation:](self, 1);
}

- (GEORPUserCredentials)initWithDictionary:(id)a3
{
  return (GEORPUserCredentials *)-[GEORPUserCredentials _initWithDictionary:isJSON:](self, a3, 0);
}

- (void)_initWithDictionary:(int)a3 isJSON:
{
  id v5 = a2;
  if (a1)
  {
    a1 = (void *)[a1 init];
    if (a1)
    {
      if (a3) {
        objc_super v6 = @"icloudUserPersonId";
      }
      else {
        objc_super v6 = @"icloud_user_person_id";
      }
      objc_super v7 = [v5 objectForKeyedSubscript:v6];
      objc_opt_class();
      if (objc_opt_isKindOfClass())
      {
        objc_super v8 = (void *)[v7 copy];
        [a1 setIcloudUserPersonId:v8];
      }
      if (a3) {
        v9 = @"icloudUserMapsAuthToken";
      }
      else {
        v9 = @"icloud_user_maps_auth_token";
      }
      v10 = [v5 objectForKeyedSubscript:v9];
      objc_opt_class();
      if (objc_opt_isKindOfClass())
      {
        v11 = (void *)[v10 copy];
        [a1 setIcloudUserMapsAuthToken:v11];
      }
    }
  }

  return a1;
}

- (GEORPUserCredentials)initWithJSON:(id)a3
{
  return (GEORPUserCredentials *)-[GEORPUserCredentials _initWithDictionary:isJSON:](self, a3, 1);
}

- (void)readAll:(BOOL)a3
{
  BOOL v3 = a3;
  p_readerLock = &self->_readerLock;
  os_unfair_lock_lock(&self->_readerLock);
  reader = self->_reader;
  if (reader)
  {
    objc_super v7 = reader;
    objc_sync_enter(v7);
    if (v3) {
      objc_super v8 = (int *)&readAll__recursiveTag_3383;
    }
    else {
      objc_super v8 = (int *)&readAll__nonRecursiveTag_3384;
    }
    GEORPUserCredentialsReadSpecified((uint64_t)self, (unint64_t)self->_reader, v8);
    objc_sync_exit(v7);
  }

  os_unfair_lock_unlock(p_readerLock);
}

+ (BOOL)isValid:(id)a3
{
  return GEORPUserCredentialsIsValid((char *)a3);
}

- (BOOL)readFrom:(id)a3
{
  return GEORPUserCredentialsReadAllFrom((uint64_t)self, a3, 0);
}

- (void)writeTo:(id)a3
{
  id v7 = a3;
  p_readerLock = &self->_readerLock;
  os_unfair_lock_lock(&self->_readerLock);
  if (self->_reader && (os_unfair_lock_assert_owner(&self->_readerLock), (*(unsigned char *)&self->_flags & 7) == 0))
  {
    id v5 = self->_reader;
    objc_sync_enter(v5);
    *(void *)((char *)self->_reader + (int)*MEMORY[0x1E4F940E0]) = self->_readerMarkLength;
    [(PBDataReader *)self->_reader seekToOffset:self->_readerMarkPos];
    objc_super v6 = [(PBDataReader *)self->_reader readBytes:self->_readerMarkLength - self->_readerMarkPos];
    [v7 writeData:v6];
    os_unfair_lock_unlock(p_readerLock);

    objc_sync_exit(v5);
  }
  else
  {
    os_unfair_lock_unlock(&self->_readerLock);
    [(GEORPUserCredentials *)self readAll:0];
    if (self->_icloudUserPersonId) {
      PBDataWriterWriteStringField();
    }
    if (self->_icloudUserMapsAuthToken) {
      PBDataWriterWriteStringField();
    }
  }
}

- (void)copyTo:(id)a3
{
  v4 = (id *)a3;
  [(GEORPUserCredentials *)self readAll:0];
  os_unfair_lock_lock_with_options();
  objc_storeStrong(v4 + 1, self->_reader);
  *((_DWORD *)v4 + 8) = self->_readerMarkPos;
  *((_DWORD *)v4 + 9) = self->_readerMarkLength;
  os_unfair_lock_unlock(&self->_readerLock);
  if (self->_icloudUserPersonId) {
    objc_msgSend(v4, "setIcloudUserPersonId:");
  }
  if (self->_icloudUserMapsAuthToken) {
    objc_msgSend(v4, "setIcloudUserMapsAuthToken:");
  }
}

- (id)copyWithZone:(_NSZone *)a3
{
  id v5 = objc_msgSend((id)objc_msgSend((id)objc_opt_class(), "allocWithZone:", a3), "init");
  os_unfair_lock_lock(&self->_readerLock);
  reader = self->_reader;
  if (!reader) {
    goto LABEL_5;
  }
  id v7 = reader;
  objc_sync_enter(v7);
  if ((*(unsigned char *)&self->_flags & 4) != 0)
  {
    objc_sync_exit(v7);

LABEL_5:
    os_unfair_lock_unlock(&self->_readerLock);
    [(GEORPUserCredentials *)self readAll:0];
    uint64_t v8 = [(NSString *)self->_icloudUserPersonId copyWithZone:a3];
    v9 = (void *)v5[3];
    v5[3] = v8;

    uint64_t v10 = [(NSString *)self->_icloudUserMapsAuthToken copyWithZone:a3];
    id v7 = (PBDataReader *)v5[2];
    v5[2] = v10;
    goto LABEL_6;
  }
  *(void *)((char *)self->_reader + (int)*MEMORY[0x1E4F940E0]) = self->_readerMarkLength;
  [(PBDataReader *)self->_reader seekToOffset:self->_readerMarkPos];
  GEORPUserCredentialsReadAllFrom((uint64_t)v5, self->_reader, 0);
  os_unfair_lock_unlock(&self->_readerLock);
  objc_sync_exit(v7);
LABEL_6:

  return v5;
}

- (BOOL)isEqual:(id)a3
{
  v4 = a3;
  if ([v4 isMemberOfClass:objc_opt_class()]
    && (([(GEORPUserCredentials *)self readAll:1],
         [v4 readAll:1],
         icloudUserPersonId = self->_icloudUserPersonId,
         !((unint64_t)icloudUserPersonId | v4[3]))
     || -[NSString isEqual:](icloudUserPersonId, "isEqual:")))
  {
    icloudUserMapsAuthToken = self->_icloudUserMapsAuthToken;
    if ((unint64_t)icloudUserMapsAuthToken | v4[2]) {
      char v7 = -[NSString isEqual:](icloudUserMapsAuthToken, "isEqual:");
    }
    else {
      char v7 = 1;
    }
  }
  else
  {
    char v7 = 0;
  }

  return v7;
}

- (unint64_t)hash
{
  [(GEORPUserCredentials *)self readAll:1];
  NSUInteger v3 = [(NSString *)self->_icloudUserPersonId hash];
  return [(NSString *)self->_icloudUserMapsAuthToken hash] ^ v3;
}

- (void)mergeFrom:(id)a3
{
  v4 = a3;
  [v4 readAll:0];
  if (v4[3]) {
    -[GEORPUserCredentials setIcloudUserPersonId:](self, "setIcloudUserPersonId:");
  }
  if (v4[2]) {
    -[GEORPUserCredentials setIcloudUserMapsAuthToken:](self, "setIcloudUserMapsAuthToken:");
  }
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_icloudUserPersonId, 0);
  objc_storeStrong((id *)&self->_icloudUserMapsAuthToken, 0);

  objc_storeStrong((id *)&self->_reader, 0);
}

@end