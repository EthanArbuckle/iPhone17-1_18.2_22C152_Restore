@interface GEORPRapResponse
+ (BOOL)isValid:(id)a3;
- (BOOL)hasDisplayDetails;
- (BOOL)hasGreenTeaWithValue:(BOOL)a3;
- (BOOL)hasNotification;
- (BOOL)hasRapStateIcon;
- (BOOL)hasResponseId;
- (BOOL)hasUserFormDetails;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (GEORPDisplayDetails)displayDetails;
- (GEORPRapNotification)notification;
- (GEORPRapResponse)init;
- (GEORPRapResponse)initWithData:(id)a3;
- (GEORPRapResponse)initWithDictionary:(id)a3;
- (GEORPRapResponse)initWithJSON:(id)a3;
- (GEORPUserFormDetails)userFormDetails;
- (NSString)responseId;
- (PBUnknownFields)unknownFields;
- (id)_dictionaryRepresentation:(uint64_t)a1;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
- (id)dictionaryRepresentation;
- (id)jsonRepresentation;
- (id)rapStateIconAsString:(int)a3;
- (int)StringAsRapStateIcon:(id)a3;
- (int)rapStateIcon;
- (unint64_t)hash;
- (void)_initWithDictionary:(int)a3 isJSON:;
- (void)_readDisplayDetails;
- (void)_readNotification;
- (void)_readResponseId;
- (void)_readUserFormDetails;
- (void)clearUnknownFields:(BOOL)a3;
- (void)copyTo:(id)a3;
- (void)mergeFrom:(id)a3;
- (void)readAll:(BOOL)a3;
- (void)setDisplayDetails:(id)a3;
- (void)setHasRapStateIcon:(BOOL)a3;
- (void)setNotification:(id)a3;
- (void)setRapStateIcon:(int)a3;
- (void)setResponseId:(id)a3;
- (void)setUserFormDetails:(id)a3;
- (void)writeTo:(id)a3;
@end

@implementation GEORPRapResponse

- (GEORPRapResponse)init
{
  v6.receiver = self;
  v6.super_class = (Class)GEORPRapResponse;
  v2 = [(GEORPRapResponse *)&v6 init];
  v3 = v2;
  if (v2)
  {
    v2->_readerLock._os_unfair_lock_opaque = 0;
    v4 = v2;
  }

  return v3;
}

- (GEORPRapResponse)initWithData:(id)a3
{
  v7.receiver = self;
  v7.super_class = (Class)GEORPRapResponse;
  v3 = [(GEORPRapResponse *)&v7 initWithData:a3];
  v4 = v3;
  if (v3)
  {
    v3->_readerLock._os_unfair_lock_opaque = 0;
    v5 = v3;
  }

  return v4;
}

- (void)_readResponseId
{
  if (a1)
  {
    os_unfair_lock_lock((os_unfair_lock_t)(a1 + 64));
    if ((*(unsigned char *)(a1 + 72) & 0x10) == 0)
    {
      v2 = *(void **)(a1 + 8);
      if (v2)
      {
        id v3 = v2;
        objc_sync_enter(v3);
        GEORPRapResponseReadSpecified(a1, *(void *)(a1 + 8), (int *)&_readResponseId_tags_2);
        objc_sync_exit(v3);
      }
    }
    os_unfair_lock_unlock((os_unfair_lock_t)(a1 + 64));
  }
}

- (BOOL)hasResponseId
{
  return self->_responseId != 0;
}

- (NSString)responseId
{
  -[GEORPRapResponse _readResponseId]((uint64_t)self);
  responseId = self->_responseId;

  return responseId;
}

- (void)setResponseId:(id)a3
{
  *(unsigned char *)&self->_flags |= 0x50u;
  objc_storeStrong((id *)&self->_responseId, a3);
}

- (int)rapStateIcon
{
  p_readerLock = &self->_readerLock;
  os_unfair_lock_lock_with_options();
  char flags = (char)self->_flags;
  os_unfair_lock_unlock(p_readerLock);
  if (flags) {
    return self->_rapStateIcon;
  }
  else {
    return 0;
  }
}

- (void)setRapStateIcon:(int)a3
{
  *(unsigned char *)&self->_flags |= 0x41u;
  self->_rapStateIcon = a3;
}

- (void)setHasRapStateIcon:(BOOL)a3
{
  *(unsigned char *)&self->_char flags = *(unsigned char *)&self->_flags & 0xFE | a3 | 0x40;
}

- (BOOL)hasRapStateIcon
{
  return *(unsigned char *)&self->_flags & 1;
}

- (id)rapStateIconAsString:(int)a3
{
  if (a3 >= 6)
  {
    objc_msgSend(NSString, "stringWithFormat:", @"(unknown: %i)", *(void *)&a3);
    id v3 = (__CFString *)objc_claimAutoreleasedReturnValue();
  }
  else
  {
    id v3 = off_1E53F68A8[a3];
  }

  return v3;
}

- (int)StringAsRapStateIcon:(id)a3
{
  id v3 = a3;
  if ([v3 isEqualToString:@"RAP_RESPONSE_RAP_STATE_ICON_UNKNOWN"])
  {
    int v4 = 0;
  }
  else if ([v3 isEqualToString:@"RAP_RESPONSE_RAP_STATE_ICON_UNRESOLVED"])
  {
    int v4 = 1;
  }
  else if ([v3 isEqualToString:@"RAP_RESPONSE_RAP_STATE_ICON_RESOLVED"])
  {
    int v4 = 2;
  }
  else if ([v3 isEqualToString:@"RAP_RESPONSE_RAP_STATE_ICON_REJECTED"])
  {
    int v4 = 3;
  }
  else if ([v3 isEqualToString:@"RAP_RESPONSE_RAP_STATE_ICON_NEEDS_ADDITIONAL_INFO"])
  {
    int v4 = 4;
  }
  else if ([v3 isEqualToString:@"RAP_RESPONSE_RAP_STATE_ICON_RESOLVED_NOT_LIVE"])
  {
    int v4 = 5;
  }
  else
  {
    int v4 = 0;
  }

  return v4;
}

- (void)_readNotification
{
  if (a1)
  {
    os_unfair_lock_lock((os_unfair_lock_t)(a1 + 64));
    if ((*(unsigned char *)(a1 + 72) & 8) == 0)
    {
      v2 = *(void **)(a1 + 8);
      if (v2)
      {
        id v3 = v2;
        objc_sync_enter(v3);
        GEORPRapResponseReadSpecified(a1, *(void *)(a1 + 8), (int *)&_readNotification_tags_935);
        objc_sync_exit(v3);
      }
    }
    os_unfair_lock_unlock((os_unfair_lock_t)(a1 + 64));
  }
}

- (BOOL)hasNotification
{
  return self->_notification != 0;
}

- (GEORPRapNotification)notification
{
  -[GEORPRapResponse _readNotification]((uint64_t)self);
  notification = self->_notification;

  return notification;
}

- (void)setNotification:(id)a3
{
  *(unsigned char *)&self->_flags |= 0x48u;
  objc_storeStrong((id *)&self->_notification, a3);
}

- (void)_readDisplayDetails
{
  if (a1)
  {
    os_unfair_lock_lock((os_unfair_lock_t)(a1 + 64));
    if ((*(unsigned char *)(a1 + 72) & 4) == 0)
    {
      v2 = *(void **)(a1 + 8);
      if (v2)
      {
        id v3 = v2;
        objc_sync_enter(v3);
        GEORPRapResponseReadSpecified(a1, *(void *)(a1 + 8), (int *)&_readDisplayDetails_tags);
        objc_sync_exit(v3);
      }
    }
    os_unfair_lock_unlock((os_unfair_lock_t)(a1 + 64));
  }
}

- (BOOL)hasDisplayDetails
{
  return self->_displayDetails != 0;
}

- (GEORPDisplayDetails)displayDetails
{
  -[GEORPRapResponse _readDisplayDetails]((uint64_t)self);
  displayDetails = self->_displayDetails;

  return displayDetails;
}

- (void)setDisplayDetails:(id)a3
{
  *(unsigned char *)&self->_flags |= 0x44u;
  objc_storeStrong((id *)&self->_displayDetails, a3);
}

- (void)_readUserFormDetails
{
  if (a1)
  {
    os_unfair_lock_lock((os_unfair_lock_t)(a1 + 64));
    if ((*(unsigned char *)(a1 + 72) & 0x20) == 0)
    {
      v2 = *(void **)(a1 + 8);
      if (v2)
      {
        id v3 = v2;
        objc_sync_enter(v3);
        GEORPRapResponseReadSpecified(a1, *(void *)(a1 + 8), (int *)&_readUserFormDetails_tags);
        objc_sync_exit(v3);
      }
    }
    os_unfair_lock_unlock((os_unfair_lock_t)(a1 + 64));
  }
}

- (BOOL)hasUserFormDetails
{
  return self->_userFormDetails != 0;
}

- (GEORPUserFormDetails)userFormDetails
{
  -[GEORPRapResponse _readUserFormDetails]((uint64_t)self);
  userFormDetails = self->_userFormDetails;

  return userFormDetails;
}

- (void)setUserFormDetails:(id)a3
{
  *(unsigned char *)&self->_flags |= 0x60u;
  objc_storeStrong((id *)&self->_userFormDetails, a3);
}

- (id)description
{
  id v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)GEORPRapResponse;
  int v4 = [(GEORPRapResponse *)&v8 description];
  v5 = [(GEORPRapResponse *)self dictionaryRepresentation];
  objc_super v6 = [v3 stringWithFormat:@"%@ %@", v4, v5];

  return v6;
}

- (id)dictionaryRepresentation
{
  return -[GEORPRapResponse _dictionaryRepresentation:]((uint64_t)self, 0);
}

- (id)_dictionaryRepresentation:(uint64_t)a1
{
  if (a1)
  {
    [(id)a1 readAll:1];
    int v4 = [MEMORY[0x1E4F1CA60] dictionary];
    v5 = [(id)a1 responseId];
    if (v5)
    {
      if (a2) {
        objc_super v6 = @"responseId";
      }
      else {
        objc_super v6 = @"response_id";
      }
      [v4 setObject:v5 forKey:v6];
    }

    if (*(unsigned char *)(a1 + 72))
    {
      uint64_t v7 = *(int *)(a1 + 68);
      if (v7 >= 6)
      {
        objc_msgSend(NSString, "stringWithFormat:", @"(unknown: %i)", *(int *)(a1 + 68));
        objc_super v8 = (__CFString *)objc_claimAutoreleasedReturnValue();
      }
      else
      {
        objc_super v8 = off_1E53F68A8[v7];
      }
      if (a2) {
        v9 = @"rapStateIcon";
      }
      else {
        v9 = @"rap_state_icon";
      }
      [v4 setObject:v8 forKey:v9];
    }
    v10 = [(id)a1 notification];
    v11 = v10;
    if (v10)
    {
      if (a2) {
        [v10 jsonRepresentation];
      }
      else {
      v12 = [v10 dictionaryRepresentation];
      }
      [v4 setObject:v12 forKey:@"notification"];
    }
    v13 = [(id)a1 displayDetails];
    v14 = v13;
    if (v13)
    {
      if (a2)
      {
        v15 = [v13 jsonRepresentation];
        v16 = @"displayDetails";
      }
      else
      {
        v15 = [v13 dictionaryRepresentation];
        v16 = @"display_details";
      }
      [v4 setObject:v15 forKey:v16];
    }
    v17 = [(id)a1 userFormDetails];
    v18 = v17;
    if (v17)
    {
      if (a2)
      {
        v19 = [v17 jsonRepresentation];
        v20 = @"userFormDetails";
      }
      else
      {
        v19 = [v17 dictionaryRepresentation];
        v20 = @"user_form_details";
      }
      [v4 setObject:v19 forKey:v20];
    }
    v21 = *(void **)(a1 + 16);
    if (v21)
    {
      v22 = [v21 dictionaryRepresentation];
      v23 = v22;
      if (a2)
      {
        v24 = objc_msgSend(MEMORY[0x1E4F1CA60], "dictionaryWithCapacity:", objc_msgSend(v22, "count"));
        v28[0] = MEMORY[0x1E4F143A8];
        v28[1] = 3221225472;
        v28[2] = __46__GEORPRapResponse__dictionaryRepresentation___block_invoke;
        v28[3] = &unk_1E53D8860;
        id v25 = v24;
        id v29 = v25;
        [v23 enumerateKeysAndObjectsUsingBlock:v28];
        id v26 = v25;

        v23 = v26;
      }
      [v4 setObject:v23 forKey:@"Unknown Fields"];
    }
  }
  else
  {
    int v4 = 0;
  }

  return v4;
}

- (id)jsonRepresentation
{
  return -[GEORPRapResponse _dictionaryRepresentation:]((uint64_t)self, 1);
}

void __46__GEORPRapResponse__dictionaryRepresentation___block_invoke(uint64_t a1, void *a2, void *a3)
{
  id v10 = a2;
  id v5 = a3;
  objc_opt_class();
  char isKindOfClass = objc_opt_isKindOfClass();
  uint64_t v7 = *(void **)(a1 + 32);
  if (isKindOfClass)
  {
    objc_super v8 = [v5 base64EncodedStringWithOptions:0];
    v9 = [v10 description];
    [v7 setObject:v8 forKey:v9];
  }
  else
  {
    objc_super v8 = [v10 description];
    [v7 setObject:v5 forKey:v8];
  }
}

- (GEORPRapResponse)initWithDictionary:(id)a3
{
  return (GEORPRapResponse *)-[GEORPRapResponse _initWithDictionary:isJSON:](self, a3, 0);
}

- (void)_initWithDictionary:(int)a3 isJSON:
{
  id v5 = a2;
  if (!a1) {
    goto LABEL_51;
  }
  a1 = (void *)[a1 init];
  if (!a1) {
    goto LABEL_51;
  }
  if (a3) {
    objc_super v6 = @"responseId";
  }
  else {
    objc_super v6 = @"response_id";
  }
  uint64_t v7 = [v5 objectForKeyedSubscript:v6];
  objc_opt_class();
  if (objc_opt_isKindOfClass())
  {
    objc_super v8 = (void *)[v7 copy];
    [a1 setResponseId:v8];
  }
  if (a3) {
    v9 = @"rapStateIcon";
  }
  else {
    v9 = @"rap_state_icon";
  }
  id v10 = [v5 objectForKeyedSubscript:v9];
  objc_opt_class();
  if (objc_opt_isKindOfClass())
  {
    id v11 = v10;
    if ([v11 isEqualToString:@"RAP_RESPONSE_RAP_STATE_ICON_UNKNOWN"])
    {
      uint64_t v12 = 0;
    }
    else if ([v11 isEqualToString:@"RAP_RESPONSE_RAP_STATE_ICON_UNRESOLVED"])
    {
      uint64_t v12 = 1;
    }
    else if ([v11 isEqualToString:@"RAP_RESPONSE_RAP_STATE_ICON_RESOLVED"])
    {
      uint64_t v12 = 2;
    }
    else if ([v11 isEqualToString:@"RAP_RESPONSE_RAP_STATE_ICON_REJECTED"])
    {
      uint64_t v12 = 3;
    }
    else if ([v11 isEqualToString:@"RAP_RESPONSE_RAP_STATE_ICON_NEEDS_ADDITIONAL_INFO"])
    {
      uint64_t v12 = 4;
    }
    else if ([v11 isEqualToString:@"RAP_RESPONSE_RAP_STATE_ICON_RESOLVED_NOT_LIVE"])
    {
      uint64_t v12 = 5;
    }
    else
    {
      uint64_t v12 = 0;
    }

    goto LABEL_28;
  }
  objc_opt_class();
  if (objc_opt_isKindOfClass())
  {
    uint64_t v12 = [v10 intValue];
LABEL_28:
    [a1 setRapStateIcon:v12];
  }

  v13 = [v5 objectForKeyedSubscript:@"notification"];
  objc_opt_class();
  if (objc_opt_isKindOfClass())
  {
    v14 = [GEORPRapNotification alloc];
    if (a3) {
      uint64_t v15 = [(GEORPRapNotification *)v14 initWithJSON:v13];
    }
    else {
      uint64_t v15 = [(GEORPRapNotification *)v14 initWithDictionary:v13];
    }
    v16 = (void *)v15;
    [a1 setNotification:v15];
  }
  if (a3) {
    v17 = @"displayDetails";
  }
  else {
    v17 = @"display_details";
  }
  v18 = [v5 objectForKeyedSubscript:v17];
  objc_opt_class();
  if (objc_opt_isKindOfClass())
  {
    v19 = [GEORPDisplayDetails alloc];
    if (a3) {
      uint64_t v20 = [(GEORPDisplayDetails *)v19 initWithJSON:v18];
    }
    else {
      uint64_t v20 = [(GEORPDisplayDetails *)v19 initWithDictionary:v18];
    }
    v21 = (void *)v20;
    [a1 setDisplayDetails:v20];
  }
  if (a3) {
    v22 = @"userFormDetails";
  }
  else {
    v22 = @"user_form_details";
  }
  v23 = [v5 objectForKeyedSubscript:v22];
  objc_opt_class();
  if (objc_opt_isKindOfClass())
  {
    v24 = [GEORPUserFormDetails alloc];
    if (a3) {
      uint64_t v25 = [(GEORPUserFormDetails *)v24 initWithJSON:v23];
    }
    else {
      uint64_t v25 = [(GEORPUserFormDetails *)v24 initWithDictionary:v23];
    }
    id v26 = (void *)v25;
    [a1 setUserFormDetails:v25];
  }
LABEL_51:

  return a1;
}

- (GEORPRapResponse)initWithJSON:(id)a3
{
  return (GEORPRapResponse *)-[GEORPRapResponse _initWithDictionary:isJSON:](self, a3, 1);
}

- (void)readAll:(BOOL)a3
{
  BOOL v3 = a3;
  p_readerLock = &self->_readerLock;
  os_unfair_lock_lock(&self->_readerLock);
  reader = self->_reader;
  if (reader)
  {
    uint64_t v7 = reader;
    objc_sync_enter(v7);
    if (v3) {
      objc_super v8 = (int *)&readAll__recursiveTag_963;
    }
    else {
      objc_super v8 = (int *)&readAll__nonRecursiveTag_964;
    }
    GEORPRapResponseReadSpecified((uint64_t)self, (unint64_t)self->_reader, v8);
    objc_sync_exit(v7);
  }
  if (v3)
  {
    [(GEORPRapNotification *)self->_notification readAll:1];
    [(GEORPDisplayDetails *)self->_displayDetails readAll:1];
    [(GEORPUserFormDetails *)self->_userFormDetails readAll:1];
  }

  os_unfair_lock_unlock(p_readerLock);
}

+ (BOOL)isValid:(id)a3
{
  return GEORPRapResponseIsValid((char *)a3);
}

- (BOOL)readFrom:(id)a3
{
  return GEORPRapResponseReadAllFrom((uint64_t)self, a3, 0);
}

- (void)writeTo:(id)a3
{
  id v8 = a3;
  p_readerLock = &self->_readerLock;
  os_unfair_lock_lock(&self->_readerLock);
  if (self->_reader && (_GEORPRapResponseIsDirty((uint64_t)self) & 1) == 0)
  {
    objc_super v6 = self->_reader;
    objc_sync_enter(v6);
    *(void *)((char *)self->_reader + (int)*MEMORY[0x1E4F940E0]) = self->_readerMarkLength;
    [(PBDataReader *)self->_reader seekToOffset:self->_readerMarkPos];
    uint64_t v7 = [(PBDataReader *)self->_reader readBytes:self->_readerMarkLength - self->_readerMarkPos];
    [v8 writeData:v7];
    os_unfair_lock_unlock(p_readerLock);

    objc_sync_exit(v6);
  }
  else
  {
    os_unfair_lock_unlock(&self->_readerLock);
    [(GEORPRapResponse *)self readAll:0];
    if (self->_responseId) {
      PBDataWriterWriteStringField();
    }
    id v5 = v8;
    if (*(unsigned char *)&self->_flags)
    {
      PBDataWriterWriteInt32Field();
      id v5 = v8;
    }
    if (self->_notification)
    {
      PBDataWriterWriteSubmessage();
      id v5 = v8;
    }
    if (self->_displayDetails)
    {
      PBDataWriterWriteSubmessage();
      id v5 = v8;
    }
    if (self->_userFormDetails)
    {
      PBDataWriterWriteSubmessage();
      id v5 = v8;
    }
    [(PBUnknownFields *)self->_unknownFields writeTo:v5];
  }
}

- (BOOL)hasGreenTeaWithValue:(BOOL)a3
{
  BOOL v3 = a3;
  -[GEORPRapResponse _readDisplayDetails]((uint64_t)self);
  displayDetails = self->_displayDetails;

  return [(GEORPDisplayDetails *)displayDetails hasGreenTeaWithValue:v3];
}

- (void)copyTo:(id)a3
{
  id v5 = (id *)a3;
  [(GEORPRapResponse *)self readAll:0];
  os_unfair_lock_lock_with_options();
  objc_storeStrong(v5 + 1, self->_reader);
  *((_DWORD *)v5 + 14) = self->_readerMarkPos;
  *((_DWORD *)v5 + 15) = self->_readerMarkLength;
  os_unfair_lock_unlock(&self->_readerLock);
  if (self->_responseId) {
    objc_msgSend(v5, "setResponseId:");
  }
  int v4 = v5;
  if (*(unsigned char *)&self->_flags)
  {
    *((_DWORD *)v5 + 17) = self->_rapStateIcon;
    *((unsigned char *)v5 + 72) |= 1u;
  }
  if (self->_notification)
  {
    objc_msgSend(v5, "setNotification:");
    int v4 = v5;
  }
  if (self->_displayDetails)
  {
    objc_msgSend(v5, "setDisplayDetails:");
    int v4 = v5;
  }
  if (self->_userFormDetails)
  {
    objc_msgSend(v5, "setUserFormDetails:");
    int v4 = v5;
  }
}

- (id)copyWithZone:(_NSZone *)a3
{
  uint64_t v5 = objc_msgSend((id)objc_msgSend((id)objc_opt_class(), "allocWithZone:", a3), "init");
  os_unfair_lock_lock(&self->_readerLock);
  reader = self->_reader;
  if (reader)
  {
    uint64_t v7 = reader;
    objc_sync_enter(v7);
    if ((*(unsigned char *)&self->_flags & 0x40) == 0)
    {
      *(void *)((char *)self->_reader + (int)*MEMORY[0x1E4F940E0]) = self->_readerMarkLength;
      [(PBDataReader *)self->_reader seekToOffset:self->_readerMarkPos];
      GEORPRapResponseReadAllFrom(v5, self->_reader, 0);
      os_unfair_lock_unlock(&self->_readerLock);
      id v8 = (id)v5;
      objc_sync_exit(v7);

      goto LABEL_8;
    }
    objc_sync_exit(v7);
  }
  os_unfair_lock_unlock(&self->_readerLock);
  [(GEORPRapResponse *)self readAll:0];
  uint64_t v9 = [(NSString *)self->_responseId copyWithZone:a3];
  id v10 = *(void **)(v5 + 40);
  *(void *)(v5 + 40) = v9;

  if (*(unsigned char *)&self->_flags)
  {
    *(_DWORD *)(v5 + 68) = self->_rapStateIcon;
    *(unsigned char *)(v5 + 72) |= 1u;
  }
  id v11 = [(GEORPRapNotification *)self->_notification copyWithZone:a3];
  uint64_t v12 = *(void **)(v5 + 32);
  *(void *)(v5 + 32) = v11;

  id v13 = [(GEORPDisplayDetails *)self->_displayDetails copyWithZone:a3];
  v14 = *(void **)(v5 + 24);
  *(void *)(v5 + 24) = v13;

  id v15 = [(GEORPUserFormDetails *)self->_userFormDetails copyWithZone:a3];
  v16 = *(void **)(v5 + 48);
  *(void *)(v5 + 48) = v15;

  v17 = self->_unknownFields;
  id v8 = *(id *)(v5 + 16);
  *(void *)(v5 + 16) = v17;
LABEL_8:

  return (id)v5;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_15;
  }
  [(GEORPRapResponse *)self readAll:1];
  [v4 readAll:1];
  responseId = self->_responseId;
  if ((unint64_t)responseId | *((void *)v4 + 5))
  {
    if (!-[NSString isEqual:](responseId, "isEqual:")) {
      goto LABEL_15;
    }
  }
  if (*(unsigned char *)&self->_flags)
  {
    if ((*((unsigned char *)v4 + 72) & 1) == 0 || self->_rapStateIcon != *((_DWORD *)v4 + 17)) {
      goto LABEL_15;
    }
  }
  else if (*((unsigned char *)v4 + 72))
  {
LABEL_15:
    char v9 = 0;
    goto LABEL_16;
  }
  notification = self->_notification;
  if ((unint64_t)notification | *((void *)v4 + 4)
    && !-[GEORPRapNotification isEqual:](notification, "isEqual:"))
  {
    goto LABEL_15;
  }
  displayDetails = self->_displayDetails;
  if ((unint64_t)displayDetails | *((void *)v4 + 3))
  {
    if (!-[GEORPDisplayDetails isEqual:](displayDetails, "isEqual:")) {
      goto LABEL_15;
    }
  }
  userFormDetails = self->_userFormDetails;
  if ((unint64_t)userFormDetails | *((void *)v4 + 6)) {
    char v9 = -[GEORPUserFormDetails isEqual:](userFormDetails, "isEqual:");
  }
  else {
    char v9 = 1;
  }
LABEL_16:

  return v9;
}

- (unint64_t)hash
{
  [(GEORPRapResponse *)self readAll:1];
  NSUInteger v3 = [(NSString *)self->_responseId hash];
  if (*(unsigned char *)&self->_flags) {
    uint64_t v4 = 2654435761 * self->_rapStateIcon;
  }
  else {
    uint64_t v4 = 0;
  }
  NSUInteger v5 = v4 ^ v3;
  unint64_t v6 = [(GEORPRapNotification *)self->_notification hash];
  unint64_t v7 = v5 ^ v6 ^ [(GEORPDisplayDetails *)self->_displayDetails hash];
  return v7 ^ [(GEORPUserFormDetails *)self->_userFormDetails hash];
}

- (void)mergeFrom:(id)a3
{
  id v11 = a3;
  [v11 readAll:0];
  uint64_t v4 = v11;
  if (v11[5])
  {
    -[GEORPRapResponse setResponseId:](self, "setResponseId:");
    uint64_t v4 = v11;
  }
  if (v4[9])
  {
    self->_rapStateIcon = *((_DWORD *)v4 + 17);
    *(unsigned char *)&self->_flags |= 1u;
  }
  notification = self->_notification;
  uint64_t v6 = v4[4];
  if (notification)
  {
    if (!v6) {
      goto LABEL_11;
    }
    -[GEORPRapNotification mergeFrom:](notification, "mergeFrom:");
  }
  else
  {
    if (!v6) {
      goto LABEL_11;
    }
    -[GEORPRapResponse setNotification:](self, "setNotification:");
  }
  uint64_t v4 = v11;
LABEL_11:
  displayDetails = self->_displayDetails;
  uint64_t v8 = v4[3];
  if (displayDetails)
  {
    if (!v8) {
      goto LABEL_17;
    }
    -[GEORPDisplayDetails mergeFrom:](displayDetails, "mergeFrom:");
  }
  else
  {
    if (!v8) {
      goto LABEL_17;
    }
    -[GEORPRapResponse setDisplayDetails:](self, "setDisplayDetails:");
  }
  uint64_t v4 = v11;
LABEL_17:
  userFormDetails = self->_userFormDetails;
  uint64_t v10 = v4[6];
  if (userFormDetails)
  {
    if (!v10) {
      goto LABEL_23;
    }
    -[GEORPUserFormDetails mergeFrom:](userFormDetails, "mergeFrom:");
  }
  else
  {
    if (!v10) {
      goto LABEL_23;
    }
    -[GEORPRapResponse setUserFormDetails:](self, "setUserFormDetails:");
  }
  uint64_t v4 = v11;
LABEL_23:
}

- (PBUnknownFields)unknownFields
{
  p_readerLock = &self->_readerLock;
  os_unfair_lock_lock(&self->_readerLock);
  if ((*(unsigned char *)&self->_flags & 2) == 0)
  {
    reader = self->_reader;
    if (reader)
    {
      NSUInteger v5 = reader;
      objc_sync_enter(v5);
      GEORPRapResponseReadSpecified((uint64_t)self, (unint64_t)self->_reader, unknownFields_tags_968);
      objc_sync_exit(v5);
    }
  }
  os_unfair_lock_unlock(p_readerLock);
  unknownFields = self->_unknownFields;

  return unknownFields;
}

- (void)clearUnknownFields:(BOOL)a3
{
  BOOL v3 = a3;
  p_readerLock = &self->_readerLock;
  os_unfair_lock_lock_with_options();
  *(unsigned char *)&self->_flags |= 0x42u;
  os_unfair_lock_unlock(p_readerLock);
  unknownFields = self->_unknownFields;
  self->_unknownFields = 0;

  if (v3)
  {
    [(GEORPRapResponse *)self readAll:0];
    [(GEORPRapNotification *)self->_notification clearUnknownFields:1];
    [(GEORPDisplayDetails *)self->_displayDetails clearUnknownFields:1];
    userFormDetails = self->_userFormDetails;
    [(GEORPUserFormDetails *)userFormDetails clearUnknownFields:1];
  }
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_userFormDetails, 0);
  objc_storeStrong((id *)&self->_responseId, 0);
  objc_storeStrong((id *)&self->_notification, 0);
  objc_storeStrong((id *)&self->_displayDetails, 0);
  objc_storeStrong((id *)&self->_unknownFields, 0);

  objc_storeStrong((id *)&self->_reader, 0);
}

@end