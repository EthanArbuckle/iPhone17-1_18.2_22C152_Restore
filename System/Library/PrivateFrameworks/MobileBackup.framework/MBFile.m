...
+ (MBFile)fileWithDecoder:(id)a3 database:(id)a4;
+ (MBFile)fileWithDomain:(id)a3 relativePath:(id)a4;
+ (MBFile)fileWithDomain:(id)a3 snapshotPath:(id)a4 relativePath:(id)a5;
...
- (BOOL)isTopLevelDirectoryToExcludeFromiCloud
{
  BOOL v3 = [(MBFile *)self isDirectory];
  if (v3)
  {
    v4 = [(MBFile *)self relativePath];
    id v5 = [v4 length];

    if (v5)
    {
      BOOL v3 = [(MBFile *)self _hasDefaultOwnershipAndAttributes];
      if (v3)
      {
        v6 = [(MBFile *)self domain];
        v7 = +[MBMobileInstallation defaultSubdirectoriesForMBContainerType:withNestedSubdirectories:](MBMobileInstallation, "defaultSubdirectoriesForMBContainerType:withNestedSubdirectories:", [v6 containerType], 1);

        v8 = [(MBFile *)self relativePath];
        LOBYTE(v6) = [v7 containsObject:v8];
    ...
    {
      v9 = [(MBFile *)self domain];
      uint64_t v10 = [v9 isContainerizedDomain];
      ...
      {
        LOBYTE(v3) = [(MBFile *)self _hasDefaultOwnershipAndAttributes];
      }
    }
  }
  return v3;
...
- (BOOL)_hasDefaultOwnershipAndAttributes
{
  unsigned __int8 v3 = [(MBFile *)self hasXattrs];
  unsigned __int8 v4 = [(MBFile *)self hasFlags];
  unsigned int v5 = [(MBFile *)self userID];
  if (v5 == MBMobileUID())
  {
    unsigned int v6 = [(MBFile *)self groupID];
    BOOL v7 = v6 == MBMobileUID();
  }
  else
  {
    BOOL v7 = 0;
  }
  unsigned int v8 = [(MBFile *)self userID];
  if (v8 == MBNobodyUID())
  {
    unsigned int v9 = [(MBFile *)self groupID];
    BOOL v10 = v9 == MBNobodyUID();
  }
  else
  {
    BOOL v10 = 0;
...
+ (MBFile)fileWithDomain:(id)a3 snapshotPath:(id)a4 relativePath:(id)a5
{
  unsigned int v5 = [[MBFile alloc] initWithDomain:a3 snapshotPath:a4 relativePath:a5];
  return v5;
}

+ (MBFile)fileWithDomain:(id)a3 relativePath:(id)a4
{
  unsigned __int8 v4 = [[MBFile alloc] initWithDomain:a3 snapshotPath:0 relativePath:a4];
  return v4;
}

+ (MBFile)fileWithDecoder:(id)a3 database:(id)a4
{
  unsigned __int8 v4 = [[MBFile alloc] initWithDecoder:a3 database:a4];
  return v4;
}
...
- (MBFile)initWithDecoder:(id)a3 database:(id)a4
{
  ...
  unsigned int v6 = [(MBFile *)&v20 init];
  if (!v6) {
    return v6;
  }
...
LABEL_20:
    id v18 = objc_msgSend(v16, "initWithCode:format:", 11, v17, v19);
    goto LABEL_21;
  }
  id v8 = v7;
  ...
  unsigned int v9 = (MBDomain *)objc_msgSend(objc_msgSend(a4, "domainManager"), "domainForName:", v8);
  v6->_domain = v9;
  if (!v9)
  {
...
LABEL_21:
  ...
  BOOL v11 = +[NSMutableDictionary dictionaryWithCapacity:v10];
  if (v10 >= 1)
  {
    while (1)
    {
...
LABEL_5:
  ...
  unsigned int v9 = [(MBFile *)&v11 init];
  if (v9)
  {
...
- (void)dealloc
{
  unsigned __int8 v3 = *(void **)&self->_hasOverriddenModifiedDate;
  if (v3) {
    free(v3);
  }
...
- (NSString)description
{
  if ([self->_snapshotID hasRootPath]) {
    unsigned __int8 v3 = [(MBFile *)self absolutePath];
  }
  else {
    unsigned __int8 v3 = +[NSString stringWithFormat:](NSString, "stringWithFormat:", @"%@:%@", [self->_snapshotID name], self->_nonRedirectedDomain);
  }
...
- (void)encode:(id)a3
{
  objc_msgSend(a3, "encodeString:", objc_msgSend(self->_snapshotID, "name"));
  ...
  objc_msgSend(a3, "encodeInt8:", (char)objc_msgSend(self->_decryptedSize, "count"));
  long long v15 = 0u;
  long long v16 = 0u;
  long long v13 = 0u;
  long long v14 = 0u;
  ...
  [a3 encodeInt32:self->_mbNode.flags];
}
...
- (void)encodeWithCoder:(id)a3
{
  ...
  {
    uint64_t v12 = 0;
    uint64_t v9 = +[NSPropertyListSerialization dataWithPropertyList:self->_decryptedSize format:200 options:0 error:&v12];
    if (v9 || !v12)
    {
      if (v9) {
  ...
  objc_msgSend(a3, "encodeInt32:forKey:", self->_mbNode.flags, @"Flags", v11);
}

- (MBFile)initWithCoder:(id)a3
{
  v12.receiver = self;
  v12.super_class = (Class)MBFile;
  id v4 = [(MBFile *)&v12 init];
  if (v4)
  {
    ...
    unsigned int v8 = objc_msgSend(a3, "decodeInt32ForKey:", @"Flags", v10);
    v4->_mbNode.flags = v8;
    if ((v8 & 0x40000000) != 0) {
      v4->_mbNode.fileSize = 0;
    }
  }
  return v4;
}
...
- (void)getStat:(stat *)a3
{
  ...
  *(_OWORD *)&a3->st_dev = 0u;
  ...
  __uint32_t flags = self->_mbNode.flags;
  a3->st_mode = WORD2(self->_mbNode.cloneID);
  a3->st_nlink = 0;
  a3->st_ino = inode;
  uint64_t v9 = *(void *)&self->_mbNode.userID;
  a3->st_dev = 0;
  *(void *)&a3->st_uid = v9;
  a3->st_rdev = 0;
  *(_OWORD *)(&a3->st_rdev + 1) = v10;
...
- (NSString)typeString
{
  return (NSString *)MBNodeTypeString(WORD2(self->_mbNode.cloneID));
}
...
- (BOOL)isCompressed
{
  return (self->_mbNode.flags & 0x40000020) == 32;
}
