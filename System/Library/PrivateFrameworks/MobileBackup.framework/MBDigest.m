...
+ (id)sha256ForData:(id)a3
{
  ...
  return (id)[v4 digestForData:a3];
}

+ (id)sha256ForFileAtPath:(id)a3 error:(id *)a4
{
  id v6 = +[MBDigest sha256];
  return (id)[v6 digestForFileAtPath:a3 error:a4];
}
...
{
  id v4 = +[MBDigest sha1];
  return (id)[v4 digestForString:a3];
}
...
+ (id)sha1HmacForString:(id)a3 key:(id)a4
{
  ...
  if (!a4) {
    objc_msgSend((id)objc_msgSend(MEMORY[0x1E4F28B00], "currentHandler"), "handleFailureInMethod:object:file:lineNumber:description:", a2, a1, @"MBDigest.m", 64, @"No key specified to sha1HmacForString");
  }
  [a4 bytes];
  [a4 length];
  [a3 UTF8String];
  ...
  CCHmacOneShot();
  return (id)[MEMORY[0x1E4F1C9B8] dataWithBytes:v7 length:20];
}
...
- (void)updateWithString:(id)a3
{
  if (a3 && [a3 length])
  {
    size_t v6 = [a3 maximumLengthOfBytesUsingEncoding:4];
    int v7 = malloc_type_malloc(v6, 0x59560A4uLL);
    uint64_t v8 = 0;
    if ((objc_msgSend(a3, "getBytes:maxLength:usedLength:encoding:options:range:remainingRange:", v7, v6, &v8, 4, 0, 0, objc_msgSend(a3, "length"), 0) & 1) == 0)objc_msgSend((id)objc_msgSend(MEMORY[0x1E4F28B00], "currentHandler"), "handleFailureInMethod:object:file:lineNumber:description:", a2, self, @"MBDigest.m", 205, @"Error encoding string"); {
    [(MBDigest *)self updateWithBytes:v7 length:v8];
    }
    free(v7);
  }
}
