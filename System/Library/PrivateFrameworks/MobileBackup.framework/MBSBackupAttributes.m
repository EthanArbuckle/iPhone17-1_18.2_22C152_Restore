...
- (BOOL)readFrom:(id)a3
{
  return MBSBackupAttributesReadFrom((uint64_t)self, (uint64_t)a3);
}
...
+ (id)systemAttributes
{
  v2 = objc_alloc_init(MBSBackupAttributes);
  NSUInteger v3 = MBDeviceClass();
  [(MBSBackupAttributes *)v2 setDeviceClass:v3];

  unint64_t v4 = MBProductType();
  [(MBSBackupAttributes *)v2 setProductType:v4];

  NSUInteger v5 = MBSerialNumber();
  [(MBSBackupAttributes *)v2 setSerialNumber:v5];

  unint64_t v6 = MBDeviceColor();
  [(MBSBackupAttributes *)v2 setDeviceColor:v6];

  NSUInteger v7 = MBDeviceEnclosureColor();
  [(MBSBackupAttributes *)v2 setDeviceEnclosureColor:v7];

  unint64_t v8 = MBHardwareModel();
  [(MBSBackupAttributes *)v2 setHardwareModel:v8];

  v9 = MBMarketingName();
  [(MBSBackupAttributes *)v2 setMarketingName:v9];
