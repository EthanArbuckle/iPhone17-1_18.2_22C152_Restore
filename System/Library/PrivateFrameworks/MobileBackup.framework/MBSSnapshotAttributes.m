...
- (id)dictionaryRepresentation
{
  ...
  if ((*(unsigned char *)&self->_has & 4) != 0) {
    objc_msgSend(v4, "setObject:forKey:", +[NSNumber numberWithUnsignedInt:](NSNumber, "numberWithUnsignedInt:", self->_keybagID), @"keybagID");
  }
  ...
  if (has)
  {
    ...
    if (backupReason >= 3) {
      ...
      v11 = off_100415BE8[backupReason];
    }
...
- (BOOL)readFrom:(id)a3
{
  return MBSSnapshotAttributesReadFrom((uint64_t)self, (uint64_t)a3);
}
...
LABEL_25:
          ...
          if (has)
          {
            if ((v10 & 1) == 0 || self->_backupReason != *((_DWORD *)a3 + 2)) {
              goto LABEL_25;
            }
          }
          else if (v10)
          ...
          if ((has & 2) != 0)
          {
            if ((v10 & 2) == 0 || self->_backupType != *((_DWORD *)a3 + 3)) {
              goto LABEL_25;
            }
            LOBYTE(v5) = 1;
          }
...
+ (id)snapshotAttributes
{
  v2 = objc_alloc_init(MBSSnapshotAttributes);
  return v2;
}
...
{
  v2 = objc_alloc_init(MBSSnapshotAttributes);
  NSUInteger v3 = MBDeviceName();
  [(MBSSnapshotAttributes *)v2 setDeviceName:v3];

  NSUInteger v4 = MBProductVersion();
  [(MBSSnapshotAttributes *)v2 setProductVersion:v4];

  char v5 = MBBuildVersion();
  [(MBSSnapshotAttributes *)v2 setBuildVersion:v5];
