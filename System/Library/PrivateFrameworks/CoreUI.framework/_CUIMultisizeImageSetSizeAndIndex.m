...
- (_CUIMultisizeImageSetSizeAndIndex)initWithSize:(CGSize)a3 index:(unsigned int)a4 idiom:(int64_t)a5 subtype:(unint64_t)a6
{
  ...
  v12.receiver = self;
  v12.super_class = (Class)_CUIMultisizeImageSetSizeAndIndex;
...
{
  v3.receiver = self;
  v3.super_class = (Class)_CUIMultisizeImageSetSizeAndIndex;
  return +[NSString stringWithFormat:@"%@ size: (%d, %d) index: %d idiom: %d subtype: %d", [(_CUIMultisizeImageSetSizeAndIndex *)&v3 description], (int)self->_size.width, (int)self->_size.height, self->_index, self->_idiom, self->_subtype];
}
