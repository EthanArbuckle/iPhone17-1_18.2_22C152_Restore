...
{
  v4 = [a3 clientState];
  v5 = [v4 objectForKeyedSubscript:@"sideCarSync"];
  if ((objc_opt_isKindOfClass() & 1) == 0)
  {
    id v7 = objc_alloc_init((Class)objc_opt_class());

    [v7 setNeedsSync:0];
    [v4 setObject:v7 forKeyedSubscript:@"sideCarSync"];
    v5 = v7;
  }
...
- (BRCSideCarSyncPersistedState)initWithZoneHealthState:(id)a3
{
  ...
  if (v5)
  {
    ...
    v5->_needsSync = *((unsigned char *)v6 + 16);
...
- (BRCSideCarSyncPersistedState)initWithCoder:(id)a3
{
  ...
  if (v5)
  {
    ...
    v5->_needsSync = [v4 decodeBoolForKey:@"sideCarSyncIdle"] ^ 1;
...
- (void)encodeWithCoder:(id)a3
{
  ...
  [v4 encodeBool:!v5->_needsSync forKey:@"sideCarSyncIdle"];
  ...
  if (requestID < a4)
  {
    if (requestID)
    {
      ...
      if (os_log_type_enabled(v13, OS_LOG_TYPE_FAULT))
      {
        ...
        __int16 v18 = 2112;
        v19 = v12;
        _os_log_fault_impl(&dword_1D353B000, v13, OS_LOG_TYPE_FAULT, "[CRIT] Assertion failed: _requestID == 0 %@: got request ID: %llu%@", (uint8_t *)&v14, 0x20u);
      }
    }
...
- (id)description
{
  ...
  uint64_t v10 = [v5 stringWithFormat:@"side-car-state{last-sync:%@, needs-sync:%@, requestID:%lld, changeToken:%@}", v6, v7, requestID, v9];
