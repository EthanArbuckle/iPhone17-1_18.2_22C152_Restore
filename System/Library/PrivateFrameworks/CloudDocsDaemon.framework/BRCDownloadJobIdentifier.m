...
- (id)matchingJobsWhereSQLClause
{
  return (id)[MEMORY[0x1E4FBA8F8] formatInjection:@"throttle_id = %lld AND download_etag = %@ AND download_kind = %u", self->_itemDBRowID, self->_etag, self->_kind];
}

- (id)columns
{
  return (id)objc_msgSend(MEMORY[0x1E4FBA908], "rawInjection:length:", "throttle_id, download_etag, download_kind", 41);
}

- (id)columnsValues
{
  return (id)[MEMORY[0x1E4FBA8F8] formatInjection:@"%lld, %@, %u", self->_itemDBRowID, self->_etag, self->_kind];
}
