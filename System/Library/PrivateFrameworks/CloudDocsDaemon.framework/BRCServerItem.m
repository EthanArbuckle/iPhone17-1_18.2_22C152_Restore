...
- (BRCServerItem)initWithServerItem:(id)a3
{
  ...
  if (v5)
  {
    ...
    v6->_session = (BRCAccountSession *)*((void *)v4 + 1);
...
- (id)initFromPQLResultSet:(id)a3 serverZone:(id)a4 error:(id *)a5
{
  ...
  uint64_t v9 = [(BRCServerItem *)&v39 init];
  if (v9)
  {
    v9->_session = (BRCAccountSession *)(id)[v8 session];
    uint64_t v10 = [v7 db];
    db = v9->_db;
      ...
      && [(BRCStatInfo *)v9->_st state] != 1)
    {
      ...
      if (os_log_type_enabled(v32, OS_LOG_TYPE_FAULT)) {
        -[BRCServerItem initFromPQLResultSet:serverZone:error:]((uint64_t)v31, v32, v33, v34, v35, v36, v37, v38);
      }
    }
    ...
    uint64_t v28 = [objc_alloc(MEMORY[0x1E4F59540]) initFromResultSet:v7 pos:36];
...
{
  id v4 = a3;
  uint64_t v5 = (void *)[objc_alloc(MEMORY[0x1E4F28E78]) initWithCapacity:128];
  if (v4)
  {
  ...
  if (v8 <= 3) {
    [v5 appendString:off_1E6997E28[v8]];
  }
...
- (id)appLibraryForRootItem
{
    ...
    || [(id)*p_itemID isDocumentsFolder])
  {
    ...
    uint64_t v6 = [(BRCAccountSession *)session appLibraryByRowID:v5];
  }
  else
  {
...
- (id)aliasTargetClientZone
{
  ...
  id v4 = [(BRCAccountSession *)session clientZoneByMangledID:v3];
...
- (id)aliasTargetAppLibrary
{
  session = self->_session;
  char v3 = [(BRCStatInfo *)self->_st _aliasTargetMangledID];
  id v4 = [(BRCAccountSession *)session appLibraryByMangledID:v3];
...
- (void)overrideCKInfoIfNecessaryForOutOfBandSyncOpWithLocalItem:(id)a3
{
  ...
  if (([v4 localDiffs] & 0x1000000000000000) != 0)
  {
    ...
    if (os_log_type_enabled(v6, OS_LOG_TYPE_DEBUG))
    {
      ...
      __int16 v22 = 2112;
      unsigned int v23 = v5;
      _os_log_debug_impl(&dword_1D353B000, v6, OS_LOG_TYPE_DEBUG, "[DEBUG] %@ has an out of band sync ack -- using client truth ckinfo %@ rather than server info %@%@", (uint8_t *)&v16, 0x2Au);
    }
    id v7 = [v4 st];
...
{
  OUTLINED_FUNCTION_2(&dword_1D353B000, a2, a3, "[CRIT] Assertion failed: !_serverZone.isSharedZone || (_sharingOptions & BRCSharingItemHasCKShareMask) != 0 || _st.state == BRC_ITEM_STATE_TOMBSTONE%@", a5, a6, a7, a8, 2u);
}
...
{
  OUTLINED_FUNCTION_10_0();
  OUTLINED_FUNCTION_15_0(&dword_1D353B000, v0, v1, "[CRIT] UNREACHABLE: unsupporter server item type: %d%@");
}
