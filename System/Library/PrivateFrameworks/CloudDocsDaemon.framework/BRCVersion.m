...
- (BRCVersion)initWithCoder:(id)a3;
...
- (id)lastEditorUserIdentityWithDB:(id)a3
{
  ...
  if (v5)
  {
    v6 = +[BRCAccountSession userIdentityForKey:v5 db:v4];
  }
  else
  {
    v6 = 0;
...
{
  id v4 = a3;
  uint64_t v5 = (void *)[objc_alloc(MEMORY[0x1E4F28E78]) initWithCapacity:64];
  ckInfo = self->_ckInfo;
  if (ckInfo)
  {
...
- (BRCVersion)initWithCoder:(id)a3
{
  ...
  v28.receiver = self;
  v28.super_class = (Class)BRCVersion;
  uint64_t v5 = [(BRCVersion *)&v28 init];
  if (v5)
  {
    ...
    v5->_mtime = [v4 decodeInt64ForKey:@"mtime"];
    uint64_t v8 = [v4 decodeObjectOfClass:objc_opt_class() forKey:@"name"];
    originalPOSIXName = v5->_originalPOSIXName;
    v5->_originalPOSIXName = (NSString *)v8;

    v5->_size = [v4 decodeInt64ForKey:@"size"];
    v5->_thumbnailSize = [v4 decodeInt64ForKey:@"thumbnailSize"];
    uint64_t v10 = [v4 decodeObjectOfClass:objc_opt_class() forKey:@"thumbnail"];
    thumbnailSignature = v5->_thumbnailSignature;
    v5->_thumbnailSignature = (NSData *)v10;
    ...
    uint64_t v14 = [v4 decodeObjectOfClass:objc_opt_class() forKey:@"editedKey"];
    editedSinceShared = v5->_editedSinceShared;
    v5->_editedSinceShared = (NSNumber *)v14;

    uint64_t v16 = [v4 decodeObjectOfClass:objc_opt_class() forKey:@"deviceKey"];
    lastEditorDeviceOrUserRowID = v5->_lastEditorDeviceOrUserRowID;
    v5->_lastEditorDeviceOrUserRowID = (NSNumber *)v16;

    uint64_t v18 = [v4 decodeObjectOfClass:objc_opt_class() forKey:@"deviceName"];
    lastEditorDeviceName = v5->_lastEditorDeviceName;
    v5->_lastEditorDeviceName = (NSString *)v18;

    if (_BRCClassesConflictLoserEtags_once != -1) {
      dispatch_once(&_BRCClassesConflictLoserEtags_once, &__block_literal_global_10);
    }
    id v20 = (id)_BRCClassesConflictLoserEtags_allowedClasses;
    uint64_t v21 = [v4 decodeObjectOfClasses:v20 forKey:@"conflictLoserEtags"];
    conflictLoserEtags = v5->_conflictLoserEtags;
    v5->_conflictLoserEtags = (NSSet *)v21;

    uint64_t v23 = [v4 decodeObjectOfClass:objc_opt_class() forKey:@"xattrSig"];
    xattrSignature = v5->_xattrSignature;
    v5->_xattrSignature = (NSData *)v23;

    uint64_t v25 = [v4 decodeObjectOfClass:objc_opt_class() forKey:@"quarantineInfo"];
    quarantineInfo = v5->_quarantineInfo;
    v5->_quarantineInfo = (NSData *)v25;
  }
  return v5;
}
...
- (void)encodeWithCoder:(id)a3
{
  ...
  [v5 encodeInt64:self->_mtime forKey:@"mtime"];
  [v5 encodeObject:self->_originalPOSIXName forKey:@"name"];
  [v5 encodeInt64:self->_size forKey:@"size"];
  [v5 encodeInt64:self->_thumbnailSize forKey:@"thumbnailSize"];
  [v5 encodeObject:self->_thumbnailSignature forKey:@"thumbnail"];
  [v5 encodeObject:self->_contentSignature forKey:@"content"];
  [v5 encodeObject:self->_editedSinceShared forKey:@"editedKey"];
  [v5 encodeObject:self->_lastEditorDeviceOrUserRowID forKey:@"deviceKey"];
  [v5 encodeObject:self->_lastEditorDeviceName forKey:@"deviceName"];
  [v5 encodeObject:self->_conflictLoserEtags forKey:@"conflictLoserEtags"];
  [v5 encodeObject:self->_xattrSignature forKey:@"xattrSig"];
  [v5 encodeObject:self->_quarantineInfo forKey:@"quarantineInfo"];
}
...
- (BOOL)isSmallAndMostRecentClientsGenerateThumbnails
{
  ...
  if ([(BRCVersion *)self size] <= v4)
  {
    ...
    int v9 = [MEMORY[0x1E4F3A6E8] canMostRecentClientsGenerateThumbnailsForUTI:v5];
    if (v9)
    {
...
LABEL_15:
        _os_log_debug_impl(&dword_1D353B000, v10, OS_LOG_TYPE_DEBUG, v12, (uint8_t *)&v14, 0x20u);
      }
    }
    else
    {
  ...
  if (os_log_type_enabled(v6, OS_LOG_TYPE_DEBUG))
  {
    ...
    __int16 v18 = 2112;
    uint64_t v19 = v5;
    _os_log_debug_impl(&dword_1D353B000, v6, OS_LOG_TYPE_DEBUG, "[DEBUG] Not being greedy because document is bigger %@ than minimum size (%@) to generate a thumbnail%@", (uint8_t *)&v14, 0x20u);
  }
LABEL_7:
  LOBYTE(v9) = 0;
LABEL_13:
      ...
      if (!v10)
      {
        ...
        if (os_log_type_enabled(v12, OS_LOG_TYPE_FAULT))
        {
          ...
          *(_DWORD *)buf = 138412802;
          ...
          __int16 v19 = 2112;
          uint64_t v20 = v11;
          _os_log_fault_impl(&dword_1D353B000, v12, OS_LOG_TYPE_FAULT, "[CRIT] UNREACHABLE: Failed to load xattr with signature %@ - %@%@", buf, 0x20u);
        }
...
- (void)setLastEditorDeviceRowID:.cold.1()
{
  ...
  OUTLINED_FUNCTION_2(&dword_1D353B000, v0, v1, "[CRIT] Assertion failed: lastEditorDeviceRowID && lastEditorDeviceRowID.longLongValue > 0%@", v2, v3, v4, v5, v6);
}

- (void)setLastEditorDeviceRowID:.cold.2()
{
  OUTLINED_FUNCTION_3_0();
  OUTLINED_FUNCTION_2(&dword_1D353B000, v0, v1, "[CRIT] Assertion failed: ![self _hasLastEditorRowID]%@", v2, v3, v4, v5, v6);
}

- (void)setLastEditorRowID:.cold.1()
{
  OUTLINED_FUNCTION_3_0();
  OUTLINED_FUNCTION_2(&dword_1D353B000, v0, v1, "[CRIT] Assertion failed: !_lastEditorDeviceOrUserRowID || ![self _hasLastEditorDeviceRowID]%@", v2, v3, v4, v5, v6);
}

- (void)isPackage
{
  OUTLINED_FUNCTION_3_0();
  OUTLINED_FUNCTION_2(&dword_1D353B000, v0, v1, "[CRIT] Assertion failed: _contentSignature%@", v2, v3, v4, v5, v6);
}
...
- (void)isSmallAndMostRecentClientsGenerateThumbnails
{
  ...
  OUTLINED_FUNCTION_3_0();
  _os_log_debug_impl(&dword_1D353B000, v0, OS_LOG_TYPE_DEBUG, "[DEBUG] Not being greedy because current version size is 0%@", v1, 0xCu);
}
