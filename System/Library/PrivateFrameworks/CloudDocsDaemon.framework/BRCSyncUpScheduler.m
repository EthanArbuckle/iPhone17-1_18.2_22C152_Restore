...
- (id)descriptionForItem:(id)a3 context:(id)a4
{
  v6 = (void *)MEMORY[0x1E4FBA908];
  id v7 = a4;
  id v8 = a3;
  v9 = objc_msgSend(v6, "rawInjection:length:", "in_flight_diffs, zone_rowid", 27);
  v12.receiver = self;
  v12.super_class = (Class)BRCSyncUpScheduler;
  v10 = [(BRCFSSchedulerBase *)&v12 descriptionForJobsMatching:v8 ordering:0 additionalColumns:v9 additionalValuesHandler:&__block_literal_global_41 context:v7];
...
- (void)createSyncUpJobForItem:(id)a3 inZone:(id)a4
{
  id v6 = a3;
  id v7 = a4;
  if ((objc_msgSend(v7, "br_isEqualToNumber:", &unk_1F2B270D8) & 1) == 0)
  {
    id v8 = [(BRCAccountSession *)self->super._session serverZoneByRowID:v7];
    v9 = [v8 clientZone];
...
- (void)createSyncUpJobForRowID:(unint64_t)a3 inZone:(id)a4
{
  ...
  id v7 = [(BRCAccountSession *)self->super._session syncContextProvider];
  id v8 = [v7 defaultSyncContext];
  v9 = [v8 uploadThrottle];
  id v10 = [MEMORY[0x1E4FBA8F8] formatInjection:@"in_flight_diffs = NULL"];
...
- (void)prepareItemForSyncUp:(id)a3 inFlightDiffs:(unint64_t)a4 inZone:(id)a5
{
  ...
  id v10 = a3;
  uint64_t v11 = [(BRCAccountSession *)session clientDB];
...
- (unint64_t)finishSyncUpForItem:(id)a3 inZone:(id)a4 success:(BOOL)a5
{
  ...
  id v10 = [(BRCAccountSession *)self->super._session clientDB];
  ...
  if (os_log_type_enabled(v13, OS_LOG_TYPE_DEBUG))
  {
    ...
    *(_DWORD *)buf = 134218754;
    ...
    __int16 v39 = 2112;
    v40 = v12;
    _os_log_debug_impl(&dword_1D353B000, v13, OS_LOG_TYPE_DEBUG, "[DEBUG] ┏%llx %@[%@]: finishing sync up%@", buf, 0x2Au);
  }
...
{
  id v8 = a3;
  int v4 = objc_msgSend(v8, "br_isEqualToNumber:", &unk_1F2B270D8);
  session = self->super._session;
  if (v4)
  {
    uint64_t v6 = [(BRCAccountSession *)session containerScheduler];
    [v6 scheduleSyncUpForSideCar];
  }
  else
  {
    uint64_t v6 = [(BRCAccountSession *)session serverZoneByRowID:v8];
    id v7 = [v6 clientZone];
    [v7 scheduleSyncUp];
  }
}
...
- (unint64_t)recoverAndReportMissingJobs
{
  ...
  v3 = [(BRCAccountSession *)self->super._session clientDB];
  ...
  if (v6)
  {
    ...
    do
    {
      ...
      for (uint64_t i = 0; i != v7; ++i)
      {
        ...
        {
          [(BRCSyncUpScheduler *)self createSyncUpJobForItem:v11 inZone:&unk_1F2B270D8];
        }
  ...
  long long v19 = [(BRCAccountSession *)self->super._session clientDB];
  ...
  v23 = [(BRCAccountSession *)self->super._session analyticsReporter];
...
- (unint64_t)postponeSyncUpCallback:(id)a3 inZone:(id)a4
{
  ...
  if (v10 <= 0)
  {
    id v12 = [(BRCAccountSession *)session syncContextProvider];
    [v12 defaultSyncContext];
  }
  else
  {
    id v12 = [(BRCAccountSession *)session serverZoneByRowID:v8];
...
- (void)createSyncUpJobForItem:inZone:.cold.1()
{
  ...
  _os_log_debug_impl(&dword_1D353B000, v0, OS_LOG_TYPE_DEBUG, "[DEBUG] Clearing app sync blocked bit because we created an item that needs sync%@", v1, 0xCu);
}
...
- (void)finishSyncUpForItem:inZone:success:.cold.1()
{
  uint64_t v2 = *MEMORY[0x1E4F143B8];
  OUTLINED_FUNCTION_3_0();
  _os_log_fault_impl(&dword_1D353B000, v0, OS_LOG_TYPE_FAULT, "[CRIT] Assertion failed: zoneRowID%@", v1, 0xCu);
}
...
- (void)finishSyncUpForItem:inZone:success:.cold.2()
{
  uint64_t v2 = *MEMORY[0x1E4F143B8];
  OUTLINED_FUNCTION_3_0();
  _os_log_fault_impl(&dword_1D353B000, v0, OS_LOG_TYPE_FAULT, "[CRIT] Assertion failed: _session.clientDB.isBatchSuspended%@", v1, 0xCu);
}
...
- (void)_scheduleSyncUpJob:.cold.1()
{
  ...
  __int16 v3 = 2112;
  uint64_t v4 = v0;
  _os_log_debug_impl(&dword_1D353B000, v1, OS_LOG_TYPE_DEBUG, "[DEBUG] Sync-Up[%@]: Scheduling a sync-up job%@", v2, 0x16u);
}
...
- (void)recoverAndReportMissingJobs
{
  ...
  __int16 v4 = 2112;
  uint64_t v5 = a1;
  _os_log_debug_impl(&dword_1D353B000, log, OS_LOG_TYPE_DEBUG, "[DEBUG] Sync-Up: Recovered from %u missing sync up jobs%@", (uint8_t *)v3, 0x12u);
}
