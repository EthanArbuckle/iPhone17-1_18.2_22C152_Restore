@interface SetVolumeLevelIntent
+ (BOOL)shouldSkipDefaultSchemaCheck;
- (SetVolumeLevelIntent)init;
- (SetVolumeLevelIntent)initWithCoder:(id)a3;
- (SetVolumeLevelIntent)initWithDomain:(id)a3 verb:(id)a4 parametersByName:(id)a5;
- (SetVolumeLevelIntent)initWithIdentifier:(id)a3 backingStore:(id)a4;
@end

@implementation SetVolumeLevelIntent

- (SetVolumeLevelIntent)init
{
  return (SetVolumeLevelIntent *)SetVolumeLevelIntent.init()();
}

+ (BOOL)shouldSkipDefaultSchemaCheck
{
  return 1;
}

- (SetVolumeLevelIntent)initWithIdentifier:(id)a3 backingStore:(id)a4
{
  if (a3)
  {
    uint64_t v5 = sub_232A68588();
    uint64_t v7 = v6;
  }
  else
  {
    uint64_t v5 = 0;
    uint64_t v7 = 0;
  }
  id v8 = a4;
  return (SetVolumeLevelIntent *)SetVolumeLevelIntent.init(identifier:backingStore:)(v5, v7, a4);
}

- (SetVolumeLevelIntent)initWithCoder:(id)a3
{
  return (SetVolumeLevelIntent *)SetVolumeLevelIntent.init(coder:)(a3);
}

- (SetVolumeLevelIntent)initWithDomain:(id)a3 verb:(id)a4 parametersByName:(id)a5
{
  if (a5) {
    sub_232A684F8();
  }
  return (SetVolumeLevelIntent *)SetVolumeLevelIntent.init(domain:verb:parametersByName:)();
}

@end