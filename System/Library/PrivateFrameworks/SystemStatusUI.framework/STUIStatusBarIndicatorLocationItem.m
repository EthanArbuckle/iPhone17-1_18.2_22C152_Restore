@interface STUIStatusBarIndicatorLocationItem
+ (STUIStatusBarIdentifier)prominentDisplayIdentifier;
+ (id)groupWithPriority:(int64_t)a3;
- (BOOL)canEnableDisplayItem:(id)a3 fromData:(id)a4;
- (STUIStatusBarImageView)prominentImageView;
- (STUIStatusBarIndicatorLocationItem)initWithIdentifier:(id)a3 statusBar:(id)a4;
- (id)applyUpdate:(id)a3 toDisplayItem:(id)a4;
- (id)createDisplayItemForIdentifier:(id)a3;
- (id)indicatorEntryKey;
- (id)overriddenStyleAttributesForData:(id)a3 identifier:(id)a4;
- (id)systemImageNameForUpdate:(id)a3;
- (id)viewForIdentifier:(id)a3;
- (int64_t)previousType;
- (void)_create_prominentImageView;
- (void)setPreviousType:(int64_t)a3;
- (void)setProminentImageView:(id)a3;
@end

@implementation STUIStatusBarIndicatorLocationItem

- (BOOL)canEnableDisplayItem:(id)a3 fromData:(id)a4
{
  id v6 = a3;
  id v7 = a4;
  v8 = [v6 identifier];
  v9 = [(id)objc_opt_class() prominentDisplayIdentifier];

  if (v8 == v9)
  {
    v11 = [v7 locationEntry];
    if ([v11 isEnabled])
    {
      v12 = [v7 locationEntry];
      unsigned __int8 v10 = [v12 type] == 2;
    }
    else
    {
      unsigned __int8 v10 = 0;
    }
  }
  else
  {
    v14.receiver = self;
    v14.super_class = (Class)STUIStatusBarIndicatorLocationItem;
    unsigned __int8 v10 = [(STUIStatusBarItem *)&v14 canEnableDisplayItem:v6 fromData:v7];
  }

  return v10;
}

- (id)indicatorEntryKey
{
  return (id)*MEMORY[0x1E4FA9780];
}

- (id)createDisplayItemForIdentifier:(id)a3
{
  v12.receiver = self;
  v12.super_class = (Class)STUIStatusBarIndicatorLocationItem;
  id v4 = a3;
  v5 = [(STUIStatusBarItem *)&v12 createDisplayItemForIdentifier:v4];
  objc_msgSend((id)objc_opt_class(), "prominentDisplayIdentifier", v12.receiver, v12.super_class);
  id v6 = (id)objc_claimAutoreleasedReturnValue();

  if (v6 == v4)
  {
    id v7 = [MEMORY[0x1E4FB1818] _systemImageNamed:@"location.circle.fill"];
    v8 = [v7 imageWithRenderingMode:1];

    [v8 alignmentRectInsets];
    UIEdgeInsetsAdd();
    v9 = objc_msgSend(v8, "imageWithAlignmentRectInsets:");

    unsigned __int8 v10 = [(STUIStatusBarIndicatorLocationItem *)self prominentImageView];
    [v10 setImage:v9];
  }
  return v5;
}

- (STUIStatusBarIndicatorLocationItem)initWithIdentifier:(id)a3 statusBar:(id)a4
{
  v5.receiver = self;
  v5.super_class = (Class)STUIStatusBarIndicatorLocationItem;
  result = [(STUIStatusBarItem *)&v5 initWithIdentifier:a3 statusBar:a4];
  if (result) {
    result->_previousType = -1;
  }
  return result;
}

- (id)overriddenStyleAttributesForData:(id)a3 identifier:(id)a4
{
  id v4 = a4;
  id v5 = [(id)objc_opt_class() prominentDisplayIdentifier];

  if (v5 == v4)
  {
    id v6 = +[STUIStatusBarStyleAttributes overriddenStyleAttributes];
    [v6 setSymbolScale:2];
  }
  else
  {
    id v6 = 0;
  }
  return v6;
}

- (id)applyUpdate:(id)a3 toDisplayItem:(id)a4
{
  id v6 = a3;
  id v7 = a4;
  v8 = [v7 identifier];
  v9 = [(id)objc_opt_class() defaultDisplayIdentifier];
  if (v8 != v9)
  {

LABEL_18:
    v18 = 0;
    goto LABEL_19;
  }
  int v10 = [v6 dataChanged];

  if (!v10) {
    goto LABEL_18;
  }
  v11 = [v6 data];
  objc_super v12 = [v11 locationEntry];
  int64_t v13 = [v12 type];

  int64_t previousType = self->_previousType;
  BOOL v15 = previousType == 2 && v13 == 0;
  if (v15
    || (!previousType ? (BOOL v16 = v13 == 2) : (BOOL v16 = 0),
        !v16 ? (char v17 = 0) : (char v17 = 1),
        v13 == previousType || previousType == -1 || (v17 & 1) != 0))
  {
    v20.receiver = self;
    v20.super_class = (Class)STUIStatusBarIndicatorLocationItem;
    v18 = [(STUIStatusBarIndicatorItem *)&v20 applyUpdate:v6 toDisplayItem:v7];
  }
  else
  {
    v21[0] = MEMORY[0x1E4F143A8];
    v21[1] = 3221225472;
    v21[2] = __64__STUIStatusBarIndicatorLocationItem_applyUpdate_toDisplayItem___block_invoke;
    v21[3] = &unk_1E6AA4048;
    id v22 = v6;
    v23 = self;
    v18 = +[STUIStatusBarAnimation animationWithBlock:v21];
  }
  self->_int64_t previousType = v13;
LABEL_19:

  return v18;
}

+ (STUIStatusBarIdentifier)prominentDisplayIdentifier
{
  return (STUIStatusBarIdentifier *)[a1 displayItemIdentifierFromString:@"prominentDisplayIdentifier"];
}

- (id)viewForIdentifier:(id)a3
{
  id v4 = a3;
  id v5 = [(id)objc_opt_class() prominentDisplayIdentifier];

  if (v5 == v4)
  {
    uint64_t v6 = [(STUIStatusBarIndicatorLocationItem *)self prominentImageView];
  }
  else
  {
    v9.receiver = self;
    v9.super_class = (Class)STUIStatusBarIndicatorLocationItem;
    uint64_t v6 = [(STUIStatusBarIndicatorItem *)&v9 viewForIdentifier:v4];
  }
  id v7 = (void *)v6;

  return v7;
}

- (STUIStatusBarImageView)prominentImageView
{
  prominentImageView = self->_prominentImageView;
  if (!prominentImageView)
  {
    [(STUIStatusBarIndicatorLocationItem *)self _create_prominentImageView];
    prominentImageView = self->_prominentImageView;
  }
  return prominentImageView;
}

- (void)_create_prominentImageView
{
  v3 = [STUIStatusBarImageView alloc];
  id v4 = -[STUIStatusBarImageView initWithFrame:](v3, "initWithFrame:", *MEMORY[0x1E4F1DB28], *(double *)(MEMORY[0x1E4F1DB28] + 8), *(double *)(MEMORY[0x1E4F1DB28] + 16), *(double *)(MEMORY[0x1E4F1DB28] + 24));
  prominentImageView = self->_prominentImageView;
  self->_prominentImageView = v4;
  MEMORY[0x1F41817F8](v4, prominentImageView);
}

+ (id)groupWithPriority:(int64_t)a3
{
  v15[1] = *MEMORY[0x1E4F143B8];
  id v5 = [a1 defaultDisplayIdentifier];
  uint64_t v6 = +[STUIStatusBarDisplayItemPlacement placementWithIdentifier:v5 priority:1];

  id v7 = [a1 prominentDisplayIdentifier];
  v8 = +[STUIStatusBarDisplayItemPlacement placementWithIdentifier:v7 priority:2];
  v15[0] = v6;
  objc_super v9 = [MEMORY[0x1E4F1C978] arrayWithObjects:v15 count:1];
  int v10 = [v8 excludingPlacements:v9];

  v14[0] = v6;
  v14[1] = v10;
  v11 = [MEMORY[0x1E4F1C978] arrayWithObjects:v14 count:2];
  objc_super v12 = +[STUIStatusBarDisplayItemPlacementGroup groupWithPriority:a3 placements:v11];

  [v12 setRegularPlacement:v6];
  [v12 setProminentPlacement:v10];

  return v12;
}

- (void).cxx_destruct
{
}

- (id)systemImageNameForUpdate:(id)a3
{
  v3 = [a3 data];
  id v4 = [v3 locationEntry];
  uint64_t v5 = [v4 type];

  if (v5 == 1) {
    return @"location";
  }
  else {
    return @"location.fill";
  }
}

void __64__STUIStatusBarIndicatorLocationItem_applyUpdate_toDisplayItem___block_invoke(uint64_t a1, uint64_t a2, void *a3, void *a4)
{
  id v6 = a3;
  id v7 = a4;
  v8 = (void *)MEMORY[0x1E4FB1EB0];
  v16[0] = MEMORY[0x1E4F143A8];
  v16[1] = 3221225472;
  v16[2] = __64__STUIStatusBarIndicatorLocationItem_applyUpdate_toDisplayItem___block_invoke_2;
  v16[3] = &unk_1E6AA37A0;
  id v17 = v6;
  v11[0] = MEMORY[0x1E4F143A8];
  v11[1] = 3221225472;
  v11[2] = __64__STUIStatusBarIndicatorLocationItem_applyUpdate_toDisplayItem___block_invoke_3;
  v11[3] = &unk_1E6AA4688;
  id v12 = *(id *)(a1 + 32);
  id v13 = v17;
  uint64_t v14 = *(void *)(a1 + 40);
  id v15 = v7;
  id v9 = v7;
  id v10 = v17;
  [v8 animateWithDuration:v16 animations:v11 completion:0.166666667];
}

uint64_t __64__STUIStatusBarIndicatorLocationItem_applyUpdate_toDisplayItem___block_invoke_2(uint64_t a1)
{
  CGAffineTransformMakeScale(&v5, 0.66, 0.66);
  v2 = *(void **)(a1 + 32);
  CGAffineTransform v4 = v5;
  [v2 setTransform:&v4];
  return [*(id *)(a1 + 32) setAlpha:0.25];
}

void __64__STUIStatusBarIndicatorLocationItem_applyUpdate_toDisplayItem___block_invoke_3(uint64_t a1, int a2)
{
  if (a2)
  {
    uint64_t v3 = *(void *)(a1 + 40);
    uint64_t v4 = *(void *)(a1 + 32);
    v10.receiver = *(id *)(a1 + 48);
    v10.super_class = (Class)STUIStatusBarIndicatorLocationItem;
    id v5 = objc_msgSendSuper2(&v10, sel_applyUpdate_toDisplayItem_, v4, v3);
    id v6 = (void *)MEMORY[0x1E4FB1EB0];
    v8[0] = MEMORY[0x1E4F143A8];
    v8[1] = 3221225472;
    v8[2] = __64__STUIStatusBarIndicatorLocationItem_applyUpdate_toDisplayItem___block_invoke_4;
    v8[3] = &unk_1E6AA37A0;
    id v9 = *(id *)(a1 + 40);
    [v6 animateWithDuration:v8 animations:*(void *)(a1 + 56) completion:0.166666667];
  }
  else
  {
    id v7 = *(void (**)(void))(*(void *)(a1 + 56) + 16);
    v7();
  }
}

uint64_t __64__STUIStatusBarIndicatorLocationItem_applyUpdate_toDisplayItem___block_invoke_4(uint64_t a1)
{
  v2 = *(void **)(a1 + 32);
  long long v3 = *(_OWORD *)(MEMORY[0x1E4F1DAB8] + 16);
  v5[0] = *MEMORY[0x1E4F1DAB8];
  v5[1] = v3;
  v5[2] = *(_OWORD *)(MEMORY[0x1E4F1DAB8] + 32);
  [v2 setTransform:v5];
  return [*(id *)(a1 + 32) setAlpha:1.0];
}

- (void)setProminentImageView:(id)a3
{
}

- (int64_t)previousType
{
  return self->_previousType;
}

- (void)setPreviousType:(int64_t)a3
{
  self->_int64_t previousType = a3;
}

@end