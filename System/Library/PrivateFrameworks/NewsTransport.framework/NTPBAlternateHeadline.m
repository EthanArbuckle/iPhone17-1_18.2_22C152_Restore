...
@implementation NTPBAlternateHeadline
...
- (id)description
{
  char v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)NTPBAlternateHeadline;
  v4 = [(NTPBAlternateHeadline *)&v8 description];
  v5 = [(NTPBAlternateHeadline *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if ((has & 2) != 0)
  {
    v5 = [NSNumber numberWithInt:self->_articleHeadlineTreatmentState];
    [v3 setObject:v5 forKey:@"article_headline_treatment_state"];

    char has = (char)self->_has;
  }
  if (has)
  {
    v6 = [NSNumber numberWithInt:self->_articleHeadlineTreatmentId];
    [v3 setObject:v6 forKey:@"article_headline_treatment_id"];
  }
...
- (BOOL)readFrom:(id)a3
{
  return NTPBAlternateHeadlineReadFrom((uint64_t)self, (uint64_t)a3);
}
...
- (void)mergeFrom:(id)a3
{
  ...
  if ((v5 & 2) != 0)
  {
    self->_articleHeadlineTreatmentState = *((_DWORD *)v4 + 3);
    *(unsigned char *)&self->_has |= 2u;
    char v5 = *((unsigned char *)v4 + 16);
  }
  if (v5)
