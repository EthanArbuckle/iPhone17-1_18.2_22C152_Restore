@interface CCAppCustomNotebookItemGroupNameContent
+ (Class)contentMessageClass;
+ (Class)metaContentMessageClass;
+ (id)descriptionForTypeIdentifier:(unsigned __int16)a3;
+ (unsigned)itemType;
- (BOOL)initializeFieldValuesFromData:(id)a3 error:(id *)a4;
- (CCAppCustomNotebookItemGroupNameContent)initWithJSONDictionary:(id)a3 error:(id *)a4;
- (CCAppCustomNotebookItemGroupNameContent)initWithVocabularyString:(id)a3 vocabularyIdentifier:(id)a4 error:(id *)a5;
...
@implementation CCAppCustomNotebookItemGroupNameContent
...
- (CCAppCustomNotebookItemGroupNameContent)initWithJSONDictionary:(id)a3 error:(id *)a4
{
  ...
  if (IsInstanceOfExpectedClass)
  {
    ...
    v11 = [[CCAppCustomNotebookItemGroupNameContent alloc] initWithVocabularyString:v9 vocabularyIdentifier:v10 error:a4];
  }
  else
  {
    CCSetError();
...
- (id)jsonDictionary
{
  ...
  {
    v4 = [(CCAppCustomNotebookItemGroupNameContent *)self vocabularyString];
    [v3 setObject:v4 forKeyedSubscript:@"vocabularyString"];
  }
  if (self->_vocabularyIdentifier)
  {
    v5 = [(CCAppCustomNotebookItemGroupNameContent *)self vocabularyIdentifier];
...
- (void)enumerateFieldsUsingBlock:(id)a3 parentFieldType:(unsigned __int16)a4
{
  ...
  if (self->_vocabularyString)
  {
    v5 = (void *)[objc_alloc(MEMORY[0x263F31708]) initWithFieldType:5598 stringValue:self->_vocabularyString];
    v7[2](v7, v5);
  }
  if (self->_vocabularyIdentifier)
  {
    id v6 = (void *)[objc_alloc(MEMORY[0x263F31708]) initWithFieldType:5599 stringValue:self->_vocabularyIdentifier];
    v7[2](v7, v6);
  }
}
...
+ (id)descriptionForTypeIdentifier:(unsigned __int16)a3
{
  if ((a3 - 5595) > 4) {
    ...
    return *(&off_26539BC40 + (unsigned __int16)(a3 - 5595));
  }
}
