@interface CCAppCustomContactGroupNameContent
+ (Class)contentMessageClass;
+ (Class)metaContentMessageClass;
+ (id)descriptionForTypeIdentifier:(unsigned __int16)a3;
+ (unsigned)itemType;
- (BOOL)initializeFieldValuesFromData:(id)a3 error:(id *)a4;
- (CCAppCustomContactGroupNameContent)initWithJSONDictionary:(id)a3 error:(id *)a4;
- (CCAppCustomContactGroupNameContent)initWithVocabularyString:(id)a3 vocabularyIdentifier:(id)a4 error:(id *)a5;
...
@implementation CCAppCustomContactGroupNameContent
...
- (CCAppCustomContactGroupNameContent)initWithJSONDictionary:(id)a3 error:(id *)a4
{
  ...
  if (IsInstanceOfExpectedClass)
  {
    ...
    v11 = [[CCAppCustomContactGroupNameContent alloc] initWithVocabularyString:v9 vocabularyIdentifier:v10 error:a4];
  }
  else
  {
    CCSetError();
...
- (id)jsonDictionary
{
  ...
  {
    v4 = [(CCAppCustomContactGroupNameContent *)self vocabularyString];
    [v3 setObject:v4 forKeyedSubscript:@"vocabularyString"];
  }
  if (self->_vocabularyIdentifier)
  {
    v5 = [(CCAppCustomContactGroupNameContent *)self vocabularyIdentifier];
...
- (void)enumerateFieldsUsingBlock:(id)a3 parentFieldType:(unsigned __int16)a4
{
  ...
  if (self->_vocabularyString)
  {
    v5 = (void *)[objc_alloc(MEMORY[0x263F31708]) initWithFieldType:63372 stringValue:self->_vocabularyString];
    v7[2](v7, v5);
  }
  if (self->_vocabularyIdentifier)
  {
    id v6 = (void *)[objc_alloc(MEMORY[0x263F31708]) initWithFieldType:63373 stringValue:self->_vocabularyIdentifier];
    v7[2](v7, v6);
  }
}
...
+ (id)descriptionForTypeIdentifier:(unsigned __int16)a3
{
  if ((unsigned __int16)(a3 + 2167) > 4u) {
    return 0;
  }
  else {
    return *(&off_26539BB28 + (unsigned __int16)(a3 + 2167));
  }
}
