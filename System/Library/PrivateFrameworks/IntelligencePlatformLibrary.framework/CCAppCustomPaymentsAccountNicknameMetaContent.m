@interface CCAppCustomPaymentsAccountNicknameMetaContent
+ (Class)contentMessageClass;
+ (Class)metaContentMessageClass;
+ (id)descriptionForTypeIdentifier:(unsigned __int16)a3;
+ (unsigned)itemType;
- (BOOL)initializeFieldValuesFromData:(id)a3 error:(id *)a4;
- (CCAppCustomPaymentsAccountNicknameMetaContent)initWithJSONDictionary:(id)a3 error:(id *)a4;
- (CCAppCustomPaymentsAccountNicknameMetaContent)initWithSourceItemIdentifier:(id)a3 error:(id *)a4;
- (NSString)sourceItemIdentifier;
- (id)jsonDictionary;
- (void)enumerateFieldsUsingBlock:(id)a3 parentFieldType:(unsigned __int16)a4;
@end

@implementation CCAppCustomPaymentsAccountNicknameMetaContent
...
- (CCAppCustomPaymentsAccountNicknameMetaContent)initWithJSONDictionary:(id)a3 error:(id *)a4
{
  ...
  if (IsInstanceOfExpectedClass)
  {
    ...
    v10 = [[CCAppCustomPaymentsAccountNicknameMetaContent alloc] initWithSourceItemIdentifier:v9 error:a4];
  }
  else
  {
    CCSetError();
...
- (id)jsonDictionary
{
  ...
  {
    v4 = [(CCAppCustomPaymentsAccountNicknameMetaContent *)self sourceItemIdentifier];
...
- (void)enumerateFieldsUsingBlock:(id)a3 parentFieldType:(unsigned __int16)a4
{
  if (self->_sourceItemIdentifier)
  {
    ...
    id v7 = a3;
    id v8 = (id)[[v6 alloc] initWithFieldType:25888 stringValue:self->_sourceItemIdentifier];
