@interface WFUserDefaultsPersistence
- (BOOL)BOOLForKey:(id)a3;
- (BOOL)synchronize;
- (NSUserDefaults)userDefaults;
- (WFUserDefaultsPersistence)init;
- (WFUserDefaultsPersistence)initWithUserDefaults:(id)a3;
- (id)arrayForKey:(id)a3;
- (id)dictionaryForKey:(id)a3;
- (id)objectForKey:(id)a3;
- (id)stringForKey:(id)a3;
- (void)setBool:(BOOL)a3 forKey:(id)a4;
- (void)setObject:(id)a3 forKey:(id)a4;
- (void)setUserDefaults:(id)a3;
@end

@implementation WFUserDefaultsPersistence

- (WFUserDefaultsPersistence)initWithUserDefaults:(id)a3
{
  id v4 = a3;
  v8.receiver = self;
  v8.super_class = (Class)WFUserDefaultsPersistence;
  v5 = [(WFUserDefaultsPersistence *)&v8 init];
  v6 = v5;
  if (v5) {
    [(WFUserDefaultsPersistence *)v5 setUserDefaults:v4];
  }

  return v6;
}

- (WFUserDefaultsPersistence)init
{
  v3 = (void *)[objc_alloc(MEMORY[0x263EFFA40]) initWithSuiteName:@"group.com.apple.weather"];
  id v4 = [(WFUserDefaultsPersistence *)self initWithUserDefaults:v3];

  return v4;
}

- (id)arrayForKey:(id)a3
{
  id v4 = a3;
  v5 = [(WFUserDefaultsPersistence *)self userDefaults];
  v6 = [v5 arrayForKey:v4];

  return v6;
}

- (id)dictionaryForKey:(id)a3
{
  id v4 = a3;
  v5 = [(WFUserDefaultsPersistence *)self userDefaults];
  v6 = [v5 dictionaryForKey:v4];

  return v6;
}

- (id)stringForKey:(id)a3
{
  id v4 = a3;
  v5 = [(WFUserDefaultsPersistence *)self userDefaults];
  v6 = [v5 stringForKey:v4];

  return v6;
}

- (id)objectForKey:(id)a3
{
  id v4 = a3;
  v5 = [(WFUserDefaultsPersistence *)self userDefaults];
  v6 = [v5 objectForKey:v4];

  return v6;
}

- (void)setObject:(id)a3 forKey:(id)a4
{
  id v6 = a4;
  id v7 = a3;
  id v8 = [(WFUserDefaultsPersistence *)self userDefaults];
  [v8 setObject:v7 forKey:v6];
}

- (BOOL)BOOLForKey:(id)a3
{
  id v4 = a3;
  v5 = [(WFUserDefaultsPersistence *)self userDefaults];
  char v6 = [v5 BOOLForKey:v4];

  return v6;
}

- (void)setBool:(BOOL)a3 forKey:(id)a4
{
  BOOL v4 = a3;
  id v6 = a4;
  id v7 = [(WFUserDefaultsPersistence *)self userDefaults];
  [v7 setBool:v4 forKey:v6];
}

- (BOOL)synchronize
{
  v2 = [(WFUserDefaultsPersistence *)self userDefaults];
  char v3 = [v2 synchronize];

  return v3;
}

- (NSUserDefaults)userDefaults
{
  return (NSUserDefaults *)objc_getProperty(self, a2, 8, 1);
}

- (void)setUserDefaults:(id)a3
{
}

- (void).cxx_destruct
{
}

@end