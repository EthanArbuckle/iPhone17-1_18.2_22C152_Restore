...
- (BOOL)purgeableStateValidForRendering
{
  return !self->_purgeableStateHasBeenSet
      || [(MTLDebugVisibleFunctionTable *)self setPurgeableState:1] == 2;
}
...
- (void)setFunction:(id)a3 atIndex:(unint64_t)a4
{
  ...
  if (objc_msgSend((id)-[MTLDevice baseObject](-[MTLToolsObject device](self, "device", v12, functionCount), "baseObject"), "supportsFunctionPointers"))
  {
    if (!a3) {
      goto LABEL_16;
    }
  ...
  {
    objc_super v13 = MTLDebugStageToString([a3 stage]);
    v15 = MTLDebugStageToString(self->_stage);
    MTLReportFailure();
  }
  objc_opt_class();
  ...
  -[MTLToolsObject validateHandleForSetFunction:](pipelineState, "validateHandleForSetFunction:", a3, v13, v15);
LABEL_16:
...
- (void)setFunctions:(const void *)a3 withRange:(_NSRange)a4
{
  ...
  if (objc_msgSend((id)-[MTLDevice baseObject](-[MTLToolsObject device](self, "device", v9, functionCount), "baseObject"), "supportsFunctionPointers"))
  {
    if (!length) {
      return;
    }
...
- (id)description
{
  return [(MTLDebugVisibleFunctionTable *)self formattedDescription:0];
}
