...
LABEL_13:
    ...
    if (os_log_type_enabled((os_log_t)WiProxLog, OS_LOG_TYPE_DEBUG))
    {
      *(_DWORD *)buf = 134218752;
      ...
      _os_log_debug_impl(&dword_2144AA000, v18, OS_LOG_TYPE_DEBUG, "ObjectDiscovery length payload:%ld state:%d data:%ld optional:%ld", buf, 0x26u);
    }
...
+ (id)addressesFromNearbyTokens:(id)a3
{
  ...
  if (v3)
  {
    unsigned int v4 = (void *)MEMORY[0x2166A2110]();
...
{
  *(_WORD *)v1 = 0;
  _os_log_debug_impl(&dword_2144AA000, log, OS_LOG_TYPE_DEBUG, "ObjectDiscovery found NearOwner payload", v1, 2u);
}

+ (void)objectDiscoveryReportFromAdvReport:(os_log_t)log .cold.5(os_log_t log)
{
  *(_WORD *)v1 = 0;
  _os_log_debug_impl(&dword_2144AA000, log, OS_LOG_TYPE_DEBUG, "ObjectDiscovery found Wild payload", v1, 2u);
}

+ (void)objectDiscoveryReportFromAdvReport:(os_log_t)log .cold.6(os_log_t log)
{
  *(_WORD *)v1 = 0;
  _os_log_error_impl(&dword_2144AA000, log, OS_LOG_TYPE_ERROR, "ObjectDiscovery found unexpected payload", v1, 2u);
}
...
+ (void)applyMaskToAddress:(unsigned char *)a1 .cold.1(unsigned char *a1, NSObject *a2)
{
  ...
  v3[0] = 67109120;
  v3[1] = v2;
  _os_log_debug_impl(&dword_2144AA000, a2, OS_LOG_TYPE_DEBUG, "ObjectDiscovery address is valid: %d", (uint8_t *)v3, 8u);
}
