...
- (SKUIApplicationLicenseViewController)initWithLicenseAgreementPage:(id)a3
{
  ...
  v9.receiver = self;
  v9.super_class = (Class)SKUIApplicationLicenseViewController;
  v5 = [(SKUIApplicationLicenseViewController *)&v9 init];
  if (v5)
  {
    uint64_t v6 = [v4 copy];
    licensePage = v5->_licensePage;
    v5->_licensePage = (SKUIApplicationLicensePage *)v6;
  }
  return v5;
}
...
- (void)reloadData
{
  if (!self->_licensePage && !self->_loadOperation)
  {
    if (self->_url)
    {
      v3 = (SSVLoadURLOperation *)[objc_alloc(MEMORY[0x1E4FA8338]) initWithURL:self->_url];
...
- (void)loadView
{
  ...
  if (!webView)
  {
    id v4 = (WKWebView *)objc_alloc_init(MEMORY[0x1E4F466F0]);
    id v5 = self->_webView;
    self->_webView = v4;
    ...
    id v8 = [(WKWebView *)self->_webView scrollView];
    ...
    if (licensePage)
    {
      id v11 = [(SKUIApplicationLicensePage *)licensePage licenseAgreementHTML];
...
- (void)_displayPage:(id)a3 error:(id)a4
{
  ...
  else
  {
    ...
    if (webView)
    {
      id v11 = [(SKUIApplicationLicensePage *)*p_licensePage licenseAgreementHTML];
      id v12 = [(WKWebView *)webView loadHTMLString:v11 baseURL:0];
    }
    uint64_t v13 = [(SKUIApplicationLicensePage *)*p_licensePage title];
    [(SKUIApplicationLicenseViewController *)self setTitle:v13];
  }
}
