...
- (SKUISearchFieldController)initWithContentsController:(id)a3 clientContext:(id)a4
{
  ...
  if (v7)
  {
    ...
    v12 = -[SKUISearchFieldTableView initWithFrame:style:](v11, "initWithFrame:style:", 0, *MEMORY[0x1E4F1DB28], *(double *)(MEMORY[0x1E4F1DB28] + 8), *(double *)(MEMORY[0x1E4F1DB28] + 16), *(double *)(MEMORY[0x1E4F1DB28] + 24));
    ...
    BOOL v14 = [(SKUISearchFieldController *)v7 _presentsInPopover:v6];
    uint64_t v15 = [objc_alloc(MEMORY[0x1E4FB1D18]) initWithStyle:0];
    ...
    v20 = [MEMORY[0x1E4FB1618] _systemBackgroundColor];
    }
    [v19 setBackgroundColor:v20];

    if (v14)
    {
      v21 = (void *)[objc_alloc(MEMORY[0x1E4FB19E8]) initWithRootViewController:v7->_searchResultsController];
    ...
    {
      v31 = [v26 searchField];
      v32 = [MEMORY[0x1E4FB1618] colorWithWhite:1.0 alpha:0.6];
...
- (BOOL)displaysSearchBarInNavigationBar
{
  return [(SKUISearchController *)self->_searchController hidesNavigationBarDuringPresentation] ^ 1;
}
...
- (BOOL)canBecomeActive
{
  ...
  id v4 = [(SKUISearchController *)self->_searchController searchBar];
  v5 = [v4 superview];
  BOOL v3 = v5 != 0;
...
- (void)becomeActive
{
  BOOL v3 = [(SKUISearchController *)self->_searchController transitionCoordinator];
  id v4 = v3;
  if (v3)
  {
...
- (UISearchBar)searchBar
{
  return (UISearchBar *)[(SKUISearchController *)self->_searchController searchBar];
}
...
- (void)setNumberOfSearchResults:(int64_t)a3
{
  if (self->_numberOfSearchResults != a3)
  {
    ...
    if (a3)
    {
      ...
      if (clientContext) {
        [(SKUIClientContext *)clientContext localizedStringForKey:v5];
      }
      else {
      id v6 = +[SKUIClientContext localizedStringForKey:v5 inBundles:0];
      }
      v7 = NSString;
...
- (void)setClientContext:(id)a3
{
  ...
  v7 = +[SKUIClientContext localizedStringForKey:@"SEARCH_SUGGESTIONS" inBundles:0];
  }
...
- (void)setSearchTerm:(id)a3
{
  searchController = self->_searchController;
  id v5 = a3;
  id v6 = [(SKUISearchController *)searchController searchBar];
  v7 = [v6 searchField];
  [v7 setText:v5];
...
- (BOOL)showsResultsForEmptyField
{
  return [(SKUISearchController *)self->_searchController _showResultsForEmptySearch];
}

- (void)searchBarSearchButtonClicked:(id)a3
{
  id v4 = a3;
  char v6 = objc_alloc_init(SKUISearchRequest);
  id v5 = [v4 text];
...
- (BOOL)searchBarShouldClear:(id)a3
{
  ...
  if (([v4 isFirstResponder] & 1) != 0
    || [(SKUISearchFieldController *)self _presentsInPopover:self->_clientContext])
  {
    BOOL v5 = 1;
  }
  else
...
- (void)presentSearchController:(id)a3
{
  ...
  id v6 = [(SKUISearchFieldController *)self contentsController];
  [v6 setDefinesPresentationContext:1];

  char v7 = [(SKUISearchFieldController *)self contentsController];
...
- (void)willDismissSearchController:(id)a3
{
  id v3 = [(SKUISearchFieldController *)self contentsController];
...
- (void)prepareForPopoverPresentation:(id)a3
{
  ...
  double v5 = [(SKUISearchController *)self->_searchController searchBar];
  [v4 setSourceView:v5];

  double v6 = [(SKUISearchController *)self->_searchController searchBar];
  [v6 bounds];
  objc_msgSend(v4, "setSourceRect:");

  double v7 = [(SKUISearchController *)self->_searchController searchBar];
  v9[0] = v7;
...
- (id)tableView:(id)a3 cellForRowAtIndexPath:(id)a4
{
  ...
  if (!v7) {
    double v7 = (void *)[objc_alloc(MEMORY[0x1E4FB1D08]) initWithStyle:0 reuseIdentifier:@"a"];
  }
  double v8 = [(SKUICompletionList *)self->_completionList completions];
  uint64_t v9 = [v6 row];
...
- (int64_t)tableView:(id)a3 numberOfRowsInSection:(int64_t)a4
{
  id v4 = [(SKUICompletionList *)self->_completionList completions];
  int64_t v5 = [v4 count];
...
- (void)tableView:(id)a3 didSelectRowAtIndexPath:(id)a4
{
  ...
  id v6 = [(SKUISearchController *)self->_searchController searchBar];
  double v7 = [v6 text];
  ...
  uint64_t v9 = [(SKUICompletionList *)self->_completionList completions];
  uint64_t v10 = [v5 row];
...
{
  id v5 = a4;
  double v12 = objc_alloc_init(SKUISearchRequest);
  ...
  uint64_t v10 = [(SKUISearchController *)self->_searchController searchBar];
...
- (id)URLForTrendingSearchPageView:(id)a3
{
  ...
  id v4 = [(SKUISearchFieldController *)self trendingSearchURLString];
...
{
  id v25 = a3;
  id v4 = [(SKUISearchFieldController *)self clientContext];
  if (SKUIUserInterfaceIdiom(v4) == 1)
  {
    ...
    {
      BOOL v8 = [(SKUIClientContext *)self->_clientContext shouldForceTransientSearchControllerBahavior];
...
{
  id v12 = a3;
  id v4 = [(SKUISearchFieldController *)self delegate];
  id v5 = v4;
...
- (void)_reloadData
{
  id v3 = [(SKUISearchController *)self->_searchController searchBar];
  id v4 = [v3 text];

  if ([v4 length])
  {
    id v5 = [(SKUISearchFieldController *)self searchHintsURLString];

    if (!v5)
    {
      objc_initWeak(&location, self);
      double v18 = [(SKUIClientContext *)self->_clientContext URLBag];
    ...
    uint64_t v7 = [(SKUISearchFieldController *)self searchHintsURLString];
    BOOL v8 = [v6 URLWithString:v7];
    double v9 = [v8 URLByAppendingQueryParameter:@"term" value:v4];

    char v10 = (SSVLoadURLOperation *)[objc_alloc(MEMORY[0x1E4FA8338]) initWithURL:v9];
    ...
    double v15 = [(SKUIClientContext *)self->_clientContext valueForConfigurationKey:@"sfsuffix"];
...
- (void)_reloadTrendingVisibility
{
  ...
  if ((v4 & 0xFFFFFFFFFFFFFFFBLL) == 1)
  {
    ...
    else
    {
      ...
      id v8 = [(SKUIClientContext *)self->_clientContext trendingSearchProvider];
      [v14 setTrendingSearchProvider:v8];

      unsigned int v9 = [(SKUISearchController *)self->_searchController isActive];
      id v10 = [(SKUISearchController *)self->_searchController searchBar];
      id v11 = [v10 text];
...
- (void)_setResponse:(id)a3 error:(id)a4
{
  ...
  if (v12)
  {
    id v8 = [(SKUISearchController *)self->_searchController searchBar];
...
- (SKUISearchFieldDelegate)delegate
{
  ...
  return (SKUISearchFieldDelegate *)WeakRetained;
}
...
- (void)initWithContentsController:clientContext:.cold.1()
{
  ...
  int v0 = 136446210;
  id v1 = "-[SKUISearchFieldController initWithContentsController:clientContext:]";
}
