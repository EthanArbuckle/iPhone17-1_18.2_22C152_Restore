...
- (Offset<siri::speech::schema_fb::StartMultilingualSpeechRequest>)addObjectToBuffer:(void *)a3
{
  ...
  uint64_t v5 = [(QSSStartMultilingualSpeechRequest *)self start_speech_request];
  unsigned int v38 = [v5 addObjectToBuffer:a3];

  char v6 = [(QSSStartMultilingualSpeechRequest *)self language_parameters_by_id];
  ...
  long long v40 = 0u;
  long long v41 = 0u;
  uint64_t v11 = [(QSSStartMultilingualSpeechRequest *)self language_parameters_by_id];
  ...
  if (v12)
  {
    ...
    do
    {
      for (uint64_t i = 0; i != v12; ++i)
      {
        ...
        int v16 = [*(id *)(*((void *)&v40 + 1) + 8 * i) addObjectToBuffer:a3];
        int v17 = v16;
        if (v14 >= v10)
        {
  ...
  *((unsigned char *)a3 + 70) = 1;
  ...
  if (v31)
  {
    int v35 = flatbuffers::FlatBufferBuilder::ReferTo((flatbuffers::FlatBufferBuilder *)a3, v31);
    ...
    if (*(unsigned __int16 *)v5->var0 >= 7u)
    {
      ...
      if (v6)
      {
        ...
        if (v9)
        {
          ...
          do
          {
            uint64_t v12 = [[QSSLanguageParameters alloc] initWithFlatbuffData:self->_data root:&v10[*(unsigned int *)v10->var0] verify:0];
            [v3 addObject:v12];
...
- (QSSStartMultilingualSpeechRequest)initWithFlatbuffData:(id)a3 root:(const StartMultilingualSpeechRequest *)a4 verify:(BOOL)a5
{
  ...
  uint64_t v10 = [(QSSStartMultilingualSpeechRequest *)&v30 init];
  uint64_t v11 = v10;
  if (v10)
  {
    if (!v9 || ![v9 length]) {
...
- (QSSStartMultilingualSpeechRequest)initWithFlatbuffData:(id)a3 root:(const StartMultilingualSpeechRequest *)a4
{
  return [(QSSStartMultilingualSpeechRequest *)self initWithFlatbuffData:a3 root:a4 verify:0];
}

- (id)initAndVerifyWithFlatbuffData:(id)a3
{
  return [(QSSStartMultilingualSpeechRequest *)self initWithFlatbuffData:a3 root:0 verify:1];
}

- (QSSStartMultilingualSpeechRequest)initWithFlatbuffData:(id)a3
{
  return [(QSSStartMultilingualSpeechRequest *)self initWithFlatbuffData:a3 root:0 verify:0];
}
