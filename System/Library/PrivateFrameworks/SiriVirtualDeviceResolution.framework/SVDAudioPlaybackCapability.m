@interface SVDAudioPlaybackCapability
+ (BOOL)supportsSecureCoding;
+ (id)capabilityDescriptionMatchingSupportStatus:(int64_t)a3;
- (SVDAudioPlaybackCapability)initWithCoder:(id)a3;
- (SVDAudioPlaybackCapability)initWithStatus:(int64_t)a3;
- (id)_swiftBacking;
- (int64_t)supportsAudioPlayback;
- (void)encodeWithCoder:(id)a3;
@end

@implementation SVDAudioPlaybackCapability

- (void).cxx_destruct
{
}

- (SVDAudioPlaybackCapability)initWithCoder:(id)a3
{
  id v4 = a3;
  v9.receiver = self;
  v9.super_class = (Class)SVDAudioPlaybackCapability;
  v5 = [(SVDAudioPlaybackCapability *)&v9 init];
  if (v5)
  {
    uint64_t v6 = [v4 decodeObjectOfClass:objc_opt_class() forKey:@"SVDAudioPlaybackCapabilityBacking"];
    backing = v5->_backing;
    v5->_backing = (_TtC27SiriVirtualDeviceResolution23AudioPlaybackCapability *)v6;
  }
  return v5;
}

- (void)encodeWithCoder:(id)a3
{
}

- (id)_swiftBacking
{
  return self->_backing;
}

- (int64_t)supportsAudioPlayback
{
  return [(AudioPlaybackCapability *)self->_backing supportsAudioPlayback];
}

- (SVDAudioPlaybackCapability)initWithStatus:(int64_t)a3
{
  v8.receiver = self;
  v8.super_class = (Class)SVDAudioPlaybackCapability;
  id v4 = [(SVDAudioPlaybackCapability *)&v8 init];
  if (v4)
  {
    v5 = [[_TtC27SiriVirtualDeviceResolution23AudioPlaybackCapability alloc] initWithStatus:a3];
    backing = v4->_backing;
    v4->_backing = v5;
  }
  return v4;
}

+ (BOOL)supportsSecureCoding
{
  return 1;
}

+ (id)capabilityDescriptionMatchingSupportStatus:(int64_t)a3
{
  v3 = +[AudioPlaybackCapability capabilityDescriptionMatchingSupportStatus:a3];
  id v4 = [[SVDCapabilityDescription alloc] initWithBacking:v3];

  return v4;
}

@end