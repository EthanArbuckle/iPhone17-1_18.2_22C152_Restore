@interface CLPInstSchemaCLPNLv4SampleEvaluationFailed
- (BOOL)hasUserProfileSandboxFailure;
- (BOOL)hasXpcClientDisconnected;
- (BOOL)hasXpcClientNLURequestTimeout;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (BOOL)userProfileSandboxFailure;
- (BOOL)xpcClientDisconnected;
- (BOOL)xpcClientNLURequestTimeout;
- (CLPInstSchemaCLPNLv4SampleEvaluationFailed)initWithDictionary:(id)a3;
- (CLPInstSchemaCLPNLv4SampleEvaluationFailed)initWithJSON:(id)a3;
- (NSData)jsonData;
- (id)dictionaryRepresentation;
- (id)suppressMessageUnderConditions;
- (unint64_t)hash;
- (void)deleteUserProfileSandboxFailure;
- (void)deleteXpcClientDisconnected;
- (void)deleteXpcClientNLURequestTimeout;
- (void)setHasUserProfileSandboxFailure:(BOOL)a3;
- (void)setHasXpcClientDisconnected:(BOOL)a3;
- (void)setHasXpcClientNLURequestTimeout:(BOOL)a3;
- (void)setUserProfileSandboxFailure:(BOOL)a3;
- (void)setXpcClientDisconnected:(BOOL)a3;
- (void)setXpcClientNLURequestTimeout:(BOOL)a3;
- (void)writeTo:(id)a3;
@end

@implementation CLPInstSchemaCLPNLv4SampleEvaluationFailed

- (id)suppressMessageUnderConditions
{
  v2 = objc_alloc_init(SISensitiveConditionsSet);
  [(SISensitiveConditionsSet *)v2 addCondition:1];
  return v2;
}

- (BOOL)xpcClientNLURequestTimeout
{
  return self->_xpcClientNLURequestTimeout;
}

- (BOOL)xpcClientDisconnected
{
  return self->_xpcClientDisconnected;
}

- (BOOL)userProfileSandboxFailure
{
  return self->_userProfileSandboxFailure;
}

- (CLPInstSchemaCLPNLv4SampleEvaluationFailed)initWithDictionary:(id)a3
{
  id v4 = a3;
  v11.receiver = self;
  v11.super_class = (Class)CLPInstSchemaCLPNLv4SampleEvaluationFailed;
  v5 = [(CLPInstSchemaCLPNLv4SampleEvaluationFailed *)&v11 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"userProfileSandboxFailure"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[CLPInstSchemaCLPNLv4SampleEvaluationFailed setUserProfileSandboxFailure:](v5, "setUserProfileSandboxFailure:", [v6 BOOLValue]);
    }
    v7 = [v4 objectForKeyedSubscript:@"xpcClientDisconnected"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[CLPInstSchemaCLPNLv4SampleEvaluationFailed setXpcClientDisconnected:](v5, "setXpcClientDisconnected:", [v7 BOOLValue]);
    }
    v8 = [v4 objectForKeyedSubscript:@"xpcClientNLURequestTimeout"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[CLPInstSchemaCLPNLv4SampleEvaluationFailed setXpcClientNLURequestTimeout:](v5, "setXpcClientNLURequestTimeout:", [v8 BOOLValue]);
    }
    v9 = v5;
  }
  return v5;
}

- (CLPInstSchemaCLPNLv4SampleEvaluationFailed)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(CLPInstSchemaCLPNLv4SampleEvaluationFailed *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (NSData)jsonData
{
  v2 = [(CLPInstSchemaCLPNLv4SampleEvaluationFailed *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  char v4 = *(&self->_xpcClientNLURequestTimeout + 1);
  if (v4)
  {
    uint64_t v7 = objc_msgSend(NSNumber, "numberWithBool:", -[CLPInstSchemaCLPNLv4SampleEvaluationFailed userProfileSandboxFailure](self, "userProfileSandboxFailure"));
    [v3 setObject:v7 forKeyedSubscript:@"userProfileSandboxFailure"];

    char v4 = *(&self->_xpcClientNLURequestTimeout + 1);
    if ((v4 & 2) == 0)
    {
LABEL_3:
      if ((v4 & 4) == 0) {
        goto LABEL_5;
      }
      goto LABEL_4;
    }
  }
  else if ((*(&self->_xpcClientNLURequestTimeout + 1) & 2) == 0)
  {
    goto LABEL_3;
  }
  v8 = objc_msgSend(NSNumber, "numberWithBool:", -[CLPInstSchemaCLPNLv4SampleEvaluationFailed xpcClientDisconnected](self, "xpcClientDisconnected"));
  [v3 setObject:v8 forKeyedSubscript:@"xpcClientDisconnected"];

  if ((*(&self->_xpcClientNLURequestTimeout + 1) & 4) != 0)
  {
LABEL_4:
    v5 = objc_msgSend(NSNumber, "numberWithBool:", -[CLPInstSchemaCLPNLv4SampleEvaluationFailed xpcClientNLURequestTimeout](self, "xpcClientNLURequestTimeout"));
    [v3 setObject:v5 forKeyedSubscript:@"xpcClientNLURequestTimeout"];
  }
LABEL_5:
  [(SISchemaInstrumentationMessage *)self willProduceDictionaryRepresentation:v3];
  return v3;
}

- (unint64_t)hash
{
  if (!*(&self->_xpcClientNLURequestTimeout + 1))
  {
    uint64_t v2 = 0;
    if ((*(&self->_xpcClientNLURequestTimeout + 1) & 2) != 0) {
      goto LABEL_3;
    }
LABEL_6:
    uint64_t v3 = 0;
    if ((*(&self->_xpcClientNLURequestTimeout + 1) & 4) != 0) {
      goto LABEL_4;
    }
LABEL_7:
    uint64_t v4 = 0;
    return v3 ^ v2 ^ v4;
  }
  uint64_t v2 = 2654435761 * self->_userProfileSandboxFailure;
  if ((*(&self->_xpcClientNLURequestTimeout + 1) & 2) == 0) {
    goto LABEL_6;
  }
LABEL_3:
  uint64_t v3 = 2654435761 * self->_xpcClientDisconnected;
  if ((*(&self->_xpcClientNLURequestTimeout + 1) & 4) == 0) {
    goto LABEL_7;
  }
LABEL_4:
  uint64_t v4 = 2654435761 * self->_xpcClientNLURequestTimeout;
  return v3 ^ v2 ^ v4;
}

- (BOOL)isEqual:(id)a3
{
  uint64_t v4 = (unsigned __int8 *)a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_14;
  }
  unsigned int v5 = *((unsigned __int8 *)&self->_xpcClientNLURequestTimeout + 1);
  unsigned int v6 = v4[11];
  if ((v5 & 1) != (v6 & 1)) {
    goto LABEL_14;
  }
  if (v5)
  {
    int userProfileSandboxFailure = self->_userProfileSandboxFailure;
    if (userProfileSandboxFailure != [v4 userProfileSandboxFailure]) {
      goto LABEL_14;
    }
    unsigned int v5 = *((unsigned __int8 *)&self->_xpcClientNLURequestTimeout + 1);
    unsigned int v6 = v4[11];
  }
  int v8 = (v5 >> 1) & 1;
  if (v8 != ((v6 >> 1) & 1)) {
    goto LABEL_14;
  }
  if (v8)
  {
    int xpcClientDisconnected = self->_xpcClientDisconnected;
    if (xpcClientDisconnected == [v4 xpcClientDisconnected])
    {
      unsigned int v5 = *((unsigned __int8 *)&self->_xpcClientNLURequestTimeout + 1);
      unsigned int v6 = v4[11];
      goto LABEL_10;
    }
LABEL_14:
    BOOL v12 = 0;
    goto LABEL_15;
  }
LABEL_10:
  int v10 = (v5 >> 2) & 1;
  if (v10 != ((v6 >> 2) & 1)) {
    goto LABEL_14;
  }
  if (v10)
  {
    int xpcClientNLURequestTimeout = self->_xpcClientNLURequestTimeout;
    if (xpcClientNLURequestTimeout != [v4 xpcClientNLURequestTimeout]) {
      goto LABEL_14;
    }
  }
  BOOL v12 = 1;
LABEL_15:

  return v12;
}

- (void)writeTo:(id)a3
{
  id v4 = a3;
  char v5 = *(&self->_xpcClientNLURequestTimeout + 1);
  id v6 = v4;
  if (v5)
  {
    PBDataWriterWriteBOOLField();
    id v4 = v6;
    char v5 = *(&self->_xpcClientNLURequestTimeout + 1);
    if ((v5 & 2) == 0)
    {
LABEL_3:
      if ((v5 & 4) == 0) {
        goto LABEL_5;
      }
      goto LABEL_4;
    }
  }
  else if ((*(&self->_xpcClientNLURequestTimeout + 1) & 2) == 0)
  {
    goto LABEL_3;
  }
  PBDataWriterWriteBOOLField();
  id v4 = v6;
  if ((*(&self->_xpcClientNLURequestTimeout + 1) & 4) != 0)
  {
LABEL_4:
    PBDataWriterWriteBOOLField();
    id v4 = v6;
  }
LABEL_5:
}

- (BOOL)readFrom:(id)a3
{
  return CLPInstSchemaCLPNLv4SampleEvaluationFailedReadFrom(self, (uint64_t)a3);
}

- (void)deleteXpcClientNLURequestTimeout
{
  *(&self->_xpcClientNLURequestTimeout + 1) &= ~4u;
}

- (void)setHasXpcClientNLURequestTimeout:(BOOL)a3
{
  if (a3) {
    char v3 = 4;
  }
  else {
    char v3 = 0;
  }
  *(&self->_xpcClientNLURequestTimeout + 1) = *(&self->_xpcClientNLURequestTimeout + 1) & 0xFB | v3;
}

- (BOOL)hasXpcClientNLURequestTimeout
{
  return (*((unsigned __int8 *)&self->_xpcClientNLURequestTimeout + 1) >> 2) & 1;
}

- (void)setXpcClientNLURequestTimeout:(BOOL)a3
{
  *(&self->_xpcClientNLURequestTimeout + 1) |= 4u;
  self->_int xpcClientNLURequestTimeout = a3;
}

- (void)deleteXpcClientDisconnected
{
  *(&self->_xpcClientNLURequestTimeout + 1) &= ~2u;
}

- (void)setHasXpcClientDisconnected:(BOOL)a3
{
  if (a3) {
    char v3 = 2;
  }
  else {
    char v3 = 0;
  }
  *(&self->_xpcClientNLURequestTimeout + 1) = *(&self->_xpcClientNLURequestTimeout + 1) & 0xFD | v3;
}

- (BOOL)hasXpcClientDisconnected
{
  return (*((unsigned __int8 *)&self->_xpcClientNLURequestTimeout + 1) >> 1) & 1;
}

- (void)setXpcClientDisconnected:(BOOL)a3
{
  *(&self->_xpcClientNLURequestTimeout + 1) |= 2u;
  self->_int xpcClientDisconnected = a3;
}

- (void)deleteUserProfileSandboxFailure
{
  *(&self->_xpcClientNLURequestTimeout + 1) &= ~1u;
}

- (void)setHasUserProfileSandboxFailure:(BOOL)a3
{
  *(&self->_xpcClientNLURequestTimeout + 1) = *(&self->_xpcClientNLURequestTimeout + 1) & 0xFE | a3;
}

- (BOOL)hasUserProfileSandboxFailure
{
  return *(&self->_xpcClientNLURequestTimeout + 1);
}

- (void)setUserProfileSandboxFailure:(BOOL)a3
{
  *(&self->_xpcClientNLURequestTimeout + 1) |= 1u;
  self->_int userProfileSandboxFailure = a3;
}

@end