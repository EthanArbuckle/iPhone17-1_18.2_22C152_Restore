@interface USPSchemaUSPMeasurementContext
- (BOOL)hasContextName;
- (BOOL)hasEnvironment;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (NSData)jsonData;
- (NSString)contextName;
- (USPSchemaUSPMeasurementContext)initWithDictionary:(id)a3;
- (USPSchemaUSPMeasurementContext)initWithJSON:(id)a3;
- (id)dictionaryRepresentation;
- (id)suppressMessageUnderConditions;
- (int)environment;
- (unint64_t)hash;
- (void)deleteContextName;
- (void)deleteEnvironment;
- (void)setContextName:(id)a3;
- (void)setEnvironment:(int)a3;
- (void)setHasContextName:(BOOL)a3;
- (void)setHasEnvironment:(BOOL)a3;
- (void)writeTo:(id)a3;
@end

@implementation USPSchemaUSPMeasurementContext

- (id)suppressMessageUnderConditions
{
  v2 = objc_alloc_init(SISensitiveConditionsSet);
  [(SISensitiveConditionsSet *)v2 addCondition:1];
  return v2;
}

- (void).cxx_destruct
{
}

- (void)setHasContextName:(BOOL)a3
{
  *((unsigned char *)&self->_has + 1) = a3;
}

- (int)environment
{
  return self->_environment;
}

- (void)setContextName:(id)a3
{
}

- (NSString)contextName
{
  return self->_contextName;
}

- (USPSchemaUSPMeasurementContext)initWithDictionary:(id)a3
{
  id v4 = a3;
  v11.receiver = self;
  v11.super_class = (Class)USPSchemaUSPMeasurementContext;
  v5 = [(USPSchemaUSPMeasurementContext *)&v11 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"contextName"];
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      v7 = (void *)[v6 copy];
      [(USPSchemaUSPMeasurementContext *)v5 setContextName:v7];
    }
    v8 = [v4 objectForKeyedSubscript:@"environment"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[USPSchemaUSPMeasurementContext setEnvironment:](v5, "setEnvironment:", [v8 intValue]);
    }
    v9 = v5;
  }
  return v5;
}

- (USPSchemaUSPMeasurementContext)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(USPSchemaUSPMeasurementContext *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (NSData)jsonData
{
  v2 = [(USPSchemaUSPMeasurementContext *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  if (self->_contextName)
  {
    id v4 = [(USPSchemaUSPMeasurementContext *)self contextName];
    v5 = (void *)[v4 copy];
    [v3 setObject:v5 forKeyedSubscript:@"contextName"];
  }
  if (*(unsigned char *)&self->_has)
  {
    int v6 = [(USPSchemaUSPMeasurementContext *)self environment];
    uint64_t v7 = @"USPENVIRONMENT_UNKNOWN";
    if (v6 == 1) {
      uint64_t v7 = @"USPENVIRONMENT_LOCAL";
    }
    if (v6 == 2) {
      v8 = @"USPENVIRONMENT_CONTINUOS_INTEGRATION";
    }
    else {
      v8 = v7;
    }
    [v3 setObject:v8 forKeyedSubscript:@"environment"];
  }
  [(SISchemaInstrumentationMessage *)self willProduceDictionaryRepresentation:v3];
  return v3;
}

- (unint64_t)hash
{
  NSUInteger v3 = [(NSString *)self->_contextName hash];
  if (*(unsigned char *)&self->_has) {
    uint64_t v4 = 2654435761 * self->_environment;
  }
  else {
    uint64_t v4 = 0;
  }
  return v4 ^ v3;
}

- (BOOL)isEqual:(id)a3
{
  uint64_t v4 = a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_12;
  }
  v5 = [(USPSchemaUSPMeasurementContext *)self contextName];
  int v6 = [v4 contextName];
  uint64_t v7 = v6;
  if ((v5 == 0) == (v6 != 0))
  {

LABEL_12:
    BOOL v14 = 0;
    goto LABEL_13;
  }
  uint64_t v8 = [(USPSchemaUSPMeasurementContext *)self contextName];
  if (v8)
  {
    v9 = (void *)v8;
    v10 = [(USPSchemaUSPMeasurementContext *)self contextName];
    objc_super v11 = [v4 contextName];
    int v12 = [v10 isEqual:v11];

    if (!v12) {
      goto LABEL_12;
    }
  }
  else
  {
  }
  if ((*(unsigned char *)&self->_has & 1) != (v4[20] & 1)) {
    goto LABEL_12;
  }
  if (*(unsigned char *)&self->_has)
  {
    int environment = self->_environment;
    if (environment != [v4 environment]) {
      goto LABEL_12;
    }
  }
  BOOL v14 = 1;
LABEL_13:

  return v14;
}

- (void)writeTo:(id)a3
{
  id v5 = a3;
  uint64_t v4 = [(USPSchemaUSPMeasurementContext *)self contextName];

  if (v4) {
    PBDataWriterWriteStringField();
  }
  if (*(unsigned char *)&self->_has) {
    PBDataWriterWriteInt32Field();
  }
}

- (BOOL)readFrom:(id)a3
{
  return USPSchemaUSPMeasurementContextReadFrom(self, (uint64_t)a3);
}

- (void)deleteEnvironment
{
  *(unsigned char *)&self->_has &= ~1u;
}

- (void)setHasEnvironment:(BOOL)a3
{
  *(unsigned char *)&self->_has = *(unsigned char *)&self->_has & 0xFE | a3;
}

- (BOOL)hasEnvironment
{
  return *(unsigned char *)&self->_has & 1;
}

- (void)setEnvironment:(int)a3
{
  *(unsigned char *)&self->_has |= 1u;
  self->_int environment = a3;
}

- (void)deleteContextName
{
}

- (BOOL)hasContextName
{
  return self->_contextName != 0;
}

@end