...
- (void)setHasRemoteType:(BOOL)a3;
- (void)setRemoteType:(int)a3;
- (void)writeTo:(id)a3;
@end
...
- (SISchemaTVRemoteInvocationContext)initWithDictionary:(id)a3
{
  id v4 = a3;
  v9.receiver = self;
  v9.super_class = (Class)SISchemaTVRemoteInvocationContext;
...
- (id)dictionaryRepresentation
{
  ...
  {
    unsigned int v4 = [(SISchemaTVRemoteInvocationContext *)self remoteType] - 1;
    if (v4 > 6) {
  ...
  if ([v4 isMemberOfClass:objc_opt_class()])
  {
    if ((*(unsigned char *)&self->_has & 1) == (v4[12] & 1))
    {
      if ((*(unsigned char *)&self->_has & 1) == 0
        || (int remoteType = self->_remoteType, remoteType == [v4 remoteType]))
      {
        BOOL v6 = 1;
      }
    }
...
- (BOOL)readFrom:(id)a3
{
  return SISchemaTVRemoteInvocationContextReadFrom(self, (uint64_t)a3);
}
