...
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (NLXSchemaCDMXPCEventProcessingFailed)initWithDictionary:(id)a3;
- (NLXSchemaCDMXPCEventProcessingFailed)initWithJSON:(id)a3;
- (NSData)jsonData;
...
- (NLXSchemaCDMXPCEventProcessingFailed)initWithDictionary:(id)a3
{
  id v4 = a3;
  v10.receiver = self;
  v10.super_class = (Class)NLXSchemaCDMXPCEventProcessingFailed;
...
- (NSData)jsonData
{
  v2 = [(NLXSchemaCDMXPCEventProcessingFailed *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if ((has & 2) != 0)
  {
    v5 = objc_msgSend(NSNumber, "numberWithUnsignedInt:", -[NLXSchemaCDMXPCEventProcessingFailed errorCode](self, "errorCode"));
  ...
  if (has)
  {
    unsigned int v6 = [(NLXSchemaCDMXPCEventProcessingFailed *)self reason] - 1;
    if (v6 > 2) {
      uint64_t v7 = @"CDMXPCEVENTPROCESSINGFAILUREREASON_UNKNOWN";
    }
    else {
      uint64_t v7 = off_1E5EBA508[v6];
    }
...
- (BOOL)isEqual:(id)a3
{
  ...
  $A19688C9564A5C22DEC551C5AAEF44CA has = self->_has;
...
- (BOOL)readFrom:(id)a3
{
  return NLXSchemaCDMXPCEventProcessingFailedReadFrom(self, (uint64_t)a3);
}
