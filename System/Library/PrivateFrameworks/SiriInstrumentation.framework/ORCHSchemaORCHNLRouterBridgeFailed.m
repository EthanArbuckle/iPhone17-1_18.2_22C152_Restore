@interface ORCHSchemaORCHNLRouterBridgeFailed
- (BOOL)hasErrorCode;
- (BOOL)hasErrorDomain;
- (BOOL)hasRoutingDecision;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (NSData)jsonData;
- (ORCHSchemaORCHNLRouterBridgeFailed)initWithDictionary:(id)a3;
- (ORCHSchemaORCHNLRouterBridgeFailed)initWithJSON:(id)a3;
- (ORCHSchemaORCHNLRouterBridgeRoutingDecision)routingDecision;
- (id)applySensitiveConditionsPolicy:(id)a3;
- (id)dictionaryRepresentation;
- (id)suppressMessageUnderConditions;
- (int)errorCode;
- (int)errorDomain;
- (unint64_t)hash;
- (void)deleteErrorCode;
- (void)deleteErrorDomain;
- (void)deleteRoutingDecision;
- (void)setErrorCode:(int)a3;
- (void)setErrorDomain:(int)a3;
- (void)setHasErrorCode:(BOOL)a3;
- (void)setHasErrorDomain:(BOOL)a3;
- (void)setHasRoutingDecision:(BOOL)a3;
- (void)setRoutingDecision:(id)a3;
- (void)writeTo:(id)a3;
@end

@implementation ORCHSchemaORCHNLRouterBridgeFailed

- (id)applySensitiveConditionsPolicy:(id)a3
{
  v9.receiver = self;
  v9.super_class = (Class)ORCHSchemaORCHNLRouterBridgeFailed;
  id v4 = a3;
  v5 = [(SISchemaInstrumentationMessage *)&v9 applySensitiveConditionsPolicy:v4];
  v6 = [(ORCHSchemaORCHNLRouterBridgeFailed *)self routingDecision];
  v7 = [v6 applySensitiveConditionsPolicy:v4];

  LODWORD(v4) = [v7 suppressMessage];
  if (v4) {
    [(ORCHSchemaORCHNLRouterBridgeFailed *)self deleteRoutingDecision];
  }
  return v5;
}

- (id)suppressMessageUnderConditions
{
  v2 = objc_alloc_init(SISensitiveConditionsSet);
  [(SISensitiveConditionsSet *)v2 addCondition:1];
  return v2;
}

- (void).cxx_destruct
{
}

- (void)setHasRoutingDecision:(BOOL)a3
{
  *((unsigned char *)&self->_has + 1) = a3;
}

- (void)setRoutingDecision:(id)a3
{
}

- (ORCHSchemaORCHNLRouterBridgeRoutingDecision)routingDecision
{
  return self->_routingDecision;
}

- (int)errorCode
{
  return self->_errorCode;
}

- (int)errorDomain
{
  return self->_errorDomain;
}

- (ORCHSchemaORCHNLRouterBridgeFailed)initWithDictionary:(id)a3
{
  id v4 = a3;
  v12.receiver = self;
  v12.super_class = (Class)ORCHSchemaORCHNLRouterBridgeFailed;
  v5 = [(ORCHSchemaORCHNLRouterBridgeFailed *)&v12 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"errorDomain"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[ORCHSchemaORCHNLRouterBridgeFailed setErrorDomain:](v5, "setErrorDomain:", [v6 intValue]);
    }
    v7 = [v4 objectForKeyedSubscript:@"errorCode"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[ORCHSchemaORCHNLRouterBridgeFailed setErrorCode:](v5, "setErrorCode:", [v7 intValue]);
    }
    v8 = [v4 objectForKeyedSubscript:@"routingDecision"];
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      objc_super v9 = [[ORCHSchemaORCHNLRouterBridgeRoutingDecision alloc] initWithDictionary:v8];
      [(ORCHSchemaORCHNLRouterBridgeFailed *)v5 setRoutingDecision:v9];
    }
    v10 = v5;
  }
  return v5;
}

- (ORCHSchemaORCHNLRouterBridgeFailed)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(ORCHSchemaORCHNLRouterBridgeFailed *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (NSData)jsonData
{
  v2 = [(ORCHSchemaORCHNLRouterBridgeFailed *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  char has = (char)self->_has;
  if ((has & 2) != 0)
  {
    v5 = objc_msgSend(NSNumber, "numberWithInt:", -[ORCHSchemaORCHNLRouterBridgeFailed errorCode](self, "errorCode"));
    [v3 setObject:v5 forKeyedSubscript:@"errorCode"];

    char has = (char)self->_has;
  }
  if (has)
  {
    if ([(ORCHSchemaORCHNLRouterBridgeFailed *)self errorDomain] == 1) {
      v6 = @"ORCHNLROUTEBRIDGERERRORDOMAIN_NL_ROUTER_CLIENT";
    }
    else {
      v6 = @"ORCHNLROUTERBRIDGEERRORDOMAIN_UNKNOWN";
    }
    [v3 setObject:v6 forKeyedSubscript:@"errorDomain"];
  }
  if (self->_routingDecision)
  {
    uint64_t v7 = [(ORCHSchemaORCHNLRouterBridgeFailed *)self routingDecision];
    v8 = [v7 dictionaryRepresentation];
    if (v8)
    {
      [v3 setObject:v8 forKeyedSubscript:@"routingDecision"];
    }
    else
    {
      objc_super v9 = [MEMORY[0x1E4F1CA98] null];
      [v3 setObject:v9 forKeyedSubscript:@"routingDecision"];
    }
  }
  [(SISchemaInstrumentationMessage *)self willProduceDictionaryRepresentation:v3];
  return v3;
}

- (unint64_t)hash
{
  if (*(unsigned char *)&self->_has)
  {
    uint64_t v2 = 2654435761 * self->_errorDomain;
    if ((*(unsigned char *)&self->_has & 2) != 0) {
      goto LABEL_3;
    }
LABEL_5:
    uint64_t v3 = 0;
    return v3 ^ v2 ^ [(ORCHSchemaORCHNLRouterBridgeRoutingDecision *)self->_routingDecision hash];
  }
  uint64_t v2 = 0;
  if ((*(unsigned char *)&self->_has & 2) == 0) {
    goto LABEL_5;
  }
LABEL_3:
  uint64_t v3 = 2654435761 * self->_errorCode;
  return v3 ^ v2 ^ [(ORCHSchemaORCHNLRouterBridgeRoutingDecision *)self->_routingDecision hash];
}

- (BOOL)isEqual:(id)a3
{
  id v4 = (unsigned __int8 *)a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_14;
  }
  $EEF5D83BDC1038D82D17C816D898FA8B has = self->_has;
  unsigned int v6 = v4[24];
  if ((*(unsigned char *)&has & 1) != (v6 & 1)) {
    goto LABEL_14;
  }
  if (*(unsigned char *)&has)
  {
    int errorDomain = self->_errorDomain;
    if (errorDomain != [v4 errorDomain]) {
      goto LABEL_14;
    }
    $EEF5D83BDC1038D82D17C816D898FA8B has = self->_has;
    unsigned int v6 = v4[24];
  }
  int v8 = (*(unsigned int *)&has >> 1) & 1;
  if (v8 == ((v6 >> 1) & 1))
  {
    if (!v8 || (int errorCode = self->_errorCode, errorCode == [v4 errorCode]))
    {
      v10 = [(ORCHSchemaORCHNLRouterBridgeFailed *)self routingDecision];
      v11 = [v4 routingDecision];
      objc_super v12 = v11;
      if ((v10 == 0) != (v11 != 0))
      {
        uint64_t v13 = [(ORCHSchemaORCHNLRouterBridgeFailed *)self routingDecision];
        if (!v13)
        {

LABEL_17:
          BOOL v18 = 1;
          goto LABEL_15;
        }
        v14 = (void *)v13;
        v15 = [(ORCHSchemaORCHNLRouterBridgeFailed *)self routingDecision];
        v16 = [v4 routingDecision];
        char v17 = [v15 isEqual:v16];

        if (v17) {
          goto LABEL_17;
        }
      }
      else
      {
      }
    }
  }
LABEL_14:
  BOOL v18 = 0;
LABEL_15:

  return v18;
}

- (void)writeTo:(id)a3
{
  id v8 = a3;
  char has = (char)self->_has;
  if (has)
  {
    PBDataWriterWriteInt32Field();
    char has = (char)self->_has;
  }
  if ((has & 2) != 0) {
    PBDataWriterWriteInt32Field();
  }
  v5 = [(ORCHSchemaORCHNLRouterBridgeFailed *)self routingDecision];

  unsigned int v6 = v8;
  if (v5)
  {
    uint64_t v7 = [(ORCHSchemaORCHNLRouterBridgeFailed *)self routingDecision];
    PBDataWriterWriteSubmessage();

    unsigned int v6 = v8;
  }
}

- (BOOL)readFrom:(id)a3
{
  return ORCHSchemaORCHNLRouterBridgeFailedReadFrom(self, (uint64_t)a3);
}

- (void)deleteRoutingDecision
{
}

- (BOOL)hasRoutingDecision
{
  return self->_routingDecision != 0;
}

- (void)deleteErrorCode
{
  *(unsigned char *)&self->_has &= ~2u;
}

- (void)setHasErrorCode:(BOOL)a3
{
  if (a3) {
    char v3 = 2;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFD | v3;
}

- (BOOL)hasErrorCode
{
  return (*(unsigned char *)&self->_has >> 1) & 1;
}

- (void)setErrorCode:(int)a3
{
  *(unsigned char *)&self->_has |= 2u;
  self->_int errorCode = a3;
}

- (void)deleteErrorDomain
{
  *(unsigned char *)&self->_has &= ~1u;
}

- (void)setHasErrorDomain:(BOOL)a3
{
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFE | a3;
}

- (BOOL)hasErrorDomain
{
  return *(unsigned char *)&self->_has & 1;
}

- (void)setErrorDomain:(int)a3
{
  *(unsigned char *)&self->_has |= 1u;
  self->_int errorDomain = a3;
}

@end