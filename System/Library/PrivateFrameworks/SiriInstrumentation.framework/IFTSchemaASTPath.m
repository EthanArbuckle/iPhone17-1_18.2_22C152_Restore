@interface IFTSchemaASTPath
- (BOOL)hasCollectionIndex;
- (BOOL)hasParameterName;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (IFTSchemaASTPath)initWithDictionary:(id)a3;
- (IFTSchemaASTPath)initWithJSON:(id)a3;
- (NSData)jsonData;
- (NSString)parameterName;
- (id)dictionaryRepresentation;
- (id)suppressMessageUnderConditions;
- (int64_t)collectionIndex;
- (unint64_t)hash;
- (void)deleteCollectionIndex;
- (void)deleteParameterName;
- (void)setCollectionIndex:(int64_t)a3;
- (void)setHasCollectionIndex:(BOOL)a3;
- (void)setHasParameterName:(BOOL)a3;
- (void)setParameterName:(id)a3;
- (void)writeTo:(id)a3;
@end

@implementation IFTSchemaASTPath

- (id)suppressMessageUnderConditions
{
  v2 = objc_alloc_init(SISensitiveConditionsSet);
  [(SISensitiveConditionsSet *)v2 addCondition:1];
  return v2;
}

- (void).cxx_destruct
{
}

- (void)setHasParameterName:(BOOL)a3
{
  *((unsigned char *)&self->_has + 1) = a3;
}

- (int64_t)collectionIndex
{
  return self->_collectionIndex;
}

- (void)setParameterName:(id)a3
{
}

- (NSString)parameterName
{
  return self->_parameterName;
}

- (IFTSchemaASTPath)initWithDictionary:(id)a3
{
  id v4 = a3;
  v11.receiver = self;
  v11.super_class = (Class)IFTSchemaASTPath;
  v5 = [(IFTSchemaASTPath *)&v11 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"parameterName"];
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      v7 = (void *)[v6 copy];
      [(IFTSchemaASTPath *)v5 setParameterName:v7];
    }
    v8 = [v4 objectForKeyedSubscript:@"collectionIndex"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[IFTSchemaASTPath setCollectionIndex:](v5, "setCollectionIndex:", [v8 longLongValue]);
    }
    v9 = v5;
  }
  return v5;
}

- (IFTSchemaASTPath)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(IFTSchemaASTPath *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (NSData)jsonData
{
  v2 = [(IFTSchemaASTPath *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  if (*(unsigned char *)&self->_has)
  {
    id v4 = objc_msgSend(NSNumber, "numberWithLongLong:", -[IFTSchemaASTPath collectionIndex](self, "collectionIndex"));
    [v3 setObject:v4 forKeyedSubscript:@"collectionIndex"];
  }
  if (self->_parameterName)
  {
    v5 = [(IFTSchemaASTPath *)self parameterName];
    v6 = (void *)[v5 copy];
    [v3 setObject:v6 forKeyedSubscript:@"parameterName"];
  }
  [(SISchemaInstrumentationMessage *)self willProduceDictionaryRepresentation:v3];
  return v3;
}

- (unint64_t)hash
{
  NSUInteger v3 = [(NSString *)self->_parameterName hash];
  if (*(unsigned char *)&self->_has) {
    uint64_t v4 = 2654435761 * self->_collectionIndex;
  }
  else {
    uint64_t v4 = 0;
  }
  return v4 ^ v3;
}

- (BOOL)isEqual:(id)a3
{
  uint64_t v4 = a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_12;
  }
  v5 = [(IFTSchemaASTPath *)self parameterName];
  v6 = [v4 parameterName];
  uint64_t v7 = v6;
  if ((v5 == 0) == (v6 != 0))
  {

LABEL_12:
    BOOL v14 = 0;
    goto LABEL_13;
  }
  uint64_t v8 = [(IFTSchemaASTPath *)self parameterName];
  if (v8)
  {
    v9 = (void *)v8;
    v10 = [(IFTSchemaASTPath *)self parameterName];
    objc_super v11 = [v4 parameterName];
    int v12 = [v10 isEqual:v11];

    if (!v12) {
      goto LABEL_12;
    }
  }
  else
  {
  }
  if ((*(unsigned char *)&self->_has & 1) != (v4[24] & 1)) {
    goto LABEL_12;
  }
  if (*(unsigned char *)&self->_has)
  {
    int64_t collectionIndex = self->_collectionIndex;
    if (collectionIndex != [v4 collectionIndex]) {
      goto LABEL_12;
    }
  }
  BOOL v14 = 1;
LABEL_13:

  return v14;
}

- (void)writeTo:(id)a3
{
  id v5 = a3;
  uint64_t v4 = [(IFTSchemaASTPath *)self parameterName];

  if (v4) {
    PBDataWriterWriteStringField();
  }
  if (*(unsigned char *)&self->_has) {
    PBDataWriterWriteInt64Field();
  }
}

- (BOOL)readFrom:(id)a3
{
  return IFTSchemaASTPathReadFrom(self, (uint64_t)a3);
}

- (void)deleteCollectionIndex
{
  *(unsigned char *)&self->_has &= ~1u;
}

- (void)setHasCollectionIndex:(BOOL)a3
{
  *(unsigned char *)&self->_has = *(unsigned char *)&self->_has & 0xFE | a3;
}

- (BOOL)hasCollectionIndex
{
  return *(unsigned char *)&self->_has & 1;
}

- (void)setCollectionIndex:(int64_t)a3
{
  *(unsigned char *)&self->_has |= 1u;
  self->_int64_t collectionIndex = a3;
}

- (void)deleteParameterName
{
}

- (BOOL)hasParameterName
{
  return self->_parameterName != 0;
}

@end