@interface DIMSchemaDIMWatchDeviceAttributes
- (BOOL)hasIsSatellitePaired;
- (BOOL)hasWatchArmOrientation;
- (BOOL)isEqual:(id)a3;
- (BOOL)isSatellitePaired;
- (BOOL)readFrom:(id)a3;
- (DIMSchemaDIMWatchDeviceAttributes)initWithDictionary:(id)a3;
- (DIMSchemaDIMWatchDeviceAttributes)initWithJSON:(id)a3;
- (NSData)jsonData;
- (id)dictionaryRepresentation;
- (id)suppressMessageUnderConditions;
- (int)watchArmOrientation;
- (unint64_t)hash;
- (void)deleteIsSatellitePaired;
- (void)deleteWatchArmOrientation;
- (void)setHasIsSatellitePaired:(BOOL)a3;
- (void)setHasWatchArmOrientation:(BOOL)a3;
- (void)setIsSatellitePaired:(BOOL)a3;
- (void)setWatchArmOrientation:(int)a3;
- (void)writeTo:(id)a3;
@end

@implementation DIMSchemaDIMWatchDeviceAttributes

- (id)suppressMessageUnderConditions
{
  v2 = objc_alloc_init(SISensitiveConditionsSet);
  [(SISensitiveConditionsSet *)v2 addCondition:1];
  return v2;
}

- (int)watchArmOrientation
{
  return self->_watchArmOrientation;
}

- (BOOL)isSatellitePaired
{
  return self->_isSatellitePaired;
}

- (DIMSchemaDIMWatchDeviceAttributes)initWithDictionary:(id)a3
{
  id v4 = a3;
  v10.receiver = self;
  v10.super_class = (Class)DIMSchemaDIMWatchDeviceAttributes;
  v5 = [(DIMSchemaDIMWatchDeviceAttributes *)&v10 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"isSatellitePaired"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[DIMSchemaDIMWatchDeviceAttributes setIsSatellitePaired:](v5, "setIsSatellitePaired:", [v6 BOOLValue]);
    }
    v7 = [v4 objectForKeyedSubscript:@"watchArmOrientation"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[DIMSchemaDIMWatchDeviceAttributes setWatchArmOrientation:](v5, "setWatchArmOrientation:", [v7 intValue]);
    }
    v8 = v5;
  }
  return v5;
}

- (DIMSchemaDIMWatchDeviceAttributes)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(DIMSchemaDIMWatchDeviceAttributes *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (NSData)jsonData
{
  v2 = [(DIMSchemaDIMWatchDeviceAttributes *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  char has = (char)self->_has;
  if (has)
  {
    v5 = objc_msgSend(NSNumber, "numberWithBool:", -[DIMSchemaDIMWatchDeviceAttributes isSatellitePaired](self, "isSatellitePaired"));
    [v3 setObject:v5 forKeyedSubscript:@"isSatellitePaired"];

    char has = (char)self->_has;
  }
  if ((has & 2) != 0)
  {
    int v6 = [(DIMSchemaDIMWatchDeviceAttributes *)self watchArmOrientation];
    uint64_t v7 = @"WATCHARMORIENTATION_UNKNOWN";
    if (v6 == 1) {
      uint64_t v7 = @"WATCHARMORIENTATION_LEFT";
    }
    if (v6 == 2) {
      v8 = @"WATCHARMORIENTATION_RIGHT";
    }
    else {
      v8 = v7;
    }
    [v3 setObject:v8 forKeyedSubscript:@"watchArmOrientation"];
  }
  [(SISchemaInstrumentationMessage *)self willProduceDictionaryRepresentation:v3];
  return v3;
}

- (unint64_t)hash
{
  if (*(unsigned char *)&self->_has)
  {
    uint64_t v2 = 2654435761 * self->_isSatellitePaired;
    if ((*(unsigned char *)&self->_has & 2) != 0) {
      goto LABEL_3;
    }
LABEL_5:
    uint64_t v3 = 0;
    return v3 ^ v2;
  }
  uint64_t v2 = 0;
  if ((*(unsigned char *)&self->_has & 2) == 0) {
    goto LABEL_5;
  }
LABEL_3:
  uint64_t v3 = 2654435761 * self->_watchArmOrientation;
  return v3 ^ v2;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = (unsigned __int8 *)a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_10;
  }
  $CECF30419B7CA681C94210455065B967 has = self->_has;
  unsigned int v6 = v4[16];
  if ((*(unsigned char *)&has & 1) != (v6 & 1)) {
    goto LABEL_10;
  }
  if (*(unsigned char *)&has)
  {
    int isSatellitePaired = self->_isSatellitePaired;
    if (isSatellitePaired != [v4 isSatellitePaired])
    {
LABEL_10:
      BOOL v10 = 0;
      goto LABEL_11;
    }
    $CECF30419B7CA681C94210455065B967 has = self->_has;
    unsigned int v6 = v4[16];
  }
  int v8 = (*(unsigned int *)&has >> 1) & 1;
  if (v8 != ((v6 >> 1) & 1)) {
    goto LABEL_10;
  }
  if (v8)
  {
    int watchArmOrientation = self->_watchArmOrientation;
    if (watchArmOrientation != [v4 watchArmOrientation]) {
      goto LABEL_10;
    }
  }
  BOOL v10 = 1;
LABEL_11:

  return v10;
}

- (void)writeTo:(id)a3
{
  id v4 = a3;
  char has = (char)self->_has;
  id v6 = v4;
  if (has)
  {
    PBDataWriterWriteBOOLField();
    id v4 = v6;
    char has = (char)self->_has;
  }
  if ((has & 2) != 0)
  {
    PBDataWriterWriteInt32Field();
    id v4 = v6;
  }
}

- (BOOL)readFrom:(id)a3
{
  return DIMSchemaDIMWatchDeviceAttributesReadFrom(self, (uint64_t)a3);
}

- (void)deleteWatchArmOrientation
{
  *(unsigned char *)&self->_has &= ~2u;
}

- (void)setHasWatchArmOrientation:(BOOL)a3
{
  if (a3) {
    char v3 = 2;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFD | v3;
}

- (BOOL)hasWatchArmOrientation
{
  return (*(unsigned char *)&self->_has >> 1) & 1;
}

- (void)setWatchArmOrientation:(int)a3
{
  *(unsigned char *)&self->_has |= 2u;
  self->_int watchArmOrientation = a3;
}

- (void)deleteIsSatellitePaired
{
  *(unsigned char *)&self->_has &= ~1u;
}

- (void)setHasIsSatellitePaired:(BOOL)a3
{
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFE | a3;
}

- (BOOL)hasIsSatellitePaired
{
  return *(unsigned char *)&self->_has & 1;
}

- (void)setIsSatellitePaired:(BOOL)a3
{
  *(unsigned char *)&self->_has |= 1u;
  self->_int isSatellitePaired = a3;
}

@end