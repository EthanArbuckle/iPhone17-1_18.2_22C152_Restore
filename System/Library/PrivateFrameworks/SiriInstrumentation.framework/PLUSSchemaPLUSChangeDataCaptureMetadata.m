...
- (NSData)jsonData
{
  v2 = [(PLUSSchemaPLUSChangeDataCaptureMetadata *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  {
    unsigned int v4 = [(PLUSSchemaPLUSChangeDataCaptureMetadata *)self operationType] - 1;
    if (v4 > 2) {
    ...
    [v3 setObject:v5 forKeyedSubscript:@"operationType"];
  }
  ...
  if ([v4 isMemberOfClass:objc_opt_class()])
  {
    if ((*(unsigned char *)&self->_has & 1) == (v4[12] & 1))
    {
      if ((*(unsigned char *)&self->_has & 1) == 0
        || (int operationType = self->_operationType, operationType == [v4 operationType]))
      {
        BOOL v6 = 1;
      }
    }
