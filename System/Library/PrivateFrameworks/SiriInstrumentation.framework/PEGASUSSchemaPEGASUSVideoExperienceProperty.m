@interface PEGASUSSchemaPEGASUSVideoExperienceProperty
- (BOOL)hasIsForegroundAppAvailable;
- (BOOL)hasIsRecommendationRequest;
- (BOOL)hasIsThirdPartyRequest;
- (BOOL)isEqual:(id)a3;
- (BOOL)isForegroundAppAvailable;
- (BOOL)isRecommendationRequest;
- (BOOL)isThirdPartyRequest;
- (BOOL)readFrom:(id)a3;
- (NSData)jsonData;
- (PEGASUSSchemaPEGASUSVideoExperienceProperty)initWithDictionary:(id)a3;
- (PEGASUSSchemaPEGASUSVideoExperienceProperty)initWithJSON:(id)a3;
- (id)dictionaryRepresentation;
- (id)suppressMessageUnderConditions;
- (unint64_t)hash;
- (void)deleteIsForegroundAppAvailable;
- (void)deleteIsRecommendationRequest;
- (void)deleteIsThirdPartyRequest;
- (void)setHasIsForegroundAppAvailable:(BOOL)a3;
- (void)setHasIsRecommendationRequest:(BOOL)a3;
- (void)setHasIsThirdPartyRequest:(BOOL)a3;
- (void)setIsForegroundAppAvailable:(BOOL)a3;
- (void)setIsRecommendationRequest:(BOOL)a3;
- (void)setIsThirdPartyRequest:(BOOL)a3;
- (void)writeTo:(id)a3;
@end

@implementation PEGASUSSchemaPEGASUSVideoExperienceProperty

- (id)suppressMessageUnderConditions
{
  v2 = objc_alloc_init(SISensitiveConditionsSet);
  [(SISensitiveConditionsSet *)v2 addCondition:1];
  return v2;
}

- (BOOL)isThirdPartyRequest
{
  return self->_isThirdPartyRequest;
}

- (BOOL)isRecommendationRequest
{
  return self->_isRecommendationRequest;
}

- (BOOL)isForegroundAppAvailable
{
  return self->_isForegroundAppAvailable;
}

- (PEGASUSSchemaPEGASUSVideoExperienceProperty)initWithDictionary:(id)a3
{
  id v4 = a3;
  v11.receiver = self;
  v11.super_class = (Class)PEGASUSSchemaPEGASUSVideoExperienceProperty;
  v5 = [(PEGASUSSchemaPEGASUSVideoExperienceProperty *)&v11 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"isForegroundAppAvailable"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[PEGASUSSchemaPEGASUSVideoExperienceProperty setIsForegroundAppAvailable:](v5, "setIsForegroundAppAvailable:", [v6 BOOLValue]);
    }
    v7 = [v4 objectForKeyedSubscript:@"isRecommendationRequest"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[PEGASUSSchemaPEGASUSVideoExperienceProperty setIsRecommendationRequest:](v5, "setIsRecommendationRequest:", [v7 BOOLValue]);
    }
    v8 = [v4 objectForKeyedSubscript:@"isThirdPartyRequest"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[PEGASUSSchemaPEGASUSVideoExperienceProperty setIsThirdPartyRequest:](v5, "setIsThirdPartyRequest:", [v8 BOOLValue]);
    }
    v9 = v5;
  }
  return v5;
}

- (PEGASUSSchemaPEGASUSVideoExperienceProperty)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(PEGASUSSchemaPEGASUSVideoExperienceProperty *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (NSData)jsonData
{
  v2 = [(PEGASUSSchemaPEGASUSVideoExperienceProperty *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  char v4 = *(&self->_isThirdPartyRequest + 1);
  if (v4)
  {
    uint64_t v7 = objc_msgSend(NSNumber, "numberWithBool:", -[PEGASUSSchemaPEGASUSVideoExperienceProperty isForegroundAppAvailable](self, "isForegroundAppAvailable"));
    [v3 setObject:v7 forKeyedSubscript:@"isForegroundAppAvailable"];

    char v4 = *(&self->_isThirdPartyRequest + 1);
    if ((v4 & 2) == 0)
    {
LABEL_3:
      if ((v4 & 4) == 0) {
        goto LABEL_5;
      }
      goto LABEL_4;
    }
  }
  else if ((*(&self->_isThirdPartyRequest + 1) & 2) == 0)
  {
    goto LABEL_3;
  }
  v8 = objc_msgSend(NSNumber, "numberWithBool:", -[PEGASUSSchemaPEGASUSVideoExperienceProperty isRecommendationRequest](self, "isRecommendationRequest"));
  [v3 setObject:v8 forKeyedSubscript:@"isRecommendationRequest"];

  if ((*(&self->_isThirdPartyRequest + 1) & 4) != 0)
  {
LABEL_4:
    v5 = objc_msgSend(NSNumber, "numberWithBool:", -[PEGASUSSchemaPEGASUSVideoExperienceProperty isThirdPartyRequest](self, "isThirdPartyRequest"));
    [v3 setObject:v5 forKeyedSubscript:@"isThirdPartyRequest"];
  }
LABEL_5:
  [(SISchemaInstrumentationMessage *)self willProduceDictionaryRepresentation:v3];
  return v3;
}

- (unint64_t)hash
{
  if (!*(&self->_isThirdPartyRequest + 1))
  {
    uint64_t v2 = 0;
    if ((*(&self->_isThirdPartyRequest + 1) & 2) != 0) {
      goto LABEL_3;
    }
LABEL_6:
    uint64_t v3 = 0;
    if ((*(&self->_isThirdPartyRequest + 1) & 4) != 0) {
      goto LABEL_4;
    }
LABEL_7:
    uint64_t v4 = 0;
    return v3 ^ v2 ^ v4;
  }
  uint64_t v2 = 2654435761 * self->_isForegroundAppAvailable;
  if ((*(&self->_isThirdPartyRequest + 1) & 2) == 0) {
    goto LABEL_6;
  }
LABEL_3:
  uint64_t v3 = 2654435761 * self->_isRecommendationRequest;
  if ((*(&self->_isThirdPartyRequest + 1) & 4) == 0) {
    goto LABEL_7;
  }
LABEL_4:
  uint64_t v4 = 2654435761 * self->_isThirdPartyRequest;
  return v3 ^ v2 ^ v4;
}

- (BOOL)isEqual:(id)a3
{
  uint64_t v4 = (unsigned __int8 *)a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_14;
  }
  unsigned int v5 = *((unsigned __int8 *)&self->_isThirdPartyRequest + 1);
  unsigned int v6 = v4[11];
  if ((v5 & 1) != (v6 & 1)) {
    goto LABEL_14;
  }
  if (v5)
  {
    int isForegroundAppAvailable = self->_isForegroundAppAvailable;
    if (isForegroundAppAvailable != [v4 isForegroundAppAvailable]) {
      goto LABEL_14;
    }
    unsigned int v5 = *((unsigned __int8 *)&self->_isThirdPartyRequest + 1);
    unsigned int v6 = v4[11];
  }
  int v8 = (v5 >> 1) & 1;
  if (v8 != ((v6 >> 1) & 1)) {
    goto LABEL_14;
  }
  if (v8)
  {
    int isRecommendationRequest = self->_isRecommendationRequest;
    if (isRecommendationRequest == [v4 isRecommendationRequest])
    {
      unsigned int v5 = *((unsigned __int8 *)&self->_isThirdPartyRequest + 1);
      unsigned int v6 = v4[11];
      goto LABEL_10;
    }
LABEL_14:
    BOOL v12 = 0;
    goto LABEL_15;
  }
LABEL_10:
  int v10 = (v5 >> 2) & 1;
  if (v10 != ((v6 >> 2) & 1)) {
    goto LABEL_14;
  }
  if (v10)
  {
    int isThirdPartyRequest = self->_isThirdPartyRequest;
    if (isThirdPartyRequest != [v4 isThirdPartyRequest]) {
      goto LABEL_14;
    }
  }
  BOOL v12 = 1;
LABEL_15:

  return v12;
}

- (void)writeTo:(id)a3
{
  id v4 = a3;
  char v5 = *(&self->_isThirdPartyRequest + 1);
  id v6 = v4;
  if (v5)
  {
    PBDataWriterWriteBOOLField();
    id v4 = v6;
    char v5 = *(&self->_isThirdPartyRequest + 1);
    if ((v5 & 2) == 0)
    {
LABEL_3:
      if ((v5 & 4) == 0) {
        goto LABEL_5;
      }
      goto LABEL_4;
    }
  }
  else if ((*(&self->_isThirdPartyRequest + 1) & 2) == 0)
  {
    goto LABEL_3;
  }
  PBDataWriterWriteBOOLField();
  id v4 = v6;
  if ((*(&self->_isThirdPartyRequest + 1) & 4) != 0)
  {
LABEL_4:
    PBDataWriterWriteBOOLField();
    id v4 = v6;
  }
LABEL_5:
}

- (BOOL)readFrom:(id)a3
{
  return PEGASUSSchemaPEGASUSVideoExperiencePropertyReadFrom(self, (uint64_t)a3);
}

- (void)deleteIsThirdPartyRequest
{
  *(&self->_isThirdPartyRequest + 1) &= ~4u;
}

- (void)setHasIsThirdPartyRequest:(BOOL)a3
{
  if (a3) {
    char v3 = 4;
  }
  else {
    char v3 = 0;
  }
  *(&self->_isThirdPartyRequest + 1) = *(&self->_isThirdPartyRequest + 1) & 0xFB | v3;
}

- (BOOL)hasIsThirdPartyRequest
{
  return (*((unsigned __int8 *)&self->_isThirdPartyRequest + 1) >> 2) & 1;
}

- (void)setIsThirdPartyRequest:(BOOL)a3
{
  *(&self->_isThirdPartyRequest + 1) |= 4u;
  self->_int isThirdPartyRequest = a3;
}

- (void)deleteIsRecommendationRequest
{
  *(&self->_isThirdPartyRequest + 1) &= ~2u;
}

- (void)setHasIsRecommendationRequest:(BOOL)a3
{
  if (a3) {
    char v3 = 2;
  }
  else {
    char v3 = 0;
  }
  *(&self->_isThirdPartyRequest + 1) = *(&self->_isThirdPartyRequest + 1) & 0xFD | v3;
}

- (BOOL)hasIsRecommendationRequest
{
  return (*((unsigned __int8 *)&self->_isThirdPartyRequest + 1) >> 1) & 1;
}

- (void)setIsRecommendationRequest:(BOOL)a3
{
  *(&self->_isThirdPartyRequest + 1) |= 2u;
  self->_int isRecommendationRequest = a3;
}

- (void)deleteIsForegroundAppAvailable
{
  *(&self->_isThirdPartyRequest + 1) &= ~1u;
}

- (void)setHasIsForegroundAppAvailable:(BOOL)a3
{
  *(&self->_isThirdPartyRequest + 1) = *(&self->_isThirdPartyRequest + 1) & 0xFE | a3;
}

- (BOOL)hasIsForegroundAppAvailable
{
  return *(&self->_isThirdPartyRequest + 1);
}

- (void)setIsForegroundAppAvailable:(BOOL)a3
{
  *(&self->_isThirdPartyRequest + 1) |= 1u;
  self->_int isForegroundAppAvailable = a3;
}

@end