@interface PSESchemaPSEMediaEntitySimilarity
- (BOOL)hasIsSameMediaAlbum;
- (BOOL)hasIsSameMediaArtist;
- (BOOL)hasIsSameMediaItem;
- (BOOL)hasIsSameMediaTitle;
- (BOOL)isEqual:(id)a3;
- (BOOL)isSameMediaAlbum;
- (BOOL)isSameMediaArtist;
- (BOOL)isSameMediaItem;
- (BOOL)isSameMediaTitle;
- (BOOL)readFrom:(id)a3;
- (NSData)jsonData;
- (PSESchemaPSEMediaEntitySimilarity)initWithDictionary:(id)a3;
- (PSESchemaPSEMediaEntitySimilarity)initWithJSON:(id)a3;
- (id)dictionaryRepresentation;
- (id)suppressMessageUnderConditions;
- (unint64_t)hash;
- (void)deleteIsSameMediaAlbum;
- (void)deleteIsSameMediaArtist;
- (void)deleteIsSameMediaItem;
- (void)deleteIsSameMediaTitle;
- (void)setHasIsSameMediaAlbum:(BOOL)a3;
- (void)setHasIsSameMediaArtist:(BOOL)a3;
- (void)setHasIsSameMediaItem:(BOOL)a3;
- (void)setHasIsSameMediaTitle:(BOOL)a3;
- (void)setIsSameMediaAlbum:(BOOL)a3;
- (void)setIsSameMediaArtist:(BOOL)a3;
- (void)setIsSameMediaItem:(BOOL)a3;
- (void)setIsSameMediaTitle:(BOOL)a3;
- (void)writeTo:(id)a3;
@end

@implementation PSESchemaPSEMediaEntitySimilarity

- (id)suppressMessageUnderConditions
{
  v2 = objc_alloc_init(SISensitiveConditionsSet);
  [(SISensitiveConditionsSet *)v2 addCondition:1];
  return v2;
}

- (BOOL)isSameMediaItem
{
  return self->_isSameMediaItem;
}

- (BOOL)isSameMediaAlbum
{
  return self->_isSameMediaAlbum;
}

- (BOOL)isSameMediaArtist
{
  return self->_isSameMediaArtist;
}

- (BOOL)isSameMediaTitle
{
  return self->_isSameMediaTitle;
}

- (PSESchemaPSEMediaEntitySimilarity)initWithDictionary:(id)a3
{
  id v4 = a3;
  v12.receiver = self;
  v12.super_class = (Class)PSESchemaPSEMediaEntitySimilarity;
  v5 = [(PSESchemaPSEMediaEntitySimilarity *)&v12 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"isSameMediaTitle"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[PSESchemaPSEMediaEntitySimilarity setIsSameMediaTitle:](v5, "setIsSameMediaTitle:", [v6 BOOLValue]);
    }
    v7 = [v4 objectForKeyedSubscript:@"isSameMediaArtist"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[PSESchemaPSEMediaEntitySimilarity setIsSameMediaArtist:](v5, "setIsSameMediaArtist:", [v7 BOOLValue]);
    }
    v8 = [v4 objectForKeyedSubscript:@"isSameMediaAlbum"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[PSESchemaPSEMediaEntitySimilarity setIsSameMediaAlbum:](v5, "setIsSameMediaAlbum:", [v8 BOOLValue]);
    }
    v9 = [v4 objectForKeyedSubscript:@"isSameMediaItem"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[PSESchemaPSEMediaEntitySimilarity setIsSameMediaItem:](v5, "setIsSameMediaItem:", [v9 BOOLValue]);
    }
    v10 = v5;
  }
  return v5;
}

- (PSESchemaPSEMediaEntitySimilarity)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(PSESchemaPSEMediaEntitySimilarity *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (NSData)jsonData
{
  v2 = [(PSESchemaPSEMediaEntitySimilarity *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  char has = (char)self->_has;
  if ((has & 4) != 0)
  {
    uint64_t v7 = objc_msgSend(NSNumber, "numberWithBool:", -[PSESchemaPSEMediaEntitySimilarity isSameMediaAlbum](self, "isSameMediaAlbum"));
    [v3 setObject:v7 forKeyedSubscript:@"isSameMediaAlbum"];

    char has = (char)self->_has;
    if ((has & 2) == 0)
    {
LABEL_3:
      if ((has & 8) == 0) {
        goto LABEL_4;
      }
      goto LABEL_11;
    }
  }
  else if ((*(unsigned char *)&self->_has & 2) == 0)
  {
    goto LABEL_3;
  }
  v8 = objc_msgSend(NSNumber, "numberWithBool:", -[PSESchemaPSEMediaEntitySimilarity isSameMediaArtist](self, "isSameMediaArtist"));
  [v3 setObject:v8 forKeyedSubscript:@"isSameMediaArtist"];

  char has = (char)self->_has;
  if ((has & 8) == 0)
  {
LABEL_4:
    if ((has & 1) == 0) {
      goto LABEL_6;
    }
    goto LABEL_5;
  }
LABEL_11:
  v9 = objc_msgSend(NSNumber, "numberWithBool:", -[PSESchemaPSEMediaEntitySimilarity isSameMediaItem](self, "isSameMediaItem"));
  [v3 setObject:v9 forKeyedSubscript:@"isSameMediaItem"];

  if (*(unsigned char *)&self->_has)
  {
LABEL_5:
    v5 = objc_msgSend(NSNumber, "numberWithBool:", -[PSESchemaPSEMediaEntitySimilarity isSameMediaTitle](self, "isSameMediaTitle"));
    [v3 setObject:v5 forKeyedSubscript:@"isSameMediaTitle"];
  }
LABEL_6:
  [(SISchemaInstrumentationMessage *)self willProduceDictionaryRepresentation:v3];
  return v3;
}

- (unint64_t)hash
{
  if (*(unsigned char *)&self->_has)
  {
    uint64_t v2 = 2654435761 * self->_isSameMediaTitle;
    if ((*(unsigned char *)&self->_has & 2) != 0)
    {
LABEL_3:
      uint64_t v3 = 2654435761 * self->_isSameMediaArtist;
      if ((*(unsigned char *)&self->_has & 4) != 0) {
        goto LABEL_4;
      }
LABEL_8:
      uint64_t v4 = 0;
      if ((*(unsigned char *)&self->_has & 8) != 0) {
        goto LABEL_5;
      }
LABEL_9:
      uint64_t v5 = 0;
      return v3 ^ v2 ^ v4 ^ v5;
    }
  }
  else
  {
    uint64_t v2 = 0;
    if ((*(unsigned char *)&self->_has & 2) != 0) {
      goto LABEL_3;
    }
  }
  uint64_t v3 = 0;
  if ((*(unsigned char *)&self->_has & 4) == 0) {
    goto LABEL_8;
  }
LABEL_4:
  uint64_t v4 = 2654435761 * self->_isSameMediaAlbum;
  if ((*(unsigned char *)&self->_has & 8) == 0) {
    goto LABEL_9;
  }
LABEL_5:
  uint64_t v5 = 2654435761 * self->_isSameMediaItem;
  return v3 ^ v2 ^ v4 ^ v5;
}

- (BOOL)isEqual:(id)a3
{
  uint64_t v4 = (unsigned __int8 *)a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_18;
  }
  $24811BC63647D0A419F0F6D69D3E169A has = self->_has;
  unsigned int v6 = v4[12];
  if ((*(unsigned char *)&has & 1) != (v6 & 1)) {
    goto LABEL_18;
  }
  if (*(unsigned char *)&has)
  {
    int isSameMediaTitle = self->_isSameMediaTitle;
    if (isSameMediaTitle != [v4 isSameMediaTitle]) {
      goto LABEL_18;
    }
    $24811BC63647D0A419F0F6D69D3E169A has = self->_has;
    unsigned int v6 = v4[12];
  }
  int v8 = (*(unsigned int *)&has >> 1) & 1;
  if (v8 != ((v6 >> 1) & 1)) {
    goto LABEL_18;
  }
  if (v8)
  {
    int isSameMediaArtist = self->_isSameMediaArtist;
    if (isSameMediaArtist != [v4 isSameMediaArtist]) {
      goto LABEL_18;
    }
    $24811BC63647D0A419F0F6D69D3E169A has = self->_has;
    unsigned int v6 = v4[12];
  }
  int v10 = (*(unsigned int *)&has >> 2) & 1;
  if (v10 != ((v6 >> 2) & 1)) {
    goto LABEL_18;
  }
  if (v10)
  {
    int isSameMediaAlbum = self->_isSameMediaAlbum;
    if (isSameMediaAlbum == [v4 isSameMediaAlbum])
    {
      $24811BC63647D0A419F0F6D69D3E169A has = self->_has;
      unsigned int v6 = v4[12];
      goto LABEL_14;
    }
LABEL_18:
    BOOL v14 = 0;
    goto LABEL_19;
  }
LABEL_14:
  int v12 = (*(unsigned int *)&has >> 3) & 1;
  if (v12 != ((v6 >> 3) & 1)) {
    goto LABEL_18;
  }
  if (v12)
  {
    int isSameMediaItem = self->_isSameMediaItem;
    if (isSameMediaItem != [v4 isSameMediaItem]) {
      goto LABEL_18;
    }
  }
  BOOL v14 = 1;
LABEL_19:

  return v14;
}

- (void)writeTo:(id)a3
{
  id v5 = a3;
  char has = (char)self->_has;
  if (has)
  {
    PBDataWriterWriteBOOLField();
    char has = (char)self->_has;
    if ((has & 2) == 0)
    {
LABEL_3:
      if ((has & 4) == 0) {
        goto LABEL_4;
      }
      goto LABEL_11;
    }
  }
  else if ((*(unsigned char *)&self->_has & 2) == 0)
  {
    goto LABEL_3;
  }
  PBDataWriterWriteBOOLField();
  char has = (char)self->_has;
  if ((has & 4) == 0)
  {
LABEL_4:
    if ((has & 8) == 0) {
      goto LABEL_6;
    }
    goto LABEL_5;
  }
LABEL_11:
  PBDataWriterWriteBOOLField();
  if ((*(unsigned char *)&self->_has & 8) != 0) {
LABEL_5:
  }
    PBDataWriterWriteBOOLField();
LABEL_6:
}

- (BOOL)readFrom:(id)a3
{
  return PSESchemaPSEMediaEntitySimilarityReadFrom(self, (uint64_t)a3);
}

- (void)deleteIsSameMediaItem
{
  *(unsigned char *)&self->_has &= ~8u;
}

- (void)setHasIsSameMediaItem:(BOOL)a3
{
  if (a3) {
    char v3 = 8;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xF7 | v3;
}

- (BOOL)hasIsSameMediaItem
{
  return (*(unsigned char *)&self->_has >> 3) & 1;
}

- (void)setIsSameMediaItem:(BOOL)a3
{
  *(unsigned char *)&self->_has |= 8u;
  self->_int isSameMediaItem = a3;
}

- (void)deleteIsSameMediaAlbum
{
  *(unsigned char *)&self->_has &= ~4u;
}

- (void)setHasIsSameMediaAlbum:(BOOL)a3
{
  if (a3) {
    char v3 = 4;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFB | v3;
}

- (BOOL)hasIsSameMediaAlbum
{
  return (*(unsigned char *)&self->_has >> 2) & 1;
}

- (void)setIsSameMediaAlbum:(BOOL)a3
{
  *(unsigned char *)&self->_has |= 4u;
  self->_int isSameMediaAlbum = a3;
}

- (void)deleteIsSameMediaArtist
{
  *(unsigned char *)&self->_has &= ~2u;
}

- (void)setHasIsSameMediaArtist:(BOOL)a3
{
  if (a3) {
    char v3 = 2;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFD | v3;
}

- (BOOL)hasIsSameMediaArtist
{
  return (*(unsigned char *)&self->_has >> 1) & 1;
}

- (void)setIsSameMediaArtist:(BOOL)a3
{
  *(unsigned char *)&self->_has |= 2u;
  self->_int isSameMediaArtist = a3;
}

- (void)deleteIsSameMediaTitle
{
  *(unsigned char *)&self->_has &= ~1u;
}

- (void)setHasIsSameMediaTitle:(BOOL)a3
{
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFE | a3;
}

- (BOOL)hasIsSameMediaTitle
{
  return *(unsigned char *)&self->_has & 1;
}

- (void)setIsSameMediaTitle:(BOOL)a3
{
  *(unsigned char *)&self->_has |= 1u;
  self->_int isSameMediaTitle = a3;
}

@end