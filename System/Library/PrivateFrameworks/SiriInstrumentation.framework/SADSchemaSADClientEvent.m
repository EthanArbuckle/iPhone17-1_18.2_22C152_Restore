@interface SADSchemaSADClientEvent
+ (id)getInnerTypeStringByTag:(unint64_t)a3;
- (BOOL)hasAvailableAssetDailyStatus;
- (BOOL)hasImmediateDownloadTriggered;
- (BOOL)hasUodAssetsPrepared;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (NSData)jsonData;
- (SADSchemaSADAvailableAssetDailyStatus)availableAssetDailyStatus;
- (SADSchemaSADClientEvent)initWithDictionary:(id)a3;
- (SADSchemaSADClientEvent)initWithJSON:(id)a3;
- (SADSchemaSADImmediateDownloadTriggered)immediateDownloadTriggered;
- (SADSchemaSADUODAssetsPrepared)uodAssetsPrepared;
- (SISchemaInstrumentationMessage)innerEvent;
- (id)applySensitiveConditionsPolicy:(id)a3;
- (id)dictionaryRepresentation;
- (id)qualifiedMessageName;
- (id)suppressMessageUnderConditions;
- (int)getAnyEventType;
- (unint64_t)hash;
- (unint64_t)whichEvent_Type;
- (void)deleteAvailableAssetDailyStatus;
- (void)deleteImmediateDownloadTriggered;
- (void)deleteUodAssetsPrepared;
- (void)setAvailableAssetDailyStatus:(id)a3;
- (void)setHasAvailableAssetDailyStatus:(BOOL)a3;
- (void)setHasImmediateDownloadTriggered:(BOOL)a3;
- (void)setHasUodAssetsPrepared:(BOOL)a3;
- (void)setImmediateDownloadTriggered:(id)a3;
- (void)setUodAssetsPrepared:(id)a3;
- (void)writeTo:(id)a3;
@end

@implementation SADSchemaSADClientEvent

- (id)applySensitiveConditionsPolicy:(id)a3
{
  id v4 = a3;
  v16.receiver = self;
  v16.super_class = (Class)SADSchemaSADClientEvent;
  v5 = [(SISchemaInstrumentationMessage *)&v16 applySensitiveConditionsPolicy:v4];
  v6 = [(SADSchemaSADClientEvent *)self uodAssetsPrepared];
  v7 = [v6 applySensitiveConditionsPolicy:v4];
  int v8 = [v7 suppressMessage];

  if (v8) {
    [(SADSchemaSADClientEvent *)self deleteUodAssetsPrepared];
  }
  v9 = [(SADSchemaSADClientEvent *)self availableAssetDailyStatus];
  v10 = [v9 applySensitiveConditionsPolicy:v4];
  int v11 = [v10 suppressMessage];

  if (v11) {
    [(SADSchemaSADClientEvent *)self deleteAvailableAssetDailyStatus];
  }
  v12 = [(SADSchemaSADClientEvent *)self immediateDownloadTriggered];
  v13 = [v12 applySensitiveConditionsPolicy:v4];
  int v14 = [v13 suppressMessage];

  if (v14) {
    [(SADSchemaSADClientEvent *)self deleteImmediateDownloadTriggered];
  }

  return v5;
}

- (id)suppressMessageUnderConditions
{
  v2 = objc_alloc_init(SISensitiveConditionsSet);
  [(SISensitiveConditionsSet *)v2 addCondition:1];
  return v2;
}

- (SISchemaInstrumentationMessage)innerEvent
{
  unint64_t v3 = [(SADSchemaSADClientEvent *)self whichEvent_Type];
  if (v3 - 101 > 2) {
    id v4 = 0;
  }
  else {
    id v4 = *(id *)((char *)&self->super.super.super.super.isa + *off_1E5EABE68[v3 - 101]);
  }
  return (SISchemaInstrumentationMessage *)v4;
}

+ (id)getInnerTypeStringByTag:(unint64_t)a3
{
  if (a3 - 101 > 2) {
    return 0;
  }
  else {
    return off_1E5EABE80[a3 - 101];
  }
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_immediateDownloadTriggered, 0);
  objc_storeStrong((id *)&self->_availableAssetDailyStatus, 0);
  objc_storeStrong((id *)&self->_uodAssetsPrepared, 0);
}

- (void)setHasImmediateDownloadTriggered:(BOOL)a3
{
  self->_hasImmediateDownloadTriggered = a3;
}

- (BOOL)hasImmediateDownloadTriggered
{
  return self->_hasImmediateDownloadTriggered;
}

- (void)setHasAvailableAssetDailyStatus:(BOOL)a3
{
  self->_hasAvailableAssetDailyStatus = a3;
}

- (BOOL)hasAvailableAssetDailyStatus
{
  return self->_hasAvailableAssetDailyStatus;
}

- (void)setHasUodAssetsPrepared:(BOOL)a3
{
  self->_hasUodAssetsPrepared = a3;
}

- (BOOL)hasUodAssetsPrepared
{
  return self->_hasUodAssetsPrepared;
}

- (unint64_t)whichEvent_Type
{
  return self->_whichEvent_Type;
}

- (SADSchemaSADClientEvent)initWithDictionary:(id)a3
{
  id v4 = a3;
  v14.receiver = self;
  v14.super_class = (Class)SADSchemaSADClientEvent;
  v5 = [(SADSchemaSADClientEvent *)&v14 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"uodAssetsPrepared"];
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      v7 = [[SADSchemaSADUODAssetsPrepared alloc] initWithDictionary:v6];
      [(SADSchemaSADClientEvent *)v5 setUodAssetsPrepared:v7];
    }
    int v8 = [v4 objectForKeyedSubscript:@"availableAssetDailyStatus"];
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      v9 = [[SADSchemaSADAvailableAssetDailyStatus alloc] initWithDictionary:v8];
      [(SADSchemaSADClientEvent *)v5 setAvailableAssetDailyStatus:v9];
    }
    v10 = [v4 objectForKeyedSubscript:@"immediateDownloadTriggered"];
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      int v11 = [[SADSchemaSADImmediateDownloadTriggered alloc] initWithDictionary:v10];
      [(SADSchemaSADClientEvent *)v5 setImmediateDownloadTriggered:v11];
    }
    v12 = v5;
  }
  return v5;
}

- (SADSchemaSADClientEvent)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(SADSchemaSADClientEvent *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (NSData)jsonData
{
  v2 = [(SADSchemaSADClientEvent *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    unint64_t v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    unint64_t v3 = 0;
  }

  return (NSData *)v3;
}

- (id)dictionaryRepresentation
{
  unint64_t v3 = [MEMORY[0x1E4F1CA60] dictionary];
  if (self->_availableAssetDailyStatus)
  {
    id v4 = [(SADSchemaSADClientEvent *)self availableAssetDailyStatus];
    v5 = [v4 dictionaryRepresentation];
    if (v5)
    {
      [v3 setObject:v5 forKeyedSubscript:@"availableAssetDailyStatus"];
    }
    else
    {
      v6 = [MEMORY[0x1E4F1CA98] null];
      [v3 setObject:v6 forKeyedSubscript:@"availableAssetDailyStatus"];
    }
  }
  if (self->_immediateDownloadTriggered)
  {
    uint64_t v7 = [(SADSchemaSADClientEvent *)self immediateDownloadTriggered];
    int v8 = [v7 dictionaryRepresentation];
    if (v8)
    {
      [v3 setObject:v8 forKeyedSubscript:@"immediateDownloadTriggered"];
    }
    else
    {
      v9 = [MEMORY[0x1E4F1CA98] null];
      [v3 setObject:v9 forKeyedSubscript:@"immediateDownloadTriggered"];
    }
  }
  if (self->_uodAssetsPrepared)
  {
    v10 = [(SADSchemaSADClientEvent *)self uodAssetsPrepared];
    int v11 = [v10 dictionaryRepresentation];
    if (v11)
    {
      [v3 setObject:v11 forKeyedSubscript:@"uodAssetsPrepared"];
    }
    else
    {
      v12 = [MEMORY[0x1E4F1CA98] null];
      [v3 setObject:v12 forKeyedSubscript:@"uodAssetsPrepared"];
    }
  }
  [(SISchemaInstrumentationMessage *)self willProduceDictionaryRepresentation:v3];
  return v3;
}

- (unint64_t)hash
{
  unint64_t v3 = [(SADSchemaSADUODAssetsPrepared *)self->_uodAssetsPrepared hash];
  unint64_t v4 = [(SADSchemaSADAvailableAssetDailyStatus *)self->_availableAssetDailyStatus hash] ^ v3;
  return v4 ^ [(SADSchemaSADImmediateDownloadTriggered *)self->_immediateDownloadTriggered hash];
}

- (BOOL)isEqual:(id)a3
{
  id v4 = a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_18;
  }
  unint64_t whichEvent_Type = self->_whichEvent_Type;
  if (whichEvent_Type != objc_msgSend(v4, "whichEvent_Type")) {
    goto LABEL_18;
  }
  v6 = [(SADSchemaSADClientEvent *)self uodAssetsPrepared];
  uint64_t v7 = [v4 uodAssetsPrepared];
  if ((v6 == 0) == (v7 != 0)) {
    goto LABEL_17;
  }
  uint64_t v8 = [(SADSchemaSADClientEvent *)self uodAssetsPrepared];
  if (v8)
  {
    v9 = (void *)v8;
    v10 = [(SADSchemaSADClientEvent *)self uodAssetsPrepared];
    int v11 = [v4 uodAssetsPrepared];
    int v12 = [v10 isEqual:v11];

    if (!v12) {
      goto LABEL_18;
    }
  }
  else
  {
  }
  v6 = [(SADSchemaSADClientEvent *)self availableAssetDailyStatus];
  uint64_t v7 = [v4 availableAssetDailyStatus];
  if ((v6 == 0) == (v7 != 0)) {
    goto LABEL_17;
  }
  uint64_t v13 = [(SADSchemaSADClientEvent *)self availableAssetDailyStatus];
  if (v13)
  {
    objc_super v14 = (void *)v13;
    v15 = [(SADSchemaSADClientEvent *)self availableAssetDailyStatus];
    objc_super v16 = [v4 availableAssetDailyStatus];
    int v17 = [v15 isEqual:v16];

    if (!v17) {
      goto LABEL_18;
    }
  }
  else
  {
  }
  v6 = [(SADSchemaSADClientEvent *)self immediateDownloadTriggered];
  uint64_t v7 = [v4 immediateDownloadTriggered];
  if ((v6 == 0) != (v7 != 0))
  {
    uint64_t v18 = [(SADSchemaSADClientEvent *)self immediateDownloadTriggered];
    if (!v18)
    {

LABEL_21:
      BOOL v23 = 1;
      goto LABEL_19;
    }
    v19 = (void *)v18;
    v20 = [(SADSchemaSADClientEvent *)self immediateDownloadTriggered];
    v21 = [v4 immediateDownloadTriggered];
    char v22 = [v20 isEqual:v21];

    if (v22) {
      goto LABEL_21;
    }
  }
  else
  {
LABEL_17:
  }
LABEL_18:
  BOOL v23 = 0;
LABEL_19:

  return v23;
}

- (void)writeTo:(id)a3
{
  id v10 = a3;
  id v4 = [(SADSchemaSADClientEvent *)self uodAssetsPrepared];

  if (v4)
  {
    v5 = [(SADSchemaSADClientEvent *)self uodAssetsPrepared];
    PBDataWriterWriteSubmessage();
  }
  v6 = [(SADSchemaSADClientEvent *)self availableAssetDailyStatus];

  if (v6)
  {
    uint64_t v7 = [(SADSchemaSADClientEvent *)self availableAssetDailyStatus];
    PBDataWriterWriteSubmessage();
  }
  uint64_t v8 = [(SADSchemaSADClientEvent *)self immediateDownloadTriggered];

  if (v8)
  {
    v9 = [(SADSchemaSADClientEvent *)self immediateDownloadTriggered];
    PBDataWriterWriteSubmessage();
  }
}

- (BOOL)readFrom:(id)a3
{
  return SADSchemaSADClientEventReadFrom(self, (uint64_t)a3);
}

- (void)deleteImmediateDownloadTriggered
{
  if (self->_whichEvent_Type == 103)
  {
    self->_unint64_t whichEvent_Type = 0;
    self->_immediateDownloadTriggered = 0;
    MEMORY[0x1F41817F8]();
  }
}

- (SADSchemaSADImmediateDownloadTriggered)immediateDownloadTriggered
{
  if (self->_whichEvent_Type == 103) {
    v2 = self->_immediateDownloadTriggered;
  }
  else {
    v2 = 0;
  }
  return v2;
}

- (void)setImmediateDownloadTriggered:(id)a3
{
  id v4 = (SADSchemaSADImmediateDownloadTriggered *)a3;
  uodAssetsPrepared = self->_uodAssetsPrepared;
  self->_uodAssetsPrepared = 0;

  availableAssetDailyStatus = self->_availableAssetDailyStatus;
  self->_availableAssetDailyStatus = 0;

  unint64_t v7 = 103;
  if (!v4) {
    unint64_t v7 = 0;
  }
  self->_unint64_t whichEvent_Type = v7;
  immediateDownloadTriggered = self->_immediateDownloadTriggered;
  self->_immediateDownloadTriggered = v4;
}

- (void)deleteAvailableAssetDailyStatus
{
  if (self->_whichEvent_Type == 102)
  {
    self->_unint64_t whichEvent_Type = 0;
    self->_availableAssetDailyStatus = 0;
    MEMORY[0x1F41817F8]();
  }
}

- (SADSchemaSADAvailableAssetDailyStatus)availableAssetDailyStatus
{
  if (self->_whichEvent_Type == 102) {
    v2 = self->_availableAssetDailyStatus;
  }
  else {
    v2 = 0;
  }
  return v2;
}

- (void)setAvailableAssetDailyStatus:(id)a3
{
  id v4 = (SADSchemaSADAvailableAssetDailyStatus *)a3;
  uodAssetsPrepared = self->_uodAssetsPrepared;
  self->_uodAssetsPrepared = 0;

  immediateDownloadTriggered = self->_immediateDownloadTriggered;
  self->_immediateDownloadTriggered = 0;

  unint64_t v7 = 102;
  if (!v4) {
    unint64_t v7 = 0;
  }
  self->_unint64_t whichEvent_Type = v7;
  availableAssetDailyStatus = self->_availableAssetDailyStatus;
  self->_availableAssetDailyStatus = v4;
}

- (void)deleteUodAssetsPrepared
{
  if (self->_whichEvent_Type == 101)
  {
    self->_unint64_t whichEvent_Type = 0;
    self->_uodAssetsPrepared = 0;
    MEMORY[0x1F41817F8]();
  }
}

- (SADSchemaSADUODAssetsPrepared)uodAssetsPrepared
{
  if (self->_whichEvent_Type == 101) {
    v2 = self->_uodAssetsPrepared;
  }
  else {
    v2 = 0;
  }
  return v2;
}

- (void)setUodAssetsPrepared:(id)a3
{
  id v4 = (SADSchemaSADUODAssetsPrepared *)a3;
  availableAssetDailyStatus = self->_availableAssetDailyStatus;
  self->_availableAssetDailyStatus = 0;

  immediateDownloadTriggered = self->_immediateDownloadTriggered;
  self->_immediateDownloadTriggered = 0;

  unint64_t v7 = 101;
  if (!v4) {
    unint64_t v7 = 0;
  }
  self->_unint64_t whichEvent_Type = v7;
  uodAssetsPrepared = self->_uodAssetsPrepared;
  self->_uodAssetsPrepared = v4;
}

- (id)qualifiedMessageName
{
  unint64_t v2 = [(SADSchemaSADClientEvent *)self whichEvent_Type];
  if (v2 - 101 > 2) {
    return @"com.apple.aiml.siri.sad.SADClientEvent";
  }
  else {
    return off_1E5EB4C18[v2 - 101];
  }
}

- (int)getAnyEventType
{
  return 33;
}

@end