...
- (NSData)jsonData
{
  v2 = [(ORCHSchemaORCHNLClassicDeprecationFlags *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  char v4 = *(&self->_isDomainServerFallbackDisabled + 1);
  if ((v4 & 2) != 0)
  {
    v5 = objc_msgSend(NSNumber, "numberWithBool:", -[ORCHSchemaORCHNLClassicDeprecationFlags isDomainServerFallbackDisabled](self, "isDomainServerFallbackDisabled"));
    [v3 setObject:v5 forKeyedSubscript:@"isDomainServerFallbackDisabled"];

    char v4 = *(&self->_isDomainServerFallbackDisabled + 1);
  }
  if (v4)
  {
    v6 = objc_msgSend(NSNumber, "numberWithBool:", -[ORCHSchemaORCHNLClassicDeprecationFlags isNLServerFallbackDisabled](self, "isNLServerFallbackDisabled"));
    [v3 setObject:v6 forKeyedSubscript:@"isNLServerFallbackDisabled"];
  }
...
- (unint64_t)hash
{
  ...
  {
    uint64_t v2 = 2654435761 * self->_isNLServerFallbackDisabled;
...
LABEL_3:
  uint64_t v3 = 2654435761 * self->_isDomainServerFallbackDisabled;
  return v3 ^ v2;
}
...
- (BOOL)isEqual:(id)a3
{
  ...
  unsigned int v5 = *((unsigned __int8 *)&self->_isDomainServerFallbackDisabled + 1);
  ...
  if (v5)
  {
    int isNLServerFallbackDisabled = self->_isNLServerFallbackDisabled;
    if (isNLServerFallbackDisabled != [v4 isNLServerFallbackDisabled])
    {
LABEL_10:
      BOOL v10 = 0;
      goto LABEL_11;
  ...
  if (v8)
  {
    int isDomainServerFallbackDisabled = self->_isDomainServerFallbackDisabled;
    if (isDomainServerFallbackDisabled != [v4 isDomainServerFallbackDisabled]) {
      goto LABEL_10;
    }
  }
  BOOL v10 = 1;
...
{
  id v4 = a3;
  char v5 = *(&self->_isDomainServerFallbackDisabled + 1);
  id v6 = v4;
  if (v5)
  {
...
{
  *(&self->_isDomainServerFallbackDisabled + 1) |= 2u;
  self->_int isDomainServerFallbackDisabled = a3;
}
...
{
  *(&self->_isDomainServerFallbackDisabled + 1) |= 1u;
  self->_int isNLServerFallbackDisabled = a3;
}
