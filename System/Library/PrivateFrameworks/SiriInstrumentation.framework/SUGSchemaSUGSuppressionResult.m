@interface SUGSchemaSUGSuppressionResult
- (BOOL)hasSuppressionReason;
- (BOOL)hasWasSuppressed;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (BOOL)wasSuppressed;
- (NSData)jsonData;
- (SUGSchemaSUGSuppressionResult)initWithDictionary:(id)a3;
- (SUGSchemaSUGSuppressionResult)initWithJSON:(id)a3;
- (id)dictionaryRepresentation;
- (id)suppressMessageUnderConditions;
- (int)suppressionReason;
- (unint64_t)hash;
- (void)deleteSuppressionReason;
- (void)deleteWasSuppressed;
- (void)setHasSuppressionReason:(BOOL)a3;
- (void)setHasWasSuppressed:(BOOL)a3;
- (void)setSuppressionReason:(int)a3;
- (void)setWasSuppressed:(BOOL)a3;
- (void)writeTo:(id)a3;
@end

@implementation SUGSchemaSUGSuppressionResult

- (id)suppressMessageUnderConditions
{
  v2 = objc_alloc_init(SISensitiveConditionsSet);
  [(SISensitiveConditionsSet *)v2 addCondition:1];
  return v2;
}

- (int)suppressionReason
{
  return self->_suppressionReason;
}

- (BOOL)wasSuppressed
{
  return self->_wasSuppressed;
}

- (SUGSchemaSUGSuppressionResult)initWithDictionary:(id)a3
{
  id v4 = a3;
  v10.receiver = self;
  v10.super_class = (Class)SUGSchemaSUGSuppressionResult;
  v5 = [(SUGSchemaSUGSuppressionResult *)&v10 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"wasSuppressed"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[SUGSchemaSUGSuppressionResult setWasSuppressed:](v5, "setWasSuppressed:", [v6 BOOLValue]);
    }
    v7 = [v4 objectForKeyedSubscript:@"suppressionReason"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[SUGSchemaSUGSuppressionResult setSuppressionReason:](v5, "setSuppressionReason:", [v7 intValue]);
    }
    v8 = v5;
  }
  return v5;
}

- (SUGSchemaSUGSuppressionResult)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(SUGSchemaSUGSuppressionResult *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (NSData)jsonData
{
  v2 = [(SUGSchemaSUGSuppressionResult *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  char has = (char)self->_has;
  if ((has & 2) != 0)
  {
    unsigned int v5 = [(SUGSchemaSUGSuppressionResult *)self suppressionReason] - 1;
    if (v5 > 0xB) {
      v6 = @"SUGSUPPRESSIONREASON_UNKNOWN";
    }
    else {
      v6 = off_1E5EBAF88[v5];
    }
    [v3 setObject:v6 forKeyedSubscript:@"suppressionReason"];
    char has = (char)self->_has;
  }
  if (has)
  {
    uint64_t v7 = objc_msgSend(NSNumber, "numberWithBool:", -[SUGSchemaSUGSuppressionResult wasSuppressed](self, "wasSuppressed"));
    [v3 setObject:v7 forKeyedSubscript:@"wasSuppressed"];
  }
  [(SISchemaInstrumentationMessage *)self willProduceDictionaryRepresentation:v3];
  return v3;
}

- (unint64_t)hash
{
  if (*(unsigned char *)&self->_has)
  {
    uint64_t v2 = 2654435761 * self->_wasSuppressed;
    if ((*(unsigned char *)&self->_has & 2) != 0) {
      goto LABEL_3;
    }
LABEL_5:
    uint64_t v3 = 0;
    return v3 ^ v2;
  }
  uint64_t v2 = 0;
  if ((*(unsigned char *)&self->_has & 2) == 0) {
    goto LABEL_5;
  }
LABEL_3:
  uint64_t v3 = 2654435761 * self->_suppressionReason;
  return v3 ^ v2;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = (unsigned __int8 *)a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_10;
  }
  $CB8788A50CCDFBB5927227C680FE78CF has = self->_has;
  unsigned int v6 = v4[16];
  if ((*(unsigned char *)&has & 1) != (v6 & 1)) {
    goto LABEL_10;
  }
  if (*(unsigned char *)&has)
  {
    int wasSuppressed = self->_wasSuppressed;
    if (wasSuppressed != [v4 wasSuppressed])
    {
LABEL_10:
      BOOL v10 = 0;
      goto LABEL_11;
    }
    $CB8788A50CCDFBB5927227C680FE78CF has = self->_has;
    unsigned int v6 = v4[16];
  }
  int v8 = (*(unsigned int *)&has >> 1) & 1;
  if (v8 != ((v6 >> 1) & 1)) {
    goto LABEL_10;
  }
  if (v8)
  {
    int suppressionReason = self->_suppressionReason;
    if (suppressionReason != [v4 suppressionReason]) {
      goto LABEL_10;
    }
  }
  BOOL v10 = 1;
LABEL_11:

  return v10;
}

- (void)writeTo:(id)a3
{
  id v4 = a3;
  char has = (char)self->_has;
  id v6 = v4;
  if (has)
  {
    PBDataWriterWriteBOOLField();
    id v4 = v6;
    char has = (char)self->_has;
  }
  if ((has & 2) != 0)
  {
    PBDataWriterWriteInt32Field();
    id v4 = v6;
  }
}

- (BOOL)readFrom:(id)a3
{
  return SUGSchemaSUGSuppressionResultReadFrom(self, (uint64_t)a3);
}

- (void)deleteSuppressionReason
{
  *(unsigned char *)&self->_has &= ~2u;
}

- (void)setHasSuppressionReason:(BOOL)a3
{
  if (a3) {
    char v3 = 2;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFD | v3;
}

- (BOOL)hasSuppressionReason
{
  return (*(unsigned char *)&self->_has >> 1) & 1;
}

- (void)setSuppressionReason:(int)a3
{
  *(unsigned char *)&self->_has |= 2u;
  self->_int suppressionReason = a3;
}

- (void)deleteWasSuppressed
{
  *(unsigned char *)&self->_has &= ~1u;
}

- (void)setHasWasSuppressed:(BOOL)a3
{
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFE | a3;
}

- (BOOL)hasWasSuppressed
{
  return *(unsigned char *)&self->_has & 1;
}

- (void)setWasSuppressed:(BOOL)a3
{
  *(unsigned char *)&self->_has |= 1u;
  self->_int wasSuppressed = a3;
}

@end