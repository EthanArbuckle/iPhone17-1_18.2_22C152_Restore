@interface FLOWSchemaFLOWNotificationContext
- (BOOL)hasIsHighlight;
- (BOOL)hasIsLongNotification;
- (BOOL)hasIsSummarized;
- (BOOL)isEqual:(id)a3;
- (BOOL)isHighlight;
- (BOOL)isLongNotification;
- (BOOL)isSummarized;
- (BOOL)readFrom:(id)a3;
- (FLOWSchemaFLOWNotificationContext)initWithDictionary:(id)a3;
- (FLOWSchemaFLOWNotificationContext)initWithJSON:(id)a3;
- (NSData)jsonData;
- (id)dictionaryRepresentation;
- (id)suppressMessageUnderConditions;
- (unint64_t)hash;
- (void)deleteIsHighlight;
- (void)deleteIsLongNotification;
- (void)deleteIsSummarized;
- (void)setHasIsHighlight:(BOOL)a3;
- (void)setHasIsLongNotification:(BOOL)a3;
- (void)setHasIsSummarized:(BOOL)a3;
- (void)setIsHighlight:(BOOL)a3;
- (void)setIsLongNotification:(BOOL)a3;
- (void)setIsSummarized:(BOOL)a3;
- (void)writeTo:(id)a3;
@end

@implementation FLOWSchemaFLOWNotificationContext

- (id)suppressMessageUnderConditions
{
  v2 = objc_alloc_init(SISensitiveConditionsSet);
  [(SISensitiveConditionsSet *)v2 addCondition:1];
  return v2;
}

- (BOOL)isHighlight
{
  return self->_isHighlight;
}

- (BOOL)isSummarized
{
  return self->_isSummarized;
}

- (BOOL)isLongNotification
{
  return self->_isLongNotification;
}

- (FLOWSchemaFLOWNotificationContext)initWithDictionary:(id)a3
{
  id v4 = a3;
  v11.receiver = self;
  v11.super_class = (Class)FLOWSchemaFLOWNotificationContext;
  v5 = [(FLOWSchemaFLOWNotificationContext *)&v11 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"isLongNotification"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[FLOWSchemaFLOWNotificationContext setIsLongNotification:](v5, "setIsLongNotification:", [v6 BOOLValue]);
    }
    v7 = [v4 objectForKeyedSubscript:@"isSummarized"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[FLOWSchemaFLOWNotificationContext setIsSummarized:](v5, "setIsSummarized:", [v7 BOOLValue]);
    }
    v8 = [v4 objectForKeyedSubscript:@"isHighlight"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[FLOWSchemaFLOWNotificationContext setIsHighlight:](v5, "setIsHighlight:", [v8 BOOLValue]);
    }
    v9 = v5;
  }
  return v5;
}

- (FLOWSchemaFLOWNotificationContext)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(FLOWSchemaFLOWNotificationContext *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (NSData)jsonData
{
  v2 = [(FLOWSchemaFLOWNotificationContext *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  char v4 = *(&self->_isHighlight + 1);
  if ((v4 & 4) != 0)
  {
    uint64_t v7 = objc_msgSend(NSNumber, "numberWithBool:", -[FLOWSchemaFLOWNotificationContext isHighlight](self, "isHighlight"));
    [v3 setObject:v7 forKeyedSubscript:@"isHighlight"];

    char v4 = *(&self->_isHighlight + 1);
    if ((v4 & 1) == 0)
    {
LABEL_3:
      if ((v4 & 2) == 0) {
        goto LABEL_5;
      }
      goto LABEL_4;
    }
  }
  else if (!*(&self->_isHighlight + 1))
  {
    goto LABEL_3;
  }
  v8 = objc_msgSend(NSNumber, "numberWithBool:", -[FLOWSchemaFLOWNotificationContext isLongNotification](self, "isLongNotification"));
  [v3 setObject:v8 forKeyedSubscript:@"isLongNotification"];

  if ((*(&self->_isHighlight + 1) & 2) != 0)
  {
LABEL_4:
    v5 = objc_msgSend(NSNumber, "numberWithBool:", -[FLOWSchemaFLOWNotificationContext isSummarized](self, "isSummarized"));
    [v3 setObject:v5 forKeyedSubscript:@"isSummarized"];
  }
LABEL_5:
  [(SISchemaInstrumentationMessage *)self willProduceDictionaryRepresentation:v3];
  return v3;
}

- (unint64_t)hash
{
  if (!*(&self->_isHighlight + 1))
  {
    uint64_t v2 = 0;
    if ((*(&self->_isHighlight + 1) & 2) != 0) {
      goto LABEL_3;
    }
LABEL_6:
    uint64_t v3 = 0;
    if ((*(&self->_isHighlight + 1) & 4) != 0) {
      goto LABEL_4;
    }
LABEL_7:
    uint64_t v4 = 0;
    return v3 ^ v2 ^ v4;
  }
  uint64_t v2 = 2654435761 * self->_isLongNotification;
  if ((*(&self->_isHighlight + 1) & 2) == 0) {
    goto LABEL_6;
  }
LABEL_3:
  uint64_t v3 = 2654435761 * self->_isSummarized;
  if ((*(&self->_isHighlight + 1) & 4) == 0) {
    goto LABEL_7;
  }
LABEL_4:
  uint64_t v4 = 2654435761 * self->_isHighlight;
  return v3 ^ v2 ^ v4;
}

- (BOOL)isEqual:(id)a3
{
  uint64_t v4 = (unsigned __int8 *)a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_14;
  }
  unsigned int v5 = *((unsigned __int8 *)&self->_isHighlight + 1);
  unsigned int v6 = v4[11];
  if ((v5 & 1) != (v6 & 1)) {
    goto LABEL_14;
  }
  if (v5)
  {
    int isLongNotification = self->_isLongNotification;
    if (isLongNotification != [v4 isLongNotification]) {
      goto LABEL_14;
    }
    unsigned int v5 = *((unsigned __int8 *)&self->_isHighlight + 1);
    unsigned int v6 = v4[11];
  }
  int v8 = (v5 >> 1) & 1;
  if (v8 != ((v6 >> 1) & 1)) {
    goto LABEL_14;
  }
  if (v8)
  {
    int isSummarized = self->_isSummarized;
    if (isSummarized == [v4 isSummarized])
    {
      unsigned int v5 = *((unsigned __int8 *)&self->_isHighlight + 1);
      unsigned int v6 = v4[11];
      goto LABEL_10;
    }
LABEL_14:
    BOOL v12 = 0;
    goto LABEL_15;
  }
LABEL_10:
  int v10 = (v5 >> 2) & 1;
  if (v10 != ((v6 >> 2) & 1)) {
    goto LABEL_14;
  }
  if (v10)
  {
    int isHighlight = self->_isHighlight;
    if (isHighlight != [v4 isHighlight]) {
      goto LABEL_14;
    }
  }
  BOOL v12 = 1;
LABEL_15:

  return v12;
}

- (void)writeTo:(id)a3
{
  id v4 = a3;
  char v5 = *(&self->_isHighlight + 1);
  id v6 = v4;
  if (v5)
  {
    PBDataWriterWriteBOOLField();
    id v4 = v6;
    char v5 = *(&self->_isHighlight + 1);
    if ((v5 & 2) == 0)
    {
LABEL_3:
      if ((v5 & 4) == 0) {
        goto LABEL_5;
      }
      goto LABEL_4;
    }
  }
  else if ((*(&self->_isHighlight + 1) & 2) == 0)
  {
    goto LABEL_3;
  }
  PBDataWriterWriteBOOLField();
  id v4 = v6;
  if ((*(&self->_isHighlight + 1) & 4) != 0)
  {
LABEL_4:
    PBDataWriterWriteBOOLField();
    id v4 = v6;
  }
LABEL_5:
}

- (BOOL)readFrom:(id)a3
{
  return FLOWSchemaFLOWNotificationContextReadFrom(self, (uint64_t)a3);
}

- (void)deleteIsHighlight
{
  *(&self->_isHighlight + 1) &= ~4u;
}

- (void)setHasIsHighlight:(BOOL)a3
{
  if (a3) {
    char v3 = 4;
  }
  else {
    char v3 = 0;
  }
  *(&self->_isHighlight + 1) = *(&self->_isHighlight + 1) & 0xFB | v3;
}

- (BOOL)hasIsHighlight
{
  return (*((unsigned __int8 *)&self->_isHighlight + 1) >> 2) & 1;
}

- (void)setIsHighlight:(BOOL)a3
{
  *(&self->_isHighlight + 1) |= 4u;
  self->_int isHighlight = a3;
}

- (void)deleteIsSummarized
{
  *(&self->_isHighlight + 1) &= ~2u;
}

- (void)setHasIsSummarized:(BOOL)a3
{
  if (a3) {
    char v3 = 2;
  }
  else {
    char v3 = 0;
  }
  *(&self->_isHighlight + 1) = *(&self->_isHighlight + 1) & 0xFD | v3;
}

- (BOOL)hasIsSummarized
{
  return (*((unsigned __int8 *)&self->_isHighlight + 1) >> 1) & 1;
}

- (void)setIsSummarized:(BOOL)a3
{
  *(&self->_isHighlight + 1) |= 2u;
  self->_int isSummarized = a3;
}

- (void)deleteIsLongNotification
{
  *(&self->_isHighlight + 1) &= ~1u;
}

- (void)setHasIsLongNotification:(BOOL)a3
{
  *(&self->_isHighlight + 1) = *(&self->_isHighlight + 1) & 0xFE | a3;
}

- (BOOL)hasIsLongNotification
{
  return *(&self->_isHighlight + 1);
}

- (void)setIsLongNotification:(BOOL)a3
{
  *(&self->_isHighlight + 1) |= 1u;
  self->_int isLongNotification = a3;
}

@end