@interface NETSchemaNETSessionConnectionHttpHeaderCreated
- (BOOL)hasAceHost;
- (BOOL)hasUserAgent;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (NETSchemaNETSessionConnectionHttpHeaderCreated)initWithDictionary:(id)a3;
- (NETSchemaNETSessionConnectionHttpHeaderCreated)initWithJSON:(id)a3;
- (NSData)jsonData;
- (NSString)aceHost;
- (NSString)userAgent;
- (id)dictionaryRepresentation;
- (id)suppressMessageUnderConditions;
- (unint64_t)hash;
- (void)deleteAceHost;
- (void)deleteUserAgent;
- (void)setAceHost:(id)a3;
- (void)setHasAceHost:(BOOL)a3;
- (void)setHasUserAgent:(BOOL)a3;
- (void)setUserAgent:(id)a3;
- (void)writeTo:(id)a3;
@end

@implementation NETSchemaNETSessionConnectionHttpHeaderCreated

- (id)suppressMessageUnderConditions
{
  v2 = objc_alloc_init(SISensitiveConditionsSet);
  [(SISensitiveConditionsSet *)v2 addCondition:1];
  return v2;
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_aceHost, 0);
  objc_storeStrong((id *)&self->_userAgent, 0);
}

- (void)setHasAceHost:(BOOL)a3
{
  self->_hasAceHost = a3;
}

- (void)setHasUserAgent:(BOOL)a3
{
  self->_hasUserAgent = a3;
}

- (void)setAceHost:(id)a3
{
}

- (NSString)aceHost
{
  return self->_aceHost;
}

- (void)setUserAgent:(id)a3
{
}

- (NSString)userAgent
{
  return self->_userAgent;
}

- (NETSchemaNETSessionConnectionHttpHeaderCreated)initWithDictionary:(id)a3
{
  id v4 = a3;
  v12.receiver = self;
  v12.super_class = (Class)NETSchemaNETSessionConnectionHttpHeaderCreated;
  v5 = [(NETSchemaNETSessionConnectionHttpHeaderCreated *)&v12 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"userAgent"];
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      v7 = (void *)[v6 copy];
      [(NETSchemaNETSessionConnectionHttpHeaderCreated *)v5 setUserAgent:v7];
    }
    v8 = [v4 objectForKeyedSubscript:@"aceHost"];
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      v9 = (void *)[v8 copy];
      [(NETSchemaNETSessionConnectionHttpHeaderCreated *)v5 setAceHost:v9];
    }
    v10 = v5;
  }
  return v5;
}

- (NETSchemaNETSessionConnectionHttpHeaderCreated)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(NETSchemaNETSessionConnectionHttpHeaderCreated *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (NSData)jsonData
{
  v2 = [(NETSchemaNETSessionConnectionHttpHeaderCreated *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  if (self->_aceHost)
  {
    id v4 = [(NETSchemaNETSessionConnectionHttpHeaderCreated *)self aceHost];
    v5 = (void *)[v4 copy];
    [v3 setObject:v5 forKeyedSubscript:@"aceHost"];
  }
  if (self->_userAgent)
  {
    v6 = [(NETSchemaNETSessionConnectionHttpHeaderCreated *)self userAgent];
    uint64_t v7 = (void *)[v6 copy];
    [v3 setObject:v7 forKeyedSubscript:@"userAgent"];
  }
  [(SISchemaInstrumentationMessage *)self willProduceDictionaryRepresentation:v3];
  return v3;
}

- (unint64_t)hash
{
  NSUInteger v3 = [(NSString *)self->_userAgent hash];
  return [(NSString *)self->_aceHost hash] ^ v3;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_12;
  }
  v5 = [(NETSchemaNETSessionConnectionHttpHeaderCreated *)self userAgent];
  v6 = [v4 userAgent];
  if ((v5 == 0) == (v6 != 0)) {
    goto LABEL_11;
  }
  uint64_t v7 = [(NETSchemaNETSessionConnectionHttpHeaderCreated *)self userAgent];
  if (v7)
  {
    v8 = (void *)v7;
    v9 = [(NETSchemaNETSessionConnectionHttpHeaderCreated *)self userAgent];
    v10 = [v4 userAgent];
    int v11 = [v9 isEqual:v10];

    if (!v11) {
      goto LABEL_12;
    }
  }
  else
  {
  }
  v5 = [(NETSchemaNETSessionConnectionHttpHeaderCreated *)self aceHost];
  v6 = [v4 aceHost];
  if ((v5 == 0) != (v6 != 0))
  {
    uint64_t v12 = [(NETSchemaNETSessionConnectionHttpHeaderCreated *)self aceHost];
    if (!v12)
    {

LABEL_15:
      BOOL v17 = 1;
      goto LABEL_13;
    }
    v13 = (void *)v12;
    v14 = [(NETSchemaNETSessionConnectionHttpHeaderCreated *)self aceHost];
    v15 = [v4 aceHost];
    char v16 = [v14 isEqual:v15];

    if (v16) {
      goto LABEL_15;
    }
  }
  else
  {
LABEL_11:
  }
LABEL_12:
  BOOL v17 = 0;
LABEL_13:

  return v17;
}

- (void)writeTo:(id)a3
{
  id v6 = a3;
  id v4 = [(NETSchemaNETSessionConnectionHttpHeaderCreated *)self userAgent];

  if (v4) {
    PBDataWriterWriteStringField();
  }
  v5 = [(NETSchemaNETSessionConnectionHttpHeaderCreated *)self aceHost];

  if (v5) {
    PBDataWriterWriteStringField();
  }
}

- (BOOL)readFrom:(id)a3
{
  return NETSchemaNETSessionConnectionHttpHeaderCreatedReadFrom(self, (uint64_t)a3);
}

- (void)deleteAceHost
{
}

- (BOOL)hasAceHost
{
  return self->_aceHost != 0;
}

- (void)deleteUserAgent
{
}

- (BOOL)hasUserAgent
{
  return self->_userAgent != 0;
}

@end