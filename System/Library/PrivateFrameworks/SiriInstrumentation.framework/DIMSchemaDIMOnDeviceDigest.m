...
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (DIMSchemaDIMOnDeviceDigest)initWithDictionary:(id)a3;
- (DIMSchemaDIMOnDeviceDigest)initWithJSON:(id)a3;
- (NSData)jsonData;
...
- (DIMSchemaDIMOnDeviceDigest)initWithDictionary:(id)a3
{
  id v4 = a3;
  v10.receiver = self;
  v10.super_class = (Class)DIMSchemaDIMOnDeviceDigest;
  v5 = [(DIMSchemaDIMOnDeviceDigest *)&v10 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"daysWithTwoValidAssistantTurnsPerWeek"];
    objc_opt_class();
    ...
    v7 = [v4 objectForKeyedSubscript:@"daysWithTwoAssistantSpeechRequestsPerWeek"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
...
- (NSData)jsonData
{
  v2 = [(DIMSchemaDIMOnDeviceDigest *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if ((has & 2) != 0)
  {
    v5 = objc_msgSend(NSNumber, "numberWithUnsignedInt:", -[DIMSchemaDIMOnDeviceDigest daysWithTwoAssistantSpeechRequestsPerWeek](self, "daysWithTwoAssistantSpeechRequestsPerWeek"));
  ...
  if (has)
  {
    v6 = objc_msgSend(NSNumber, "numberWithUnsignedInt:", -[DIMSchemaDIMOnDeviceDigest daysWithTwoValidAssistantTurnsPerWeek](self, "daysWithTwoValidAssistantTurnsPerWeek"));
...
- (unint64_t)hash
{
  ...
  {
    uint64_t v2 = 2654435761 * self->_daysWithTwoValidAssistantTurnsPerWeek;
...
LABEL_3:
  uint64_t v3 = 2654435761 * self->_daysWithTwoAssistantSpeechRequestsPerWeek;
  return v3 ^ v2;
}
...
LABEL_10:
  ...
  if (v8)
  {
    ...
    if (daysWithTwoAssistantSpeechRequestsPerWeek != [v4 daysWithTwoAssistantSpeechRequestsPerWeek])goto LABEL_10; {
  }
    }
  BOOL v10 = 1;
LABEL_11:
...
- (BOOL)readFrom:(id)a3
{
  return DIMSchemaDIMOnDeviceDigestReadFrom(self, (uint64_t)a3);
}
