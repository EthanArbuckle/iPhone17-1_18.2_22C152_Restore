@interface IASchemaIASiriMissEvaluationEnded
- (BOOL)hasIsSiriMiss;
- (BOOL)hasVoiceTriggerSummary;
- (BOOL)isEqual:(id)a3;
- (BOOL)isSiriMiss;
- (BOOL)readFrom:(id)a3;
- (IASchemaIASiriMissEvaluationEnded)initWithDictionary:(id)a3;
- (IASchemaIASiriMissEvaluationEnded)initWithJSON:(id)a3;
- (IASchemaIAVoiceTriggerEventSummary)voiceTriggerSummary;
- (NSData)jsonData;
- (id)applySensitiveConditionsPolicy:(id)a3;
- (id)dictionaryRepresentation;
- (id)suppressMessageUnderConditions;
- (unint64_t)hash;
- (void)deleteIsSiriMiss;
- (void)deleteVoiceTriggerSummary;
- (void)setHasIsSiriMiss:(BOOL)a3;
- (void)setHasVoiceTriggerSummary:(BOOL)a3;
- (void)setIsSiriMiss:(BOOL)a3;
- (void)setVoiceTriggerSummary:(id)a3;
- (void)writeTo:(id)a3;
@end

@implementation IASchemaIASiriMissEvaluationEnded

- (id)applySensitiveConditionsPolicy:(id)a3
{
  v9.receiver = self;
  v9.super_class = (Class)IASchemaIASiriMissEvaluationEnded;
  id v4 = a3;
  v5 = [(SISchemaInstrumentationMessage *)&v9 applySensitiveConditionsPolicy:v4];
  v6 = [(IASchemaIASiriMissEvaluationEnded *)self voiceTriggerSummary];
  v7 = [v6 applySensitiveConditionsPolicy:v4];

  LODWORD(v4) = [v7 suppressMessage];
  if (v4) {
    [(IASchemaIASiriMissEvaluationEnded *)self deleteVoiceTriggerSummary];
  }
  return v5;
}

- (id)suppressMessageUnderConditions
{
  v2 = objc_alloc_init(SISensitiveConditionsSet);
  [(SISensitiveConditionsSet *)v2 addCondition:1];
  return v2;
}

- (void).cxx_destruct
{
}

- (void)setHasVoiceTriggerSummary:(BOOL)a3
{
  *((unsigned char *)&self->_has + 1) = a3;
}

- (void)setVoiceTriggerSummary:(id)a3
{
}

- (IASchemaIAVoiceTriggerEventSummary)voiceTriggerSummary
{
  return self->_voiceTriggerSummary;
}

- (BOOL)isSiriMiss
{
  return self->_isSiriMiss;
}

- (IASchemaIASiriMissEvaluationEnded)initWithDictionary:(id)a3
{
  id v4 = a3;
  v11.receiver = self;
  v11.super_class = (Class)IASchemaIASiriMissEvaluationEnded;
  v5 = [(IASchemaIASiriMissEvaluationEnded *)&v11 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"isSiriMiss"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[IASchemaIASiriMissEvaluationEnded setIsSiriMiss:](v5, "setIsSiriMiss:", [v6 BOOLValue]);
    }
    v7 = [v4 objectForKeyedSubscript:@"voiceTriggerSummary"];
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      v8 = [[IASchemaIAVoiceTriggerEventSummary alloc] initWithDictionary:v7];
      [(IASchemaIASiriMissEvaluationEnded *)v5 setVoiceTriggerSummary:v8];
    }
    objc_super v9 = v5;
  }
  return v5;
}

- (IASchemaIASiriMissEvaluationEnded)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(IASchemaIASiriMissEvaluationEnded *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (NSData)jsonData
{
  v2 = [(IASchemaIASiriMissEvaluationEnded *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  if (*(unsigned char *)&self->_has)
  {
    id v4 = objc_msgSend(NSNumber, "numberWithBool:", -[IASchemaIASiriMissEvaluationEnded isSiriMiss](self, "isSiriMiss"));
    [v3 setObject:v4 forKeyedSubscript:@"isSiriMiss"];
  }
  if (self->_voiceTriggerSummary)
  {
    v5 = [(IASchemaIASiriMissEvaluationEnded *)self voiceTriggerSummary];
    v6 = [v5 dictionaryRepresentation];
    if (v6)
    {
      [v3 setObject:v6 forKeyedSubscript:@"voiceTriggerSummary"];
    }
    else
    {
      uint64_t v7 = [MEMORY[0x1E4F1CA98] null];
      [v3 setObject:v7 forKeyedSubscript:@"voiceTriggerSummary"];
    }
  }
  [(SISchemaInstrumentationMessage *)self willProduceDictionaryRepresentation:v3];
  return v3;
}

- (unint64_t)hash
{
  if (*(unsigned char *)&self->_has) {
    uint64_t v2 = 2654435761 * self->_isSiriMiss;
  }
  else {
    uint64_t v2 = 0;
  }
  return [(IASchemaIAVoiceTriggerEventSummary *)self->_voiceTriggerSummary hash] ^ v2;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = a3;
  if ([v4 isMemberOfClass:objc_opt_class()])
  {
    if ((*(unsigned char *)&self->_has & 1) == (v4[24] & 1))
    {
      if ((*(unsigned char *)&self->_has & 1) == 0
        || (int isSiriMiss = self->_isSiriMiss, isSiriMiss == [v4 isSiriMiss]))
      {
        v6 = [(IASchemaIASiriMissEvaluationEnded *)self voiceTriggerSummary];
        uint64_t v7 = [v4 voiceTriggerSummary];
        v8 = v7;
        if ((v6 == 0) != (v7 != 0))
        {
          uint64_t v9 = [(IASchemaIASiriMissEvaluationEnded *)self voiceTriggerSummary];
          if (!v9)
          {

LABEL_13:
            BOOL v14 = 1;
            goto LABEL_11;
          }
          v10 = (void *)v9;
          objc_super v11 = [(IASchemaIASiriMissEvaluationEnded *)self voiceTriggerSummary];
          v12 = [v4 voiceTriggerSummary];
          char v13 = [v11 isEqual:v12];

          if (v13) {
            goto LABEL_13;
          }
        }
        else
        {
        }
      }
    }
  }
  BOOL v14 = 0;
LABEL_11:

  return v14;
}

- (void)writeTo:(id)a3
{
  id v7 = a3;
  if (*(unsigned char *)&self->_has) {
    PBDataWriterWriteBOOLField();
  }
  id v4 = [(IASchemaIASiriMissEvaluationEnded *)self voiceTriggerSummary];

  v5 = v7;
  if (v4)
  {
    v6 = [(IASchemaIASiriMissEvaluationEnded *)self voiceTriggerSummary];
    PBDataWriterWriteSubmessage();

    v5 = v7;
  }
}

- (BOOL)readFrom:(id)a3
{
  return IASchemaIASiriMissEvaluationEndedReadFrom(self, (uint64_t)a3);
}

- (void)deleteVoiceTriggerSummary
{
}

- (BOOL)hasVoiceTriggerSummary
{
  return self->_voiceTriggerSummary != 0;
}

- (void)deleteIsSiriMiss
{
  *(unsigned char *)&self->_has &= ~1u;
}

- (void)setHasIsSiriMiss:(BOOL)a3
{
  *(unsigned char *)&self->_has = *(unsigned char *)&self->_has & 0xFE | a3;
}

- (BOOL)hasIsSiriMiss
{
  return *(unsigned char *)&self->_has & 1;
}

- (void)setIsSiriMiss:(BOOL)a3
{
  *(unsigned char *)&self->_has |= 1u;
  self->_int isSiriMiss = a3;
}

@end