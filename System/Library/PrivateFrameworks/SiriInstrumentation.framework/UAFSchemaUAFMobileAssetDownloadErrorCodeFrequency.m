...
- (UAFSchemaUAFMobileAssetDownloadErrorCodeFrequency)initWithDictionary:(id)a3
{
  ...
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"mobileAssetDownloadErrorCode"];
    objc_opt_class();
    ...
    v7 = [v4 objectForKeyedSubscript:@"timesOccurred"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
...
- (NSData)jsonData
{
  v2 = [(UAFSchemaUAFMobileAssetDownloadErrorCodeFrequency *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if (has)
  {
    v5 = objc_msgSend(NSNumber, "numberWithUnsignedInt:", -[UAFSchemaUAFMobileAssetDownloadErrorCodeFrequency mobileAssetDownloadErrorCode](self, "mobileAssetDownloadErrorCode"));
    [v3 setObject:v5 forKeyedSubscript:@"mobileAssetDownloadErrorCode"];

    char has = (char)self->_has;
  }
  if ((has & 2) != 0)
  {
    v6 = objc_msgSend(NSNumber, "numberWithUnsignedInt:", -[UAFSchemaUAFMobileAssetDownloadErrorCodeFrequency timesOccurred](self, "timesOccurred"));
    [v3 setObject:v6 forKeyedSubscript:@"timesOccurred"];
  }
...
- (unint64_t)hash
{
  ...
  {
    uint64_t v2 = 2654435761 * self->_mobileAssetDownloadErrorCode;
...
LABEL_3:
  uint64_t v3 = 2654435761 * self->_timesOccurred;
  return v3 ^ v2;
}
...
- (BOOL)isEqual:(id)a3
{
  ...
  $5F679CBEC03C2785C6C351C1025FF84A has = self->_has;
  ...
  {
    unsigned int mobileAssetDownloadErrorCode = self->_mobileAssetDownloadErrorCode;
    if (mobileAssetDownloadErrorCode != [v4 mobileAssetDownloadErrorCode])
    {
LABEL_10:
      BOOL v10 = 0;
      goto LABEL_11;
  ...
  if (v8)
  {
    unsigned int timesOccurred = self->_timesOccurred;
    if (timesOccurred != [v4 timesOccurred]) {
      goto LABEL_10;
    }
  }
  BOOL v10 = 1;
...
{
  *(unsigned char *)&self->_has |= 2u;
  self->_unsigned int timesOccurred = a3;
}
...
{
  *(unsigned char *)&self->_has |= 1u;
  self->_unsigned int mobileAssetDownloadErrorCode = a3;
}
