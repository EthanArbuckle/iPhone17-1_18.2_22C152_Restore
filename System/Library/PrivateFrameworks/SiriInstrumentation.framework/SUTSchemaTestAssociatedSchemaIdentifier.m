@interface SUTSchemaTestAssociatedSchemaIdentifier
- (BOOL)hasRequestId;
- (BOOL)hasTurnID;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (NSData)jsonData;
- (SUTSchemaTestAssociatedSchemaIdentifier)initWithDictionary:(id)a3;
- (SUTSchemaTestAssociatedSchemaIdentifier)initWithJSON:(id)a3;
- (SUTSchemaTestGeneratedRequestId)requestId;
- (SUTSchemaTestGeneratedTurnID)turnID;
- (id)applySensitiveConditionsPolicy:(id)a3;
- (id)dictionaryRepresentation;
- (id)suppressMessageUnderConditions;
- (unint64_t)hash;
- (unint64_t)whichIdentifier_Type;
- (void)deleteRequestId;
- (void)deleteTurnID;
- (void)setHasRequestId:(BOOL)a3;
- (void)setHasTurnID:(BOOL)a3;
- (void)setRequestId:(id)a3;
- (void)setTurnID:(id)a3;
- (void)writeTo:(id)a3;
@end

@implementation SUTSchemaTestAssociatedSchemaIdentifier

- (id)applySensitiveConditionsPolicy:(id)a3
{
  id v4 = a3;
  v13.receiver = self;
  v13.super_class = (Class)SUTSchemaTestAssociatedSchemaIdentifier;
  v5 = [(SISchemaInstrumentationMessage *)&v13 applySensitiveConditionsPolicy:v4];
  v6 = [(SUTSchemaTestAssociatedSchemaIdentifier *)self turnID];
  v7 = [v6 applySensitiveConditionsPolicy:v4];
  int v8 = [v7 suppressMessage];

  if (v8) {
    [(SUTSchemaTestAssociatedSchemaIdentifier *)self deleteTurnID];
  }
  v9 = [(SUTSchemaTestAssociatedSchemaIdentifier *)self requestId];
  v10 = [v9 applySensitiveConditionsPolicy:v4];
  int v11 = [v10 suppressMessage];

  if (v11) {
    [(SUTSchemaTestAssociatedSchemaIdentifier *)self deleteRequestId];
  }

  return v5;
}

- (id)suppressMessageUnderConditions
{
  v2 = objc_alloc_init(SISensitiveConditionsSet);
  [(SISensitiveConditionsSet *)v2 addCondition:1];
  return v2;
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_requestId, 0);
  objc_storeStrong((id *)&self->_turnID, 0);
}

- (void)setHasRequestId:(BOOL)a3
{
  self->_hasRequestId = a3;
}

- (BOOL)hasRequestId
{
  return self->_hasRequestId;
}

- (void)setHasTurnID:(BOOL)a3
{
  self->_hasTurnID = a3;
}

- (BOOL)hasTurnID
{
  return self->_hasTurnID;
}

- (unint64_t)whichIdentifier_Type
{
  return self->_whichIdentifier_Type;
}

- (SUTSchemaTestAssociatedSchemaIdentifier)initWithDictionary:(id)a3
{
  id v4 = a3;
  v12.receiver = self;
  v12.super_class = (Class)SUTSchemaTestAssociatedSchemaIdentifier;
  v5 = [(SUTSchemaTestAssociatedSchemaIdentifier *)&v12 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"turnID"];
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      v7 = [[SUTSchemaTestGeneratedTurnID alloc] initWithDictionary:v6];
      [(SUTSchemaTestAssociatedSchemaIdentifier *)v5 setTurnID:v7];
    }
    int v8 = [v4 objectForKeyedSubscript:@"requestId"];
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      v9 = [[SUTSchemaTestGeneratedRequestId alloc] initWithDictionary:v8];
      [(SUTSchemaTestAssociatedSchemaIdentifier *)v5 setRequestId:v9];
    }
    v10 = v5;
  }
  return v5;
}

- (SUTSchemaTestAssociatedSchemaIdentifier)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(SUTSchemaTestAssociatedSchemaIdentifier *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (NSData)jsonData
{
  v2 = [(SUTSchemaTestAssociatedSchemaIdentifier *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  if (self->_requestId)
  {
    id v4 = [(SUTSchemaTestAssociatedSchemaIdentifier *)self requestId];
    v5 = [v4 dictionaryRepresentation];
    if (v5)
    {
      [v3 setObject:v5 forKeyedSubscript:@"requestId"];
    }
    else
    {
      v6 = [MEMORY[0x1E4F1CA98] null];
      [v3 setObject:v6 forKeyedSubscript:@"requestId"];
    }
  }
  if (self->_turnID)
  {
    uint64_t v7 = [(SUTSchemaTestAssociatedSchemaIdentifier *)self turnID];
    int v8 = [v7 dictionaryRepresentation];
    if (v8)
    {
      [v3 setObject:v8 forKeyedSubscript:@"turnID"];
    }
    else
    {
      v9 = [MEMORY[0x1E4F1CA98] null];
      [v3 setObject:v9 forKeyedSubscript:@"turnID"];
    }
  }
  [(SISchemaInstrumentationMessage *)self willProduceDictionaryRepresentation:v3];
  return v3;
}

- (unint64_t)hash
{
  unint64_t v3 = [(SUTSchemaTestGeneratedTurnID *)self->_turnID hash];
  return [(SUTSchemaTestGeneratedRequestId *)self->_requestId hash] ^ v3;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_13;
  }
  unint64_t whichIdentifier_Type = self->_whichIdentifier_Type;
  if (whichIdentifier_Type != objc_msgSend(v4, "whichIdentifier_Type")) {
    goto LABEL_13;
  }
  v6 = [(SUTSchemaTestAssociatedSchemaIdentifier *)self turnID];
  uint64_t v7 = [v4 turnID];
  if ((v6 == 0) == (v7 != 0)) {
    goto LABEL_12;
  }
  uint64_t v8 = [(SUTSchemaTestAssociatedSchemaIdentifier *)self turnID];
  if (v8)
  {
    v9 = (void *)v8;
    v10 = [(SUTSchemaTestAssociatedSchemaIdentifier *)self turnID];
    int v11 = [v4 turnID];
    int v12 = [v10 isEqual:v11];

    if (!v12) {
      goto LABEL_13;
    }
  }
  else
  {
  }
  v6 = [(SUTSchemaTestAssociatedSchemaIdentifier *)self requestId];
  uint64_t v7 = [v4 requestId];
  if ((v6 == 0) != (v7 != 0))
  {
    uint64_t v13 = [(SUTSchemaTestAssociatedSchemaIdentifier *)self requestId];
    if (!v13)
    {

LABEL_16:
      BOOL v18 = 1;
      goto LABEL_14;
    }
    v14 = (void *)v13;
    v15 = [(SUTSchemaTestAssociatedSchemaIdentifier *)self requestId];
    v16 = [v4 requestId];
    char v17 = [v15 isEqual:v16];

    if (v17) {
      goto LABEL_16;
    }
  }
  else
  {
LABEL_12:
  }
LABEL_13:
  BOOL v18 = 0;
LABEL_14:

  return v18;
}

- (void)writeTo:(id)a3
{
  id v8 = a3;
  id v4 = [(SUTSchemaTestAssociatedSchemaIdentifier *)self turnID];

  if (v4)
  {
    v5 = [(SUTSchemaTestAssociatedSchemaIdentifier *)self turnID];
    PBDataWriterWriteSubmessage();
  }
  v6 = [(SUTSchemaTestAssociatedSchemaIdentifier *)self requestId];

  if (v6)
  {
    uint64_t v7 = [(SUTSchemaTestAssociatedSchemaIdentifier *)self requestId];
    PBDataWriterWriteSubmessage();
  }
}

- (BOOL)readFrom:(id)a3
{
  return SUTSchemaTestAssociatedSchemaIdentifierReadFrom(self, (uint64_t)a3);
}

- (void)deleteRequestId
{
  if (self->_whichIdentifier_Type == 102)
  {
    self->_unint64_t whichIdentifier_Type = 0;
    self->_requestId = 0;
    MEMORY[0x1F41817F8]();
  }
}

- (SUTSchemaTestGeneratedRequestId)requestId
{
  if (self->_whichIdentifier_Type == 102) {
    v2 = self->_requestId;
  }
  else {
    v2 = 0;
  }
  return v2;
}

- (void)setRequestId:(id)a3
{
  id v4 = (SUTSchemaTestGeneratedRequestId *)a3;
  turnID = self->_turnID;
  self->_turnID = 0;

  unint64_t v6 = 102;
  if (!v4) {
    unint64_t v6 = 0;
  }
  self->_unint64_t whichIdentifier_Type = v6;
  requestId = self->_requestId;
  self->_requestId = v4;
}

- (void)deleteTurnID
{
  if (self->_whichIdentifier_Type == 101)
  {
    self->_unint64_t whichIdentifier_Type = 0;
    self->_turnID = 0;
    MEMORY[0x1F41817F8]();
  }
}

- (SUTSchemaTestGeneratedTurnID)turnID
{
  if (self->_whichIdentifier_Type == 101) {
    v2 = self->_turnID;
  }
  else {
    v2 = 0;
  }
  return v2;
}

- (void)setTurnID:(id)a3
{
  id v4 = (SUTSchemaTestGeneratedTurnID *)a3;
  requestId = self->_requestId;
  self->_requestId = 0;

  unint64_t v6 = 101;
  if (!v4) {
    unint64_t v6 = 0;
  }
  self->_unint64_t whichIdentifier_Type = v6;
  turnID = self->_turnID;
  self->_turnID = v4;
}

@end