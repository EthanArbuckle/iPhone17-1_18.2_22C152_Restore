@interface IFTSchemaIFTSearchExpression
- (BOOL)hasPickOne;
- (BOOL)isEqual:(id)a3;
- (BOOL)pickOne;
- (BOOL)readFrom:(id)a3;
- (IFTSchemaIFTSearchExpression)initWithDictionary:(id)a3;
- (IFTSchemaIFTSearchExpression)initWithJSON:(id)a3;
- (NSData)jsonData;
- (id)dictionaryRepresentation;
- (id)suppressMessageUnderConditions;
- (unint64_t)hash;
- (void)deletePickOne;
- (void)setHasPickOne:(BOOL)a3;
- (void)setPickOne:(BOOL)a3;
- (void)writeTo:(id)a3;
@end

@implementation IFTSchemaIFTSearchExpression

- (id)suppressMessageUnderConditions
{
  v2 = objc_alloc_init(SISensitiveConditionsSet);
  [(SISensitiveConditionsSet *)v2 addCondition:1];
  return v2;
}

- (BOOL)pickOne
{
  return self->_pickOne;
}

- (IFTSchemaIFTSearchExpression)initWithDictionary:(id)a3
{
  id v4 = a3;
  v9.receiver = self;
  v9.super_class = (Class)IFTSchemaIFTSearchExpression;
  v5 = [(IFTSchemaIFTSearchExpression *)&v9 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"pickOne"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[IFTSchemaIFTSearchExpression setPickOne:](v5, "setPickOne:", [v6 BOOLValue]);
    }
    v7 = v5;
  }
  return v5;
}

- (IFTSchemaIFTSearchExpression)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(IFTSchemaIFTSearchExpression *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (NSData)jsonData
{
  v2 = [(IFTSchemaIFTSearchExpression *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  if (*(&self->_pickOne + 1))
  {
    id v4 = objc_msgSend(NSNumber, "numberWithBool:", -[IFTSchemaIFTSearchExpression pickOne](self, "pickOne"));
    [v3 setObject:v4 forKeyedSubscript:@"pickOne"];
  }
  [(SISchemaInstrumentationMessage *)self willProduceDictionaryRepresentation:v3];
  return v3;
}

- (unint64_t)hash
{
  if (*(&self->_pickOne + 1)) {
    return 2654435761 * self->_pickOne;
  }
  else {
    return 0;
  }
}

- (BOOL)isEqual:(id)a3
{
  id v4 = a3;
  BOOL v6 = 0;
  if ([v4 isMemberOfClass:objc_opt_class()])
  {
    if (*(&self->_pickOne + 1) == (v4[9] & 1))
    {
      if (!*(&self->_pickOne + 1) || (int pickOne = self->_pickOne, pickOne == [v4 pickOne])) {
        BOOL v6 = 1;
      }
    }
  }

  return v6;
}

- (void)writeTo:(id)a3
{
  if (*(&self->_pickOne + 1)) {
    PBDataWriterWriteBOOLField();
  }
}

- (BOOL)readFrom:(id)a3
{
  return IFTSchemaIFTSearchExpressionReadFrom(self, (uint64_t)a3);
}

- (void)deletePickOne
{
  *(&self->_pickOne + 1) &= ~1u;
}

- (void)setHasPickOne:(BOOL)a3
{
  *(&self->_pickOne + 1) = *(&self->_pickOne + 1) & 0xFE | a3;
}

- (BOOL)hasPickOne
{
  return *(&self->_pickOne + 1);
}

- (void)setPickOne:(BOOL)a3
{
  *(&self->_pickOne + 1) |= 1u;
  self->_int pickOne = a3;
}

@end