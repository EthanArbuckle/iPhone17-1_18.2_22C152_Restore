@interface ODBATCHSiriSchemaODBATCHDataReported
- (BOOL)hasDaysWithTwoAssistantSpeechRequestsPerWeek;
- (BOOL)hasDaysWithTwoValidAssistantTurnsPerWeek;
- (BOOL)hasOriginalClockId;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (NSData)jsonData;
- (ODBATCHSiriSchemaODBATCHDataReported)initWithDictionary:(id)a3;
- (ODBATCHSiriSchemaODBATCHDataReported)initWithJSON:(id)a3;
- (SISchemaUUID)originalClockId;
- (id)applySensitiveConditionsPolicy:(id)a3;
- (id)dictionaryRepresentation;
- (id)suppressMessageUnderConditions;
- (unint64_t)hash;
- (unsigned)daysWithTwoAssistantSpeechRequestsPerWeek;
- (unsigned)daysWithTwoValidAssistantTurnsPerWeek;
- (void)deleteDaysWithTwoAssistantSpeechRequestsPerWeek;
- (void)deleteDaysWithTwoValidAssistantTurnsPerWeek;
- (void)deleteOriginalClockId;
- (void)setDaysWithTwoAssistantSpeechRequestsPerWeek:(unsigned int)a3;
- (void)setDaysWithTwoValidAssistantTurnsPerWeek:(unsigned int)a3;
- (void)setHasDaysWithTwoAssistantSpeechRequestsPerWeek:(BOOL)a3;
- (void)setHasDaysWithTwoValidAssistantTurnsPerWeek:(BOOL)a3;
- (void)setHasOriginalClockId:(BOOL)a3;
- (void)setOriginalClockId:(id)a3;
- (void)writeTo:(id)a3;
@end

@implementation ODBATCHSiriSchemaODBATCHDataReported

- (id)applySensitiveConditionsPolicy:(id)a3
{
  v9.receiver = self;
  v9.super_class = (Class)ODBATCHSiriSchemaODBATCHDataReported;
  id v4 = a3;
  v5 = [(SISchemaInstrumentationMessage *)&v9 applySensitiveConditionsPolicy:v4];
  v6 = [(ODBATCHSiriSchemaODBATCHDataReported *)self originalClockId];
  v7 = [v6 applySensitiveConditionsPolicy:v4];

  LODWORD(v4) = [v7 suppressMessage];
  if (v4) {
    [(ODBATCHSiriSchemaODBATCHDataReported *)self deleteOriginalClockId];
  }
  return v5;
}

- (id)suppressMessageUnderConditions
{
  v2 = objc_alloc_init(SISensitiveConditionsSet);
  [(SISensitiveConditionsSet *)v2 addCondition:1];
  return v2;
}

- (void).cxx_destruct
{
}

- (void)setHasOriginalClockId:(BOOL)a3
{
  *((unsigned char *)&self->_has + 1) = a3;
}

- (unsigned)daysWithTwoAssistantSpeechRequestsPerWeek
{
  return self->_daysWithTwoAssistantSpeechRequestsPerWeek;
}

- (unsigned)daysWithTwoValidAssistantTurnsPerWeek
{
  return self->_daysWithTwoValidAssistantTurnsPerWeek;
}

- (void)setOriginalClockId:(id)a3
{
}

- (SISchemaUUID)originalClockId
{
  return self->_originalClockId;
}

- (ODBATCHSiriSchemaODBATCHDataReported)initWithDictionary:(id)a3
{
  id v4 = a3;
  v12.receiver = self;
  v12.super_class = (Class)ODBATCHSiriSchemaODBATCHDataReported;
  v5 = [(ODBATCHSiriSchemaODBATCHDataReported *)&v12 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"originalClockId"];
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      v7 = [[SISchemaUUID alloc] initWithDictionary:v6];
      [(ODBATCHSiriSchemaODBATCHDataReported *)v5 setOriginalClockId:v7];
    }
    v8 = [v4 objectForKeyedSubscript:@"daysWithTwoValidAssistantTurnsPerWeek"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[ODBATCHSiriSchemaODBATCHDataReported setDaysWithTwoValidAssistantTurnsPerWeek:](v5, "setDaysWithTwoValidAssistantTurnsPerWeek:", [v8 unsignedIntValue]);
    }
    objc_super v9 = [v4 objectForKeyedSubscript:@"daysWithTwoAssistantSpeechRequestsPerWeek"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[ODBATCHSiriSchemaODBATCHDataReported setDaysWithTwoAssistantSpeechRequestsPerWeek:](v5, "setDaysWithTwoAssistantSpeechRequestsPerWeek:", [v9 unsignedIntValue]);
    }
    v10 = v5;
  }
  return v5;
}

- (ODBATCHSiriSchemaODBATCHDataReported)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(ODBATCHSiriSchemaODBATCHDataReported *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (NSData)jsonData
{
  v2 = [(ODBATCHSiriSchemaODBATCHDataReported *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  char has = (char)self->_has;
  if ((has & 2) != 0)
  {
    v5 = objc_msgSend(NSNumber, "numberWithUnsignedInt:", -[ODBATCHSiriSchemaODBATCHDataReported daysWithTwoAssistantSpeechRequestsPerWeek](self, "daysWithTwoAssistantSpeechRequestsPerWeek"));
    [v3 setObject:v5 forKeyedSubscript:@"daysWithTwoAssistantSpeechRequestsPerWeek"];

    char has = (char)self->_has;
  }
  if (has)
  {
    v6 = objc_msgSend(NSNumber, "numberWithUnsignedInt:", -[ODBATCHSiriSchemaODBATCHDataReported daysWithTwoValidAssistantTurnsPerWeek](self, "daysWithTwoValidAssistantTurnsPerWeek"));
    [v3 setObject:v6 forKeyedSubscript:@"daysWithTwoValidAssistantTurnsPerWeek"];
  }
  if (self->_originalClockId)
  {
    uint64_t v7 = [(ODBATCHSiriSchemaODBATCHDataReported *)self originalClockId];
    v8 = [v7 dictionaryRepresentation];
    if (v8)
    {
      [v3 setObject:v8 forKeyedSubscript:@"originalClockId"];
    }
    else
    {
      objc_super v9 = [MEMORY[0x1E4F1CA98] null];
      [v3 setObject:v9 forKeyedSubscript:@"originalClockId"];
    }
  }
  [(SISchemaInstrumentationMessage *)self willProduceDictionaryRepresentation:v3];
  return v3;
}

- (unint64_t)hash
{
  unint64_t v3 = [(SISchemaUUID *)self->_originalClockId hash];
  if (*(unsigned char *)&self->_has)
  {
    uint64_t v4 = 2654435761 * self->_daysWithTwoValidAssistantTurnsPerWeek;
    if ((*(unsigned char *)&self->_has & 2) != 0) {
      goto LABEL_3;
    }
LABEL_5:
    uint64_t v5 = 0;
    return v4 ^ v3 ^ v5;
  }
  uint64_t v4 = 0;
  if ((*(unsigned char *)&self->_has & 2) == 0) {
    goto LABEL_5;
  }
LABEL_3:
  uint64_t v5 = 2654435761 * self->_daysWithTwoAssistantSpeechRequestsPerWeek;
  return v4 ^ v3 ^ v5;
}

- (BOOL)isEqual:(id)a3
{
  uint64_t v4 = (unsigned __int8 *)a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_16;
  }
  uint64_t v5 = [(ODBATCHSiriSchemaODBATCHDataReported *)self originalClockId];
  v6 = [v4 originalClockId];
  uint64_t v7 = v6;
  if ((v5 == 0) == (v6 != 0))
  {

    goto LABEL_16;
  }
  uint64_t v8 = [(ODBATCHSiriSchemaODBATCHDataReported *)self originalClockId];
  if (v8)
  {
    objc_super v9 = (void *)v8;
    v10 = [(ODBATCHSiriSchemaODBATCHDataReported *)self originalClockId];
    v11 = [v4 originalClockId];
    int v12 = [v10 isEqual:v11];

    if (!v12) {
      goto LABEL_16;
    }
  }
  else
  {
  }
  $02D520F6C6BCB9EC5D6012E047627FA1 has = self->_has;
  unsigned int v14 = v4[24];
  if ((*(unsigned char *)&has & 1) != (v14 & 1))
  {
LABEL_16:
    BOOL v18 = 0;
    goto LABEL_17;
  }
  if (*(unsigned char *)&has)
  {
    unsigned int daysWithTwoValidAssistantTurnsPerWeek = self->_daysWithTwoValidAssistantTurnsPerWeek;
    if (daysWithTwoValidAssistantTurnsPerWeek == [v4 daysWithTwoValidAssistantTurnsPerWeek])
    {
      $02D520F6C6BCB9EC5D6012E047627FA1 has = self->_has;
      unsigned int v14 = v4[24];
      goto LABEL_12;
    }
    goto LABEL_16;
  }
LABEL_12:
  int v16 = (*(unsigned int *)&has >> 1) & 1;
  if (v16 != ((v14 >> 1) & 1)) {
    goto LABEL_16;
  }
  if (v16)
  {
    unsigned int daysWithTwoAssistantSpeechRequestsPerWeek = self->_daysWithTwoAssistantSpeechRequestsPerWeek;
    if (daysWithTwoAssistantSpeechRequestsPerWeek != [v4 daysWithTwoAssistantSpeechRequestsPerWeek])goto LABEL_16; {
  }
    }
  BOOL v18 = 1;
LABEL_17:

  return v18;
}

- (void)writeTo:(id)a3
{
  id v8 = a3;
  uint64_t v4 = [(ODBATCHSiriSchemaODBATCHDataReported *)self originalClockId];

  if (v4)
  {
    uint64_t v5 = [(ODBATCHSiriSchemaODBATCHDataReported *)self originalClockId];
    PBDataWriterWriteSubmessage();
  }
  char has = (char)self->_has;
  if (has)
  {
    PBDataWriterWriteUint32Field();
    char has = (char)self->_has;
  }
  uint64_t v7 = v8;
  if ((has & 2) != 0)
  {
    PBDataWriterWriteUint32Field();
    uint64_t v7 = v8;
  }
}

- (BOOL)readFrom:(id)a3
{
  return ODBATCHSiriSchemaODBATCHDataReportedReadFrom(self, (uint64_t)a3);
}

- (void)deleteDaysWithTwoAssistantSpeechRequestsPerWeek
{
  *(unsigned char *)&self->_has &= ~2u;
}

- (void)setHasDaysWithTwoAssistantSpeechRequestsPerWeek:(BOOL)a3
{
  if (a3) {
    char v3 = 2;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFD | v3;
}

- (BOOL)hasDaysWithTwoAssistantSpeechRequestsPerWeek
{
  return (*(unsigned char *)&self->_has >> 1) & 1;
}

- (void)setDaysWithTwoAssistantSpeechRequestsPerWeek:(unsigned int)a3
{
  *(unsigned char *)&self->_has |= 2u;
  self->_unsigned int daysWithTwoAssistantSpeechRequestsPerWeek = a3;
}

- (void)deleteDaysWithTwoValidAssistantTurnsPerWeek
{
  *(unsigned char *)&self->_has &= ~1u;
}

- (void)setHasDaysWithTwoValidAssistantTurnsPerWeek:(BOOL)a3
{
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFE | a3;
}

- (BOOL)hasDaysWithTwoValidAssistantTurnsPerWeek
{
  return *(unsigned char *)&self->_has & 1;
}

- (void)setDaysWithTwoValidAssistantTurnsPerWeek:(unsigned int)a3
{
  *(unsigned char *)&self->_has |= 1u;
  self->_unsigned int daysWithTwoValidAssistantTurnsPerWeek = a3;
}

- (void)deleteOriginalClockId
{
}

- (BOOL)hasOriginalClockId
{
  return self->_originalClockId != 0;
}

@end