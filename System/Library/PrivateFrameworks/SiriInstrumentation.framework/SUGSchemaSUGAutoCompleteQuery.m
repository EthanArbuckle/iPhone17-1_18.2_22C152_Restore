@interface SUGSchemaSUGAutoCompleteQuery
- (BOOL)hasNumCharactersInQuery;
- (BOOL)hasNumWordsInQuery;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (NSData)jsonData;
- (SUGSchemaSUGAutoCompleteQuery)initWithDictionary:(id)a3;
- (SUGSchemaSUGAutoCompleteQuery)initWithJSON:(id)a3;
- (id)dictionaryRepresentation;
- (id)suppressMessageUnderConditions;
- (int)numCharactersInQuery;
- (int)numWordsInQuery;
- (unint64_t)hash;
- (void)deleteNumCharactersInQuery;
- (void)deleteNumWordsInQuery;
- (void)setHasNumCharactersInQuery:(BOOL)a3;
- (void)setHasNumWordsInQuery:(BOOL)a3;
- (void)setNumCharactersInQuery:(int)a3;
- (void)setNumWordsInQuery:(int)a3;
- (void)writeTo:(id)a3;
@end

@implementation SUGSchemaSUGAutoCompleteQuery

- (id)suppressMessageUnderConditions
{
  v2 = objc_alloc_init(SISensitiveConditionsSet);
  [(SISensitiveConditionsSet *)v2 addCondition:1];
  return v2;
}

- (int)numWordsInQuery
{
  return self->_numWordsInQuery;
}

- (int)numCharactersInQuery
{
  return self->_numCharactersInQuery;
}

- (SUGSchemaSUGAutoCompleteQuery)initWithDictionary:(id)a3
{
  id v4 = a3;
  v10.receiver = self;
  v10.super_class = (Class)SUGSchemaSUGAutoCompleteQuery;
  v5 = [(SUGSchemaSUGAutoCompleteQuery *)&v10 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"numCharactersInQuery"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[SUGSchemaSUGAutoCompleteQuery setNumCharactersInQuery:](v5, "setNumCharactersInQuery:", [v6 intValue]);
    }
    v7 = [v4 objectForKeyedSubscript:@"numWordsInQuery"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[SUGSchemaSUGAutoCompleteQuery setNumWordsInQuery:](v5, "setNumWordsInQuery:", [v7 intValue]);
    }
    v8 = v5;
  }
  return v5;
}

- (SUGSchemaSUGAutoCompleteQuery)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(SUGSchemaSUGAutoCompleteQuery *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (NSData)jsonData
{
  v2 = [(SUGSchemaSUGAutoCompleteQuery *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  char has = (char)self->_has;
  if (has)
  {
    v5 = objc_msgSend(NSNumber, "numberWithInt:", -[SUGSchemaSUGAutoCompleteQuery numCharactersInQuery](self, "numCharactersInQuery"));
    [v3 setObject:v5 forKeyedSubscript:@"numCharactersInQuery"];

    char has = (char)self->_has;
  }
  if ((has & 2) != 0)
  {
    v6 = objc_msgSend(NSNumber, "numberWithInt:", -[SUGSchemaSUGAutoCompleteQuery numWordsInQuery](self, "numWordsInQuery"));
    [v3 setObject:v6 forKeyedSubscript:@"numWordsInQuery"];
  }
  [(SISchemaInstrumentationMessage *)self willProduceDictionaryRepresentation:v3];
  return v3;
}

- (unint64_t)hash
{
  if (*(unsigned char *)&self->_has)
  {
    uint64_t v2 = 2654435761 * self->_numCharactersInQuery;
    if ((*(unsigned char *)&self->_has & 2) != 0) {
      goto LABEL_3;
    }
LABEL_5:
    uint64_t v3 = 0;
    return v3 ^ v2;
  }
  uint64_t v2 = 0;
  if ((*(unsigned char *)&self->_has & 2) == 0) {
    goto LABEL_5;
  }
LABEL_3:
  uint64_t v3 = 2654435761 * self->_numWordsInQuery;
  return v3 ^ v2;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = (unsigned __int8 *)a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_10;
  }
  $332E8A560DB8FF72B2120F61D9D77569 has = self->_has;
  unsigned int v6 = v4[16];
  if ((*(unsigned char *)&has & 1) != (v6 & 1)) {
    goto LABEL_10;
  }
  if (*(unsigned char *)&has)
  {
    int numCharactersInQuery = self->_numCharactersInQuery;
    if (numCharactersInQuery != [v4 numCharactersInQuery])
    {
LABEL_10:
      BOOL v10 = 0;
      goto LABEL_11;
    }
    $332E8A560DB8FF72B2120F61D9D77569 has = self->_has;
    unsigned int v6 = v4[16];
  }
  int v8 = (*(unsigned int *)&has >> 1) & 1;
  if (v8 != ((v6 >> 1) & 1)) {
    goto LABEL_10;
  }
  if (v8)
  {
    int numWordsInQuery = self->_numWordsInQuery;
    if (numWordsInQuery != [v4 numWordsInQuery]) {
      goto LABEL_10;
    }
  }
  BOOL v10 = 1;
LABEL_11:

  return v10;
}

- (void)writeTo:(id)a3
{
  id v4 = a3;
  char has = (char)self->_has;
  id v6 = v4;
  if (has)
  {
    PBDataWriterWriteInt32Field();
    id v4 = v6;
    char has = (char)self->_has;
  }
  if ((has & 2) != 0)
  {
    PBDataWriterWriteInt32Field();
    id v4 = v6;
  }
}

- (BOOL)readFrom:(id)a3
{
  return SUGSchemaSUGAutoCompleteQueryReadFrom(self, (uint64_t)a3);
}

- (void)deleteNumWordsInQuery
{
  *(unsigned char *)&self->_has &= ~2u;
}

- (void)setHasNumWordsInQuery:(BOOL)a3
{
  if (a3) {
    char v3 = 2;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFD | v3;
}

- (BOOL)hasNumWordsInQuery
{
  return (*(unsigned char *)&self->_has >> 1) & 1;
}

- (void)setNumWordsInQuery:(int)a3
{
  *(unsigned char *)&self->_has |= 2u;
  self->_int numWordsInQuery = a3;
}

- (void)deleteNumCharactersInQuery
{
  *(unsigned char *)&self->_has &= ~1u;
}

- (void)setHasNumCharactersInQuery:(BOOL)a3
{
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFE | a3;
}

- (BOOL)hasNumCharactersInQuery
{
  return *(unsigned char *)&self->_has & 1;
}

- (void)setNumCharactersInQuery:(int)a3
{
  *(unsigned char *)&self->_has |= 1u;
  self->_int numCharactersInQuery = a3;
}

@end