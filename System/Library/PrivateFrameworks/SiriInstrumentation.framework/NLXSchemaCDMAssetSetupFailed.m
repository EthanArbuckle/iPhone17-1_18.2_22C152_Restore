...
- (NSData)jsonData
{
  v2 = [(NLXSchemaCDMAssetSetupFailed *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if ((has & 2) != 0)
  {
    v5 = objc_msgSend(NSNumber, "numberWithUnsignedInt:", -[NLXSchemaCDMAssetSetupFailed errorCode](self, "errorCode"));
  ...
  if (has)
  {
    unsigned int v6 = [(NLXSchemaCDMAssetSetupFailed *)self errorDomain] - 1;
    if (v6 > 7) {
      uint64_t v7 = @"CDMASSETSETUPERRORDOMAIN_UNKNOWN";
    }
    else {
      uint64_t v7 = off_1E5EBAF48[v6];
    }
