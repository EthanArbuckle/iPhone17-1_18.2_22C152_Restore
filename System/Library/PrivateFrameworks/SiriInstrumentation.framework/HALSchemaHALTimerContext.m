...
- (unint64_t)hash;
- (unint64_t)timeSinceTimerFiredInSeconds;
- (void)deleteTimeSinceTimerFiredInSeconds;
- (void)setHasTimeSinceTimerFiredInSeconds:(BOOL)a3;
- (void)setTimeSinceTimerFiredInSeconds:(unint64_t)a3;
- (void)writeTo:(id)a3;
@end
...
- (unint64_t)timeSinceTimerFiredInSeconds
{
  return self->_timeSinceTimerFiredInSeconds;
}
...
- (HALSchemaHALTimerContext)initWithDictionary:(id)a3
{
  id v4 = a3;
  v9.receiver = self;
  v9.super_class = (Class)HALSchemaHALTimerContext;
...
- (id)dictionaryRepresentation
{
  ...
  {
    id v4 = objc_msgSend(NSNumber, "numberWithUnsignedLongLong:", -[HALSchemaHALTimerContext timeSinceTimerFiredInSeconds](self, "timeSinceTimerFiredInSeconds"));
  ...
  if ([v4 isMemberOfClass:objc_opt_class()])
  {
    if ((*(unsigned char *)&self->_has & 1) == (v4[16] & 1))
    {
      if ((*(unsigned char *)&self->_has & 1) == 0
        || (unint64_t timeSinceTimerFiredInSeconds = self->_timeSinceTimerFiredInSeconds,
            timeSinceTimerFiredInSeconds == [v4 timeSinceTimerFiredInSeconds]))
      {
        BOOL v6 = 1;
      }
    }
...
- (BOOL)readFrom:(id)a3
{
  return HALSchemaHALTimerContextReadFrom(self, (uint64_t)a3);
}
...
{
  *(unsigned char *)&self->_has |= 1u;
  self->_unint64_t timeSinceTimerFiredInSeconds = a3;
}
