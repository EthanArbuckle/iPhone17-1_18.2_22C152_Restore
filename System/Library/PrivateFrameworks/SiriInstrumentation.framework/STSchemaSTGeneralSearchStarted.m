...
- (STSchemaSTGeneralSearchStarted)initWithDictionary:(id)a3
{
  id v4 = a3;
  v10.receiver = self;
  v10.super_class = (Class)STSchemaSTGeneralSearchStarted;
...
- (id)dictionaryRepresentation
{
  ...
  char v4 = *(&self->_preWarmed + 1);
  if (v4)
  {
    v5 = objc_msgSend(NSNumber, "numberWithBool:", -[STSchemaSTGeneralSearchStarted exists](self, "exists"));
  ...
  if ((v4 & 2) != 0)
  {
    v6 = objc_msgSend(NSNumber, "numberWithBool:", -[STSchemaSTGeneralSearchStarted preWarmed](self, "preWarmed"));
...
LABEL_3:
  uint64_t v3 = 2654435761 * self->_preWarmed;
  return v3 ^ v2;
}
...
- (BOOL)isEqual:(id)a3
{
  ...
  unsigned int v5 = *((unsigned __int8 *)&self->_preWarmed + 1);
...
- (BOOL)readFrom:(id)a3
{
  return STSchemaSTGeneralSearchStartedReadFrom(self, (uint64_t)a3);
}

- (void)deletePreWarmed
{
  *(&self->_preWarmed + 1) &= ~2u;
}
...
- (BOOL)hasPreWarmed
{
  return (*((unsigned __int8 *)&self->_preWarmed + 1) >> 1) & 1;
}

- (void)setPreWarmed:(BOOL)a3
{
  *(&self->_preWarmed + 1) |= 2u;
  self->_int preWarmed = a3;
}

- (void)deleteExists
{
  *(&self->_preWarmed + 1) &= ~1u;
}

- (void)setHasExists:(BOOL)a3
{
  *(&self->_preWarmed + 1) = *(&self->_preWarmed + 1) & 0xFE | a3;
}

- (BOOL)hasExists
{
  return *(&self->_preWarmed + 1);
}
