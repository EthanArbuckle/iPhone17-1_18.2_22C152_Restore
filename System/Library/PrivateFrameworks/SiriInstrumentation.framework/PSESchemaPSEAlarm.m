@interface PSESchemaPSEAlarm
- (BOOL)hasFollowUpType;
- (BOOL)hasIsSiriResultUseful;
- (BOOL)isEqual:(id)a3;
- (BOOL)isSiriResultUseful;
- (BOOL)readFrom:(id)a3;
- (NSData)jsonData;
- (PSESchemaPSEAlarm)initWithDictionary:(id)a3;
- (PSESchemaPSEAlarm)initWithJSON:(id)a3;
- (id)dictionaryRepresentation;
- (id)suppressMessageUnderConditions;
- (int)followUpType;
- (unint64_t)hash;
- (void)deleteFollowUpType;
- (void)deleteIsSiriResultUseful;
- (void)setFollowUpType:(int)a3;
- (void)setHasFollowUpType:(BOOL)a3;
- (void)setHasIsSiriResultUseful:(BOOL)a3;
- (void)setIsSiriResultUseful:(BOOL)a3;
- (void)writeTo:(id)a3;
@end

@implementation PSESchemaPSEAlarm

- (id)suppressMessageUnderConditions
{
  v2 = objc_alloc_init(SISensitiveConditionsSet);
  [(SISensitiveConditionsSet *)v2 addCondition:1];
  return v2;
}

- (int)followUpType
{
  return self->_followUpType;
}

- (BOOL)isSiriResultUseful
{
  return self->_isSiriResultUseful;
}

- (PSESchemaPSEAlarm)initWithDictionary:(id)a3
{
  id v4 = a3;
  v10.receiver = self;
  v10.super_class = (Class)PSESchemaPSEAlarm;
  v5 = [(PSESchemaPSEAlarm *)&v10 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"isSiriResultUseful"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[PSESchemaPSEAlarm setIsSiriResultUseful:](v5, "setIsSiriResultUseful:", [v6 BOOLValue]);
    }
    v7 = [v4 objectForKeyedSubscript:@"followUpType"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[PSESchemaPSEAlarm setFollowUpType:](v5, "setFollowUpType:", [v7 intValue]);
    }
    v8 = v5;
  }
  return v5;
}

- (PSESchemaPSEAlarm)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(PSESchemaPSEAlarm *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (NSData)jsonData
{
  v2 = [(PSESchemaPSEAlarm *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  char has = (char)self->_has;
  if ((has & 2) != 0)
  {
    unsigned int v5 = [(PSESchemaPSEAlarm *)self followUpType] - 1;
    if (v5 > 3) {
      v6 = @"PSEALARMFOLLOWUP_UNKNOWN";
    }
    else {
      v6 = off_1E5EBF250[v5];
    }
    [v3 setObject:v6 forKeyedSubscript:@"followUpType"];
    char has = (char)self->_has;
  }
  if (has)
  {
    uint64_t v7 = objc_msgSend(NSNumber, "numberWithBool:", -[PSESchemaPSEAlarm isSiriResultUseful](self, "isSiriResultUseful"));
    [v3 setObject:v7 forKeyedSubscript:@"isSiriResultUseful"];
  }
  [(SISchemaInstrumentationMessage *)self willProduceDictionaryRepresentation:v3];
  return v3;
}

- (unint64_t)hash
{
  if (*(unsigned char *)&self->_has)
  {
    uint64_t v2 = 2654435761 * self->_isSiriResultUseful;
    if ((*(unsigned char *)&self->_has & 2) != 0) {
      goto LABEL_3;
    }
LABEL_5:
    uint64_t v3 = 0;
    return v3 ^ v2;
  }
  uint64_t v2 = 0;
  if ((*(unsigned char *)&self->_has & 2) == 0) {
    goto LABEL_5;
  }
LABEL_3:
  uint64_t v3 = 2654435761 * self->_followUpType;
  return v3 ^ v2;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = (unsigned __int8 *)a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_10;
  }
  $F79B13673CD05D96932B1EEEF809A6B6 has = self->_has;
  unsigned int v6 = v4[16];
  if ((*(unsigned char *)&has & 1) != (v6 & 1)) {
    goto LABEL_10;
  }
  if (*(unsigned char *)&has)
  {
    int isSiriResultUseful = self->_isSiriResultUseful;
    if (isSiriResultUseful != [v4 isSiriResultUseful])
    {
LABEL_10:
      BOOL v10 = 0;
      goto LABEL_11;
    }
    $F79B13673CD05D96932B1EEEF809A6B6 has = self->_has;
    unsigned int v6 = v4[16];
  }
  int v8 = (*(unsigned int *)&has >> 1) & 1;
  if (v8 != ((v6 >> 1) & 1)) {
    goto LABEL_10;
  }
  if (v8)
  {
    int followUpType = self->_followUpType;
    if (followUpType != [v4 followUpType]) {
      goto LABEL_10;
    }
  }
  BOOL v10 = 1;
LABEL_11:

  return v10;
}

- (void)writeTo:(id)a3
{
  id v4 = a3;
  char has = (char)self->_has;
  id v6 = v4;
  if (has)
  {
    PBDataWriterWriteBOOLField();
    id v4 = v6;
    char has = (char)self->_has;
  }
  if ((has & 2) != 0)
  {
    PBDataWriterWriteInt32Field();
    id v4 = v6;
  }
}

- (BOOL)readFrom:(id)a3
{
  return PSESchemaPSEAlarmReadFrom(self, (uint64_t)a3);
}

- (void)deleteFollowUpType
{
  *(unsigned char *)&self->_has &= ~2u;
}

- (void)setHasFollowUpType:(BOOL)a3
{
  if (a3) {
    char v3 = 2;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFD | v3;
}

- (BOOL)hasFollowUpType
{
  return (*(unsigned char *)&self->_has >> 1) & 1;
}

- (void)setFollowUpType:(int)a3
{
  *(unsigned char *)&self->_has |= 2u;
  self->_int followUpType = a3;
}

- (void)deleteIsSiriResultUseful
{
  *(unsigned char *)&self->_has &= ~1u;
}

- (void)setHasIsSiriResultUseful:(BOOL)a3
{
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFE | a3;
}

- (BOOL)hasIsSiriResultUseful
{
  return *(unsigned char *)&self->_has & 1;
}

- (void)setIsSiriResultUseful:(BOOL)a3
{
  *(unsigned char *)&self->_has |= 1u;
  self->_int isSiriResultUseful = a3;
}

@end