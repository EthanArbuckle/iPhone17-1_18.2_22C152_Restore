@interface ODDSiriSchemaODDmacOSAssistantProperties
- (BOOL)hasHeadGestures;
- (BOOL)hasIsExternalMicrophoneHSEnabled;
- (BOOL)isEqual:(id)a3;
- (BOOL)isExternalMicrophoneHSEnabled;
- (BOOL)readFrom:(id)a3;
- (NSData)jsonData;
- (ODDSiriSchemaODDHeadGestureProperties)headGestures;
- (ODDSiriSchemaODDmacOSAssistantProperties)initWithDictionary:(id)a3;
- (ODDSiriSchemaODDmacOSAssistantProperties)initWithJSON:(id)a3;
- (id)applySensitiveConditionsPolicy:(id)a3;
- (id)dictionaryRepresentation;
- (id)suppressMessageUnderConditions;
- (unint64_t)hash;
- (void)deleteHeadGestures;
- (void)deleteIsExternalMicrophoneHSEnabled;
- (void)setHasHeadGestures:(BOOL)a3;
- (void)setHasIsExternalMicrophoneHSEnabled:(BOOL)a3;
- (void)setHeadGestures:(id)a3;
- (void)setIsExternalMicrophoneHSEnabled:(BOOL)a3;
- (void)writeTo:(id)a3;
@end

@implementation ODDSiriSchemaODDmacOSAssistantProperties

- (id)applySensitiveConditionsPolicy:(id)a3
{
  v9.receiver = self;
  v9.super_class = (Class)ODDSiriSchemaODDmacOSAssistantProperties;
  id v4 = a3;
  v5 = [(SISchemaInstrumentationMessage *)&v9 applySensitiveConditionsPolicy:v4];
  v6 = [(ODDSiriSchemaODDmacOSAssistantProperties *)self headGestures];
  v7 = [v6 applySensitiveConditionsPolicy:v4];

  LODWORD(v4) = [v7 suppressMessage];
  if (v4) {
    [(ODDSiriSchemaODDmacOSAssistantProperties *)self deleteHeadGestures];
  }
  return v5;
}

- (id)suppressMessageUnderConditions
{
  v2 = objc_alloc_init(SISensitiveConditionsSet);
  [(SISensitiveConditionsSet *)v2 addCondition:1];
  return v2;
}

- (void).cxx_destruct
{
}

- (void)setHasHeadGestures:(BOOL)a3
{
  *((unsigned char *)&self->_has + 1) = a3;
}

- (void)setHeadGestures:(id)a3
{
}

- (ODDSiriSchemaODDHeadGestureProperties)headGestures
{
  return self->_headGestures;
}

- (BOOL)isExternalMicrophoneHSEnabled
{
  return self->_isExternalMicrophoneHSEnabled;
}

- (ODDSiriSchemaODDmacOSAssistantProperties)initWithDictionary:(id)a3
{
  id v4 = a3;
  v11.receiver = self;
  v11.super_class = (Class)ODDSiriSchemaODDmacOSAssistantProperties;
  v5 = [(ODDSiriSchemaODDmacOSAssistantProperties *)&v11 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"isExternalMicrophoneHSEnabled"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[ODDSiriSchemaODDmacOSAssistantProperties setIsExternalMicrophoneHSEnabled:](v5, "setIsExternalMicrophoneHSEnabled:", [v6 BOOLValue]);
    }
    v7 = [v4 objectForKeyedSubscript:@"headGestures"];
    objc_opt_class();
    if (objc_opt_isKindOfClass())
    {
      v8 = [[ODDSiriSchemaODDHeadGestureProperties alloc] initWithDictionary:v7];
      [(ODDSiriSchemaODDmacOSAssistantProperties *)v5 setHeadGestures:v8];
    }
    objc_super v9 = v5;
  }
  return v5;
}

- (ODDSiriSchemaODDmacOSAssistantProperties)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(ODDSiriSchemaODDmacOSAssistantProperties *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (NSData)jsonData
{
  v2 = [(ODDSiriSchemaODDmacOSAssistantProperties *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  if (self->_headGestures)
  {
    id v4 = [(ODDSiriSchemaODDmacOSAssistantProperties *)self headGestures];
    v5 = [v4 dictionaryRepresentation];
    if (v5)
    {
      [v3 setObject:v5 forKeyedSubscript:@"headGestures"];
    }
    else
    {
      v6 = [MEMORY[0x1E4F1CA98] null];
      [v3 setObject:v6 forKeyedSubscript:@"headGestures"];
    }
  }
  if (*(unsigned char *)&self->_has)
  {
    uint64_t v7 = objc_msgSend(NSNumber, "numberWithBool:", -[ODDSiriSchemaODDmacOSAssistantProperties isExternalMicrophoneHSEnabled](self, "isExternalMicrophoneHSEnabled"));
    [v3 setObject:v7 forKeyedSubscript:@"isExternalMicrophoneHSEnabled"];
  }
  [(SISchemaInstrumentationMessage *)self willProduceDictionaryRepresentation:v3];
  return v3;
}

- (unint64_t)hash
{
  if (*(unsigned char *)&self->_has) {
    uint64_t v2 = 2654435761 * self->_isExternalMicrophoneHSEnabled;
  }
  else {
    uint64_t v2 = 0;
  }
  return [(ODDSiriSchemaODDHeadGestureProperties *)self->_headGestures hash] ^ v2;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = a3;
  if ([v4 isMemberOfClass:objc_opt_class()])
  {
    if ((*(unsigned char *)&self->_has & 1) == (v4[24] & 1))
    {
      if ((*(unsigned char *)&self->_has & 1) == 0
        || (int isExternalMicrophoneHSEnabled = self->_isExternalMicrophoneHSEnabled,
            isExternalMicrophoneHSEnabled == [v4 isExternalMicrophoneHSEnabled]))
      {
        v6 = [(ODDSiriSchemaODDmacOSAssistantProperties *)self headGestures];
        uint64_t v7 = [v4 headGestures];
        v8 = v7;
        if ((v6 == 0) != (v7 != 0))
        {
          uint64_t v9 = [(ODDSiriSchemaODDmacOSAssistantProperties *)self headGestures];
          if (!v9)
          {

LABEL_13:
            BOOL v14 = 1;
            goto LABEL_11;
          }
          v10 = (void *)v9;
          objc_super v11 = [(ODDSiriSchemaODDmacOSAssistantProperties *)self headGestures];
          v12 = [v4 headGestures];
          char v13 = [v11 isEqual:v12];

          if (v13) {
            goto LABEL_13;
          }
        }
        else
        {
        }
      }
    }
  }
  BOOL v14 = 0;
LABEL_11:

  return v14;
}

- (void)writeTo:(id)a3
{
  id v7 = a3;
  if (*(unsigned char *)&self->_has) {
    PBDataWriterWriteBOOLField();
  }
  id v4 = [(ODDSiriSchemaODDmacOSAssistantProperties *)self headGestures];

  v5 = v7;
  if (v4)
  {
    v6 = [(ODDSiriSchemaODDmacOSAssistantProperties *)self headGestures];
    PBDataWriterWriteSubmessage();

    v5 = v7;
  }
}

- (BOOL)readFrom:(id)a3
{
  return ODDSiriSchemaODDmacOSAssistantPropertiesReadFrom(self, (uint64_t)a3);
}

- (void)deleteHeadGestures
{
}

- (BOOL)hasHeadGestures
{
  return self->_headGestures != 0;
}

- (void)deleteIsExternalMicrophoneHSEnabled
{
  *(unsigned char *)&self->_has &= ~1u;
}

- (void)setHasIsExternalMicrophoneHSEnabled:(BOOL)a3
{
  *(unsigned char *)&self->_has = *(unsigned char *)&self->_has & 0xFE | a3;
}

- (BOOL)hasIsExternalMicrophoneHSEnabled
{
  return *(unsigned char *)&self->_has & 1;
}

- (void)setIsExternalMicrophoneHSEnabled:(BOOL)a3
{
  *(unsigned char *)&self->_has |= 1u;
  self->_int isExternalMicrophoneHSEnabled = a3;
}

@end