@interface SISchemaISOLocale
- (BOOL)hasCountryCode;
- (BOOL)hasLanguageCode;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (NSData)jsonData;
- (SISchemaISOLocale)initWithDictionary:(id)a3;
- (SISchemaISOLocale)initWithJSON:(id)a3;
- (id)dictionaryRepresentation;
- (id)suppressMessageUnderConditions;
- (int)countryCode;
- (int)languageCode;
- (unint64_t)hash;
- (void)deleteCountryCode;
- (void)deleteLanguageCode;
- (void)setCountryCode:(int)a3;
- (void)setHasCountryCode:(BOOL)a3;
- (void)setHasLanguageCode:(BOOL)a3;
- (void)setLanguageCode:(int)a3;
- (void)writeTo:(id)a3;
@end

@implementation SISchemaISOLocale

- (id)suppressMessageUnderConditions
{
  v2 = objc_alloc_init(SISensitiveConditionsSet);
  [(SISensitiveConditionsSet *)v2 addCondition:1];
  return v2;
}

- (int)countryCode
{
  return self->_countryCode;
}

- (int)languageCode
{
  return self->_languageCode;
}

- (SISchemaISOLocale)initWithDictionary:(id)a3
{
  id v4 = a3;
  v10.receiver = self;
  v10.super_class = (Class)SISchemaISOLocale;
  v5 = [(SISchemaISOLocale *)&v10 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"languageCode"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[SISchemaISOLocale setLanguageCode:](v5, "setLanguageCode:", [v6 intValue]);
    }
    v7 = [v4 objectForKeyedSubscript:@"countryCode"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[SISchemaISOLocale setCountryCode:](v5, "setCountryCode:", [v7 intValue]);
    }
    v8 = v5;
  }
  return v5;
}

- (SISchemaISOLocale)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(SISchemaISOLocale *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (NSData)jsonData
{
  v2 = [(SISchemaISOLocale *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  char has = (char)self->_has;
  if ((has & 2) != 0)
  {
    unsigned int v5 = [(SISchemaISOLocale *)self countryCode] - 1;
    if (v5 > 0xF9) {
      v6 = @"COUNTRYCODE_UNKNOWN";
    }
    else {
      v6 = off_1E5EB6E50[v5];
    }
    [v3 setObject:v6 forKeyedSubscript:@"countryCode"];
    char has = (char)self->_has;
  }
  if (has)
  {
    unsigned int v7 = [(SISchemaISOLocale *)self languageCode] - 1;
    if (v7 > 0xB8) {
      v8 = @"ISOLANGUAGECODE_UNKNOWN";
    }
    else {
      v8 = off_1E5EB7620[v7];
    }
    [v3 setObject:v8 forKeyedSubscript:@"languageCode"];
  }
  [(SISchemaInstrumentationMessage *)self willProduceDictionaryRepresentation:v3];
  return v3;
}

- (unint64_t)hash
{
  if (*(unsigned char *)&self->_has)
  {
    uint64_t v2 = 2654435761 * self->_languageCode;
    if ((*(unsigned char *)&self->_has & 2) != 0) {
      goto LABEL_3;
    }
LABEL_5:
    uint64_t v3 = 0;
    return v3 ^ v2;
  }
  uint64_t v2 = 0;
  if ((*(unsigned char *)&self->_has & 2) == 0) {
    goto LABEL_5;
  }
LABEL_3:
  uint64_t v3 = 2654435761 * self->_countryCode;
  return v3 ^ v2;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = (unsigned __int8 *)a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_10;
  }
  $B3AAB0480A0469B05FAC7230D4C85713 has = self->_has;
  unsigned int v6 = v4[16];
  if ((*(unsigned char *)&has & 1) != (v6 & 1)) {
    goto LABEL_10;
  }
  if (*(unsigned char *)&has)
  {
    int languageCode = self->_languageCode;
    if (languageCode != [v4 languageCode])
    {
LABEL_10:
      BOOL v10 = 0;
      goto LABEL_11;
    }
    $B3AAB0480A0469B05FAC7230D4C85713 has = self->_has;
    unsigned int v6 = v4[16];
  }
  int v8 = (*(unsigned int *)&has >> 1) & 1;
  if (v8 != ((v6 >> 1) & 1)) {
    goto LABEL_10;
  }
  if (v8)
  {
    int countryCode = self->_countryCode;
    if (countryCode != [v4 countryCode]) {
      goto LABEL_10;
    }
  }
  BOOL v10 = 1;
LABEL_11:

  return v10;
}

- (void)writeTo:(id)a3
{
  id v4 = a3;
  char has = (char)self->_has;
  id v6 = v4;
  if (has)
  {
    PBDataWriterWriteInt32Field();
    id v4 = v6;
    char has = (char)self->_has;
  }
  if ((has & 2) != 0)
  {
    PBDataWriterWriteInt32Field();
    id v4 = v6;
  }
}

- (BOOL)readFrom:(id)a3
{
  return SISchemaISOLocaleReadFrom(self, (uint64_t)a3);
}

- (void)deleteCountryCode
{
  *(unsigned char *)&self->_has &= ~2u;
}

- (void)setHasCountryCode:(BOOL)a3
{
  if (a3) {
    char v3 = 2;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFD | v3;
}

- (BOOL)hasCountryCode
{
  return (*(unsigned char *)&self->_has >> 1) & 1;
}

- (void)setCountryCode:(int)a3
{
  *(unsigned char *)&self->_has |= 2u;
  self->_int countryCode = a3;
}

- (void)deleteLanguageCode
{
  *(unsigned char *)&self->_has &= ~1u;
}

- (void)setHasLanguageCode:(BOOL)a3
{
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFE | a3;
}

- (BOOL)hasLanguageCode
{
  return *(unsigned char *)&self->_has & 1;
}

- (void)setLanguageCode:(int)a3
{
  *(unsigned char *)&self->_has |= 1u;
  self->_int languageCode = a3;
}

@end