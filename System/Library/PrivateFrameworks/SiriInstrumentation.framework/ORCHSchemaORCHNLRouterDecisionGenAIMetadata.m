@interface ORCHSchemaORCHNLRouterDecisionGenAIMetadata
- (BOOL)hasGenAIAppIntent;
- (BOOL)hasIsExplicit;
- (BOOL)isEqual:(id)a3;
- (BOOL)isExplicit;
- (BOOL)readFrom:(id)a3;
- (NSData)jsonData;
- (ORCHSchemaORCHNLRouterDecisionGenAIMetadata)initWithDictionary:(id)a3;
- (ORCHSchemaORCHNLRouterDecisionGenAIMetadata)initWithJSON:(id)a3;
- (id)dictionaryRepresentation;
- (id)suppressMessageUnderConditions;
- (int)genAIAppIntent;
- (unint64_t)hash;
- (void)deleteGenAIAppIntent;
- (void)deleteIsExplicit;
- (void)setGenAIAppIntent:(int)a3;
- (void)setHasGenAIAppIntent:(BOOL)a3;
- (void)setHasIsExplicit:(BOOL)a3;
- (void)setIsExplicit:(BOOL)a3;
- (void)writeTo:(id)a3;
@end

@implementation ORCHSchemaORCHNLRouterDecisionGenAIMetadata

- (id)suppressMessageUnderConditions
{
  v2 = objc_alloc_init(SISensitiveConditionsSet);
  [(SISensitiveConditionsSet *)v2 addCondition:1];
  return v2;
}

- (int)genAIAppIntent
{
  return self->_genAIAppIntent;
}

- (BOOL)isExplicit
{
  return self->_isExplicit;
}

- (ORCHSchemaORCHNLRouterDecisionGenAIMetadata)initWithDictionary:(id)a3
{
  id v4 = a3;
  v10.receiver = self;
  v10.super_class = (Class)ORCHSchemaORCHNLRouterDecisionGenAIMetadata;
  v5 = [(ORCHSchemaORCHNLRouterDecisionGenAIMetadata *)&v10 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"isExplicit"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[ORCHSchemaORCHNLRouterDecisionGenAIMetadata setIsExplicit:](v5, "setIsExplicit:", [v6 BOOLValue]);
    }
    v7 = [v4 objectForKeyedSubscript:@"genAIAppIntent"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[ORCHSchemaORCHNLRouterDecisionGenAIMetadata setGenAIAppIntent:](v5, "setGenAIAppIntent:", [v7 intValue]);
    }
    v8 = v5;
  }
  return v5;
}

- (ORCHSchemaORCHNLRouterDecisionGenAIMetadata)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(ORCHSchemaORCHNLRouterDecisionGenAIMetadata *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (NSData)jsonData
{
  v2 = [(ORCHSchemaORCHNLRouterDecisionGenAIMetadata *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  char has = (char)self->_has;
  if ((has & 2) != 0)
  {
    unsigned int v5 = [(ORCHSchemaORCHNLRouterDecisionGenAIMetadata *)self genAIAppIntent] - 1;
    if (v5 > 2) {
      v6 = @"ORCHNLROUTERDECISIONGENAIAPPINTENT_UNKNOWN";
    }
    else {
      v6 = off_1E5EC0F40[v5];
    }
    [v3 setObject:v6 forKeyedSubscript:@"genAIAppIntent"];
    char has = (char)self->_has;
  }
  if (has)
  {
    uint64_t v7 = objc_msgSend(NSNumber, "numberWithBool:", -[ORCHSchemaORCHNLRouterDecisionGenAIMetadata isExplicit](self, "isExplicit"));
    [v3 setObject:v7 forKeyedSubscript:@"isExplicit"];
  }
  [(SISchemaInstrumentationMessage *)self willProduceDictionaryRepresentation:v3];
  return v3;
}

- (unint64_t)hash
{
  if (*(unsigned char *)&self->_has)
  {
    uint64_t v2 = 2654435761 * self->_isExplicit;
    if ((*(unsigned char *)&self->_has & 2) != 0) {
      goto LABEL_3;
    }
LABEL_5:
    uint64_t v3 = 0;
    return v3 ^ v2;
  }
  uint64_t v2 = 0;
  if ((*(unsigned char *)&self->_has & 2) == 0) {
    goto LABEL_5;
  }
LABEL_3:
  uint64_t v3 = 2654435761 * self->_genAIAppIntent;
  return v3 ^ v2;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = (unsigned __int8 *)a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_10;
  }
  $EA3C3D040CDD5D060D828277F5FDFD65 has = self->_has;
  unsigned int v6 = v4[16];
  if ((*(unsigned char *)&has & 1) != (v6 & 1)) {
    goto LABEL_10;
  }
  if (*(unsigned char *)&has)
  {
    int isExplicit = self->_isExplicit;
    if (isExplicit != [v4 isExplicit])
    {
LABEL_10:
      BOOL v10 = 0;
      goto LABEL_11;
    }
    $EA3C3D040CDD5D060D828277F5FDFD65 has = self->_has;
    unsigned int v6 = v4[16];
  }
  int v8 = (*(unsigned int *)&has >> 1) & 1;
  if (v8 != ((v6 >> 1) & 1)) {
    goto LABEL_10;
  }
  if (v8)
  {
    int genAIAppIntent = self->_genAIAppIntent;
    if (genAIAppIntent != [v4 genAIAppIntent]) {
      goto LABEL_10;
    }
  }
  BOOL v10 = 1;
LABEL_11:

  return v10;
}

- (void)writeTo:(id)a3
{
  id v4 = a3;
  char has = (char)self->_has;
  id v6 = v4;
  if (has)
  {
    PBDataWriterWriteBOOLField();
    id v4 = v6;
    char has = (char)self->_has;
  }
  if ((has & 2) != 0)
  {
    PBDataWriterWriteInt32Field();
    id v4 = v6;
  }
}

- (BOOL)readFrom:(id)a3
{
  return ORCHSchemaORCHNLRouterDecisionGenAIMetadataReadFrom(self, (uint64_t)a3);
}

- (void)deleteGenAIAppIntent
{
  *(unsigned char *)&self->_has &= ~2u;
}

- (void)setHasGenAIAppIntent:(BOOL)a3
{
  if (a3) {
    char v3 = 2;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFD | v3;
}

- (BOOL)hasGenAIAppIntent
{
  return (*(unsigned char *)&self->_has >> 1) & 1;
}

- (void)setGenAIAppIntent:(int)a3
{
  *(unsigned char *)&self->_has |= 2u;
  self->_int genAIAppIntent = a3;
}

- (void)deleteIsExplicit
{
  *(unsigned char *)&self->_has &= ~1u;
}

- (void)setHasIsExplicit:(BOOL)a3
{
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFE | a3;
}

- (BOOL)hasIsExplicit
{
  return *(unsigned char *)&self->_has & 1;
}

- (void)setIsExplicit:(BOOL)a3
{
  *(unsigned char *)&self->_has |= 1u;
  self->_int isExplicit = a3;
}

@end