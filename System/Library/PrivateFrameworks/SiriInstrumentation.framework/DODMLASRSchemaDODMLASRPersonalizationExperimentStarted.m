@interface DODMLASRSchemaDODMLASRPersonalizationExperimentStarted
- (BOOL)exists;
- (BOOL)hasDeviceThermalState;
- (BOOL)hasExists;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (DODMLASRSchemaDODMLASRPersonalizationExperimentStarted)initWithDictionary:(id)a3;
- (DODMLASRSchemaDODMLASRPersonalizationExperimentStarted)initWithJSON:(id)a3;
- (NSData)jsonData;
- (id)dictionaryRepresentation;
- (id)suppressMessageUnderConditions;
- (int)deviceThermalState;
- (unint64_t)hash;
- (void)deleteDeviceThermalState;
- (void)deleteExists;
- (void)setDeviceThermalState:(int)a3;
- (void)setExists:(BOOL)a3;
- (void)setHasDeviceThermalState:(BOOL)a3;
- (void)setHasExists:(BOOL)a3;
- (void)writeTo:(id)a3;
@end

@implementation DODMLASRSchemaDODMLASRPersonalizationExperimentStarted

- (id)suppressMessageUnderConditions
{
  v2 = objc_alloc_init(SISensitiveConditionsSet);
  [(SISensitiveConditionsSet *)v2 addCondition:1];
  return v2;
}

- (int)deviceThermalState
{
  return self->_deviceThermalState;
}

- (BOOL)exists
{
  return self->_exists;
}

- (DODMLASRSchemaDODMLASRPersonalizationExperimentStarted)initWithDictionary:(id)a3
{
  id v4 = a3;
  v10.receiver = self;
  v10.super_class = (Class)DODMLASRSchemaDODMLASRPersonalizationExperimentStarted;
  v5 = [(DODMLASRSchemaDODMLASRPersonalizationExperimentStarted *)&v10 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"exists"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[DODMLASRSchemaDODMLASRPersonalizationExperimentStarted setExists:](v5, "setExists:", [v6 BOOLValue]);
    }
    v7 = [v4 objectForKeyedSubscript:@"deviceThermalState"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[DODMLASRSchemaDODMLASRPersonalizationExperimentStarted setDeviceThermalState:](v5, "setDeviceThermalState:", [v7 intValue]);
    }
    v8 = v5;
  }
  return v5;
}

- (DODMLASRSchemaDODMLASRPersonalizationExperimentStarted)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(DODMLASRSchemaDODMLASRPersonalizationExperimentStarted *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (NSData)jsonData
{
  v2 = [(DODMLASRSchemaDODMLASRPersonalizationExperimentStarted *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  char has = (char)self->_has;
  if ((has & 2) != 0)
  {
    unsigned int v5 = [(DODMLASRSchemaDODMLASRPersonalizationExperimentStarted *)self deviceThermalState] - 1;
    if (v5 > 3) {
      v6 = @"DEVICETHERMALSTATE_UNKNOWN";
    }
    else {
      v6 = off_1E5EAE3C0[v5];
    }
    [v3 setObject:v6 forKeyedSubscript:@"deviceThermalState"];
    char has = (char)self->_has;
  }
  if (has)
  {
    uint64_t v7 = objc_msgSend(NSNumber, "numberWithBool:", -[DODMLASRSchemaDODMLASRPersonalizationExperimentStarted exists](self, "exists"));
    [v3 setObject:v7 forKeyedSubscript:@"exists"];
  }
  [(SISchemaInstrumentationMessage *)self willProduceDictionaryRepresentation:v3];
  return v3;
}

- (unint64_t)hash
{
  if (*(unsigned char *)&self->_has)
  {
    uint64_t v2 = 2654435761 * self->_exists;
    if ((*(unsigned char *)&self->_has & 2) != 0) {
      goto LABEL_3;
    }
LABEL_5:
    uint64_t v3 = 0;
    return v3 ^ v2;
  }
  uint64_t v2 = 0;
  if ((*(unsigned char *)&self->_has & 2) == 0) {
    goto LABEL_5;
  }
LABEL_3:
  uint64_t v3 = 2654435761 * self->_deviceThermalState;
  return v3 ^ v2;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = (unsigned __int8 *)a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_10;
  }
  $DB693F8A8D3875129846F6D623E7C534 has = self->_has;
  unsigned int v6 = v4[16];
  if ((*(unsigned char *)&has & 1) != (v6 & 1)) {
    goto LABEL_10;
  }
  if (*(unsigned char *)&has)
  {
    int exists = self->_exists;
    if (exists != [v4 exists])
    {
LABEL_10:
      BOOL v10 = 0;
      goto LABEL_11;
    }
    $DB693F8A8D3875129846F6D623E7C534 has = self->_has;
    unsigned int v6 = v4[16];
  }
  int v8 = (*(unsigned int *)&has >> 1) & 1;
  if (v8 != ((v6 >> 1) & 1)) {
    goto LABEL_10;
  }
  if (v8)
  {
    int deviceThermalState = self->_deviceThermalState;
    if (deviceThermalState != [v4 deviceThermalState]) {
      goto LABEL_10;
    }
  }
  BOOL v10 = 1;
LABEL_11:

  return v10;
}

- (void)writeTo:(id)a3
{
  id v4 = a3;
  char has = (char)self->_has;
  id v6 = v4;
  if (has)
  {
    PBDataWriterWriteBOOLField();
    id v4 = v6;
    char has = (char)self->_has;
  }
  if ((has & 2) != 0)
  {
    PBDataWriterWriteInt32Field();
    id v4 = v6;
  }
}

- (BOOL)readFrom:(id)a3
{
  return DODMLASRSchemaDODMLASRPersonalizationExperimentStartedReadFrom(self, (uint64_t)a3);
}

- (void)deleteDeviceThermalState
{
  *(unsigned char *)&self->_has &= ~2u;
}

- (void)setHasDeviceThermalState:(BOOL)a3
{
  if (a3) {
    char v3 = 2;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFD | v3;
}

- (BOOL)hasDeviceThermalState
{
  return (*(unsigned char *)&self->_has >> 1) & 1;
}

- (void)setDeviceThermalState:(int)a3
{
  *(unsigned char *)&self->_has |= 2u;
  self->_int deviceThermalState = a3;
}

- (void)deleteExists
{
  *(unsigned char *)&self->_has &= ~1u;
}

- (void)setHasExists:(BOOL)a3
{
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFE | a3;
}

- (BOOL)hasExists
{
  return *(unsigned char *)&self->_has & 1;
}

- (void)setExists:(BOOL)a3
{
  *(unsigned char *)&self->_has |= 1u;
  self->_int exists = a3;
}

@end