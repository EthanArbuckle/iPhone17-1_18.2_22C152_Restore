@interface GATSchemaGATGenerativeRequestEventStarted
- (BOOL)hasLlmAgentName;
- (BOOL)hasMediaQAUseCase;
- (BOOL)hasUsecase;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (GATSchemaGATGenerativeRequestEventStarted)initWithDictionary:(id)a3;
- (GATSchemaGATGenerativeRequestEventStarted)initWithJSON:(id)a3;
- (NSData)jsonData;
- (id)dictionaryRepresentation;
- (id)suppressMessageUnderConditions;
- (int)llmAgentName;
- (int)mediaQAUseCase;
- (int)usecase;
- (unint64_t)hash;
- (void)deleteLlmAgentName;
- (void)deleteMediaQAUseCase;
- (void)deleteUsecase;
- (void)setHasLlmAgentName:(BOOL)a3;
- (void)setHasMediaQAUseCase:(BOOL)a3;
- (void)setHasUsecase:(BOOL)a3;
- (void)setLlmAgentName:(int)a3;
- (void)setMediaQAUseCase:(int)a3;
- (void)setUsecase:(int)a3;
- (void)writeTo:(id)a3;
@end

@implementation GATSchemaGATGenerativeRequestEventStarted

- (id)suppressMessageUnderConditions
{
  v2 = objc_alloc_init(SISensitiveConditionsSet);
  [(SISensitiveConditionsSet *)v2 addCondition:1];
  return v2;
}

- (int)llmAgentName
{
  return self->_llmAgentName;
}

- (int)mediaQAUseCase
{
  return self->_mediaQAUseCase;
}

- (int)usecase
{
  return self->_usecase;
}

- (GATSchemaGATGenerativeRequestEventStarted)initWithDictionary:(id)a3
{
  id v4 = a3;
  v11.receiver = self;
  v11.super_class = (Class)GATSchemaGATGenerativeRequestEventStarted;
  v5 = [(GATSchemaGATGenerativeRequestEventStarted *)&v11 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"usecase"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[GATSchemaGATGenerativeRequestEventStarted setUsecase:](v5, "setUsecase:", [v6 intValue]);
    }
    v7 = [v4 objectForKeyedSubscript:@"mediaQAUseCase"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[GATSchemaGATGenerativeRequestEventStarted setMediaQAUseCase:](v5, "setMediaQAUseCase:", [v7 intValue]);
    }
    v8 = [v4 objectForKeyedSubscript:@"llmAgentName"];
    objc_opt_class();
    if (objc_opt_isKindOfClass()) {
      -[GATSchemaGATGenerativeRequestEventStarted setLlmAgentName:](v5, "setLlmAgentName:", [v8 intValue]);
    }
    v9 = v5;
  }
  return v5;
}

- (GATSchemaGATGenerativeRequestEventStarted)initWithJSON:(id)a3
{
  uint64_t v7 = 0;
  id v4 = [MEMORY[0x1E4F28D90] JSONObjectWithData:a3 options:0 error:&v7];
  if (v7 || (objc_opt_class(), (objc_opt_isKindOfClass() & 1) == 0))
  {
    v5 = 0;
  }
  else
  {
    self = [(GATSchemaGATGenerativeRequestEventStarted *)self initWithDictionary:v4];
    v5 = self;
  }

  return v5;
}

- (NSData)jsonData
{
  v2 = [(GATSchemaGATGenerativeRequestEventStarted *)self dictionaryRepresentation];
  if ([MEMORY[0x1E4F28D90] isValidJSONObject:v2])
  {
    v3 = [MEMORY[0x1E4F28D90] dataWithJSONObject:v2 options:0 error:0];
  }
  else
  {
    v3 = 0;
  }

  return (NSData *)v3;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  char has = (char)self->_has;
  if ((has & 4) == 0)
  {
    if ((*(unsigned char *)&self->_has & 2) == 0) {
      goto LABEL_3;
    }
LABEL_9:
    unsigned int v6 = [(GATSchemaGATGenerativeRequestEventStarted *)self mediaQAUseCase] - 1;
    if (v6 > 3) {
      uint64_t v7 = @"GATMEDIAQNAUSECASE_UNKNOWN";
    }
    else {
      uint64_t v7 = off_1E5EC14F8[v6];
    }
    [v3 setObject:v7 forKeyedSubscript:@"mediaQAUseCase"];
    if ((*(unsigned char *)&self->_has & 1) == 0) {
      goto LABEL_17;
    }
    goto LABEL_13;
  }
  if ([(GATSchemaGATGenerativeRequestEventStarted *)self llmAgentName] == 1) {
    v5 = @"GATLLMAGENT_OPENAI";
  }
  else {
    v5 = @"GATLLMAGENT_UNKNOWN";
  }
  [v3 setObject:v5 forKeyedSubscript:@"llmAgentName"];
  char has = (char)self->_has;
  if ((has & 2) != 0) {
    goto LABEL_9;
  }
LABEL_3:
  if (has)
  {
LABEL_13:
    unsigned int v8 = [(GATSchemaGATGenerativeRequestEventStarted *)self usecase] - 1;
    if (v8 > 5) {
      v9 = @"GATUSECASE_UNKNOWN";
    }
    else {
      v9 = off_1E5EC1518[v8];
    }
    [v3 setObject:v9 forKeyedSubscript:@"usecase"];
  }
LABEL_17:
  [(SISchemaInstrumentationMessage *)self willProduceDictionaryRepresentation:v3];
  return v3;
}

- (unint64_t)hash
{
  if ((*(unsigned char *)&self->_has & 1) == 0)
  {
    uint64_t v2 = 0;
    if ((*(unsigned char *)&self->_has & 2) != 0) {
      goto LABEL_3;
    }
LABEL_6:
    uint64_t v3 = 0;
    if ((*(unsigned char *)&self->_has & 4) != 0) {
      goto LABEL_4;
    }
LABEL_7:
    uint64_t v4 = 0;
    return v3 ^ v2 ^ v4;
  }
  uint64_t v2 = 2654435761 * self->_usecase;
  if ((*(unsigned char *)&self->_has & 2) == 0) {
    goto LABEL_6;
  }
LABEL_3:
  uint64_t v3 = 2654435761 * self->_mediaQAUseCase;
  if ((*(unsigned char *)&self->_has & 4) == 0) {
    goto LABEL_7;
  }
LABEL_4:
  uint64_t v4 = 2654435761 * self->_llmAgentName;
  return v3 ^ v2 ^ v4;
}

- (BOOL)isEqual:(id)a3
{
  uint64_t v4 = (unsigned __int8 *)a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_14;
  }
  $3C9A0042CCEA8B2F362F95C182B5AF50 has = self->_has;
  unsigned int v6 = v4[20];
  if ((*(unsigned char *)&has & 1) != (v6 & 1)) {
    goto LABEL_14;
  }
  if (*(unsigned char *)&has)
  {
    int usecase = self->_usecase;
    if (usecase != [v4 usecase]) {
      goto LABEL_14;
    }
    $3C9A0042CCEA8B2F362F95C182B5AF50 has = self->_has;
    unsigned int v6 = v4[20];
  }
  int v8 = (*(unsigned int *)&has >> 1) & 1;
  if (v8 != ((v6 >> 1) & 1)) {
    goto LABEL_14;
  }
  if (v8)
  {
    int mediaQAUseCase = self->_mediaQAUseCase;
    if (mediaQAUseCase == [v4 mediaQAUseCase])
    {
      $3C9A0042CCEA8B2F362F95C182B5AF50 has = self->_has;
      unsigned int v6 = v4[20];
      goto LABEL_10;
    }
LABEL_14:
    BOOL v12 = 0;
    goto LABEL_15;
  }
LABEL_10:
  int v10 = (*(unsigned int *)&has >> 2) & 1;
  if (v10 != ((v6 >> 2) & 1)) {
    goto LABEL_14;
  }
  if (v10)
  {
    int llmAgentName = self->_llmAgentName;
    if (llmAgentName != [v4 llmAgentName]) {
      goto LABEL_14;
    }
  }
  BOOL v12 = 1;
LABEL_15:

  return v12;
}

- (void)writeTo:(id)a3
{
  id v4 = a3;
  char has = (char)self->_has;
  id v6 = v4;
  if (has)
  {
    PBDataWriterWriteInt32Field();
    id v4 = v6;
    char has = (char)self->_has;
    if ((has & 2) == 0)
    {
LABEL_3:
      if ((has & 4) == 0) {
        goto LABEL_5;
      }
      goto LABEL_4;
    }
  }
  else if ((*(unsigned char *)&self->_has & 2) == 0)
  {
    goto LABEL_3;
  }
  PBDataWriterWriteInt32Field();
  id v4 = v6;
  if ((*(unsigned char *)&self->_has & 4) != 0)
  {
LABEL_4:
    PBDataWriterWriteInt32Field();
    id v4 = v6;
  }
LABEL_5:
}

- (BOOL)readFrom:(id)a3
{
  return GATSchemaGATGenerativeRequestEventStartedReadFrom(self, (uint64_t)a3);
}

- (void)deleteLlmAgentName
{
  *(unsigned char *)&self->_has &= ~4u;
}

- (void)setHasLlmAgentName:(BOOL)a3
{
  if (a3) {
    char v3 = 4;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFB | v3;
}

- (BOOL)hasLlmAgentName
{
  return (*(unsigned char *)&self->_has >> 2) & 1;
}

- (void)setLlmAgentName:(int)a3
{
  *(unsigned char *)&self->_has |= 4u;
  self->_int llmAgentName = a3;
}

- (void)deleteMediaQAUseCase
{
  *(unsigned char *)&self->_has &= ~2u;
}

- (void)setHasMediaQAUseCase:(BOOL)a3
{
  if (a3) {
    char v3 = 2;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFD | v3;
}

- (BOOL)hasMediaQAUseCase
{
  return (*(unsigned char *)&self->_has >> 1) & 1;
}

- (void)setMediaQAUseCase:(int)a3
{
  *(unsigned char *)&self->_has |= 2u;
  self->_int mediaQAUseCase = a3;
}

- (void)deleteUsecase
{
  *(unsigned char *)&self->_has &= ~1u;
}

- (void)setHasUsecase:(BOOL)a3
{
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFE | a3;
}

- (BOOL)hasUsecase
{
  return *(unsigned char *)&self->_has & 1;
}

- (void)setUsecase:(int)a3
{
  *(unsigned char *)&self->_has |= 1u;
  self->_int usecase = a3;
}

@end