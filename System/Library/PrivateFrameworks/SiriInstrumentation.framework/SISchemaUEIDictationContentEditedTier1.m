...
- (NSData)jsonData;
- (SISchemaUEIDictationContentEditedTier1)initWithDictionary:(id)a3;
- (SISchemaUEIDictationContentEditedTier1)initWithJSON:(id)a3;
...
- (id)applySensitiveConditionsPolicy:(id)a3
{
  id v4 = a3;
  v9.receiver = self;
  v9.super_class = (Class)SISchemaUEIDictationContentEditedTier1;
  ...
  v6 = [(SISchemaUEIDictationContentEditedTier1 *)self autoPunctuationEdits];
...
- (SISchemaUEIDictationContentEditedTier1)initWithDictionary:(id)a3
{
  ...
  id v4 = a3;
  v21.receiver = self;
  v21.super_class = (Class)SISchemaUEIDictationContentEditedTier1;
  v5 = [(SISchemaUEIDictationContentEditedTier1 *)&v21 init];
  if (v5)
  {
    v6 = [v4 objectForKeyedSubscript:@"autoPunctuationEdits"];
    objc_opt_class();
        ...
        do
        {
          ...
          do
          {
            ...
            if (objc_opt_isKindOfClass())
            {
              ...
              v14 = -[SISchemaAutoPunctuationEdit initWithDictionary:](v13, "initWithDictionary:", v12, (void)v17);
...
- (NSData)jsonData
{
  v2 = [(SISchemaUEIDictationContentEditedTier1 *)self dictionaryRepresentation];
...
- (unint64_t)hash
{
  return [(NSArray *)self->_autoPunctuationEdits hash];
}
...
- (BOOL)isEqual:(id)a3
{
  ...
  {
    v5 = [(SISchemaUEIDictationContentEditedTier1 *)self autoPunctuationEdits];
    uint64_t v6 = [v4 autoPunctuationEdits];
    uint64_t v7 = v6;
    if ((v5 == 0) != (v6 != 0))
    {
      uint64_t v8 = [(SISchemaUEIDictationContentEditedTier1 *)self autoPunctuationEdits];
      if (!v8)
      {
...
LABEL_10:
      ...
      uint64_t v9 = (void *)v8;
      uint64_t v10 = [(SISchemaUEIDictationContentEditedTier1 *)self autoPunctuationEdits];
      uint64_t v11 = [v4 autoPunctuationEdits];
...
- (BOOL)readFrom:(id)a3
{
  return SISchemaUEIDictationContentEditedTier1ReadFrom(self, (uint64_t)a3);
}
