...
- (void)setCancellationInitiator:(int)a3;
- (void)setHasCancellationInitiator:(BOOL)a3;
- (void)writeTo:(id)a3;
@end
...
- (int)cancellationInitiator
{
  return self->_cancellationInitiator;
}
...
- (FLOWLINKSchemaFLOWLinkActionCancelled)initWithDictionary:(id)a3
{
  id v4 = a3;
  v9.receiver = self;
  v9.super_class = (Class)FLOWLINKSchemaFLOWLinkActionCancelled;
...
- (id)dictionaryRepresentation
{
  ...
  {
    int v4 = [(FLOWLINKSchemaFLOWLinkActionCancelled *)self cancellationInitiator];
  ...
  if ([v4 isMemberOfClass:objc_opt_class()])
  {
    if ((*(unsigned char *)&self->_has & 1) == (v4[12] & 1))
    {
      if ((*(unsigned char *)&self->_has & 1) == 0
        || (int cancellationInitiator = self->_cancellationInitiator,
            cancellationInitiator == [v4 cancellationInitiator]))
      {
        BOOL v6 = 1;
      }
    }
...
- (BOOL)readFrom:(id)a3
{
  return FLOWLINKSchemaFLOWLinkActionCancelledReadFrom(self, (uint64_t)a3);
}
...
{
  *(unsigned char *)&self->_has |= 1u;
  self->_int cancellationInitiator = a3;
}
