@interface NPKProtoOpenURLRequest
- (BOOL)hasUrlString;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (NSString)urlString;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
- (id)dictionaryRepresentation;
- (unint64_t)hash;
- (void)copyTo:(id)a3;
- (void)mergeFrom:(id)a3;
- (void)setUrlString:(id)a3;
- (void)writeTo:(id)a3;
@end

@implementation NPKProtoOpenURLRequest

- (BOOL)hasUrlString
{
  return self->_urlString != 0;
}

- (id)description
{
  v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)NPKProtoOpenURLRequest;
  v4 = [(NPKProtoOpenURLRequest *)&v8 description];
  v5 = [(NPKProtoOpenURLRequest *)self dictionaryRepresentation];
  v6 = [v3 stringWithFormat:@"%@ %@", v4, v5];

  return v6;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x263EFF9A0] dictionary];
  v4 = v3;
  urlString = self->_urlString;
  if (urlString) {
    [v3 setObject:urlString forKey:@"urlString"];
  }
  return v4;
}

- (BOOL)readFrom:(id)a3
{
  return NPKProtoOpenURLRequestReadFrom((uint64_t)self, (uint64_t)a3);
}

- (void)writeTo:(id)a3
{
  if (self->_urlString) {
    PBDataWriterWriteStringField();
  }
}

- (void)copyTo:(id)a3
{
  urlString = self->_urlString;
  if (urlString) {
    [a3 setUrlString:urlString];
  }
}

- (id)copyWithZone:(_NSZone *)a3
{
  v5 = objc_msgSend((id)objc_msgSend((id)objc_opt_class(), "allocWithZone:", a3), "init");
  uint64_t v6 = [(NSString *)self->_urlString copyWithZone:a3];
  v7 = (void *)v5[1];
  v5[1] = v6;

  return v5;
}

- (BOOL)isEqual:(id)a3
{
  v4 = a3;
  if ([v4 isMemberOfClass:objc_opt_class()])
  {
    urlString = self->_urlString;
    if ((unint64_t)urlString | v4[1]) {
      char v6 = -[NSString isEqual:](urlString, "isEqual:");
    }
    else {
      char v6 = 1;
    }
  }
  else
  {
    char v6 = 0;
  }

  return v6;
}

- (unint64_t)hash
{
  return [(NSString *)self->_urlString hash];
}

- (void)mergeFrom:(id)a3
{
  if (*((void *)a3 + 1)) {
    -[NPKProtoOpenURLRequest setUrlString:](self, "setUrlString:");
  }
}

- (NSString)urlString
{
  return self->_urlString;
}

- (void)setUrlString:(id)a3
{
}

- (void).cxx_destruct
{
}

@end