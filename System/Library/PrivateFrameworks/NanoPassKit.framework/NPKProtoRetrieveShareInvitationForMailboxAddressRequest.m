...
- (id)description
{
  v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)NPKProtoRetrieveShareInvitationForMailboxAddressRequest;
...
- (BOOL)readFrom:(id)a3
{
  return NPKProtoRetrieveShareInvitationForMailboxAddressRequestReadFrom((uint64_t)self, (uint64_t)a3);
}
...
- (BOOL)isEqual:(id)a3
{
  ...
  if ([v4 isMemberOfClass:objc_opt_class()])
  {
    ...
    if ((unint64_t)mailboxAddress | v4[1]) {
      char v6 = -[NSString isEqual:](mailboxAddress, "isEqual:");
    }
    else {
      char v6 = 1;
    }
...
- (unint64_t)hash
{
  return [(NSString *)self->_mailboxAddress hash];
}

- (void)mergeFrom:(id)a3
{
  if (*((void *)a3 + 1)) {
    -[NPKProtoRetrieveShareInvitationForMailboxAddressRequest setMailboxAddress:](self, "setMailboxAddress:");
  }
}
...
- (void)writeTo:.cold.1()
{
  __assert_rtn("-[NPKProtoRetrieveShareInvitationForMailboxAddressRequest writeTo:]", "NPKProtoRetrieveShareInvitationForMailboxAddressRequest.m", 72, "nil != self->_mailboxAddress");
}
