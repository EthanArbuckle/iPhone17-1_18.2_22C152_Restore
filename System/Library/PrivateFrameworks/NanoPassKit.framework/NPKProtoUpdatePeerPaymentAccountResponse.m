@interface NPKProtoUpdatePeerPaymentAccountResponse
- (BOOL)hasPeerPaymentAccountData;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (NSData)peerPaymentAccountData;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
- (id)dictionaryRepresentation;
- (unint64_t)hash;
- (void)copyTo:(id)a3;
- (void)mergeFrom:(id)a3;
- (void)setPeerPaymentAccountData:(id)a3;
- (void)writeTo:(id)a3;
@end

@implementation NPKProtoUpdatePeerPaymentAccountResponse

- (BOOL)hasPeerPaymentAccountData
{
  return self->_peerPaymentAccountData != 0;
}

- (id)description
{
  v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)NPKProtoUpdatePeerPaymentAccountResponse;
  v4 = [(NPKProtoUpdatePeerPaymentAccountResponse *)&v8 description];
  v5 = [(NPKProtoUpdatePeerPaymentAccountResponse *)self dictionaryRepresentation];
  v6 = [v3 stringWithFormat:@"%@ %@", v4, v5];

  return v6;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x263EFF9A0] dictionary];
  v4 = v3;
  peerPaymentAccountData = self->_peerPaymentAccountData;
  if (peerPaymentAccountData) {
    [v3 setObject:peerPaymentAccountData forKey:@"peerPaymentAccountData"];
  }
  return v4;
}

- (BOOL)readFrom:(id)a3
{
  return NPKProtoUpdatePeerPaymentAccountResponseReadFrom((uint64_t)self, (uint64_t)a3);
}

- (void)writeTo:(id)a3
{
  if (self->_peerPaymentAccountData) {
    PBDataWriterWriteDataField();
  }
}

- (void)copyTo:(id)a3
{
  peerPaymentAccountData = self->_peerPaymentAccountData;
  if (peerPaymentAccountData) {
    [a3 setPeerPaymentAccountData:peerPaymentAccountData];
  }
}

- (id)copyWithZone:(_NSZone *)a3
{
  v5 = objc_msgSend((id)objc_msgSend((id)objc_opt_class(), "allocWithZone:", a3), "init");
  uint64_t v6 = [(NSData *)self->_peerPaymentAccountData copyWithZone:a3];
  v7 = (void *)v5[1];
  v5[1] = v6;

  return v5;
}

- (BOOL)isEqual:(id)a3
{
  v4 = a3;
  if ([v4 isMemberOfClass:objc_opt_class()])
  {
    peerPaymentAccountData = self->_peerPaymentAccountData;
    if ((unint64_t)peerPaymentAccountData | v4[1]) {
      char v6 = -[NSData isEqual:](peerPaymentAccountData, "isEqual:");
    }
    else {
      char v6 = 1;
    }
  }
  else
  {
    char v6 = 0;
  }

  return v6;
}

- (unint64_t)hash
{
  return [(NSData *)self->_peerPaymentAccountData hash];
}

- (void)mergeFrom:(id)a3
{
  if (*((void *)a3 + 1)) {
    -[NPKProtoUpdatePeerPaymentAccountResponse setPeerPaymentAccountData:](self, "setPeerPaymentAccountData:");
  }
}

- (NSData)peerPaymentAccountData
{
  return self->_peerPaymentAccountData;
}

- (void)setPeerPaymentAccountData:(id)a3
{
}

- (void).cxx_destruct
{
}

@end