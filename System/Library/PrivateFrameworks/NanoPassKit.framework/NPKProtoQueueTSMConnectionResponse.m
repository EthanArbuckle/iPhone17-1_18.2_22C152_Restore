...
- (id)description
{
  v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)NPKProtoQueueTSMConnectionResponse;
...
- (BOOL)readFrom:(id)a3
{
  return NPKProtoQueueTSMConnectionResponseReadFrom((uint64_t)self, (uint64_t)a3);
}
...
- (void)copyTo:(id)a3
{
  if (*(unsigned char *)&self->_has)
  {
    *((unsigned char *)a3 + 8) = self->_pending;
...
- (void)mergeFrom:(id)a3
{
  ...
  {
    self->_pending = *((unsigned char *)a3 + 8);
    *(unsigned char *)&self->_has |= 1u;
  }
}
