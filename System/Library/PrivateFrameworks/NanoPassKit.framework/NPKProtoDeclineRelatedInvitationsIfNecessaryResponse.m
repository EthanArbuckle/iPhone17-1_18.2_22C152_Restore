@interface NPKProtoDeclineRelatedInvitationsIfNecessaryResponse
...
- (id)description
{
  ...
  v4 = [(NPKProtoDeclineRelatedInvitationsIfNecessaryResponse *)&v8 description];
  v5 = [(NPKProtoDeclineRelatedInvitationsIfNecessaryResponse *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  {
    v4 = [NSNumber numberWithBool:self->_didDeclineInvitations];
    [v3 setObject:v4 forKey:@"didDeclineInvitations"];
  }
  return v3;
}
...
- (void)copyTo:(id)a3
{
  if (*(unsigned char *)&self->_has)
  {
    *((unsigned char *)a3 + 8) = self->_didDeclineInvitations;
...
- (id)copyWithZone:(_NSZone *)a3
{
  ...
  if (*(unsigned char *)&self->_has)
  {
    *((unsigned char *)result + 8) = self->_didDeclineInvitations;
...
- (void)mergeFrom:(id)a3
{
  ...
  {
    self->_didDeclineInvitations = *((unsigned char *)a3 + 8);
    *(unsigned char *)&self->_has |= 1u;
  }
}
