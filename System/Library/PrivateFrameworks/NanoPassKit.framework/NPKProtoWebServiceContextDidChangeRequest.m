@interface NPKProtoWebServiceContextDidChangeRequest
- (BOOL)hasWebServiceContextBytes;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (NSData)webServiceContextBytes;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
- (id)dictionaryRepresentation;
- (unint64_t)hash;
- (void)copyTo:(id)a3;
- (void)mergeFrom:(id)a3;
- (void)setWebServiceContextBytes:(id)a3;
- (void)writeTo:(id)a3;
@end

@implementation NPKProtoWebServiceContextDidChangeRequest

- (BOOL)hasWebServiceContextBytes
{
  return self->_webServiceContextBytes != 0;
}

- (id)description
{
  v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)NPKProtoWebServiceContextDidChangeRequest;
  v4 = [(NPKProtoWebServiceContextDidChangeRequest *)&v8 description];
  v5 = [(NPKProtoWebServiceContextDidChangeRequest *)self dictionaryRepresentation];
  v6 = [v3 stringWithFormat:@"%@ %@", v4, v5];

  return v6;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x263EFF9A0] dictionary];
  v4 = v3;
  webServiceContextBytes = self->_webServiceContextBytes;
  if (webServiceContextBytes) {
    [v3 setObject:webServiceContextBytes forKey:@"webServiceContextBytes"];
  }
  return v4;
}

- (BOOL)readFrom:(id)a3
{
  return NPKProtoWebServiceContextDidChangeRequestReadFrom((uint64_t)self, (uint64_t)a3);
}

- (void)writeTo:(id)a3
{
  if (self->_webServiceContextBytes) {
    PBDataWriterWriteDataField();
  }
}

- (void)copyTo:(id)a3
{
  webServiceContextBytes = self->_webServiceContextBytes;
  if (webServiceContextBytes) {
    [a3 setWebServiceContextBytes:webServiceContextBytes];
  }
}

- (id)copyWithZone:(_NSZone *)a3
{
  v5 = objc_msgSend((id)objc_msgSend((id)objc_opt_class(), "allocWithZone:", a3), "init");
  uint64_t v6 = [(NSData *)self->_webServiceContextBytes copyWithZone:a3];
  v7 = (void *)v5[1];
  v5[1] = v6;

  return v5;
}

- (BOOL)isEqual:(id)a3
{
  v4 = a3;
  if ([v4 isMemberOfClass:objc_opt_class()])
  {
    webServiceContextBytes = self->_webServiceContextBytes;
    if ((unint64_t)webServiceContextBytes | v4[1]) {
      char v6 = -[NSData isEqual:](webServiceContextBytes, "isEqual:");
    }
    else {
      char v6 = 1;
    }
  }
  else
  {
    char v6 = 0;
  }

  return v6;
}

- (unint64_t)hash
{
  return [(NSData *)self->_webServiceContextBytes hash];
}

- (void)mergeFrom:(id)a3
{
  if (*((void *)a3 + 1)) {
    -[NPKProtoWebServiceContextDidChangeRequest setWebServiceContextBytes:](self, "setWebServiceContextBytes:");
  }
}

- (NSData)webServiceContextBytes
{
  return self->_webServiceContextBytes;
}

- (void)setWebServiceContextBytes:(id)a3
{
}

- (void).cxx_destruct
{
}

@end