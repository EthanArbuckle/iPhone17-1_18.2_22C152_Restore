...
- (id)description
{
  v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)NPKProtoPrewarmCreateShareForPassIdentifierRequest;
...
- (BOOL)readFrom:(id)a3
{
  return NPKProtoPrewarmCreateShareForPassIdentifierRequestReadFrom((uint64_t)self, (uint64_t)a3);
}
...
{
  if (*((void *)a3 + 1)) {
    -[NPKProtoPrewarmCreateShareForPassIdentifierRequest setPassIdentifier:](self, "setPassIdentifier:");
  }
}
...
- (void)writeTo:.cold.1()
{
  __assert_rtn("-[NPKProtoPrewarmCreateShareForPassIdentifierRequest writeTo:]", "NPKProtoPrewarmCreateShareForPassIdentifierRequest.m", 72, "nil != self->_passIdentifier");
}
