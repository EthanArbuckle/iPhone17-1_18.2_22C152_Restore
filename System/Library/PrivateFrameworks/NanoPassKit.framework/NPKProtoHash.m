@interface NPKProtoHash
- (BOOL)hasHashData;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (NSData)hashData;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
- (id)dictionaryRepresentation;
- (unint64_t)hash;
- (void)copyTo:(id)a3;
- (void)mergeFrom:(id)a3;
- (void)setHashData:(id)a3;
- (void)writeTo:(id)a3;
@end

@implementation NPKProtoHash

- (BOOL)hasHashData
{
  return self->_hashData != 0;
}

- (id)description
{
  v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)NPKProtoHash;
  v4 = [(NPKProtoHash *)&v8 description];
  v5 = [(NPKProtoHash *)self dictionaryRepresentation];
  v6 = [v3 stringWithFormat:@"%@ %@", v4, v5];

  return v6;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x263EFF9A0] dictionary];
  v4 = v3;
  hashData = self->_hashData;
  if (hashData) {
    [v3 setObject:hashData forKey:@"hashData"];
  }
  return v4;
}

- (BOOL)readFrom:(id)a3
{
  return NPKProtoHashReadFrom((uint64_t)self, (uint64_t)a3);
}

- (void)writeTo:(id)a3
{
  if (self->_hashData) {
    PBDataWriterWriteDataField();
  }
}

- (void)copyTo:(id)a3
{
  hashData = self->_hashData;
  if (hashData) {
    [a3 setHashData:hashData];
  }
}

- (id)copyWithZone:(_NSZone *)a3
{
  v5 = objc_msgSend((id)objc_msgSend((id)objc_opt_class(), "allocWithZone:", a3), "init");
  uint64_t v6 = [(NSData *)self->_hashData copyWithZone:a3];
  v7 = (void *)v5[1];
  v5[1] = v6;

  return v5;
}

- (BOOL)isEqual:(id)a3
{
  v4 = a3;
  if ([v4 isMemberOfClass:objc_opt_class()])
  {
    hashData = self->_hashData;
    if ((unint64_t)hashData | v4[1]) {
      char v6 = -[NSData isEqual:](hashData, "isEqual:");
    }
    else {
      char v6 = 1;
    }
  }
  else
  {
    char v6 = 0;
  }

  return v6;
}

- (unint64_t)hash
{
  return [(NSData *)self->_hashData hash];
}

- (void)mergeFrom:(id)a3
{
  if (*((void *)a3 + 1)) {
    -[NPKProtoHash setHashData:](self, "setHashData:");
  }
}

- (NSData)hashData
{
  return self->_hashData;
}

- (void)setHashData:(id)a3
{
}

- (void).cxx_destruct
{
}

@end