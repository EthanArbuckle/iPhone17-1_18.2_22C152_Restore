...
- (id)description
{
  v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)NPKProtoPass;
  v4 = [(NPKProtoPass *)&v8 description];
  v5 = [(NPKProtoPass *)self dictionaryRepresentation];
...
- (BOOL)readFrom:(id)a3
{
  return NPKProtoPassReadFrom((uint64_t)self, (uint64_t)a3);
}
...
{
  if (*((void *)a3 + 1)) {
    -[NPKProtoPass setPassData:](self, "setPassData:");
  }
}
...
- (void)writeTo:.cold.1()
{
  __assert_rtn("-[NPKProtoPass writeTo:]", "NPKProtoPass.m", 72, "nil != self->_passData");
}
