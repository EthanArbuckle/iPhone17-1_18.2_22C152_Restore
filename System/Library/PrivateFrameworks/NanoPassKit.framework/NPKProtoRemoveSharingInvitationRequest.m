...
- (id)description
{
  v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)NPKProtoRemoveSharingInvitationRequest;
  v4 = [(NPKProtoRemoveSharingInvitationRequest *)&v8 description];
  v5 = [(NPKProtoRemoveSharingInvitationRequest *)self dictionaryRepresentation];
...
- (BOOL)readFrom:(id)a3
{
  return NPKProtoRemoveSharingInvitationRequestReadFrom((uint64_t)self, (uint64_t)a3);
}
...
{
  if (*((void *)a3 + 1)) {
    -[NPKProtoRemoveSharingInvitationRequest setInvitationData:](self, "setInvitationData:");
  }
}
