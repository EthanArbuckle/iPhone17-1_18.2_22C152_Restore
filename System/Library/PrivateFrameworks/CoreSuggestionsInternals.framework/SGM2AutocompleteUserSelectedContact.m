@interface SGM2AutocompleteUserSelectedContact
- (BOOL)hasApp;
- (BOOL)hasKey;
- (BOOL)hasWasKnownContact;
- (BOOL)hasWasSuggestedContact;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (BOOL)wasKnownContact;
- (BOOL)wasSuggestedContact;
- (NSString)key;
- (id)appAsString:(int)a3;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
- (id)dictionaryRepresentation;
- (int)StringAsApp:(id)a3;
- (int)app;
- (unint64_t)hash;
- (void)copyTo:(id)a3;
- (void)mergeFrom:(id)a3;
- (void)setApp:(int)a3;
- (void)setHasApp:(BOOL)a3;
- (void)setHasWasKnownContact:(BOOL)a3;
- (void)setHasWasSuggestedContact:(BOOL)a3;
- (void)setKey:(id)a3;
- (void)setWasKnownContact:(BOOL)a3;
- (void)setWasSuggestedContact:(BOOL)a3;
- (void)writeTo:(id)a3;
@end

@implementation SGM2AutocompleteUserSelectedContact

- (void).cxx_destruct
{
}

- (BOOL)wasKnownContact
{
  return self->_wasKnownContact;
}

- (BOOL)wasSuggestedContact
{
  return self->_wasSuggestedContact;
}

- (void)setKey:(id)a3
{
}

- (void)mergeFrom:(id)a3
{
  id v4 = a3;
  if (*((void *)v4 + 2))
  {
    id v6 = v4;
    -[SGM2AutocompleteUserSelectedContact setKey:](self, "setKey:");
    id v4 = v6;
  }
  char v5 = *((unsigned char *)v4 + 28);
  if ((v5 & 4) != 0)
  {
    self->_wasSuggestedContact = *((unsigned char *)v4 + 25);
    *(unsigned char *)&self->_has |= 4u;
    char v5 = *((unsigned char *)v4 + 28);
    if ((v5 & 2) == 0)
    {
LABEL_5:
      if ((v5 & 1) == 0) {
        goto LABEL_7;
      }
      goto LABEL_6;
    }
  }
  else if ((*((unsigned char *)v4 + 28) & 2) == 0)
  {
    goto LABEL_5;
  }
  self->_wasKnownContact = *((unsigned char *)v4 + 24);
  *(unsigned char *)&self->_has |= 2u;
  if (*((unsigned char *)v4 + 28))
  {
LABEL_6:
    self->_app = *((_DWORD *)v4 + 2);
    *(unsigned char *)&self->_has |= 1u;
  }
LABEL_7:
}

- (unint64_t)hash
{
  NSUInteger v3 = [(NSString *)self->_key hash];
  if ((*(unsigned char *)&self->_has & 4) == 0)
  {
    uint64_t v4 = 0;
    if ((*(unsigned char *)&self->_has & 2) != 0) {
      goto LABEL_3;
    }
LABEL_6:
    uint64_t v5 = 0;
    if (*(unsigned char *)&self->_has) {
      goto LABEL_4;
    }
LABEL_7:
    uint64_t v6 = 0;
    return v4 ^ v3 ^ v5 ^ v6;
  }
  uint64_t v4 = 2654435761 * self->_wasSuggestedContact;
  if ((*(unsigned char *)&self->_has & 2) == 0) {
    goto LABEL_6;
  }
LABEL_3:
  uint64_t v5 = 2654435761 * self->_wasKnownContact;
  if ((*(unsigned char *)&self->_has & 1) == 0) {
    goto LABEL_7;
  }
LABEL_4:
  uint64_t v6 = 2654435761 * self->_app;
  return v4 ^ v3 ^ v5 ^ v6;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_24;
  }
  key = self->_key;
  if ((unint64_t)key | *((void *)v4 + 2))
  {
    if (!-[NSString isEqual:](key, "isEqual:")) {
      goto LABEL_24;
    }
  }
  if ((*(unsigned char *)&self->_has & 4) != 0)
  {
    if ((*((unsigned char *)v4 + 28) & 4) == 0) {
      goto LABEL_24;
    }
    if (self->_wasSuggestedContact)
    {
      if (!*((unsigned char *)v4 + 25)) {
        goto LABEL_24;
      }
    }
    else if (*((unsigned char *)v4 + 25))
    {
      goto LABEL_24;
    }
  }
  else if ((*((unsigned char *)v4 + 28) & 4) != 0)
  {
    goto LABEL_24;
  }
  if ((*(unsigned char *)&self->_has & 2) != 0)
  {
    if ((*((unsigned char *)v4 + 28) & 2) != 0)
    {
      if (self->_wasKnownContact)
      {
        if (!*((unsigned char *)v4 + 24)) {
          goto LABEL_24;
        }
        goto LABEL_20;
      }
      if (!*((unsigned char *)v4 + 24)) {
        goto LABEL_20;
      }
    }
LABEL_24:
    BOOL v6 = 0;
    goto LABEL_25;
  }
  if ((*((unsigned char *)v4 + 28) & 2) != 0) {
    goto LABEL_24;
  }
LABEL_20:
  BOOL v6 = (*((unsigned char *)v4 + 28) & 1) == 0;
  if (*(unsigned char *)&self->_has)
  {
    if ((*((unsigned char *)v4 + 28) & 1) == 0 || self->_app != *((_DWORD *)v4 + 2)) {
      goto LABEL_24;
    }
    BOOL v6 = 1;
  }
LABEL_25:

  return v6;
}

- (id)copyWithZone:(_NSZone *)a3
{
  uint64_t v5 = objc_msgSend((id)objc_msgSend((id)objc_opt_class(), "allocWithZone:", a3), "init");
  uint64_t v6 = [(NSString *)self->_key copyWithZone:a3];
  v7 = *(void **)(v5 + 16);
  *(void *)(v5 + 16) = v6;

  char has = (char)self->_has;
  if ((has & 4) == 0)
  {
    if ((*(unsigned char *)&self->_has & 2) == 0) {
      goto LABEL_3;
    }
LABEL_7:
    *(unsigned char *)(v5 + 24) = self->_wasKnownContact;
    *(unsigned char *)(v5 + 28) |= 2u;
    if ((*(unsigned char *)&self->_has & 1) == 0) {
      return (id)v5;
    }
    goto LABEL_4;
  }
  *(unsigned char *)(v5 + 25) = self->_wasSuggestedContact;
  *(unsigned char *)(v5 + 28) |= 4u;
  char has = (char)self->_has;
  if ((has & 2) != 0) {
    goto LABEL_7;
  }
LABEL_3:
  if (has)
  {
LABEL_4:
    *(_DWORD *)(v5 + 8) = self->_app;
    *(unsigned char *)(v5 + 28) |= 1u;
  }
  return (id)v5;
}

- (void)copyTo:(id)a3
{
  id v4 = a3;
  if (self->_key)
  {
    id v6 = v4;
    objc_msgSend(v4, "setKey:");
    id v4 = v6;
  }
  char has = (char)self->_has;
  if ((has & 4) != 0)
  {
    *((unsigned char *)v4 + 25) = self->_wasSuggestedContact;
    *((unsigned char *)v4 + 28) |= 4u;
    char has = (char)self->_has;
    if ((has & 2) == 0)
    {
LABEL_5:
      if ((has & 1) == 0) {
        goto LABEL_7;
      }
      goto LABEL_6;
    }
  }
  else if ((*(unsigned char *)&self->_has & 2) == 0)
  {
    goto LABEL_5;
  }
  *((unsigned char *)v4 + 24) = self->_wasKnownContact;
  *((unsigned char *)v4 + 28) |= 2u;
  if (*(unsigned char *)&self->_has)
  {
LABEL_6:
    *((_DWORD *)v4 + 2) = self->_app;
    *((unsigned char *)v4 + 28) |= 1u;
  }
LABEL_7:
}

- (void)writeTo:(id)a3
{
  id v4 = a3;
  id v6 = v4;
  if (self->_key)
  {
    PBDataWriterWriteStringField();
    id v4 = v6;
  }
  char has = (char)self->_has;
  if ((has & 4) != 0)
  {
    PBDataWriterWriteBOOLField();
    id v4 = v6;
    char has = (char)self->_has;
    if ((has & 2) == 0)
    {
LABEL_5:
      if ((has & 1) == 0) {
        goto LABEL_7;
      }
      goto LABEL_6;
    }
  }
  else if ((*(unsigned char *)&self->_has & 2) == 0)
  {
    goto LABEL_5;
  }
  PBDataWriterWriteBOOLField();
  id v4 = v6;
  if (*(unsigned char *)&self->_has)
  {
LABEL_6:
    PBDataWriterWriteInt32Field();
    id v4 = v6;
  }
LABEL_7:
}

- (BOOL)readFrom:(id)a3
{
  return SGM2AutocompleteUserSelectedContactReadFrom((uint64_t)self, (uint64_t)a3);
}

- (id)dictionaryRepresentation
{
  NSUInteger v3 = [MEMORY[0x1E4F1CA60] dictionary];
  id v4 = v3;
  key = self->_key;
  if (key) {
    [v3 setObject:key forKey:@"key"];
  }
  char has = (char)self->_has;
  if ((has & 4) == 0)
  {
    if ((*(unsigned char *)&self->_has & 2) == 0) {
      goto LABEL_5;
    }
LABEL_8:
    v8 = [NSNumber numberWithBool:self->_wasKnownContact];
    [v4 setObject:v8 forKey:@"wasKnownContact"];

    if ((*(unsigned char *)&self->_has & 1) == 0) {
      goto LABEL_13;
    }
LABEL_9:
    uint64_t app = self->_app;
    if (app >= 0xC)
    {
      objc_msgSend(NSString, "stringWithFormat:", @"(unknown: %i)", self->_app);
      v10 = (__CFString *)objc_claimAutoreleasedReturnValue();
    }
    else
    {
      v10 = off_1E65B5210[app];
    }
    [v4 setObject:v10 forKey:@"app"];

    goto LABEL_13;
  }
  v7 = [NSNumber numberWithBool:self->_wasSuggestedContact];
  [v4 setObject:v7 forKey:@"wasSuggestedContact"];

  char has = (char)self->_has;
  if ((has & 2) != 0) {
    goto LABEL_8;
  }
LABEL_5:
  if (has) {
    goto LABEL_9;
  }
LABEL_13:
  return v4;
}

- (id)description
{
  NSUInteger v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)SGM2AutocompleteUserSelectedContact;
  id v4 = [(SGM2AutocompleteUserSelectedContact *)&v8 description];
  uint64_t v5 = [(SGM2AutocompleteUserSelectedContact *)self dictionaryRepresentation];
  id v6 = [v3 stringWithFormat:@"%@ %@", v4, v5];

  return v6;
}

- (int)StringAsApp:(id)a3
{
  id v3 = a3;
  if ([v3 isEqualToString:@"SGMContactDetailUsedAppMessages"])
  {
    int v4 = 0;
  }
  else if ([v3 isEqualToString:@"SGMContactDetailUsedAppPhone"])
  {
    int v4 = 1;
  }
  else if ([v3 isEqualToString:@"SGMContactDetailUsedAppMail"])
  {
    int v4 = 2;
  }
  else if ([v3 isEqualToString:@"SGMContactDetailUsedAppCalendar"])
  {
    int v4 = 3;
  }
  else if ([v3 isEqualToString:@"SGMContactDetailUsedAppOther"])
  {
    int v4 = 4;
  }
  else if ([v3 isEqualToString:@"SGMContactDetailUsedAppFaceTime"])
  {
    int v4 = 5;
  }
  else if ([v3 isEqualToString:@"SGMContactDetailUsedAppAddressBook"])
  {
    int v4 = 6;
  }
  else if ([v3 isEqualToString:@"SGMContactDetailUsedAppSpringBoard"])
  {
    int v4 = 7;
  }
  else if ([v3 isEqualToString:@"SGMContactDetailUsedAppHome"])
  {
    int v4 = 8;
  }
  else if ([v3 isEqualToString:@"SGMContactDetailUsedAppMaps"])
  {
    int v4 = 9;
  }
  else if ([v3 isEqualToString:@"SGMContactDetailUsedAppFitness"])
  {
    int v4 = 10;
  }
  else if ([v3 isEqualToString:@"SGMContactDetailUsedAppReminders"])
  {
    int v4 = 11;
  }
  else
  {
    int v4 = 0;
  }

  return v4;
}

- (id)appAsString:(int)a3
{
  if (a3 >= 0xC)
  {
    objc_msgSend(NSString, "stringWithFormat:", @"(unknown: %i)", *(void *)&a3);
    id v3 = (__CFString *)objc_claimAutoreleasedReturnValue();
  }
  else
  {
    id v3 = off_1E65B5210[a3];
  }
  return v3;
}

- (BOOL)hasApp
{
  return *(unsigned char *)&self->_has & 1;
}

- (void)setHasApp:(BOOL)a3
{
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFE | a3;
}

- (void)setApp:(int)a3
{
  *(unsigned char *)&self->_has |= 1u;
  self->_uint64_t app = a3;
}

- (int)app
{
  if (*(unsigned char *)&self->_has) {
    return self->_app;
  }
  else {
    return 0;
  }
}

- (BOOL)hasWasKnownContact
{
  return (*(unsigned char *)&self->_has >> 1) & 1;
}

- (void)setHasWasKnownContact:(BOOL)a3
{
  if (a3) {
    char v3 = 2;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFD | v3;
}

- (void)setWasKnownContact:(BOOL)a3
{
  *(unsigned char *)&self->_has |= 2u;
  self->_wasKnownContact = a3;
}

- (BOOL)hasWasSuggestedContact
{
  return (*(unsigned char *)&self->_has >> 2) & 1;
}

- (void)setHasWasSuggestedContact:(BOOL)a3
{
  if (a3) {
    char v3 = 4;
  }
  else {
    char v3 = 0;
  }
  *(unsigned char *)&self->_char has = *(unsigned char *)&self->_has & 0xFB | v3;
}

- (void)setWasSuggestedContact:(BOOL)a3
{
  *(unsigned char *)&self->_has |= 4u;
  self->_wasSuggestedContact = a3;
}

- (BOOL)hasKey
{
  return self->_key != 0;
}

- (NSString)key
{
  if (self->_key) {
    return self->_key;
  }
  else {
    return (NSString *)@"AutocompleteUserSelectedContact";
  }
}

@end