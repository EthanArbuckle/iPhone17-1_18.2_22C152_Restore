...
- (unint64_t)hash
{
  ...
  if (*(unsigned char *)&self->_has) {
    uint64_t v4 = 2654435761 * self->_exitCode;
  }
  else {
    uint64_t v4 = 0;
  }
...
- (BOOL)isEqual:(id)a3
{
  ...
  if (*(unsigned char *)&self->_has)
  {
    if ((*((unsigned char *)v4 + 24) & 1) != 0 && self->_exitCode == *((_DWORD *)v4 + 2))
    {
      BOOL v6 = 1;
      goto LABEL_9;
    }
...
- (id)dictionaryRepresentation
{
  ...
  if (*(unsigned char *)&self->_has)
  {
    uint64_t v6 = [NSNumber numberWithUnsignedInt:self->_exitCode];
    [v4 setObject:v6 forKey:@"exitCode"];
  }
  return v4;
}
...
- (id)description
{
  ...
  id v4 = [(SGM2SuggestdExitReason *)&v8 description];
  id v5 = [(SGM2SuggestdExitReason *)self dictionaryRepresentation];
...
- (NSString)key
{
  if (self->_key) {
    ...
    return (NSString *)@"SuggestdExitReason";
  }
}
