...
- (void)addInvalidationBlock:(id)a3
{
  ...
  if (v4)
  {
    ...
    v9 = (void *)MEMORY[0x2611E12C0](v10);
...
- (NSString)description
{
  ...
  objc_super v3 = (void *)[objc_alloc(MEMORY[0x263F089D8]) initWithFormat:@"<%@: %p", objc_opt_class(), self];
...
- (void)invalidateWithReason:(id)a3
{
  ...
  if (![v5 length])
  {
    ...
    [v17 handleFailureInMethod:a2, self, @"SAAssertion.m", 69, @"Invalid parameter not satisfying: %@", @"[reason length] > 0" object file lineNumber description];
  }
  if ([(SAAssertion *)self isValid])
  {
    v6 = self;
    ...
    uint64_t v9 = SALogElement;
    if (os_log_type_enabled((os_log_t)SALogElement, OS_LOG_TYPE_DEFAULT))
    {
      *(_DWORD *)buf = 138543362;
      long long v24 = v6;
      _os_log_impl(&dword_25E6DD000, v9, OS_LOG_TYPE_DEFAULT, "Assertion did invalidate: %{public}@", buf, 0xCu);
    }
    uint64_t v10 = v6;
    objc_sync_enter(v10);
    long long v18 = 0u;
  ...
  else
  {
    ...
    if (os_log_type_enabled((os_log_t)SALogSystem, OS_LOG_TYPE_ERROR)) {
      -[SAAssertion invalidateWithReason:]((uint64_t)self, v16);
    }
  }
}
...
- (NSArray)_descriptionConstituents
{
  ...
  BOOL v4 = [(SAAssertion *)self isValid];
...
- (void)invalidateWithReason:(uint64_t)a1 .cold.1(uint64_t a1, NSObject *a2)
{
  ...
  int v2 = 138543362;
  uint64_t v3 = a1;
  _os_log_error_impl(&dword_25E6DD000, a2, OS_LOG_TYPE_ERROR, "Attempt to invalidate invalid assertion: %{public}@", (uint8_t *)&v2, 0xCu);
}
