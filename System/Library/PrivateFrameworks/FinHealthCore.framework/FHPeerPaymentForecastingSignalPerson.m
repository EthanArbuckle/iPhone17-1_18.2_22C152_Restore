@interface FHPeerPaymentForecastingSignalPerson
+ (BOOL)supportsSecureCoding;
- (BOOL)isEqual:(id)a3;
- (FHPeerPaymentForecastingSignalPerson)init;
- (FHPeerPaymentForecastingSignalPerson)initWithCoder:(id)a3;
- (NSString)personId;
- (id)description;
- (int64_t)direction;
- (unint64_t)hash;
- (void)encodeWithCoder:(id)a3;
- (void)setDirection:(int64_t)a3;
- (void)setPersonId:(id)a3;
@end

@implementation FHPeerPaymentForecastingSignalPerson

+ (BOOL)supportsSecureCoding
{
  return 1;
}

- (void)encodeWithCoder:(id)a3
{
  v5.receiver = self;
  v5.super_class = (Class)FHPeerPaymentForecastingSignalPerson;
  id v4 = a3;
  [(FHPeerPaymentForecastingSignal *)&v5 encodeWithCoder:v4];
  objc_msgSend(v4, "encodeObject:forKey:", self->_personId, @"personId", v5.receiver, v5.super_class);
  [v4 encodeInteger:self->_direction forKey:@"direction"];
}

- (FHPeerPaymentForecastingSignalPerson)initWithCoder:(id)a3
{
  id v4 = a3;
  v8.receiver = self;
  v8.super_class = (Class)FHPeerPaymentForecastingSignalPerson;
  objc_super v5 = [(FHPeerPaymentForecastingSignal *)&v8 initWithCoder:v4];
  if (v5)
  {
    v6 = [v4 decodeObjectOfClass:objc_opt_class() forKey:@"personId"];
    [(FHPeerPaymentForecastingSignalPerson *)v5 setPersonId:v6];

    -[FHPeerPaymentForecastingSignalPerson setDirection:](v5, "setDirection:", [v4 decodeIntegerForKey:@"direction"]);
  }

  return v5;
}

- (FHPeerPaymentForecastingSignalPerson)init
{
  v5.receiver = self;
  v5.super_class = (Class)FHPeerPaymentForecastingSignalPerson;
  v2 = [(FHPeerPaymentForecastingSignalPerson *)&v5 init];
  v3 = v2;
  if (v2) {
    [(FHPeerPaymentForecastingSignal *)v2 setForecastingType:2];
  }
  return v3;
}

- (id)description
{
  v3 = [MEMORY[0x263F089D8] stringWithFormat:@"<%@: %p; ", objc_opt_class(), self];
  v6.receiver = self;
  v6.super_class = (Class)FHPeerPaymentForecastingSignalPerson;
  id v4 = [(FHPeerPaymentForecastingSignal *)&v6 description];
  [v3 appendString:v4];

  [v3 appendFormat:@"%@: '%@'; ", @"personId", self->_personId];
  [v3 appendFormat:@"%@: '%lu'; ", @"direction", self->_direction];
  return v3;
}

- (unint64_t)hash
{
  v7.receiver = self;
  v7.super_class = (Class)FHPeerPaymentForecastingSignalPerson;
  unint64_t v3 = [(FHPeerPaymentForecastingSignal *)&v7 hash];
  id v4 = [(FHPeerPaymentForecastingSignalPerson *)self personId];
  uint64_t v5 = [v4 hash] + 43 * v3;

  return [(FHPeerPaymentForecastingSignalPerson *)self direction] + 43 * v5;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = (FHPeerPaymentForecastingSignalPerson *)a3;
  uint64_t v5 = v4;
  if (v4 == self)
  {
    BOOL v10 = 1;
  }
  else if (v4 {
         && (objc_opt_class(), (objc_opt_isKindOfClass() & 1) != 0)
  }
         && (unint64_t v6 = [(FHPeerPaymentForecastingSignal *)self forecastingType],
             v6 == [(FHPeerPaymentForecastingSignal *)v5 forecastingType]))
  {
    objc_super v7 = [(FHPeerPaymentForecastingSignalPerson *)self personId];
    objc_super v8 = [(FHPeerPaymentForecastingSignalPerson *)v5 personId];
    if ([v7 isEqual:v8])
    {
      int64_t v9 = [(FHPeerPaymentForecastingSignalPerson *)self direction];
      BOOL v10 = v9 == [(FHPeerPaymentForecastingSignalPerson *)v5 direction];
    }
    else
    {
      BOOL v10 = 0;
    }
  }
  else
  {
    BOOL v10 = 0;
  }

  return v10;
}

- (NSString)personId
{
  return self->_personId;
}

- (void)setPersonId:(id)a3
{
}

- (int64_t)direction
{
  return self->_direction;
}

- (void)setDirection:(int64_t)a3
{
  self->_direction = a3;
}

- (void).cxx_destruct
{
}

@end