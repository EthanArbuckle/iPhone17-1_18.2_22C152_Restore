...
- (void)setHasEUVolumeLimit:(BOOL)a3;
...
- (id)description
{
  char v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)NACEUVolumeLimitMessage;
...
- (id)dictionaryRepresentation
{
  ...
  {
    *(float *)&double v4 = self->_eUVolumeLimit;
...
- (BOOL)readFrom:(id)a3
{
  return NACEUVolumeLimitMessageReadFrom((uint64_t)self, (uint64_t)a3);
}
...
- (id)copyWithZone:(_NSZone *)a3
{
  ...
  if (*(unsigned char *)&self->_has)
  {
    *(float *)(v5 + 16) = self->_eUVolumeLimit;
  ...
  *(void *)(v6 + 8) = v7;
  ...
  if ((*(unsigned char *)&self->_has & 2) != 0)
  {
    *(_DWORD *)(v6 + 20) = self->_originIdentifier;
...
- (BOOL)isEqual:(id)a3
{
  ...
  if (has)
  {
    if ((*((unsigned char *)v4 + 24) & 1) == 0 || self->_eUVolumeLimit != *((float *)v4 + 4)) {
      goto LABEL_14;
    }
  }
...
LABEL_14:
  ...
  if ((has & 2) != 0)
  {
    if ((*((unsigned char *)v4 + 24) & 2) == 0 || self->_originIdentifier != *((_DWORD *)v4 + 5)) {
      goto LABEL_14;
    }
    BOOL v7 = 1;
  }
...
- (unint64_t)hash
{
  if (*(unsigned char *)&self->_has)
  {
    float eUVolumeLimit = self->_eUVolumeLimit;
    float v5 = -eUVolumeLimit;
    if (eUVolumeLimit >= 0.0) {
      float v5 = self->_eUVolumeLimit;
    }
...
- (void)mergeFrom:(id)a3
{
  ...
  {
    self->_float eUVolumeLimit = *((float *)v4 + 4);
    *(unsigned char *)&self->_has |= 1u;
  }
  if (*((void *)v4 + 1))
  {
    id v5 = v4;
    -[NACEUVolumeLimitMessage setCategory:](self, "setCategory:");
    id v4 = v5;
  }
  if ((*((unsigned char *)v4 + 24) & 2) != 0)
  {
    self->_originIdentifier = *((_DWORD *)v4 + 5);
    *(unsigned char *)&self->_has |= 2u;
  }
}
