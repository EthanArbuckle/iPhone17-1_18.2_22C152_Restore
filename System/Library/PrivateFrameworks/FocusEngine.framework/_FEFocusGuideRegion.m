...
- (id)_focusRegionWithAdjustedFrame:(CGRect)a3 coordinateSpace:(id)a4
{
  ...
  objc_super v5 = -[_FEFocusRegion _focusRegionWithAdjustedFrame:coordinateSpace:](&v9, sel__focusRegionWithAdjustedFrame_coordinateSpace_, a4, a3.origin.x, a3.origin.y, a3.size.width, a3.size.height);
  v6 = [(_FEFocusGuideRegion *)self delegate];
  [v5 setDelegate:v6];

  v7 = [(_FEFocusGuideRegion *)self owningEnvironment];
  [v5 setOwningEnvironment:v7];

  objc_msgSend(v5, "_setIsUnoccludable:", -[_FEFocusGuideRegion _isUnoccludable](self, "_isUnoccludable"));
  [(_FEFocusGuideRegion *)self _focusPriority];
...
- (BOOL)isEqual:(id)a3
{
  ...
  if ([(_FEFocusRegion *)&v12 isEqual:v4])
  {
    objc_super v5 = (id *)v4;
...
- (BOOL)_canBeOccludedByRegionsAbove
{
  return ![(_FEFocusGuideRegion *)self _isUnoccludable];
}
...
- (id)_nextFocusedItemForFocusMovementRequest:(id)a3 inMap:(id)a4 withSnapshot:(id)a5
{
  ...
  BOOL v13 = [(_FEFocusGuideRegion *)self _isEnabledForFocusedRegion:v12 inSnapshot:v10];

  if (v13)
  {
    CGFloat v14 = [(_FEFocusGuideRegion *)self _delegatePreferredFocusEnvironmentsForMovementRequest:v8];
    if (![v14 count])
    {
      double v15 = [(_FEFocusGuideRegion *)self owningEnvironment];
      uint64_t v16 = _FEFocusEnvironmentPreferredFocusEnvironments(v15);
    ...
    CGFloat v24 = [(_FEFocusGuideRegion *)self _fallbackFocusItemForMovementRequest:v8 inFocusMap:v9 withSnapshot:v10];
LABEL_15:
  }
  else
  {
...
- (id)_focusedItemForLinearSorting:(id)a3 inMap:(id)a4 withSnapshot:(id)a5
{
  ...
  id v9 = a4;
  id v10 = a5;
  double v11 = [(_FEFocusGuideRegion *)self delegate];
  ...
  else
  {
    ...
    uint64_t v12 = [(_FEFocusRegion *)&v15 _focusedItemForLinearSorting:v8 inMap:v9 withSnapshot:v10];
  }
  BOOL v13 = (void *)v12;
...
{
  id v6 = a3;
  id v4 = [(_FEFocusGuideRegion *)self delegate];
  if (objc_opt_respondsToSelector())
  {
...
{
  id v4 = a3;
  objc_super v5 = [(_FEFocusGuideRegion *)self delegate];
  if (objc_opt_respondsToSelector())
  {
...
- (id)_debugDrawingConfigurationWithDebugInfo:(id)a3
{
  ...
  LODWORD(v4) = [(_FEFocusGuideRegion *)self _isEnabledForFocusedRegion:v6 inSnapshot:v7];
  if (v4)
  {
    id v8 = +[_FEFocusRegionDebugDrawingConfiguration guideConfigurationForRegion:self];
  }
  else
  {
    id v8 = 0;
...
{
  id WeakRetained = objc_loadWeakRetained((id *)&self->_delegate);
  return (_FEFocusGuideRegionDelegate *)WeakRetained;
}
