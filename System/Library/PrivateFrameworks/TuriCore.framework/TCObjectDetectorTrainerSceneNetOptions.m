@interface TCObjectDetectorTrainerSceneNetOptions
- (Config)config;
- (TCObjectDetectorTrainerSceneNetOptions)init;
- (int64_t)batchSize;
- (int64_t)maxIterations;
- (int64_t)numClasses;
- (int64_t)randomSeed;
- (void)setBatchSize:(int64_t)a3;
- (void)setMaxIterations:(int64_t)a3;
- (void)setNumClasses:(int64_t)a3;
- (void)setRandomSeed:(int64_t)a3;
@end

@implementation TCObjectDetectorTrainerSceneNetOptions

- (TCObjectDetectorTrainerSceneNetOptions)init
{
  v3.receiver = self;
  v3.super_class = (Class)TCObjectDetectorTrainerSceneNetOptions;
  result = [(TCObjectDetectorTrainerSceneNetOptions *)&v3 init];
  if (result)
  {
    *(_OWORD *)&result->_maxIterations = xmmword_2246C6890;
    result->_numClasses = 0;
    result->_randomSeed = 0;
  }
  return result;
}

- (Config)config
{
  *(_OWORD *)&retstr->var0 = xmmword_2246C68C0;
  retstr->var4 = -1;
  retstr->var0 = [(TCObjectDetectorTrainerSceneNetOptions *)self maxIterations];
  retstr->var1 = [(TCObjectDetectorTrainerSceneNetOptions *)self batchSize];
  result = [(TCObjectDetectorTrainerSceneNetOptions *)self numClasses];
  retstr->var4 = (int)result;
  return result;
}

- (int64_t)maxIterations
{
  return self->_maxIterations;
}

- (void)setMaxIterations:(int64_t)a3
{
  self->_maxIterations = a3;
}

- (int64_t)batchSize
{
  return self->_batchSize;
}

- (void)setBatchSize:(int64_t)a3
{
  self->_batchSize = a3;
}

- (int64_t)numClasses
{
  return self->_numClasses;
}

- (void)setNumClasses:(int64_t)a3
{
  self->_numClasses = a3;
}

- (int64_t)randomSeed
{
  return self->_randomSeed;
}

- (void)setRandomSeed:(int64_t)a3
{
  self->_randomSeed = a3;
}

@end