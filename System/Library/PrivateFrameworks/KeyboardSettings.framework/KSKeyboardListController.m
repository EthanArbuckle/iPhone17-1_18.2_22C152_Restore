...
- (void)viewDidLoad
{
  objc_msgSend((id)objc_msgSend(MEMORY[0x263F1C748], "sharedInputModeController"), "setDisableFloatingKeyboardFilter:", 1);
  v4.receiver = self;
...
- (KSKeyboardListController)init
{
  ...
  {
    uint64_t v3 = (PSSpecifier *)(id)objc_msgSend(MEMORY[0x263F5FC40], "preferenceSpecifierNamed:target:set:get:detail:cell:edit:", objc_msgSend((id)objc_msgSend(MEMORY[0x263F086E0], "bundleForClass:", objc_opt_class()), "localizedStringForKey:value:table:", @"ADD_NEW_KEYBOARD", &stru_2703C6C20, @"Keyboard"), v2, 0, 0, 0, 13, 0);
...
- (void)updateEditButtonState
{
  v2 = (void *)[(KSKeyboardListController *)self parentController];
  if (objc_opt_respondsToSelector())
  {
...
- (void)setEditing:(BOOL)a3 animated:(BOOL)a4
{
  ...
  [(KSKeyboardListController *)&v15 setEditing:a3 animated:a4];
  if (!self->deletingRow) {
...
- (void)tableView:(id)a3 willBeginEditingRowAtIndexPath:(id)a4
{
  ...
  [(KSKeyboardListController *)self setEditing:1 animated:1];
}

- (void)tableView:(id)a3 didEndEditingRowAtIndexPath:(id)a4
{
  if ([(KSKeyboardListController *)self isEditing] && self->deletingRow) {
...
- (void)tableView:(id)a3 moveRowAtIndexPath:(id)a4 toIndexPath:(id)a5
{
  uint64_t v7 = objc_msgSend(a4, "row", a3) + 1;
  uint64_t v8 = [a5 row] + 1;
  id v9 = objc_alloc_init(MEMORY[0x263EFF980]);
  uint64_t v10 = (int)*MEMORY[0x263F5FDB8];
  [v9 addObjectsFromArray:*(Class *)((char *)&self->super.super.super.super.super.isa + v10)];
  id v11 = (void *)[v9 objectAtIndex:v7];
  id v12 = v11;
  [v9 removeObject:v11];
  [v9 insertObject:v11 atIndex:v8];
...
- (void)reloadSpecifiers
{
  ...
  BOOL v3 = ([(KSKeyboardListController *)self isEditing] & 1) != 0 || self->_numberOfEnabledKeyboards > 1;
...
- (id)specifiers
{
  ...
  -[KSKeyboardListController setTitle:](self, "setTitle:", objc_msgSend((id)objc_msgSend(MEMORY[0x263F086E0], "bundleForClass:", objc_opt_class()), "localizedStringForKey:value:table:", @"KEYBOARDS_SHORT", &stru_2703C6C20, @"Keyboard"));
  long long v21 = v3;
  ...
  do
  {
    for (uint64_t i = 0; i != v7; ++i)
    {
      ...
      if (objc_msgSend((id)objc_msgSend(MEMORY[0x263F1C748], "sharedInputModeController"), "identifierIsValidSystemInputMode:", NormalizedIdentifier))
      {
        ...
        {
          id v14 = [(KSKeyboardListController *)self specifierForMultilingualInputModes:MultilingualSetFromInputModes];
        }
        else
        {
          id v14 = [(KSKeyboardListController *)self specifierForInputMode:v10];
        }
        [v21 addObject:v14];
  ...
  while (v7);
LABEL_30:
  if ((self->deletingRow || ([(KSKeyboardListController *)self isEditing] & 1) == 0)
    && ![(KSKeyboardListController *)self allKeyboardsEnabled]
  ...
  if (v18)
  {
    if ([(KSKeyboardListController *)self aboutPrivacyController]) {
      [(KSAboutKeyboardPrivacyController *)[(KSKeyboardListController *)self aboutPrivacyController] setSpecifier:v16];
    }
    else {
      [(KSKeyboardListController *)self setAboutPrivacyController:[[KSAboutKeyboardPrivacyController alloc] initWithGroupSpecifier:v16 asHeader:0 inListController:self]];
    }
  }
...
- (void)tableView:(id)a3 willDisplayFooterView:(id)a4 forSection:(int64_t)a5
{
  uint64_t v7 = [(KSKeyboardListController *)self aboutPrivacyController];
...
- (id)tableView:(id)a3 cellForRowAtIndexPath:(id)a4
{
  ...
  id v6 = -[KSKeyboardListController tableView:cellForRowAtIndexPath:](&v9, sel_tableView_cellForRowAtIndexPath_, a3);
  uint64_t v7 = (void *)[(KSKeyboardListController *)self specifierAtIndex:[(KSKeyboardListController *)self indexForIndexPath:a4]];
...
- (void)addNewKeyboards:(id)a3
{
  uint64_t v5 = objc_alloc_init(KSAddKeyboardLanguageListSetupController);
  ...
  if (v4)
  {
    ...
    do
    {
      ...
      do
      {
        ...
        if ([*(id *)((char *)&self->super.super.super.super.super.isa + *(int *)(v8 + 2604)) containsObject:v12])
        {
          if (_os_feature_enabled_impl())
          {
            uint64_t v13 = v7;
...
- (void)tableView:(id)a3 commitEditingStyle:(int64_t)a4 forRowAtIndexPath:(id)a5
{
  if (a4 == 1)
  {
    ...
    -[KSKeyboardListController removeInputModeWithIdentifier:](self, "removeInputModeWithIdentifier:", [v6 propertyForKey:*MEMORY[0x263F60138]]);
...
- (void)viewWillAppear:(BOOL)a3
{
  v5.receiver = self;
  v5.super_class = (Class)KSKeyboardListController;
  [(KSKeyboardListController *)&v5 viewWillAppear:a3];
  [(KSKeyboardListController *)self reloadSpecifiers];
...
- (void)viewDidAppear:(BOOL)a3
{
  v4.receiver = self;
  v4.super_class = (Class)KSKeyboardListController;
  [(KSKeyboardListController *)&v4 viewDidAppear:a3];
...
- (void)viewWillDisappear:(BOOL)a3
{
  ...
  v5.receiver = self;
  v5.super_class = (Class)KSKeyboardListController;
  [(KSKeyboardListController *)&v5 viewWillDisappear:v3];
}
...
+ (id)availableSoftwareLayoutsForBaseInputMode:(id)a3
{
  ...
  if (KSInputModeIsChineseShuangpin()) {
    id v4 = (id)[KSFilteredLayoutsByCurrentShuangpinType(v4) mutableCopy];
  }
  long long v14 = 0u;
  long long v15 = 0u;
  long long v13 = 0u;
...
+ (id)displayNameForHardwareLayout:(id)a3 inputMode:(id)a4
{
  ...
  {
    uint64_t v6 = objc_msgSend((id)objc_msgSend(MEMORY[0x263F086E0], "bundleForClass:", objc_opt_class()), "localizedStringForKey:value:table:", @"Automatic", &stru_2703C6C20, @"KeyboardLayouts");
  ...
  if (displayNameForHardwareLayout_inputMode____keyboardLayoutsBundle
    || (uint64_t v8 = (void *)[(id)MEMORY[0x25A278370]() stringByAppendingPathComponent:@"System/Library/KeyboardLayouts/USBKeyboardLayouts.bundle"], v7 = (id)objc_msgSend(MEMORY[0x263F086E0], "bundleWithPath:", objc_msgSend(v8, "stringByResolvingSymlinksInPath")), (displayNameForHardwareLayout_inputMode____keyboardLayoutsBundle = (uint64_t)v7) != 0)|| (NSLog(&cfstr_SErrorCouldNot.isa, "+[KSKeyboardListController displayNameForHardwareLayout:inputMode:]", v8), (v7 = (id)displayNameForHardwareLayout_inputMode____keyboardLayoutsBundle) != 0))
  {
    id v5 = (id)[v7 localizedStringForKey:v5 value:0 table:@"InfoPlist"];
    if (v6) {
      return (id)[NSString localizedStringWithFormat:objc_msgSend((id)objc_msgSend(MEMORY[0x263F086E0], "bundleForClass:", objc_opt_class()), "localizedStringForKey:value:table:", @"%@ — %@", &stru_2703C6C20, @"Automatic - <Resolved Keyboard Layout Name>", v6, v5];
    }
  }
  else if (v6)
  {
...
+ (id)attributedTitleForSymbolName:(id)a3 cellTitle:(id)a4
{
  ...
  id v9 = (id)objc_msgSend((id)objc_msgSend(MEMORY[0x263F81658], "defaultParagraphStyle"), "mutableCopy");
  id v10 = objc_alloc(MEMORY[0x263F816F0]);
  uint64_t v11 = MEMORY[0x263EFFA78];
  v15[0] = (id)[v10 initWithTextAlignment:4 location:MEMORY[0x263EFFA78] options:0.0];
  v15[1] = (id)[objc_alloc(MEMORY[0x263F816F0]) initWithTextAlignment:1 location:v11 options:10.0];
  v15[2] = (id)[objc_alloc(MEMORY[0x263F816F0]) initWithTextAlignment:4 location:v11 options:28.0];
...
+ (id)attributedStringForSymbolName:(id)a3
{
  ...
  uint64_t v6 = v5;
  id v7 = (id)[objc_alloc(MEMORY[0x263F81678]) initWithData:0 ofType:0];
  [v7 setImage:v6];
  if (![v7 image]) {
    return v4;
  }
...
{
  __assert_rtn("+[KSKeyboardListController supportedBaseInputModesForLanguage:]", "KSKeyboardListController.m", 606, "languageIdentifier != nil");
}

+ (void)softwareLayoutsForBaseInputMode:.cold.1()
{
  __assert_rtn("+[KSKeyboardListController softwareLayoutsForBaseInputMode:]", "KSKeyboardListController.m", 629, "keyboardLayout != nil");
}
