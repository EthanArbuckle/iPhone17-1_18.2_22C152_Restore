...
- (AUHelperServiceProtocol)remoteObject
{
  ...
  if (xpcConnection)
  {
    xpcConnection = [xpcConnection synchronousRemoteObjectProxyWithErrorHandler:&__block_literal_global_2];
  }
...
+ (id)xpcConnectionToHelper
{
  ...
  if (v2)
  {
    uint64_t v3 = [MEMORY[0x263F08D80] interfaceWithProtocol:&unk_26FB6D028];
...
void __32__AUHelperInstance_remoteObject__block_invoke_cold_1(uint64_t a1)
{
  ...
  __int16 v3 = 2112;
  uint64_t v4 = a1;
  _os_log_error_impl(&dword_247215000, &_os_log_internal, OS_LOG_TYPE_ERROR, "%s: Failed to get remote object: %@", (uint8_t *)&v1, 0x16u);
}
...
+ (void)xpcConnectionToHelper
{
  ...
  _os_log_error_impl(&dword_247215000, &_os_log_internal, OS_LOG_TYPE_ERROR, "%s: Failed to get xpc connection", (uint8_t *)&v0, 0xCu);
}
