...
+ (uint64_t)mf_UUID
{
  v0 = (void *)[MEMORY[0x1E4F29128] UUID];
...
- (uint64_t)mf_stringByLocalizingReOrFwdPrefix
{
  int v2 = mf_stringByLocalizingReOrFwdPrefix_localizedRePrefix;
  if (mf_stringByLocalizingReOrFwdPrefix_localizedRePrefix == -1)
  {
    mf_stringByLocalizingReOrFwdPrefix_localizedRePrefix = objc_msgSend((id)MFLookupLocalizedString(@"REPLY_SUBJECT_PREFIX", @"Re:", 0), "isEqualToString:", @"Re:") ^ 1;
    mf_stringByLocalizingReOrFwdPrefix_localizedFwdPrefix = objc_msgSend((id)MFLookupLocalizedString(@"FORWARD_SUBJECT_PREFIX", @"Fwd:", 0), "isEqualToString:", @"Fwd:") ^ 1;
  ...
  if (v2 == 1
    && !objc_msgSend(a1, "compare:options:range:", @"Re:", 1, 0, mf_stringByLocalizingReOrFwdPrefix_nonLocalizedReLength))
  {
    a1 = (void *)[NSString stringWithFormat:@"%@⁨%@⁩", MFLookupLocalizedString(@"REPLY_SUBJECT_PREFIX", @"Re:", 0), objc_msgSend(a1, "substringFromIndex:", mf_stringByLocalizingReOrFwdPrefix_nonLocalizedReLength)];
  }
  if (mf_stringByLocalizingReOrFwdPrefix_localizedFwdPrefix == 1
    && !objc_msgSend(a1, "compare:options:range:", @"Fwd:", 1, 0, mf_stringByLocalizingReOrFwdPrefix_nonLocalizedFwdLength))
  {
    return [NSString stringWithFormat:@"%@⁨%@⁩", MFLookupLocalizedString(@"FORWARD_SUBJECT_PREFIX", @"Fwd:", 0), objc_msgSend(a1, "substringFromIndex:", mf_stringByLocalizingReOrFwdPrefix_nonLocalizedFwdLength)];
  }
  else
  {
    return (uint64_t)a1;
...
- (id)mf_fileSystemString
{
  ...
  if (v2)
  {
    int v4 = [a1 getFileSystemRepresentation:v5 maxLength:1023];
    id result = a1;
    if (v4)
    {
      v5[1023] = 0;
      id result = (id)[objc_allocWithZone(NSString) initWithUTF8String:v5];
    }
  }
  if (!result) {
    return a1;
...
- (id)mf_stringByEscapingForXML
{
  __int16 v1 = (void *)[a1 mutableCopy];
