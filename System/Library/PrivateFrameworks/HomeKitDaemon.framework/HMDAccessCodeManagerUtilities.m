...
+ (BOOL)anyModificationFailed:(id)a3
{
  return objc_msgSend(a3, "na_any:", &__block_literal_global_278);
}
  ...
  if (v8)
  {
    if (v12)
    {
      ...
      *(_DWORD *)buf = 138543618;
      ...
      __int16 v23 = 2112;
      v24 = v14;
      _os_log_impl(&dword_22F52A000, v11, OS_LOG_TYPE_INFO, "%{public}@User: %@ is an admin, so we can safely send all the fetch responses.", buf, 0x16u);
    }
    id v15 = v6;
  }
  else
  {
    if (v12)
    {
      ...
      *(_DWORD *)buf = 138543618;
      ...
      __int16 v23 = 2112;
      v24 = v17;
      _os_log_impl(&dword_22F52A000, v11, OS_LOG_TYPE_INFO, "%{public}@User: %@ is not an admin, so redact all access codes except the user's own.", buf, 0x16u);
    }
...
id __64__HMDAccessCodeManagerUtilities_filteredFetchResponses_forUser___block_invoke(uint64_t a1, void *a2)
{
  ...
  id v6 = objc_alloc(MEMORY[0x263F0DF18]);
...
+ (id)removedAccessoryAccessCodesFromModificationResponses:(id)a3
{
  return (id)objc_msgSend(a3, "na_map:", &__block_literal_global_272);
}
