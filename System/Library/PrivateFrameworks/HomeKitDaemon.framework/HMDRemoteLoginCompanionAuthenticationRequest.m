@interface HMDRemoteLoginCompanionAuthenticationRequest
+ (BOOL)supportsSecureCoding;
+ (NSString)messageName;
+ (id)objWithDict:(id)a3;
+ (id)objWithMessage:(id)a3;
- (ACAccount)account;
- (AKDevice)companionDevice;
- (HMDRemoteLoginCompanionAuthenticationRequest)initWithCoder:(id)a3;
- (NSDictionary)messagePayload;
- (NSString)description;
- (NSString)messageName;
- (void)encodeWithCoder:(id)a3;
- (void)setAccount:(id)a3;
- (void)setCompanionDevice:(id)a3;
@end

@implementation HMDRemoteLoginCompanionAuthenticationRequest

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_companionDevice, 0);
  objc_storeStrong((id *)&self->_account, 0);
}

- (void)setCompanionDevice:(id)a3
{
}

- (AKDevice)companionDevice
{
  return self->_companionDevice;
}

- (void)setAccount:(id)a3
{
}

- (ACAccount)account
{
  return self->_account;
}

- (HMDRemoteLoginCompanionAuthenticationRequest)initWithCoder:(id)a3
{
  id v4 = a3;
  v11.receiver = self;
  v11.super_class = (Class)HMDRemoteLoginCompanionAuthenticationRequest;
  v5 = [(HMDRemoteLoginAuthenticationRequest *)&v11 initWithCoder:v4];
  if (v5)
  {
    uint64_t v6 = [v4 decodeObjectOfClass:objc_opt_class() forKey:@"account"];
    account = v5->_account;
    v5->_account = (ACAccount *)v6;

    uint64_t v8 = [v4 decodeObjectOfClass:objc_opt_class() forKey:@"companionDevice"];
    companionDevice = v5->_companionDevice;
    v5->_companionDevice = (AKDevice *)v8;
  }
  return v5;
}

- (void)encodeWithCoder:(id)a3
{
  v9.receiver = self;
  v9.super_class = (Class)HMDRemoteLoginCompanionAuthenticationRequest;
  id v4 = a3;
  [(HMDRemoteLoginAuthenticationRequest *)&v9 encodeWithCoder:v4];
  v5 = objc_msgSend(MEMORY[0x263F290F0], "sharedInstance", v9.receiver, v9.super_class);
  uint64_t v6 = [(HMDRemoteLoginCompanionAuthenticationRequest *)self account];
  v7 = [v5 transportableAuthKitAccount:v6];

  [v4 encodeObject:v7 forKey:@"account"];
  uint64_t v8 = [(HMDRemoteLoginCompanionAuthenticationRequest *)self companionDevice];
  [v4 encodeObject:v8 forKey:@"companionDevice"];
}

- (NSString)description
{
  v3 = NSString;
  v9.receiver = self;
  v9.super_class = (Class)HMDRemoteLoginCompanionAuthenticationRequest;
  id v4 = [(HMDRemoteLoginAuthenticationRequest *)&v9 description];
  v5 = [(HMDRemoteLoginCompanionAuthenticationRequest *)self account];
  uint64_t v6 = [(HMDRemoteLoginCompanionAuthenticationRequest *)self companionDevice];
  v7 = [v3 stringWithFormat:@"HMDRemoteLoginCompanionAuthenticationRequest: %@, Account: %@, Companion-Device: %@", v4, v5, v6];

  return (NSString *)v7;
}

- (NSDictionary)messagePayload
{
  v8[1] = *MEMORY[0x263EF8340];
  v3 = encodeRootObject();
  id v4 = [(HMDRemoteLoginCompanionAuthenticationRequest *)self messageName];
  v7 = v4;
  v8[0] = v3;
  v5 = [NSDictionary dictionaryWithObjects:v8 forKeys:&v7 count:1];

  return (NSDictionary *)v5;
}

- (NSString)messageName
{
  return (NSString *)@"kHMDRemoteLoginCompanionAuthenticationRequest";
}

+ (NSString)messageName
{
  return (NSString *)@"kHMDRemoteLoginCompanionAuthenticationRequest";
}

+ (BOOL)supportsSecureCoding
{
  return 1;
}

+ (id)objWithMessage:(id)a3
{
  uint64_t v19 = *MEMORY[0x263EF8340];
  id v4 = a3;
  v5 = [v4 messagePayload];
  uint64_t v6 = objc_msgSend(v5, "hmf_dataForKey:", @"kHMDRemoteLoginCompanionAuthenticationRequest");

  id v14 = 0;
  v7 = [MEMORY[0x263F08928] unarchivedObjectOfClass:objc_opt_class() fromData:v6 error:&v14];
  id v8 = v14;
  if (!v7)
  {
    objc_super v9 = (void *)MEMORY[0x230FBD990]();
    id v10 = a1;
    objc_super v11 = HMFGetOSLogHandle();
    if (os_log_type_enabled(v11, OS_LOG_TYPE_ERROR))
    {
      v12 = HMFGetLogIdentifier();
      *(_DWORD *)buf = 138543618;
      v16 = v12;
      __int16 v17 = 2112;
      id v18 = v8;
      _os_log_impl(&dword_22F52A000, v11, OS_LOG_TYPE_ERROR, "%{public}@Failed to unarchive object with message from archive data: %@", buf, 0x16u);
    }
  }

  return v7;
}

+ (id)objWithDict:(id)a3
{
  uint64_t v18 = *MEMORY[0x263EF8340];
  id v4 = a3;
  v5 = objc_msgSend(v4, "hmf_dataForKey:", @"kHMDRemoteLoginCompanionAuthenticationRequest");
  id v13 = 0;
  uint64_t v6 = [MEMORY[0x263F08928] unarchivedObjectOfClass:objc_opt_class() fromData:v5 error:&v13];
  id v7 = v13;
  if (!v6)
  {
    id v8 = (void *)MEMORY[0x230FBD990]();
    id v9 = a1;
    id v10 = HMFGetOSLogHandle();
    if (os_log_type_enabled(v10, OS_LOG_TYPE_ERROR))
    {
      objc_super v11 = HMFGetLogIdentifier();
      *(_DWORD *)buf = 138543618;
      v15 = v11;
      __int16 v16 = 2112;
      id v17 = v7;
      _os_log_impl(&dword_22F52A000, v10, OS_LOG_TYPE_ERROR, "%{public}@Failed to unarchive object with dictionary from archive data: %@", buf, 0x16u);
    }
  }

  return v6;
}

@end