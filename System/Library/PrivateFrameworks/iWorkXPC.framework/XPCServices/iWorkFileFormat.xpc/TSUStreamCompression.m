...
- (void)dealloc
{
  ...
  v3.receiver = self;
  v3.super_class = (Class)TSUStreamCompression;
  [(TSUStreamCompression *)&v3 dealloc];
}

- (BOOL)processBytes:(char *)a3 size:(unint64_t)a4
{
  return [(TSUStreamCompression *)self processBytes:a3 size:a4 flags:0];
}

- (BOOL)didFinishProcessing
{
  return [(TSUStreamCompression *)self processBytes:&unk_1001EB9A4 size:0 flags:1];
}
...
- (BOOL)processBytes:(char *)a3 size:(unint64_t)a4 flags:(int)a5
{
  ...
  while (1)
  {
    ...
    if (!p_stream->dst_ptr || dst_ptr == (uint8_t *)&unk_1001EB9A4) {
      goto LABEL_46;
    }
    dispatch_data_t v17 = dispatch_data_create(dst_ptr - 1024, 0x400uLL, 0, _dispatch_data_destructor_free);
    self->_stream.dst_ptr = (char *)&unk_1001EB9A4;
    self->_stream.unint64_t dst_size = 0;
    id handler = self->_handler;
    if (handler
      && ((*((uint64_t (**)(id, TSUStreamCompression *, void, dispatch_data_t, void))handler + 2))(handler, self, 0, v17, 0) & 1) == 0)
    {
      self->_int status = -1;
    ...
    {
LABEL_46:
      self->_stream.dst_ptr = (char *)malloc_type_malloc(0x400uLL, 0x39E6C050uLL);
...
LABEL_10:
    ...
    if (v11 == -1)
    {
      v20 = (void (**)(id, TSUStreamCompression *, uint64_t, void, void *))self->_handler;
      if (!v20) {
        goto LABEL_42;
      }
    ...
    else
    {
      ...
      if (!p_stream->dst_ptr || v12 == (uint8_t *)&unk_1001EB9A4)
      {
      ...
      v24 = (uint64_t (**)(id, TSUStreamCompression *, uint64_t, void *, void))self->_handler;
...
LABEL_42:
  BOOL result = 0;
LABEL_43:
  self->_stream.src_ptr = (char *)&unk_1001EB9A4;
