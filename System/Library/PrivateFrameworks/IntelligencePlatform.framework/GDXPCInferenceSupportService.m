...
- (id)assetEntryResultDataForAssetId:(id)a3 inDomainId:(id)a4 remoteOptionsData:(id)a5 error:(id *)a6
{
  ...
  {
    *(_WORD *)buf = 0;
    _os_log_impl(&dword_1B2996000, v13, OS_LOG_TYPE_DEFAULT, "GDXPCInferenceSupportService: assetEntryDataForAssetId called.", buf, 2u);
  }
...
- (void)locked_establishConnection
{
  if (!self->_connection)
  {
    ...
    {
      *(_WORD *)buf = 0;
      _os_log_impl(&dword_1B2996000, v3, OS_LOG_TYPE_DEFAULT, "GDXPCInferenceSupportService: establishing connection.", buf, 2u);
    }

    id v4 = objc_alloc(MEMORY[0x1E4F29268]);
    uint64_t v7 = (NSXPCConnection *)objc_msgSend_initWithMachServiceName_options_(v4, v5, @"com.apple.intelligenceplatform.InferenceSupport", 0, v6);
    connection = self->_connection;
    self->_connection = v7;

    objc_msgSend_setRemoteObjectInterface_(self->_connection, v9, (uint64_t)self->_serverInterface, v10, v11);
    objc_msgSend_setInterruptionHandler_(self->_connection, v12, (uint64_t)&unk_1F0AE1EF0, v13, v14);
    *(void *)buf = 0;
...
- (void)dealloc
{
  ...
  v6.receiver = self;
  v6.super_class = (Class)GDXPCInferenceSupportService;
  [(GDXPCInferenceSupportService *)&v6 dealloc];
}

- (GDXPCInferenceSupportService)init
{
  v9.receiver = self;
  v9.super_class = (Class)GDXPCInferenceSupportService;
  v5 = [(GDXPCInferenceSupportService *)&v9 init];
  if (v5)
  {
    uint64_t v6 = objc_msgSend_interfaceWithProtocol_(MEMORY[0x1E4F29280], v2, (uint64_t)&unk_1F0B27038, v3, v4);
