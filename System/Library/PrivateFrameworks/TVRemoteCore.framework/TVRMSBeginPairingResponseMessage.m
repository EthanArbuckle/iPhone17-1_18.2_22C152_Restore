@interface TVRMSBeginPairingResponseMessage
...
@implementation TVRMSBeginPairingResponseMessage
...
- (id)description
{
  char v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)TVRMSBeginPairingResponseMessage;
  v4 = [(TVRMSBeginPairingResponseMessage *)&v8 description];
  v5 = [(TVRMSBeginPairingResponseMessage *)self dictionaryRepresentation];
...
- (BOOL)readFrom:(id)a3
{
  return TVRMSBeginPairingResponseMessageReadFrom((uint64_t)self, (uint64_t)a3);
}
...
LABEL_11:
  ...
  {
    if ((*((unsigned char *)v4 + 16) & 1) == 0 || self->_responseCode != *((_DWORD *)v4 + 2)) {
      goto LABEL_11;
    }
    BOOL v5 = 1;
  }
...
- (void)mergeFrom:(id)a3
{
  ...
  if (v5)
  {
    self->_responseCode = *((_DWORD *)v4 + 2);
    *(unsigned char *)&self->_has |= 1u;
  }
}
