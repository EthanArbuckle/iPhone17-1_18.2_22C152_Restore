...
- (TVRMSPairingSession)init
{
  ...
  v2 = [(TVRMSPairingSession *)&v11 init];
  if (v2)
  {
...
- (void)dealloc
{
  ...
  [(TVRMSPairingSession *)&v5 dealloc];
}
...
- (void)beginPairing
{
  ...
  id v9 = [(TVRMSPairingSession *)self _generatePublicKey];
  objc_super v5 = -[TVRMSPairingSession _expectedPasscodeHashForPasscode:publicKey:](self, "_expectedPasscodeHashForPasscode:publicKey:", self->_passcode);
  uint64_t v6 = (TVRMSPairingServer *)objc_opt_new();
  pairingServer = self->_pairingServer;
  self->_pairingServer = v6;

  [(TVRMSPairingServer *)self->_pairingServer setDelegate:self];
  if ([(TVRMSPairingServer *)self->_pairingServer startServerWithExpectedPasscodeHash:v5 advertisedDeviceName:self->_advertisedDeviceName advertisedDeviceModel:self->_advertisedDeviceModel])
  {
    [(TVRMSPairingSession *)self _startBonjourAdvertisingWithPublicKey:v9 httpServerPort:[(TVRMSPairingServer *)self->_pairingServer port]];
  }
  else
  {
...
- (void)endPairing
{
  [(TVRMSPairingServer *)self->_pairingServer stopServer];
...
- (void)netServiceDidPublish:(id)a3
{
  ...
  {
    *(_WORD *)uint64_t v8 = 0;
    _os_log_impl(&dword_2266D3000, v4, OS_LOG_TYPE_DEFAULT, "Pairing bonjour service published", v8, 2u);
  }
...
- (void)netServiceDidStop:(id)a3
{
  ...
  if (os_log_type_enabled(v3, OS_LOG_TYPE_DEFAULT))
  {
    *(_WORD *)objc_super v5 = 0;
    _os_log_impl(&dword_2266D3000, v3, OS_LOG_TYPE_DEFAULT, "Pairing bonjour service stopped", v5, 2u);
  }
...
- (void)pairingServer:(id)a3 didPairWithService:(id)a4 pairingGUID:(id)a5
{
  ...
  if (os_log_type_enabled(v9, OS_LOG_TYPE_DEFAULT))
  {
    ...
    __int16 v13 = 2112;
    id v14 = v8;
    _os_log_impl(&dword_2266D3000, v9, OS_LOG_TYPE_DEFAULT, "Pairing server successfully paired, serviceName=[%@] pairingGUID=[%@]", (uint8_t *)&v11, 0x16u);
  }

  [(TVRMSPairingSession *)self endPairing];
  id WeakRetained = objc_loadWeakRetained((id *)&self->_delegate);
...
- (id)_expectedPasscodeHashForPasscode:(id)a3 publicKey:(id)a4
{
  ...
  id v9 = [(TVRMSPairingSession *)self passcode];
  unsigned __int16 v10 = (const void *)[v9 cStringUsingEncoding:10];
  int v11 = [(TVRMSPairingSession *)self passcode];
