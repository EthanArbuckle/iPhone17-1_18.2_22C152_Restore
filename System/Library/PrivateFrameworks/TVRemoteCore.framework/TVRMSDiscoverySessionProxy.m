@interface TVRMSDiscoverySessionProxy
...
- (TVRMSDiscoverySessionProxy)init
{
  ...
  v2 = [(TVRMSSessionProxy *)&v7 initWithTimeout:*(double *)&TVRMSDiscoverySessionTimeout];
  if (v2)
  {
    uint64_t v3 = +[TVRMSIDSClient sharedClient];
    ...
    [(TVRMSDiscoverySessionProxy *)v2 _updateNetworkAvailability];
  }
  return v2;
}
...
- (void)beginDiscovery
{
  if (!self->_discovering)
  {
    ...
    v6[4] = self;
    [(TVRMSIDSClient *)idsClient beginDiscoverySessionWithDiscoveryTypes:discoveryTypes_low pairedNetworkNames:pairedNetworkNames completionHandler:v6];
    objc_destroyWeak(&v7);
...
void __44__TVRMSDiscoverySessionProxy_beginDiscovery__block_invoke(uint64_t a1, int a2, uint64_t a3)
{
  ...
  if (WeakRetained)
  {
    if (a2)
    {
      ...
      {
        *(_WORD *)buf = 0;
        _os_log_impl(&dword_2266D3000, v9, OS_LOG_TYPE_DEFAULT, "Begin discovery success, session established", buf, 2u);
      }
...
- (void)setPairedNetworkNames:(id)a3
{
  ...
  if ([(TVRMSSessionProxy *)self sessionIdentifier]) {
    [(TVRMSIDSClient *)self->_idsClient updatePairedNetworkNames:self->_pairedNetworkNames sessionIdentifier:[(TVRMSSessionProxy *)self sessionIdentifier]];
  }
}
...
{
  self->_discovering = 0;
  [(TVRMSDiscoverySessionProxy *)self beginDiscovery];
}
...
- (void)endDiscovery
{
  ...
  {
    *(_WORD *)id v5 = 0;
    _os_log_impl(&dword_2266D3000, v3, OS_LOG_TYPE_DEFAULT, "end discovery invoked", v5, 2u);
  }

  if ([(TVRMSSessionProxy *)self sessionIdentifier])
  {
    [(TVRMSIDSClient *)self->_idsClient endDiscoverySession:[(TVRMSSessionProxy *)self sessionIdentifier]];
...
- (void)_availableServicesDidUpdateNotification:(id)a3
{
  ...
  if ([(TVRMSSessionProxy *)self sessionMatchesNotification:v4])
  {
    ...
    if (os_log_type_enabled(v8, OS_LOG_TYPE_DEFAULT))
    {
      ...
      int v11 = 138412290;
      id v12 = v9;
      _os_log_impl(&dword_2266D3000, v8, OS_LOG_TYPE_DEFAULT, "Discovery proxy handling available services update: %@", (uint8_t *)&v11, 0xCu);
    }
...
- (void)_wifiAvailabilityDidChange:(id)a3
{
  ...
  if (-[TVRMSSessionProxy sessionMatchesNotification:](self, "sessionMatchesNotification:"))
  {
    ...
    [(TVRMSDiscoverySessionProxy *)self _updateNetworkAvailability];
  }
}

- (void)_updateNetworkAvailability
{
  if ([(TVRMSIDSClient *)self->_idsClient isCompanionAvailable]) {
