@interface TVRMSWifiAvailabilityDidChangeMessage
...
@implementation TVRMSWifiAvailabilityDidChangeMessage
...
- (id)description
{
  char v3 = NSString;
  v8.receiver = self;
  v8.super_class = (Class)TVRMSWifiAvailabilityDidChangeMessage;
  v4 = [(TVRMSWifiAvailabilityDidChangeMessage *)&v8 description];
  v5 = [(TVRMSWifiAvailabilityDidChangeMessage *)self dictionaryRepresentation];
...
- (BOOL)readFrom:(id)a3
{
  return TVRMSWifiAvailabilityDidChangeMessageReadFrom((uint64_t)self, (uint64_t)a3);
}
...
- (BOOL)isEqual:(id)a3
{
  ...
  {
    if ((v4[16] & 1) == 0 || self->_sessionIdentifier != *((_DWORD *)v4 + 2)) {
      goto LABEL_11;
    }
  }
  else if (v4[16])
...
LABEL_11:
  ...
  {
    if ((v4[16] & 2) == 0 || self->_wifiAvailable != v4[12]) {
      goto LABEL_11;
    }
    BOOL v5 = 1;
  }
