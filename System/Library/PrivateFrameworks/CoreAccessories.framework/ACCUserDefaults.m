...
{
  sharedDefaults_sharedInstance = [objc_alloc(MEMORY[0x1E4F1CB18]) initWithSuiteName:@"com.apple.CoreAccessories"];
...
+ (id)sharedDefaultsIapd
{
  if (sharedDefaultsIapd_once != -1) {
    dispatch_once(&sharedDefaultsIapd_once, &__block_literal_global_274_0);
  }
...
{
  sharedDefaultsIapd_sharedInstance = [objc_alloc(MEMORY[0x1E4F1CB18]) initWithSuiteName:@"com.apple.iapd"];
...
+ (id)sharedDefaultsLogging
{
  if (sharedDefaultsLogging_once != -1) {
    dispatch_once(&sharedDefaultsLogging_once, &__block_literal_global_276_0);
  }
...
{
  sharedDefaultsLogging_sharedInstance = [objc_alloc(MEMORY[0x1E4F1CB18]) initWithSuiteName:@"com.apple.logging"];
