...
- (OrgApacheLuceneUtilPackedPacked64SingleBlock_Packed64SingleBlock4)initWithInt:(int)a3;
...
- (int64_t)getWithInt:(int)a3
{
  ...
  if ((int)(a3 >> 4) >= (int)size) {
    IOSArray_throwOutOfBoundsWithMsg(size, a3 >> 4);
  }
  return ((unint64_t)blocks->buffer_[a3 >> 4] >> (4 * (a3 & 0xFu))) & 0xF;
}
