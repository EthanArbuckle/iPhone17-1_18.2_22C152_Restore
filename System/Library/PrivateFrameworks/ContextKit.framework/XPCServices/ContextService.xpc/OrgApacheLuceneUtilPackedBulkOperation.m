...
+ (id)ofWithOrgApacheLuceneUtilPackedPackedInts_FormatEnum:(id)a3 withInt:(int)a4;
+ (void)initialize;
...
+ (id)ofWithOrgApacheLuceneUtilPackedPackedInts_FormatEnum:(id)a3 withInt:(int)a4
{
  return (id)OrgApacheLuceneUtilPackedBulkOperation_ofWithOrgApacheLuceneUtilPackedPackedInts_FormatEnum_withInt_(a3, a4);
}
...
- (int)writeLongWithLong:(int64_t)a3 withByteArray:(id)a4 withInt:(int)a5
{
  ...
  for (uint64_t i = 56; i != -8; i -= 8)
  {
    ...
    *((unsigned char *)a4 + a5++ + 12) = (unint64_t)a3 >> i;
  }
  return v6 + 1;
}
...
- (int)computeIterationsWithInt:(int)a3 withInt:(int)a4
{
  unsigned int v7 = [(OrgApacheLuceneUtilPackedBulkOperation *)self byteBlockCount];
  int v8 = a4
     / (int)(v7 + 8 * [(OrgApacheLuceneUtilPackedBulkOperation *)self byteValueCount]);
  if (!v8) {
    return 1;
  }
  if ((int)([(OrgApacheLuceneUtilPackedBulkOperation *)self byteValueCount] * (v8 - 1)) >= a3)
  {
    double v9 = JavaLangMath_ceilWithDouble_((double)a3 / (double)(int)[(OrgApacheLuceneUtilPackedBulkOperation *)self byteValueCount]);
    if (v9 < 0.0) {
      int v10 = 0x80000000;
    }
    else {
...
+ (void)initialize
{
  if ((id)objc_opt_class() == a1)
  {
    ...
    id v2 = +[IOSObjectArray newArrayWithObjects:v14 count:64 type:OrgApacheLuceneUtilPackedBulkOperation_class_()];
    JreStrongAssignAndConsume((id *)&qword_100560490, v2);
    ...
    long long v5 = 0u;
    long long v6 = 0u;
    unsigned int v7 = new_OrgApacheLuceneUtilPackedBulkOperationPackedSingleBlock_initWithInt_(21);
    long long v8 = 0u;
    long long v9 = 0u;
    long long v10 = 0u;
    long long v11 = 0u;
    ...
    id v3 = +[IOSObjectArray newArrayWithObjects:v4 count:32 type:OrgApacheLuceneUtilPackedBulkOperation_class_()];
    JreStrongAssignAndConsume((id *)&qword_100560498, v3);
    atomic_store(1u, (unsigned __int8 *)OrgApacheLuceneUtilPackedBulkOperation__initialized);
  }
}
