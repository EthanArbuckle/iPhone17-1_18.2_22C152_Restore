...
- (JavaUtilConcurrentLocksReentrantLock)init;
- (JavaUtilConcurrentLocksReentrantLock)initWithBoolean:(BOOL)a3;
...
- (NSString)description
{
  ...
  if ([(JavaUtilConcurrentLocksAbstractQueuedSynchronizer *)self->sync_ getState]) {
    id v4 = [(JavaUtilConcurrentLocksAbstractOwnableSynchronizer *)sync getExclusiveOwnerThread];
  }
  else {
    id v4 = 0;
  }
  v21.receiver = self;
  v21.super_class = (Class)JavaUtilConcurrentLocksReentrantLock;
  ...
  return (NSString *)JreStrcat("$$", v5, v6, v7, v8, v9, v10, v11, v12);
}

- (void)dealloc
{
  v3.receiver = self;
  v3.super_class = (Class)JavaUtilConcurrentLocksReentrantLock;
  [(JavaUtilConcurrentLocksReentrantLock *)&v3 dealloc];
}

+ (const)__metadata
{
  return (const J2ObjcClassInfo *)&unk_100462020;
}
