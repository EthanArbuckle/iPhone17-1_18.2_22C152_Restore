@interface RemindersSiriSnippetFactory
- (_TtC21RemindersSiriUIPlugin27RemindersSiriSnippetFactory)init;
- (id)viewControllerForSnippet:(id)a3;
@end

@implementation RemindersSiriSnippetFactory

- (id)viewControllerForSnippet:(id)a3
{
  id v5 = a3;
  v6 = self;
  id v7 = _s21RemindersSiriUIPlugin0aB14SnippetFactoryC14viewController3forSo0b6UIBased4ViewG0CSgSo11SAUISnippetCSg_tF_0((uint64_t)a3);

  return v7;
}

- (_TtC21RemindersSiriUIPlugin27RemindersSiriSnippetFactory)init
{
  v3.receiver = self;
  v3.super_class = (Class)type metadata accessor for RemindersSiriSnippetFactory();
  return [(RemindersSiriSnippetFactory *)&v3 init];
}

@end