...
- (id)description
{
  ...
  return (id)[NSString stringWithFormat:@"[ACCStatInfoAccumulator:%@ start %llu, max %llu, last %llu, accum %llu, count %llu, average %llu, startTime=%@, lastUpdateTime=%@]", name, self->_start, self->_max, self->_last, self->_accum, self->_count, self->_average, self->_startTime, self->_lastUpdateTime];
}
...
- (void)set:(id)a3
{
  ...
  self->_unint64_t count = [v4 count];
  unint64_t v9 = [v4 average];
