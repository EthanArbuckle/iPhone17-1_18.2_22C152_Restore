@interface H13FastRawScaleStageArgs
- (H13FastRawScaleStageArgs)initWithConfig:(id)a3 bounds:(id)a4 inputFrame:(id)a5 outputFrame:(id)a6;
- (MTLTexture)inputTex;
- (MTLTexture)outputSyntheticThumbnailTex;
...
- (void)setOutputSyntheticThumbnailTex:(id)a3;
- (void)setOutputTex:(id)a3;
@end

@implementation H13FastRawScaleStageArgs
...
- (H13FastRawScaleStageArgs)initWithConfig:(id)a3 bounds:(id)a4 inputFrame:(id)a5 outputFrame:(id)a6
{
  ...
  id v14 = a6;
  v24.receiver = self;
  v24.super_class = (Class)H13FastRawScaleStageArgs;
  v15 = [(H13FastRawScaleStageArgs *)&v24 init];
  if (!v15)
  {
    FigDebugAssert3();
LABEL_13:
...
- (void)dealloc
{
  ...
  FigMetalDecRef();
  v3.receiver = self;
  v3.super_class = (Class)H13FastRawScaleStageArgs;
  [(H13FastRawScaleStageArgs *)&v3 dealloc];
}
...
- (MTLTexture)outputSyntheticThumbnailTex
{
  return self->_outputSyntheticThumbnailTex;
}

- (void)setOutputSyntheticThumbnailTex:(id)a3
{
  id v5 = a3;
  v6 = self->_outputSyntheticThumbnailTex;
  objc_storeStrong((id *)&self->_outputSyntheticThumbnailTex, a3);
  if (v5) {
    FigMetalIncRef();
  }
  FigMetalDecRef();
...
- (void).cxx_destruct
{
  ...
  objc_storeStrong((id *)&self->_outputSyntheticThumbnailTex, 0);
