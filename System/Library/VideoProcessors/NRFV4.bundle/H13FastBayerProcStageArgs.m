@interface H13FastBayerProcStageArgs
- (H13FastBayerProcStageArgs)initWithConfig:(id)a3 bounds:(id)a4 inputFrame:(id)a5 outputFrame:(id)a6;
- (MTLTexture)inputSyntheticThumbnailTex;
- (MTLTexture)inputTex;
...
- (void)setInputSyntheticThumbnailTex:(id)a3;
- (void)setInputTex:(id)a3;
- (void)setOutputFrame:(id)a3;
- (void)setOutputTex:(id)a3;
@end

@implementation H13FastBayerProcStageArgs
...
- (H13FastBayerProcStageArgs)initWithConfig:(id)a3 bounds:(id)a4 inputFrame:(id)a5 outputFrame:(id)a6
{
  ...
  id v14 = a6;
  v24.receiver = self;
  v24.super_class = (Class)H13FastBayerProcStageArgs;
  v15 = [(H13FastBayerProcStageArgs *)&v24 init];
  if (!v15)
  {
    FigDebugAssert3();
LABEL_13:
...
- (void)dealloc
{
  ...
  FigMetalDecRef();
  v3.receiver = self;
  v3.super_class = (Class)H13FastBayerProcStageArgs;
  [(H13FastBayerProcStageArgs *)&v3 dealloc];
}
...
- (MTLTexture)inputSyntheticThumbnailTex
{
  return self->_inputSyntheticThumbnailTex;
}

- (void)setInputSyntheticThumbnailTex:(id)a3
{
  id v5 = a3;
  v6 = self->_inputSyntheticThumbnailTex;
  objc_storeStrong((id *)&self->_inputSyntheticThumbnailTex, a3);
  if (v5) {
    FigMetalIncRef();
  }
  FigMetalDecRef();
...
- (void).cxx_destruct
{
  ...
  objc_storeStrong((id *)&self->_inputSyntheticThumbnailTex, 0);
