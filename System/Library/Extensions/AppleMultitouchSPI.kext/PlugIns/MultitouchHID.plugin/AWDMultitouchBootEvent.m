@interface AWDMultitouchBootEvent
- (BOOL)hasTimestamp;
- (BOOL)hasVersion;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
...
@implementation AWDMultitouchBootEvent
...
{
  v3.receiver = self;
  v3.super_class = (Class)AWDMultitouchBootEvent;
  return (id)[NSString stringWithFormat:@"%@ %@", -[AWDMultitouchBootEvent description](&v3, sel_description), -[AWDMultitouchBootEvent dictionaryRepresentation](self, "dictionaryRepresentation")];
}
...
- (BOOL)readFrom:(id)a3
{
  return AWDMultitouchBootEventReadFrom((uint64_t)self, (uint64_t)a3);
}
...
LABEL_11:
    ...
    if ((*(unsigned char *)&self->_has & 2) != 0)
    {
      if ((*((unsigned char *)a3 + 20) & 2) == 0 || self->_version != *((_DWORD *)a3 + 4)) {
        goto LABEL_11;
      }
      LOBYTE(v5) = 1;
    }
