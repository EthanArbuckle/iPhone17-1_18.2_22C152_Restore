...
{
  v12.receiver = self;
  v12.super_class = (Class)PFBilateralSolverCPU;
...
- (void)dealloc
{
  ...
  free(self->_d_buf);
  v3.receiver = self;
  v3.super_class = (Class)PFBilateralSolverCPU;
...
- (void)_doBistochastize:(id)a3 nIterations:(int)a4
{
  ...
  if (v6 >= 1) {
    memset_pattern16(self->_Dn_buf, &unk_55EB0, 4 * v6);
  }
  if (a4 >= 1)
  {
    int v7 = 1;
...
- (void)_doSolve:(id)a3 t:(const __CVBuffer *)a4 c:(const __CVBuffer *)a5 x:(float *)a6 lambda:(float)a7 pcgMaxIterations:(int)a8
{
  uint64_t v8 = *(void *)&a8;
  id v36 = a3;
  int v14 = [v36 countVertices];
  unsigned int v15 = [v36 countDims];
  if (v14 <= 0)
  {
    [v36 splat:a5 pout:self->_Sc_buf];
    objc_msgSend(v36, "splat_w_mul_x:inPixelBuffer:pout:", a5, a4, self->_b_buf);
  }
  else
  {
    Dn_buf = self->_Dn_buf;
    ...
    do
    {
      ...
      if (v35 == 0.0) {
        ...
        *(float *)&double v31 = self->_b_buf[v33] / v35;
      }
      a6[v33++] = *(float *)&v31;
    }
    while (v14 != v33);
