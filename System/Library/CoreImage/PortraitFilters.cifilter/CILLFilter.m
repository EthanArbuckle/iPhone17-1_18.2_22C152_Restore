...
{
  if (qword_8CC00 != -1) {
    dispatch_once(&qword_8CC00, &stru_75080);
  }
  return (id)qword_8CBF8;
}

- (id)_polyKernel
{
  if (qword_8CC10 != -1) {
    dispatch_once(&qword_8CC10, &stru_750A0);
  }
  return (id)qword_8CC08;
}
...
- (id)outputImage
{
  ...
  if (!inputLightMapImage) {
    inputLightMapImage = sub_18A44(self->inputLightMap, self->inputLightMapWidth, self->inputLightMapHeight);
  }
  id v19 = sub_18BCC(inputLightMapImage, inputGuideImage, (uint64_t)&off_79BF0);
  if (!v19) {
    return self->inputImage;
  }
  id v20 = [(CILLFilter *)self _polyKernel];
  if (!self->inputImage) {
    sub_4BF6C();
  }
  if (!inputGuideImage) {
  ...
  if (fabs(v35) >= 1.0e-10)
  {
    ...
    inputImage = (CIImage *)objc_msgSend(v21, "applyWithExtent:arguments:", +[NSArray arrayWithObjects:count:](NSArray, "arrayWithObjects:count:", v48, 3), v36, v37, v38, v39);
  }
  [(NSNumber *)self->inputSmartShadows doubleValue];
  if (fabs(v40) >= 1.0e-10)
  {
    id v41 = [(CILLFilter *)self _shadowKernel];
    [(CIImage *)self->inputImage extent];
    v47[0] = inputImage;
    v47[1] = v19;
    v47[2] = self->inputSmartShadows;
    return objc_msgSend(v41, "applyWithExtent:arguments:", +[NSArray arrayWithObjects:count:](NSArray, "arrayWithObjects:count:", v47, 3), v42, v43, v44, v45);
  }
  return inputImage;
}
