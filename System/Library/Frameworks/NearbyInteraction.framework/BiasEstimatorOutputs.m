...
- (id)copyWithZone:(_NSZone *)a3
{
  ...
  return (id)[v4 initWithBiasEstimatorOutputs:self];
}
...
LABEL_12:
    ...
    char v16 = [(NSArray *)outProbabilities isEqualToArray:v15];
...
- (unint64_t)hash
{
  ...
  uint64_t v6 = [v5 hash];
  unint64_t v7 = v6 ^ v4 ^ [(NSArray *)self->_outProbabilities hash];
...
- (id)description
{
  ...
  for (unint64_t i = 0; [(NSArray *)self->_outProbabilities count] > i; ++i)
  {
    ...
    [v8 doubleValue];
    objc_msgSend(v6, "appendFormat:", @" out_prob_%d: %.6f,", i, v9);
  }
  double rawRange = self->_rawRange;
  double correctedRange = self->_correctedRange;
  objc_msgSend(v6, "appendFormat:", @" rawRange: %.3f,", *(void *)&rawRange);
  objc_msgSend(v6, "appendFormat:", @" bias_correction_estimate: %.3f,", correctedRange - rawRange);
  objc_msgSend(v6, "appendFormat:", @" correctedRange: %.3f>", *(void *)&self->_correctedRange);
