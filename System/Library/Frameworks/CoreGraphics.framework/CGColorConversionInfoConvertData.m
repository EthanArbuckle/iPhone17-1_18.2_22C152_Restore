@interface CGColorConversionInfoConvertData
@end

@implementation CGColorConversionInfoConvertData

uint64_t (*__CGColorConversionInfoConvertData_block_invoke_4())()
{
  if (CGLibraryLoadColorSyncDYLD_once != -1) {
    dispatch_once(&CGLibraryLoadColorSyncDYLD_once, &__block_literal_global_21589);
  }
  result = (uint64_t (*)())CGLibraryLoadColorSyncDYLD_handle;
  if (CGLibraryLoadColorSyncDYLD_handle) {
    result = (uint64_t (*)())dlsym((void *)CGLibraryLoadColorSyncDYLD_handle, "ColorSyncTransformConvert");
  }
  if (result) {
    v1 = result;
  }
  else {
    v1 = colorsync_smart_null_19766;
  }
  CGColorConversionInfoConvertData_f_31 = v1;
  return result;
}

uint64_t (*__CGColorConversionInfoConvertData_block_invoke_3())()
{
  if (CGLibraryLoadColorSyncDYLD_once != -1) {
    dispatch_once(&CGLibraryLoadColorSyncDYLD_once, &__block_literal_global_21589);
  }
  result = (uint64_t (*)())CGLibraryLoadColorSyncDYLD_handle;
  if (CGLibraryLoadColorSyncDYLD_handle) {
    result = (uint64_t (*)())dlsym((void *)CGLibraryLoadColorSyncDYLD_handle, "ColorSyncTransformGetDstComponentCount");
  }
  if (result) {
    v1 = result;
  }
  else {
    v1 = colorsync_smart_null_19766;
  }
  CGColorConversionInfoConvertData_f_23 = (uint64_t (*)(void))v1;
  return result;
}

uint64_t (*__CGColorConversionInfoConvertData_block_invoke_2())()
{
  if (CGLibraryLoadColorSyncDYLD_once != -1) {
    dispatch_once(&CGLibraryLoadColorSyncDYLD_once, &__block_literal_global_21589);
  }
  result = (uint64_t (*)())CGLibraryLoadColorSyncDYLD_handle;
  if (CGLibraryLoadColorSyncDYLD_handle) {
    result = (uint64_t (*)())dlsym((void *)CGLibraryLoadColorSyncDYLD_handle, "ColorSyncTransformGetSrcComponentCount");
  }
  if (result) {
    v1 = result;
  }
  else {
    v1 = colorsync_smart_null_19766;
  }
  CGColorConversionInfoConvertData_f_17 = (uint64_t (*)(void))v1;
  return result;
}

uint64_t (*__CGColorConversionInfoConvertData_block_invoke())()
{
  if (CGLibraryLoadColorSyncDYLD_once != -1) {
    dispatch_once(&CGLibraryLoadColorSyncDYLD_once, &__block_literal_global_21589);
  }
  result = (uint64_t (*)())CGLibraryLoadColorSyncDYLD_handle;
  if (CGLibraryLoadColorSyncDYLD_handle) {
    result = (uint64_t (*)())dlsym((void *)CGLibraryLoadColorSyncDYLD_handle, "ColorSyncPrivateAPIVersion");
  }
  if (result) {
    v1 = result;
  }
  else {
    v1 = colorsync_smart_null_19766;
  }
  CGColorConversionInfoConvertData_f = v1;
  return result;
}

@end