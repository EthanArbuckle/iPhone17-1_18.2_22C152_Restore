...
- (SCNCylinder)init
{
  v3 = (const void *)C3DParametricGeometryCreate(0, (long long *)kC3DCylinderCallBacks);
  v8.receiver = self;
...
- (__C3DGeometry)__createCFObject
{
  return (__C3DGeometry *)C3DParametricGeometryCreate(0, (long long *)kC3DCylinderCallBacks);
}
...
- (id)presentationCylinder
{
  id v2 = [[SCNCylinder alloc] initPresentationParametricGeometryWithParametricGeometryRef:[(SCNGeometry *)self geometryRef]];
...
- (BOOL)getBoundingBoxMin:(SCNVector3 *)a3 max:(SCNVector3 *)a4
{
  ...
  if ([(SCNGeometry *)self isPresentationInstance])
  {
    ...
    {
      C3DCylinderGetBoundingBox((uint64_t)[(SCNGeometry *)self geometryRef], (uint64_t)&v23, (uint64_t)&v21);
      BOOL v10 = v9;
      if (!v8) {
        goto LABEL_12;
      }
...
- (BOOL)getBoundingSphereCenter:(SCNVector3 *)a3 radius:(double *)a4
{
  ...
  if ([(SCNGeometry *)self isPresentationInstance])
  {
    ...
    if ([(SCNGeometry *)self geometryRef]
      && C3DCylinderGetBoundingSphere((uint64_t)[(SCNGeometry *)self geometryRef], &v16))
    {
      if (a3)
      {
...
- (id)description
{
  ...
  uint64_t v6 = v5;
  [(SCNCylinder *)self height];
  return (id)[v3 stringWithFormat:@"<%@ | radius=%.3f height=%.3f>", v4, v6, v7];
}
...
- (id)copy
{
  return [(SCNCylinder *)self copyWithZone:0];
}
