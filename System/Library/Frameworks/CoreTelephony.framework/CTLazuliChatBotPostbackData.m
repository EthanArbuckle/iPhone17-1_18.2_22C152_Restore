@interface CTLazuliChatBotPostbackData
+ (BOOL)supportsSecureCoding;
- (BOOL)isEqual:(id)a3;
- (BOOL)isEqualToCTLazuliChatBotPostbackData:(id)a3;
- (CTLazuliChatBotPostbackData)initWithCoder:(id)a3;
- (CTLazuliChatBotPostbackData)initWithReflection:(const void *)a3;
- (NSString)data;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
- (void)encodeWithCoder:(id)a3;
- (void)setData:(id)a3;
@end

@implementation CTLazuliChatBotPostbackData

- (id)description
{
  v3 = [MEMORY[0x1E4F28E78] stringWithFormat:@"<%@", objc_opt_class()];
  v4 = [(CTLazuliChatBotPostbackData *)self data];
  [v3 appendFormat:@", data = %@", v4];

  [v3 appendString:@">"];

  return v3;
}

- (BOOL)isEqualToCTLazuliChatBotPostbackData:(id)a3
{
  id v4 = a3;
  v5 = [(CTLazuliChatBotPostbackData *)self data];
  v6 = [v4 data];
  char v7 = [v5 isEqualToString:v6];

  return v7;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = (CTLazuliChatBotPostbackData *)a3;
  v5 = v4;
  if (v4 == self) {
    BOOL v6 = 1;
  }
  else {
    BOOL v6 = v4
  }
      && (objc_opt_class(), (objc_opt_isKindOfClass() & 1) != 0)
      && [(CTLazuliChatBotPostbackData *)self isEqualToCTLazuliChatBotPostbackData:v5];

  return v6;
}

- (id)copyWithZone:(_NSZone *)a3
{
  id v4 = +[CTLazuliChatBotPostbackData allocWithZone:a3];
  [(CTLazuliChatBotPostbackData *)v4 setData:self->_data];
  return v4;
}

- (void)encodeWithCoder:(id)a3
{
}

- (CTLazuliChatBotPostbackData)initWithCoder:(id)a3
{
  id v4 = a3;
  v9.receiver = self;
  v9.super_class = (Class)CTLazuliChatBotPostbackData;
  v5 = [(CTLazuliChatBotPostbackData *)&v9 init];
  if (v5)
  {
    uint64_t v6 = [v4 decodeObjectOfClass:objc_opt_class() forKey:@"kDataKey"];
    data = v5->_data;
    v5->_data = (NSString *)v6;
  }
  return v5;
}

- (CTLazuliChatBotPostbackData)initWithReflection:(const void *)a3
{
  v9.receiver = self;
  v9.super_class = (Class)CTLazuliChatBotPostbackData;
  id v4 = [(CTLazuliChatBotPostbackData *)&v9 init];
  if (v4)
  {
    if (*((char *)a3 + 23) >= 0) {
      v5 = a3;
    }
    else {
      v5 = *(const void **)a3;
    }
    uint64_t v6 = [NSString stringWithUTF8String:v5];
    data = v4->_data;
    v4->_data = (NSString *)v6;
  }
  return v4;
}

+ (BOOL)supportsSecureCoding
{
  return 1;
}

- (NSString)data
{
  return self->_data;
}

- (void)setData:(id)a3
{
}

- (void).cxx_destruct
{
}

@end