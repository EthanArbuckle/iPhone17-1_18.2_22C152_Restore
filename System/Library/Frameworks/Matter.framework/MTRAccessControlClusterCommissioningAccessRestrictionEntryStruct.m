@interface MTRAccessControlClusterCommissioningAccessRestrictionEntryStruct
- (MTRAccessControlClusterCommissioningAccessRestrictionEntryStruct)init;
- (NSArray)restrictions;
- (NSNumber)cluster;
- (NSNumber)endpoint;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
- (void)setCluster:(id)a3;
- (void)setEndpoint:(id)a3;
- (void)setRestrictions:(id)a3;
@end

@implementation MTRAccessControlClusterCommissioningAccessRestrictionEntryStruct

- (MTRAccessControlClusterCommissioningAccessRestrictionEntryStruct)init
{
  v11.receiver = self;
  v11.super_class = (Class)MTRAccessControlClusterCommissioningAccessRestrictionEntryStruct;
  v2 = [(MTRAccessControlClusterCommissioningAccessRestrictionEntryStruct *)&v11 init];
  v3 = v2;
  if (v2)
  {
    endpoint = v2->_endpoint;
    v2->_endpoint = (NSNumber *)&unk_26F9C8620;

    cluster = v3->_cluster;
    v3->_cluster = (NSNumber *)&unk_26F9C8620;

    uint64_t v8 = objc_msgSend_array(MEMORY[0x263EFF8C0], v6, v7);
    restrictions = v3->_restrictions;
    v3->_restrictions = (NSArray *)v8;
  }
  return v3;
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = objc_alloc_init(MTRAccessControlClusterCommissioningAccessRestrictionEntryStruct);
  uint64_t v7 = objc_msgSend_endpoint(self, v5, v6);
  objc_msgSend_setEndpoint_(v4, v8, (uint64_t)v7);

  objc_super v11 = objc_msgSend_cluster(self, v9, v10);
  objc_msgSend_setCluster_(v4, v12, (uint64_t)v11);

  v15 = objc_msgSend_restrictions(self, v13, v14);
  objc_msgSend_setRestrictions_(v4, v16, (uint64_t)v15);

  return v4;
}

- (id)description
{
  v3 = NSString;
  v4 = (objc_class *)objc_opt_class();
  v5 = NSStringFromClass(v4);
  objc_msgSend_stringWithFormat_(v3, v6, @"<%@: endpoint:%@; cluster:%@; restrictions:%@; >",
    v5,
    self->_endpoint,
    self->_cluster,
  uint64_t v7 = self->_restrictions);

  return v7;
}

- (NSNumber)endpoint
{
  return self->_endpoint;
}

- (void)setEndpoint:(id)a3
{
}

- (NSNumber)cluster
{
  return self->_cluster;
}

- (void)setCluster:(id)a3
{
}

- (NSArray)restrictions
{
  return self->_restrictions;
}

- (void)setRestrictions:(id)a3
{
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_restrictions, 0);
  objc_storeStrong((id *)&self->_cluster, 0);

  objc_storeStrong((id *)&self->_endpoint, 0);
}

@end