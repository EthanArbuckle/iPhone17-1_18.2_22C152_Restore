@interface MTRCameraAVStreamManagementClusterAudioStreamChangedEvent
- (MTRCameraAVStreamManagementClusterAudioStreamChangedEvent)init;
- (NSNumber)audioCodec;
...
{
  v12.receiver = self;
  v12.super_class = (Class)MTRCameraAVStreamManagementClusterAudioStreamChangedEvent;
  v2 = [(MTRCameraAVStreamManagementClusterAudioStreamChangedEvent *)&v12 init];
  v3 = v2;
  if (v2)
  {
...
- (id)copyWithZone:(_NSZone *)a3
{
  v4 = objc_alloc_init(MTRCameraAVStreamManagementClusterAudioStreamChangedEvent);
...
- (id)description
{
  ...
  objc_msgSend_stringWithFormat_(v3, v6, @"<%@: audioStreamID:%@; streamType:%@; audioCodec:%@; channelCount:%@; sampleRate:%@; bitRate:%@; bitDepth:%@; >",
    v5,
    self->_audioStreamID,
    self->_streamType,
    self->_audioCodec,
