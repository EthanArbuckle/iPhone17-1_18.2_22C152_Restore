@interface MTRThreadBorderRouterManagementClusterSetActiveDatasetRequestParams
- (ChipError)_encodeToTLVReader:(SEL)a3;
- (MTRThreadBorderRouterManagementClusterSetActiveDatasetRequestParams)init;
- (NSData)activeDataset;
- (NSNumber)breadcrumb;
...
{
  v11.receiver = self;
  v11.super_class = (Class)MTRThreadBorderRouterManagementClusterSetActiveDatasetRequestParams;
  v4 = [(MTRThreadBorderRouterManagementClusterSetActiveDatasetRequestParams *)&v11 init];
  if (v4)
  {
    uint64_t v5 = objc_msgSend_data(MEMORY[0x263EFF8F8], v2, v3);
    activeDataset = v4->_activeDataset;
    v4->_activeDataset = (NSData *)v5;
...
- (id)copyWithZone:(_NSZone *)a3
{
  v4 = objc_alloc_init(MTRThreadBorderRouterManagementClusterSetActiveDatasetRequestParams);
  v7 = objc_msgSend_activeDataset(self, v5, v6);
  objc_msgSend_setActiveDataset_(v4, v8, (uint64_t)v7);
...
- (id)description
{
  ...
  v7 = objc_msgSend_base64EncodedStringWithOptions_(self->_activeDataset, v6, 0);
  objc_msgSend_stringWithFormat_(v3, v8, @"<%@: activeDataset:%@; breadcrumb:%@; >",
    v5,
    v7,
  v9 = self->_breadcrumb);
...
- (ChipError)_encodeToTLVReader:(SEL)a3
{
  ...
  v30[1] = 0;
  v29 = v30;
  objc_msgSend_activeDataset(self, a3, (uint64_t)a4);
