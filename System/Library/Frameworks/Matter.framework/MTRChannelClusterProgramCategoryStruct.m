@interface MTRChannelClusterProgramCategoryStruct
- (MTRChannelClusterProgramCategoryStruct)init;
- (NSString)category;
- (NSString)subCategory;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
- (void)setCategory:(id)a3;
- (void)setSubCategory:(id)a3;
@end

@implementation MTRChannelClusterProgramCategoryStruct

- (MTRChannelClusterProgramCategoryStruct)init
{
  v7.receiver = self;
  v7.super_class = (Class)MTRChannelClusterProgramCategoryStruct;
  v2 = [(MTRChannelClusterProgramCategoryStruct *)&v7 init];
  v3 = v2;
  if (v2)
  {
    category = v2->_category;
    v2->_category = (NSString *)&stru_26F969DC8;

    subCategory = v3->_subCategory;
    v3->_subCategory = 0;
  }
  return v3;
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = objc_alloc_init(MTRChannelClusterProgramCategoryStruct);
  objc_super v7 = objc_msgSend_category(self, v5, v6);
  objc_msgSend_setCategory_(v4, v8, (uint64_t)v7);

  v11 = objc_msgSend_subCategory(self, v9, v10);
  objc_msgSend_setSubCategory_(v4, v12, (uint64_t)v11);

  return v4;
}

- (id)description
{
  v3 = NSString;
  v4 = (objc_class *)objc_opt_class();
  v5 = NSStringFromClass(v4);
  objc_msgSend_stringWithFormat_(v3, v6, @"<%@: category:%@; subCategory:%@; >",
    v5,
    self->_category,
  objc_super v7 = self->_subCategory);

  return v7;
}

- (NSString)category
{
  return self->_category;
}

- (void)setCategory:(id)a3
{
}

- (NSString)subCategory
{
  return self->_subCategory;
}

- (void)setSubCategory:(id)a3
{
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_subCategory, 0);

  objc_storeStrong((id *)&self->_category, 0);
}

@end