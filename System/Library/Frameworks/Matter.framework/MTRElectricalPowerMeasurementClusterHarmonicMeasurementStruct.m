@interface MTRElectricalPowerMeasurementClusterHarmonicMeasurementStruct
- (MTRElectricalPowerMeasurementClusterHarmonicMeasurementStruct)init;
- (NSNumber)measurement;
- (NSNumber)order;
- (id)copyWithZone:(_NSZone *)a3;
- (id)description;
- (void)setMeasurement:(id)a3;
- (void)setOrder:(id)a3;
@end

@implementation MTRElectricalPowerMeasurementClusterHarmonicMeasurementStruct

- (MTRElectricalPowerMeasurementClusterHarmonicMeasurementStruct)init
{
  v7.receiver = self;
  v7.super_class = (Class)MTRElectricalPowerMeasurementClusterHarmonicMeasurementStruct;
  v2 = [(MTRElectricalPowerMeasurementClusterHarmonicMeasurementStruct *)&v7 init];
  v3 = v2;
  if (v2)
  {
    order = v2->_order;
    v2->_order = (NSNumber *)&unk_26F9C8620;

    measurement = v3->_measurement;
    v3->_measurement = 0;
  }
  return v3;
}

- (id)copyWithZone:(_NSZone *)a3
{
  v4 = objc_alloc_init(MTRElectricalPowerMeasurementClusterHarmonicMeasurementStruct);
  objc_super v7 = objc_msgSend_order(self, v5, v6);
  objc_msgSend_setOrder_(v4, v8, (uint64_t)v7);

  v11 = objc_msgSend_measurement(self, v9, v10);
  objc_msgSend_setMeasurement_(v4, v12, (uint64_t)v11);

  return v4;
}

- (id)description
{
  v3 = NSString;
  v4 = (objc_class *)objc_opt_class();
  v5 = NSStringFromClass(v4);
  objc_msgSend_stringWithFormat_(v3, v6, @"<%@: order:%@; measurement:%@; >",
    v5,
    self->_order,
  objc_super v7 = self->_measurement);

  return v7;
}

- (NSNumber)order
{
  return self->_order;
}

- (void)setOrder:(id)a3
{
}

- (NSNumber)measurement
{
  return self->_measurement;
}

- (void)setMeasurement:(id)a3
{
}

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_measurement, 0);

  objc_storeStrong((id *)&self->_order, 0);
}

@end