...
{
  if (a3 >= 4) {
    MTLReportFailure(0, "-[MTLResourceStatePassSampleBufferAttachmentDescriptorArrayInternal objectAtIndexedSubscript:]", 188, @"attachmentIndex(%lu) must be < %lu", v3, v4, v5, v6, a3);
  }
  sampleDescriptors = self->_sampleDescriptors;
  id result = sampleDescriptors[a3];
  if (!result)
  {
    id result = objc_alloc_init(MTLResourceStatePassSampleBufferAttachmentDescriptorInternal);
    uint64_t v11 = 0;
    ...
    {
      sampleDescriptors[a3] = (MTLResourceStatePassSampleBufferAttachmentDescriptorInternal *)result;
    }
  }
  return result;
}
...
{
  if (a4 >= 4) {
    MTLReportFailure(0, "-[MTLResourceStatePassSampleBufferAttachmentDescriptorArrayInternal setObject:atIndexedSubscript:]", 204, @"attachmentIndex(%lu) must be < %lu", v4, v5, v6, v7, a4);
  }
  sampleDescriptors = self->_sampleDescriptors;
  if (sampleDescriptors[a4] != a3)
  {
    v12 = sampleDescriptors[a4];
    sampleDescriptors[a4] = (MTLResourceStatePassSampleBufferAttachmentDescriptorInternal *)[a3 copy];
  }
}
