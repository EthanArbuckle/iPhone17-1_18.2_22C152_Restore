...
+ (INPaymentStatusResolutionResult)confirmationRequiredWithPaymentStatusToConfirm:(INPaymentStatus)paymentStatusToConfirm
{
  ...
  {
    id v7 = [MEMORY[0x1E4F1CA00] exceptionWithName:*MEMORY[0x1E4F1C3C8] reason:@"Confirmation of INPaymentStatusUnknown is not supported." userInfo:0];
...
+ (INPaymentStatusResolutionResult)successWithResolvedPaymentStatus:(INPaymentStatus)resolvedPaymentStatus
{
  ...
  {
    NSLog((NSString *)@"Success resolution with INPaymentStatusUnknown will be reformed to notRequired.", a2);
    v5 = [a1 notRequired];
  }
  ...
  if ([v6 valueType] == 58)
  {
    ...
    if ([v6 valueStyle] != 3)
    {
      ...
      if (objc_opt_isKindOfClass())
      {
        ...
        [(_INPBIntentSlotValue *)v7 addPayloadPaymentStatus:v9];
      }
    }
  }
