...
- (void)transformResolutionResultForIntent:(id)a3 intentSlotDescription:(id)a4 withOptionsProvider:(id)a5 completion:(id)a6
{
  ...
  id v9 = a3;
  id v10 = a4;
  v11 = (void (**)(id, INCallRecordTypeResolutionResult *))a6;
...
- (id)_intentSlotValueForObject:(id)a3 slotDescription:(id)a4
{
  ...
  if ([v6 valueType] == 71 || objc_msgSend(v6, "valueType") == 72)
  {
    [(_INPBIntentSlotValue *)v7 setType:1000];
    objc_opt_class();
    if ((objc_opt_isKindOfClass() & 1) != 0 && (unint64_t)([v5 integerValue] - 1) <= 7)
    {
      ...
      if (v8 > 7) {
        ...
        uint64_t v9 = dword_18CFC4600[v8];
      }
      [(_INPBIntentSlotValue *)v7 addPayloadCallRecordType:v9];
    }
  }
...
+ (INCallRecordTypeResolutionResult)confirmationRequiredWithCallRecordTypeToConfirm:(INCallRecordType)callRecordTypeToConfirm
{
  if (callRecordTypeToConfirm == INCallRecordTypeUnknown)
  {
    id v7 = [MEMORY[0x1E4F1CA00] exceptionWithName:*MEMORY[0x1E4F1C3C8] reason:@"Confirmation of INCallRecordTypeUnknown is not supported." userInfo:0];
    objc_exception_throw(v7);
  }
  v4 = objc_msgSend(NSNumber, "numberWithInteger:");
  id v5 = [a1 resolutionResultConfirmationRequiredWithItemToConfirm:v4];

  return (INCallRecordTypeResolutionResult *)v5;
}

+ (INCallRecordTypeResolutionResult)successWithResolvedCallRecordType:(INCallRecordType)resolvedCallRecordType
{
  if (resolvedCallRecordType)
  {
    v4 = objc_msgSend(NSNumber, "numberWithInteger:");
  ...
  {
    NSLog((NSString *)@"Success resolution with INCallRecordTypeUnknown will be reformed to notRequired.", a2);
