@interface _INPBWorkoutCustomization
+ (BOOL)supportsSecureCoding;
- (BOOL)hasEnvironment;
- (BOOL)hasFocus;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (NSString)environment;
- (NSString)focus;
- (_INPBWorkoutCustomization)initWithCoder:(id)a3;
- (id)copyWithZone:(_NSZone *)a3;
- (id)dictionaryRepresentation;
- (unint64_t)hash;
- (void)encodeWithCoder:(id)a3;
- (void)setEnvironment:(id)a3;
- (void)setFocus:(id)a3;
- (void)writeTo:(id)a3;
@end

@implementation _INPBWorkoutCustomization

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_focus, 0);

  objc_storeStrong((id *)&self->_environment, 0);
}

- (NSString)focus
{
  return self->_focus;
}

- (NSString)environment
{
  return self->_environment;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  if (self->_environment)
  {
    v4 = [(_INPBWorkoutCustomization *)self environment];
    v5 = (void *)[v4 copy];
    [v3 setObject:v5 forKeyedSubscript:@"environment"];
  }
  if (self->_focus)
  {
    v6 = [(_INPBWorkoutCustomization *)self focus];
    v7 = (void *)[v6 copy];
    [v3 setObject:v7 forKeyedSubscript:@"focus"];
  }

  return v3;
}

- (unint64_t)hash
{
  NSUInteger v3 = [(NSString *)self->_environment hash];
  return [(NSString *)self->_focus hash] ^ v3;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_12;
  }
  v5 = [(_INPBWorkoutCustomization *)self environment];
  v6 = [v4 environment];
  if ((v5 == 0) == (v6 != 0)) {
    goto LABEL_11;
  }
  uint64_t v7 = [(_INPBWorkoutCustomization *)self environment];
  if (v7)
  {
    v8 = (void *)v7;
    v9 = [(_INPBWorkoutCustomization *)self environment];
    v10 = [v4 environment];
    int v11 = [v9 isEqual:v10];

    if (!v11) {
      goto LABEL_12;
    }
  }
  else
  {
  }
  v5 = [(_INPBWorkoutCustomization *)self focus];
  v6 = [v4 focus];
  if ((v5 == 0) != (v6 != 0))
  {
    uint64_t v12 = [(_INPBWorkoutCustomization *)self focus];
    if (!v12)
    {

LABEL_15:
      BOOL v17 = 1;
      goto LABEL_13;
    }
    v13 = (void *)v12;
    v14 = [(_INPBWorkoutCustomization *)self focus];
    v15 = [v4 focus];
    char v16 = [v14 isEqual:v15];

    if (v16) {
      goto LABEL_15;
    }
  }
  else
  {
LABEL_11:
  }
LABEL_12:
  BOOL v17 = 0;
LABEL_13:

  return v17;
}

- (id)copyWithZone:(_NSZone *)a3
{
  v5 = [+[_INPBWorkoutCustomization allocWithZone:](_INPBWorkoutCustomization, "allocWithZone:") init];
  v6 = (void *)[(NSString *)self->_environment copyWithZone:a3];
  [(_INPBWorkoutCustomization *)v5 setEnvironment:v6];

  uint64_t v7 = (void *)[(NSString *)self->_focus copyWithZone:a3];
  [(_INPBWorkoutCustomization *)v5 setFocus:v7];

  return v5;
}

- (void)encodeWithCoder:(id)a3
{
  id v4 = a3;
  id v6 = [(_INPBWorkoutCustomization *)self data];
  v5 = NSStringFromSelector(sel_bytes);
  objc_msgSend(v4, "if_encodeBytesNoCopy:forKey:", v6, v5);
}

- (_INPBWorkoutCustomization)initWithCoder:(id)a3
{
  id v4 = a3;
  v5 = NSStringFromSelector(sel_bytes);
  objc_msgSend(v4, "if_decodeBytesNoCopyForKey:", v5);
  id v6 = (_INPBWorkoutCustomization *)objc_claimAutoreleasedReturnValue();

  if (v6
    || (uint64_t v7 = objc_opt_class(),
        NSStringFromSelector(sel_data),
        v8 = objc_claimAutoreleasedReturnValue(),
        [v4 decodeObjectOfClass:v7 forKey:v8],
        id v6 = (_INPBWorkoutCustomization *)objc_claimAutoreleasedReturnValue(),
        v8,
        v6))
  {
    self = [(_INPBWorkoutCustomization *)self initWithData:v6];

    id v6 = self;
  }

  return v6;
}

- (void)writeTo:(id)a3
{
  id v6 = a3;
  id v4 = [(_INPBWorkoutCustomization *)self environment];

  if (v4) {
    PBDataWriterWriteStringField();
  }
  v5 = [(_INPBWorkoutCustomization *)self focus];

  if (v5) {
    PBDataWriterWriteStringField();
  }
}

- (BOOL)readFrom:(id)a3
{
  return _INPBWorkoutCustomizationReadFrom(self, (uint64_t)a3);
}

- (BOOL)hasFocus
{
  return self->_focus != 0;
}

- (void)setFocus:(id)a3
{
  id v4 = (NSString *)[a3 copy];
  focus = self->_focus;
  self->_focus = v4;

  MEMORY[0x1F41817F8](v4, focus);
}

- (BOOL)hasEnvironment
{
  return self->_environment != 0;
}

- (void)setEnvironment:(id)a3
{
  id v4 = (NSString *)[a3 copy];
  environment = self->_environment;
  self->_environment = v4;

  MEMORY[0x1F41817F8](v4, environment);
}

+ (BOOL)supportsSecureCoding
{
  return 1;
}

@end