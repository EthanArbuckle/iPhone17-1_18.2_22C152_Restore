...
- (id)stoppedAnnouncementRecordsAtIndex:(unint64_t)a3;
- (unint64_t)hash;
- (unint64_t)stoppedAnnouncementRecordsCount;
...
- (NSArray)stoppedAnnouncementRecords
{
  return self->_stoppedAnnouncementRecords;
}
...
- (unint64_t)hash
{
  return [(NSArray *)self->_stoppedAnnouncementRecords hash];
}
...
- (BOOL)isEqual:(id)a3
{
  ...
  if ([v4 isMemberOfClass:objc_opt_class()])
  {
    ...
    uint64_t v6 = [v4 stoppedAnnouncementRecords];
    uint64_t v7 = v6;
    if ((v5 == 0) != (v6 != 0))
    {
      uint64_t v8 = [(_INPBStopAnnouncementIntentResponse *)self stoppedAnnouncementRecords];
      if (!v8)
      {
...
- (id)copyWithZone:(_NSZone *)a3
{
  v5 = [+[_INPBStopAnnouncementIntentResponse allocWithZone:](_INPBStopAnnouncementIntentResponse, "allocWithZone:") init];
  uint64_t v6 = (void *)[(NSArray *)self->_stoppedAnnouncementRecords copyWithZone:a3];
...
{
  id v4 = a3;
  id v6 = [(_INPBStopAnnouncementIntentResponse *)self data];
...
- (_INPBStopAnnouncementIntentResponse)initWithCoder:(id)a3
{
  ...
  id v6 = (_INPBStopAnnouncementIntentResponse *)objc_claimAutoreleasedReturnValue();
...
- (BOOL)readFrom:(id)a3
{
  return _INPBStopAnnouncementIntentResponseReadFrom(self, (uint64_t)a3);
}

- (id)stoppedAnnouncementRecordsAtIndex:(unint64_t)a3
{
  return [(NSArray *)self->_stoppedAnnouncementRecords objectAtIndexedSubscript:a3];
}

- (unint64_t)stoppedAnnouncementRecordsCount
{
  return [(NSArray *)self->_stoppedAnnouncementRecords count];
}
...
- (void)addStoppedAnnouncementRecords:(id)a3
{
  ...
  if (!stoppedAnnouncementRecords)
  {
    ...
    uint64_t v7 = self->_stoppedAnnouncementRecords;
...
- (void)setStoppedAnnouncementRecords:(id)a3
{
  ...
  MEMORY[0x1F41817F8](v4, stoppedAnnouncementRecords);
}
