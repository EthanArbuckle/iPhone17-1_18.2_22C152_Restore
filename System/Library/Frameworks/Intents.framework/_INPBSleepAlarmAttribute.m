@interface _INPBSleepAlarmAttribute
+ (BOOL)supportsSecureCoding;
- (BOOL)hasBedtime;
- (BOOL)hasOverride;
- (BOOL)isEqual:(id)a3;
- (BOOL)override;
- (BOOL)readFrom:(id)a3;
- (_INPBDateTime)bedtime;
- (_INPBSleepAlarmAttribute)initWithCoder:(id)a3;
- (id)copyWithZone:(_NSZone *)a3;
- (id)dictionaryRepresentation;
- (unint64_t)hash;
- (void)encodeWithCoder:(id)a3;
- (void)setBedtime:(id)a3;
- (void)setHasOverride:(BOOL)a3;
- (void)setOverride:(BOOL)a3;
- (void)writeTo:(id)a3;
@end

@implementation _INPBSleepAlarmAttribute

- (void).cxx_destruct
{
}

- (BOOL)override
{
  return self->_override;
}

- (_INPBDateTime)bedtime
{
  return self->_bedtime;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  v4 = [(_INPBSleepAlarmAttribute *)self bedtime];
  v5 = [v4 dictionaryRepresentation];
  [v3 setObject:v5 forKeyedSubscript:@"bedtime"];

  if ([(_INPBSleepAlarmAttribute *)self hasOverride])
  {
    v6 = objc_msgSend(NSNumber, "numberWithBool:", -[_INPBSleepAlarmAttribute override](self, "override"));
    [v3 setObject:v6 forKeyedSubscript:@"override"];
  }

  return v3;
}

- (unint64_t)hash
{
  unint64_t v3 = [(_INPBDateTime *)self->_bedtime hash];
  if ([(_INPBSleepAlarmAttribute *)self hasOverride]) {
    uint64_t v4 = 2654435761 * self->_override;
  }
  else {
    uint64_t v4 = 0;
  }
  return v4 ^ v3;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_9;
  }
  v5 = [(_INPBSleepAlarmAttribute *)self bedtime];
  v6 = [v4 bedtime];
  v7 = v6;
  if ((v5 == 0) != (v6 != 0))
  {
    uint64_t v8 = [(_INPBSleepAlarmAttribute *)self bedtime];
    if (v8)
    {
      v9 = (void *)v8;
      v10 = [(_INPBSleepAlarmAttribute *)self bedtime];
      v11 = [v4 bedtime];
      int v12 = [v10 isEqual:v11];

      if (!v12) {
        goto LABEL_9;
      }
    }
    else
    {
    }
    int v13 = [(_INPBSleepAlarmAttribute *)self hasOverride];
    if (v13 == [v4 hasOverride])
    {
      if (![(_INPBSleepAlarmAttribute *)self hasOverride]
        || ![v4 hasOverride]
        || (int override = self->_override, override == [v4 override]))
      {
        BOOL v14 = 1;
        goto LABEL_10;
      }
    }
  }
  else
  {
  }
LABEL_9:
  BOOL v14 = 0;
LABEL_10:

  return v14;
}

- (id)copyWithZone:(_NSZone *)a3
{
  v5 = [+[_INPBSleepAlarmAttribute allocWithZone:](_INPBSleepAlarmAttribute, "allocWithZone:") init];
  id v6 = [(_INPBDateTime *)self->_bedtime copyWithZone:a3];
  [(_INPBSleepAlarmAttribute *)v5 setBedtime:v6];

  if ([(_INPBSleepAlarmAttribute *)self hasOverride]) {
    [(_INPBSleepAlarmAttribute *)v5 setOverride:[(_INPBSleepAlarmAttribute *)self override]];
  }
  return v5;
}

- (void)encodeWithCoder:(id)a3
{
  id v4 = a3;
  id v6 = [(_INPBSleepAlarmAttribute *)self data];
  v5 = NSStringFromSelector(sel_bytes);
  objc_msgSend(v4, "if_encodeBytesNoCopy:forKey:", v6, v5);
}

- (_INPBSleepAlarmAttribute)initWithCoder:(id)a3
{
  id v4 = a3;
  v5 = NSStringFromSelector(sel_bytes);
  objc_msgSend(v4, "if_decodeBytesNoCopyForKey:", v5);
  id v6 = (_INPBSleepAlarmAttribute *)objc_claimAutoreleasedReturnValue();

  if (v6
    || (uint64_t v7 = objc_opt_class(),
        NSStringFromSelector(sel_data),
        uint64_t v8 = objc_claimAutoreleasedReturnValue(),
        [v4 decodeObjectOfClass:v7 forKey:v8],
        id v6 = (_INPBSleepAlarmAttribute *)objc_claimAutoreleasedReturnValue(),
        v8,
        v6))
  {
    self = [(_INPBSleepAlarmAttribute *)self initWithData:v6];

    id v6 = self;
  }

  return v6;
}

- (void)writeTo:(id)a3
{
  id v6 = a3;
  id v4 = [(_INPBSleepAlarmAttribute *)self bedtime];

  if (v4)
  {
    v5 = [(_INPBSleepAlarmAttribute *)self bedtime];
    PBDataWriterWriteSubmessage();
  }
  if ([(_INPBSleepAlarmAttribute *)self hasOverride]) {
    PBDataWriterWriteBOOLField();
  }
}

- (BOOL)readFrom:(id)a3
{
  return _INPBSleepAlarmAttributeReadFrom(self, (uint64_t)a3);
}

- (void)setHasOverride:(BOOL)a3
{
  *(unsigned char *)&self->_has = *(unsigned char *)&self->_has & 0xFE | a3;
}

- (BOOL)hasOverride
{
  return *(unsigned char *)&self->_has & 1;
}

- (void)setOverride:(BOOL)a3
{
  *(unsigned char *)&self->_has |= 1u;
  self->_int override = a3;
}

- (BOOL)hasBedtime
{
  return self->_bedtime != 0;
}

- (void)setBedtime:(id)a3
{
}

+ (BOOL)supportsSecureCoding
{
  return 1;
}

@end