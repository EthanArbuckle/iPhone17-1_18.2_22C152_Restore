@interface _INPBArchivedObject
+ (BOOL)supportsSecureCoding;
- (BOOL)hasMessageData;
- (BOOL)hasTypeName;
- (BOOL)isEqual:(id)a3;
- (BOOL)readFrom:(id)a3;
- (NSData)messageData;
- (NSString)typeName;
- (_INPBArchivedObject)initWithCoder:(id)a3;
- (id)copyWithZone:(_NSZone *)a3;
- (id)dictionaryRepresentation;
- (unint64_t)hash;
- (void)encodeWithCoder:(id)a3;
- (void)setMessageData:(id)a3;
- (void)setTypeName:(id)a3;
- (void)writeTo:(id)a3;
@end

@implementation _INPBArchivedObject

- (void).cxx_destruct
{
  objc_storeStrong((id *)&self->_typeName, 0);

  objc_storeStrong((id *)&self->_messageData, 0);
}

- (NSString)typeName
{
  return self->_typeName;
}

- (NSData)messageData
{
  return self->_messageData;
}

- (id)dictionaryRepresentation
{
  v3 = [MEMORY[0x1E4F1CA60] dictionary];
  if (self->_messageData)
  {
    v4 = [(_INPBArchivedObject *)self messageData];
    v5 = (void *)[v4 copy];
    [v3 setObject:v5 forKeyedSubscript:@"messageData"];
  }
  if (self->_typeName)
  {
    v6 = [(_INPBArchivedObject *)self typeName];
    v7 = (void *)[v6 copy];
    [v3 setObject:v7 forKeyedSubscript:@"typeName"];
  }

  return v3;
}

- (unint64_t)hash
{
  uint64_t v3 = [(NSData *)self->_messageData hash];
  return [(NSString *)self->_typeName hash] ^ v3;
}

- (BOOL)isEqual:(id)a3
{
  id v4 = a3;
  if (![v4 isMemberOfClass:objc_opt_class()]) {
    goto LABEL_12;
  }
  v5 = [(_INPBArchivedObject *)self messageData];
  v6 = [v4 messageData];
  if ((v5 == 0) == (v6 != 0)) {
    goto LABEL_11;
  }
  uint64_t v7 = [(_INPBArchivedObject *)self messageData];
  if (v7)
  {
    v8 = (void *)v7;
    v9 = [(_INPBArchivedObject *)self messageData];
    v10 = [v4 messageData];
    int v11 = [v9 isEqual:v10];

    if (!v11) {
      goto LABEL_12;
    }
  }
  else
  {
  }
  v5 = [(_INPBArchivedObject *)self typeName];
  v6 = [v4 typeName];
  if ((v5 == 0) != (v6 != 0))
  {
    uint64_t v12 = [(_INPBArchivedObject *)self typeName];
    if (!v12)
    {

LABEL_15:
      BOOL v17 = 1;
      goto LABEL_13;
    }
    v13 = (void *)v12;
    v14 = [(_INPBArchivedObject *)self typeName];
    v15 = [v4 typeName];
    char v16 = [v14 isEqual:v15];

    if (v16) {
      goto LABEL_15;
    }
  }
  else
  {
LABEL_11:
  }
LABEL_12:
  BOOL v17 = 0;
LABEL_13:

  return v17;
}

- (id)copyWithZone:(_NSZone *)a3
{
  v5 = [+[_INPBArchivedObject allocWithZone:](_INPBArchivedObject, "allocWithZone:") init];
  v6 = (void *)[(NSData *)self->_messageData copyWithZone:a3];
  [(_INPBArchivedObject *)v5 setMessageData:v6];

  uint64_t v7 = (void *)[(NSString *)self->_typeName copyWithZone:a3];
  [(_INPBArchivedObject *)v5 setTypeName:v7];

  return v5;
}

- (void)encodeWithCoder:(id)a3
{
  id v4 = a3;
  id v6 = [(_INPBArchivedObject *)self data];
  v5 = NSStringFromSelector(sel_bytes);
  objc_msgSend(v4, "if_encodeBytesNoCopy:forKey:", v6, v5);
}

- (_INPBArchivedObject)initWithCoder:(id)a3
{
  id v4 = a3;
  v5 = NSStringFromSelector(sel_bytes);
  objc_msgSend(v4, "if_decodeBytesNoCopyForKey:", v5);
  id v6 = (_INPBArchivedObject *)objc_claimAutoreleasedReturnValue();

  if (v6
    || (uint64_t v7 = objc_opt_class(),
        NSStringFromSelector(sel_data),
        v8 = objc_claimAutoreleasedReturnValue(),
        [v4 decodeObjectOfClass:v7 forKey:v8],
        id v6 = (_INPBArchivedObject *)objc_claimAutoreleasedReturnValue(),
        v8,
        v6))
  {
    self = [(_INPBArchivedObject *)self initWithData:v6];

    id v6 = self;
  }

  return v6;
}

- (void)writeTo:(id)a3
{
  id v6 = a3;
  id v4 = [(_INPBArchivedObject *)self messageData];

  if (v4) {
    PBDataWriterWriteDataField();
  }
  v5 = [(_INPBArchivedObject *)self typeName];

  if (v5) {
    PBDataWriterWriteStringField();
  }
}

- (BOOL)readFrom:(id)a3
{
  return _INPBArchivedObjectReadFrom(self, (uint64_t)a3);
}

- (BOOL)hasTypeName
{
  return self->_typeName != 0;
}

- (void)setTypeName:(id)a3
{
  id v4 = (NSString *)[a3 copy];
  typeName = self->_typeName;
  self->_typeName = v4;

  MEMORY[0x1F41817F8](v4, typeName);
}

- (BOOL)hasMessageData
{
  return self->_messageData != 0;
}

- (void)setMessageData:(id)a3
{
  id v4 = (NSData *)[a3 copy];
  messageData = self->_messageData;
  self->_messageData = v4;

  MEMORY[0x1F41817F8](v4, messageData);
}

+ (BOOL)supportsSecureCoding
{
  return 1;
}

@end