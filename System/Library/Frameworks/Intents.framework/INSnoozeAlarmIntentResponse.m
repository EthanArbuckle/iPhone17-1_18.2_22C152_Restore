@interface INSnoozeAlarmIntentResponse
...
- (id)_dictionaryRepresentation
{
  ...
  v7 = @"code";
  unint64_t v2 = [(INSnoozeAlarmIntentResponse *)self code];
  unint64_t v3 = v2;
  if (v2 <= 5)
  {
    v4 = off_1E5520790[v2];
...
{
  id v3 = a3;
  [v3 isEqualToString:@"INSnoozeAlarmIntentResponseCodeUnspecified"];
  uint64_t v4 = [v3 isEqualToString:@"INSnoozeAlarmIntentResponseCodeReady"];
  if ([v3 isEqualToString:@"INSnoozeAlarmIntentResponseCodeInProgress"]) {
    uint64_t v4 = 2;
  }
  if ([v3 isEqualToString:@"INSnoozeAlarmIntentResponseCodeSuccess"]) {
    uint64_t v4 = 3;
  }
  if ([v3 isEqualToString:@"INSnoozeAlarmIntentResponseCodeFailure"]) {
    int64_t v5 = 4;
  }
  else {
    int64_t v5 = v4;
  }
  int v6 = [v3 isEqualToString:@"INSnoozeAlarmIntentResponseCodeFailureRequiringAppLaunch"];
...
- (int64_t)_intentResponseCode
{
  int64_t v2 = [(INSnoozeAlarmIntentResponse *)self code];
  if ((unint64_t)(v2 - 1) > 4) {
    return 0;
  }
  else {
...
- (INSnoozeAlarmIntentResponse)init
{
  return [(INSnoozeAlarmIntentResponse *)self initWithCode:0 userActivity:0];
}
