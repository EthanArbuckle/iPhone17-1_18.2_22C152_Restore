...
- (SHNotificationArtworkImageView)attachmentImageView;
...
{
  v3.receiver = self;
  v3.super_class = (Class)SHNotificationViewController;
...
- (void)didReceiveNotification:(id)a3
{
  ...
  if (v15)
  {
    ...
    uint64_t v38 = SHAnalyticsEventMusicRecognitionResultExpanded;
  ...
  else
  {
    ...
    if (os_log_type_enabled(v18, OS_LOG_TYPE_ERROR))
    {
      ...
      *(_DWORD *)buf = 138412290;
      v45 = v32;
      _os_log_impl((void *)&_mh_execute_header, v18, OS_LOG_TYPE_ERROR, "Missing attachment from notification with identifier %@", buf, 0xCu);
    }
  }
}
...
- (void)didReceiveNotificationResponse:(id)a3 completionHandler:(id)a4
{
  ...
  if (v10)
  {
    ...
    uint64_t v21 = SHAnalyticsEventMusicRecognitionResultAction;
  ...
  else
  {
    ...
    if (os_log_type_enabled(v23, OS_LOG_TYPE_ERROR))
    {
      ...
      *(_DWORD *)buf = 138412290;
      v26 = v24;
      _os_log_impl((void *)&_mh_execute_header, v23, OS_LOG_TYPE_ERROR, "Notification action %@ is not supported", buf, 0xCu);
    }
    v7[2](v7, 1);
  }
}
...
- (void)setupView
{
  ...
  id v4 = [(SHNotificationViewController *)self attachmentImageView];
  [v4 setShadowColor:v3];

  v5 = [(SHNotificationViewController *)self attachmentImageView];
  [v5 setShadowOpacity:0.100000001];

  id v6 = [(SHNotificationViewController *)self attachmentImageView];
  [v6 setShadowRadius:8.0];

  id v7 = [(SHNotificationViewController *)self attachmentImageView];
  objc_msgSend(v7, "setShadowOffset:", 0.0, 10.0);

  id v8 = [(SHNotificationViewController *)self attachmentImageView];
  [v8 setCornerRadius:8.0];

  id v9 = [(SHNotificationViewController *)self attachmentImageView];
  [v9 layoutIfNeeded];

  id v14 = [objc_alloc((Class)UIFontMetrics) initForTextStyle:UIFontTextStyleBody];
  id v10 = +[UIFontDescriptor preferredFontDescriptorWithTextStyle:UIFontTextStyleBody];
  [v10 pointSize];
  id v11 = +[UIFont systemFontOfSize:weight:](UIFont, "systemFontOfSize:weight:");
...
- (SHNotificationArtworkImageView)attachmentImageView
{
  return self->_attachmentImageView;
}
...
- (void).cxx_destruct
{
  ...
  objc_storeStrong((id *)&self->_attachmentImageView, 0);
}
