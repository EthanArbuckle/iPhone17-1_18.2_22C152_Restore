@interface String:Decodable&
- (void)Encodable.Type;
@end

@implementation String:Decodable&

- (void)Encodable.Type
{
  if (!lazy cache variable for type metadata for [String : Decodable & Encodable.Type])
  {
    type metadata accessor for Decodable & Encodable.Type();
    unint64_t v0 = type metadata accessor for Dictionary();
    if (!v1) {
      atomic_store(v0, (unint64_t *)&lazy cache variable for type metadata for [String : Decodable & Encodable.Type]);
    }
  }
}

@end