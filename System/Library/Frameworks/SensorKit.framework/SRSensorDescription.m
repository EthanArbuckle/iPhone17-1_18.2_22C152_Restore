  ...
  if (v4)
  {
    ...
    if (!v5)
    {
      ...
      if (os_log_type_enabled((os_log_t)SRLogDataStream, OS_LOG_TYPE_FAULT))
      {
        *(_DWORD *)buf = 138543362;
        id v19 = a3;
        _os_log_fault_impl(&dword_21FCA3000, v6, OS_LOG_TYPE_FAULT, "Data stream defined without a name: %{public}@", buf, 0xCu);
      }
    }
    ...
    if (!v7)
    {
      ...
      if (os_log_type_enabled((os_log_t)SRLogDataStream, OS_LOG_TYPE_FAULT))
      {
        *(_DWORD *)buf = 138543362;
        id v19 = a3;
        _os_log_fault_impl(&dword_21FCA3000, v8, OS_LOG_TYPE_FAULT, "Data stream defined without an authorization service: %{public}@", buf, 0xCu);
      }
    }
    ...
    if (!v11)
    {
      ...
      if (os_log_type_enabled((os_log_t)SRLogDataStream, OS_LOG_TYPE_FAULT))
      {
        *(_DWORD *)buf = 138543362;
        id v19 = a3;
        _os_log_fault_impl(&dword_21FCA3000, v12, OS_LOG_TYPE_FAULT, "Data stream defined without an Info.plist authorization category: %{public}@", buf, 0xCu);
      }
    }
...
- (NSString)localizedAdditionalSampleDataNote
{
  ...
  if (!result)
  {
    ...
    if (v4)
    {
      result = (NSString *)(id)[v4 localizedStringForKey:self->_additionalSampleDataNoteKey value:&stru_26D165EC8 table:0];
...
- (NSString)sampleClass
{
  ...
  else
  {
    ...
    {
      *(_WORD *)v7 = 0;
      _os_log_fault_impl(&dword_21FCA3000, v6, OS_LOG_TYPE_FAULT, "Found a linked before version but no legacy sample class", v7, 2u);
    }
    return 0;
  }
}
