...
- (id)description
{
  return (id)[NSString stringWithFormat:@"<QLTBitmapFormat %p s=(%zd, %zd) bpc=%zd bpp=%zd bpr=%zd i=%u>", self, self->_width, self->_height, self->_bitsPerComponent, self->_bitsPerPixel, self->_bytesPerRow, self->_bitmapInfo];
}
...
- (BOOL)isEqual:(id)a3
{
  id v4 = a3;
  objc_opt_class();
  BOOL v5 = (objc_opt_isKindOfClass() & 1) != 0 && [(QLTBitmapFormat *)self isEqualToFormat:v4];
...
+ (QLTBitmapFormat)bitmapFormatWithBitmapImage:(CGImage *)a3
{
  id v4 = objc_alloc_init(QLTBitmapFormat);
  if (a3)
  {
