...
{
  v21.receiver = self;
  v21.super_class = (Class)StewieAssetCell;
  ...
  if (v5)
  {
    ...
    v14 = (UILabel *)objc_msgSend(v10, "initWithFrame:", CGRectZero.origin.x, y, width, height);
    ...
    v18 = (UIButton *)objc_msgSend(objc_alloc((Class)UIButton), "initWithFrame:", CGRectZero.origin.x, y, width, height);
...
- (void)setUpConstraints
{
  ...
  id v6 = [(StewieAssetCell *)self contentView];
  ...
  id v9 = [objc_alloc((Class)UITapGestureRecognizer) initWithTarget:self action:"tryOutButtonWasTapped:"];
  id v10 = [(StewieAssetCell *)self contentView];
...
- (void)tryOutButtonWasTapped:(id)a3
{
  id v3 = [(StewieAssetCell *)self delegate];
  [v3 presentStewieTryOutModeIfPossible];
}

- (void)refreshCellContentsWithSpecifier:(id)a3
{
  id v5 = [a3 propertyForKey:@"StewieSectionDemoDelegate"];
  id v4 = [v5 nonretainedObjectValue];
...
- (StewieDemoPresenterDelegate)delegate
{
  ...
  return (StewieDemoPresenterDelegate *)WeakRetained;
}
...
- (void).cxx_destruct
{
  ...
  objc_storeStrong((id *)&self->tryOutButton, 0);
  objc_storeStrong((id *)&self->separatorLine, 0);
  objc_storeStrong((id *)&self->tryOutText, 0);

  objc_storeStrong((id *)&self->assetView, 0);
}
