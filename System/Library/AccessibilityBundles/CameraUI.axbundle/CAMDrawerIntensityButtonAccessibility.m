...
+ (id)safeCategoryTargetClassName
{
  return @"CAMDrawerIntensityButton";
}
...
+ (void)_accessibilityPerformValidations:(id)a3
{
  ...
  objc_msgSend(v3, "validateClass:hasInstanceMethod:withFullSignature:", @"CAMDrawerIntensityButton", @"isOn", "B", 0);
}

- (int64_t)_accessibilityExpandedStatus
{
  if ([(CAMDrawerIntensityButtonAccessibility *)self safeBoolForKey:@"isExpanded"])return 1; {
  else
  }
    return 2;
}
...
- (id)accessibilityValue
{
  if ([(CAMDrawerIntensityButtonAccessibility *)self safeBoolForKey:@"isExpanded"])
  {
    ...
    id v3 = [(CAMDrawerIntensityButtonAccessibility *)&v6 accessibilityValue];
  }
  else
  {
    if ([(CAMDrawerIntensityButtonAccessibility *)self safeBoolForKey:@"isOn"]) {
      v4 = @"on.state";
    }
    else {
      v4 = @"off.state";
