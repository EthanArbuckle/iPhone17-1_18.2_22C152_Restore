...
- (BOOL)accessibilityActivate
{
  BOOL v3 = [(SKUIStackedBarCellAccessibility *)self _axIsTappable];
  if (v3)
  {
    objc_opt_class();
...
- (id)accessibilityHint
{
  if ([(SKUIStackedBarCellAccessibility *)self _axIsTappable]
    && [(SKUIStackedBarCellAccessibility *)self safeBoolForKey:@"_expanded"])
  {
    BOOL v3 = accessibilitySKUILocalizedString(@"explore.category.collapse");
  }
  else
  {
    BOOL v3 = 0;
...
{
  objc_opt_class();
  BOOL v3 = [(SKUIStackedBarCellAccessibility *)self safeValueForKey:@"_backButton"];
