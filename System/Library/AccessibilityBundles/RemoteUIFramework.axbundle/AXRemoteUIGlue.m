...
+ (void)accessibilityInitializeBundle
{
  if (!_Failover)
  {
    ...
    [v2 performValidations:&__block_literal_global withPreValidationHandler:&__block_literal_global_285 postValidationHandler:0 safeCategoryInstallationHandler:&__block_literal_global_291];

    _Failover = [objc_allocWithZone((Class)AXRemoteUIGlue) init];
    MEMORY[0x270F9A758]();
  }
}
...
uint64_t __47__AXRemoteUIGlue_accessibilityInitializeBundle__block_invoke(uint64_t a1, void *a2)
{
  ...
  objc_msgSend(v2, "validateClass:hasInstanceMethod:withFullSignature:", @"RemoteUITableViewCell", @"setRemoteUIAccessoryView:", "v", "@", 0);
  objc_msgSend(v2, "validateClass:hasInstanceMethod:withFullSignature:", @"RemoteUITableViewCell", @"_accessoriesChanged", "v", 0);
