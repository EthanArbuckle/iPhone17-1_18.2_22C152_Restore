...
+ (void)initializeIconAndTitle:(id)a3
{
  ...
  else
  {
    ...
    {
      id v5 = [objc_alloc((Class)UITabBarItem) initWithTabBarSystemItem:[objc_opt_class() tabBarSystemItem] tag:0];
      v6 = objc_opt_new();
  ...
  if (objc_opt_respondsToSelector())
  {
    v8 = [(id)objc_opt_class() tabBarIconName];
  }
  if (v4) {
    v9 = &stru_1000A3488;
  }
...
- (void)_updateCachedCellHeight
{
  ...
  {
    id v7 = [objc_alloc([self cellClassForCachedCellHeight]) initWithStyle:1 reuseIdentifier:@"PHSizingCell"];
