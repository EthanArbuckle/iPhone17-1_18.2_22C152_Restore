...
{
  HIDWORD(v3) = 0;
  id v1 = [@"[^[[:Hani:][:Hiragana:][:Katakana:][:Katakana_Or_Hiragana:][:Bopomofo:][:Hangul:][ˉˊˇˋ˙]]]" length];
  ...
  {
    __chkstk_darwin(0);
    [@"[^[[:Hani:][:Hiragana:][:Katakana:][:Katakana_Or_Hiragana:][:Bopomofo:][:Hangul:][ˉˊˇˋ˙]]]" getCharacters:&v3 - ((v2 + 15) & 0xFFFFFFFFFFFFFFF0) range:0];
    MusicStringContainsCJKScripts_sNonCJKCharacterSet = uset_openPattern();
  }
  if (SHIDWORD(v3) >= 1)
  {
