  ...
  if (v5)
  {
    if (!v4)
    {
      ...
      if (CRLAssertCat_init_token != -1) {
        dispatch_once(&CRLAssertCat_init_token, &stru_100081DA0);
      }
      if (os_log_type_enabled((os_log_t)CRLAssertCat_log_t, OS_LOG_TYPE_ERROR)) {
        sub_10005D7E4();
      }
      if (CRLAssertCat_init_token != -1) {
        dispatch_once(&CRLAssertCat_init_token, &stru_100081DC0);
      }
...
- (BOOL)supportsGPUFamily:(int64_t)a3
{
  ...
  +[CRLAssertionHandler _atomicIncrementAssertCount];
  if (CRLAssertCat_init_token != -1) {
    dispatch_once(&CRLAssertCat_init_token, &stru_100081DE0);
  }
  if (os_log_type_enabled((os_log_t)CRLAssertCat_log_t, OS_LOG_TYPE_ERROR)) {
    sub_10005D894();
  }
  if (CRLAssertCat_init_token != -1) {
    dispatch_once(&CRLAssertCat_init_token, &stru_100081E00);
  }
