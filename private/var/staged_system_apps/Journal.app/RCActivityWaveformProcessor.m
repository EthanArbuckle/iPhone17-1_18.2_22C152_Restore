...
- (void)prepareToProcessWithFormat:(id)a3 audioTime:(id)a4 punchInTime:(double)a5
{
  ...
  sub_100004BBC((uint64_t)&self->_ringBuffer, (int)(sampleRate * v15), 1, 0);
...
- (void)_tearDown
{
  sub_100004D80((uint64_t)&self->_ringBuffer);
...
{
  id v4 = a3;
  v5 = (const void **)[v4 floatChannelData];
  id v6 = [v4 frameLength];
  id v7 = v6;
  if (v6)
  {
    int mCapacity = self->_ringBuffer.mCapacity;
    unsigned int v9 = atomic_load((unsigned int *)&self->_ringBuffer.mFill);
    if (mCapacity - v9 < v6) {
      [(RCActivityWaveformProcessor *)self consumeRingBufferIfNecessary];
    }
    int v10 = self->_ringBuffer.mCapacity;
    unsigned int v11 = atomic_load((unsigned int *)&self->_ringBuffer.mFill);
    if (v10 - v11 >= v7)
    {
      memcpy(*(void **)self->_ringBuffer.mWritePointers, *v5, 4 * v7);
      sub_100004F74((uint64_t)&self->_ringBuffer, v7);
      [(RCAmplitudeAnalyzer *)self->_audioQueueAmplitudeAnalyzer calculateAmplitude:v5 sampleCount:v7 channelCount:1];
    }
    else
    {
      double v12 = OSLogForCategory(@"Default");
...
- (void)consumeRingBufferIfNecessary
{
  if ([(RCActivityWaveformProcessor *)self _excessFrames] >= 1)
  {
    ...
    {
      unsigned int v4 = v3;
      sub_1000052E8((uint64_t)&self->_ringBuffer, v3);
