...
- (TUIHoverInteraction)initWithDelegate:(id)a3 view:(id)a4;
...
- (TUIHoverInteraction)initWithDelegate:(id)a3 view:(id)a4
{
  ...
  id v7 = a4;
  v15.receiver = self;
  v15.super_class = (Class)TUIHoverInteraction;
...
- (id)activeRegions
{
  v2 = [(NSSet *)self->_regionState allObjects];
...
- (void)rebuildAllRegionsWithBlock:(id)a3
{
  v4 = (void (**)(id, TUIHoverInteraction *))a3;
...
{
  id v11 = a3;
  id v6 = (void (**)(id, _TUIHoverRegionBuilder *))a4;
  id v7 = objc_alloc_init(_TUIHoverRegionBuilder);
  if (v6) {
    v6[2](v6, v7);
  }
  v8 = [(_TUIHoverRegionBuilder *)v7 bounds];
  id v9 = [v8 count];

  if (v9)
  {
    uint64_t v10 = [(_TUIHoverRegionBuilder *)v7 bounds];
...
- (BOOL)isHovering
{
  return (char *)[(UIHoverGestureRecognizer *)self->_gestureRecognizer state] - 1 < (unsigned char *)&def_141F14 + 2;
}
...
- (void)_updateSettled
{
  if (self->_checkSettled)
  {
    ...
    [(TUIHoverInteraction *)self _checkIfSettledWithTime:v3];
  }
}
...
- (void)_updateState
{
  unsigned int v3 = [(TUIHoverInteraction *)self isHovering];
  if ((char *)[(UIHoverGestureRecognizer *)self->_gestureRecognizer state] - 1 <= (unsigned char *)&def_141F14 + 1) {
...
- (TUIHoverInteractionDelegate)delegate
{
  ...
  return (TUIHoverInteractionDelegate *)WeakRetained;
}
