@interface ToastContainerView
- (NSArray)interactions;
- (_TtC5Books18ToastContainerView)init;
- (_TtC5Books18ToastContainerView)initWithCoder:(id)a3;
- (_TtC5Books18ToastContainerView)initWithFrame:(CGRect)a3;
- (void)addInteraction:(id)a3;
- (void)removeInteraction:(id)a3;
- (void)setInteractions:(id)a3;
@end

@implementation ToastContainerView

- (NSArray)interactions
{
  v2 = *(Class *)((char *)&self->super.super.super.isa + OBJC_IVAR____TtC5Books18ToastContainerView_containerView);
  v3 = self;
  id v4 = [v2 interactions];
  sub_100058D18((uint64_t *)&unk_100B2C6D0);
  sub_1007FDFF0();

  v5.super.isa = sub_1007FDFE0().super.isa;
  swift_bridgeObjectRelease();

  return (NSArray *)v5.super.isa;
}

- (_TtC5Books18ToastContainerView)init
{
  return (_TtC5Books18ToastContainerView *)sub_10003FE98();
}

- (_TtC5Books18ToastContainerView)initWithCoder:(id)a3
{
  id v3 = a3;
  sub_10048FE70();
}

- (void)addInteraction:(id)a3
{
}

- (void)removeInteraction:(id)a3
{
}

- (void)setInteractions:(id)a3
{
  sub_100058D18((uint64_t *)&unk_100B2C6D0);
  sub_1007FDFF0();
  id v4 = *(Class *)((char *)&self->super.super.super.isa + OBJC_IVAR____TtC5Books18ToastContainerView_containerView);
  NSArray v5 = self;
  Class isa = sub_1007FDFE0().super.isa;
  swift_bridgeObjectRelease();
  [v4 setInteractions:isa];
}

- (_TtC5Books18ToastContainerView)initWithFrame:(CGRect)a3
{
  result = (_TtC5Books18ToastContainerView *)_swift_stdlib_reportUnimplementedInitializer();
  __break(1u);
  return result;
}

- (void).cxx_destruct
{
  swift_bridgeObjectRelease();
}

@end